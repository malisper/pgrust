...
```bash
aws s3 sync web/wasm-demo/ s3://demo-bucket/ --delete
```

5. Re-upload the wasm binary with the correct MIME type. Do this even after `sync`:

```bash
aws s3 cp web/wasm-demo/pkg/pgrust_bg.wasm s3://demo-bucket/pkg/pgrust_bg.wasm --content-type application/wasm
```
...
```bash
aws s3 ls s3://demo-bucket/ --recursive
