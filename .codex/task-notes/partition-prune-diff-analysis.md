...
Latest slice is at 639/750 queries matched, 111 mismatched queries, 120 diff hunks, 2600 diff lines. Latest diff copied to /tmp/diffs/partition_prune.diff.
...
- MERGE source grammar is fixed for joined sources. Remaining MERGE blockers are now data-modifying CTE syntax/execution and ordinary runtime-pruning/view-update plan mismatches.
...
Latest category counts in `/tmp/diffs/partition_prune.diff`: `Subplans Removed` 37, `never executed` 117, `ERROR:` 13, `syntax error` 2, `rlp` 35, `mc2p` 8, `mc3p` 125, `pph_arrpart` 0, `pp_arrpart` 0, `pp_enumpart` 0, `pp_recpart` 0, `to_char` 7, `ToChar` 0, `LocalTimestamp` 0, `MERGE` 13, `UPDATE` 7, `ATTACH PARTITION` 0, `ANY` 55, `ALL` 4.
