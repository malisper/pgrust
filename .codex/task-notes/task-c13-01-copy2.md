...
- Restored `Session::copy_from_rows` null-marker handling after CI showed the direct row API was treating raw `\N` text as a literal instead of null.
...
- `env -u CARGO_TARGET_DIR PGRUST_TARGET_POOL_DIR=/tmp/pgrust-target-pool/baghdad-copy2-ci PGRUST_TARGET_SLOT=0 scripts/cargo_isolated.sh test --lib --quiet copy_from_rows`
- `env -u CARGO_TARGET_DIR PGRUST_TARGET_POOL_DIR=/tmp/pgrust-target-pool/baghdad-copy2-ci PGRUST_TARGET_SLOT=0 scripts/cargo_isolated.sh test --lib --quiet update_and_copy_from_enforce_check_and_not_null_constraints`
- `env -u CARGO_TARGET_DIR PGRUST_TARGET_POOL_DIR=/tmp/pgrust-target-pool/baghdad-copy2-ci PGRUST_TARGET_SLOT=0 scripts/cargo_isolated.sh check --lib -q`
...
- `scripts/run_regression.sh --test copy2 --port 62456 --results-dir /tmp/pgrust-task-c13-01-copy2-ci-fix`
