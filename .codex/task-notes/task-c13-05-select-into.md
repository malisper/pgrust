...
CI follow-up: narrow the preparse SELECT INTO rejection so an INTO token only counts as SELECT INTO when the nearest same-depth clause opener before it is SELECT. This preserves disallowed nested SELECT INTO errors without rejecting writable CTEs or INSERT INTO ... SELECT statements.
...
scripts/cargo_isolated.sh test --lib --quiet on_conflict_update_skips_row_already_touched_by_writable_cte
scripts/cargo_isolated.sh test --lib --quiet with_recursive_select_cte_feeds_writable_cte
scripts/cargo_isolated.sh test --lib --quiet recursive_writable_cte_forward_dependency_runs_in_dependency_order
scripts/cargo_isolated.sh test --lib --quiet writable_cte_on_conflict_update_same_row_returns_no_outer_rows
scripts/cargo_isolated.sh check
scripts/run_regression.sh --test select_into --port 59616 --results-dir /tmp/pgrust-task-c13-05-select-into-ci-fix
