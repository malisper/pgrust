...
There is a full PostgreSQL source checkout alongside this repo at [../postgres](~/postgres). Use it as the first reference when pgrust behavior should match upstream PostgreSQL semantics, output formatting, planner structure, catalog behavior, or regression expectations.

Useful reference paths inside that tree include:

- [../postgres/src/backend](~/postgres/src/backend) for parser, planner, executor, catalog, and command behavior.
- [../postgres/src/include](~/postgres/src/include) for canonical node/type definitions and shared headers.
- [../postgres/src/test/regress](~/postgres/src/test/regress) for regression SQL and expected output files.
...
**When ending a work session**, you MUST complete ALL steps below. If code changed, work is not complete until the intended local changes are committed.
...
4. **Commit local changes**:
   ...
   git commit -m "..."
   ```
5. **Clean up** - Clear stashes, prune remote branches when relevant
6. **Verify** - All intended changes committed locally
7. **Hand off** - Provide context for next session

Remote sync note: `bd dolt push` and `git push` are optional until remote access/auth is configured for this environment.

**CRITICAL RULES:**
- If code changed, do not stop before committing it locally or explicitly explaining why it is being left uncommitted
- If a local commit fails, resolve the problem and retry until it succeeds
