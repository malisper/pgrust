...
## Residue sweep 2 (2026-08-01, rides main)

Safe-tier residue sweep over everything the 07-15 pass skipped or that
accreted since (Michael: retire the legacy name). Same mechanical rule and
protections as above, plus:

- crates: remaining comment prose renamed; the two post-t24 identifiers
  `cbstore_eoxact_callback`/`ensure_cbstore_eoxact_registered` (tableam) ->
  `pgrcolumnar_*`. Every .rs string literal byte-identical (multiset-verified
  before/after).
- scripts/bench/benchmarks: comment prose + internal shell vars
  (`cbstore_rc` -> `pgrcolumnar_rc`) + echo/step/expect labels renamed.
  NOT renamed (functional surface, verified case-by-case): SQL `USING
  cbstore`/AM mints, server-literal grep patterns (`ANALYZE|TRACE|cbstore
  acquire:`, `not a cbstore relation`), emitted SQL (`SET
  cbstore_parallel_setup_cost`), env-value vocabularies (`LOWDISTH_AM=cbstore`),
  banked rig cell names (`dcell cbstore-gap`), rig/e2e basenames + `cbstore
  e2e`/`cbstore corpus` rig references, `LANE_GATES_SKIP_CBSTORE`,
  bank/lineage/S3 keys.
- docs: living prose renamed; the four cbstore-*.md stubs kept at their old
  paths; ratified records (m5-coverage.tsv both copies, m5-ratifications/,
  postmortems/, wedge-bisect-ledger.md, regex-engine-ab-verdict.md,
  gather-census.tsv) untouched.
- notes/**: DELIBERATELY untouched again — a trial sweep falsified historical
  worklogs (the rename lane's own "cbstore -> pgrcolumnar" row, battery names,
  knob-value vocabularies); a curated per-file pass is the only safe route.
- proofs/coverage/fulltree/rf-remap/regress.lcov: banked evidence, untouched.
- tools/coverage-viewer/site/data/src/**: generated snapshot pinned to the
  measurement sha (tools/coverage-viewer/generate.py snapshot_sources); never
  hand-edit — regenerate via generate.py when a new capture lands.
