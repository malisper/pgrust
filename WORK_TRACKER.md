...
_Last updated: 2026-06-18 · origin/main_

---

## Where we are
- **Passing:** `smoke` (SELECT 1; `pg_class` seqscan 415/68 rows) — verified live.
- **Closest test win:** **`boolean` = 1 hunk from GREEN** — multi-key-sort SIGSEGV + parallel-CREATE-INDEX crash both FIXED; now walls on the by-ref `bool` error-case path / btbuild #341. `select_having`/`numeric` also ~1 hunk.
- **Now executing broadly:** SELECT/joins/HashJoin · aggregates incl. array_agg/string_agg + **count(\*)/sum over user tables + count(\*)/sum OVER (window aggregates)** · **window funcs (row_number/rank/dense_rank, single + multiple, ±PARTITION BY)** · set-ops (UNION/ALL/INTERSECT/EXCEPT) · subqueries (scalar/EXISTS/IN/ANY) · CTEs · DDL (TABLE/INDEX/VIEW/SEQUENCE/DOMAIN + PK/UNIQUE spine) · DML (INSERT/INSERT…SELECT/ON CONFLICT spine; **UPDATE/DELETE execute via ctid-junk**) · transactions · EXPLAIN/SHOW/SET · **PREPARE/EXECUTE (no-param)** · arrays (int + text elements).
- **Stability fixes (shared-cluster enablers):** **SIGALRM async-signal-safe SetLatch** (was a `RefCell`-reentrancy abort — likely a major random-crash source; stress-verified 960 timer arms, 0 aborts) · **3 UAF/drop-order fixes** (tuplesort-over-join, BuildCachedPlan qlist, by-ref headers). These directly attack the `SERVER_DIED` crashes blocking shared-cluster runs.
- **Measurement keystone:** per-file `initdb` makes full runs crawl; SHARED-cluster is ~2× faster + gives `test_setup`'s tables but needs per-test crashes eliminated. Strategy = kill the crashes (signal-safety + UAFs), NOT port WAL-recovery #157. **`test_setup.sql` = TOP priority**: `tuple concurrently deleted` FIXED (HOT-redirect TID); now walls on by-ref **aclitem[]** in GRANT (by-ref Datum keystone). **Shared-cluster killer pinpointed: INSERT of any text value crashes `heap_fill_tuple` (by-ref header at INSERT→heap-tuple)** — dedicated lane in flight; fixing it unblocks the cluster + ~all text INSERTs.
- **Open keystones surfaced by the measurement lanes (next-wall convergence):** (1) **by-ref Datum VALUE unification** — the dominant one; surfaces as text-INSERT crash, aclitem[] deform, was numeric/array/jsonb header facets (several already fixed piecemeal); (2) **btbuild #341** — serial index build needs real IndexInfo (gates test_setup indexes + PK/UNIQUE; lane in flight); (3) **COPY FROM** — copy.c+copyfrom.c driver campaign (constraint cluster landed; driver lane in flight); (4) **config-file GUC application** — max_stack_depth not applied at backend start (lane in flight).
- **Node-opaque migration:** P1 done; **P2 in progress (~64/101 consumer crates) 🔄** via workflow (chunk=12, runs clean at 15-lanes+workflow — rate-limit was transient, not a hard cap); ~22 non-mechanical crates = the hard tail gating P3 (the build-speed payoff).
