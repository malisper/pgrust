...
_Last updated: 2026-06-18 · origin/main ≈ `2b61a3042`_

---

## Where we are
- **Passing:** `smoke` (SELECT 1; `pg_class` seqscan 415/68 rows) — verified live.
- **Closest test win:** **`boolean` = 1 hunk from GREEN** — first ~150 lines pass; last blocker = a SIGSEGV on a cross-join + 2-key ORDER BY (lane in flight). `select_having`/`numeric` also ~1 hunk.
- **Now executing broadly:** SELECT/joins/HashJoin · aggregates incl. array_agg/string_agg · set-ops (UNION/ALL/INTERSECT/EXCEPT) · subqueries (scalar/EXISTS/IN/ANY) · CTEs · DDL (TABLE/INDEX/VIEW/SEQUENCE/DOMAIN + PK/UNIQUE) · DML (INSERT/INSERT…SELECT/ON CONFLICT spine; UPDATE/DELETE→ctid-junk) · transactions · EXPLAIN/SHOW/SET · PREPARE.
- **Measurement keystone:** per-file `initdb` makes full runs crawl; a SHARED-cluster run is ~2× faster + gives `test_setup`'s tables, but needs per-test crashes eliminated (kill the crashes — by-ref Datum, multi-key-sort — NOT port WAL-recovery #157). **`test_setup.sql` = TOP measurement priority** (creates onek/tenk1/int*_tbl that ~half the suite needs).
- **Node-opaque migration:** P1 done; **P2 in progress (~37/101 consumer crates) 🔄** via workflow; 22 non-mechanical crates = the hard tail gating P3 (the build-speed payoff).
