...
- **★ BY-REF DATUM KEYSTONE DONE (header-ful-everywhere flip, `1907af523` + batches):** eliminated the invented header-LESS varlena convention — ONE representation now, header-ful, like PG. Unlocked EXTRACT/datetime, bpchar/char comparison, numeric, jsonb, text arrays, AND was the last blocker under UPDATE/DELETE + COPY. Follow-on facets fixed as found: heap-text 3-byte truncation (`bbc01c351`), LIKE `name`-arg (`e50f63208`). A few more non-varlena-by-ref read sites may remain (sweep pending).
...
- **Node-opaque migration (P3 build-speed payoff):** P1 done; **P2 hard-tail in progress** — 9+ big consumer crates migrated this session (outfuncs 245 / readfuncs 77 / setrefs / vars / ruleutils / init-subselect / prepjointree / explain / clauses) + **`Expr` tag surface added** (`b449f0090`, unlocks Expr-table migration). Measured: migrated crates compile ~13% faster (per-crate); the big win (delete enum + split the 446-crate monolith) needs P3→P4. Remaining: nodes-core, coerce, prepagg, appendinfo, func (lanes in flight).
