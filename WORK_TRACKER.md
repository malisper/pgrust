...
- **Closest test win:** **`boolean` = 1 hunk from GREEN** — multi-key-sort SIGSEGV FIXED (was a tuplesort UAF); now walls only on parallel-CREATE-INDEX-worker planning (lane forcing serial build). `select_having`/`numeric` also ~1 hunk.
...
- **Measurement keystone:** per-file `initdb` makes full runs crawl; SHARED-cluster is ~2× faster + gives `test_setup`'s tables but needs per-test crashes eliminated. Strategy = kill the crashes (signal-safety + UAFs landing now), NOT port WAL-recovery #157. **`test_setup.sql` = TOP measurement priority**; first wall = `tuple concurrently deleted` (lane in flight).
- **Node-opaque migration:** P1 done; **P2 in progress (~55/101 consumer crates) 🔄** via workflow (chunk=12, runs clean at 15-lanes+workflow — rate-limit was transient, not a hard cap); ~22 non-mechanical crates = the hard tail gating P3 (the build-speed payoff).
