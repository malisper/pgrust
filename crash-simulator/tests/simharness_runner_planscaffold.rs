//! Scaffold plan-format tests: round-trip law + reserved-tag policy.
//! (The authoritative format round-trip property test is WS-GEN's G-G1 on
//! src/plan.rs; this pins the SCAFFOLD until the swap so the runner's own
//! gates stand on verified ground.)
...
fn session_switch_is_hard_parse_error() {
    ...
    assert_eq!(err, "reserved: multi-session", "§0 A1: reserved tag must hard-error");
}
...
#[test]
fn statement_without_mark_rejected() {
    let text = format!(
        "{}\n-- seed: 1 profile: t profile-sha256: {} generator: g\nSELECT 1;\n",
        PLAN_HEADER_LINE,
        "0".repeat(64)
    );
    assert!(Plan::parse(&text).unwrap_err().contains("without MARK"));
}
