...
///
/// Deliberately distinct from `nodes::fmgr::FunctionCallInfoBaseData<'mcx>`
...
///
/// In the owned model the live `AggState` back-pointer is the `nodes`
...
///
/// `types-fmgr` cannot name `nodes::EStateData`, so it carries only the
