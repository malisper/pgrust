...
//! dual-home `fmgr` <-> `nodes` fcinfo boundary.
//!
//! ## Why a thread-local sink, not a struct field
//!
//! The executor frame (`nodes::FunctionCallInfoBaseData<'mcx>`) carries a
//! real `ReturnSetInfo<'mcx>` in `resultinfo`. The fmgr frame
//! (`fmgr::FunctionCallInfoBaseData`) carries only a tag-only
