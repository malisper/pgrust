...
//! `rel::RelationData`. It carries the full `rd_*` surface that the C
...
/// the entry (lifetime-free mirror of [`rel::FormData_pg_class`]).
#[derive(Clone, Debug, Default)]
...
/// by the entry (lifetime-free mirror of [`rel::FormData_pg_index`]).
#[derive(Clone, Debug, Default)]
...
/// [`nodes::copy_query::Query`]`<'static>` allocated in the process-lifetime
...
pub struct RewriteRule {
    ...
    pub qual: Option<mcx::PgBox<'static, nodes::nodes::Node<'static>>>,
    ...
    pub actions: mcx::PgVec<'static, nodes::copy_query::Query<'static>>,
}
...
pub struct RowSecurityPolicy {
    ...
    pub with_check_qual: Option<mcx::PgBox<'static, nodes::nodes::Node<'static>>>,
...
pub struct RelationData {
    ...
    /// `rel::RelationData.rd_trigdesc` (`'mcx`) when it serves a consumer.
    ...
    /// presence-flag rendering ([`nodes::FdwRoutine`]) is cached here; the
    /// extension-owned function pointers live behind the FDW-provider seam.
    pub rd_fdwroutine: Option<nodes::FdwRoutine>,
}
...
impl RelationData {
     ...
     *
     * F0'' additive: these mirror the methods the trimmed `rel::
     * RelationData` exposes, but read the OWNED entry fields. They are placed
     * here so that after the later Deref-target flip (when `rel::
