// GUC vocabulary from utils/guc.h + guc_tables.h; discriminants and flag bits
// are value-exact C copies (transcription errors are silent corruption).

#![no_std]
#![allow(non_camel_case_types)]
#![allow(non_upper_case_globals)]

// `Ord` mirrors the C `context >= PGC_*` privilege comparisons.
#[repr(i32)]
...
// `Ord` mirrors the C source-priority comparisons (`source <= PGC_S_OVERRIDE`
// sets the RESET default).
#[repr(i32)]
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash, Ord, PartialOrd)]
pub enum GucSource {
    PGC_S_DEFAULT = 0,
...
// C's NULL-terminated `config_enum_entry` arrays become plain slices.
