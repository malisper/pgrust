//! Owned-tree representation for the PostgreSQL `jsonb` type vocabulary.
//!
//! These types mirror `postgres-18.3/src/include/utils/jsonb.h`: the
//! `Jsonb` varlena, the `JsonbContainer` node header, the `JEntry` child header
//! word, and the associated flag/type-bit constants and GIN-opclass marker
//! bytes.  In the idiomatic rewrite the on-disk flexible array member becomes an
//! owned `Vec`; the type names, module location, and constant values are kept
//! identical to the source so the cross-crate reference graph resolves
//! unchanged.
//!
//! In-memory working types (`JsonbValue`, `JsonbPair`, `JsonbParseState`,
//! `JsonbIterator`, ...) are deliberately *not* defined here -- they are
//! idiomatic Rust types that live in the `backend-utils-adt-jsonb-util` crate.
...
/// A jsonb array or object node header (`struct JsonbContainer`).  The C
/// `JEntry children[FLEXIBLE_ARRAY_MEMBER]` flexible array member becomes an
/// owned `Vec<JEntry>` in this rewrite.
