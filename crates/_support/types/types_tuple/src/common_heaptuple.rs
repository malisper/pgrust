...
use datum::{flatten_expanded, ExpandedObject};
...
// `uintptr_t` Datum). The former `ByVal(datum::Datum)` indirection was
...
// `datum::Datum` survives only at the two irreducible ABI edges (the
...
impl Datum<'_> {
    ...
    // to forward to the bare-word `datum::Datum` newtype).
    //
