//! Encoding identifiers (`enum pg_enc` in `mb/pg_wchar.h`).
...
/// Last backend-usable encoding (`PG_ENCODING_BE_LAST` in `mb/pg_wchar.h`).
pub const PG_ENCODING_BE_LAST: pg_enc = PG_KOI8U;

/// `PG_VALID_BE_ENCODING(_enc)` — usable as a server (backend) encoding.
pub const fn pg_valid_be_encoding(encoding: pg_enc) -> bool {
    encoding >= 0 && encoding <= PG_ENCODING_BE_LAST
}

/// `PG_VALID_FE_ENCODING(_enc)` — usable as a client (frontend) encoding.
pub const fn pg_valid_fe_encoding(encoding: pg_enc) -> bool {
    pg_valid_encoding(encoding)
}

/// `PG_VALID_ENCODING(_enc)` — a recognized encoding id.
