...
//! the [`types_brin::OpaqueOpcInfo`] carrier (its [`InclusionOpaque`] variant).
//!
...
//! [`types_brin::OpaqueOpcInfo`]; inclusion's variant is
//! [`types_brin::InclusionOpaque`], whose every cached `FmgrInfo` is reduced to
...
//! [`backend_utils_fmgr_fmgr_seams::function_call1_coll_datum`] /
