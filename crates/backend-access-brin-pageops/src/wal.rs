...
pub fn relation_is_local(rel: &Relation<'_>) -> bool {
    backend_utils_cache_relcache_seams::relation_is_local::call(rel)
}
...
pub fn get_page_with_free_space(rel: &Relation<'_>, space_needed: usize) -> PgResult<BlockNumber> {
    backend_storage_freespace_seams::get_page_with_free_space::call(rel, space_needed)
}
...
pub use backend_storage_lmgr_lmgr_seams::lock_relation_for_extension;
pub use backend_storage_lmgr_lmgr_seams::RelationExtensionLockGuard;
...
pub fn log_newpage_buffer(buffer: Buffer, page_std: bool) -> PgResult<types_core::XLogRecPtr> {
    backend_access_transam_xloginsert_seams::log_newpage_buffer::call(buffer, page_std)
}
