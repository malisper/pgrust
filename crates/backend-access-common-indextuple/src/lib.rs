...
pub fn index_form_tuple_context<'mcx>(
    ...
    for i in 0..natts {
        ...
        if varatt_is_external(value.as_ref_bytes()) {
            let detoasted = backend_access_common_detoast_seams::detoast_external_attr::call(
                mcx,
                value.as_ref_bytes(),
            )?;
...
pub fn index_truncate_tuple<'mcx>(
    ...
    let truncdesc =
        backend_access_common_tupdesc::CreateTupleDescTruncatedCopy(mcx, source_descriptor, leavenatts)?;
