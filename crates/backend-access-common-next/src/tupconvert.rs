...
use backend_access_common_heaptuple::{heap_deform_tuple, heap_form_tuple, HeapTupleError};
...
/// header (see `backend_access_common_heaptuple`), so the caller supplies the
