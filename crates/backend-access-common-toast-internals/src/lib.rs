...
use backend_access_common_heaptuple::{heap_form_tuple, Datum};
use backend_access_common_scankey::ScanKeyInit;
use backend_access_index_indexam::index_insert;
...
use backend_access_heap_heapam_seams as heapam_seams;
...
/// TOAST chunk row.
const TOAST_MAX_CHUNK_SIZE: i32 = backend_access_heap_heaptoast::TOAST_MAX_CHUNK_SIZE;
...
pub fn toast_fetch_datum<'mcx>(mcx: Mcx<'mcx>, attr: &[u8]) -> PgResult<PgVec<'mcx, u8>> {
    ...
    let toastrel = backend_access_table_table_seams::table_open::call(
        mcx,
        toast_pointer.va_toastrelid,
        AccessShareLock,
    )?;
    let have_snapshot =
        backend_utils_time_snapmgr_seams::have_registered_or_active_snapshot::call();
    backend_access_heap_heaptoast::heap_fetch_toast_slice(
        mcx,
        &toastrel,
...
pub fn toast_save_datum<'mcx>(
    ...
    let reltoastrelid = backend_utils_cache_relcache_seams::rel_reltoastrelid::call(rel)?;
    let toastrel =
        backend_access_table_table_seams::table_open::call(mcx, reltoastrelid, RowExclusiveLock)?;
    ...
    let rd_toastoid = backend_utils_cache_relcache_seams::rel_rd_toastoid::call(rel)?;
    ...
    if !OidIsValid(rd_toastoid) {
        ...
        toast_pointer.va_valueid =
            backend_catalog_catalog::GetNewOidWithIndex(&toastrel, valid_index_oid, 1 as AttrNumber)?;
    } else {
        ...
        if !OidIsValid(toast_pointer.va_valueid) {
            ...
            // or new toast table.
            loop {
                toast_pointer.va_valueid = backend_catalog_catalog::GetNewOidWithIndex(
                    &toastrel,
                    valid_index_oid,
                    1 as AttrNumber,
                )?;
...
fn toastid_valueid_exists<'mcx>(mcx: Mcx<'mcx>, toastrelid: Oid, valueid: Oid) -> PgResult<bool> {
    let toastrel =
        backend_access_table_table_seams::table_open::call(mcx, toastrelid, AccessShareLock)?;
