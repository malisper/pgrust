...
fn populate_compact_attribute_internal(
    ...
    } else if backend_catalog_catalog_seams::is_catalog_relation_oid::call(src.attrelid) {
        ATTNULLABLE_VALID
    } else {
        ATTNULLABLE_UNKNOWN
    };
...
pub fn TupleDescInitEntry(
    ...
    let info = backend_utils_cache_syscache_seams::search_type_attr_info::call(oidtypeid)?
        ...
        for i in 0..constr.num_defval as usize {
            ...
            if attrdef.adnum == attnum {
                if let Some(adbin) = attrdef.adbin.as_ref() {
                    let node = backend_nodes_read_seams::string_to_node::call(mcx, adbin.as_str())?;
                    return Ok(Some(node));
                }
                return Ok(None);
            }
...
fn hash_uint32(k: u32) -> u32 {
    common_hashfn_seams::hash_bytes_uint32::call(k)
}
...
pub fn init_seams() {
    backend_access_common_tupdesc_seams::hash_row_type::set(hashRowType);
    ...
    backend_access_common_tupdesc_seams::create_tuple_desc::set(CreateTupleDesc);
