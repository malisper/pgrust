...
use types_tsearch::gin::{GIN_FALSE, GIN_MAYBE, GIN_SEARCH_MODE_EVERYTHING, GIN_TRUE};
...
type TriHook = Box<dyn FnMut(&mut GinScanKey) -> types_tsearch::gin::GinTernaryValue>;
...
fn install(boolfn: Option<BoolHook>, trifn: Option<TriHook>) {
    INSTALL.call_once(|| {
        backend_access_gin_core_probe_seams::gin_consistent_call_bool::set(|key| {
            BOOL_HOOK.with(|h| {
        ...
        backend_access_gin_core_probe_seams::gin_consistent_call_tri::set(|key| {
            TRI_HOOK.with(|h| {
                let mut slot = h.borrow_mut();
...
fn install_bool(f: fn(&[types_tsearch::gin::GinTernaryValue]) -> (bool, bool)) {
    install(
...
fn install_tri(f: fn(&[types_tsearch::gin::GinTernaryValue]) -> types_tsearch::gin::GinTernaryValue) {
