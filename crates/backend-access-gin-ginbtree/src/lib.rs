...
//! [`types_gin::GinBtreeData`] method table. Those callbacks
...
use backend_storage_page::{PageGetSpecialPointer, PageRef};
    ...
    match begin.rc {
        ...
        GinPlaceToPageRC::GPTP_SPLIT => {
            ...
            if is_root_split {
                ...
                gin_init_page(
                    &mut root_tmp,
                    (newl_opaque.flags & !(GIN_LEAF | types_gin::GIN_COMPRESSED)) as u32,
                    BLCKSZ,
                )?;
...
fn gin_init_page(page: &mut [u8], flags: u32, page_size: usize) -> PgResult<()> {
    backend_storage_page::PageInit(
        page,
        page_size,
        core::mem::size_of::<types_gin::GinPageOpaqueData>(),
    )?;
...
fn page_get_temp_page(src: &[u8]) -> PgResult<Vec<u8>> {
    ...
    let tmp = backend_storage_page::PageGetTempPage(&pr)?;
    Ok(tmp.into_bytes())
}
