...
use types_gin::{GIN_DATA, GIN_LEAF, GIN_META};
...
fn category_offset_and_roundtrip() {
    ...
    GinSetNullCategory(&itup, &mut bytes, true, types_gin::GIN_CAT_NULL_KEY);
    assert_eq!(
        GinGetNullCategory(&itup, &bytes, true),
        types_gin::GIN_CAT_NULL_KEY
    );

    GinSetNullCategory(&itup, &mut bytes, false, types_gin::GIN_CAT_EMPTY_QUERY);
    assert_eq!(
        GinGetNullCategory(&itup, &bytes, false),
        types_gin::GIN_CAT_EMPTY_QUERY
    );
}
