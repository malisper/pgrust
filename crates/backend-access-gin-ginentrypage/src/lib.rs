...
//! (`ginbtree.c`, the landed [`backend_access_gin_ginbtree`] crate) dispatches
//! through.
//!
...
//! the [`backend_access_gin_gindatapage`] byte substrate. The null-category
//! comparison (`ginCompareAttEntries`), the key/attnum deform
//! (`gintuple_get_key` / `gintuple_get_attrnum`), `index_form_tuple`, and
//! `GinInitPage` are the [`backend_access_gin_ginutil`] owner's logic (catalog /
...
use backend_access_common_indextuple::{index_form_tuple, FormedIndexTuple};
...
/// substrate ([`backend_access_gin_ginutil`]'s indextuple owner); the GIN
...
/// audited [`backend_access_gin_core_probe`] lane. Decode a compressed posting
/// list image into item pointers.
fn gin_posting_list_decode(plist: &[u8], ndecoded: &mut i32) -> Vec<ItemPointerData> {
    backend_access_gin_core_probe::ginpostinglist::ginPostingListDecode(plist, Some(ndecoded))
}
