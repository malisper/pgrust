...
//! vacuum itself ([`backend_access_gin_gindatapage::ginVacuumPostingTreeLeaf`])
...
//! flush (`ginInsertCleanup`), via [`backend_access_gin_ginvacuum_seams`].
//!
...
use backend_access_gin_core_probe::ginpostinglist::{ginCompressPostingList, ginPostingListDecode};
...
use backend_access_gin_ginvacuum_seams as gvsx;
...
pub fn init_seams() {
    backend_access_gin_ginutil_seams::ginbulkdelete::set(
...
fn read_next_transaction_id() -> PgResult<TransactionId> {
    backend_access_heap_vacuumlazy_seams::read_next_transaction_id::call()
}
...
fn vacuum_tid_is_dead(tid: ItemPointerData, callback_state: u64) -> bool {
    backend_commands_vacuum_seams::vacuum_tid_is_dead::call(tid, callback_state)
}
...
fn relation_is_local(relid: Oid) -> PgResult<bool> {
    backend_access_heap_hio_seams::relation_is_local::call(relid)
}
fn lock_relation_for_extension<'mcx>(rel: &Relation<'mcx>) -> PgResult<()> {
    ...
    let guard = backend_storage_lmgr_lmgr_seams::lock_relation_for_extension::call(rel)?;
