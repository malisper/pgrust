...
//! [`backend_access_gin_gindatapage`] (`ginpage`/`postinglist` accessors),
//! [`backend_access_gin_core_probe`] (the posting-list codec
//! `ginCompressPostingList`/`ginPostingListDecode`/`ginMergeItemPointers`),
//! and [`backend_access_gin_ginutil`] (`GinInitBuffer`/`GinInitMetabuffer`).
...
use backend_access_gin_gindatapage as gdp;
use backend_access_gin_gindatapage::datatree::GinDataPageAddPostingItem;
use backend_access_gin_ginutil::{GinInitBuffer, GinInitMetabuffer};
...
pub fn init_seams() {
    backend_access_gin_core_seams::gin_redo::set(gin_redo);
    backend_access_gin_core_seams::gin_xlog_startup::set(gin_xlog_startup);
    backend_access_gin_core_seams::gin_xlog_cleanup::set(gin_xlog_cleanup);
    backend_access_gin_core_seams::gin_mask::set(gin_mask);
}
