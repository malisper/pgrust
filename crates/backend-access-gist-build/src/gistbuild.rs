...
use backend_access_gist_core::gist_insert::{gistdoinsert, gistplacetopage, gistSplit, PlaceToPage};

use backend_access_index_indexam_seams as indexam;
use backend_access_table_tableam_seams as tableam;
use backend_access_transam_parallel_rt_seams as parallel_rt;
...
use types_gist::GISTBuildBuffers;
...
struct GISTBuildState<'mcx> {
    ...
    sortstate: Option<mcx::PgBox<'mcx, types_nodes::nodesort::Tuplesortstate<'mcx>>>,
...
pub fn gistbuild<'mcx>(
    ...
    if state.buildMode == GistBuildMode::SortedBuild {
        ...
        let work_mem = backend_access_heap_vacuumlazy_seams::maintenance_work_mem::call()?;
        let sortstate = backend_utils_sort_tuplesort_seams::tuplesort_begin_index_gist::call(
            mcx,
            heap,
            index,
            work_mem,
        ...
        backend_utils_sort_tuplesort_seams::tuplesort_performsort::call(
            state.sortstate.as_mut().expect("sortstate set"),
        )?;

        gist_indexsortbuild(&mut state)?;

        backend_utils_sort_tuplesort_seams::tuplesort_end::call(
...
fn gistSortedBuildCallback<'mcx>(
    ...
    backend_utils_sort_tuplesort_seams::tuplesort_putindextuplevalues::call(
        state.sortstate.as_mut().expect("sortstate set"),
        &state.indexrel,
        *tid,
        &compressed_values,
...
fn gist_indexsortbuild<'mcx>(state: &mut GISTBuildState<'mcx>) -> PgResult<()> {
    ...
    while let Some(itup) = backend_utils_sort_tuplesort_seams::tuplesort_getindextuple::call(
        state.sortstate.as_mut().expect("sortstate set"),
        true,
    )? {
...
fn gist_indexsortbuild_levelstate_flush<'mcx>(
    ...
    if levelstate.current_page > 0 {
        ...
        dist = alloc::vec![SplitPageLayout {
            block: types_gist::gistxlogPage { blkno: 0, num },
            list,
            lenlist,
...
fn gistInitBuffering<'mcx>(state: &mut GISTBuildState<'mcx>) -> PgResult<()> {
    ...
    let effective_cache_size =
        backend_utils_misc_guc_tables::vars::effective_cache_size.read() as f64;
    let maintenance_work_mem =
        backend_access_heap_vacuumlazy_seams::maintenance_work_mem::call()? as f64;
...
fn gistBuildCallback<'mcx>(
    ...
    let effective_cache_size =
        backend_utils_misc_guc_tables::vars::effective_cache_size.read();
