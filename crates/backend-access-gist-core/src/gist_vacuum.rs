...
use backend_lib_integerset::{intset_create, IntegerSet};
use mcx::Mcx;
...
use types_gist::{GistNSN, GIST_ROOT_BLKNO};
...
fn relation_is_local<'mcx>(rel: &Relation<'mcx>) -> PgResult<bool> {
    backend_access_heap_hio_seams::relation_is_local::call(rel.rd_id)
}
...
fn read_next_full_transaction_id() -> FullTransactionId {
    backend_access_transam_varsup_seams::read_next_full_transaction_id::call()
}
