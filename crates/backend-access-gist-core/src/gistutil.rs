...
use alloc::vec::Vec;
use backend_access_common_heaptuple::{heap_form_tuple, FormedTuple};
use backend_access_common_indextuple_seams::{index_form_tuple_desc, nocache_index_getattr};
use backend_access_common_tupdesc::CreateTupleDescTruncatedCopy;
use backend_access_gist_dispatch_seams::{
    ...
    while i <= maxoff {
        ...
        if j == nkeyatts && result != i {
            if keep_current_best == -1 {
                keep_current_best = if pg_prng::global_prng(|s| s.next_bool()) { 1 } else { 0 };
            }
...
pub fn gistNewBuffer<'mcx>(
    ...
    loop {
        let blkno = backend_storage_freespace_seams::get_free_index_page::call(r)?;
        ...
        if conditional_lock_buffer::call(buffer)? {
            ...
            if recyclable {
                ...
                if backend_access_transam_xlog_seams::xlog_standby_info_active::call()
                    && backend_utils_cache_relcache_seams::relation_needs_wal::call(r)
                {
                    let page = buffer_get_page::call(mcx, buffer)?;
...
pub fn gist_page_recyclable(page: &[u8]) -> PgResult<bool> {
    ...
    if GistPageIsDeleted(page)? {
        ...
        let state =
            backend_storage_ipc_procarray_seams::global_vis_test_for::call(InvalidOid)?;
        return Ok(
            backend_storage_ipc_procarray_seams::global_vis_test_is_removable_fullxid::call(
                state,
                deletexid_full,
            ),
        );
