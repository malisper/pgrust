...
use types_gist::SplitPageLayout;
...
fn record_get_block_tag_blkno(
    ...
    let tag = backend_access_transam_xlogreader_seams::xlog_rec_get_block_tag_extended::call(
        record, block_id,
    )?;
...
pub fn gist_xlog_page_reuse(
    ...
    let xlrec_reuse = GistxlogPageReuse {
        is_catalog_rel:
            backend_utils_cache_relcache_seams::relation_is_accessible_in_logical_decoding::call(
                heaprel,
            )?,
...
pub fn gist_get_fake_lsn(rel: &Relation<'_>) -> PgResult<XLogRecPtr> {
    ...
    if persistence == RELPERSISTENCE_TEMP {
        ...
        debug_assert!(!backend_utils_cache_relcache_seams::relation_needs_wal::call(rel));
...
fn in_hot_standby() -> bool {
    types_wal::xlogutils::in_hot_standby(
        backend_access_transam_xlogutils::standby_state(),
    )
}
