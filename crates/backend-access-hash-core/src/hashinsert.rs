    ...
    loop {
        ...
        let max_item = with_page_ref(metabuf, |p| {
            Ok(crate::pagebytes::hash_max_item_size(
                backend_storage_page::PageGetPageSize(p),
            ))
        })?;
        if itemsz > max_item {
        ...
        let mut usedmetap = types_hash::hashpage::HashMetaPageData::default();
        let mut buf =
...
fn relation_is_accessible_in_logical_decoding<'mcx>(hrel: &Relation<'mcx>) -> PgResult<bool> {
    backend_utils_cache_relcache_seams::relation_is_accessible_in_logical_decoding::call(hrel)
}
