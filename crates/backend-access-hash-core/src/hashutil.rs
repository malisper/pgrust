...
pub fn _hash_checkpage<'mcx>(
    ...
    bufmgr::with_buffer_page::call(buf, &mut |page: &mut [u8]| {
        ...
        if backend_storage_page::PageGetSpecialSize(&pref) as usize
...
pub fn hashoptions(reloptions: Option<&[u8]>, validate: bool) -> PgResult<Option<alloc::vec::Vec<u8>>> {
    backend_access_common_reloptions_seams::build_reloptions_hash::call(reloptions, validate)
}
...
pub fn _hash_get_oldblock_from_newbucket<'mcx>(
    ...
    let metabuf = _hash_getbuf(rel, HASH_METAPAGE, types_hash::hashpage::HASH_READ, LH_META_PAGE as i32)?;
...
pub fn _hash_kill_items<'mcx>(scan: &mut HashScan<'mcx>) -> PgResult<()> {
    ...
    debug_assert!(types_hash::hashpage::HashScanPosIsValid(&scan.opaque.currPos));
    ...
    let buf;
    if types_hash::hashpage::HashScanPosIsPinned(&scan.opaque.currPos) {
        ...
        buf = _hash_getbuf(&rel, blkno, types_hash::hashpage::HASH_READ, LH_OVERFLOW_PAGE as i32)?;
    }
