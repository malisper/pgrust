...
fn int_family_matches_uint32_mixer() {
    for n in [-7i32, -1, 0, 1, 42, 1_000_000, i32::MIN, i32::MAX] {
        assert_eq!(hashint4(n), common_hashfn::hash_bytes_uint32(n as u32));
    }
    assert_eq!(hashint2(-5), common_hashfn::hash_bytes_uint32((-5i32) as u32));
    assert_eq!(hashchar(-1), common_hashfn::hash_bytes_uint32((-1i32) as u32));
    assert_eq!(hashchar(127), common_hashfn::hash_bytes_uint32(127u32));
    assert_eq!(hashoid(12345), common_hashfn::hash_bytes_uint32(12345));
    assert_eq!(hashenum(12345), common_hashfn::hash_bytes_uint32(12345));
}
...
fn extended_seed_threads_through() {
    ...
    assert_eq!(
        ...
        common_hashfn::hash_bytes_uint32_extended(42u32, seed)
    );
    assert_eq!(
        hashoidextended(99, seed),
        common_hashfn::hash_bytes_uint32_extended(99u32, seed)
    );
...
fn float_nan_canonicalized() {
    ...
    let expected = common_hashfn::hash_bytes(&get_float8_nan().to_ne_bytes());
...
fn oidvector_hashes_value_image() {
    ...
    assert_eq!(
        ...
        common_hashfn::hash_bytes(&values_image)
    );
    let seed = 7;
    assert_eq!(
        hashoidvectorextended(&image, seed).unwrap(),
        common_hashfn::hash_bytes_extended(&values_image, seed)
    );
}
...
fn name_varlena_bytea_hash_their_image() {
    ...
    assert_eq!(hashname(bytes), common_hashfn::hash_bytes(bytes));
    assert_eq!(hashvarlena(bytes), common_hashfn::hash_bytes(bytes));
    ...
    assert_eq!(
        ...
        common_hashfn::hash_bytes_extended(bytes, seed)
    );
}
...
fn text_deterministic_hashes_image() {
    ...
    assert_eq!(hashtext(key, 100).unwrap(), common_hashfn::hash_bytes(key));
    let seed = 55;
    assert_eq!(
        hashtextextended(key, 100, seed).unwrap(),
        common_hashfn::hash_bytes_extended(key, seed)
    );
}
...
fn text_nondeterministic_uses_strxfrm() {
    ...
    assert_eq!(
        ...
        common_hashfn::hash_bytes(&transformed)
    );
    DETERMINISTIC.store(true, Ordering::SeqCst);
}
