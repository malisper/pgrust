...
pub fn heapam_relation_copy_for_cluster<'mcx>(
    ...
    let is_system_catalog = backend_catalog_catalog::IsSystemRelation(old_heap);
    ...
    let mut slot = backend_access_table_tableam::table_slot_create(mcx, old_heap)?;
        ...
        let isdead: bool = match htsv {
            ...
            HTSV_Result::HEAPTUPLE_INSERT_IN_PROGRESS => {
                ...
                if !is_system_catalog
                    && !backend_access_transam_xact::TransactionIdIsCurrentTransactionId(xmin)
                {
                    ereport(WARNING)
            ...
            HTSV_Result::HEAPTUPLE_DELETE_IN_PROGRESS => {
                ...
                if !is_system_catalog
                    && !backend_access_transam_xact::TransactionIdIsCurrentTransactionId(update_xid)
                {
                    ereport(WARNING)
                        .errmsg_internal(format!(
...
fn reform_and_rewrite_tuple<'mcx>(
    mcx: Mcx<'mcx>,
    tuple: &backend_access_common_heaptuple::FormedTuple<'mcx>,
    old_heap: &Relation<'mcx>,
    new_heap: &Relation<'mcx>,
    ...
    let cols =
        backend_access_common_heaptuple::heap_deform_tuple(mcx, &tuple.tuple, old_tup_desc, &tuple.data)?;

    let new_natts = new_tup_desc.natts as usize;
    let mut values: Vec<backend_access_common_heaptuple::Datum<'mcx>> = Vec::with_capacity(new_natts);
    ...
    for i in 0..new_natts {
        if i < cols.len() {
            ...
            values.push(backend_access_common_heaptuple::Datum::null());
            isnull.push(true);
        }
    ...
    let copied_tuple = backend_access_common_heaptuple::heap_form_tuple(mcx, new_tup_desc, &values, &isnull)
        .map_err(|e| {
            ereport(ERROR)
