...
//! `types_xlog_records` value structs.
//!
...
//! the change with the `backend_storage_page` codec (`PageInit`, `PageAddItem`,
...
use backend_access_transam_xlogutils::{
...
use backend_storage_freespace::XLogRecordPageWithFreeSpace;
...
use backend_rmgrdesc_next::heapdesc::heap_xlog_deserialize_prune_and_freeze;

use backend_access_common_bufmask_seams as bufmask;
use backend_access_heap_pruneheap_seams as pruneheap;
use backend_access_heap_rewriteheap_seams as rewrite;
...
fn visibilitymap_clear_fake(rlocator: RelFileLocator, blkno: BlockNumber, flags: u8) -> PgResult<()> {
    with_redo_ctx(|mcx| {
        ...
        let reln = types_rel::Relation::open(
            relcache::create_fake_relcache_entry::call(mcx, rlocator)?,
            None,
        );
        let mut vmbuffer: Buffer = InvalidBuffer;
        backend_access_heap_visibilitymap::visibilitymap_pin(&reln, blkno, &mut vmbuffer)?;
        backend_access_heap_visibilitymap::visibilitymap_clear(&reln, blkno, vmbuffer, flags)?;
...
fn heap_xlog_visible(record: &XLogReaderState<'_>) -> PgResult<()> {
    ...
    if vmaction == XLogRedoAction::BlkNeedsRedo {
        ...
        with_redo_ctx(|mcx| {
            ...
            backend_access_heap_visibilitymap::visibilitymap_pin(&reln, blkno, &mut vmbuffer)?;
            backend_access_heap_visibilitymap::visibilitymap_set(
                &reln,
                blkno,
                InvalidBuffer,
                lsn,
...
fn backend_access_transam_xlog_hint_bit_is_needed() -> bool {
    backend_access_transam_xlog_seams::xlog_hint_bit_is_needed::call()
}
...
pub fn init_seams() {
    backend_access_heap_heapam_xlog_seams::heap_redo::set(heap_redo);
    backend_access_heap_heapam_xlog_seams::heap2_redo::set(heap2_redo);
    backend_access_heap_heapam_xlog_seams::heap_mask::set(heap_mask);
}
