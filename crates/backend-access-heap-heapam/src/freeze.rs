...
use types_xlog_records::multixact::{MultiXactMember, MultiXactStatus};
...
pub fn heap_pre_freeze_checks<'mcx>(
    ...
    bufmgr_seam::with_buffer_page::call(buffer, &mut |page_bytes| {
        ...
        for frz in tuples.iter() {
            let item_id = backend_storage_page::PageGetItemId(&page, frz.offset)?;
            ...
            if frz.checkflags & HEAP_FREEZE_CHECK_XMIN_COMMITTED != 0 {
                ...
                debug_assert!(!backend_access_heap_heapam_visibility::htup::HeapTupleHeaderXminFrozen(&htup));
