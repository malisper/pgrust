...
//! [`backend_access_heap_rewriteheap_seams::RewriteState`] (mirroring
...
use backend_storage_smgr_bulkwrite as bulkwrite;
use backend_storage_page::{
    PageAddItemExtended, PageGetHeapFreeSpace, PageGetItem, PageGetItemId, PageInit, PageMut,
    PageRef,
};

use backend_access_heap_heaptoast::{heap_toast_insert_or_update, TOAST_TUPLE_THRESHOLD};
use backend_access_heap_heapam::freeze::heap_freeze_tuple;
use backend_access_heap_heapam::insert::{HEAP_INSERT_NO_LOGICAL, HEAP_INSERT_SKIP_FSM};
...
use backend_utils_init_small_seams as globals_seam;
...
pub use backend_access_heap_rewriteheap_seams as seams;
use seams::{
...
const RM_HEAP2_ID: types_core::RmgrId = types_wal::wal::RM_HEAP2_ID;
...
fn logical_heap_rewrite_flush_mappings<'mcx>(state: &mut RewriteStateData<'mcx>) -> PgResult<()> {
    ...
    for xid in xids {
        ...
        let written = backend_storage_file_fd::vfd_io::FileWriteV(
            vfd,
...
fn logical_end_heap_rewrite<'mcx>(state: &mut RewriteStateData<'mcx>) -> PgResult<()> {
    ...
    for (vfd, path) in entries {
        ...
        if let Err(_e) = backend_storage_file_fd::vfd_io::FileSync(vfd, WAIT_EVENT_LOGICAL_REWRITE_SYNC) {
            return Err(ereport(ERROR)
                .errcode_for_file_access()
        ...
        // FileClose(src->vfd);
        backend_storage_file_fd::vfd_io::FileClose(vfd)?;
    }
...
fn logical_rewrite_log_mapping<'mcx>(
    ...
    if !state.rs_logical_mappings.contains_key(&xid) {
        ...
        let vfd = backend_storage_file_fd::vfd_io::PathNameOpenFile(
            &path,
...
fn RelationGetNumberOfBlocks(rel: &RelationData<'_>) -> PgResult<BlockNumber> {
    ...
    backend_storage_smgr_smgr::smgrnblocks(key, MAIN_FORKNUM)
}
