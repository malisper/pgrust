...
//! `xl_replorigin_*` payloads are parsed by [`types_wal`]'s bounds-checked
//! `from_bytes`.
...
use types_wal::{xl_replorigin_drop, xl_replorigin_set, DecodedXLogRecord, XLR_INFO_MASK};
...
pub fn init_seams() {
    backend_access_rmgrdesc_replorigindesc_seams::replorigin_desc::set(replorigin_desc_seam);
    backend_access_rmgrdesc_replorigindesc_seams::replorigin_identify::set(replorigin_identify);
}
