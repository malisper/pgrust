...
//!   exact C struct field offsets by [`types_wal`]'s bounds-checked
...
pub fn init_seams() {
    backend_access_rmgrdesc_xlogdesc_seams::xlog_desc::set(xlog_desc_seam);
    backend_access_rmgrdesc_xlogdesc_seams::xlog_identify::set(xlog_identify);
}
