...
//! ported sibling [`backend_access_transam_slru`]. The SLRU control struct
...
fn commit_ts_lock() -> &'static types_storage::LWLock {
    backend_storage_lmgr_lwlock::main_lock_ref(COMMIT_TS_LOCK_OFFSET)
}
...
fn error_commit_ts_disabled() -> PgError {
    ...
    let builder = if backend_access_transam_xlog_seams::recovery_in_progress::call() {
        builder.errhint(
...
pub fn ExtendCommitTs(state: &mut CommitTsState, newestXact: TransactionId) -> PgResult<()> {
    ...
    debug_assert!(!backend_access_transam_xlogrecovery_seams::in_recovery::call());
    if !state.shared.commitTsActive {
        return Ok(());
    }
    ...
    let in_recovery = backend_access_transam_xlogrecovery_seams::in_recovery::call();
...
fn WriteZeroPageXlogRec(pageno: i64) -> PgResult<()> {
    ...
    backend_access_transam_xloginsert_seams::xlog_insert::call(
        RM_COMMIT_TS_ID,
        COMMIT_TS_ZEROPAGE,
        0,
        &[&pageno_bytes],
...
pub fn init_seams() {
    use backend_access_transam_commit_ts_seams as seams;
    ...
    backend_utils_misc_guc_tables::hooks::check_commit_ts_buffers.install(
    ...
    seams::commit_ts_redo::set(|record| {
        ...
        let info = decoded.info() & !types_wal::XLR_INFO_MASK;
