...
fn install_seams() {
    ...
    INSTALLED.call_once(|| {
        backend_utils_cache_relcache_seams::relation_needs_wal::set(|_rel| {
            with_rt(|rt| rt.is_logged)
        });
        backend_storage_buffer_bufmgr_seams::with_buffer_page::set(|buf, f| {
        ...
        backend_storage_buffer_bufmgr_seams::mark_buffer_dirty::set(|buf| {
            with_rt(|rt| rt.dirty.push(buf))
        });
        backend_storage_buffer_bufmgr_seams::unlock_release_buffer::set(|_buf| {});
        backend_access_transam_xloginsert_seams::xlog_begin_insert::set(|| Ok(()));
        backend_access_transam_xloginsert_seams::xlog_register_buffer::set(|bid, buf, flags| {
            with_rt(|rt| rt.registered.push((bid, buf, flags)));
            Ok(())
        });
        backend_access_transam_xloginsert_seams::xlog_register_buf_data::set(|bid, data| {
            with_rt(|rt| {
                rt.buf_data.insert(bid, data.to_vec());
            });
            Ok(())
        });
        backend_access_transam_xloginsert_seams::xlog_insert_record::set(|_rmid, _info| {
            Ok(with_rt(|rt| rt.inserted_lsn))
        });
        backend_access_common_bufmask_seams::mask_page_lsn_and_checksum::set(|page| {
            for b in page.iter_mut().take(10) {
                *b = 0;
            }
        });
        backend_access_common_bufmask_seams::mask_unused_space::set(|page| {
            let lower =
