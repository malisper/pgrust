...
use backend_access_transam_transam::{TransactionIdFollowsOrEquals, TransactionIdPrecedes};
...
use backend_storage_sync_seams as sync_seams;
use backend_utils_activity_small::pgstat_checkpointer::with_pending_checkpointer_stats;
...
fn SimpleLruWaitIO(ctl: &mut SlruCtlData, slotno: usize) -> PgResult<()> {
    ...
    LWLockAcquire(&ctl.shared.buffer_locks[slotno].lock, LW_SHARED)?;
    LWLockRelease(&ctl.shared.buffer_locks[slotno].lock)?;
    LWLockAcquire(&ctl.shared.bank_locks[bankno].lock, LW_EXCLUSIVE)?;
...
pub fn SimpleLruReadPage(
    ...
    loop {
        ...
        LWLockAcquire(&ctl.shared.buffer_locks[slotno].lock, LW_EXCLUSIVE)?;
...
pub fn SimpleLruReadPage_ReadOnly(
    ...
    LWLockAcquire(&ctl.shared.bank_locks[bankno].lock, LW_SHARED)?;
...
pub fn SimpleLruWriteAll(ctl: &mut SlruCtlData, allow_redirtied: bool) -> PgResult<()> {
    ...
    LWLockAcquire(&ctl.shared.bank_locks[prevbank].lock, LW_EXCLUSIVE)?;
    ...
    for slotno in 0..ctl.shared.num_slots as usize {
        ...
        if curbank != prevbank {
            ...
            LWLockAcquire(&ctl.shared.bank_locks[curbank].lock, LW_EXCLUSIVE)?;
            prevbank = curbank;
        }
