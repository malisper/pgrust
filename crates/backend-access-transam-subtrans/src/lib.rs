...
//! ported sibling [`backend_access_transam_slru`]. The shared control struct
...
pub fn check_subtrans_buffers(newval: i32) -> PgResult<bool> {
    let (ok, detail) =
        backend_access_transam_slru::check_slru_buffers("subtransaction_buffers", newval);
    if ok {
        Ok(true)
    } else {
...
pub fn StartupSUBTRANS(
    ...
    let nextXid = backend_access_transam_varsup_seams::read_next_full_transaction_id::call();
...
pub fn init_seams() {
    use backend_access_transam_subtrans_seams as seams;
