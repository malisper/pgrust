...
use types_wal::wal::{BackupState, SessionBackupState, RM_XLOG_ID};
...
fn xlog_is_needed() -> bool {
    backend_utils_misc_guc_tables::vars::wal_level.read()
        >= types_wal::xlog_consts::WalLevel::Replica as i32
}
...
fn collect_tablespaces(
    ...
    for d_name in backend_storage_file_fd_seams::read_dir_names::call(PG_TBLSPC_DIR)? {
        ...
        let de_type = backend_storage_file_fd_seams::get_dirent_type::call(&fullpath);
        ...
        if de_type == PGFILETYPE_LNK {
            ...
            let target = match backend_storage_file_fd_seams::read_link::call(&fullpath)? {
                Some(t) => t,
                None => {
                    ereport(WARNING)
...
pub fn do_pg_backup_stop(
    ...
    if backup_stopped_in_recovery {
        ...
        let history_file =
            backend_access_transam_xlogbackup::build_backup_content_default(&state, true)?;

        // fp = AllocateFile(histfilepath, "w"); fprintf("%s"); fflush/FreeFile.
        backend_storage_file_fd_seams::allocate_file_write::call(&histfilepath, &history_file)?;
    ...
    let archive_mode = backend_utils_misc_guc_tables::vars::XLogArchiveMode.read();
    ...
    if should_wait {
        ...
        while backend_access_transam_xlogarchive::XLogArchiveIsBusy(&lastxlogfilename)
            || backend_access_transam_xlogarchive::XLogArchiveIsBusy(&histfilename)
        {
...
fn CleanupBackupHistory() -> PgResult<()> {
    for d_name in backend_storage_file_fd_seams::read_dir_names::call(XLOGDIR)? {
        if IsBackupHistoryFileName(&d_name)
            && backend_access_transam_xlogarchive::XLogArchiveCheckDone(&d_name)?
        {
            ...
            backend_storage_file_fd_seams::unlink_file::call(&path);
            backend_access_transam_xlogarchive::XLogArchiveCleanup(&d_name);
        }
    }
    Ok(())
}
...
fn do_pg_backup_start_for_basebackup(
    backupidstr: &str,
    fast: bool,
) -> PgResult<backend_backup_basebackup_seams::BackupStartResult> {
    let (state, tablespaces, tablespace_map) = do_pg_backup_start(backupidstr, fast)?;
    Ok(backend_backup_basebackup_seams::BackupStartResult {
        state,
        tablespaces,
        tablespace_map,
    })
...
pub fn init_seams() {
    use backend_access_transam_xlog_seams as xs;
    ...
    xs::xlog_archive_mode::set(|| {
        match backend_utils_misc_guc_tables::vars::XLogArchiveMode.read() {
            0 => types_wal::xlog_consts::ArchiveMode::Off,
            1 => types_wal::xlog_consts::ArchiveMode::On,
            2 => types_wal::xlog_consts::ArchiveMode::Always,
            other => panic!("invalid archive_mode GUC value {other}"),
        }
    });

    backend_backup_basebackup_seams::do_pg_backup_start_for_basebackup::set(
        do_pg_backup_start_for_basebackup,
    );
    backend_backup_basebackup_seams::do_pg_abort_backup::set(do_pg_abort_backup_seam);
}
