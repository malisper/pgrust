...
use backend_utils_init_small_seams as init_globals;
...
fn build_record(rmid: u8, info: u8, data: &[u8]) -> Vec<u8> {
    ...
    let partial = port_crc32c::pg_comp_crc32c_sb8(0xFFFF_FFFF, data);
...
fn insert_single_record_lands_in_wal_buffer() {
    ...
    let usable_per_page =
        (types_wal::xlog_consts::XLOG_BLCKSZ - types_wal::xlog_consts::SIZE_OF_XLOG_SHORT_PHD) as u64;
    let seg = shmem::wal_segment_size();
    let start_bytepos = usable_per_page;
    // SAFETY: live region; single-threaded test.
    unsafe {
        (*ctl).SharedRecoveryState = types_wal::xlog_consts::RecoveryState::Done;
    ...
    let blcksz = types_wal::xlog_consts::XLOG_BLCKSZ as u64;
    let page_idx = ((start / blcksz) % 8) as usize;
    let page_off = (start % blcksz) as usize;
    // SAFETY: live region.
    unsafe {
        let page = (*ctl).pages.add(page_idx * types_wal::xlog_consts::XLOG_BLCKSZ);
        // Page header magic.
        ...
        let seed = port_crc32c::pg_comp_crc32c_sb8(0xFFFF_FFFF, &data);
        let expected_crc = port_crc32c::pg_comp_crc32c_sb8(seed, &hdr20) ^ 0xFFFF_FFFF;
