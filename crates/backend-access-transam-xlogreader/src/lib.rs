...
//!
//! [`decode_arena`]: types_wal::rmgr::XLogReaderState::decode_arena
//!
...
use types_wal::rmgr::{XLogReaderState, RmgrIdIsValid, XLREAD_FAIL, XLREAD_SUCCESS, XLREAD_WOULDBLOCK};
...
fn decode_block_image_page(
    ...
    if BKPIMAGE_COMPRESSED(bimg_info) {
        if (bimg_info & BKPIMAGE_COMPRESS_PGLZ) != 0 {
            match common_pglz::pglz_decompress_to_slice(
                &raw[..bimg_len],
...
pub fn read_ahead_record_info(
    state: &XLogReaderState<'_>,
) -> Option<types_wal::ReadAheadRecordInfo> {
    let r = queue_tail(state)?;
    Some(types_wal::ReadAheadRecordInfo {
        lsn: r.lsn(),
...
// `types_wal::RelFileLocator`. They are the same ABI (three Oids).
// ---------------------------------------------------------------------------
