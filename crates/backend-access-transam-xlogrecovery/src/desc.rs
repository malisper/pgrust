...
pub fn xlog_outdesc<'mcx>(
    ...
    let info = backend_access_transam_xlogreader::XLogRecGetInfo(reader_state());
...
pub(crate) fn xlog_outrec(buf: &mut PgString<'_>, record: RecordRef) -> Result<(), PgError> {
    buf.try_push_str(&format!(
        "prev {}; xid {}",
        lsn_fmt(backend_access_transam_xlogreader::XLogRecGetPrev(reader_state())),
        backend_access_transam_xlogreader::XLogRecGetXid(reader_state())
    ))?;

    buf.try_push_str(&format!(
        "; len {}",
        backend_access_transam_xlogreader::XLogRecGetDataLen(reader_state())
    ))?;
...
pub(crate) fn xlog_block_info(buf: &mut PgString<'_>, _record: RecordRef) -> Result<(), PgError> {
    let max_block_id = backend_access_transam_xlogreader::reader_max_block_id(reader_state());
    ...
    while block_id <= max_block_id {
        ...
        if backend_access_transam_xlogreader::xlog_rec_has_block_image(reader_state(), bid) {
