...
use types_wal::rmgr::{XLogReaderState, XLREAD_FAIL, XLREAD_WOULDBLOCK};
use types_wal::xlog_consts::XLOG_BLCKSZ;
use types_wal::xlogrecovery_carriers::{XLogPageReadResult, XLogSource};
...
use backend_access_transam_xlogarchive_seams as xlogarchive_seam;
    ...
    loop {
            ...
            && (target_page_ptr % wal_segment_size as u64) == 0
        {
            let valid = {
                ...
                backend_access_transam_xlogreader::XLogReaderValidatePageHeader(
                    reader,
                    target_page_ptr,
                    &page[..XLOG_BLCKSZ],
                )
            ...
            if !valid {
                ...
                backend_access_transam_xlogreader::XLogReaderResetError(reader);
                match next_record_is_invalid(reader) {
                    NextRecord::Retry => continue,
                    NextRecord::Return(code) => return Ok(code),
                }
...
fn wait_for_wal_to_become_available(
    ...
    loop {
        ...
        if crate::shmem::get_recovery_pause_state()
            != types_wal::wal::RecoveryPauseState::NotPaused
        {
...
fn recovery_wait_latch(wake_events: u32, timeout: i64, wait_event: u32) -> Result<(), PgError> {
    ...
    let _ = backend_storage_ipc_latch::WaitLatch(Some(handle), wake_events, timeout, wait_event)?;
    backend_storage_ipc_latch::ResetLatch(handle);
    Ok(())
}
...
fn x_log_file_read_any_tli(
    ...
    let tles: alloc::vec::Vec<types_wal::wal::TimeLineHistoryEntry> = if !st.expected_tles.is_empty() {
