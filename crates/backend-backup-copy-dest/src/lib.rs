...
//! [`backend_backup_copy_seams`] (`create_dest_remote_simple` /
...
use backend_backup_copy_seams::{
    ResultColumn, ResultColumnType, ResultValue, TupOutputState as SeamTupOutputState,
};

type RealTupOutputState<'mcx> = types_nodes::tuptable::TupOutputState<'mcx>;
...
fn create_dest_remote_simple() -> types_nodes::parsestmt::DestReceiverHandle {
    backend_tcop_dest_seams::create_dest_receiver::call(types_dest::CommandDest::RemoteSimple)
}
...
fn begin_tup_output_tupdesc(
    ...
    let state = backend_executor_execTuples_seams::begin_tup_output_tupdesc::call(
        mcx,
        dest,
        tupdesc,
...
fn do_tup_output(_tstate: SeamTupOutputState, values: Vec<Option<ResultValue>>) {
    IN_FLIGHT.with(|cell| {
        ...
        backend_executor_execTuples_seams::do_tup_output::call(
            mcx,
            state_mcx,
            &datum_values,
            &isnull,
...
fn end_tup_output(_tstate: SeamTupOutputState) {
    ...
    backend_executor_execTuples_seams::end_tup_output::call(mcx, state_mcx)
...
pub fn init_seams() {
    backend_backup_copy_seams::create_dest_remote_simple::set(create_dest_remote_simple);
    backend_backup_copy_seams::begin_tup_output_tupdesc::set(begin_tup_output_tupdesc);
    backend_backup_copy_seams::do_tup_output::set(do_tup_output);
    backend_backup_copy_seams::end_tup_output::set(end_tup_output);
}
