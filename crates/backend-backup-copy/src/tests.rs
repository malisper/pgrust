...
use backend_backup_sink::{BbsinkState, TablespaceInfo};
...
fn install_fixtures() {
    ...
    INSTALL.call_once(|| {
        ...
        backend_utils_mb_mbutils_seams::pg_server_to_client::set(|_mcx, _s| Ok(None));
        backend_utils_adt_timestamp_seams::get_current_timestamp::set(|| NOW.with(|n| n.get()));
        backend_utils_adt_timestamp_seams::timestamp_difference_milliseconds::set(
            |start, stop| (stop - start) / 1000,
        );
        backend_backup_copy_seams::create_dest_remote_simple::set(|| {
            EVENTS.with(|e| e.borrow_mut().push(Event::NewDest));
            types_nodes::parsestmt::DestReceiverHandle(1)
        });
        backend_backup_copy_seams::begin_tup_output_tupdesc::set(|dest, columns| {
            EVENTS.with(|e| e.borrow_mut().push(Event::Begin(columns)));
            backend_backup_copy_seams::TupOutputState { dest }
        });
        backend_backup_copy_seams::do_tup_output::set(|_tstate, values| {
            EVENTS.with(|e| e.borrow_mut().push(Event::Row(values)));
        });
        backend_backup_copy_seams::end_tup_output::set(|_tstate| {
...
fn begin_backup_emits_two_resultsets_command_complete_and_copyout() {
    ...
    backend_backup_sink::bbsink_begin_backup(&mut sink, &mut state, 8192).unwrap();
...
fn begin_archive_frames_new_archive_with_path() {
    ...
    backend_backup_sink::bbsink_begin_archive(&mut sink, &mut state, "base.tar").unwrap();
...
fn begin_archive_null_path_sends_empty_string() {
    ...
    backend_backup_sink::bbsink_begin_archive(&mut sink, &mut state, "x").unwrap();
...
fn archive_contents_ships_data_with_leading_type_byte() {
    ...
    state.bytes_done = 4;
    backend_backup_sink::bbsink_archive_contents(&mut sink, &mut state, 4).unwrap();
...
fn archive_contents_not_to_client_sends_nothing_but_may_report() {
    ...
    backend_backup_sink::bbsink_archive_contents(&mut sink, &mut state, 1).unwrap();
...
fn end_archive_forces_progress_report() {
    ...
    backend_backup_sink::bbsink_end_archive(&mut sink, &mut state).unwrap();
...
fn begin_manifest_sends_m_marker() {
    ...
    backend_backup_sink::bbsink_begin_manifest(&mut sink, &mut state).unwrap();
...
fn manifest_contents_ships_data_with_leading_type_byte() {
    ...
    backend_backup_sink::bbsink_manifest_contents(&mut sink, &mut state, 3).unwrap();
...
fn end_manifest_does_nothing() {
    ...
    backend_backup_sink::bbsink_end_manifest(&mut sink, &mut state).unwrap();
...
fn end_backup_sends_copydone_then_xlogpos_result() {
    ...
    backend_backup_sink::bbsink_end_backup(&mut sink, &mut state, 0x0000_0002_0000_0040, 9)
        .unwrap();
...
fn cleanup_does_nothing() {
    ...
    backend_backup_sink::bbsink_cleanup(&mut sink, &mut state).unwrap();
