...
use backend_catalog_pg_attrdef_seams as attrdef_seams;
use backend_catalog_pg_constraint_seams as constraint_seams;
use backend_catalog_pg_depend_seams as pg_depend_seams;
use backend_catalog_pg_largeobject_seams as largeobject_seams;
use backend_catalog_pg_operator_seams as operator_seams;
use backend_catalog_pg_shdepend_seams as shdepend_seams;
use backend_commands_comment_seams as comment_seams;
use backend_commands_event_trigger_seams as event_trigger_seams;
use backend_commands_extension_seams as extension_seams;
use backend_commands_functioncmds_seams as functioncmds_seams;
use backend_commands_policy_seams as policy_seams;
use backend_commands_publicationcmds_seams as publicationcmds_seams;
use backend_commands_seclabel_seams as seclabel_seams;
use backend_commands_sequence_seams as sequence_seams;
use backend_commands_statscmds_seams as statscmds_seams;
...
fn reportDependentObjects(
    ...
    if behavior == DROP_CASCADE
        && !backend_utils_error::message_level_is_interesting(msglevel)
    {
        return Ok(());
    }
...
fn doDeletion(object: &ObjectAddress, flags: i32) -> PgResult<()> {
    ...
    if classId == RelationRelationId {
        ...
        backend_rewrite_rewriteRemove_seams::RemoveRewriteRuleById::call(object.objectId)?;
...
pub fn recordDependencyOnExpr(
    ...
    let mut owned_rtable: Vec<types_nodes::parsenodes::RangeTblEntry<'_>> =
