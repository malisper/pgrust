...
pub fn heap_drop_with_catalog<'mcx>(mcx: Mcx<'mcx>, relid: Oid) -> PgResult<()> {
    ...
    let Some(relispartition) =
        backend_utils_cache_syscache_seams::rel_relispartition::call(relid)?
    else {
    ...
    if relispartition {
         ...
         */
        parentOid = backend_catalog_partition::get_partition_parent(relid, true)?;
        backend_storage_lmgr_lmgr::LockRelationOid(parentOid, AccessExclusiveLock)?;
         ...
         */
        defaultPartOid = backend_catalog_partition::get_default_partition_oid(parentOid)?;
        if OidIsValid(defaultPartOid) && relid != defaultPartOid {
            backend_storage_lmgr_lmgr::LockRelationOid(defaultPartOid, AccessExclusiveLock)?;
        }
    }
     ...
     */
    let rel = backend_access_common_relation::relation_open(mcx, relid, AccessExclusiveLock)?;
     ...
     * our own session.
     */
    backend_commands_tablecmds_seams::check_table_not_in_use::call(&rel, "DROP TABLE")?;
     ...
     * the table.
     */
    backend_storage_lmgr_predicate_seams::check_table_for_serializable_conflict_in::call(&rel)?;

    /*
     * Delete pg_foreign_table tuple first.
     */
    if rel.rd_rel.relkind == RELKIND_FOREIGN_TABLE {
        let ftrel = backend_access_table_table::table_open(
            mcx,
        ...
        backend_catalog_indexing_seams::catalog_tuple_delete::call(&ftrel, fttuple.tuple.t_self)?;
    ...
    if relid == defaultPartOid {
        backend_catalog_partition::update_default_partition_oid(parentOid, InvalidOid)?;
    }
    ...
    if RELKIND_HAS_STORAGE(rel.rd_rel.relkind) {
        backend_catalog_storage_seams::relation_drop_storage::call(rel.rd_locator, rel.rd_backend)?;
    }

    /* ensure that stats are dropped if transaction commits */
    backend_utils_activity_pgstat_seams::pgstat_drop_relation::call(relid, rel.rd_rel.relisshared)?;
     ...
     */
    backend_catalog_pg_subscription_seams::remove_subscription_rel::call(InvalidOid, relid)?;

    /*
     * Forget any ON COMMIT action for the rel
     */
    backend_commands_tablecmds_seams::remove_on_commit_action::call(relid)?;

    /*
     * Flush the relation from the relcache.
     */
    backend_utils_cache_relcache::invalidate::RelationForgetRelation(relid)?;
    ...
    if OidIsValid(parentOid) {
         ...
         */
        if OidIsValid(defaultPartOid) && relid != defaultPartOid {
            backend_utils_cache_inval::cache_invalidate::CacheInvalidateRelcacheByRelid(defaultPartOid)?;
        }
         ...
         */
        backend_utils_cache_inval::cache_invalidate::CacheInvalidateRelcacheByRelid(parentOid)?;
        /* keep the lock */
    }
