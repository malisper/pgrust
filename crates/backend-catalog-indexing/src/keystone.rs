...
use backend_executor_execTuples::slot_ops_vtables::slot_getsysattr;
use backend_executor_execTuples::slot_store_fetch::ExecStoreHeapTuple;
...
pub fn CatalogOpenIndexes<'mcx>(
    ...
    if heap_rel.rd_rel.relhasindex {
        ...
        let indexoidlist =
            backend_utils_cache_relcache_seams::relation_get_index_list::call(mcx, &heap_relation)?;
...
pub fn CatalogCloseIndexes(indstate: CatalogIndexState<'_>) -> PgResult<()> {
    ...
    for index_desc in index_descs {
        backend_access_index_indexam::index_close(index_desc, NoLock)?;
    }
...
fn form_index_datum<'mcx>(
    ...
    Vec<types_tuple::backend_access_common_heaptuple::Datum<'mcx>>,
    Vec<bool>,
)> {
    ...
    let mut values: Vec<types_tuple::backend_access_common_heaptuple::Datum<'mcx>> =
        Vec::with_capacity(n);
...
pub fn CatalogTupleInsert<'mcx>(
    ...
    backend_access_heap_heapam_seams::simple_heap_insert::call(mcx, heap_rel, tup)?;
...
pub fn CatalogTupleUpdate<'mcx>(
    ...
    let update_indexes =
        backend_access_heap_heapam_seams::simple_heap_update::call(mcx, heap_rel, otid, tup)?;
...
pub fn CatalogTupleDelete<'mcx>(
    ...
    backend_access_heap_heapam_seams::simple_heap_delete::call(mcx, heap_rel, tid)
}
