...
use types_nodes::parsenodes::{makeRangeVar, RangeVar, OBJECT_SCHEMA};
use types_syscache::{AUTHMEMROLEMEM, AUTHOID, DATABASEOID, NAMESPACEOID};
use types_tuple::access::{AccessShareLock, LOCKMODE, NoLock};
...
use backend_catalog_pg_namespace_seams as pg_namespace_seams;
        ...
        if catalogname_differs_from_database(mcx, catalogname)? {
            return ereport(ERROR)
                ...
                .errmsg(format!(
                    ...
                    relation.relname.as_deref().unwrap_or_default()
                ))
        ...
        if catalogname_differs_from_database(mcx, catalogname)? {
            return ereport(ERROR)
                ...
                .errmsg(format!(
                    ...
                    newRelation.relname.as_deref().unwrap_or_default()
                ))
