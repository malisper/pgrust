...
pub fn get_relation_type_description<'mcx>(
    ...
    let relkind = match backend_utils_cache_syscache_seams::rel_relkind::call(relid)? {
        Some(rk) => rk,
        None => {
            if !missing_ok {
                return Err(PgError::error(format!(
...
pub fn get_constraint_type_description<'mcx>(
    ...
    let (conrelid, contypid, oid) =
        match backend_catalog_pg_constraint_seams::constraint_type_oids::call(constroid)? {
            Some(t) => t,
            None => {
                if !missing_ok {
                    return Err(PgError::error(format!(
...
pub fn get_procedure_type_description<'mcx>(
    ...
    let prokind = match backend_utils_cache_syscache_seams::pg_proc_form::call(mcx, procid)? {
