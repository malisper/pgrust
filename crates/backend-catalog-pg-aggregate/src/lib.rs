...
use backend_catalog_aclchk_seams::{aclcheck_error_type, object_aclcheck};
...
use backend_parser_parse_func_seams::{func_get_detail, FuncDetailCode};
use backend_parser_parse_oper_seams::lookup_oper_name;
use backend_utils_adt_format_type_seams::format_type_be;
use backend_utils_cache_lsyscache_seams::{func_strict, get_func_name};
use backend_utils_cache_syscache_seams::{aggregate_tuple_by_fnoid, pg_proc_form};

use backend_catalog_pg_aggregate_seams::AggregateCreateArgs;
...
fn name_list(names: &[types_parsenodes::Node]) -> Vec<String> {
    names
        .iter()
...
/// Install this unit's inward seam ([`backend_catalog_pg_aggregate_seams`]).
/// `aggregatecmds.c`'s `DefineAggregate` consumes `aggregate_create`.
pub fn init_seams() {
    backend_catalog_pg_aggregate_seams::aggregate_create::set(AggregateCreate);
}
