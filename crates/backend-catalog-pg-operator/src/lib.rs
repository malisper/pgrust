...
use types_nodes::parsenodes::{OBJECT_OPERATOR, OBJECT_SCHEMA};
...
pub fn OperatorLookup(
    ...
    let operatorObjectId = backend_parser_parse_oper::LookupOperName(
        None,
        operatorName,
        leftObjectId,
        rightObjectId,
    ...
    let oprcode = backend_utils_cache_lsyscache::opfamily_operator::get_opcode(operatorObjectId)?;
...
fn OperatorShellMake(
    ...
    let form = FormPgOperator {
        ...
        oprowner: backend_utils_init_miscinit::GetUserId(),
...
pub fn OperatorCreate(
    ...
    let operResultType = backend_utils_cache_lsyscache::function::get_func_rettype(procedureId)?;
...
fn get_other_operator(
    ...
    let (otherNamespace, otherName) =
        backend_catalog_namespace::QualifiedNameGetCreationNamespace(mcx, &other_nl)?;
    ...
    if aclresult != ACLCHECK_OK {
        backend_catalog_aclchk::aclcheck_error(
            aclresult,
            OBJECT_SCHEMA,
            backend_utils_cache_lsyscache::namespace_range_index_pubsub::get_namespace_name(mcx, otherNamespace)?
...
fn NameListToString(mcx: Mcx<'_>, names: &[String]) -> PgResult<String> {
    ...
    Ok(backend_catalog_namespace::NameListToString(mcx, &nl)?
        .as_str()
        .to_string())
}
    ...
    if let Some(tuple) = tup {
        ...
        if isDelete && OidIsValid(form.oprcom) {
            ...
            if OidIsValid(form.oprcom) {
                let thirdop = backend_utils_cache_lsyscache::opfamily_operator::get_opname(mcx, form.oprcom)?;
                let third = thirdop
                    .map(|s| s.as_str().to_string())
    ...
    if let Some(tuple) = tup {
        ...
        if isDelete && OidIsValid(form.oprnegate) {
            ...
            if OidIsValid(form.oprnegate) {
                let thirdop = backend_utils_cache_lsyscache::opfamily_operator::get_opname(mcx, form.oprnegate)?;
                let third = thirdop
                    .map(|s| s.as_str().to_string())
...
pub fn makeOperatorDependencies(
     ...
     */
    if isUpdate {
        backend_catalog_pg_depend::deleteDependencyRecordsFor(myself.classId, myself.objectId, true)?;
        backend_catalog_pg_shdepend::deleteSharedDependencyRecordsFor(
            myself.classId,
            myself.objectId,
            0,
        )?;
    ...
    /* Dependency on owner */
    backend_catalog_pg_shdepend::recordDependencyOnOwner(OperatorRelationId, oper.oid, oper.oprowner)?;

    /* Dependency on extension */
    if makeExtensionDep {
        backend_catalog_pg_depend::recordDependencyOnCurrentExtension(mcx, &myself, isUpdate)?;
    }
...
pub fn init_seams() {
    use backend_catalog_pg_operator_seams as seams;
