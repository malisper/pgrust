...
//!     `as_*` codec on it.
//!   * The relation is the real `types_rel::Relation<'mcx>` returned by
...
use backend_access_table_tableam::table_slot_create;
use backend_access_table_tableam_seams as tableam;
...
use backend_utils_cache_lsyscache::opfamily_operator::get_opcode;
use backend_parser_parse_oper::get_sort_group_operators;
use backend_utils_fmgr_fmgr_seams as fmgr;
use backend_utils_sort_sortsupport::PrepareSortSupportFromOrderingOp;
use backend_utils_sort_sortsupport_seams::apply_sort_comparator;

use backend_access_common_detoast_seams as detoast;
use backend_access_common_heaptuple::nocachegetattr;
use backend_access_common_next::tupconvert::{convert_tuples_by_name, execute_attr_map_tuple};
use backend_access_common_tupdesc_seams::equal_row_types;
...
use backend_utils_activity_small_seams as progress;
...
struct AnlIndexData<'mcx> {
    indexInfo: types_nodes::execnodes::IndexInfo<'mcx>,
...
fn do_analyze_rel<'mcx>(
    ...
    let save_nestlevel = backend_utils_misc_guc::NewGUCNestLevel();
    RestrictSearchPath()?;
    ...
    if nindexes > 0 {
        for ind in 0..nindexes {
            ...
            if has_exprs && va_cols.is_empty() {
                let exprs: Vec<types_nodes::primnodes::Expr> = thisdata
                    .indexInfo
                    .ii_Expressions
                    .as_ref()
    ...
    if !inh {
        ...
        if relkind_has_storage(onerel.rd_rel.relkind) {
            let (av, af) =
                backend_access_heap_visibilitymap::visibilitymap_count(onerel)?;
            relallvisible = av;
            relallfrozen = af;
        }
    ...
    backend_utils_init_miscinit::SetUserIdAndSecContext(save_userid, save_sec_context);
...
fn compute_index_stats<'mcx>(
    ...
    for thisdata in indexdata.iter_mut() {
        ...
        let predicate_src: Option<Vec<types_nodes::primnodes::Expr>> = thisdata
            .indexInfo
            .ii_Predicate
            .as_ref()
...
fn examine_attribute<'mcx>(
    ...
    onerel: &Relation<'mcx>,
    attnum: i32,
    index_expr: Option<&types_nodes::primnodes::Expr>,
...
fn acquire_sample_rows<'mcx>(
    ...
    let oldest_xmin =
        backend_storage_ipc_procarray_seams::get_oldest_non_removable_transaction_id::call(
            onerel.rd_id,
        )?;

    // Prepare for sampling block numbers.
    let randseed = pg_prng::global_prng(pg_prng::PgPrng::next_u32);
...
fn update_attstats<'mcx>(
    ...
    for stats in vacattrstats.iter() {
        ...
        if let Some(oldtup) = oldtup {
            let mut stup = backend_access_common_heaptuple::heap_modify_tuple(
                mcx,
                &oldtup,
                &sd.rd_att,
                &values,
...
fn RelationGetNumberOfBlocks(rel: &Relation<'_>) -> PgResult<BlockNumber> {
    backend_access_heap_hio_seams::relation_get_number_of_blocks::call(rel.rd_id)
}
...
fn vac_update_relstats(
    ...
    backend_access_heap_vacuumlazy_seams::vac_update_relstats::call(
...
fn vacuum_delay_point() -> PgResult<()> {
    backend_access_heap_vacuumlazy_seams::vacuum_delay_point::call(true)
}
...
fn AtEOXact_GUC(is_commit: bool, nestlevel: i32) -> PgResult<()> {
    backend_commands_vacuum_seams::at_eoxact_guc::call(is_commit, nestlevel)
}
...
fn varsize_any_datum(value: &Datum<'_>) -> usize {
    match value {
        Datum::ByRef(b) => backend_access_common_heaptuple::varsize_any(b),
