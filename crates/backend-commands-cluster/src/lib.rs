...
//! [`types_rel::Relation`] handles (opened through the table/index owner's
...
pub fn cluster(
    ...
    for opt in &stmt.params {
        if opt.defname == "verbose" {
            verbose = backend_commands_define_seams::def_get_boolean::call(
                opt.defname.clone(),
...
) -> backend_commands_define_seams::DefElemArg {
    use backend_commands_define_seams::DefElemArg as D;
    match a {
...
fn types_nodes_drop_restrict() -> types_nodes::parsenodes::DropBehavior {
