...
seam_core::seam!(
    ...
    /// `options` is the parser's `List *` of `DefElem`s, opaque to the driver.
    /// `Err` carries the option-validation `ereport(ERROR)`s.
    pub fn process_copy_options<'mcx>(
        mcx: Mcx<'mcx>,
        pstate: Option<&types_nodes::copy_query::ParseState<'mcx>>,
        is_from: bool,
...
seam_core::seam!(
    ...
    /// order. `Err` carries the "column does not exist" / "specified more than
    /// once" `ereport(ERROR)`s. The result list is allocated in `mcx`.
    pub fn copy_get_attnums<'mcx>(
        mcx: Mcx<'mcx>,
        tup_desc: &TupleDesc<'mcx>,
        rel: Option<&Relation<'mcx>>,
        attnamelist: Option<&[mcx::PgString<'mcx>]>,
