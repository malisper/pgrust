...
use types_nodes::parsenodes::{OBJECT_DATABASE, OBJECT_TABLESPACE};
...
use types_wal::{RM_DBASE_ID, XLR_SPECIAL_REL_UPDATE};
...
use backend_catalog_pg_tablespace_seams as tscat;
use backend_catalog_storage_seams as storage;
use backend_commands_collationcmds_seams as coll;
...
fn CreateDatabaseUsingWalLog(
    ...
    let srcpath = backend_common_relpath::GetDatabasePath(src_dboid, src_tsid);
    let dstpath = backend_common_relpath::GetDatabasePath(dst_dboid, dst_tsid);
    ...
    backend_utils_cache_relmapper::RelationMapCopy(dst_dboid, dst_tsid, &srcpath, &dstpath)?;
    ...
    for relinfo in rlocatorlist.iter() {
        ...
        backend_storage_lmgr_lmgr::LockRelationId(&srcrelid, AccessShareLock)?;
        backend_storage_lmgr_lmgr::LockRelationId(&dstrelid, AccessShareLock)?;
        ...
        backend_storage_lmgr_lmgr::UnlockRelationId(&srcrelid, AccessShareLock)?;
        backend_storage_lmgr_lmgr::UnlockRelationId(&dstrelid, AccessShareLock)?;
    }
...
fn CreateDirAndVersionFile(dbpath: &str, dbid: Oid, tsid: Oid, isRedo: bool) -> PgResult<()> {
    ...
    if !isRedo {
        ...
        backend_access_transam_xloginsert::XLogRegisterData(&xlrec)?;
        backend_access_transam_xloginsert::XLogInsert(RM_DBASE_ID, XLOG_DBASE_CREATE_WAL_LOG)?;
        // END_CRIT_SECTION();
    }
...
fn CreateDatabaseUsingFileCopy(
    ...
    if !backend_commands_tablespace_globals_seams::IsBinaryUpgrade::call()? {
        backend_postmaster_checkpointer::RequestCheckpoint(
    ...
    for srctablespace in oids {
        ...
        let srcpath = backend_common_relpath::GetDatabasePath(src_dboid, srctablespace);

        match backend_storage_file_fd_seams::stat_file::call(&srcpath, true)? {
            Some(st) if st.isdir && !backend_commands_tablespace::directory_is_empty(&srcpath)? => {}
            _ => {
        ...
        let dstpath = backend_common_relpath::GetDatabasePath(dst_dboid, dsttablespace);

        // Copy this subdirectory to the new location (no subdirectories).
        backend_storage_file_copydir::copydir(&srcpath, &dstpath, false)?;
        ...
        backend_access_transam_xloginsert::XLogBeginInsert()?;
        backend_access_transam_xloginsert::XLogRegisterData(&xlrec)?;
        backend_access_transam_xloginsert::XLogInsert(
            RM_DBASE_ID,
...
pub fn createdb<'mcx>(pstate: &ParseState<'mcx>, stmt: &CreatedbStmt<'mcx>) -> PgResult<Oid> {
        ...
        match def_name(defel) {
            ...
            "oid" => {
                ...
                if dboid < FirstNormalObjectId
                    ...
                    && !backend_commands_tablespace_globals_seams::IsBinaryUpgrade::call()?
                {
    ...
    if let Some(el) = encoding_el {
        if defel_has_arg(el) {
            if arg_is_integer(el) {
                ...
                let encoding_name =
                    common_extra_encnames::pg_encoding_to_char(encoding);
                if encoding_name.is_empty()
                    || common_extra_encnames::pg_valid_server_encoding(encoding_name) < 0
                {
                    return Err(ereport(ERROR)
                ...
                encoding = common_extra_encnames::pg_valid_server_encoding(&encoding_name);
                if encoding < 0 {
                    return Err(ereport(ERROR)
                        .errcode(ERRCODE_UNDEFINED_OBJECT)
    ...
    let datdba = match &dbowner {
        Some(owner) => backend_utils_adt_acl::role_membership::get_role_oid(owner, false)?,
    ...
    if !src_istemplate
        && !backend_catalog_aclchk::object_ownercheck(
            mcx,
            DatabaseRelationId,
            src_dboid,
            backend_utils_init_miscinit::GetUserId(),
    ...
    let (ok, canon) =
        backend_utils_adt_pg_locale::setup::check_locale(libc::LC_COLLATE, &dbcollate)?;
    if !ok {
    ...
    let (ok, canon) =
        backend_utils_adt_pg_locale::setup::check_locale(libc::LC_CTYPE, &dbctype)?;
    if !ok {
    ...
    if src_collversion.is_some() && collversion_el.is_none() {
        ...
        if actual_versionstr != scv {
            let quoted = backend_utils_adt_ruleutils_seams::quote_identifier::call(
                mcx,
                &dbtemplate_name,
            )?;
            return Err(ereport(ERROR)
    ...
    if let Some(el) = tablespacename_el {
        if defel_has_arg(el) {
            ...
            let resolved =
                backend_commands_tablespace::get_tablespace_oid(mcx, &tablespacename, false)?;
            // Check permissions.
            ...
            if aclresult != AclResult::AclcheckOk {
                backend_catalog_aclchk::aclcheck_error(
                    aclresult,
                    OBJECT_TABLESPACE,
            ...
            if resolved != src_deftablespace {
                let srcpath = backend_common_relpath::GetDatabasePath(src_dboid, resolved);
                if let Some(st) = backend_storage_file_fd_seams::stat_file::call(&srcpath, true)? {
                    if st.isdir && !backend_commands_tablespace::directory_is_empty(&srcpath)? {
                        return Err(ereport(ERROR)
                            .errcode(ERRCODE_FEATURE_NOT_SUPPORTED)
    ...
    let (busy, notherbackends, npreparedxacts) =
        backend_storage_ipc_procarray::visibility_lookup::CountOtherDBBackends(src_dboid)?;
    if busy {
        return Err(ereport(ERROR)
    ...
    if OidIsValid(dboid) {
        ...
        loop {
            dboid = backend_catalog_catalog::GetNewOidWithIndex(
                &pg_database_rel,
                DatabaseOidIndexId,
    ...
    backend_catalog_pg_shdepend::recordDependencyOnOwner(DatabaseRelationId, dboid, datdba)?;

    // Create pg_shdepend entries for objects within database.
    backend_catalog_pg_shdepend::copyTemplateDependencies(src_dboid, dboid)?;
...
fn coll_actual_version(mcx: Mcx<'_>, provider: i8, locale: &str) -> PgResult<Option<String>> {
    Ok(
        backend_utils_adt_pg_locale_seams::get_collation_actual_version::call(
            mcx, provider, locale,
        )?
...
pub fn check_encoding_locale_matches(
    ...
    let _ = mcx;
    let ctype_encoding =
        backend_utils_adt_pg_locale_env_seams::pg_get_encoding_from_locale::call(ctype)?;
    let collate_encoding =
        backend_utils_adt_pg_locale_env_seams::pg_get_encoding_from_locale::call(collate)?;

    let superuser = backend_utils_misc_superuser_seams::superuser::call()?;
        ...
        || (encoding == PG_SQL_ASCII && superuser))
    {
        return Err(ereport(ERROR)
            ...
            .errdetail(format!(
                ...
                common_extra_encnames::pg_encoding_to_char(ctype_encoding)
            ))
        ...
        || (encoding == PG_SQL_ASCII && superuser))
    {
        return Err(ereport(ERROR)
            ...
            .errdetail(format!(
                ...
                common_extra_encnames::pg_encoding_to_char(collate_encoding)
            ))
...
fn createdb_failure_callback(
    ...
    if strategy == CreateDBStrategy::WalLog {
        backend_storage_buffer_bufmgr_seams::drop_database_buffers::call(dest_dboid)?;
        backend_storage_smgr_md::ForgetDatabaseSyncRequests(dest_dboid)?;
...
pub fn dropdb(mcx: Mcx<'_>, dbname: &str, missing_ok: bool, force: bool) -> PgResult<()> {
    ...
    // Permission checks.
    if !backend_catalog_aclchk::object_ownercheck(
        mcx,
        DatabaseRelationId,
        db_id,
        backend_utils_init_miscinit::GetUserId(),
    ...
    // DROP hook.
    backend_catalog_objectaccess::invoke_object_drop_hook(DatabaseRelationId, db_id, 0, 0)?;
    ...
    if db_id == backend_commands_tablespace_globals_seams::MyDatabaseId::call()? {
        return Err(ereport(ERROR)
            .errcode(ERRCODE_OBJECT_IN_USE)
    ...
    let nsubscriptions =
        backend_catalog_pg_subscription_seams::count_db_subscriptions::call(db_id)?;
    if nsubscriptions > 0 {
        return Err(ereport(ERROR)
            .errcode(ERRCODE_OBJECT_IN_USE)
            .errmsg(format!(
    ...
    if force {
        backend_storage_ipc_procarray::visibility_lookup::TerminateOtherDBBackends(mcx, db_id)?;
    }

    // Check for other backends in the target database.
    let (busy, notherbackends, npreparedxacts) =
        backend_storage_ipc_procarray::visibility_lookup::CountOtherDBBackends(db_id)?;
    if busy {
        return Err(ereport(ERROR)
            .errcode(ERRCODE_OBJECT_IN_USE)
    ...
    backend_commands_comment::DeleteSharedComments(db_id, DatabaseRelationId)?;
    backend_commands_seclabel::DeleteSharedSecurityLabel(mcx, db_id, DatabaseRelationId)?;

    // Remove settings.
    backend_catalog_pg_db_role_setting::DropSetting(mcx, db_id, InvalidOid)?;

    // Remove shared dependency references.
    backend_catalog_pg_shdepend::dropDatabaseDependencies(db_id)?;
...
pub fn RenameDatabase(mcx: Mcx<'_>, oldname: &str, newname: &str) -> PgResult<ObjectAddress> {
    ...
    let my_db = backend_commands_tablespace_globals_seams::MyDatabaseId::call()?;
    ...
    backend_catalog_objectaccess::invoke_object_post_alter_hook(
        DatabaseRelationId,
        db_id,
        0,
        InvalidOid,
...
fn movedb(mcx: Mcx<'_>, dbname: &str, tblspcname: &str) -> PgResult<()> {
    ...
    // Get tablespace OID.
    let dst_tblspcoid =
        backend_commands_tablespace::get_tablespace_oid(mcx, tblspcname, false)?;
    ...
    let src_dbpath = backend_common_relpath::GetDatabasePath(db_id, src_tblspcoid);
    let dst_dbpath = backend_common_relpath::GetDatabasePath(db_id, dst_tblspcoid);
    ...
    let dstdir = backend_storage_file_fd::allocated_desc::AllocateDir(&dst_dbpath)?;
    if dstdir.is_some() {
        while let Some(ent) = backend_storage_file_fd::allocated_desc::ReadDir(dstdir, &dst_dbpath)? {
        ...
        backend_storage_file_fd::allocated_desc::FreeDir(dstdir)?;

        // The directory exists but is empty; remove it before copydir.
        if let Err(errno) = backend_storage_file_fd_seams::rmdir::call(&dst_dbpath) {
            return Err(ereport(ERROR)
    ...
    let body = (|| -> PgResult<()> {
        ...
        backend_storage_file_copydir::copydir(&src_dbpath, &dst_dbpath, false)?;
    ...
    backend_utils_time_snapmgr::PopActiveSnapshot()?;
    backend_access_transam_xact::CommitTransactionCommand()?;

    // Start a new transaction for the remaining work.
    backend_access_transam_xact::StartTransactionCommand()?;

    // Remove files from the old tablespace.
    if !backend_storage_file_fd_seams::rmtree::call(&src_dbpath, true) {
        ereport(WARNING)
            .errmsg(format!(
    ...
    backend_access_transam_xloginsert::XLogBeginInsert()?;
    backend_access_transam_xloginsert::XLogRegisterData(&buf)?;
    backend_access_transam_xloginsert::XLogInsert(
        RM_DBASE_ID,
...
fn movedb_failure_callback(dest_dboid: Oid, dest_tsoid: Oid) -> PgResult<()> {
    let dstpath = backend_common_relpath::GetDatabasePath(dest_dboid, dest_tsoid);
    let _ = backend_storage_file_fd_seams::rmtree::call(&dstpath, true);
    Ok(())
}
...
pub fn AlterDatabaseSet<'mcx>(stmt: &AlterDatabaseSetStmt<'mcx>) -> PgResult<Oid> {
    ...
    backend_catalog_pg_shdepend::shdepLockAndCheckObject(DatabaseRelationId, datid)?;
    ...
    backend_catalog_pg_db_role_setting_seams::alter_database_setting::call(
        mcx,
        datid,
        InvalidOid,
        setstmt,
...
fn remove_dbtablespaces(mcx: Mcx<'_>, db_id: Oid) -> PgResult<()> {
    ...
    for dsttablespace in oids {
        ...
        let dstpath = backend_common_relpath::GetDatabasePath(db_id, dsttablespace);

        match backend_storage_file_fd_seams::lstat_file::call(&dstpath, true)? {
            Some(st) if st.isdir => {}
            _ => continue,
        }

        if !backend_storage_file_fd_seams::rmtree::call(&dstpath, true) {
            ereport(WARNING)
                .errmsg(format!(
    ...
    backend_access_transam_xloginsert::XLogRegisterData(&oidbuf)?;
    backend_access_transam_xloginsert::XLogInsert(
        RM_DBASE_ID,
        XLOG_DBASE_DROP | XLR_SPECIAL_REL_UPDATE,
    )?;
...
fn check_db_file_conflict(mcx: Mcx<'_>, db_id: Oid) -> PgResult<bool> {
    ...
    for dsttablespace in oids {
        ...
        let dstpath = backend_common_relpath::GetDatabasePath(db_id, dsttablespace);
        if backend_storage_file_fd_seams::lstat_file::call(&dstpath, true)?.is_some() {
