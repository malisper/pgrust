...
pub fn ExplainPrintPlan<'es, 'p>(
    ...
    //   if (es->verbose && queryDesc->plannedstmt->queryId != 0 &&
    //       compute_query_id != COMPUTE_QUERY_ID_REGRESS)
    //       ExplainPropertyInteger("Query Identifier", NULL, queryId, es);
    if es.verbose
        && pstmt.queryId != 0
        && backend_utils_misc_guc_tables::vars::compute_query_id.read()
            != backend_utils_misc_guc_tables::consts::COMPUTE_QUERY_ID_REGRESS
    {
        fmt::ExplainPropertyInteger("Query Identifier", None, pstmt.queryId, es)?;
    }
    Ok(())
}
