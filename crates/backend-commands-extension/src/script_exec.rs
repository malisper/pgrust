...
use types_nodes::nodes::T_TransactionStmt;
use types_nodes::parsestmt::ProcessUtilityContext;
...
pub fn read_extension_script_file(
    ...
    let _ = backend_utils_mb_mbutils::pg_verify_mbstr(src_encoding, src_bytes, false)?;
    ...
    let dest = backend_utils_mb_mbutils::pg_any_to_server(scratch.mcx(), src_bytes, src_encoding)?;
...
pub fn execute_sql_string(sql: &str, _filename: &str) -> PgResult<()> {
    ...
    let raw_parsetree_list = backend_tcop_postgres::simple_query::pg_parse_query(mcx, sql_ref)?;
    ...
    for parsetree in raw_parsetree_list.iter() {
        ...
        let query_list = backend_tcop_postgres::simple_query::pg_analyze_and_rewrite_fixedparams(
            mcx, parsetree, sql_ref, &[],
        )?;

        // stmt_list = pg_plan_queries(stmt_list, sql, CURSOR_OPT_PARALLEL_OK, NULL);
        let stmt_list = backend_tcop_postgres::simple_query::pg_plan_queries(
            mcx,
            query_list,
            sql_ref,
...
fn run_executor(
    ...
    let mut qdesc =
        backend_executor_execMain::CreateQueryDesc(&parent, stmt, sql, snap, None, dest, None, 0)?;

    // ExecutorStart(qdesc, 0);
    backend_executor_execMain::ExecutorStart(&mut qdesc, 0)?;
    // ExecutorRun(qdesc, ForwardScanDirection, 0); (count==0 ⇒ all rows)
    backend_executor_execMain::ExecutorRun(&mut qdesc, types_scan::sdir::ForwardScanDirection, 0)?;
    // ExecutorFinish(qdesc);
    backend_executor_execMain::ExecutorFinish(&mut qdesc)?;
    // ExecutorEnd(qdesc);
    backend_executor_execMain::ExecutorEnd(&mut qdesc)?;
    // FreeQueryDesc(qdesc);
    backend_executor_execMain::FreeQueryDesc(qdesc)?;
...
pub fn execute_extension_script(
    ...
    if control.superuser && !backend_utils_misc_superuser_seams::superuser::call()? {
    ...
    if switch_to_superuser {
        let (uid, sec) = backend_utils_init_miscinit::GetUserIdAndSecContext();
        save_userid = uid;
    ...
    if guc_below_warning("client_min_messages") {
        backend_utils_misc_guc::live::set_config_option_global(
            ...
            backend_utils_misc_guc::GUC_ACTION_SAVE,
            true,
    ...
    if backend_utils_misc_guc::live::get_bool("check_function_bodies").unwrap_or(true) {
        backend_utils_misc_guc::live::set_config_option_global(
    ...
    backend_utils_misc_guc::at_eoxact_guc(true, save_nestlevel);
...
fn guc_below_warning(name: &str) -> bool {
    match backend_utils_misc_guc::live::get_enum(name) {
...
fn quote_identifier_cstr(mcx: Mcx<'_>, s: &str) -> PgResult<String> {
    Ok(backend_utils_adt_ruleutils::quote_identifier(mcx, s)?
        .as_str()
        .to_string())
}
...
fn run_script_body(
    ...
    // backslash-echo line. (REG_ADVANCED is the default base cflags.)
    {
        ...
        let out = backend_utils_adt_varlena_seams::replace_text_regexp::call(
            scratch.mcx(),
            &t_sql,
            br"^\\echo.*$",
            b"",
            types_regex::REG_ADVANCED | types_regex::REG_NEWLINE,
            C_COLLATION_OID,
            0, // search_start
    ...
    if c_sql.contains("@extowner@") {
        ...
        let user_name = match backend_utils_init_miscinit::GetUserNameFromId(mcx, uid, false)? {
...
pub fn ApplyExtensionUpdates(
    ...
    use backend_access_common_heaptuple::{heap_copytuple, heap_deform_tuple, heap_modify_tuple};
    ...
    for version_name in update_versions {
        ...
        values[cat::Anum_pg_extension_extversion as usize - 1] =
            backend_utils_adt_varlena_seams::cstring_to_text_v::call(mcx, version_name)?;
