...
use backend_nodes_core::makefuncs::make_index_info;
...
pub fn CheckIndexCompatible<'mcx>(
    ...
    attribute_list: &[&types_nodes::ddlnodes::IndexElem<'mcx>],
    exclusion_op_names: Option<&mcx::PgVec<'mcx, types_nodes::nodes::NodePtr<'mcx>>>,
...
pub fn check_index_compatible_stmt<'mcx>(
    mcx: Mcx<'mcx>,
    old_id: Oid,
    stmt: &types_nodes::ddlnodes::IndexStmt<'mcx>,
) -> PgResult<bool> {
    ...
    let att_list: Vec<&types_nodes::ddlnodes::IndexElem<'mcx>> = stmt
        .indexParams
        .iter()
