...
//! (`utility.c`) dispatches — `types_nodes::ddlnodes::{CreateOpClassStmt,
//! CreateOpFamilyStmt, AlterOpFamilyStmt}` — into the flattened
//! `types_opclass` images the in-crate command logic consumes.
//!
...
//! `types_opclass` structs name (`String`, `TypeName`, `ObjectWithArgs`,
//! `CreateOpClassItem`), so a mismatch is an internal error.

use backend_utils_error::ereport;
use types_error::{PgResult, ERROR};
use types_nodes::ddlnodes as pnode;
use types_nodes::nodes::Node;
use types_nodes::rawnodes::TypeName as PTypeName;
use types_opclass::{
...
/// (a qualified name).
fn string_list(nodes: &[types_nodes::nodes::NodePtr<'_>], ctx: &str) -> PgResult<Vec<StringNode>> {
    nodes.iter().map(|n| string_node(n, ctx)).collect()
}

/// Flatten a parse-node `TypeName` (`rawnodes::TypeName`) into the
/// `types_opclass::TypeName` the type-resolver seams consume. `names` is the
...
fn type_name_list(
    nodes: &[types_nodes::nodes::NodePtr<'_>],
    ctx: &str,
