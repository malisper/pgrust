...
use backend_catalog_pg_shdepend_seams::record_dependency_on_owner;
use backend_commands_amcmds_seams::get_index_am_oid;
use backend_commands_event_trigger_seams::{
    event_trigger_collect_alter_opfam, event_trigger_collect_create_opclass,
    event_trigger_collect_simple_command,
};
use backend_parser_parse_func_seams::lookup_func_with_args;
use backend_parser_parse_oper_seams::{lookup_oper_name, lookup_oper_with_args};
...
fn OpClassCacheLookup(
    ...
    let opc: Option<Oid> = if let Some(schemaname) = schemaname {
        ...
        if !OidIsValid(namespaceId) {
            ...
            let oid = backend_utils_cache_syscache_seams::get_opclass_oid::call(
...
fn CreateOpFamily(
    ...
    /* dependency on owner */
    record_dependency_on_owner::call(OPERATOR_FAMILY_RELATION_ID, opfamilyoid, owner)?;
...
pub fn DefineOpClass(mcx: Mcx<'_>, stmt: &CreateOpClassStmt) -> PgResult<ObjectAddress> {
    ...
    /* dependency on owner */
    record_dependency_on_owner::call(OPERATOR_CLASS_RELATION_ID, opclassoid, owner)?;
...
fn get_namespace_name_for_acl(mcx: Mcx<'_>, nspid: Oid) -> PgResult<Option<String>> {
    Ok(
        backend_utils_cache_lsyscache_seams::get_namespace_name::call(mcx, nspid)?
            .map(|s| s.as_str().to_string()),
    )
}
