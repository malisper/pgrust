...
use backend_commands_define::{defGetBoolean, defGetQualifiedName, defGetTypeName};
use backend_parser_parse_oper::op_signature_string;

use backend_catalog_aclchk_seams::{aclcheck_error, object_aclcheck, object_ownercheck};
use backend_catalog_pg_operator_seams::{
...
use backend_commands_functioncmds_seams::{lookup_func_name, name_list_to_string};
use backend_parser_parse_type_seams::typename_type_id;
use backend_utils_adt_format_type_seams::format_type_be_owned;
use backend_utils_cache_lsyscache_seams::{get_element_type, get_func_rettype, get_namespace_name};
use backend_utils_init_miscinit_seams::get_user_id;
use backend_utils_misc_superuser_seams::superuser;
...
use types_nodes::parsenodes::{OBJECT_FUNCTION, OBJECT_OPERATOR, OBJECT_SCHEMA, OBJECT_TYPE};
use types_parsenodes::{AlterOperatorStmt, DefElem, Node, TypeName};
...
/// Convert the raw-parser `TypeName` (`types_parsenodes`, returned by
/// `defGetTypeName`) into the resolver-facing `TypeName` (`types_opclass`) that
...
pub fn AlterOperator(stmt: &AlterOperatorStmt) -> PgResult<ObjectAddress> {
    ...
    let oprId = backend_parser_parse_oper_seams::lookup_oper_with_args_node::call(opername, false)?;
