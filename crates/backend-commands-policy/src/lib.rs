...
use types_nodes::ddlnodes::{AlterPolicyStmt, CreatePolicyStmt, RoleSpec};
...
use backend_catalog_pg_shdepend::{deleteSharedDependencyRecordsFor, recordSharedDependencyOn};
use backend_catalog_dependency::recordDependencyOnExpr;
use backend_parser_relation::{addNSItemToQuery, addRangeTableEntryForRelation};
use backend_utils_adt_acl::role_membership::get_rolespec_oid;
...
fn RangeVarCallbackForPolicy(rv: &RangeVar, relid: Oid, _oldrelid: Oid) -> PgResult<()> {
    ...
    if !aclchk_seams::object_ownercheck::call(RELATION_RELATION_ID, relid, GetUserId())? {
        let objtype = backend_catalog_objectaddress_seams::get_relkind_objtype::call(relkind);
...
pub fn CreatePolicy<'mcx>(mcx: Mcx<'mcx>, stmt: &CreatePolicyStmt<'mcx>) -> PgResult<ObjectAddress> {
    ...
    backend_utils_cache_inval::cache_invalidate::CacheInvalidateRelcache(&target_table)?;
...
///
/// `stmt` is the `types_parsenodes::RenameStmt` shape the ALTER dispatcher
...
fn to_parsenodes_rolespec<'mcx>(
    mcx: Mcx<'mcx>,
    spec: &RoleSpec<'mcx>,
) -> PgResult<types_nodes::parsenodes::RoleSpec<'mcx>> {
    Ok(types_nodes::parsenodes::RoleSpec {
