...
use backend_catalog_pg_depend::{deleteDependencyRecordsFor, recordDependencyOnCurrentExtension};
...
fn name_list_strings<'mcx>(
    mcx: Mcx<'mcx>,
    names: &[types_nodes::nodes::NodePtr<'_>],
...
/// nodes.)
fn name_list_to_string(names: &[types_nodes::nodes::NodePtr<'_>]) -> String {
...
pub fn CreateProceduralLanguage<'mcx>(
    ...
    let languageOwner: Oid = backend_utils_init_miscinit_seams::get_user_id::call();
    let handlerOid: Oid;
    let inlineOid: Oid;
    let valOid: Oid;
    let funcrettype: Oid;
    ...
    handlerOid =
        backend_parser_parse_func_seams::lookup_func_name::call(&plhandler_names, 0, &[], false)?;
    // funcrettype = get_func_rettype(handlerOid);
    funcrettype = backend_utils_cache_lsyscache_seams::get_func_rettype::call(handlerOid)?;
    ...
    if !stmt.plinline.is_empty() {
        ...
        inlineOid = backend_parser_parse_func_seams::lookup_func_name::call(
            &names,
            1,
            &[INTERNALOID],
            false,
    ...
    if !stmt.plvalidator.is_empty() {
        ...
        valOid =
            backend_parser_parse_func_seams::lookup_func_name::call(&names, 1, &[OIDOID], false)?;
        /* return value is ignored, so we don't check the type */
    } else {
        valOid = InvalidOid;
    }
    ...
    let rel = backend_access_table_table::table_open(mcx, LanguageRelationId, RowExclusiveLock)?;
    ...
    let oldtup = backend_utils_cache_syscache_seams::language_tuple_by_name::call(mcx, languageName)?;
    ...
    if let Some((oldtuple, oldform)) = oldtup {
        ...
        backend_catalog_indexing_seams::catalog_tuple_update_pg_language::call(
        ...
        langoid =
            backend_catalog_indexing_seams::catalog_tuple_insert_pg_language::call(mcx, &rel, &row)?;
        is_update = false;
    }
...
pub fn get_language_oid(langname: &str, missing_ok: bool) -> PgResult<Oid> {
    ...
    let oid = backend_utils_cache_syscache_seams::language_oid_by_name::call(langname)?;
