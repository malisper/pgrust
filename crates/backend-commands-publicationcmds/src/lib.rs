...
use backend_catalog_dependency::performDeletion;
use backend_catalog_indexing::keystone::{CatalogTupleDelete, CatalogTupleInsert, CatalogTupleUpdate};
use backend_catalog_pg_shdepend::{changeDependencyOnOwner, recordDependencyOnOwner};
use backend_commands_define_seams::DefElemArg;
use backend_nodes_core::nodefuncs::check_functions_in_node;
use backend_nodes_core::node_walker::expression_tree_walker;
...
use backend_utils_cache_syscache::{ReleaseSysCache, SearchSysCache1, SearchSysCache2, SysCacheGetAttr};
...
use backend_catalog_pg_publication_seams as pubcat;
    ...
    for pubobj_node in pubobjspec_list {
        ...
        match pubobj.pubobjtype {
            ...
            PUBLICATIONOBJ_TABLES_IN_SCHEMA => {
                ...
                let schemaid = backend_catalog_namespace::get_namespace_oid(name, false)?;
                list_append_unique_oid(schemas, schemaid);
            }
            PUBLICATIONOBJ_TABLES_IN_CUR_SCHEMA => {
                /* nothing valid in search_path? */
                let search_path = backend_catalog_namespace::fetch_search_path(mcx, false)?;
...
fn check_simple_rowfilter_expr_walker(node: &Node, err: &RefCell<Option<PgError>>) -> bool {
    ...
    if let Some(expr) = node.as_expr() {
        ...
        if errdetail_msg.is_none() {
            let typid = match backend_nodes_nodeFuncs_seams::expr_type_info::call(expr) {
                Ok(info) => info.typid,
                Err(e) => {
                    *err.borrow_mut() = Some(e);
                    return true;
...
fn to_access_rangevar(rv: &types_nodes::rawnodes::RangeVar<'_>) -> types_tuple::access::RangeVar {
...
fn OpenTableList<'mcx>(
    ...
    for t in tables {
        ...
        let rel = {
            ...
            backend_access_table_table::table_openrv(mcx, &rv, ShareUpdateExclusiveLock)?
        };
        ...
        if list_member_oid(&relids, myrelid) {
            ...
            backend_access_table_table::table_close(rel, ShareUpdateExclusiveLock)?;
            continue;
        }
        ...
        if recurse && !is_partitioned {
            ...
            for childrelid in children.iter().copied() {
                ...
                let crel = backend_access_table_table::table_open(mcx, childrelid, NoLock)?;
...
fn CloseTableList<'mcx>(rels: Vec<PublicationRelInfo<'mcx>>) -> PgResult<()> {
    for pub_rel in rels {
        backend_access_table_table::table_close(pub_rel.relation, NoLock)?;
    }
    Ok(())
}
...
fn LockSchemaList<'mcx>(mcx: Mcx<'mcx>, schemalist: &[Oid]) -> PgResult<()> {
    for &schemaid in schemalist {
        /* Allow query cancel in case this takes a long time */
        check_for_interrupts()?;
        backend_storage_lmgr_lmgr::LockDatabaseObject(
...
pub fn CreatePublication<'mcx>(
    ...
    let rel = backend_access_table_table::table_open(mcx, PublicationRelationId, RowExclusiveLock)?;
    ...
    if xlog_seams::wal_level::call() != types_wal::xlog_consts::WalLevel::Logical {
        ereport(WARNING)
...
fn AlterPublicationTables<'mcx>(
    ...
    if action == AP_AddObjects {
        ...
        for oldrelid in oldrelids {
            ...
            if !found {
                let oldrel = backend_access_table_table::table_open(
                    mcx,
                    oldrelid,
                    ShareUpdateExclusiveLock,
                )?;
...
pub fn RemovePublicationRelById<'mcx>(mcx: Mcx<'mcx>, proid: Oid) -> PgResult<()> {
    let rel =
        backend_access_table_table::table_open(mcx, PublicationRelRelationId, RowExclusiveLock)?;
...
fn relation_get_replident_attrs<'mcx>(
    ...
    let bms = backend_utils_cache_relcache_seams::relation_get_index_attr_bitmap::call(
        mcx,
        relation,
        types_relcache_entry::IndexAttrBitmapKind::Identity,
    )?;
    Ok(bitmapset_members(bms.as_deref()))
}

/// `tupdesc->attrs[attnum-1].attgenerated` for an opened relation.
fn tupdesc_attgenerated<'mcx>(relation: &types_rel::Relation<'mcx>, attnum: i16) -> i8 {
    if attnum < 1 {
        return 0;
    }
...
fn relation_has_generated_stored<'mcx>(relation: &types_rel::Relation<'mcx>) -> bool {
...
fn relation_has_generated_virtual<'mcx>(relation: &types_rel::Relation<'mcx>) -> bool {
    let desc = &relation.rd_att;
    for i in 0..(desc.natts as usize) {
        let att = desc.attr(i);
...
fn bitmapset_members(bms: Option<&types_nodes::bitmapset::Bitmapset<'_>>) -> Vec<i32> {
    let mut out = Vec::new();
    let mut i = -1;
    loop {
