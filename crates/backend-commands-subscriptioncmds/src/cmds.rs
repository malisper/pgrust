...
use backend_catalog_namespace::RangeVarGetRelid;
use backend_catalog_objectaccess::invoke_object_drop_hook;
use backend_catalog_pg_shdepend::{deleteSharedDependencyRecordsFor, recordDependencyOnOwner};
use backend_replication_libpqwalreceiver::libpqrcv_check_conninfo;
use backend_replication_logical_origin::{
    replorigin_by_name, replorigin_create, replorigin_drop_by_name, replorigin_get_progress,
};
use backend_storage_lmgr_lmgr::LockSharedObject;
use backend_utils_cache_syscache::cacheinfo::SUBSCRIPTIONNAME;
use backend_utils_cache_syscache::{ReleaseSysCache, SearchSysCache2};
...
use backend_commands_event_trigger_seams as evttrig_seams;
...
use backend_tcop_utility_out_seams as utility_seams;
...
fn replication_slot_validate_name(name: &str) -> PgResult<()> {
    match backend_replication_slot_seams::replication_slot_validate_name_internal::call(name) {
        Ok(()) => Ok(()),
...
fn pg_lsn_in_str(s: &str) -> PgResult<u64> {
    backend_utils_adt_lsn_trigfuncs::pg_lsn::pg_lsn_in(s, None)
}
...
fn publication_list_to_array<'mcx>(
    ...
    let image = backend_catalog_objectaddress::fmgr_sql::strlist_to_textarray(mcx, &list)?;
...
pub fn CreateSubscription<'mcx>(
    ...
    // (backend_replication_libpqwalreceiver), so no dynamic load is needed.
    ...
    backend_utils_activity_pgstat_subscription::pgstat_create_subscription(subid)?;
...
pub fn AlterSubscription<'mcx>(
    ...
    let sub = match backend_catalog_pg_subscription_seams::get_subscription::call(mcx, subid, false)? {
        Some(s) => s,
        None => {
            ReleaseSysCache(tup);
            backend_access_table_table::table_close(rel, RowExclusiveLock)?;
...
fn alter_fail<'mcx>(
    ...
    let _ = mcx;
    ReleaseSysCache(tup);
    let _ = backend_access_table_table::table_close(rel, RowExclusiveLock);
    Err(e)
}
...
pub fn DropSubscription<'mcx>(
    ...
    let rstates = backend_catalog_pg_subscription_seams::get_subscription_relations::call(mcx, subid, true)?;
    ...
    backend_catalog_pg_subscription_seams::remove_subscription_rel::call(subid, InvalidOid)?;
    ...
    backend_utils_activity_pgstat_subscription::pgstat_drop_subscription(subid)?;
...
fn _connect_true_leg_surface(mcx: Mcx<'_>) {
    ...
    let _ = |elems: &[types_datum::datum::Datum]| {
