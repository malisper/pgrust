...
fn RangeVarCallbackForAttachIndex(
    ...
    // to do.
    let relkind = match backend_utils_cache_syscache_seams::rel_relkind::call(rel_oid)? {
        Some(k) => k,
        None => return Ok(()),
    };
...
fn refuseDupeIndexAttach(
    ...
    let existing_idx =
        backend_catalog_partition::index_get_partition(partition_tbl, parent_idx.rd_id)?;
...
fn verifyPartitionIndexNotNull(
    iinfo: &types_nodes::execnodes::IndexInfo<'_>,
...
pub(crate) fn ATExecAttachPartitionIdx<'mcx>(
    ...
    cmd: &types_nodes::ddlnodes::PartitionCmd<'mcx>,
    ...
    if curr_parent != parent_idx.rd_id {
        ...
        let part_desc =
            backend_partitioning_partdesc::RelationGetPartitionDesc(mcx, &parent_tbl, true)?;
        let mut found = false;
        ...
        let child_info = backend_catalog_index::BuildIndexInfo(mcx, &part_idx)?;
        let parent_info = backend_catalog_index::BuildIndexInfo(mcx, parent_idx)?;
        ...
        if OidIsValid(constraint_oid) {
            cld_constr_id = backend_catalog_pg_constraint::get_relation_idx_constraint_oid(
                part_tbl.rd_id,
                part_idx_id,
            )?;
        ...
        // All good -- do it.
        backend_commands_indexcmds::IndexSetParentIndex(mcx, &part_idx, parent_idx.rd_id)?;
        if OidIsValid(constraint_oid) {
...
fn validatePartitionedIndex<'mcx>(
    ...
    let children =
        backend_catalog_pg_inherits::find_inheritance_children(mcx, parted_idx.rd_id, NoLock)?;
    let mut tuples = 0i32;
    ...
    let part_desc = backend_partitioning_partdesc::RelationGetPartitionDesc(mcx, parted_tbl, true)?;
