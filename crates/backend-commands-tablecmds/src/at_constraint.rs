...
pub fn ATPrepAddPrimaryKey<'mcx>(
    ...
    for column_node in pkconstr.keys.iter() {
        ...
        let tuple =
            backend_catalog_pg_constraint::findNotNullConstraint(mcx, relid, colname)?;
        if let Some(tuple) = tuple {
            backend_catalog_pg_constraint::verifyNotNullPKCompatible(mcx, &tuple, colname)?;
            ...
            for &childrelid in children.iter() {
                let tup = backend_catalog_pg_constraint::findNotNullConstraint(
                    mcx, childrelid, colname,
                )?;
                match tup {
                    None => {
                        let childname = backend_utils_cache_lsyscache_seams::get_rel_name::call(
                            mcx, childrelid,
                        )?
                    ...
                    Some(tup) => {
                        backend_catalog_pg_constraint::verifyNotNullPKCompatible(
                            mcx, &tup, colname,
                        )?;
                    }
                }
...
pub fn at_verify_not_null<'mcx>(mcx: Mcx<'mcx>, relid: Oid) -> PgResult<()> {
    ...
    let result: PgResult<()> = (|| {
        let mut scan = backend_access_table_tableam_seams::table_beginscan::call(
