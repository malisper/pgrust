...
use types_nodes::ddlnodes::{AlterTableCmd, AlterTableType};
...
use backend_access_table_table::{table_open, table_close};
...
use backend_utils_adt_acl_seams as acl_seam;
use backend_storage_lmgr_lmgr::{LockRelationOid, ConditionalLockRelationOid};
...
pub fn AlterTableMoveAll<'mcx>(
    mcx: Mcx<'mcx>,
    stmt: &types_nodes::ddlnodes::AlterTableMoveAllStmt<'mcx>,
) -> PgResult<Oid> {
    ...
    while let Some(tup) = genam::systable_getnext::call(mcx, scan.desc_mut())? {
        ...
        if !aclchk_seam::object_ownercheck::call(RelationRelationId, relOid, GetUserId())? {
            ...
            let objtype = backend_catalog_objectaddress::resolve::get_relkind_objtype(actual_relkind);
    ...
    for &relOid in relations.iter() {
        ...
        backend_tcop_utility_out_seams::event_trigger_alter_table_start::call(&stmt_node)?;
