...
use backend_commands_tablecmds_seams as me;
...
fn oid_search_key(value: Oid) -> types_cache::SysCacheKey<'static> {
    types_cache::SysCacheKey::Value(types_datum::Datum::from_oid(value))
}
...
pub fn ATExecChangeOwner<'mcx>(
    ...
    if relowner != new_owner_id {
        ...
        if !recursing && !miscinit_seam::superuser::call(mcx)? {
            ...
            backend_utils_adt_acl_seams::check_can_set_role::call(GetUserId(), new_owner_id)?;
...
fn change_owner_fix_column_acls<'mcx>(
    ...
    old_owner_id: Oid,
    new_owner_id: Oid,
    target_rel: &types_rel::Relation<'mcx>,
) -> PgResult<()> {
