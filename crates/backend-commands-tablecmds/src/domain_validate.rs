...
use backend_catalog_objectaddress::consts::{RelationRelationId, TypeRelationId};
...
use backend_executor_execTuples::slot_deform::slot_getattr;
use backend_utils_cache_lsyscache_seams::get_typtype;
...
pub fn validate_domain_not_null_constraint<'mcx>(mcx: Mcx<'mcx>, domainoid: Oid) -> PgResult<()> {
    ...
    for rtc in rels.into_iter() {
        ...
        let snap_rc = backend_utils_time_snapmgr_seams::get_active_snapshot::call()?
            .expect("ALTER DOMAIN NOT NULL validate scan with no active snapshot");

        let rel_alias = testrel.alias();

        let mut scan =
            backend_access_table_tableam_seams::table_beginscan::call(mcx, &rel_alias, snap_rc)?;
        let mut slot = backend_access_table_tableam::table_slot_create(mcx, &rel_alias)?;

        // while (table_scan_getnextslot(scan, ForwardScanDirection, slot))
        while backend_access_table_tableam_seams::table_scan_getnextslot::call(
...
pub fn validate_domain_check_constraint<'mcx>(
    ...
    let cnode = backend_nodes_read_seams::string_to_node::call(mcx, ccbin)?;
    ...
    let mut estate = backend_executor_execExpr_seams::create_executor_state::call(mcx)?;

    // build execution state for expr (ExecPrepareExpr runs expression_planner).
    let mut exprstate =
        backend_executor_execExpr_seams::exec_prepare_expr::call(&expr, &mut estate)?;
...
fn get_rels_with_domain<'mcx>(
    ...
    let domain_type_name =
        backend_utils_adt_format_type_seams::format_type_be::call(mcx, domain_oid)?
            .as_str()
            .to_string();
