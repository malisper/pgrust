...
pub fn define_sequence_relation<'mcx>(
    ...
    table_elts.push(alloc_in(mcx, Node::ColumnDef(last_value))?);
    ...
    table_elts.push(alloc_in(mcx, Node::ColumnDef(log_cnt))?);
    ...
    table_elts.push(alloc_in(mcx, Node::ColumnDef(is_called))?);
