...
use types_parsenodes::{AlterObjectSchemaStmt, Node, RenameStmt};
...
use backend_catalog_pg_constraint_seams as pgcon_seam;
use backend_catalog_pg_depend_seams as pgdepend_seam;
...
use backend_catalog_pg_constraint::{AlterConstraintNamespaces, RenameConstraintById};
use backend_utils_cache_relcache::derived::RelationGetIndexList;
...
fn has_inheritance_children(mcx: Mcx<'_>, relid: Oid) -> PgResult<bool> {
    Ok(!backend_catalog_pg_inherits::find_inheritance_children(mcx, relid, NoLock)?.is_empty())
}
...
fn rename_constraint_internal<'mcx>(
    ...
    let Some(con_tup) = backend_utils_cache_syscache::SearchSysCache1(
        mcx,
        backend_utils_cache_syscache::CONSTROID,
        types_cache::SysCacheKey::Value(types_datum::Datum::from_oid(constraint_oid)),
    )?
    else {
...
pub fn AlterRelationNamespaceInternal<'mcx>(
    mcx: Mcx<'mcx>,
    class_rel: &types_rel::Relation<'mcx>,
    rel_oid: Oid,
    old_nsp_oid: Oid,
    new_nsp_oid: Oid,
