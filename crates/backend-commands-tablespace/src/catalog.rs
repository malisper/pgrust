...
use backend_catalog_pg_tablespace_seams::{self as cat, TablespaceTuple};
...
use backend_access_heap_heapam::scan::heap_getnext;
use backend_catalog_indexing::keystone::{
    CatalogTupleDelete, CatalogTupleInsert, CatalogTupleUpdate,
};
use backend_access_common_reloptions_seams as reloptions_seam;
...
fn defel_arg(def: &DefElem<'_>) -> PgResult<Option<backend_commands_define_seams::DefElemArg>> {
...
fn defel_type_name_to_string(tn: &types_nodes::rawnodes::TypeName<'_>) -> PgResult<String> {
    ...
    for (i, name) in tn.names.iter().enumerate() {
        ...
        let node: &types_nodes::nodes::Node = name;
        match node.node_tag() {
...
/// `transformRelOptions` takes its own `DefElem` view + `types_datum` Datum, a
...
fn transform_options_bytes<'mcx, 'a>(
    ...
    if let Some(old) = old_options {
        let olds = backend_utils_adt_arrayfuncs_seams::deconstruct_text_array::call(mcx, old)?;
