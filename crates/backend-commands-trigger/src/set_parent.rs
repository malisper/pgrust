...
use backend_catalog_objectaddress::consts::{RelationRelationId, TriggerRelationId};
...
pub fn TriggerSetParentTrigger<'mcx>(
    mcx: Mcx<'mcx>,
    trigrel: &types_rel::Relation<'mcx>,
    child_trig_id: Oid,
    parent_trig_id: Oid,
    child_table_id: Oid,
) -> PgResult<()> {
    ...
    if OidIsValid(parent_trig_id) {
        ...
        backend_catalog_pg_depend::deleteDependencyRecordsForClass(
            TriggerRelationId,
            child_trig_id,
            TriggerRelationId,
