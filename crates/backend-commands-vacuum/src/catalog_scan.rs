...
use backend_commands_vacuum_seams::{
...
fn scan_all_pg_class<'caller>(caller_mcx: Mcx<'caller>) -> PgResult<Vec<PgClassScanRow<'caller>>> {
    ...
    while let Some(tup) = genam::systable_getnext::call(mcx, scan.desc_mut())? {
        ...
        let class_form = types_rel::FormData_pg_class {
...
fn fixed_attr_offset(tupdesc: &types_tuple::heaptuple::TupleDescData<'_>, anum: i16) -> Option<usize> {
    use backend_utils_adt_arrayfuncs::foundation::att_align_nominal;
    let mut off: usize = 0;
...
fn vac_update_relstats_apply(relation: Oid, args: UpdateRelStatsArgs) -> PgResult<RelStatsApplyResult> {
    ...
    let has_rules = !args.in_outer_xact && {
        backend_utils_cache_relcache_seams::relation_rules::call(mcx, relid)?.is_some()
    };
    let has_triggers = !args.in_outer_xact && {
        let r = backend_access_common_relation_seams::relation_open::call(mcx, relid, NoLock)?;
...
pub(crate) fn install() {
    backend_commands_vacuum_seams::scan_pg_class_frozenids::set(scan_pg_class_frozenids);
    backend_commands_vacuum_seams::scan_pg_database_frozenids::set(scan_pg_database_frozenids);
    backend_commands_vacuum_seams::scan_all_pg_class::set(scan_all_pg_class);
    backend_commands_vacuum_seams::vac_update_relstats_apply::set(vac_update_relstats_apply);
    backend_commands_vacuum_seams::vac_update_datfrozenxid_apply::set(vac_update_datfrozenxid_apply);
}
