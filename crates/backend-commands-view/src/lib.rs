...
//! ([`backend_nodes_core::nodefuncs`]), `makeColumnDef` / `makeDefElem`
//! ([`backend_nodes_core::makefuncs`]), `type_is_collatable` /
//! `get_collation_name` ([`backend_utils_cache_lsyscache`]),
//! `format_type_with_typemod` ([`backend_utils_adt_format_type`]),
//! `RangeVarGetAndCheckCreationNamespace` ([`backend_catalog_namespace`]),
//! `recordDependencyOnCurrentExtension` ([`backend_catalog_pg_depend`]),
//! `parse_analyze_fixedparams` ([`backend_parser_analyze`]),
//! `DefineQueryRewrite` ([`backend_rewrite_rewriteDefine`]),
//! `isQueryUsingTempRelation` ([`backend_parser_relation`]),
//! `CheckTableNotInUse` ([`backend_commands_tablecmds_seams`]),
//! `relation_open` / `relation_close` ([`backend_access_common_relation`]), and
//! `CommandCounterIncrement` ([`backend_access_transam_xact`]).
//!
//! The genuine cross-subsystem externals whose owners are not yet ported cross
//! the [`backend_commands_view_seams`] outward seams: `DefineRelation`,
...
use backend_catalog_namespace::RangeVarGetAndCheckCreationNamespace;
use backend_catalog_pg_depend::recordDependencyOnCurrentExtension;
use backend_nodes_core::makefuncs::make_column_def;
use backend_nodes_core::nodefuncs::{expr_collation, expr_type, expr_typmod};
use backend_parser_analyze::parse_analyze_fixedparams;
use backend_parser_relation::isQueryUsingTempRelation;
use backend_rewrite_rewriteDefine::DefineQueryRewrite;
...
fn make_check_option_defelem<'mcx>(
    ...
    let string_arg = Node::String(StringNode {
    ...
    alloc_in(mcx, Node::DefElem(defelem))
}
