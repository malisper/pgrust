use super::*;
use types_wchar::encoding::{PG_EUC_KR, PG_UTF8};
...
fn ascii_passes_through() {
    install_check_seam();
    let r = utf8_to_euc_kr(PG_UTF8, PG_EUC_KR, b"ascii", false).unwrap();
    assert_eq!(&*r.bytes, b"ascii");
    assert_eq!(r.converted, 5);
    let back = euc_kr_to_utf8(PG_EUC_KR, PG_UTF8, b"ascii", false).unwrap();
...
fn multibyte_round_trips() {
    ...
    let utf8: &[u8] = &[237, 149, 156, 234, 181, 173];
    let local = utf8_to_euc_kr(PG_UTF8, PG_EUC_KR, utf8, false).unwrap();
    assert_eq!(local.converted, utf8.len() as i32);
    assert!(!local.bytes.is_empty());
    let back = euc_kr_to_utf8(PG_EUC_KR, PG_UTF8, &local.bytes, false).unwrap();
...
fn wrong_source_encoding_is_rejected() {
    install_check_seam();
    assert!(utf8_to_euc_kr(PG_EUC_KR, PG_EUC_KR, b"x", false).is_err());
}
