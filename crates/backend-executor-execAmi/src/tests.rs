...
use types_nodes::execnodes::ExprContext;
use types_nodes::instrument::Instrumentation;
use types_nodes::nodeindexscan::CUSTOMPATH_SUPPORT_BACKWARD_SCAN;
use types_nodes::pathnodes::{
    AppendPath, CustomPath, GroupResultPath, IndexOptInfo, IndexPath, MergeAppendPath,
    MinMaxAggPath, PathData, ProjectionPath,
};
use types_nodes::{Material, MaterialState, TupleTableSlot};
...
fn exec_re_scan_walks_params_and_dispatches() {
    ...
    let splan_plan = types_nodes::nodes::Node::mk_material(mcx, splan_plan);
    let child_plan = types_nodes::nodes::Node::mk_material(mcx, Material::default());
...
fn supports_backward_scan() {
    ...
    let mat = types_nodes::nodes::Node::mk_material(mcx, Material::default());
    ...
    let parallel = types_nodes::nodes::Node::mk_material(mcx, parallel);
