...
fn exec_re_scan_walks_params_and_dispatches() {
    ...
    child.ss.ps.plan =
        Some(alloc_in(mcx, types_nodes::nodes::Node::Material(Material::default())).unwrap());
...
fn supports_mark_restore_matches_c_table() {
    ...
    // default: false.
    assert!(!exec_supports_mark_restore(&PathNode::Path(PathData {
        pathtype: types_core::NodeTag(9999)
    })));
}
...
fn supports_backward_scan() {
    ...
    let mat = types_nodes::nodes::Node::Material(Material::default());
    ...
    let parallel = types_nodes::nodes::Node::Material(parallel);
