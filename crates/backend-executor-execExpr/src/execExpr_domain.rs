...
use types_cache::typcache::{DomainCtxHandle, ExprStateHandle};
...
pub fn exec_init_expr(check_expr: &Expr, execctx: DomainCtxHandle) -> PgResult<ExprStateHandle> {
    ...
    let bundle = McxOwned::<DomainExprBundleTy>::try_new(
        ...
        |mcx| {
            ...
            let econtext =
                backend_executor_execUtils_seams::create_standalone_expr_context::call(mcx)?;
...
pub fn domain_check_exec(
    ...
    DOMAIN_EXPRSTATES.with(|m| {
        ...
        bundle.with_mut(|b| {
            ...
            backend_executor_execUtils_seams::reset_expr_context::call(&mut b.estate, b.econtext)?;
