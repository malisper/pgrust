...
//! `void *` became the [`ExecPlanLink`](types_nodes::ExecPlanLink) index — the
//! leader's *shared-per-CTE* state is hoisted out of the leader node into
//! [`EState.es_cte_shared[cteParam]`](types_nodes::execnodes::EStateData::es_cte_shared)
...
use types_nodes::execnodes::CteSharedState;
use types_nodes::nodectescan::{CteScan, CteScanState};
use types_nodes::{EStateData, PlanStateNode, SlotId, Tuplestorestate};

use backend_executor_execMain_seams as seams;
use backend_executor_execTuples_seams as execTuples;
use backend_utils_sort_storage::tuplestore;
