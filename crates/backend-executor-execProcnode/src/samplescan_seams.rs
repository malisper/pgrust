...
use types_nodes::execnodes::{EPQState, EStateData};
use types_nodes::{PlanStateNode, SlotId};
use types_samplescan::SampleScanState;
...
fn exec_sample_scan_node<'mcx>(
    ...
    let node = match pstate {
        PlanStateNode::SampleScan(s) => {
            types_nodes::samplescanstate_carrier::downcast_sample_scan_state_mut::<
                SampleScanState<'mcx>,
            >(&mut **s)
    ...
    let have = backend_executor_nodeSamplescan::ExecSampleScan(node, estate)?;
    if !have {
        return Ok(None);
    }
    Ok(node
...
pub fn init_seams() {
    ...
    seam::exec_init_scan_tuple_slot::set(|samplestate, estate| {
        ...
        let tts_ops = backend_access_table_tableam::table_slot_callbacks(rel);
    ...
    seam::exec_init_expr_list::set(|samplestate, node, estate| {
        let mcx = estate.es_query_cxt;
        let nodes: Vec<Option<&types_nodes::primnodes::Expr>> = node
            .tablesample
            .as_deref()
    ...
    seam::exec_eval_arg_in_per_tuple_context::set(|samplestate, i, is_null, estate| {
        ...
        *is_null = null;
        ...
        // across as the bare `types_datum::Datum` the sampler consumes.
        Ok(types_datum::datum::Datum::from_usize(datum.as_usize()))
    });
    ...
    seam::exec_eval_repeatable_in_per_tuple_context::set(|samplestate, is_null, estate| {
        ...
        *is_null = null;
        ...
        // the bare `types_datum::Datum` (later fed to hashfloat8).
        Ok(types_datum::datum::Datum::from_usize(datum.as_usize()))
    });
    ...
    seam::pg_prng_uint32_global::set(|| Ok(pg_prng::global_prng(|prng| prng.next_u32())));
    ...
    seam::hashfloat8::set(|datum| {
        ...
        Ok(backend_access_hashfunc::hashfloat8(key))
    });
    ...
    seam::table_beginscan_sampling::set(|samplestate, allow_sync, estate| {
        ...
        let scandesc = backend_access_table_tableam::table_beginscan_sampling(
            mcx,
            rel,
            snapshot,
            0,
    ...
    seam::table_rescan_set_params::set(|samplestate, allow_sync, estate| {
        ...
        backend_access_table_tableam::table_rescan_set_params(
            mcx,
            scandesc,
            None,
            use_bulkread,
    ...
    seam::table_endscan::set(|samplestate| {
        if let Some(scandesc) = samplestate.ss_currentScanDesc.take() {
            backend_access_table_tableam::table_endscan(scandesc)?;
        }
        Ok(())
    });
    ...
    seam::table_scan_sample_next_block::set(|samplestate, estate| {
        ...
        let res = backend_access_table_tableam::table_scan_sample_next_block(
            mcx,
            &mut scandesc,
            samplestate,
        );
    ...
    seam::table_scan_sample_next_tuple::set(|samplestate, estate| {
        ...
        let res = {
            ...
            backend_access_table_tableam::table_scan_sample_next_tuple(
                mcx,
                &mut scandesc,
                samplestate,
                slot,
    ...
    seam::epq_param_is_member_of_ext_param::set(|node, estate| {
        ...
        let ext_param = plan
            ...
            .extParam
            .as_deref();
        Ok(backend_nodes_core_seams::bms_is_member::call(
            epq_param, ext_param,
        ))
    });
    ...
    seam::eval_plan_qual_fetch_row_mark::set(|node, scanrelid, estate| {
        ...
        backend_executor_execMain_seams::eval_plan_qual_fetch_row_mark::call(estate, scanrelid, slot)
    });
}
