...
fn exec_tidrange_scan_node<'mcx>(
    ...
    let have = backend_executor_nodeTidrangescan::ExecTidRangeScan(node, estate)?;
    if !have {
        return Ok(None);
    }
    Ok(node
...
fn operand_expr<'a, 'mcx>(
    ...
    match side {
        types_tidrange::OperandSide::Left => op.args.first(),
        types_tidrange::OperandSide::Right => op.args.get(1),
    }
}
...
pub fn init_seams() {
    ...
    seam::table_beginscan_tidrange::set(|node, estate| {
        ...
        let scandesc = backend_access_table_tableam::table_beginscan_tidrange(
    ...
    seam::table_rescan_tidrange::set(|node, estate| {
        ...
        backend_access_table_tableam::table_rescan_tidrange(mcx, scandesc, &mintid, &maxtid)
    });
    ...
    seam::table_scan_getnextslot_tidrange::set(|node, estate| {
        ...
        backend_access_table_tableam::table_scan_getnextslot_tidrange(
            mcx,
            scandesc,
            direction,
