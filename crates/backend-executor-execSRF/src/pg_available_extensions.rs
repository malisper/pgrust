...
//! [`backend_commands_extension`]. This module is the executor-frame adapter:
...
fn pg_available_extensions<'mcx>(
    ...
    let rows = backend_commands_extension::pg_available_extensions()?;
...
fn pg_available_extension_versions<'mcx>(
    ...
    let rows = backend_commands_extension::pg_available_extension_versions()?;
    ...
    for row in &rows {
        ...
        if row.requires.is_empty() {
            ...
            values[6] = backend_commands_extension::convert_requires_to_datum(mcx, &row.requires)?;
        }
        // [7] comment — text / NULL
        match &row.comment {
