...
//! [`backend_utils_adt_mcxtfuncs::pg_get_backend_memory_contexts_core`]. That
...
//!    ([`backend_utils_mmgr_mcxt_seams::top_memory_context`]) via
...
use backend_utils_adt_mcxtfuncs_seams::{
...
const PG_GET_BACKEND_MEMORY_CONTEXTS_COLS: usize =
    backend_utils_adt_mcxtfuncs::PG_GET_BACKEND_MEMORY_CONTEXTS_COLS;
...
fn put_mcxt_row(row: McxtRow) -> PgResult<()> {
    MCXT_SINK.with(|cell| {
        ...
        let path_img = backend_utils_adt_array_more_seams::construct_int4_array::call(&row.path)?;
...
fn pg_get_backend_memory_contexts<'mcx>(
    ...
    let res = backend_utils_adt_mcxtfuncs::pg_get_backend_memory_contexts_core();
