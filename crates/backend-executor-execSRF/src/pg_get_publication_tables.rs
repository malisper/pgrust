...
//! [`backend_catalog_pg_publication`] and exposed via the
...
fn pg_get_publication_tables<'mcx>(
    ...
    let pubnames: alloc::vec::Vec<alloc::string::String> = {
        ...
        let strs = backend_utils_adt_arrayfuncs::construct::text_array_to_strings_bytes(mcx, image)?;
    ...
    let rows = backend_catalog_pg_publication_seams::build_publication_table_rows::call(
        mcx,
        &pubname_refs,
    )?;
