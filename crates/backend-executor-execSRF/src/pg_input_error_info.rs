...
//! [`backend_utils_adt_misc::pg_input_error_info`]. Here we only marshal the two
...
fn pg_input_error_info<'mcx>(fcinfo: &mut FunctionCallInfoBaseData<'mcx>) -> Datum<'mcx> {
    ...
    for (k, col) in cols.iter().enumerate() {
        if let Some(bytes) = col {
            ...
            values[k] = backend_utils_adt_varlena_seams::cstring_to_text_v::call(mcx, s)
                .expect("pg_input_error_info: cstring_to_text");
            isnull[k] = false;
        }
    }

    let formed = backend_access_common_heaptuple::heap_form_tuple(mcx, &tupdesc, &values, &isnull)
