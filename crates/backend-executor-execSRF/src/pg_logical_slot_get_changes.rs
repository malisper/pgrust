...
//! decode core itself
//! ([`backend_replication_logical_logicalfuncs::run_changes_into_collector`]) is
...
fn pg_logical_slot_get_changes_guts<'mcx>(
    ...
    let user_id = backend_utils_misc_backendstate_pc_seams::get_user_id::call()?;
    slot::CheckSlotPermissions(mcx, user_id)?;

    // CheckLogicalDecodingRequirements();
    let wal_level = types_logical::WalLevel(backend_access_transam_xlog_seams::wal_level::call() as i32);
    backend_replication_logical_logical::CheckLogicalDecodingRequirements(wal_level, my_database_id)?;
    ...
    let options: Vec<(String, Option<String>)> = {
        ...
        let strs =
            backend_utils_adt_arrayfuncs::construct::text_array_to_strings_bytes(mcx, image)?;
    ...
    let decode_result: PgResult<()> = (|| {
        ...
        let end_of_wal = if !backend_access_transam_xlog_seams::recovery_in_progress::call() {
            backend_access_transam_xlog_seams::get_flush_rec_ptr::call().0
        } else {
            backend_access_transam_xlog_seams::get_xlog_replay_rec_ptr::call()
        };
        ...
        backend_replication_logical_logicalfuncs::run_changes_into_collector(
            upto_lsn,
            upto_nchanges,
            options,
            confirm,
    ...
    for (lsn, xid, data) in logical_seams::sql_srf_take_rows() {
        ...
        let data_datum = backend_utils_adt_varlena_seams::bytes_to_varlena_v::call(mcx, &data)?;
        let values = [
            Datum::from_u64(lsn),
