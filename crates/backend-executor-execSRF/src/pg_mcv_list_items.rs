...
//!
//! The MCV deserialize core lives in [`backend_statistics_mcv`]; here it is
...
fn pg_stats_ext_mcvlist_items<'mcx>(
    ...
    let mcvlist = backend_statistics_mcv::statext_mcv_deserialize(mcx, Some(&image))?;
    ...
    for d in 0..ndim {
        ...
        let (outfunc, _isvarlena) =
            backend_utils_cache_lsyscache_seams::get_type_output_info::call(mcvlist.types[d])?;
    ...
    for (call_cntr, item) in mcvlist.items.iter().enumerate() {
        ...
        for d in 0..ndim {
            ...
            if !item.isnull[d] {
                ...
                let cstr = backend_utils_fmgr_fmgr_seams::oid_output_function_call_datum::call(
                    mcx,
                    out_funcs[d],
                    item.values[d].clone(),
                )?;
                let txt = backend_utils_adt_varlena_seams::cstring_to_text_v::call(
                    mcx,
                    cstr.as_str(),
                )?;
        ...
        let v_lbs = [1];
        let values_arr = backend_utils_adt_arrayfuncs::construct::construct_md_array_values(
            mcx,
            &value_elems,
            Some(&value_nulls),
            v_ndims,
        ...
        let n_lbs = [1];
        let nulls_arr = backend_utils_adt_arrayfuncs::construct::construct_md_array_values(
            mcx,
            &null_elems,
            None,
            n_ndims,
