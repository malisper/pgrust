...
use backend_utils_activity_status::{
    ...
    for curr_backend in 1..=num_backends {
        ...
        if has_pgstat_permissions(beentry.st_userid)? {
            ...
            let mut proc_no: Option<types_core::ProcNumber> =
                backend_storage_ipc_procarray_seams::backend_pid_get_proc_role::call(
                    beentry.st_procpid,
                )
                .map(|(_role, procno)| procno);
            if proc_no.is_none() && beentry.st_backend_type != BackendType::Backend {
                proc_no = backend_storage_lmgr_proc_seams::auxiliary_pid_get_proc::call(
                    beentry.st_procpid,
                );
            }
            ...
            if let Some(procno) = proc_no {
                ...
                wait_event_type =
                    backend_utils_activity_waitevent::pgstat_get_wait_event_type(raw_wait_event);
                wait_event =
                    backend_utils_activity_waitevent::pgstat_get_wait_event(raw_wait_event)?;

                // leader = proc->lockGroupLeader;
                let leader = backend_storage_lmgr_proc_seams::proc_lock_group_leader::call(procno);
                let leader_pid = if leader != types_core::INVALID_PROC_NUMBER {
                    backend_storage_lmgr_proc_seams::proc_pid::call(leader)
                } else {
                    INVALID_PID
                };
                ...
                if leader != types_core::INVALID_PROC_NUMBER && leader_pid != beentry.st_procpid {
                    ...
                    let lpid = backend_replication_logical_launcher_seams::GetLeaderApplyWorkerPid::call(
                        beentry.st_procpid,
                    )?;
                    if lpid != INVALID_PID {
            ...
            if beentry.st_backend_type == BackendType::BgWorker {
                match backend_postmaster_bgworker::GetBackgroundWorkerTypeByPid(beentry.st_procpid) {
                ...
                values[17] = text_datum(
                    mcx,
                    backend_utils_init_miscinit::GetBackendTypeDesc(beentry.st_backend_type),
                )?;
            }
...
fn numeric_in_datum<'mcx>(mcx: Mcx<'mcx>, s: &str) -> PgResult<Datum<'mcx>> {
    let (var, _endptr) = backend_utils_adt_numeric::io::set_var_from_str(mcx, s, 0)?;
...
fn inet_in_datum<'mcx>(mcx: Mcx<'mcx>, host: &str) -> PgResult<Datum<'mcx>> {
    ...
    let addr = backend_utils_adt_network::inet_in(host.as_bytes(), None)?
...
fn fill_client_addr<'mcx>(
    ...
    if common_ip::sockaddr_is_all_zeros(&beentry.st_clientaddr) {
        nulls[12] = true;
        nulls[13] = true;
        nulls[14] = true;
        return Ok(());
    ...
    let family = common_ip::sockaddr_family(&beentry.st_clientaddr);
    ...
    if family == libc::AF_INET || family == libc::AF_INET6 {
        ...
        if ret == 0 {
            ...
            backend_utils_adt_network::clean_ipv6_addr(family, &mut host_bytes);
