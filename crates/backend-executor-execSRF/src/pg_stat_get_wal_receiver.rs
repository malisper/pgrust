...
//! ([`backend_replication_walreceiver::pg_stat_get_wal_receiver`] →
...
fn pg_stat_get_wal_receiver<'mcx>(
    ...
    let activity = backend_replication_walreceiver::pg_stat_get_wal_receiver()?;
    ...
    let formed =
        backend_access_common_heaptuple::heap_form_tuple(mcx, &tupdesc, &values, &nulls)
