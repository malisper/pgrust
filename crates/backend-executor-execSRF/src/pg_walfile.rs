...
//! WAL-file-name parse) live in [`backend_access_transam_xlogfuncs`]; here we
...
fn pg_walfile_name_offset<'mcx>(
    ...
    let row = backend_access_transam_xlogfuncs::pg_walfile_name_offset(mcx, locationpoint)?;
...
fn pg_split_walfile_name<'mcx>(
    ...
    let row = backend_access_transam_xlogfuncs::pg_split_walfile_name(mcx, &fname)?;
