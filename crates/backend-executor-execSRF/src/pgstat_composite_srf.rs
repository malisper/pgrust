...
fn numeric_from_u64<'mcx>(mcx: Mcx<'mcx>, val: u64) -> PgResult<Datum<'mcx>> {
    let var = backend_utils_adt_numeric::convert::int128_to_numericvar(mcx, val as i128)?;
    let buf = backend_utils_adt_numeric::convert::make_result(mcx, &var)?;
    Ok(Datum::ByRef(buf))
}
...
fn pg_stat_get_archiver<'mcx>(
    ...
    let a = backend_utils_activity_small::pgstat_fetch_stat_archiver()?;
