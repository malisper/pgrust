...
//! [`backend_utils_misc_guc_funcs::pg_settings_rows`] (the `config_generic`
...
use backend_utils_misc_guc_funcs::PgSettingsRow;
...
const NUM_PG_SETTINGS_ATTS: usize = backend_utils_misc_guc_funcs::NUM_PG_SETTINGS_ATTS;
...
fn put_settings_row<'mcx>(
    ...
    match &row.enumvals {
        Some(opts) => {
            ...
            let img =
                backend_utils_adt_array_more_seams::construct_text_array::call(&elems)?;
...
fn show_all_settings<'mcx>(
    ...
    let rows = backend_utils_misc_guc_funcs::pg_settings_rows();
    let rsinfo = fcinfo
        .resultinfo
        .as_mut()
