...
pub fn with_hinstrument_mut<R>(
    ...
    match slot {
        ...
        HashInstrumentSlot::Dsm {
            ...
            let a = align_of::<HashInstrumentation>();
            let off = (h + a - 1) & !(a - 1);
            let elem = types_execparallel::SerializeCursor(
...
fn resolve_retrieve_mcx<'mcx>(_node: types_execparallel::PlanStateHandle) -> Mcx<'mcx> {
    panic!(
...
pub fn init_pq_seams() {
    backend_executor_nodeHash_pq_seams::exec_hash_estimate::set(exec_hash_estimate_shim);
