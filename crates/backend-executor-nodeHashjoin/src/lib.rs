...
pub fn ExecInitHashJoin<'mcx>(
    ...
    execUtils::exec_assign_projection_info::call(estate, &mut hjstate.js.ps)?;
    ...
    let ops = match hjstate.js.ps.lefttree.as_deref() {
        Some(child) => execTuples::exec_get_result_slot_ops::call(child),
...
fn exec_get_result_type_of<'mcx>(
    ...
    match child.as_deref() {
        Some(c) => execTuples::exec_get_result_type::call(mcx, c),
        None => Ok(None),
    }
}
