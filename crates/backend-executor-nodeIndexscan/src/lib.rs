...
use backend_storage_buffer_bufmgr_seams as _bufmgr;
...
use types_nodes::nodebitmapindexscan::IndexArrayKeyInfo;
use types_nodes::nodeindexscan::IndexScan;
use types_nodes::primnodes::Expr;
use types_nodes::{EStateData, EcxtId, PlanStateData, SlotId};
...
pub fn ExecIndexScan<'mcx>(
    ...
    let node = match pstate {
        types_nodes::PlanStateNode::IndexScan(n) => &mut **n,
        _ => return Err(elog("ExecIndexScan dispatched to wrong node type")),
    };
        ...
        match clause {
            ...
            Expr::NullTest(ntest) => {
                ...
                let flags = match ntest.nulltesttype {
                    types_nodes::primnodes::NullTestType::IS_NULL => SK_ISNULL | SK_SEARCHNULL,
...
fn table_slot_callbacks() -> types_nodes::TupleSlotKind {
    types_nodes::TupleSlotKind::BufferHeapTuple
}

/// `node->iss_RelationDesc` alias.
fn index_alias<'mcx>(node: &IndexScanState<'mcx>) -> PgResult<types_rel::Relation<'mcx>> {
...
fn current_rel_alias<'mcx>(node: &IndexScanState<'mcx>) -> PgResult<types_rel::Relation<'mcx>> {
    node.ss
        .ss_currentRelation
        .as_ref()
        .map(|r| r.alias())
...
fn relation_amcanorder(index: &types_rel::Relation<'_>) -> PgResult<bool> {
    relcache::rd_indam_amcanorder::call(index)
}

/// `index->rd_indam->amsearcharray`.
fn relation_amsearcharray(index: &types_rel::Relation<'_>) -> PgResult<bool> {
...
fn exec_init_expr_in<'mcx>(
    ...
    planstate: &mut PlanStateData<'mcx>,
    estate: &mut EStateData<'mcx>,
) -> PgResult<PgBox<'mcx, types_nodes::execexpr::ExprState<'mcx>>> {
...
pub fn init_seams() {
    use backend_executor_nodeIndexscan_seams as sx;
