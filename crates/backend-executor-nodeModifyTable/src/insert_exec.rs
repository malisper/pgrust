...
pub fn ExecInsert<'mcx>(
    ...
    if relhasindex && !indexes_open {
        backend_executor_execIndexing_seams::exec_open_indices::call(
            estate,
            result_rel_info,
            onconflict != types_nodes::modifytable::ONCONFLICT_NONE,
        )?;
    }
    ...
    if relation_trig_insert_instead_row(estate, result_rel_info) {
        ...
        if onconflict != types_nodes::modifytable::ONCONFLICT_NONE
            && estate.result_rel(result_rel_info).ri_NumIndices > 0
        {
            ...
            loop {
                ...
                if !exec_check_index_constraints::call(
                    ...
                    if onconflict == types_nodes::modifytable::ONCONFLICT_UPDATE {
                        ...
                        debug_assert!(onconflict == types_nodes::modifytable::ONCONFLICT_NOTHING);
                        let temp_slot = backend_executor_execMain_seams::exec_get_returning_slot::call(
                            estate,
                            result_rel_info,
                        )?;
    ...
    if estate.result_rel(result_rel_info).ri_has_project_returning {
        ...
        if let Some(cp_deleted) = context.cpDeletedSlot {
            backend_executor_execTuples_seams::exec_materialize_slot::call(
                estate,
