...
use types_nodes::execexpr::{ExprState, SetExprState};
use types_nodes::execnodes::{ExprContext, PlanStateData};
use types_nodes::executor::TupleSlotKind;
use types_nodes::nodeprojectset::ProjectSet as ProjectSetPlan;
use types_nodes::nodes::Node;
use types_nodes::primnodes::{Const, Expr, FuncExpr, OpExpr, TargetEntry};
...
fn funcexpr(funcretset: bool) -> FuncExpr {
    FuncExpr {
        ...
        funcretset,
        funcvariadic: false,
        funcformat: types_nodes::primnodes::CoercionForm::COERCE_EXPLICIT_CALL,
        funccollid: 0,
        inputcollid: 0,
        args: Vec::new(),
    }
...
fn attach_leaf_child<'mcx>(state: &mut ProjectSetState<'mcx>, mcx: Mcx<'mcx>) {
    let mut leaf = types_nodes::noderesult::ResultState::default();
