...
//! [`exec_scan_worktable`](backend_executor_execScan_seams::exec_scan_worktable)
...
use types_nodes::execnodes::{EStateData, RecursiveUnionSharedState};
...
use types_nodes::{SlotId, Tuplestorestate, TupleSlotKind};
...
fn exec_work_table_scan_node<'mcx>(
    ...
    let node = match pstate {
        types_nodes::PlanStateNode::WorkTableScan(node) => node,
...
pub fn publish_wtparam_slot<'mcx>(
    rustate: &mut types_nodes::noderecursiveunion::RecursiveUnionStateData<'mcx>,
    estate: &mut EStateData<'mcx>,
    wt_param: i32,
) -> PgResult<()> {
    let idx = usize::try_from(wt_param)
