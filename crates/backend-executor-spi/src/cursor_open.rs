...
use types_nodes::parsestmt::{CachedPlanHandle as SeamCachedPlanHandle, PlpgsqlExprParseState};
use types_parsenodes::RawParseMode;
use types_portal::{CachedPlanHandle as PortalCachedPlanHandle, FetchDirection, Portal};
...
use types_nodes::portalcmds::{CURSOR_OPT_NO_SCROLL, CURSOR_OPT_SCROLL};
...
fn open_internal(
    ...
    let source_h = types_nodes::parsestmt::CachedPlanSourceHandle(source);
    ...
    // CMDTAG_SELECT. `types_portal::CommandTag` is the bare i32; unwrap the
    // newtype the seam returns.
    let command_tag: types_portal::CommandTag =
    ...
    // the portal copy.
    let stmts: &[types_nodes::nodeindexscan::PlannedStmt] = &stmt_list;
    ...
    debug_assert_ne!(
        ...
        types_portal::PORTAL_MULTI_QUERY
    );
