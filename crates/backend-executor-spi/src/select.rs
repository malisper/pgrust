...
// aliases; the plancache *seams* take the `types_nodes` newtype handles. We
...
fn prepare_oneshot_select<'mcx>(
    ...
    let query_node =
        backend_parser_analyze::parse_analyze_fixedparams(mcx, parsetree, query, &[])?;
    let mut querytree_list: PgVec<'mcx, types_nodes::copy_query::Query<'mcx>> = PgVec::new_in(mcx);
...
fn run_query_tupdesc(query: &str) -> PgResult<Vec<SpiColumn>> {
    ...
    let desc = plancache_seams::plansource_result_desc::call(
        mcx,
        types_nodes::parsestmt::CachedPlanSourceHandle(plansource),
    );
    let _ = snapmgr::pop_active_snapshot::call();
    let _ = plancache::DropCachedPlan(plansource);
