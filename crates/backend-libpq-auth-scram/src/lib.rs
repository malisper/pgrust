...
//!     installed as the inward seam [`backend_libpq_auth_seams::check_scram_sasl_auth`]
...
use common_prng_base64::base64::{pg_b64_dec_len, pg_b64_decode, pg_b64_enc_len, pg_b64_encode};
use common_sha2::PG_SHA256_DIGEST_LENGTH;
use types_control::MOCK_AUTH_NONCE_LEN;
...
use backend_libpq_auth_scram_seams as scram_seams;
...
const PASSWORD_TYPE_SCRAM_SHA_256: i32 = types_authid::PasswordType::ScramSha256 as i32;
...
fn scram_init(
    ...
    if let Some(shadow_pass) = shadow_pass {
        let password_type =
            backend_commands_user_seams::get_password_type::call(shadow_pass.to_string())? as i32;
...
pub fn pg_be_scram_build_secret(password: &[u8]) -> PgResult<String> {
    ...
    if !port_pg_strong_random::pg_strong_random::call(&mut saltbuf) {
        return Err(ereport(ERROR)
            .errcode(ERRCODE_INTERNAL_ERROR)
...
use port_pg_strong_random_seams as port_pg_strong_random;
...
fn build_server_first_message(state: &mut scram_state) -> PgResult<Vec<u8>> {
    ...
    if !port_pg_strong_random::pg_strong_random::call(&mut raw_nonce) {
        return Err(ereport(ERROR)
            .errcode(ERRCODE_INTERNAL_ERROR)
...
fn scram_mock_salt(
    ...
    let ctx = common_cryptohash_seams::pg_cryptohash_create::call(hash_type);
    if ctx.is_null() {
        return Ok(None);
    }
    ...
    let failed = common_cryptohash_seams::pg_cryptohash_init::call(ctx) < 0
        || common_cryptohash_seams::pg_cryptohash_update::call(
            ctx,
        ...
        || common_cryptohash_seams::pg_cryptohash_final::call(
            ctx,
    ...
    if failed {
        common_cryptohash_seams::pg_cryptohash_free::call(ctx);
        return Ok(None);
    }
    common_cryptohash_seams::pg_cryptohash_free::call(ctx);
...
fn check_scram_sasl_auth_impl(
    ...
    backend_libpq_auth::sendAuthRequest(port, backend_libpq_auth::AUTH_REQ_SASL, &sasl_mechs)?;
    ...
    loop {
        backend_libpq_pqcomm::pq_startmsgread()?;
        let mtype = backend_libpq_pqcomm::pq_getbyte()?;
        ...
        let mut buf = types_stringinfo::StringInfo::new_in(mcx.mcx());
        if backend_libpq_pqcomm::pq_getmessage(&mut buf, PG_MAX_SASL_MESSAGE_LENGTH)? != 0 {
        ...
        if initial {
            let selected_mech = backend_libpq_pqformat::pq_getmsgrawstring(&mut buf)?.to_vec();
            ...
            let inputlen = backend_libpq_pqformat::pq_getmsgint(&mut buf, 4)? as i32;
            if inputlen == -1 {
                input = None;
            } else {
                input = Some(
                    backend_libpq_pqformat::pq_getmsgbytes(&mut buf, inputlen as usize)?.to_vec(),
                );
            }
            initial = false;
        } else {
            let inputlen = buf.len();
            input = Some(backend_libpq_pqformat::pq_getmsgbytes(&mut buf, inputlen)?.to_vec());
        }
        backend_libpq_pqformat::pq_getmsgend(&buf)?;
        ...
        if let Some(output) = output {
            ...
            if result == PG_SASL_EXCHANGE_SUCCESS {
                backend_libpq_auth::sendAuthRequest(
                    port,
                    backend_libpq_auth::AUTH_REQ_SASL_FIN,
                    &output,
                )?;
            } else {
                backend_libpq_auth::sendAuthRequest(
                    port,
                    backend_libpq_auth::AUTH_REQ_SASL_CONT,
                    &output,
                )?;
            }
        }
...
/// Seam entry for `backend_libpq_auth_seams::check_scram_sasl_auth`: read the
