...
fn install_mocks() {
    INIT.call_once(|| {
        ...
        port_pg_strong_random::pg_strong_random::set(|buf| {
        ...
        common_cryptohash_seams::pg_cryptohash_create::set(|_t| {
            core::ptr::NonNull::<types_crypto::pg_cryptohash_ctx>::dangling().as_ptr()
        });
        common_cryptohash_seams::pg_cryptohash_init::set(|_c| 0);
        common_cryptohash_seams::pg_cryptohash_update::set(|_c, _d, _l| 0);
        common_cryptohash_seams::pg_cryptohash_final::set(|_c, dest, len| {
        ...
        common_cryptohash_seams::pg_cryptohash_free::set(|_c| {});
        // get_password_type: classify by the scheme prefix.
        backend_commands_user_seams::get_password_type::set(|p| {
            if p.starts_with("SCRAM-SHA-256$") {
                Ok(types_authid::PasswordType::ScramSha256)
            } else if p.starts_with("md5") {
                Ok(types_authid::PasswordType::Md5)
            } else {
                Ok(types_authid::PasswordType::Plaintext)
            }
        });
    });
}
