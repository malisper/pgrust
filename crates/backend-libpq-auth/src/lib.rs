...
use types_net::{clientCertFull, clientCertOff, Port};

pub(crate) use backend_commands_user_seams as user_seams;
pub(crate) use backend_libpq_auth_seams as seams;
...
const PASSWORD_TYPE_MD5: i32 = types_authid::PasswordType::Md5 as i32;
...
pub fn set_authn_id(port: &Port, id: &str) -> PgResult<()> {
    ...
    backend_utils_init_miscinit::set_client_connection_info(Some(id.to_string()), auth_method);
...
pub fn sendAuthRequest(_port: &Port, areq: AuthRequest, extradata: &[u8]) -> PgResult<()> {
    ...
    let mut buf = backend_libpq_pqformat::pq_beginmessage(mcx.mcx(), PqMsg_AuthenticationRequest)?;
    backend_libpq_pqformat::pq_sendint32(&mut buf, areq as u32)?;
    if !extradata.is_empty() {
        backend_libpq_pqformat::pq_sendbytes(&mut buf, extradata)?;
    }
...
pub fn recv_password_packet(_port: &Port) -> PgResult<Option<String>> {
    ...
    if backend_libpq_pqcomm::pq_getmessage(&mut buf, PG_MAX_AUTH_TOKEN_LENGTH)? != 0 {
...
pub fn CheckMD5Auth(port: &Port, shadow_pass: Option<String>) -> PgResult<(i32, Option<String>)> {
    ...
    if !port_pg_strong_random(&mut md5_salt) {
        ereport(LOG)
...
pub fn ClientAuthentication(port: &mut Port) -> PgResult<()> {
    ...
    if seams::log_connection_authentication::call()
        && status == STATUS_OK
        && backend_utils_init_miscinit::client_connection_info().authn_id.is_none()
    {
...
fn reject_arm(port: &Port, explicit_reject: bool) -> PgResult<()> {
    ...
    let user_name = port.user_name.as_deref().unwrap_or("");
    let is_repl = seams_am_walsender() && !backend_replication_walsender_seams::am_db_walsender::call();
...
pub fn CheckCertAuth(port: &Port) -> PgResult<i32> {
    ...
    let peer_username: Option<&str> = match hba.clientcertname {
        types_net::clientCertDN => port.peer_dn.as_deref(),
...
pub(crate) fn port_pg_strong_random(buf: &mut [u8]) -> bool {
    port_pg_strong_random_seams::pg_strong_random::call(buf)
}

/// `am_walsender` (walsender.c global).
fn seams_am_walsender() -> bool {
    backend_replication_walsender_seams::am_walsender::call()
}
...
/// Build a `types_net::SockAddr` from the connection's remote address.
pub(crate) fn raddr_sockaddr(port: &Port) -> types_net::SockAddr {
    types_net::SockAddr { addr: port.raddr.addr, salen: port.raddr.salen }
}
...
pub fn init_seams() {
    backend_libpq_auth_seams::client_authentication::set(client_authentication_entry);
...
fn log_connection_authorization_entry() -> bool {
    ...
    backend_libpq_auth_seams::log_connection_authentication::call()
}
