...
use backend_libpq_be_secure_common_seams as my_seams;
...
pub fn run_ssl_passphrase_command<'mcx>(
    ...
    if pclose_rc == -1 {
        ...
        let reason = common_wait_error_seams::wait_result_to_str::call(pclose_rc);
        report(loglevel, "run_ssl_passphrase_command", |b| {
...
where
    F: FnOnce(backend_utils_error::ErrorBuilder) -> backend_utils_error::ErrorBuilder,
{
