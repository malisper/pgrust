...
//!
//!   * `AUTHNAME` syscache lookup → `backend_utils_cache_syscache_seams::fetch_role_password`,
//!   * `GetCurrentTimestamp` → `backend_utils_adt_timestamp_seams::get_current_timestamp`,
//!   * `pg_md5_encrypt` and the three SCRAM routines → [`backend_libpq_crypt_seams`].
