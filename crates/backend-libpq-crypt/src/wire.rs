...
//!
//!   * `backend_libpq_auth_seams`: `get_role_password`, `md5_crypt_verify`
//!     (consumed by `auth.c`),
//!   * `backend_commands_user_seams`: `encrypt_password`, `plain_crypt_verify`,
...
/// Adapter for `backend_libpq_auth_seams::get_role_password`: the seam returns
...
/// Adapter for `backend_libpq_auth_seams::md5_crypt_verify`: returns
/// `(status, logdetail)`.
...
/// Adapter for `backend_commands_user_seams::encrypt_password`. The C
...
/// Adapter for `backend_commands_user_seams::plain_crypt_verify`.
...
/// Adapter for `backend_commands_user_seams::get_password_type`.
...
pub fn init_seams() {
    backend_libpq_auth_seams::get_role_password::set(seam_get_role_password);
    backend_libpq_auth_seams::md5_crypt_verify::set(seam_md5_crypt_verify);
