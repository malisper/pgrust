...
//!     `UserAuth`, `ConnType`, ...) come from `types_net` (defined by the merged
...
pub(crate) use backend_utils_error::ereport;
use types_error::{
...
pub(crate) use backend_libpq_auth_seams as auth_seams;
pub(crate) use backend_libpq_hba_seams as hba_seams;
...
pub struct IdentLine {
    ...
    pub system_user: types_net::AuthToken,
    /// `AuthToken *pg_user`.
    pub pg_user: types_net::AuthToken,
}
...
pub struct TokenizedAuthLine {
    ...
    pub fields: Vec<Vec<types_net::AuthToken>>,
    /// `char *file_name`.
...
pub(crate) struct CheckNetworkData {
    ...
    pub method: types_net::IPCompareMethod,
...
//
// `AuthToken` is a foreign (`types_net`) type, so these are free functions
...
pub(crate) fn tok_str(t: &types_net::AuthToken) -> &[u8] {
    match &t.string {
        Some(s) => s.as_bytes(),
        None => b"",
    }
...
pub(crate) fn token_has_regexp(t: &types_net::AuthToken) -> bool {
    t.regex.is_some()
}

/// `token_is_member_check(t)` (hba.c:70).
pub(crate) fn token_is_member_check(t: &types_net::AuthToken) -> bool {
    !t.quoted && tok_str(t).first() == Some(&b'+')
}

/// `token_is_keyword(t, k)` (hba.c:71).
pub(crate) fn token_is_keyword(t: &types_net::AuthToken, k: &[u8]) -> bool {
    !t.quoted && tok_str(t) == k
}

/// `token_matches(t, k)` (hba.c:72).
pub(crate) fn token_matches(t: &types_net::AuthToken, k: &[u8]) -> bool {
    tok_str(t) == k
}

/// `token_matches_insensitive(t, k)` (hba.c:73).
pub(crate) fn token_matches_insensitive(t: &types_net::AuthToken, k: &[u8]) -> bool {
...
///   * `backend_libpq_auth_seams`: `hba_getauthmethod`, `check_usermap`,
///     `hba_authname_of` (consumed by `auth.c`);
///   * `backend_libpq_hba_seams`: `fill_hba_view`, `fill_ident_view`,
