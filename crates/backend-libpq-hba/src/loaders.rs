...
fn addr_view(h: &HbaLine) -> types_net::SockAddr {
    types_net::SockAddr { addr: h.addr, salen: h.addrlen as u32 }
}
fn mask_view(h: &HbaLine) -> types_net::SockAddr {
    types_net::SockAddr { addr: h.mask, salen: h.masklen as u32 }
}
...
pub(crate) fn hba_file_name() -> String {
    backend_utils_misc_guc::live::get_string("hba_file")
        .flatten()
        .unwrap_or_default()
}
...
pub(crate) fn ident_file_name() -> String {
    backend_utils_misc_guc::live::get_string("ident_file")
        .flatten()
        .unwrap_or_default()
}
