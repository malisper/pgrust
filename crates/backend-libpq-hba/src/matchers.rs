...
use backend_libpq_ifaddr::{self as ifaddr, AddressFamily};
...
use types_net::{ipCmpSameHost, AuthToken, IPCompareMethod, Port, SockAddr};
...
pub(crate) fn check_hostname(port: &mut Port, hostname: &[u8]) -> PgResult<bool> {
    ...
    if port.remote_hostname.is_none() {
        ...
        let ret = common_ip::pg_getnameinfo_all(
            &port.raddr,
            Some(&mut node),
            None,
            libc::NI_NAMEREQD,
    ...
    let mut gai_result: Vec<types_net::PgAddrInfo> = Vec::new();
    let hint = types_net::AddrInfoHint::default(); // hints == NULL in C
    let ret = common_ip::pg_getaddrinfo_all(
