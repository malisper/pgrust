...
use backend_libpq_ifaddr::AddressFamily;
...
fn numeric_host_hint() -> types_net::AddrInfoHint {
    types_net::AddrInfoHint {
...
pub fn parse_hba_line(
    ...
    if parsedline.conntype != ctLocal {
        ...
        if token_is_keyword(token, b"all") {
            ...
            let _ = mcx; // common_ip allocates into Vec, no mcx needed
            let mut gai_result: Vec<types_net::PgAddrInfo> = Vec::new();
            let ret =
                common_ip::pg_getaddrinfo_all(Some(addr_part), None, &hint, &mut gai_result);
            ...
            if let Some(cidr_bits) = cidr_slash {
                ...
                match backend_libpq_ifaddr::pg_sockaddr_cidr_mask(Some(cidr_bits), fam) {
                    Ok(mask_ip) => {
                ...
                let hint = numeric_host_hint();
                let mut gai_result: Vec<types_net::PgAddrInfo> = Vec::new();
                let ret = common_ip::pg_getaddrinfo_all(Some(&mstr), None, &hint, &mut gai_result);
                if ret != 0 || gai_result.is_empty() {
                    report_config(
                        elevel,
                        "parse_hba_line",
    ...
    if parsedline.auth_method == uaOAuth {
        ...
        let (ok, oauth_err) = backend_libpq_oauth_seams::check_oauth_validator::call(
            parsedline.clone(),
            elevel.0,
        )?;
...
fn addr_view(p: &HbaLine) -> types_net::SockAddr {
    types_net::SockAddr { addr: p.addr, salen: p.addrlen as u32 }
}
fn mask_view(p: &HbaLine) -> types_net::SockAddr {
    types_net::SockAddr { addr: p.mask, salen: p.masklen as u32 }
}
...
/// raw family.
fn sockaddr_view(p: &HbaLine) -> types_net::SockAddr {
    types_net::SockAddr {
        addr: p.addr,
...
pub(crate) fn parse_hba_auth_opt(
    ...
    if name == "map" {
        ...
        for srv in &parsed {
            let hint = types_net::AddrInfoHint {
                flags: 0,
                family: libc::AF_UNSPEC,
                socktype: libc::SOCK_DGRAM,
            };
            let mut gai_result: Vec<types_net::PgAddrInfo> = Vec::new();
            let ret = common_ip::pg_getaddrinfo_all(Some(srv), None, &hint, &mut gai_result);
            if ret != 0 || gai_result.is_empty() {
                report_config(
                    elevel,
                    "parse_hba_auth_opt",
...
fn split_guc_list(rawstring: &str) -> Option<Vec<String>> {
    ...
    let mcx = memctx.mcx();
    let result = backend_utils_adt_varlena::split_format::split_guc_list(mcx, rawstring, ',');
    match result {
        Ok(Some(list)) => {
