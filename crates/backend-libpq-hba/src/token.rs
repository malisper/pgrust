...
use types_regex::{RegMatch, RegcompResult, RegexCompiled, RegexecResult};
...
pub(crate) fn free_auth_token(token: &mut AuthToken) {
    if token_has_regexp(token) {
        ...
        if let Some(re) = token.regex.take() {
            backend_regex_core_seams::pg_regfree::call(re);
        }
    }
}
...
pub(crate) fn regcomp_auth_token(
    ...
    let wstr = backend_utils_mb_mbutils_seams::pg_mb2wchar_with_len::call(mcx.mcx(), &pat_owned)?;

    match backend_regex_core_seams::pg_regcomp::call(&wstr, REG_ADVANCED, C_COLLATION_OID)? {
...
pub(crate) fn regexec_auth_token(
    ...
    let wmatch = backend_utils_mb_mbutils_seams::pg_mb2wchar_with_len::call(mcx.mcx(), &m_owned)?;

    let mut pmatch = vec![RegMatch::UNSET; nmatch];
    match backend_regex_core_seams::pg_regexec::call(re, &wmatch, 0, &mut pmatch)? {
