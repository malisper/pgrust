...
fn install_fakes() {
    ...
    ONCE.call_once(|| {
        backend_utils_init_small_seams::with_my_proc_port::set(fake_with_my_proc_port);
        backend_libpq_be_secure_seams::secure_read::set(fake_secure_read);
        backend_libpq_be_secure_seams::secure_write::set(fake_secure_write);
    });
}
