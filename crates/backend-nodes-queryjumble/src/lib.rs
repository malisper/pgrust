...
//! [`types_nodes::copy_query::Query`] flat node tree. Half (1) is the
...
use backend_utils_misc_guc_tables::consts::{COMPUTE_QUERY_ID_OFF, COMPUTE_QUERY_ID_ON};
...
fn do_jumble(jstate: &mut JumbleState, query: &Query) -> i64 {
    ...
    common_hashfn::hash_bytes_extended(&jstate.jumble, 0) as i64
}
...
pub fn init_seams() {
    use backend_nodes_queryjumble_seams as seams;
...
fn seams_jumble_state() -> types_nodes::portalcmds::JumbleState {
    ...
    types_nodes::portalcmds::JumbleState::opaque()
}
