...
//! reconstructs the post-analysis [`types_nodes::primnodes::Expr`] form (carried
...
//! the public `backend_nodes_core::read` tokenizer (`pg_strtok` / `node_read` /
...
pub(crate) fn try_read<'mcx>(mcx: Mcx<'mcx>, label: &[u8]) -> Option<PgResult<Node<'mcx>>> {
    let res: PgResult<Node<'mcx>> = match label {
        ...
        b"SUBPLAN" => crate::read_plan_family::read_subplan(mcx).and_then(|sp| {
            Node::mk_expr(
                mcx,
                Expr::SubPlan(types_nodes::primnodes::SubPlanExpr(Box::new(sp))),
            )
        }),
        b"ALTERNATIVESUBPLAN" => {
            crate::read_plan_family::read_alternative_subplan(mcx).and_then(|asp| {
                Node::mk_expr(
                    mcx,
                    Expr::AlternativeSubPlan(types_nodes::primnodes::AlternativeSubPlanExpr(
                        Box::new(asp),
                    )),
                )
            })
...
mod tests {
    ...
    use backend_nodes_outfuncs::out_node;
    use mcx::MemoryContext;
    use types_nodes::primnodes::{Expr, Var, VarReturningType};
    ...
    fn raw_currentof_out_text() {
        ...
        let mcx = ctx.mcx();
        let n = types_nodes::rawexprnodes::CurrentOfExpr {
            cvarno: 3,
