...
//! [`MemoryContext`](backend_utils_mmgr) and deep-copied by `copyObject`, so
...
//! allocate through [`backend_utils_mmgr`] (`palloc`/`MemoryContextScope`) and
//! must not introduce `Box`/`Vec`/`Drop` ownership for node storage.

use core::ffi::{c_char, c_int};

use pgrust_pg_ffi::{Bitmapset, List, NodeTag};
