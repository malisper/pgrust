...
fn compute_gather_rows(root: &PlannerInfo, path: PathId) -> f64 {
    backend_optimizer_path_costsize::compute_gather_rows(root.path(path).base())
}
...
fn get_useful_pathkeys_for_relation(
    ...
    if !root.query_pathkeys.is_empty() {
        ...
        for pathkey in &root.query_pathkeys {
            ...
            if !backend_optimizer_path_equivclass::relation_can_be_sorted_early(
                root,
                rel,
                pathkey_ec,
...
fn pathkeys_count_contained_in(
    ...
    let (contained, common) =
        backend_optimizer_path_pathkeys::pathkeys_count_contained_in(keys1, keys2);
    *n_common = common;
    contained
}
