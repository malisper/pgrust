...
fn is_var(root: &PlannerInfo, node: NodeId) -> bool {
    matches!(root.node(node), types_nodes::primnodes::Expr::Var(_))
}

fn var_belongs(root: &PlannerInfo, node: NodeId, relid_idx: u32) -> bool {
    matches!(root.node(node), types_nodes::primnodes::Expr::Var(v) if v.varno as u32 == relid_idx)
}

fn var_fields(root: &PlannerInfo, node: NodeId) -> (i16, Oid, i32) {
    match root.node(node) {
        types_nodes::primnodes::Expr::Var(v) => (v.varattno, v.vartype, v.vartypmod),
