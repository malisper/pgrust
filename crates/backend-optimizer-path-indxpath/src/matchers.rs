...
use types_nodes::primnodes::{CompareType, Expr, RowCompareExpr};
use types_pathnodes::{IndexClause, IndexOptInfo, PlannerInfo, RinfoId};

use backend_nodes_core::makefuncs::{make_bool_const, make_opclause};
use backend_nodes_core::nodefuncs::expr_type;
use backend_optimizer_util_clauses::{contain_volatile_functions, make_SAOP_expr};
use backend_utils_cache_lsyscache_seams as lsyscache;
use backend_utils_fmgr_support_seams as fmgr_support;
...
pub fn match_saopclause_to_indexcol(
    ...
    let right_relids = backend_optimizer_util_var_seams::pull_varnos::call(mcx, &rightop)?;
    ...
    if match_index_to_operand(root, &leftop, indexcol, index)
        && !backend_nodes_core::bitmapset::bms_is_member(index_relid as i32, right_relids.as_deref())
        && !contain_volatile_functions(Some(&rightop))?
    {
        if index_coll_matches_expr_coll(idxcollation, expr_coll)
            && lsyscache::op_in_opfamily::call(expr_op, opfamily)?
