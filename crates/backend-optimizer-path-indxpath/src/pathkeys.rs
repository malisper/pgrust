...
use types_pathnodes::{EmId, IndexOptInfo, NodeId, PathKey, PlannerInfo};

use backend_optimizer_util_var_seams::contain_var_clause;
...
pub fn match_pathkeys_to_index(
    ...
    if !index.amcanorderbyop {
        return (orderby_clauses, clause_columns);
    }
    ...
    for pathkey in pathkeys {
        ...
        // PathKey.pk_cmptype is the `types_pathnodes::CompareType` (= i32);
        ...
        let ec_id = match pathkey.pk_eclass {
            Some(e) => e,
            None => return (orderby_clauses, clause_columns),
        };
        ...
        for em_id in members {
            ...
            // Any column of the index may match each pathkey.
            ...
            for indexcol in 0..nkeycolumns {
                if let Some(expr) =
                    match_clause_to_ordering_op(root, index, indexcol, &em_expr, pk_opfamily)
                {
    ...
    (orderby_clauses, clause_columns)
}
...
fn collect_ec_members(root: &PlannerInfo, ec_id: types_pathnodes::EcId) -> Vec<EmId> {
...
pub fn match_clause_to_ordering_op(
    ...
    let op = clause.as_opexpr()?;
    if op.args.len() != 2 {
        return None;
    }
    let leftop = op.args[0].clone();
    let rightop = op.args[1].clone();
    ...
    // We have a match. Return the clause or a commuted version thereof.
    if commuted {
        let newclause = OpExpr {
            ...
            args: alloc::vec![rightop, leftop],
            location: op.location,
        };
        Some(Expr::OpExpr(newclause))
    } else {
        Some(clause.clone())
    }
}
...
fn relids_equal(a: &types_pathnodes::Relids, b: &types_pathnodes::Relids) -> bool {
    let aw: &[u64] = match a {
        None => &[],
        Some(x) => &x.words,
    };
