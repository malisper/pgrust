...
//! [`expression_tree_mutator`](backend_nodes_core::nodefuncs::expression_tree_mutator)
//! over `Expr`.
//!
...
fn is_andclause(node: &Expr) -> bool {
    matches!(
        node,
        Expr::BoolExpr(b) if b.boolop == types_nodes::primnodes::BoolExprType::AND_EXPR
    )
}
...
fn is_orclause(node: &Expr) -> bool {
    matches!(
        node,
        Expr::BoolExpr(b) if b.boolop == types_nodes::primnodes::BoolExprType::OR_EXPR
    )
}
...
fn replace_correlation_vars_mutator<'mcx>(
    ...
    if let Expr::Var(v) = &node {
        if v.varlevelsup > 0 {
            let prm = backend_optimizer_util_paramassign::replace_outer_var(root, v)?;
    ...
    if let Expr::PlaceHolderVar(phv) = &node {
        if phv.phlevelsup > 0 {
            let prm =
                backend_optimizer_util_paramassign::replace_outer_placeholdervar(mcx, root, phv)?;
            return Ok(Expr::Param(prm));
        }
    }
    if let Expr::Aggref(agg) = &node {
        if agg.agglevelsup > 0 {
            let prm = backend_optimizer_util_paramassign::replace_outer_agg(mcx, root, agg)?;
            return Ok(Expr::Param(prm));
        }
    }
    if let Expr::GroupingFunc(grp) = &node {
        if grp.agglevelsup > 0 {
            let prm = backend_optimizer_util_paramassign::replace_outer_grouping(mcx, root, grp)?;
            return Ok(Expr::Param(prm));
        }
    }
    if let Expr::MergeSupportFunc(msf) = &node {
        // C: if (root->parse->commandType != CMD_MERGE)
        if run.resolve(root.parse).commandType != types_nodes::nodes::CmdType::CMD_MERGE {
            let prm = backend_optimizer_util_paramassign::replace_outer_merge_support(
                mcx, root, run, msf,
            )?;
            return Ok(Expr::Param(prm));
        }
    ...
    if let Expr::ReturningExpr(rexpr) = &node {
        if rexpr.retlevelsup > 0 {
            let prm = backend_optimizer_util_paramassign::replace_outer_returning(mcx, root, rexpr)?;
            return Ok(Expr::Param(prm));
        }
    }
...
pub fn SS_identify_outer_params(root: &mut PlannerInfo) {
    ...
    let mut outer_params: types_pathnodes::Relids = None;
