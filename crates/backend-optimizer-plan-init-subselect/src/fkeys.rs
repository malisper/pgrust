...
//! [`types_pathnodes::ForeignKeyOptInfo`] with which ECs/RestrictInfos its
...
use alloc::vec::Vec;
use backend_optimizer_path_equivclass::match_eclasses_to_foreign_key_col;
use types_core::{Oid, OidIsValid, InvalidOid};
use types_nodes::primnodes::{Expr, Var};
use types_pathnodes::{NodeId, PlannerInfo, RELOPT_BASEREL};
            ...
            for rinfo in joininfo {
                    ...
                    && con_attno == leftvar.varattno
                {
                    ...
                    if !OidIsValid(fpeqop) {
                        fpeqop = backend_utils_cache_lsyscache_seams::get_commutator::call(
