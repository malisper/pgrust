...
//! [`pull_up_sublinks`](backend_optimizer_prep_prepjointree_seams::pull_up_sublinks)
...
//! * Relids are the lifetime-free [`Relids`](types_pathnodes::Relids) the
...
use backend_nodes_core::makefuncs::make_andclause;
use backend_nodes_nodeFuncs_seams as nodefuncs;

use backend_optimizer_plan_subselect_pullup_seams as subselect;
...
fn splice_join_and_recurse<'mcx>(
    ...
    root: &mut PlannerInfo,
    parse: &mut Query<'mcx>,
    mut j: types_nodes::rawnodes::JoinExpr<'mcx>,
    jtlink: &mut Option<NodePtr<'mcx>>,
