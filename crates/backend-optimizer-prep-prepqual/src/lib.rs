...
//! owned [`Expr`](types_nodes::primnodes::Expr) tree (the same model
...
//! [`backend_nodes_core::makefuncs`] (`makefuncs.c`), never redefined here.
//!
...
fn equal_node(a: &Expr, b: &Expr) -> bool {
    backend_nodes_equalfuncs_seams::equal_expr::call(a, b)
}
...
pub fn init_seams() {
    backend_optimizer_prep_prepqual_seams::negate_clause::set(negate_clause);
    backend_optimizer_prep_prepqual_seams::canonicalize_qual::set(canonicalize_qual);
}
