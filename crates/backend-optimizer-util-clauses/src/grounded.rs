    ...
    if let Some(ctor) = node.as_jsonconstructorexpr() {
        ...
        let is_jsonb = match &ctor.returning {
            Some(r) => match &r.format {
                Some(f) => {
                    f.format_type == types_nodes::primnodes::JsonFormatType::JS_FORMAT_JSONB
                }
                None => false,
            },
            None => false,
        ...
        for arg in &ctor.args {
            let typid = backend_nodes_core::nodefuncs::expr_type(Some(arg))?;
