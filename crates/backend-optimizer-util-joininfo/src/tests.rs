...
use backend_optimizer_util_relnode_seams as bms_seam;

fn words_of(a: &types_pathnodes::Relids) -> &[u64] {
    match a {
        Some(b) => &b.words,
        None => &[],
    }
}
fn member(x: i32, a: &types_pathnodes::Relids) -> bool {
    let w = words_of(a);
    if x < 0 {
        return false;
    }
...
fn from_members(xs: &[i32]) -> types_pathnodes::Relids {
    let mut r = None;
    for &x in xs {
...
fn union(a: &types_pathnodes::Relids, b: &types_pathnodes::Relids) -> types_pathnodes::Relids {
    let aw = words_of(a);
    let bw = words_of(b);
...
fn overlap(a: &types_pathnodes::Relids, b: &types_pathnodes::Relids) -> bool {
    let aw = words_of(a);
    let bw = words_of(b);
    let n = aw.len().min(bw.len());
    (0..n).any(|i| aw[i] & bw[i] != 0)
}
fn is_subset(a: &types_pathnodes::Relids, b: &types_pathnodes::Relids) -> bool {
    let aw = words_of(a);
    let bw = words_of(b);
    aw.iter()
        .enumerate()
...
fn mk_rinfo(
    ...
    clause_relids: types_pathnodes::Relids,
    outer_relids: types_pathnodes::Relids,
    is_clone: bool,
) -> RinfoId {
    ...
    let clause = root.alloc_node(types_nodes::primnodes::Expr::Var(
        types_nodes::primnodes::Var::default(),
    ));
    let ri = RestrictInfo {
        clause,
        orclause: None,
