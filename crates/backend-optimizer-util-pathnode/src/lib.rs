...
//! # Arena model
//!
//! Paths live in the [`PlannerInfo`](types_pathnodes::PlannerInfo) arena
//! (`root.path_arena`, keyed by [`PathId`]). A constructor allocates its new
//! [`PathNode`](types_pathnodes::PathNode) into the arena via
...
//! [`backend_optimizer_util_pathnode_seams`].
//!
...
use types_pathnodes::{Path, PathId, PathKey, PathNode, PlannerInfo, RelId, Relids};
...
fn create_memoize_path_jp(
    ...
    rel: RelId,
    subpath: PathId,
    param_exprs: &[types_pathnodes::NodeId],
