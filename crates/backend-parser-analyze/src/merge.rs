...
use types_nodes::modifytable::{MergeMatchKind, NUM_MERGE_MATCH_KINDS};
use types_nodes::nodes::{CmdType, Node, NodePtr};
use types_nodes::parsestmt::{ParseExprKind, ParseNamespaceItem, ParseState};
use types_nodes::rawnodes::{MergeAction, MergeStmt, MergeWhenClause};
...
pub fn transformMergeStmt<'mcx>(
    ...
    if relkind != RELKIND_RELATION && relkind != RELKIND_PARTITIONED_TABLE && relkind != RELKIND_VIEW
    {
        ...
        return Err(ereport(ERROR)
            ...
            .errdetail(backend_catalog_pg_class::errdetail_relkind_not_supported(relkind)?)
            .into_error());
    }
    ...
    backend_parser_clause::transformFromClause(mcx, pstate, &from_list[..])?;
    ...
    let source_aliasname = {
        let nsitem =
            backend_parser_relation::GetNSItemByRangeTablePosn(pstate, source_rti, 0)?;
    ...
    let target_nsitem = pstate
        ...
        .ok_or_else(|| elog_error("MERGE target namespace item is missing"))?
        .clone_in(mcx)?;
    backend_parser_relation::addNSItemToQuery(mcx, pstate, target_nsitem, false, true, true)?;

    let join_condition = match stmt.joinCondition.as_ref() {
        Some(n) => Some(n.as_ref().clone_in(mcx)?),
        None => None,
    };
    let merge_join_cond = backend_parser_parse_expr::transformExpr(
        pstate,
        join_condition,
    ...
    for l in stmt.mergeWhenClauses.iter() {
        ...
        let mut action = MergeAction {
            ...
            r#override: types_nodes::modifytable::OverridingKind::OVERRIDING_NOT_SET,
            qual: None,
        ...
        match when_command_type {
            CmdType::CMD_INSERT => {
                ...
                let (icolumns, attrnos) =
                    backend_parser_parse_target::checkInsertTargets(mcx, pstate, when_target_cols)?;
                ...
                let expr_list: PgVec<'mcx, types_nodes::primnodes::Expr> = if when_values_empty {
                    /*
...
) -> PgResult<PgVec<'mcx, types_nodes::rawnodes::ResTarget<'mcx>>> {
    let mut out: PgVec<'mcx, types_nodes::rawnodes::ResTarget<'mcx>> =
