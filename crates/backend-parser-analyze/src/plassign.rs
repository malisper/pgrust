...
use types_nodes::ddlnodes::{CoercionContext, PLAssignStmt};
...
use types_nodes::rawnodes::{ColumnRef, SelectStmt};
...
fn iscomplex(typeid: Oid) -> PgResult<bool> {
    ...
    Ok(backend_utils_cache_lsyscache_seams::get_typtype::call(typeid)? == TYPTYPE_COMPOSITE)
}
...
pub fn transformPLAssignStmt<'mcx>(
    ...
    backend_parser_clause::transformFromClause(mcx, pstate, &sstmt.fromClause)?;
        ...
        && (type_id == RECORDOID || iscomplex(type_id)?)
    {
        ...
        match coerced {
            ...
            None => {
                ...
                let targettype_name =
                    backend_utils_adt_format_type_seams::format_type_be::call(mcx, targettype)?;
                let type_id_name =
                    backend_utils_adt_format_type_seams::format_type_be::call(mcx, type_id)?;
                return Err(ereport(ERROR)
                    ...
                    .errposition(backend_parser_small1::parser_errposition(pstate, orig_location))
                    .into_error());
            }
        }
    }
