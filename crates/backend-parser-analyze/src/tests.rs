...
/// `seams_init::init_all()` is not used here: it eagerly installs the entire
...
fn init_seams_once() {
    ...
    ONCE.call_once(|| {
        ...
        backend_parser_parse_collate::init_seams();
        backend_parser_relation::init_seams();
        backend_parser_cte::init_seams();
...
fn analyze_one<'mcx>(
    ...
    let raw = backend_parser_driver::raw_parser(
        mcx,
        sql,
    ...
    let mut pstate = backend_parser_small1::make_parsestate(mcx, None).unwrap();
...
fn select_const_where_order_builds_owned_query() {
    ...
    assert_eq!(
        query.querySource,
        types_nodes::copy_query::QuerySource::QSRC_ORIGINAL
    );
    ...
    assert_eq!(as_node.tag(), types_nodes::nodes::T_Query);
}
