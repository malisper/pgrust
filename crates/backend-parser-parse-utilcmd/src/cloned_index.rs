...
use types_nodes::ddlnodes::{DefElem, IndexElem, IndexStmt, DEFELEM_UNSPEC};
use types_nodes::nodes::Node;
use types_nodes::rawnodes::{RangeVar, SORTBY_DEFAULT, SORTBY_DESC, SORTBY_NULLS_DEFAULT, SORTBY_NULLS_FIRST, SORTBY_NULLS_LAST};
...
use backend_access_common_reloptions::untransformRelOptions;
...
pub fn generateClonedIndexStmt<'mcx>(
    ...
    let table_space = if OidIsValid(reltablespace) {
        backend_commands_tablespace::get_tablespace_name(mcx, reltablespace)?
    } else {
        None
    };
    ...
    for keyno in 0..idxinfo.indnkeyatts as usize {
        ...
        if attribute_number_is_valid(attnum) {
            ...
            backend_rewrite_core::replace::map_variable_attnos(
                &mut indexkey,
                1,
                0,
                &attmap.attnums,
...
fn get_opclassopts<'mcx>(
    ...
    let mut result = PgVec::new_in(mcx);
    let attoptions = backend_utils_cache_lsyscache::attribute::get_attoptions(mcx, source_relid, attnum)?;
    let bytes: &[u8] = match &attoptions {
        Some(types_tuple::backend_access_common_heaptuple::Datum::ByRef(b)) => &b[..],
        _ => return Ok(result),
    };
