...
//!   * `ScanKeywordLookup`/`GetScanKeyword` from `common-keywords`,
...
//!     `backend-utils-mb`,
//!   * `pg_strtoint32_safe` from `backend-utils-adt-numutils`,
...
use backend_utils_mb::{pg_get_client_encoding, pg_verifymbstr};
use pgrust_pg_ffi::error::make_sqlstate;
use pgrust_pg_ffi::{SqlState, ERRCODE_FEATURE_NOT_SUPPORTED, PG_ENCODING_IS_CLIENT_ONLY};
...
pub struct LexError {
    ...
    pub source: Option<backend_utils_error::PgError>,
}
...
impl<'a> Scanner<'a> {
    ...
    fn lexerr(&self, message: &'static str) -> LexError {
        LexError {
            sqlstate: pgrust_pg_ffi::ERRCODE_SYNTAX_ERROR,
            message,
            detail: None,
            hint: None,
