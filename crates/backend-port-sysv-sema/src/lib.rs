...
pub fn PGReserveSemaphores(max_semas: i32) -> PgResult<()> {
    ...
    let _shim = backend_storage_ipc_shmem_seams::shmem_alloc_unlocked::call(
...
pub fn init_seams() {
    backend_port_pg_sema_seams::pg_semaphore_shmem_size::set(PGSemaphoreShmemSize);
    backend_port_pg_sema_seams::pg_reserve_semaphores::set(PGReserveSemaphores);
    backend_port_pg_sema_seams::pg_semaphore_reset::set(PGSemaphoreReset);
    backend_port_pg_sema_seams::pg_semaphore_lock::set(PGSemaphoreLock);
    backend_port_pg_sema_seams::pg_semaphore_unlock::set(PGSemaphoreUnlock);
}
