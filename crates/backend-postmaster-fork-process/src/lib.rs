...
//!
//!  * `&BlockSig` / signal-mask install → [`backend_libpq_pqsignal`]'s owned
//!    [`backend_libpq_pqsignal::SignalMasks`] (the backend-private
//!    `BlockSig`/`UnBlockSig` globals) plus direct `libc::sigprocmask`.
//!  * `MyProcPid = getpid()` → [`backend_utils_init_small::globals::SetMyProcPid`]
//!    fed by `libc::getpid()`.
//!  * `pg_strong_random_init()` → the
//!    [`port_pg_strong_random_seams::pg_strong_random_init`] seam, whose owner
...
pub fn init_seams() {
    backend_postmaster_fork_process_seams::fork_process::set(fork_process);
}
...
pub fn fork_process() -> pid_t {
    ...
    if result == 0 {
        ...
        port_pg_strong_random_seams::pg_strong_random_init::call();
    } else {
