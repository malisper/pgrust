...
fn max_connections() -> i32 {
    backend_utils_init_small_seams::max_connections::call()
}
...
fn autovacuum_worker_slots() -> i32 {
    backend_postmaster_autovacuum_seams::autovacuum_worker_slots::call()
}

/// `max_worker_processes` (globals.c GUC).
fn max_worker_processes() -> i32 {
    backend_utils_init_small_seams::max_worker_processes::call()
}

/// `PostmasterChildName(btype)` (launch_backend.c) — used in DEBUG2 logging.
fn postmaster_child_name(btype: BackendType) -> &'static str {
    backend_postmaster_launch_backend_seams::postmaster_child_name::call(btype)
}
...
pub fn AssignPostmasterChildSlot(btype: BackendType) -> Option<PMChild> {
    ...
    backend_storage_ipc_pmsignal::MarkPostmasterChildSlotAssigned(pmchild.child_slot)
        .unwrap_or_else(|e| {
...
pub fn ReleasePostmasterChildSlot(pmchild: PMChild) -> bool {
    ...
    backend_storage_ipc_pmsignal::MarkPostmasterChildSlotUnassigned(pmchild.child_slot)
}
...
pub fn init_seams() {
    backend_postmaster_pmchild_seams::init_postmaster_child_slots::set(InitPostmasterChildSlots);
    backend_postmaster_pmchild_seams::max_live_postmaster_children::set(MaxLivePostmasterChildren);
}
