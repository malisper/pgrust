...
//! is walked through [`backend_postmaster_pmchild::ActiveChildListSnapshot`],
...
use backend_postmaster_pmchild::{PMChild, SetActiveChildBkendType, ActiveChildListSnapshot};
...
pub fn signal_child(pmchild: PMChild, signal: i32) {
    let pid = pmchild.pid;
    let desc = backend_utils_init_miscinit_seams::get_backend_type_desc::call(pmchild.bkend_type);
...
pub fn SignalChildren(signal: i32, targetMask: BackendTypeMask) -> bool {
    ...
    for mut bp in ActiveChildListSnapshot() {
        ...
        if btmask_contains(targetMask, B_WAL_SENDER) != btmask_contains(targetMask, B_BACKEND)
            ...
            && backend_storage_ipc_pmsignal::IsPostmasterChildWalSender(bp.child_slot)
        {
    ...
    for mut bp in ActiveChildListSnapshot() {
        ...
        let _ = ereport(DEBUG4)
            .errmsg_internal(format!(
                ...
                backend_utils_init_miscinit_seams::get_backend_type_desc::call(bp.bkend_type),
                bp.pid,
            ))
