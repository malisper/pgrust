...
pub fn ConfigurePostmasterWaitSet(accept_connections: bool) {
    ...
    set.add_event(WL_LATCH_SET, PGINVALID_SOCKET, backend_storage_ipc_latch::my_latch(), None)
    ...
    loop {
        ...
        if now - last_lockfile_recheck_time >= SECS_PER_MINUTE {
            if !sp::recheck_data_dir_lock_file::call() {
                ...
                kill(backend_utils_init_miscinit_seams::my_proc_pid::call(), SIGQUIT);
            }
...
pub fn BackendStartup(client_sock: &ClientSocket) -> i32 {
     ...
     */
    let socket_created = backend_utils_adt_timestamp_seams::get_current_timestamp::call();
    ...
    let _ = ereport(DEBUG2)
        .errmsg_internal(format!(
            ...
            backend_utils_init_miscinit_seams::get_backend_type_desc::call(bn.bkend_type),
            pid,
            client_sock.sock,
        ))
        .finish(here("BackendStartup"));
