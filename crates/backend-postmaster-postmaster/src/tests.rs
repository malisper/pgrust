...
fn marks_matching_pid_and_returns_found() {
    ...
    backend_postmaster_pmchild::SetActiveChildBgworkerInfo(av.child_slot, None, false);
    assert!(SetActiveChildPid(av.child_slot, 5151));

    let before = backend_postmaster_pmchild::ActiveChildListSnapshot();
    assert!(before
        .iter()
    ...
    let after = backend_postmaster_pmchild::ActiveChildListSnapshot();
    assert!(after
        .iter()
        .any(|c| c.child_slot == av.child_slot && c.bgworker_notify));
}
