...
enum Call {
    ...
    DisableTimeout { id: u32, keep_indicator: bool },
    EnableTimeoutEvery { id: u32, fin_time: i64, delay_ms: i32 },
    SetBackendType(u32),
    AuxProcessMainCommon,
    OnShmemExit,
    InitializeTimeouts,
    RegisterTimeout { id: u32 },
    Pqsignal { signo: i32 },
    StartupXlog,
}
...
fn install_mocks() -> MutexGuard<'static, ()> {
    ...
    *MOCK.lock().unwrap() = Some(MockState {
    ...
    INSTALL.call_once(|| {
        ...
        backend_utils_misc_guc_seams::process_config_file::set(|_context| {
            with_mock(|m| {
        ...
        backend_storage_ipc_seams::on_shmem_exit::set(|_f, _arg| record(Call::OnShmemExit));
...
fn startup_process_main_runs_boot_sequence_and_exits_zero() {
    ...
    assert_eq!(
        seq,
        vec![
            Call::SetBackendType(types_miscadmin::B_STARTUP),
