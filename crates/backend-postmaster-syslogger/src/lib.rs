...
///
/// `start_time` is the C `MyStartTime` and `mode_mask` the C `pg_mode_mask`
/// (`common/file_perm.c`) — foreign per-backend/process globals taken as
/// explicit parameters per AGENTS.md; the child-launch machinery that sets
/// them passes them in.
pub fn SysLoggerMain(startup_data: &[u8], start_time: pg_time_t, mode_mask: u32) -> PgResult<()> {
    ...
    wes.add_event(WL_LATCH_SET, PGINVALID_SOCKET, true)?;
    wes.add_event(WL_SOCKET_READABLE, config::syslog_pipe()[0], false)?;
