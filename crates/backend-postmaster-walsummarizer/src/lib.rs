...
use backend_utils_misc_guc_file_seams as gucfile;
...
use backend_utils_activity_walstats_seams as walstats;
...
use backend_replication_walreceiver_seams as walreceiver;
...
// types_wal), RM_SMGR_ID=2, RM_DBASE_ID=4.
...
pub fn WalSummarizerShmemInit() -> PgResult<()> {
    ...
    if !found {
        ...
        c.summary_file_cv = types_condvar::ConditionVariable::new();
    }
    Ok(())
}
