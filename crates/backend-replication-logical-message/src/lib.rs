...
use types_wal::wal::{RM_LOGICALMSG_ID, XLOG_INCLUDE_ORIGIN, XLOG_LOGICAL_MESSAGE, XLR_INFO_MASK};
...
pub fn LogLogicalMessage(
     ...
     */
    if transactional {
        debug_assert!(backend_access_transam_xact::IsTransactionState());
        backend_access_transam_xact::GetCurrentTransactionId()?;
    }

    let db_id = backend_utils_init_small::globals::MyDatabaseId();
    ...
    backend_access_transam_xloginsert::XLogRegisterData(&serialize_header(
        db_id,
        transactional,
        prefix_size,
        message_size,
    ))?;
    backend_access_transam_xloginsert::XLogRegisterData(&prefix_with_nul(prefix))?;
    backend_access_transam_xloginsert::XLogRegisterData(&message[..size])?;

    /* allow origin filtering */
    backend_access_transam_xloginsert::XLogSetRecordFlags(XLOG_INCLUDE_ORIGIN);

    lsn = backend_access_transam_xloginsert::XLogInsert(RM_LOGICALMSG_ID, XLOG_LOGICAL_MESSAGE)?;
    ...
    if !transactional && flush {
        backend_access_transam_xlog::XLogFlush(lsn)?;
    }
    Ok(lsn)
}
...
pub fn init_seams() {
    backend_replication_logical_message_seams::logicalmsg_redo::set(logicalmsg_redo);
}
