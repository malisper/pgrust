...
fn install_fixtures() {
    ...
    INSTALL.call_once(|| {
        ...
        backend_utils_mb_mbutils_seams::pg_server_to_client::set(|_, _| Ok(None));
        backend_utils_mb_mbutils_seams::pg_client_to_server::set(|_, _| Ok(None));
...
fn txn_fixture<'mcx>(f: &'mcx Fixture, gid: Option<&[u8]>) -> ReorderBufferTXN<'mcx> {
    ReorderBufferTXN {
        txn_flags: types_wal::reorderbuffer::RBTXN_IS_PREPARED,
        xid: 654,
