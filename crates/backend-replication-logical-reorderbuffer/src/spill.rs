...
impl ReorderBuffer {
    ...
    fn serialized_path(&self, xid: TransactionId, segno: u64) -> String {
        let wal_segment_size = backend_access_transam_xlog_seams::wal_segment_size::call();
        let recptr = xlog_segno_offset_to_recptr(segno, wal_segment_size);
        let slot = backend_replication_slot_seams::slot_name::call();
    ...
    pub(crate) fn serialize_txn(&mut self, xid: TransactionId) {
        ...
        for change in changes {
            ...
            if fd == -1 || !xlbyte_in_seg(change.lsn, cur_open_segno, wal_segment_size) {
                ...
                fd = backend_storage_file_fd_seams::open_transient_file::call(
                    &path,
    ...
    fn serialize_change(&mut self, xid: TransactionId, fd: i32, change: &ReorderBufferChange) {
        ...
        let written = backend_storage_file_fd_seams::transient_write::call(fd, &record);
    ...
    pub(crate) fn restore_changes(
        ...
        while restored < MAX_CHANGES_IN_MEMORY && *segno <= last_segno {
            ...
            if file.vfd.is_none() {
                ...
                let opened = backend_storage_file_fd_seams::path_name_open_file::call(
                    &path,
                ...
                file.cur_offset = 0;
                if opened.0 < 0 {
                    let errno = backend_storage_file_fd_seams::last_errno::call();
            ...
            let read_bytes = backend_storage_file_fd_seams::file_read::call(
                vfd,
                &mut len_buf,
                file.cur_offset,
    ...
    pub(crate) fn cleanup_serialized_txns(slotname: &str) {
        ...
        match backend_storage_file_fd_seams::lstat_file::call(&dir, true) {
            Ok(Some(st)) if !st.isdir => return,
            Ok(_) => {}
            Err(_) => return,
        }

        let names = match backend_storage_file_fd_seams::read_dir_names::call(&dir) {
            Ok(n) => n,
            Err(_) => return,
        };
        for name in names {
...
fn encode_snapshot(buf: &mut Vec<u8>, snap: &types_snapshot::SnapshotData) {
...
fn decode_snapshot(r: &mut Reader) -> types_snapshot::SnapshotData {
...
fn snapshot_type_to_u32(t: types_snapshot::SnapshotType) -> u32 {
    use types_snapshot::SnapshotType as T;
    match t {
        T::SNAPSHOT_MVCC => 0,
        T::SNAPSHOT_SELF => 1,
        T::SNAPSHOT_ANY => 2,
...
fn u32_to_snapshot_type(v: u32) -> types_snapshot::SnapshotType {
    use types_snapshot::SnapshotType as T;
    match v {
        0 => T::SNAPSHOT_MVCC,
        1 => T::SNAPSHOT_SELF,
