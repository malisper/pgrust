...
use backend_access_transam_transam::{TransactionIdPrecedes, TransactionIdPrecedesOrEquals};
...
use backend_utils_activity_pgstat_replslot_seams as pgstat_replslot;
...
fn spin_acquire(slot: &ReplicationSlot) {
    backend_storage_lmgr_s_lock::s_lock_macro(&slot.mutex, Some(SLOT_C), 0, None);
}
...
fn timestamp_difference(start: TimestampTz, stop: TimestampTz) -> (i64, i32) {
    backend_utils_adt_timestamp_seams::timestamp_difference::call(start, stop)
}
fn timestamp_difference_exceeds_seconds(start: TimestampTz, stop: TimestampTz, secs: i32) -> bool {
    backend_utils_adt_timestamp_seams::timestamp_difference_exceeds_seconds::call(start, stop, secs)
}
...
fn compute_on_disk_checksum(cp: &ReplicationSlotOnDisk) -> u32 {
    ...
    crc = port_pg_crc32c_seams::pg_comp_crc32c::call(crc, &bytes[start..start + len]);
...
fn RestoreSlotFromDisk(name: &str) -> PgResult<()> {
    ...
    let wal_level = xlog::wal_level::call();
...
fn validate_sync_standby_slots(
    ...
    let elemlist = match backend_utils_adt_varlena_seams::split_identifier_string::call(rawname, ',')
    {
        Some(list) => list,
        None => {
...
pub fn WaitForStandbyConfirmation(wait_for_lsn: XLogRecPtr) -> PgResult<()> {
    ...
    loop {
        ...
        if backend_postmaster_interrupt::ConfigReloadPending() {
            ...
            backend_utils_misc_guc_file_seams::process_config_file::call(
