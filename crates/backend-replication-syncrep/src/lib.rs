...
use types_replication::walsender::{SyncRepStandbyData, WalSndState};
...
use backend_replication_syncrep_gram::{syncrep_yyparse, SYNC_REP_PRIORITY};
use backend_replication_walsender::core as wsctl;
...
use backend_utils_init_small_seams as misc;
...
pub fn SyncRepWaitForLSN(lsn: XLogRecPtr, commit: bool) -> PgResult<()> {
    ...
    loop {
        ...
        if misc::proc_die_pending::call() {
            ...
            backend_tcop_postgres_seams::set_where_to_send_output_none::call();
            SyncRepCancelWait()?;
            break;
        }
...
pub fn check_synchronous_standby_names(newval: Option<&str>) -> PgResult<CheckResult> {
    if let Some(s) = newval {
        if !s.is_empty() {
            ...
            let scanner =
                backend_replication_syncrep_scanner_seams::syncrep_scanner_init::call(mcx, s)?;
            let parse_outcome = syncrep_yyparse(mcx, scanner);
            backend_replication_syncrep_scanner_seams::syncrep_scanner_finish::call(scanner);

            let parsed = match parse_outcome {
                Ok(parsed) => parsed,
                Err(err) => {
                    use backend_utils_misc_guc::{GUC_check_errcode, GUC_check_errdetail};
            ...
            if parsed.num_sync() <= 0 {
                use backend_utils_misc_guc::GUC_check_errmsg;
