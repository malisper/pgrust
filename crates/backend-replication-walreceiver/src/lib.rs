...
fn WalRcvWaitForStartPosition(
    ...
    loop {
        ...
        // main's seam is the MyLatch-implicit `wait_latch_my_latch`
        // (u32 events / u32 wait-event id, fallible); adapt to it.
        let _ = latch::wait_latch_my_latch::call(
            (WL_LATCH_SET | WL_EXIT_ON_PM_DEATH) as u32,
            0,
            WAIT_EVENT_WAL_RECEIVER_WAIT_START as u32,
        );
    }
...
fn WalRcvFetchTimeLineHistoryFiles(first: TimeLineID, last: TimeLineID) -> PgResult<()> {
    ...
    while tli <= last {
        ...
        if tli != 1 && !timeline::exists_time_line_history::call(scratch.mcx(), tli, false)? {
            emit(ereport(LOG).errmsg(format!(
            ...
            timeline::write_time_line_history_file::call(tli, &content)?;
...
fn ProcessWalSndrMessage(walEnd: XLogRecPtr, sendTime: TimestampTz) {
    ...
    if message_level_is_interesting(DEBUG2) {
         ...
         * main's seam allocates the formatted copy in an mcx and is fallible;
         * use a local scratch context and own the strings for the debug line. */
        let scratch = mcx::MemoryContext::new("walrcv_sndr_msg_debug");
        let smcx = scratch.mcx();
        let sendtime = timestamp::timestamptz_to_str::call(smcx, sendTime)
            .map(|s| s.as_str().to_string())
            .unwrap_or_default();
        let receipttime = timestamp::timestamptz_to_str::call(smcx, lastMsgReceiptTime)
            .map(|s| s.as_str().to_string())
            .unwrap_or_default();
