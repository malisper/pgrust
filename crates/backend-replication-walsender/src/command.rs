...
pub fn ReadReplicationSlot(cmd: crate::core::ReadReplicationSlotCmd) -> types_error::PgResult<()> {
    ...
    let mut tupdesc = backend_access_common_tupdesc::CreateTemplateTupleDesc(mcx, READ_REPLICATION_SLOT_COLS as i32)?;
    backend_access_common_tupdesc::TupleDescInitBuiltinEntry(&mut tupdesc, 1, "slot_type", TEXTOID, -1, 0)?;
    backend_access_common_tupdesc::TupleDescInitBuiltinEntry(&mut tupdesc, 2, "restart_lsn", TEXTOID, -1, 0)?;
    // TimeLineID is unsigned, so int4 is not wide enough.
    backend_access_common_tupdesc::TupleDescInitBuiltinEntry(&mut tupdesc, 3, "restart_tli", INT8OID, -1, 0)?;
    ...
    let control_lock = backend_storage_lmgr_lwlock_seams::lwlock_acquire_main::call(
    ...
    if slot.is_none() || !crate::slot::slot_in_use::call(slot) {
        ...
        // slot type
        values[0] = backend_utils_adt_varlena_seams::cstring_to_text_v::call(mcx, "physical")?;
        nulls[0] = false;
        ...
        if slot_restart_lsn != InvalidXLogRecPtr {
            ...
            values[1] = backend_utils_adt_varlena_seams::cstring_to_text_v::call(mcx, &xloc)?;
            nulls[1] = false;
        }
...
pub fn SendTimeLineHistory(cmd: crate::core::TimeLineHistoryCmd) -> types_error::PgResult<()> {
    ...
    let mut tupdesc = backend_access_common_tupdesc::CreateTemplateTupleDesc(mcx, 2)?;
    backend_access_common_tupdesc::TupleDescInitBuiltinEntry(&mut tupdesc, 1, "filename", TEXTOID, -1, 0)?;
    backend_access_common_tupdesc::TupleDescInitBuiltinEntry(&mut tupdesc, 2, "content", TEXTOID, -1, 0)?;
    ...
    dest::dest_rstartup::call(mcx, dest, types_nodes::nodes::CmdType::CMD_SELECT, &*tupdesc)?;
    ...
    let fd = backend_storage_file_fd_seams::open_transient_file::call(&path, O_RDONLY | PG_BINARY);
    if fd < 0 {
        return backend_utils_error::ereport(types_error::ERROR)
    ...
    let histfilelen = backend_storage_file_fd_seams::transient_lseek::call(fd, 0, SEEK_END);
    if histfilelen < 0 {
        return backend_utils_error::ereport(types_error::ERROR)
            .errcode_for_file_access()
    ...
    if backend_storage_file_fd_seams::transient_lseek::call(fd, 0, SEEK_SET) != 0 {
        return backend_utils_error::ereport(types_error::ERROR)
            .errcode_for_file_access()
    ...
    while bytesleft > 0 {
        ...
        let nread = backend_storage_file_fd_seams::transient_read::call(fd, &mut rbuf);
    ...
    if backend_storage_file_fd_seams::close_transient_file::call(fd) != 0 {
        return backend_utils_error::ereport(types_error::ERROR)
            .errcode_for_file_access()
