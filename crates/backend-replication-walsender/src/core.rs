...
pub use types_replication::replnodes::{
...
pub use types_replication::walsender::{SyncState, WalSenderRow, WalSnd, WalSndState};
...
pub fn slot_spin_acquire(slot: &WalSndSlot) {
    if backend_storage_lmgr_s_lock::tas(&slot.mutex) != 0 {
        backend_storage_lmgr_s_lock::s_lock(&slot.mutex, None, 0, None);
    }
}
