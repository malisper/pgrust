...
//! (`relation->rd_rules->rules[i]`). The trimmed per-query `types_rel::Relation`
...
//! [`backend_utils_cache_relcache_seams::relation_rules`] reader, which
...
use types_nodes::modifytable::{OnConflictAction, OverridingKind};
...
use backend_nodes_core::makefuncs::{flat_copy_target_entry, make_null_const, make_target_entry};
...
use backend_rewrite_core::manip_rule::{AddInvertedQual, AddQual, CombineRangeTables};
use backend_rewrite_core::offset::OffsetVarNodes;
use backend_rewrite_core::replace::{ReplaceVarsFromTargetList, ReplaceVarsNoMatchOption};
use backend_rewrite_core::walkers::{checkExprHasSubLink, rangeTableEntry_used};
use backend_utils_cache_relcache_seams::{relation_rules, RewriteRuleImage};
...
fn process_matched_tle<'mcx>(
    mcx: Mcx<'mcx>,
    src_tle: &types_nodes::primnodes::TargetEntry<'mcx>,
    prior_tle: Option<&types_nodes::primnodes::TargetEntry<'mcx>>,
    attr_name: &str,
...
fn findDefaultOnlyColumns<'mcx>(
    ...
    use backend_nodes_core::bitmapset::{bms_add_member, bms_del_member, bms_is_empty};
    let mut default_only_cols: Option<PgBox<'mcx, types_nodes::bitmapset::Bitmapset<'mcx>>> = None;
...
pub fn rewriteTargetListIU<'mcx>(
    ...
    unused_values_attrnos: &mut Option<PgBox<'mcx, types_nodes::bitmapset::Bitmapset<'mcx>>>,
    ...
    let mut new_tles: Vec<Option<types_nodes::primnodes::TargetEntry<'mcx>>> =
        (0..numattrs).map(|_| None).collect();
    let mut junk_tlist: Vec<types_nodes::primnodes::TargetEntry<'mcx>> = Vec::new();
    ...
    let mut new_tlist: Vec<types_nodes::primnodes::TargetEntry<'mcx>> = Vec::new();
    ...
    let mut result: PgVec<'mcx, types_nodes::primnodes::TargetEntry<'mcx>> = PgVec::new_in(mcx);
    for t in new_tlist {
        result.push(t);
    }
    for t in junk_tlist {
...
pub fn rewriteValuesRTE<'mcx>(
    ...
    rti: i32,
    target_relation: &Relation<'mcx>,
    unused_cols: Option<&types_nodes::bitmapset::Bitmapset<'mcx>>,
) -> PgResult<bool> {
    use backend_nodes_core::bitmapset::bms_is_member;
...
fn query_range_table_entry_used(parsetree: &Query<'_>, rt_index: i32) -> bool {
    ...
    backend_nodes_core::node_walker::query_tree_walker(parsetree, &mut walker, 0);
    used
}
        ...
        match rtekind {
            ...
            RTEKind::RTE_SUBQUERY => {
                let pushed = for_update_pushed_down
                    || backend_parser_relation::get_parse_rowmark(parsetree, rt_index as Index)
                        .is_some();
...
pub fn rewriteRuleAction<'mcx>(
    ...
    let sub_action_idx = backend_rewrite_core::getInsertSelectQueryIndex(&rule_action)?;
    ...
    // Generate the expanded rtable: main parsetree rtable + rule action rtable.
    {
        ...
        let perminfos_tail: PgVec<'mcx, types_nodes::parsenodes::RTEPermissionInfo<'mcx>> =
        ...
        let mut new_perminfos: PgVec<'mcx, types_nodes::parsenodes::RTEPermissionInfo<'mcx>> =
            PgVec::new_in(mcx);
    ...
    if sub_action.commandType != CmdType::CMD_UTILITY {
        ...
        let newjointree = backend_rewrite_core::manip_rule::adjustJoinTreeList(
        ...
        && sub_action.commandType != CmdType::CMD_UTILITY
    {
        ...
        let tlist: Vec<types_nodes::primnodes::TargetEntry<'mcx>> = parsetree
            .targetList
            .iter()
    ...
    if parsetree.returningList.is_empty() {
        ...
        *returning_flag = true;
        ...
        let action_tlist: Vec<types_nodes::primnodes::TargetEntry<'mcx>> = rule_action
            .returningList
            .iter()
            .map(|t| t.clone_in(mcx))
            .collect::<PgResult<_>>()?;
        ...
        let mut ret_list: PgVec<'mcx, types_nodes::primnodes::TargetEntry<'mcx>> =
            PgVec::new_in(mcx);
...
fn ApplyRetrieveRule<'mcx>(
    ...
    let rc = backend_parser_relation::get_parse_rowmark(&parsetree, rt_index as Index).cloned();
        ...
        match event {
            CmdType::CMD_INSERT => {
                ...
                if let Some(vidx) = values_rte_found {
                    let mut unused: Option<PgBox<'mcx, types_nodes::bitmapset::Bitmapset<'mcx>>> =
                        None;
