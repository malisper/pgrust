...
//! [`backend_utils_cache_relcache_seams::relation_rules`] reader seam. This
...
use types_nodes::ddlnodes::CoercionContext;
use types_nodes::nodes::{CmdType, Node};
use types_nodes::parsenodes::{RTEKind, RangeTblEntry};
use types_nodes::primnodes::{CoercionForm, CollateExpr, Expr, NextValueExpr, TargetEntry};
use types_nodes::rawnodes::RangeTblRef;

use backend_nodes_core::bitmapset::{bms_add_member, bms_is_member, bms_next_member};
...
pub fn build_column_default<'mcx>(
    ...
    if attidentity != 0 {
        let seqid =
            backend_catalog_pg_depend::getIdentitySequence(mcx, rel, attrno as i16, false)?;
    ...
    if atthasdef {
        let d = backend_access_common_tupdesc::TupleDescGetDefault(mcx, rd_att, attrno as i16)?;
        match d {
    ...
    if expr.is_none() && attgenerated == 0 {
        if let Some(node) =
            backend_utils_cache_lsyscache_seams::get_typdefault::call(mcx, atttype)?
        {
    ...
    let Some(coerced) = coerced else {
        return Err(PgError::new(
            ...
            format!(
                ...
                attname,
                backend_utils_adt_format_type::format_type_be_str(atttype)?,
                backend_utils_adt_format_type::format_type_be_str(exprtype)?,
            ),
        )
...
pub fn view_has_instead_trigger(
    view: &Relation<'_>,
    event: CmdType,
    merge_action_list: &[types_nodes::nodes::NodePtr<'_>],
) -> PgResult<bool> {
...
fn view_cols_are_auto_updatable<'mcx>(
    ...
    mut updatable_cols: Option<&mut Option<PgBox<'mcx, types_nodes::bitmapset::Bitmapset<'mcx>>>>,
...
fn adjust_view_column_set<'mcx>(
    ...
    let mut result: Option<PgBox<'mcx, types_nodes::bitmapset::Bitmapset<'mcx>>> = None;
    ...
    loop {
        ...
        if attno == 0 {
            ...
            let tle = backend_parser_relation::get_tle_by_resno(targetlist, attno as i16);
            match tle {
...
mod tests {
    ...
    fn simple_view_query(mcx: Mcx<'_>) -> Query<'_> {
        ...
        fromlist.push(mcx::alloc_in(mcx, Node::RangeTblRef(RangeTblRef { rtindex: 1 })).unwrap());
        q.jointree = Some(
            mcx::alloc_in(
                mcx,
        ...
        for attno in 1..=2 {
            let var = backend_nodes_core::makefuncs::make_var(1, attno, 23, -1, InvalidOid, 0);
            let tle =
    ...
    fn distinct_view_is_not_auto_updatable() {
        ...
        q.distinctClause
            .push(mcx::alloc_in(mcx, Node::RangeTblRef(RangeTblRef { rtindex: 1 })).unwrap());
