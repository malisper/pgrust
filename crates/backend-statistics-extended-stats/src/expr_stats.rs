...
use backend_utils_adt_arrayfuncs::construct::construct_md_array_values;
use backend_utils_adt_scalar_datum_core::datum_copy_v;
use backend_utils_cache_attoptcache::get_attribute_options;
use backend_utils_cache_lsyscache_seams::get_rel_type_id;
...
pub fn decode_stxexprs<'mcx>(mcx: Mcx<'mcx>, exprs_string: &str) -> PgResult<Vec<Expr>> {
    // exprs = (List *) stringToNode(exprsString);
    let node = backend_nodes_read_seams::string_to_node::call(mcx, exprs_string)?;
    ...
    let mut result: Vec<Expr> = Vec::with_capacity(out.len());
    for e in out {
        let mut e =
    ...
    for stats in vacstats {
        ...
        for k in 0..STATISTIC_NUM_SLOTS {
            ...
            if nnum > 0 {
                ...
                let arry = backend_utils_adt_arrayfuncs::construct::construct_array_values(
        ...
        let stup = backend_access_common_heaptuple::heap_form_tuple(mcx, &sd.rd_att, &values, &nulls)
        ...
        let composite =
            backend_access_common_heaptuple::heap_copy_tuple_as_datum(mcx, &stup, &sd.rd_att)?;
