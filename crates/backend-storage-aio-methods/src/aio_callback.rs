...
pub fn pgaio_result_report(
    ...
    backend_storage_aio_completion_seams::pgaio_cb_report::call(cb_id, ioh_index as u32, elevel)
}
...
pub fn pgaio_io_call_stage(ioh_index: usize) -> PgResult<()> {
    ...
    while i > 0 {
        ...
        backend_storage_aio_completion_seams::pgaio_cb_stage::call(cb_id, ioh_index as u32)?;
        i -= 1;
    }
    Ok(())
}
...
pub fn pgaio_io_call_complete_shared(ioh_index: usize) -> PgResult<()> {
    ...
    while i > 0 {
        let cb_id = callbacks[i - 1];
        backend_storage_aio_completion_seams::pgaio_cb_complete_shared::call(
            cb_id,
            ioh_index as u32,
        )?;
...
pub fn pgaio_io_call_complete_local(ioh_index: usize) -> PgResult<PgAioResult> {
    ...
    while i > 0 {
        let cb_id = callbacks[i - 1];
        backend_storage_aio_completion_seams::pgaio_cb_complete_local::call(
            cb_id,
            ioh_index as u32,
        )?;
        debug_assert!(h.data().distilled_result.status != PgAioResultStatus::Unknown);
