...
use backend_storage_aio_methods::aio::{
    pgaio_io_get_distilled_result, pgaio_io_owner, pgaio_io_set_distilled_result,
};
use backend_storage_aio_methods::aio_callback::pgaio_io_get_handle_data;
use backend_storage_aio_methods::aio_target::pgaio_io_get_target_data;
use backend_storage_aio_methods::{PgAioResult, PgAioResultStatus, PgAioTargetData};
...
fn buffer_readv_complete_one(
    ...
    if !failed {
        ...
        if buffer_invalid || failed_checksum || zeroed_buffer {
            ...
            backend_utils_error::emit_error_report_for(
                &backend_utils_error::ereport(types_error::error::LOG_SERVER_ONLY)
    ...
    if is_temp {
        backend_storage_buffer_support_seams::terminate_local_buffer_io::call(
            buffer,
            false,
            set_flag_bits,
        )?;
...
fn buffer_readv_report(ioh: usize, elevel: i32) -> PgResult<()> {
    ...
    backend_utils_error::emit_error_report_for(
        &backend_utils_error::ereport(types_error::error::ErrorLevel(elevel))
            .errcode(types_error::error::ERRCODE_DATA_CORRUPTED)
            .errmsg_internal(msg)
            .into_error(),
    );
...
fn readv_cb_data(ioh: usize, cb_id: u8) -> u8 {
    backend_storage_aio_methods::aio_callback::pgaio_io_get_callback_data_for(ioh, cb_id)
}

/// The handle's wait reference (for the stage callback's io_wref stamp).
fn aio_wref_of(ioh: usize) -> PgAioWaitRef {
    let w = backend_storage_aio_methods::aio::pgaio_io_get_wref(ioh);
    PgAioWaitRef {
...
pub fn init_seams() {
    backend_storage_aio_completion_seams::pgaio_cb_stage::set(pgaio_cb_stage);
    backend_storage_aio_completion_seams::pgaio_cb_complete_shared::set(pgaio_cb_complete_shared);
    backend_storage_aio_completion_seams::pgaio_cb_complete_local::set(pgaio_cb_complete_local);
    backend_storage_aio_completion_seams::pgaio_cb_report::set(pgaio_cb_report);
    backend_storage_aio_completion_seams::pgaio_perform_io_syscall::set(pgaio_perform_io_syscall);
}
