...
//! [`backend_storage_smgr_smgr`] fork-size accessors. The per-buffer drop holds
...
impl BufferManager {
    ...
    pub fn DropRelationBuffers(
        ...
        if smgr_reln.backend != types_core::primitive::INVALID_PROC_NUMBER {
            if smgr_reln.backend == backend_storage_lmgr_proc_seams::my_proc_number::call() {
                backend_storage_buffer_bufmgr_seams::drop_relation_local_buffers::call(
                    smgr_reln.locator,
                    fork_num,
                    first_del_block,
                )?;
        ...
        for j in 0..nforks {
            ...
            let nblocks = backend_storage_smgr_smgr::smgrnblocks_cached(smgr_reln, fork_num[j]);
    ...
    pub fn DropRelationsAllBuffers(&self, smgr_reln: &[RelFileLocatorBackend]) -> PgResult<()> {
        ...
        for reln in smgr_reln.iter() {
            if reln.backend != types_core::primitive::INVALID_PROC_NUMBER {
                if reln.backend == backend_storage_lmgr_proc_seams::my_proc_number::call() {
                    backend_storage_buffer_bufmgr_seams::drop_relation_all_local_buffers::call(
                        reln.locator,
                    )?;
                }
            } else {
        ...
        'outer: while i < n {
            for j in 0..NFORKS {
                ...
                let nb = backend_storage_smgr_smgr::smgrnblocks_cached(rels[i], forknum);
                block[i][j] = nb;

                // We need to consider only the relation forks that exist.
                if nb == InvalidBlockNumber {
                    if !backend_storage_smgr_smgr::smgrexists(rels[i], forknum)? {
                        continue;
                    }
                    cached = false;
                    break 'outer;
