    ...
    pub fn UnlockBuffers(&self) {
        ...
        if buf_id >= 0 {
            ...
            if buf_state & BM_PIN_COUNT_WAITER != 0
                && self.wait_backend_pgprocno(buf_id) == backend_storage_lmgr_proc_seams::my_proc_number::call()
            {
        ...
        loop {
            ...
            if buf_state_get_refcount(buf_state) == 1 {
                ...
                if waiting {
                    ...
                    backend_utils_misc_ps_status_seams::set_ps_display_remove_suffix::call();
                    waiting = false;
                }
                let _ = waiting;
                return Ok(());
            ...
            if sb::in_hot_standby::call() {
                ...
                backend_storage_lmgr_proc_seams::proc_wait_for_signal::call(WAIT_EVENT_BUFFER_PIN)?;
            }
            ...
            != (BM_DIRTY | BM_JUST_DIRTIED)
        {
                ...
                && (self.read_state(buf_id) & BM_PERMANENT) != 0
            {
                ...
                if backend_access_transam_xlog_seams::recovery_in_progress::call()
                    || backend_catalog_storage_seams::rel_file_locator_skipping_wal::call(rlocator)
                {
                    return Ok(());
                }
                ...
                backend_storage_lmgr_proc_seams::my_proc_set_delay_chkpt_start::call(true);
                delay_chkpt_flags = true;
                lsn = backend_access_transam_xloginsert_seams::xlog_save_buffer_for_hint::call(
                    buffer, buffer_std,
                )?;
            }
            ...
            if delay_chkpt_flags {
                backend_storage_lmgr_proc_seams::my_proc_set_delay_chkpt_start::call(false);
            }
