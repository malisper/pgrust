...
fn enable_fsync() -> bool {
    backend_access_transam_xlog_seams::enable_fsync::call()
}

/// `wal_sync_method` (xlog.c GUC). Routed through the xlog owner's seam.
fn wal_sync_method() -> types_wal::WalSyncMethod {
    backend_access_transam_xlog_seams::wal_sync_method::call()
}
...
fn begin_startup_progress_phase() {
    backend_postmaster_startup_seams::begin_startup_progress_phase::call();
}
...
fn elog(level: ErrorLevel, message: String) {
    let _ = backend_utils_error_seams::ereport::call(PgError::new(level, message));
}
...
fn report_errno(level: ErrorLevel, message: String, errno: i32) -> PgResult<()> {
    ...
    if level >= ERROR {
        ...
        let _ = backend_utils_error_seams::ereport::call(err);
        Ok(())
    }
}
...
pub fn pg_fsync(file: &StdFile) -> PgResult<()> {
    ...
    if wal_sync_method() == types_wal::WalSyncMethod::FsyncWritethrough {
