...
fn max_live_postmaster_children() -> i32 {
    backend_postmaster_pmchild_seams::max_live_postmaster_children::call()
}
...
pub fn PostmasterIsAliveInternal() -> bool {
    ...
    let (rc, errno) = backend_postmaster_postmaster_seams::read_postmaster_death_watch::call();
...
pub fn PostmasterDeathSignalInit() -> PgResult<()> {
    ...
    port_pqsignal_seams::pqsignal::call(
        signum,
        types_signal::SigHandler::Handler(postmaster_death_handler),
    );

    // Request a signal on parent exit.
    backend_postmaster_postmaster_seams::request_parent_death_signal::call(signum)?;
