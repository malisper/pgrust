...
//!   [`backend_storage_ipc_waiteventset_seams::WaitEventSet`] guard naming a
...
pub fn AddWaitEventToSet(
    ...
    let pos = run_with_set(handle, |set| -> PgResult<i32> {
        ...
        if let Some(l) = latch {
            if backend_storage_ipc_latch_seams::latch_owner_pid::call(l) != my_proc_pid {
...
pub fn WaitEventSetWait(
    ...
    let result: PgResult<i32> = (|| {
        while returned_events == 0 {
            ...
            if let Some(l) = latch {
                if !backend_storage_ipc_latch_seams::latch_is_set::call(l) {
                    // about to sleep on a latch
                    backend_storage_ipc_latch_seams::set_latch_maybe_sleeping::call(l, true);
                    fence(SeqCst);
                    // and recheck
                }
            }
            ...
            if let Some(l) = latch {
                if backend_storage_ipc_latch_seams::latch_is_set::call(l) {
                    ...
                    backend_storage_ipc_latch_seams::set_latch_maybe_sleeping::call(l, false);
...
pub fn init_seams() {
    use backend_storage_ipc_waiteventset_seams as s;
