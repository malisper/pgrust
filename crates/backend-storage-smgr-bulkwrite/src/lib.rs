...
//! [`backend_storage_smgr_bulkwrite_seams::BulkWriteState`] carrier; the real
//! engine state is [`BulkWriteEngine`], which this crate boxes into it.

use backend_storage_page::{PageMut, PageSetChecksumInplace};
...
use backend_access_transam_xlog_seams::get_redo_rec_ptr as get_redo_rec_ptr_seam;
use backend_access_transam_xloginsert_seams::log_newpages as log_newpages_seam;
use backend_storage_lmgr_proc_seams::set_delay_chkpt_start as set_delay_chkpt_start_seam;
use backend_storage_smgr_bulkwrite_seams::{self as seam, BulkWriteState};
