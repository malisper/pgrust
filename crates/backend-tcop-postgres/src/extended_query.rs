...
use backend_utils_error::{ereport, errfinish, errhidestmt, errmsg, errstart};
...
// Seam crate aliases.
use backend_access_common_printtup_seams as printtup_seams;
use backend_access_transam_xact_seams as xact_seams;
use backend_nodes_params_seams as params_seams;
...
fn is_transaction_stmt_list(stmts: &[types_nodes::nodeindexscan::PlannedStmt<'_>]) -> bool {
    if stmts.len() == 1 {
        let pstmt = &stmts[0];
        if pstmt.commandType == types_nodes::nodes::CmdType::CMD_UTILITY {
...
fn is_transaction_exit_stmt_list(stmts: &[types_nodes::nodeindexscan::PlannedStmt<'_>]) -> bool {
    if stmts.len() == 1 {
        let pstmt = &stmts[0];
        if pstmt.commandType == types_nodes::nodes::CmdType::CMD_UTILITY {
            if let Some(u) = pstmt.utilityStmt.as_deref() {
...
pub fn exec_parse_message<'mcx>(
    ...
    if !parsetree_list.is_empty() {
        ...
        let command_tag =
            backend_tcop_utility_seams::create_command_tag::call(&raw_parse_tree.stmt)?;
        ...
        psrc = plancache_seams::create_cached_plan_empty::call(
            mcx,
            query_string,
            types_core::cmdtag::CommandTag(types_portal::CMDTAG_UNKNOWN),
        )?;
        ...
        for paramno in 0..num_params {
            ...
            let (value, isnull) = if pformat == 0 {
                ...
                if is_null {
                    ...
                    (types_tuple::backend_access_common_heaptuple::Datum::null(), true)
                } else {
...
pub fn exec_execute_message<'mcx>(
    ...
    let (source_text, is_xact_command, is_xact_exit, do_log_statement, portal_params) = {
        ...
        let stmts: &[types_nodes::nodeindexscan::PlannedStmt<'_>] =
