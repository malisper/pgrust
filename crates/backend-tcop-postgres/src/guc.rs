...
fn set_config_option(
    ...
    backend_utils_misc_guc_seams::set_config_option::call(name, value, context, source)
}
    ...
    while let Some(flag) = opt.next() {
        match flag {
            ...
            'b' => {
                ...
                if secure {
                    backend_utils_init_small::globals::SetIsBinaryUpgrade(true);
                }
            }
            ...
            '-' | 'c' => {
                ...
                if flag == '-'
                    && backend_main_main_seams::parse_dispatch_option::call(&opt.optarg_str())
            ...
            'r' => {
                ...
                if secure {
                    ...
                    backend_utils_init_small::globals::SetOutputFileNameStr(&opt.optarg_str());
                }
            }
            ...
            'v' => {
                ...
                if secure {
                    backend_utils_init_small::globals::SetFrontendProtocol(
...
fn progname() -> alloc::string::String {
    backend_tcop_postgres_seams::progname::call()
}
...
fn parse_long_option(string: &str) -> PgResult<(alloc::string::String, Option<alloc::string::String>)> {
    ...
    let (name, value) = backend_utils_misc_guc_seams::parse_long_option::call(ctx.mcx(), string)?;
    Ok((
...
fn assign_transaction_timeout(newval: i32, _extra: Option<&GucHookExtra>) {
    if backend_access_transam_xact_seams::is_transaction_state::call() {
        ...
        if newval > 0
            && !backend_utils_misc_timeout::get_timeout_active(types_timeout::TimeoutId::TRANSACTION_TIMEOUT)
        {
            // The C ignores enable_timeout_after's error surface (the assign hook
            // is `void`); a failure here would be a wiring bug.
            let _ = backend_utils_misc_timeout::enable_timeout_after(
            ...
            && backend_utils_misc_timeout::get_timeout_active(types_timeout::TimeoutId::TRANSACTION_TIMEOUT)
        {
            backend_utils_misc_timeout::disable_timeout(
                types_timeout::TimeoutId::TRANSACTION_TIMEOUT,
                false,
            );
        }
...
fn check_restrict_nonsystem_relation_kind(
    ...
    let mut flags = 0;
    for tok in &elemlist {
        if port_pgstrcasecmp::pg_strcasecmp(tok.as_bytes(), b"view") == 0 {
            flags |= RESTRICT_RELKIND_VIEW;
        } else if port_pgstrcasecmp::pg_strcasecmp(tok.as_bytes(), b"foreign-table") == 0 {
...
fn guc_check_errdetail(detail: &str) {
    backend_utils_misc_guc_seams::guc_check_errdetail::call(detail.to_string());
}
...
fn split_identifier_string(rawstring: &str) -> Option<Vec<alloc::string::String>> {
    backend_commands_user_seams::split_identifier_string::call(rawstring.to_string())
        .ok()
        .flatten()
}
