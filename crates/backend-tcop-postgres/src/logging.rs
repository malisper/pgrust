...
use backend_utils_error::{errdetail_internal, errfinish, errmsg_internal, errstart};
...
pub fn log_statement_is_all() -> bool {
    log_statement() == backend_utils_misc_guc_tables::consts::LOGSTMT_ALL
}
...
pub fn log_executor_stats() -> bool {
    backend_utils_misc_guc_tables::vars::log_executor_stats.read()
}

fn log_duration() -> bool {
    backend_utils_misc_guc_tables::vars::log_duration.read()
}

fn log_min_duration_sample() -> i32 {
    backend_utils_misc_guc_tables::vars::log_min_duration_sample.read()
}

fn log_min_duration_statement() -> i32 {
    backend_utils_misc_guc_tables::vars::log_min_duration_statement.read()
}

fn log_statement_sample_rate() -> f64 {
    backend_utils_misc_guc_tables::vars::log_statement_sample_rate.read()
}

fn statement_timeout() -> i32 {
    backend_utils_misc_guc_tables::vars::StatementTimeout.read()
}
...
pub fn check_log_duration<'mcx>(mcx: Mcx<'mcx>, was_logged: bool) -> PgResult<(i32, PgString<'mcx>)> {
    let xact_is_sampled = backend_access_transam_xact::xact_is_sampled();
        ...
        || xact_is_sampled
    {
        ...
        let start = backend_access_transam_xact_seams::get_current_statement_start_timestamp::call();
        let stop = backend_utils_adt_timestamp_seams::get_current_timestamp::call();
        let (secs, usecs) = backend_utils_adt_timestamp_seams::timestamp_difference::call(start, stop);
...
fn pg_prng_double_global() -> f64 {
    pg_prng::global_prng(|p| p.next_f64())
}
...
pub fn log_disconnections() -> PgResult<()> {
    ...
    let stop = backend_utils_adt_timestamp_seams::get_current_timestamp::call();
    let (mut secs, usecs) = backend_utils_adt_timestamp_seams::timestamp_difference::call(start, stop);
    let msecs = usecs / 1000;
    ...
    if errstart(LOG, None) {
        ...
        backend_utils_error::errmsg(&alloc::format!(
...
pub fn enable_statement_timeout() -> PgResult<()> {
    ...
    if stmt > 0 && (stmt < xact || xact == 0) {
        if !backend_utils_misc_timeout::get_timeout_active(TimeoutId::STATEMENT_TIMEOUT) {
            backend_utils_misc_timeout::enable_timeout_after(TimeoutId::STATEMENT_TIMEOUT, stmt)?;
        }
    } else if backend_utils_misc_timeout::get_timeout_active(TimeoutId::STATEMENT_TIMEOUT) {
        backend_utils_misc_timeout::disable_timeout(TimeoutId::STATEMENT_TIMEOUT, false);
    }
    Ok(())
}
