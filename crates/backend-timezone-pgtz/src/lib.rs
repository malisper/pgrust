...
use types_pgtime::pgtime::state;
use types_pgtime::{pg_tz, TZ_STRLEN_MAX};
...
fn scan_directory_ci(dirname: &str, fname: &str) -> Option<String> {
    let entries = backend_storage_file_fd_seams::read_dir_names_logged::call(dirname);
...
pub fn pg_tzenumerate_next(dir: &mut PgTzEnum) -> PgResult<Option<&pg_tz>> {
    while dir.depth >= 0 {
        ...
        if backend_storage_file_fd_seams::get_dirent_type::call(&fullname) == PGFILETYPE_DIR {
...
fn open_dir(name: &str) -> PgResult<Vec<String>> {
    backend_storage_file_fd_seams::read_dir_names::call(name)
}
...
pub fn init_seams() {
    backend_timezone_pgtz_seams::pg_open_tzfile::set(pg_open_tzfile);
}
