...
//! **index into the carrying [`DictThesaurus`](types_tsearch::DictThesaurus)'s
//! `arena`** (`Option<usize>`), reproducing `findVariant` / `matchIdSubst` /
//! `checkMatch` 1:1; the cross-call state is carried in
//! [`ThesaurusSubState`](types_tsearch::ThesaurusSubState). No raw pointers, no
...
//! ([`backend_tsearch_dict_seams::subdict_lexize`] /
//! [`backend_tsearch_dict_seams::get_ts_dict_oid_from_name`]); their owners
...
/// debug function.
pub fn init_seams() {
    backend_tsearch_dict_seams::dsimple_init::set(dict_simple::dsimple_init);
    backend_tsearch_dict_seams::dsimple_lexize::set(dict_simple::dsimple_lexize);
    backend_tsearch_dict_seams::dsynonym_init::set(dict_synonym::dsynonym_init);
    backend_tsearch_dict_seams::dsynonym_lexize::set(dict_synonym::dsynonym_lexize);
