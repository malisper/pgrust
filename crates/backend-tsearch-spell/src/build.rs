...
use types_regex::{RegcompResult, REG_ADVANCED, REG_NOSUB};
...
impl<'mcx> IspellDict<'mcx> {
    ...
    fn ni_add_affix(
        ...
        let reg = if mask == b"." || mask.is_empty() {
            ...
            let wmask = backend_utils_mb_mbutils_seams::pg_mb2wchar_with_len::call(mcx, &tmask)?;
            match backend_regex_core_seams::pg_regcomp::call(
                &wmask,
...
fn open_file(filename: &[u8], kind: &str) -> PgResult<Vec<u8>> {
    backend_tsearch_ts_locale_seams::readfile::call(filename).map_err(|errno| {
