...
use backend_tsearch_parse::{ParsedText, ParsedWord};
use backend_utils_adt_tsvector_core::access::{
    set_tsv_size, set_varsize, shortalign, strptr_off, SIZEOF_NPOS, SIZEOF_WEP, SIZEOF_WORDENTRY,
};
use types_tsearch::tsearch::DATAHDRSIZE;
use backend_utils_error::ereport;
use types_error::{PgResult, ERRCODE_PROGRAM_LIMIT_EXCEEDED};
use types_tsearch::tsearch::{WordEntry, WordEntryPos, LIMITPOS, MAXENTRYPOS, MAXNUMPOS, MAXSTRPOS};
    ...
    for (i, w) in words.iter().enumerate() {
        ...
        if count > 0 {
            ...
            for j in 0..count {
                ...
                types_tsearch::tsearch::WEP_SETWEIGHT(&mut wep, 0);
                types_tsearch::tsearch::WEP_SETPOS(&mut wep, apos[i][j + 1]);
