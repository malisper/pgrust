...
use backend_utils_init_small_seams::my_database_id;
...
pub fn pgstat_init_function_usage(fn_stats: u8, fn_oid: Oid) -> PgResult<PgStat_FunctionCallUsage> {
    ...
    if created_entry {
        ...
        if !backend_utils_cache_syscache_seams::procoid_exists::call(fn_oid)? {
...
fn pgstat_track_functions() -> i32 {
    backend_utils_misc_guc_tables::vars::pgstat_track_functions.read()
}
