...
pub fn pgstat_fetch_slru() -> PgResult<[PgStat_SLRUStats; SLRU_NUM_ELEMENTS]> {
    backend_utils_activity_pgstat_seams::snapshot_fixed::call(PGSTAT_KIND_SLRU)?;
    Ok(backend_utils_activity_pgstat::local::with_local(|l| l.snapshot.slru))
}
...
pub fn pgstat_slru_flush_cb(nowait: bool) -> PgResult<bool> {
    ...
    backend_utils_activity_pgstat::local::with_local(|l| {
        ...
        if !nowait {
            ...
            match backend_storage_lmgr_lwlock_seams::lwlock_conditional_acquire::call(
                &stats_shmem.lock,
                LW_EXCLUSIVE,
            )? {
                Some(g) => guard = g,
...
pub fn init_seams() {
    ...
    backend_utils_activity_stat_seams::pgstat_get_slru_index::set(pgstat_get_slru_index);
    backend_utils_activity_stat_seams::pgstat_count_slru_page_zeroed::set(
        pgstat_count_slru_page_zeroed,
    );
    backend_utils_activity_stat_seams::pgstat_count_slru_page_hit::set(pgstat_count_slru_page_hit);
    backend_utils_activity_stat_seams::pgstat_count_slru_page_read::set(pgstat_count_slru_page_read);
    backend_utils_activity_stat_seams::pgstat_count_slru_page_written::set(
        pgstat_count_slru_page_written,
    );
    backend_utils_activity_stat_seams::pgstat_count_slru_page_exists::set(
        pgstat_count_slru_page_exists,
    );
    backend_utils_activity_stat_seams::pgstat_count_slru_flush::set(pgstat_count_slru_flush);
    backend_utils_activity_stat_seams::pgstat_count_slru_truncate::set(pgstat_count_slru_truncate);
}
