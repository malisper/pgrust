...
use backend_executor_instrument::WalUsageAccumDiff;
...
pub fn pgstat_report_wal(force: bool) -> PgResult<()> {
    ...
    let _ = backend_utils_activity_pgstat_wal_seams::pgstat_flush_backend_wal::call(nowait);

    // flush IO stats
    backend_utils_activity_stat_seams::pgstat_flush_io::call(nowait)?;
    ...
    let _ = backend_utils_activity_pgstat_io_seams::pgstat_flush_backend_io::call(nowait);
...
pub fn pgstat_fetch_stat_wal() -> PgResult<types_pgstat::activity_pgstat::PgStat_WalStats> {
    backend_utils_activity_pgstat_seams::snapshot_fixed::call(PGSTAT_KIND_WAL)?;
    Ok(backend_utils_activity_pgstat::local::with_local(|l| l.snapshot.wal))
}
...
pub fn init_seams() {
    ...
    backend_utils_activity_walstats_seams::pgstat_report_wal::set(pgstat_report_wal);
    backend_utils_activity_pgstat_wal_seams::pgstat_report_wal::set(|force| {
