...
//! * `ArrayAnalyzeExtraData` lives in `types_statistics`; `cmp` / `hash` carry
...
use backend_access_common_detoast_seams::toast_raw_datum_size;
use backend_access_heap_vacuumlazy_seams::vacuum_delay_point;
use backend_commands_analyze_seams::{std_compute_stats, std_typanalyze};
use backend_commands_vacuum_seams::check_for_interrupts;
use backend_utils_adt_arrayfuncs_seams::deconstruct_array_v;
...
pub fn init_seams() {
    backend_utils_adt_array_typanalyze_seams::array_typanalyze::set(array_typanalyze);
}
