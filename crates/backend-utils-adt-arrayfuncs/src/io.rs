...
pub fn array_in<'mcx>(
    ...
    let meta = lsyscache::get_type_io_data::call(element_type, ArrayIoFuncSelector::Input)?;
...
pub fn array_out<'mcx>(mcx: Mcx<'mcx>, array: &[u8]) -> PgResult<PgVec<'mcx, u8>> {
    ...
    let meta = lsyscache::get_type_io_data::call(element_type, ArrayIoFuncSelector::Output)?;
    let typlen = meta.typlen as i32;
    let typbyval = meta.typbyval;
    let typalign = meta.typalign;
    let typdelim = meta.typdelim;
...
pub fn array_recv<'mcx>(
    ...
    let meta = lsyscache::get_type_io_data::call(element_type, ArrayIoFuncSelector::Receive)?;
    if meta.typiofunc == 0 {
...
pub fn array_send<'mcx>(mcx: Mcx<'mcx>, array: &[u8]) -> PgResult<PgVec<'mcx, u8>> {
    ...
    let meta = lsyscache::get_type_io_data::call(element_type, ArrayIoFuncSelector::Send)?;
    if meta.typiofunc == 0 {
        let name = format_type::format_type_be::call(mcx, element_type)?;
        return Err(PgError::error(format!(
