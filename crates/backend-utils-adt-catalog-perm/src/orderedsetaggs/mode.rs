...
pub fn fc_mode_final(fcinfo: &mut FunctionCallInfoBaseData) -> Word {
    ...
    if osastate.qstate.equal_fn_oid == 0 {
        let fn_oid = ok(backend_utils_cache_lsyscache_seams::get_opcode::call(
...
impl HeldVal {
    fn from_cdatum(d: &types_tuple::backend_access_common_heaptuple::Datum<'_>, by_val: bool) -> Self {
    ...
    fn into_cdatum<'mcx>(self) -> types_tuple::backend_access_common_heaptuple::Datum<'mcx> {
        use types_tuple::backend_access_common_heaptuple::Datum as CDatum;
        match self {
