...
//! from `backend_utils_adt_numutils` (`pg_ultostr_zeropad`/`pg_ultostr`), which
...
/// buffer form `backend_utils_adt_numutils::pg_ultostr_zeropad`. A `u32`
...
fn pg_ultostr_zeropad(value: u32, minwidth: i32) -> String {
    let mut buf = [0u8; 32];
    let n = backend_utils_adt_numutils::pg_ultostr_zeropad(&mut buf, value, minwidth);
...
fn pg_ultostr(value: u32) -> String {
    let mut buf = [0u8; 32];
    let n = backend_utils_adt_numutils::pg_ultostr(&mut buf, value);
    String::from_utf8_lossy(&buf[..n]).into_owned()
}
