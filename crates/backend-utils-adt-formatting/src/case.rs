...
//! `pg_locale.c`) go through the `backend_utils_adt_pg_locale_seams` slots, and
//! `GetDatabaseEncoding` (`mbutils.c`) through `backend_utils_mb_mbutils_seams`.
...
fn pg_newlocale_from_collation<'mcx>(mcx: Mcx<'mcx>, collid: Oid) -> PgResult<PgLocale<'mcx>> {
    backend_utils_adt_pg_locale_seams::pg_newlocale_from_collation::call(mcx, collid)
}

/// C: `GetDatabaseEncoding` via the mbutils seam.
fn get_database_encoding() -> i32 {
    backend_utils_mb_mbutils_seams::get_database_encoding::call()
}
