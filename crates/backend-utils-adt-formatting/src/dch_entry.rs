...
//! centralized `seams::backend_utils_adt_formatting` slots: `timestamp2tm`,
...
fn timestamp2tm(
    ...
    backend_utils_adt_timestamp_seams::timestamp2tm::call(dt, want_tz)
}
fn tm2timestamp(tm: &pg_tm, fsec: fsec_t, tz: Option<i32>) -> Result<Timestamp, ()> {
    backend_utils_adt_timestamp_seams::tm2timestamp::call(tm, fsec, tz)
}
fn interval2itm(span: Interval) -> types_datetime::pg_itm {
    backend_utils_adt_timestamp_seams::interval2itm::call(span)
}
fn tm2time(tm: &pg_tm, fsec: fsec_t) -> TimeADT {
    backend_utils_adt_timestamp_seams::tm2time::call(tm, fsec)
}
fn tm2timetz(tm: &pg_tm, fsec: fsec_t, tz: i32) -> TimeTzADT {
    backend_utils_adt_timestamp_seams::tm2timetz::call(tm, fsec, tz)
}
fn adjust_timestamp_for_typmod(value: Timestamp, typmod: i32) -> PgResult<Timestamp> {
    backend_utils_adt_timestamp_seams::adjust_timestamp_for_typmod::call(value, typmod)
}
...
fn isoweekdate2date_seam(isoweek: i32, wday: i32, year: i32) -> YmdDate {
    backend_utils_adt_isoweek_seams::isoweekdate2date::call(isoweek, wday, year)
}
