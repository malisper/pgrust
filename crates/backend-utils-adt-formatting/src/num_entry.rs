...
//! `seams::backend_utils_adt_formatting` slots: `numeric_in_var`,
...
pub fn numeric_to_number<'mcx>(
    ...
    let packed = backend_utils_adt_numeric::io::numeric_in(
        mcx,
        &s,
        make_numeric_typmod(precision, scale),
    )?;
    let mut var = backend_utils_adt_numeric::convert::set_var_from_num(mcx, &packed)?;

    if num.is_multi() {
        // result *= 10^(-multi)
        let a = backend_utils_adt_numeric::kernel_transcendental::int64_to_numericvar(mcx, 10)?;
        let b = backend_utils_adt_numeric::kernel_transcendental::int64_to_numericvar(
            mcx,
            -(num.multi as i64),
        )?;
        let x = backend_utils_adt_numeric::kernel_transcendental::power_var(mcx, &a, &b)?;
        let rscale = select_mul_rscale(&var, &x);
        var = backend_utils_adt_numeric::kernel_var::mul_var(mcx, &var, &x, rscale)?;
    }
...
pub fn numeric_to_char<'mcx>(
    ...
    if num.is_roman() {
        ...
        if num.is_multi() {
            ...
            val = backend_utils_adt_numeric::kernel_var::mul_var(mcx, value, &x, rscale)?;
            num.pre += num.multi;
        }
...
pub fn int8_to_char<'mcx>(
    ...
    if num.is_roman() {
        ...
        let v = backend_utils_adt_numeric::kernel_transcendental::int64_to_numericvar(mcx, value)?;
...
fn numericvar_to_int4_opt(value: &NumericVar) -> PgResult<Option<i32>> {
    ...
    backend_utils_adt_numeric::kernel_var::round_var(&mut rounded, 0);
    backend_utils_adt_numeric::convert::numericvar_to_int32(&rounded)
}
