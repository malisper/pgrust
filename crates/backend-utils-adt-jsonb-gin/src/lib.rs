...
//! [`backend_utils_adt_jsonb_util`]; the `jsonpath` node-tree traversal from
//! [`backend_utils_adt_jsonpath`]; numeric rendering (`numeric_normalize`) is
//! ported in-crate on top of [`backend_utils_adt_numeric`]; the C-collation text
//! comparison (`gin_compare_jsonb`) reduces to a plain byte compare; overlength
//! hashing uses [`common_hashfn`].
//!
...
//! [`backend_utils_adt_jsonb_gin_seams`] from [`init_seams`]. A future GIN
...
/// [`backend_utils_adt_jsonb_gin_seams`]. The future GIN by-OID dispatcher
/// routes the support-proc OIDs to these.
pub fn init_seams() {
    use backend_utils_adt_jsonb_gin_seams as seams;
...
pub fn make_text_key(mut flag: u8, str_in: &[u8]) -> Vec<u8> {
    ...
    if str_bytes.len() as i32 > JGIN_MAXLENGTH {
        ...
        let hashval = common_hashfn::hash_bytes(str_bytes);
...
/// [`backend_utils_adt_numeric`] (`set_var_from_num` + `get_str_from_var`); the
...
fn numeric_normalize<'mcx>(mcx: Mcx<'mcx>, num: &[u8]) -> PgResult<String> {
    ...
    use backend_utils_adt_numeric::io::get_str_from_var;
    use backend_utils_adt_numeric::on_disk::var::NumericSign;
