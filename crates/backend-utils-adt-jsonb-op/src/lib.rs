...
//! [`backend_utils_adt_jsonb_util`], reached through a normal path dependency
...
//! `backend_utils_adt_jsonb_util`.  The `?|` / `?&` operators receive their
...
pub fn jsonb_exists_any(jb_root: &[u8], keys: &[u8]) -> PgResult<bool> {
    let key_elems = backend_utils_adt_array_more_seams::deconstruct_text_array::call(keys)?;
