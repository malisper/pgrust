...
//! `JsonbIterator`) live in [`types_jsonb::backend_utils_adt_jsonb_util`]: they
...
pub use types_jsonb::backend_utils_adt_jsonb_util::{
...
fn convertJsonbScalar<'mcx>(
    ...
    match &scalar_val.val {
        ...
        JsonbValueData::Datetime(dt) => {
            ...
            //
            // Datum-unification edge: `json_encode_datetime` is the json/
            // timestamp owner's seam, whose contract still carries the
            // bare-word `types_datum::Datum` shim (not yet migrated to the
            // canonical `&Datum<'mcx>` enum on this tip). `dt.value` is a
            // by-value datetime word (timestamp/date int), not a by-reference
            // payload, so it is forwarded as a plain word at this fixed seam
    ...
    if a.typ == b.typ {
        return match a.typ {
            ...
            jbvNumeric => {
                ...
                Ok(backend_utils_adt_numeric_seams::numeric_eq::call(
...
fn compareJsonbScalarValue(a: &JsonbValue, b: &JsonbValue) -> PgResult<i32> {
    if a.typ == b.typ {
        return match a.typ {
            jbvNull => Ok(0),
            jbvString => backend_utils_adt_varlena_seams::varstr_cmp::call(
                jsonb_string_bytes(a),
                jsonb_string_bytes(b),
                DEFAULT_COLLATION_OID,
            ),
            jbvNumeric => Ok(backend_utils_adt_numeric_seams::numeric_cmp::call(
                jsonb_numeric_bytes(a),
                jsonb_numeric_bytes(b),
            )),
            jbvBool => {
...
fn hash_numeric(num: &[u8]) -> u32 {
    ...
    let digit_hash = common_hashfn_seams::hash_bytes::call(&digit_bytes[byte_start..byte_end]);
...
fn hash_numeric_extended(num: &[u8], seed: u64) -> u64 {
    ...
    let byte_start = start_offset * 2;
    let byte_end = byte_start + hash_len * 2;
    let digit_hash = common_hashfn_seams::hash_bytes_extended::call(&digit_bytes[byte_start..byte_end], seed);
...
pub fn JsonbHashScalarValue(scalar_val: &JsonbValue, hash: &mut u32) -> PgResult<()> {
    let tmp: u32 = match scalar_val.typ {
        jbvNull => 0x01,
        jbvString => common_hashfn_seams::hash_bytes::call(jsonb_string_bytes(scalar_val)),
...
pub fn JsonbHashScalarValueExtended(
    ...
    let tmp: u64 = match scalar_val.typ {
        jbvNull => seed + 0x01,
        jbvString => common_hashfn_seams::hash_bytes_extended::call(jsonb_string_bytes(scalar_val), seed),
        ...
        jbvBool => {
            ...
            if seed != 0 {
                ...
                common_hashfn_seams::hash_bytes_uint32_extended::call(b as u32, seed)
            } else if b {
                0x02
            } else {
                0x04
