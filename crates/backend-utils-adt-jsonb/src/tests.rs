...
use super::*;
use backend_utils_adt_jsonb_util as jbu;
...
fn install_seams() {
    INSTALL.call_once(|| {
        ...
        backend_utils_adt_jsonb_seams::numeric_int4::set(|num| {
            let f = backend_utils_adt_numeric::convert::numeric_to_float8(num)?;
            Ok(f as i32)
        });
    });
}
