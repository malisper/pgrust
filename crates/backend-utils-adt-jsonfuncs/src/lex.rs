...
pub fn json_errsave_error(
    ...
    {
        // jsonfuncs.c:647-651
        let detail = common_jsonapi_seams::json_errdetail::call(error, lex)?;
...
fn report_json_context(lex: &JsonLexContext) -> PgResult<String> {
    ...
    while context_end - context_start >= 50 {
        ...
        if (lex.byte_at(context_start) & HIGHBIT) != 0 {
            ...
            context_start += common_jsonapi_seams::pg_mblen::call(&lex.input[context_start..]);
        } else {
            context_start += 1;
        }
    }
...
pub fn json_get_first_token(json: &[u8], throw_error: bool) -> PgResult<JsonTokenType> {
    ...
    let (result, lex) = common_jsonapi_seams::json_lex_first::call(json, encoding)?;
