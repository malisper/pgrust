...
// `backend_utils_adt_jsonpath::JsonPathItemType`.
use backend_utils_adt_jsonpath::JsonPathItemType;
...
pub fn parse_datetime(
    ...
    use backend_utils_adt_formatting::ParseDatetimeResult as R;
    ...
    let parsed = backend_utils_adt_formatting::parse_datetime(
        mcx,
        datetime,
        template,
        collid,
...
fn cast_time_to_timetz(time: TimeADT, use_tz: bool) -> PgResult<TimeTzADT> {
    ...
    Ok(backend_utils_adt_datetime::date::time_timetz(time))
}
...
fn cmp_date_to_timestamp(date1: DateADT, ts2: Timestamp, _use_tz: bool) -> i32 {
    backend_utils_adt_datetime::date::date_cmp_timestamp_internal(date1, ts2)
}
...
fn cmp_date_to_timestamptz(date1: DateADT, tstz2: TimestampTz, use_tz: bool) -> PgResult<i32> {
    ...
    Ok(backend_utils_adt_datetime::date::date_cmp_timestamptz_internal(
        date1, tstz2,
    ))
}
...
fn cmp_timestamp_to_timestamptz(ts1: Timestamp, tstz2: TimestampTz, use_tz: bool) -> PgResult<i32> {
    ...
    Ok(
        backend_utils_adt_datetime::timestamp::timestamp_cmp_timestamptz_internal(ts1, tstz2),
    )
}
...
pub fn compare_datetime(val1: DtOperand, val2: DtOperand, use_tz: bool) -> PgResult<Option<i32>> {
    use backend_utils_adt_datetime::date::date_cmp;
    use backend_utils_adt_datetime::time::time_cmp;
    use backend_utils_adt_datetime::timestamp::timestamp_cmp;
    use backend_utils_adt_datetime::timetz::timetz_cmp;
        ...
        jpiTimestampTz => {
            match typid {
                DATEOID => {
                    ...
                    let (y, m, d) =
                        backend_utils_adt_datetime::calendar::j2date(date + POSTGRES_EPOCH_JDATE);
                    let mut tm = pg_tm {
                        tm_year: y,
                        tm_mon: m,
                        tm_mday: d,
                    ...
                    tz = backend_utils_adt_datetime::decode::DetermineTimeZoneOffset(
                        &mut tm,
                ...
                TIMESTAMPOID => {
                    ...
                    if backend_utils_adt_datetime::timestamp::timestamp2tm(
