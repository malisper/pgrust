...
//!
//! The token stream is produced by [`backend_utils_adt_jsonpath_scan`]; the
...
fn numeric_in_bytes(text: &[u8]) -> PgResult<Vec<u8>> {
    ...
    let v = backend_utils_adt_numeric::io::numeric_in(cx.mcx(), &s, -1)?;
...
fn numeric_uminus_bytes(num: &[u8]) -> PgResult<Vec<u8>> {
    ...
    let v = backend_utils_adt_numeric::ops_sql::numeric_uminus(cx.mcx(), num)?;
    Ok(v.as_slice().to_vec())
}
    ...
    fn parse_any_level(&mut self) -> POut<i32> {
        match self.peek_tok() {
            Some(Token::INT_P) => {
                ...
                let n = backend_utils_adt_numutils::pg_strtoint32(&text)?;
                Ok(Some(n))
            }
            Some(Token::LAST_P) => {
                self.idx += 1;
