...
fn convert_regex_flags_default_advanced() {
    ...
    use backend_regex_core::regex_consts::{REG_ADVANCED, REG_NLSTOP};
    assert_eq!(jspConvertRegexFlags(0).unwrap(), REG_ADVANCED | REG_NLSTOP);
}

#[test]
fn convert_regex_flags_icase_dotall_mline() {
    use backend_regex_core::regex_consts::{REG_ADVANCED, REG_ICASE, REG_NLANCH};
...
fn convert_regex_flags_quote_overrides() {
    use backend_regex_core::regex_consts::{REG_ADVANCED, REG_QUOTE};
