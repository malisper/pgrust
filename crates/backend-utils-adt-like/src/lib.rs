...
use backend_utils_adt_pg_locale_seams as locale_seam;
use backend_utils_mb_mbutils_seams as mb_seam;
use mcx::{Mcx, PgVec};
use port_pgstrcasecmp::{pg_ascii_tolower, pg_tolower};
...
pub fn Generic_Text_IC_like(str: &[u8], pat: &[u8], collation: Oid, mcx: Mcx<'_>) -> PgResult<i32> {
    ...
    if mb_seam::pg_database_encoding_max_length::call() > 1 || locale.provider == CollProvider::Icu
    {
        ...
        let lpat = backend_utils_adt_formatting::case::str_tolower(mcx, pat, collation)?;
        let lstr = backend_utils_adt_formatting::case::str_tolower(mcx, str, collation)?;
...
pub fn nameiclike(str: &[u8], pat: &[u8], collation: Oid, mcx: Mcx<'_>) -> PgResult<bool> {
    ...
    let strtext = backend_utils_adt_varlena::wire_io::name_text(mcx, str)?;
