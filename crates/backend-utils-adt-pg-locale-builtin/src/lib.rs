...
//! (`common/unicode_case.c` → [`common_unicode_case`]) and word boundaries use
//! the Unicode category tables (`common/unicode_category.c` →
//! [`common_unicode_category`]). The only genuinely-external input is the
...
//! trimmed [`types_locale::PgLocaleStruct`]; the `info.builtin.casemap_full`
...
use backend_utils_adt_pg_locale_builtin_seams::{self as seams, PgLocaleBuiltinResult};
