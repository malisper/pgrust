...
fn arg_text(fcinfo: &FunctionCallInfoBaseData, i: usize) -> String {
    ...
    String::from_utf8_lossy(&image[types_datum::varlena::VARHDRSZ..]).into_owned()
}
...
fn fc_pg_backend_pid(_fc: &mut FunctionCallInfoBaseData) -> PgResult<Datum> {
    Ok(ret_i32(backend_utils_init_small_seams::my_proc_pid::call()))
}
...
fn fc_pg_stat_reset_shared(fc: &mut FunctionCallInfoBaseData) -> PgResult<Datum> {
    if arg_isnull(fc, 0) {
        ...
        backend_access_transam_xlogprefetcher::XLogPrefetchResetStats();
    ...
    match target.as_str() {
        ...
        "recovery_prefetch" => backend_access_transam_xlogprefetcher::XLogPrefetchResetStats(),
...
fn fc_pg_stat_reset_single_table_counters(fc: &mut FunctionCallInfoBaseData) -> PgResult<Datum> {
    ...
    let dboid = if backend_catalog_catalog_seams::is_shared_relation::call(taboid) {
...
fn fc_pg_stat_reset_single_function_counters(fc: &mut FunctionCallInfoBaseData) -> PgResult<Datum> {
    ...
    let dboid = backend_utils_init_small_seams::my_database_id::call();
...
fn fc_pg_stat_reset_slru(fc: &mut FunctionCallInfoBaseData) -> PgResult<Datum> {
    if arg_isnull(fc, 0) {
        ...
        let target = arg_text(fc, 0);
        backend_utils_activity_pgstat_slru::pgstat_reset_slru(&target)?;
    }
    Ok(ret_void())
}
...
fn fc_pg_stat_reset_replication_slot(fc: &mut FunctionCallInfoBaseData) -> PgResult<Datum> {
    if arg_isnull(fc, 0) {
        ...
        let target = arg_text(fc, 0);
        backend_utils_activity_pgstat_replslot::pgstat_reset_replslot(&target)?;
    }
    Ok(ret_void())
}
