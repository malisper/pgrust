...
fn install_mb_mocks() {
    INSTALL_MB.call_once(|| {
        backend_utils_mb_mbutils_seams::pg_client_to_server::set(mock_no_conversion);
        backend_utils_mb_mbutils_seams::pg_server_to_client::set(mock_no_conversion);
    });
}
...
fn void_send_is_empty_payload() {
    ...
    assert_eq!(bytea.as_bytes().len(), types_datum::VARHDRSZ);
}
