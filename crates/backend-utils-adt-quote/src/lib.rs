...
//! ([`backend_utils_adt_ruleutils_seams::quote_identifier`]); it panics loudly
...
/// seam** ([`backend_utils_adt_quote_seams::quote_literal_cstr`]): the result is
...
pub fn quote_ident<'mcx>(mcx: Mcx<'mcx>, t: &str) -> PgResult<PgVec<'mcx, u8>> {
    let qstr = backend_utils_adt_ruleutils_seams::quote_identifier::call(mcx, t)?;
