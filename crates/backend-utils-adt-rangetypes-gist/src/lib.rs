...
//! `GistEntryVector` / `GIST_SPLITVEC` are the real `types_gist` carriers; the
...
use backend_utils_adt_rangetypes::range_canonical_subdiff_hash::range_subdiff;
use backend_utils_adt_rangetypes::range_repr_serialize::{
    make_range, range_deserialize, range_get_flags, range_set_contain_empty,
};

use backend_utils_adt_multirangetypes::operators::{
...
use backend_utils_adt_multirangetypes::serialize_core::{
    multirange_get_bounds, multirange_get_union_range,
};
use backend_utils_adt_multirangetypes::typcache_io::multirange_get_typcache;

use backend_utils_adt_float::get_float4_infinity;
...
fn range_type_p_get_datum<'mcx>(
    ...
    types_tuple::backend_access_common_heaptuple::Datum::from_usize(r.ptr as usize)
}
