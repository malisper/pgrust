...
pub fn ri_init_hash_tables() -> PgResult<()> {
    ...
    backend_catalog_pg_constraint_seams::register_constraint_inval_callback::call()?;
...
pub fn ri_fetch_constraint_info(
    ...
    let constraint_oid = backend_commands_trigger_seams::trigger_constraint::call(trigger);

    if constraint_oid == InvalidOid {
        let tgname = String::from_utf8_lossy(
            &backend_commands_trigger_seams::trigger_name::call(mcx, trigger)?,
        )
        .into_owned();
    ...
    let tgconstrrelid = backend_commands_trigger_seams::trigger_constrrelid::call(trigger);
...
pub fn ri_load_constraint_info(mcx: Mcx<'_>, constraint_oid: Oid) -> PgResult<RiConstraintInfo> {
    ...
    let row = match backend_catalog_pg_constraint_seams::load_fk_constraint::call(
        mcx,
        constraint_oid,
    )? {
        Some(row) => row,
    ...
    if row.conparentid != InvalidOid {
        riinfo.constraint_root_id =
            backend_catalog_pg_constraint_seams::get_ri_constraint_root::call(row.conparentid)?;
    } else {
        riinfo.constraint_root_id = constraint_oid;
    }
    riinfo.oid_hash_value = row.oid_hash_value;
    riinfo.root_hash_value =
        backend_catalog_pg_constraint_seams::constraint_hash_value::call(riinfo.constraint_root_id)?;
...
pub fn ri_fetch_prepared_plan(key: &RiQueryKey) -> PgResult<Option<SpiPlanPtr>> {
    ...
    if backend_executor_spi_seams::spi_plan_is_valid::call(plan) {
        return Ok(Some(plan));
    }
    ...
    backend_executor_spi_seams::spi_freeplan::call(plan)?;
