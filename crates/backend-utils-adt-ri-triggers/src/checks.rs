...
pub fn ri_check_trigger(trigdata: TriggerDataRef, funcname: &str, tgkind: i32) -> PgResult<()> {
    if !backend_commands_trigger_seams::called_as_trigger::call(trigdata) {
    ...
    let event = backend_commands_trigger_seams::tg_event::call(trigdata);
...
pub fn ri_plan_check(
    ...
    let qplan = backend_executor_spi_seams::spi_prepare::call(querystr, argtypes)?;

    let qplan = match qplan {
        Some(p) => p,
        None => {
            let code = backend_executor_spi_seams::spi_result_code_string::call(mcx, -1)?;
            backend_utils_init_miscinit_seams::set_user_id_and_sec_context::call(
                save_userid,
                save_sec_context,
            );
    ...
    backend_executor_spi_seams::spi_keepplan::call(qplan)?;
...
pub fn ri_perform_check(
    ...
    let (test_snapshot, crosscheck_snapshot) =
        if backend_access_transam_xact_seams::isolation_uses_xact_snapshot::call() && detect_new_rows
        {
            backend_access_transam_xact_seams::command_counter_increment::call()?;
            (
                Some(backend_utils_time_snapmgr_seams::get_latest_snapshot::call()?),
                Some(backend_utils_time_snapmgr_seams::get_transaction_snapshot::call()?),
            )
        } else {
            (None, None)
        };
    ...
    let exec = backend_executor_spi_seams::spi_execute_snapshot::call(
        qplan,
        &vals[..nparams],
        &nulls[..nparams],
        test_snapshot,
    ...
    if spi_result < 0 {
        let code = backend_executor_spi_seams::spi_result_code_string::call(mcx, spi_result)?;
        return Err(PgError::error(format!(
    ...
    let (key_names, key_values) = if has_perm {
        ...
        let columns = match source {
            ViolatorSource::Slot(slot) => {
                backend_utils_fmgr_fmgr_seams::render_slot_columns::call(mcx, slot, &attnums_vec)?
            }
            ViolatorSource::SpiResult => {
                backend_executor_spi_seams::spi_first_row_columns::call(mcx, &attnums_vec)?
            }
        };
...
pub fn ri_null_check(
    ...
    for i in 0..riinfo.nkeys as usize {
        if backend_commands_trigger_seams::slot_attisnull::call(slot, attnums[i])? {
            nonenull = false;
        } else {
            allnull = false;
        }
...
pub fn ri_check_pk_match(
    ...
    backend_executor_spi_seams::spi_connect::call()?;
    ...
    if backend_executor_spi_seams::spi_finish::call()? != SPI_OK_FINISH {
