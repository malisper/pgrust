...
pub fn ri_initial_check(
    mcx: Mcx<'_>,
    trigger: TriggerRef,
    fk_rel: &types_rel::Relation<'_>,
    pk_rel: &types_rel::Relation<'_>,
) -> PgResult<bool> {
    let fk_side = RelSide::Open(fk_rel);
    let pk_side = RelSide::Open(pk_rel);
    let riinfo = ri_fetch_constraint_info(mcx, trigger, fk_side, false)?;
    ...
    if !backend_executor_execMain_seams::exec_check_permissions_select::call(
        &[
...
fn build_join_check_query(
    ...
    match kind {
        ...
        JoinKind::PartitionInner => {
            let constraint_def =
                backend_utils_adt_ruleutils_seams::partition_constraint_def::call(mcx, pk_rel.oid())?;
