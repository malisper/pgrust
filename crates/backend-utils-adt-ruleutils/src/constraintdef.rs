...
pub(crate) fn pg_get_constraintdef_worker_inner<'mcx>(
    ...
    let info = match backend_utils_cache_syscache_seams::search_pg_constraintdef_info::call(
        mcx,
        constraint_id,
    )? {
        Some(i) => i,
    ...
    match con.contype {
        ...
        x if x == CONSTRAINT_PRIMARY || x == CONSTRAINT_UNIQUE => {
            ...
            if indnatts as i32 > keyatts {
                ...
                for j in (keyatts as usize)..indkey.len() {
                    let col_name = backend_utils_cache_lsyscache_seams::get_attname::call(
                        mcx,
                        con.conrelid,
                        indkey[j],
                        false,
            ...
            if full_command && oid_is_valid_pub(con.conindid) {
                ...
                let tblspc =
                    backend_utils_cache_lsyscache_seams::get_rel_tablespace::call(con.conindid)?;
        ...
        x if x == CONSTRAINT_CHECK => {
            ...
            let expr = backend_nodes_read_seams::string_to_node::call(mcx, conbin)?;

            // context = relation constraint ? deparse_context_for(...) : NIL.
            let consrc = if con.conrelid != Oid::default() {
                let relname =
                    backend_utils_cache_lsyscache_seams::get_rel_name::call(mcx, con.conrelid)?
                        .ok_or_else(|| {
                            PgError::error(format!(
...
fn decompile_column_index_array<'mcx>(
    ...
    for (j, &attnum) in keys.data.iter().enumerate() {
        let col_name =
            backend_utils_cache_lsyscache_seams::get_attname::call(mcx, rel_id, attnum, false)?
                .ok_or_else(|| {
                    PgError::error(format!(
