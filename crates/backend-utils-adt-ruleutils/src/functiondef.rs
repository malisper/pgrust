...
pub fn pg_get_functiondef<'mcx>(
    ...
    // replaced.
    let nsp = backend_utils_cache_lsyscache_seams::get_namespace_name_or_temp::call(
        mcx,
        proc.pronamespace,
    )?
    .ok_or_else(|| {
    ...
    let langname =
        backend_commands_functioncmds_seams::get_language_name::call(proc.prolang)?;
    ...
    if let Some(config) = info.proconfig.as_ref() {
        for item in config.iter() {
            ...
            let flags = backend_utils_misc_guc_funcs_seams::get_config_option_flags::call(
                String::from(cfgname),
                true,
            )?;
            if flags & GUC_LIST_QUOTE != 0 {
                let namelist = backend_utils_misc_guc_funcs_seams::split_guc_list::call(
                    String::from(value),
                    b',',
                )?
...
fn print_function_rettype<'mcx>(
    ...
    if ntabargs == 0 {
        ...
        let ty = backend_utils_adt_format_type_seams::format_type_be::call(mcx, proc.prorettype)?;
...
fn print_function_arguments<'mcx>(
    ...
    let mut argdefaults: Vec<types_nodes::nodes::Node<'mcx>> = Vec::new();
    ...
    while i < numargs {
        ...
        let ty = backend_utils_adt_format_type_seams::format_type_be::call(mcx, argtype)?;
    ...
    if let Some(trftypes) = info.protrftypes.as_ref() {
        if !trftypes.is_empty() {
            ...
            for (i, &t) in trftypes.iter().enumerate() {
                ...
                let ty = backend_utils_adt_format_type_seams::format_type_be::call(mcx, t)?;
...
fn print_function_sqlbody<'mcx>(
    ...
    let n = backend_nodes_read_seams::string_to_node::call(mcx, body.as_str())?;

    let mut out_buf = types_stringinfo::StringInfo::new_in(mcx);
