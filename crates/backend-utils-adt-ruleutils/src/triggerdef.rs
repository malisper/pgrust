...
pub fn pg_get_triggerdef_worker<'mcx>(
    ...
    // tgnewtable).
    let trig = match backend_access_index_genam_seams::trigger_by_oid::call(mcx, trigid)? {
        Some(t) => t,
        None => return Ok(None),
    };
    ...
    if let Some(qual_text) = trig.tgqual.as_ref() {
        ...
        let qual = backend_nodes_read_seams::string_to_node::call(mcx, qual_text)?;
        let relkind = backend_utils_cache_lsyscache_seams::get_rel_relkind::call(trig.tgrelid)?;
