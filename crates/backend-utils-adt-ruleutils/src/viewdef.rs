...
fn is_view_select_rule(r: &backend_access_index_genam_seams::ScannedPgRewrite) -> bool {
    r.ev_type == b'1'
        && r.is_instead
...
pub fn pg_get_viewdef_worker<'mcx>(
    ...
    let rules = backend_access_index_genam_seams::relcache_scan_pg_rewrite::call(viewoid)?;
...
fn make_viewdef<'mcx>(
    mcx: Mcx<'mcx>,
    ruletup: &backend_access_index_genam_seams::ScannedPgRewrite,
    ev_class: Oid,
    pretty_flags: i32,
    wrap_column: i32,
) -> PgResult<Option<PgString<'mcx>>> {
    ...
    let action_node = match &ruletup.ev_action {
        Some(text) => backend_nodes_read_seams::string_to_node_opt::call(mcx, text.as_str())?,
        None => None,
    };
    ...
    let ev_relation =
        backend_access_common_relation_seams::relation_open::call(mcx, ev_class, AccessShareLock)?;

    // get_query_def(query, buf, NIL, RelationGetDescr(ev_relation), true,
    //               prettyFlags, wrapColumn, 0);
    let result_desc = ev_relation.rd_att_clone_in(mcx)?;
    let buf = types_stringinfo::StringInfo::new_in(mcx);
...
pub fn pg_get_ruledef_worker<'mcx>(
    ...
    // renderer needs.
    let row = match backend_access_index_genam_seams::rule_by_oid::call(mcx, ruleoid)? {
        None => return Ok(None),
        Some(r) => r,
    };
...
fn make_ruledef<'mcx>(
    mcx: Mcx<'mcx>,
    ruletup: &backend_access_index_genam_seams::RuleByOid,
    pretty_flags: i32,
) -> PgResult<Option<PgString<'mcx>>> {
    // actions = (List *) stringToNode(ev_action);
    let action_node = match &ruletup.ev_action {
    ...
    // ev_relation = table_open(ev_class, AccessShareLock);
    let ev_relation = backend_access_common_relation_seams::relation_open::call(
        mcx,
        ruletup.ev_class,
        AccessShareLock,
    )?;
    ...
    let relname =
        backend_utils_adt_ruleutils_seams::generate_relation_name::call(mcx, ruletup.ev_class, need_qual)?;
    ...
    if ruletup.ev_qual.as_deref() != Some("<>") {
        if let Some(ev_qual) = &ruletup.ev_qual {
            ...
            let qual = backend_nodes_read_seams::string_to_node::call(mcx, ev_qual.as_str())?;
            ...
            let qual_query = backend_rewrite_core::getInsertSelectQuery(action0)?;
            let mut qual_query = qual_query.clone_in(mcx)?;

            // AcquireRewriteLocks(query, false, false);
            backend_utils_adt_ruleutils_seams::acquire_rewrite_locks::call(
                mcx,
                &mut qual_query,
                false,
                false,
...
fn action_list_queries<'mcx>(
    mcx: Mcx<'mcx>,
    action_node: Option<PgBox<'mcx, Node<'mcx>>>,
) -> PgResult<Vec<types_nodes::copy_query::Query<'mcx>>> {
    let mut out: Vec<types_nodes::copy_query::Query<'mcx>> = Vec::new();
...
fn append_str<'mcx>(
    mcx: Mcx<'mcx>,
    buf: &mut types_stringinfo::StringInfo<'mcx>,
    s: &str,
) -> PgResult<()> {
