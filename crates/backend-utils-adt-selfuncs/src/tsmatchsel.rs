...
use types_tsearch::tsearch::{QueryItem, OP_AND, OP_NOT, OP_OR, OP_PHRASE, QI_VAL};

use backend_utils_adt_ts_small::util::{get_operand, get_query, tsq_size};
...
fn const_tsquery_image<'mcx>(
    ...
    let image = backend_access_common_detoast_seams::detoast_attr::call(mcx, raw)?;
    Ok(image.to_vec())
}
...
fn tsquerysel<'mcx>(
    ...
    if let Some(stats_tuple) = vardata.stats_tuple {
        ...
        let slot = backend_utils_cache_lsyscache_seams::get_attstatsslot::call(
            mcx,
            stats_tuple,
