...
use backend_utils_adt_ts_small::cleanup::{clean_NOT, cleanup_tsquery_stopwords};
use backend_utils_adt_ts_small::util::{
...
use backend_tsearch_ts_locale_seams as locale;
use backend_utils_adt_tsvector_core_seams as tsvparser;
use backend_utils_error::ereport;
use backend_utils_mb_mbutils_seams as mb;

use port_pgstrcasecmp::pg_strncasecmp;
...
fn push_value(
    ...
    let valcrc = backend_utils_hash_small_seams::legacy_crc32_lexeme::call(&strval[..lenval]);
...
pub fn tsqueryrecv(mcx: Mcx<'_>, buf: &mut StringInfo<'_>) -> PgResult<Vec<u8>> {
    ...
    for i in 0..size {
        ...
        if ty == QI_VAL {
            ...
            let valcrc = backend_utils_hash_small_seams::legacy_crc32_lexeme::call(&val_bytes);
