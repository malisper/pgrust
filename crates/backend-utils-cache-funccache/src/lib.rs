...
fn trigger_oid(fcinfo: &FunctionCallInfoBaseData) -> PgResult<Oid> {
    backend_utils_cache_funccache_seams::trigger_context_oid::call(fcinfo)
}
...
pub fn cached_function_compile<'mcx>(
    ...
    let proc = backend_utils_cache_funccache_seams::search_proc_compile_info::call(mcx, fn_oid)?
...
fn elog_warning(msg: &str) {
    let _ = backend_utils_error::elog(WARNING, msg);
}
