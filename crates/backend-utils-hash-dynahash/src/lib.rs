...
//! `HASHHDR`/`HTAB`, owned by [`types_hash::hsearch`]).
//!
//! ## Memory
//!
...
//! `backend_storage_lmgr_s_lock`, never `std::sync`.
...
use types_hash::hsearch::HASHACTION::{HASH_ENTER, HASH_ENTER_NULL, HASH_FIND, HASH_REMOVE};
...
unsafe fn SpinLockInit(m: *mut AtomicI32) {
    backend_storage_lmgr_s_lock::s_init_lock(as_spinlock(m));
}

#[inline]
unsafe fn SpinLockAcquire(m: *mut AtomicI32) {
    // s_lock cannot fail on a free lock; it returns the delay count (void in C).
    let _ = backend_storage_lmgr_s_lock::s_lock(as_spinlock(m), Some(file!()), line!() as i32, Some("dynahash"));
}

#[inline]
unsafe fn SpinLockRelease(m: *mut AtomicI32) {
    backend_storage_lmgr_s_lock::s_unlock(as_spinlock(m));
}
...
fn string_hash(key: &[u8], keysize: Size) -> u32 {
    common_hashfn_seams::string_hash::call(key, keysize)
}

/// `tag_hash` — `tag_hash(key, keysize)` via common/hashfn.
fn tag_hash(key: &[u8], keysize: Size) -> u32 {
    common_hashfn_seams::tag_hash::call(key, keysize)
}
...
fn uint32_hash(key: &[u8], _keysize: Size) -> u32 {
    ...
    common_hashfn_seams::hash_bytes_uint32::call(v)
}
...
unsafe fn register_seq_scan(hashp: *mut HTAB) {
    SEQ_SCAN_TABLES.with(|cell| {
        ...
        let level = backend_access_transam_xact_seams::get_current_transaction_nest_level::call();
