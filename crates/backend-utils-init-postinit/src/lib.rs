...
fn GetDatabaseTuple<'mcx>(mcx: Mcx<'mcx>, dbname: &str) -> PgResult<Option<FormPgDatabase<'mcx>>> {
    backend_catalog_pg_database_seams::get_database_tuple_by_name::call(mcx, dbname)
}
...
fn GetDatabaseTupleByOid<'mcx>(mcx: Mcx<'mcx>, dboid: Oid) -> PgResult<Option<FormPgDatabase<'mcx>>> {
    backend_catalog_pg_database_seams::get_database_tuple_by_oid::call(mcx, dboid)
}
...
fn PerformAuthentication(mcx: Mcx<'_>) -> PgResult<()> {
    ...
    backend_postmaster_postmaster_seams::set_client_auth_in_progress::call(true);
    ...
    let auth_timeout_ms = backend_libpq_auth_seams::authentication_timeout::call() * 1000;
    backend_utils_misc_more2_seams::enable_timeout_after::call(
        CoreTimeoutId::STATEMENT_TIMEOUT,
        auth_timeout_ms,
    )?;

    // Now perform authentication exchange.
    backend_utils_misc_more_seams::set_ps_display::call("authentication");
    backend_libpq_auth_seams::client_authentication::call()?; // might not return

    // Done with authentication. Disable the timeout, and log if needed.
    backend_utils_misc_more2_seams::disable_timeout::call(CoreTimeoutId::STATEMENT_TIMEOUT, false)?;
    ...
    backend_utils_misc_more_seams::set_ps_display::call("startup");

    backend_postmaster_postmaster_seams::set_client_auth_in_progress::call(false);
...
fn build_auth_logmsg(mcx: Mcx<'_>) -> PgResult<PgString<'_>> {
    ...
    let user = backend_utils_init_small_seams::my_proc_port_user_name::call(mcx)?;
    if am_walsender {
    ...
    if !am_walsender {
        let db = backend_utils_init_small_seams::my_proc_port_database_name::call(mcx)?;
        logmsg.try_push_str(" database=")?;
        logmsg.try_push_str(db.as_str())?;
    }

    if let Some(app) = backend_utils_init_small_seams::my_proc_port_application_name::call(mcx)? {
...
fn CheckMyDatabase(
    ...
    let dbform = match backend_catalog_pg_database_seams::search_database_syscache::call(
        mcx,
        my_database_id,
    )? {
        Some(f) => f,
    ...
    if backend_utils_init_small_seams::is_under_postmaster::call() {
        ...
        if !am_superuser && !override_allow_connections {
            ...
            let res = backend_catalog_aclchk_seams::object_aclcheck::call(
        ...
        if dbform.datconnlimit >= 0
            && backend_storage_lmgr_proc_seams::am_regular_backend_process::call()
            && !am_superuser
            && backend_storage_ipc_procarray_seams::count_db_connections::call(my_database_id)?
                > dbform.datconnlimit
        {
            return ereport(FATAL)
    ...
    backend_utils_mb_mbutils_seams::set_database_encoding::call(dbform.encoding)?;
    ...
    if backend_utils_adt_pg_locale_seams::pg_perm_setlocale::call(
        mcx,
        backend_utils_adt_pg_locale_seams::LcCategory::LcCollate,
        collate,
    )?
    .is_none()
    {
    ...
    if backend_utils_adt_pg_locale_seams::pg_perm_setlocale::call(
        mcx,
        backend_utils_adt_pg_locale_seams::LcCategory::LcCtype,
        ctype,
    )?
    .is_none()
    {
    ...
    if ctype == "C" || ctype == "POSIX" {
        backend_utils_adt_pg_locale_seams::set_database_ctype_is_c::call(true);
    }

    backend_utils_adt_pg_locale_seams::init_database_collation::call()?;
    ...
    if let Some(collversion) = dbform.datcollversion.as_ref() {
        ...
        let actual = backend_utils_adt_pg_locale_seams::get_collation_actual_version::call(
            mcx,
        ...
        match actual {
            ...
            Some(actual_versionstr) if actual_versionstr.as_str() != collversionstr => {
                let quoted =
                    backend_utils_adt_ruleutils_seams::quote_identifier::call(mcx, name)?;
                let _ = ereport(WARNING)
...
pub fn InitializeMaxBackends() -> PgResult<()> {
    ...
    backend_utils_init_small_seams::set_max_backends::call(max_backends);
...
pub fn InitializeFastPathLocks() -> PgResult<()> {
    ...
    let max_locks_per_xact = backend_storage_lmgr_lock_seams::max_locks_per_xact::call();
    let value = pg_max(
        pg_min(
...
pub fn BaseInit() -> PgResult<()> {
    ...
    backend_utils_error::DebugFileOpen()?;

    // Initialize file access. Done early so other subsystems can access files.
    backend_storage_file_fd_seams::init_file_access::call()?;
    ...
    backend_utils_activity_pgstat_seams::pgstat_initialize::call()?;
    ...
    backend_storage_file_fd_seams::init_temporary_file_access::call()?;

    // Initialize local buffers for WAL record construction.
    backend_access_transam_xloginsert_seams::init_xlog_insert::call()?;

    // Initialize lock manager's local structs.
    backend_storage_lmgr_lock_seams::init_lock_manager_access::call()?;

    // Initialize replication slots after pgstat. The exit hook might need to
    // drop ephemeral slots, which in turn triggers stats reporting.
    backend_replication_slot_seams::replication_slot_initialize::call()?;
...
pub fn InitPostgres(
    ...
    let bootstrap = backend_utils_init_miscinit_seams::is_bootstrap_processing_mode::call();
    ...
    if !bootstrap {
        backend_utils_activity_status_seams::pgstat_bestart_initial::call()?;
    ...
    backend_storage_ipc_sinval_seams::shared_inval_backend_init::call(false)?;
    ...
    backend_utils_cache_relcache_seams::relation_cache_initialize::call()?;
    backend_utils_cache_syscache::InitCatalogCache()?;
    backend_utils_cache_plancache_seams::init_plan_cache::call()?;

    // Initialize portal manager.
    backend_utils_mmgr_portalmem_seams::enable_portal_manager::call()?;
    ...
    if backend_postmaster_autovacuum_seams::am_autovacuum_launcher_process::call() {
        backend_utils_activity_status_seams::pgstat_bestart_final::call()?;
        return Ok(());
    }
    ...
    if !bootstrap {
        ...
        backend_access_transam_xact_seams::set_current_statement_start_timestamp::call();
        ...
        backend_access_transam_xact_seams::set_xact_iso_level_read_committed::call();
    }
        ...
        || backend_utils_init_miscinit_seams::am_logical_slot_sync_worker_process::call()
    {
        ...
        am_superuser = true;
    } else if !backend_utils_init_small_seams::is_under_postmaster::call() {
        backend_utils_init_miscinit_seams::initialize_session_user_id_standalone::call()?;
        am_superuser = true;
    ...
    if backend_utils_init_small_seams::has_my_proc_port::call() {
        debug_assert!(!bootstrap);
        backend_utils_activity_status_seams::pgstat_bestart_security::call()?;
    }

    // Binary upgrades only allow super-user connections.
    if backend_utils_init_small_seams::is_binary_upgrade::call() && !am_superuser {
        return ereport(FATAL)
            .errcode(ERRCODE_INSUFFICIENT_PRIVILEGE)
    ...
    let su_reserved = backend_utils_init_small_seams::superuser_reserved_connections::call();
    let reserved_conns = backend_utils_init_small_seams::reserved_connections::call();
    if backend_storage_lmgr_proc_seams::am_regular_backend_process::call()
        && !am_superuser
        && (su_reserved + reserved_conns) > 0
    {
        ...
        if !have {
            ...
            let userid = backend_utils_init_miscinit_seams::get_user_id::call();
            if !backend_utils_adt_acl_seams::has_privs_of_role::call(
                userid,
    ...
    if backend_replication_walsender_seams::am_walsender::call() {
        ...
        let userid = backend_utils_init_miscinit_seams::get_user_id::call();
        if !backend_utils_init_miscinit_seams::has_rolreplication::call(mcx, userid)? {
            return ereport(FATAL)
                .errcode(ERRCODE_INSUFFICIENT_PRIVILEGE)
    ...
    if backend_replication_walsender_seams::am_walsender::call()
        && !backend_replication_walsender_seams::am_db_walsender::call()
    {
        ...
        backend_utils_mb_mbutils_seams::initialize_client_encoding::call()?;
    ...
    if bootstrap {
        ...
        backend_utils_init_small_seams::set_my_database_table_space::call(DEFAULTTABLESPACE_OID);
    ...
    if !bootstrap {
        ...
        if backend_commands_dbcommands_seams::database_is_invalid_form::call(
        ...
        backend_utils_init_small_seams::set_my_database_table_space::call(datform.dattablespace);
        backend_utils_init_small_seams::set_my_database_has_login_event_triggers::call(
    ...
    backend_utils_init_small_seams::set_my_database_id::call(dboid);

    // Now we can mark our PGPROC entry with the database ID.
    backend_storage_lmgr_proc_seams::set_my_proc_database_id::call(dboid);
    ...
    if !bootstrap {
        match backend_storage_file_fd_seams::access_f_ok::call(fullpath.as_str())? {
            backend_storage_file_fd_seams::AccessResult::Ok => {}
            backend_storage_file_fd_seams::AccessResult::NoEnt => {
                return ereport(FATAL)
                    .errcode(ERRCODE_UNDEFINED_DATABASE)
            ...
            backend_storage_file_fd_seams::AccessResult::Other(errno) => {
                return ereport(FATAL)
    ...
    backend_utils_adt_acl_seams::initialize_acl::call()?;
    ...
    let my_database_id = backend_utils_init_small_seams::my_database_id::call();
    let session_user_id = backend_utils_init_miscinit_seams::get_session_user_id::call();
    ...
    backend_catalog_namespace::InitializeSearchPath()?;

    // initialize client encoding
    backend_utils_mb_mbutils_seams::initialize_client_encoding::call()?;

    // Initialize this backend's session state.
    backend_access_common_session_seams::initialize_session::call()?;
...
fn process_startup_options(mcx: Mcx<'_>, am_superuser: bool) -> PgResult<()> {
    ...
    let cmdline_options = backend_utils_init_small_seams::my_proc_port_cmdline_options::call(mcx)?;
    let guc_options = backend_utils_init_small_seams::my_proc_port_guc_options::call(mcx)?;
    ...
    if let Some(cmdline_options) = cmdline_options {
        ...
        backend_tcop_postgres_seams::process_postgres_switches::call(&av[..ac], gucctx)?;
    }
...
fn process_settings(mcx: Mcx<'_>, databaseid: Oid, roleid: Oid) -> PgResult<()> {
    ...
    backend_catalog_pg_db_role_setting_seams::apply_db_role_settings::call(mcx, databaseid, roleid)
}
...
pub fn ShutdownPostgres(_code: i32, _arg: types_tuple::Datum<'static>) -> PgResult<()> {
    ...
    backend_storage_lmgr_lock_seams::lock_release_all_user::call()?;
...
/// ([`backend_access_transam_xlog::do_checkpoint::ShutdownXLOG`], which writes
/// the shutdown checkpoint and flips the control file to `Shutdowned`). The
/// top-level `backend_access_transam_xlog::ShutdownXLOG(code, arg)` re-export is
...
fn shutdown_xlog_cb(code: i32, arg: types_tuple::Datum<'static>) -> PgResult<()> {
    let _ = (code, arg);
    backend_access_transam_xlog::do_checkpoint::ShutdownXLOG()
}
...
pub fn StatementTimeoutHandler() {
    ...
    if backend_postmaster_postmaster_seams::client_auth_in_progress::call() {
        sig = libc::SIGTERM;
    }

    // HAVE_SETSID: signal the whole process group, then self.
    let pid = backend_utils_init_small_seams::my_proc_pid::call();
...
pub fn TransactionTimeoutHandler() {
    backend_tcop_postgres_seams::set_transaction_timeout_pending::call(true);
...
pub fn IdleInTransactionSessionTimeoutHandler() {
    backend_tcop_postgres_seams::set_idle_in_transaction_session_timeout_pending::call(true);
    backend_utils_init_small_seams::set_interrupt_pending::call(true);
    backend_storage_ipc_latch_seams::set_latch_my_latch::call();
}
...
pub fn IdleSessionTimeoutHandler() {
    backend_tcop_postgres_seams::set_idle_session_timeout_pending::call(true);
    backend_utils_init_small_seams::set_interrupt_pending::call(true);
    backend_storage_ipc_latch_seams::set_latch_my_latch::call();
}
...
pub fn IdleStatsUpdateTimeoutHandler() {
    backend_tcop_postgres_seams::set_idle_stats_update_timeout_pending::call(true);
    backend_utils_init_small_seams::set_interrupt_pending::call(true);
    backend_storage_ipc_latch_seams::set_latch_my_latch::call();
}
...
pub fn ClientCheckTimeoutHandler() {
    backend_tcop_postgres_seams::set_check_client_connection_pending::call(true);
    backend_utils_init_small_seams::set_interrupt_pending::call(true);
    backend_storage_ipc_latch_seams::set_latch_my_latch::call();
}
