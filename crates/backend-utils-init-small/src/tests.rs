...
fn init_seams_installs_work_mem() {
    crate::init_seams();
    assert!(backend_utils_init_small_seams::work_mem::is_installed());
    std::thread::spawn(|| {
        globals::set_work_mem(1234);
        assert_eq!(backend_utils_init_small_seams::work_mem::call(), 1234);
    })
    .join()
    .unwrap();
}
...
fn install_fakes() {
    ...
    INSTALL.call_once(|| {
        backend_utils_init_miscinit_seams::get_user_id_and_sec_context::set(|| CURRENT.get());
        backend_utils_init_miscinit_seams::set_user_id_and_sec_context::set(|userid, sec| {
            CURRENT.set((userid, sec));
            SET_CALLS.with_borrow_mut(|calls| calls.push((userid, sec)));
        });
        backend_utils_init_miscinit_seams::get_user_name_from_id::set(|mcx, roleid, noerr| {
