...
pub fn SwitchToUntrustedUser(mcx: Mcx<'_>, userid: Oid, context: &mut UserContext) -> PgResult<()> {
    ...
    if !backend_utils_adt_acl_seams::member_can_set_role::call(save_userid, userid)? {
        let save_user_name = backend_utils_init_miscinit_seams::get_user_name_from_id::call(
            mcx,
            save_userid,
            false,
        )?
        .expect("GetUserNameFromId(noerr = false) returns a name");
        let target_user_name = backend_utils_init_miscinit_seams::get_user_name_from_id::call(
            mcx, userid, false,
        )?
        .expect("GetUserNameFromId(noerr = false) returns a name");
    ...
    // anyway.
    if backend_utils_adt_acl_seams::member_can_set_role::call(userid, save_userid)? {
