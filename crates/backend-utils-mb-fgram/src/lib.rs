...
use common_extra_encnames::pg_encoding_to_char;
use common_wchar::{
...
fn wchar_table_for_encoding(encoding: pg_enc) -> PgResult<&'static pgrust_pg_ffi::pg_wchar_tbl> {
    if !pg_valid_encoding(encoding) {
        return Err(invalid_encoding_error("encoding", encoding));
    }
...
mod tests {
    ...
    use super::*;
    use pgrust_pg_ffi::{PG_EUC_CN, PG_LATIN1};
