//! One typed slot per C hook function referenced by the GUC tables,
//! named after the C symbol. The unit that owns the hook installs it
//! from its `init_seams()`; calling an uninstalled hook panics with the
//! symbol name.
