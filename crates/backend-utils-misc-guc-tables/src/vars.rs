//! One typed storage slot per C GUC variable (`conf->variable` in
//! guc_tables.c), named after the C global. The owning subsystem
//! installs accessors over its own `thread_local` backing store from its
//! `init_seams()`.
