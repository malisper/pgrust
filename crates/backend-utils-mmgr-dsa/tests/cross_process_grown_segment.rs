...
use backend_storage_lmgr_lwlock::LWLockRegisterTranche;
use backend_utils_mmgr_dsa::runtime::{
    dsa_allocate_extended, dsa_attach, dsa_attach_in_place, dsa_create_in_place_ext,
    dsa_get_address, dsa_get_handle,
};
use backend_utils_mmgr_dsa::{
...
fn worker_reads_leader_bytes_from_on_demand_segment() {
    ...
    let area = backend_utils_mmgr_dsa::runtime::dsa_create_ext(
        TEST_TRANCHE_ID,
...
fn worker_in_place_reads_leader_bytes_from_on_demand_segment() {
    ...
    let area = dsa_create_in_place_ext(
        place_addr,
        backend_utils_mmgr_dsa::runtime::dsa_minimum_size(),
        TEST_TRANCHE_ID,
        Some(place_seg_id),
