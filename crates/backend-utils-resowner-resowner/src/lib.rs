...
use common_hashfn::{hash_combine64, murmurhash64};
    ...
    let result = (|| -> PgResult<()> {
        if phase == RESOURCE_RELEASE_BEFORE_LOCKS {
            ...
            loop {
                ...
                match node {
                    Some(node) => backend_storage_aio_aio_seams::pgaio_io_release_resowner::call(
                        node, !is_commit,
                    ),
                    None => break,
                }
            ...
            if is_top_level {
                if Some(owner) == TopTransactionResourceOwner() {
                    backend_storage_lmgr_proc::proc_waitqueue::ProcReleaseLocks(is_commit)?;
                    backend_storage_lmgr_predicate::ReleasePredicateLocks(is_commit, false)?;
                }
            } else {
                ...
                if is_commit {
                    backend_storage_lmgr_lock::LockReassignCurrentOwner(locks.as_deref())?;
                } else {
                    backend_storage_lmgr_lock::LockReleaseCurrentOwner(locks.as_deref())?;
                }
            }
