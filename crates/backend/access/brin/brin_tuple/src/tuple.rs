...
pub fn brin_form_tuple<'mcx>(
    ...
    for keyno in 0..brdesc.natts() {
        ...
        if tuple.bt_columns[keyno].bv_has_serialize {
            ...
            brin_entry_seams::brin_serialize::call(mcx, keyno, brdesc, mem, &mut dst)?;
            serialized = Some(dst);
        }
        ...
        for datumno in 0..nstored {
            ...
            if varatt_is_external(&value) {
                value = detoast_seams::detoast_external_attr::call(
                    mcx, &value,
                )?;
            }
...
pub fn brin_deform_tuple<'mcx>(
    ...
    for keyno in 0..brdesc.natts() {
        ...
        for i in 0..nstored {
            ...
            let copied = scalar_seams::datum_copy::call(
                mcx,
                &values[valueno],
                tce.typbyval,
                tce.typlen,
