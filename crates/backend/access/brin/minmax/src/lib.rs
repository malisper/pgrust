...
use bloom as bloom;
use inclusion as inclusion;
use minmax_multi as mmm;
...
/// Convert a canonical `Datum<'mcx>` into the bare-word `datum::Datum`
/// the fmgr seams dispatch on (mirrors nbtree's `to_word`).
#[inline]
fn to_word(d: &Datum) -> datum::Datum {
    datum::Datum::from_usize(d.as_usize())
}
...
fn register_opclass_support_builtins() {
    ...
    // The two BRIN opclass-options support procedures are real fmgr builtins:
    // `index_opclass_options` resolves them by OID and invokes them through
    // fmgr, passing the `local_relopts` on the `internal` lane. Registering them
    // as Result-native bodies routes the dispatch to the proc directly.
