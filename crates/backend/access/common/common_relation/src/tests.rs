...
fn setup() {
    INSTALL.call_once(|| {
        lmgr_seams::lock_relation_oid::set(|relid, lockmode| {
            record(std::format!("lock_relation_oid({relid},{lockmode})"));
            Ok(lmgr_seams::LockGuard::relation(relid, lockmode))
        });
        lmgr_seams::unlock_relation_oid::set(|relid, lockmode| {
            record(std::format!("unlock_relation_oid({relid},{lockmode})"));
            Ok(())
        });
        lmgr_seams::check_relation_locked_by_me::set(
            |_relid, _lockmode, _orstronger| {
                record("check_relation_locked_by_me");
                true
            },
        );
        relcache_seams::relation_id_get_relation::set(|mcx, relid| {
        ...
        relcache_seams::relation_id_get_relation_shared::set(|relid| {
            record(std::format!("relation_id_get_relation_shared({relid})"));
            if REL_FOUND.load(Ordering::SeqCst) {
                let mut entry = relcache_entry::RelationData::default();
                entry.rd_id = relid;
        ...
        relcache_seams::relation_close::set(|relid| {
            record(std::format!("relation_close({relid})"));
            Ok(())
        });
        syscache_seams::search_syscache_exists_reloid::set(|relid| {
            record(std::format!("search_syscache_exists_reloid({relid})"));
            Ok(EXISTS.load(Ordering::SeqCst))
        });
        namespace_seams::range_var_get_relid::set(
        ...
        miscinit_seams::is_bootstrap_processing_mode::set(|| {
            record("is_bootstrap_processing_mode");
            false
        });
        pgstat_seams::pgstat_init_relation::set(|relid, _relkind| {
            record(std::format!("pgstat_init_relation({relid})"));
            true
        });
        transam_xact_seams::set_xact_accessed_temp_namespace::set(|| {
