...
pub fn transformRelOptions(
    ...
    match strings {
        ...
        Some(astate) => {
            ...
            let datum =
                arrayfuncs_seams::construct_text_array::call(mcx, &refs)?;
            Ok(Some(datum))
        }
    }
}
...
pub fn transformRelOptionsBytes<'mcx>(
    ...
    match strings {
        ...
        Some(astate) => {
            let refs: Vec<&str> = astate.iter().map(|s| s.as_str()).collect();
            let bytes =
                arrayfuncs_seams::construct_text_array_bytes::call(mcx, &refs)?;
            Ok(Some(bytes))
        }
    }
}
...
fn transform_rel_options_strings(
    ...
    if def_list.is_empty() {
        match old_options {
            ...
            Some(old_options) => {
                let oldoptions =
                    arrayfuncs_seams::deconstruct_text_array::call(mcx, old_options)?;
    ...
    if let Some(old_options) = old_options {
        let oldoptions = arrayfuncs_seams::deconstruct_text_array::call(mcx, old_options)?;
...
pub fn untransformRelOptions(mcx: Mcx<'_>, options: Option<&[u8]>) -> PgResult<Vec<(String, Option<String>)>> {
    ...
    let optiondatums = arrayfuncs_seams::deconstruct_text_array::call(mcx, options)?;
...
pub fn parse_one_reloption(option: &mut RelOptValue, text_str: &str, validate: bool) -> PgResult<()> {
    ...
    match &gen.data {
        ...
        RelOptData::Int { min, max, .. } => {
            let int_val;
            match guc_seams::parse_int::call(value.to_string()) {
                Some(v) => {
                    int_val = v;
        ...
        RelOptData::Real { min, max, .. } => {
            let real_val;
            match guc_seams::parse_real::call(value.to_string()) {
                Some(v) => {
                    real_val = v;
...
pub fn index_reloptions(mcx: Mcx<'_>, amoptions: types_core::Oid, reloptions: Option<&[u8]>, validate: bool) -> PgResult<Option<RelOptStruct>> {
    ...
    let bytea = amapi_seams::am_reloptions::call(mcx, amoptions, reloptions, validate)?;
...
fn index_build_local_reloptions_seam<'mcx>(
    ...
    let shared = fmgr_seams::index_options_function_call::call(
        procinfo.fn_oid,
        shared,
    )?;
...
pub fn init_seams() {
    reloptions_seams::extract_rel_options::set(extract_rel_options_seam);
    reloptions_seams::attribute_reloptions::set(attribute_reloptions_seam);
    reloptions_seams::tablespace_reloptions::set(tablespace_reloptions_seam);
    reloptions_seams::init_local_reloptions::set(init_local_reloptions_seam);
    reloptions_seams::add_local_int_reloption::set(
    ...
    reloptions_seams::build_reloptions_spgist::set(
        build_reloptions_spgist_seam,
    );
    reloptions_seams::build_reloptions_gist::set(build_reloptions_gist_seam);
    reloptions_seams::build_reloptions_brin::set(build_reloptions_brin_seam);
    reloptions_seams::index_build_local_reloptions::set(
