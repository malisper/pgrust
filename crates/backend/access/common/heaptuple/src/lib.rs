...
///
/// Defined in `::types_tuple::heaptuple` so seam signatures can
...
pub fn heap_compute_data_size(
    ...
    for i in 0..number_of_attributes as usize {
        ...
        } else if atti.attlen == -1 && varatt_is_external_expanded(varlena_image.as_deref().unwrap())
        {
            ...
            data_length += misc2_seams::eoh_get_flat_size::call(
                datum::ExpandedObjectRef::from_expanded_datum_bytes(
...
pub use ::types_tuple::heaptuple::FormedTuple;
...
pub use ::types_tuple::heaptuple::DeformedColumn;
...
/// `heaptoast_seams::toast_flatten_tuple_to_datum` seam.
pub fn heap_copy_tuple_as_datum<'mcx>(
    ...
    if (header.t_infomask & HEAP_HASEXTERNAL) != 0 {
        return heaptoast_seams::toast_flatten_tuple_to_datum::call(
