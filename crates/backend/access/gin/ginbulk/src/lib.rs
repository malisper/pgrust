...
fn ginBuildCallback<'mcx>(
    ...
    let mwm_bytes =
        guc_seams::maintenance_work_mem::call() as usize * 1024;
...
pub fn ginbuild<'mcx>(
    ...
    let reltuples = {
        ...
        table_tableam_seams::table_index_build_scan::call(
            mcx,
            heap,
            index,
            index_info,
    ...
    if relation_needs_wal(index) {
        ...
        xloginsert_seams::log_newpage_range::call(
            index,
...
pub fn ginbuildempty<'mcx>(index: &Relation<'mcx>) -> PgResult<()> {
    ...
    bufmgr::mark_buffer_dirty::call(meta_buffer);
    xloginsert_seams::log_newpage_buffer::call(meta_buffer, true)?;
    bufmgr::with_buffer_page::call(root_buffer, &mut |page: &mut [u8]| {
        GinInitBuffer(page, GIN_LEAF as u32)
    })?;
    bufmgr::mark_buffer_dirty::call(root_buffer);
    xloginsert_seams::log_newpage_buffer::call(root_buffer, false)?;
...
pub fn init_seams() {
    ginutil_seams::ginbuild::set(|mcx, heap, index, index_info| {
    ...
    ginutil_seams::ginbuildempty::set(|_mcx, index| ginbuildempty(index));
}
