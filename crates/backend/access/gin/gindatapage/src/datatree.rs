...
//! (the landed [`ginbtree`] crate) can drive insertion and
...
fn gin_data_leaf_page_free_space(page: &[u8]) -> PgResult<usize> {
    let pr = page::PageRef::new(page)?;
    Ok(page::PageGetExactFreeSpace(&pr) as usize)
}
...
/// from a leaf data page directly to a [`tidbitmap::TIDBitmap`], returning
...
pub fn GinDataLeafPageGetItemsToTbm(
    ...
    if GinPageIsCompressed(page) {
        ...
        if nitems > 0 {
            nodes_core_seams::tbm_add_tuples::call(tbm, &uncompressed, false)?;
        }
        Ok(nitems)
    }
}
...
pub fn ginInsertItemPointers<'mcx>(
    ...
    while insertdata.curitem < insertdata.nitem {
        ...
        let stack = ginbtree::ginFindLeafPage(
...
pub fn ginScanBeginPostingTree<'mcx>(
    ...
    btree.fullScan = true;
    ginbtree::ginFindLeafPage(btree, mcx, true, false, index)
}
