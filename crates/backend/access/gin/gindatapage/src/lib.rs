...
//! carriers come from `gin`; page header initialization is delegated to
...
// layout and the `OFF_PD_LOWER` in `page`.
...
pub fn GinInitPage(page: &mut [u8], f: u32, page_size: usize) -> PgResult<()> {
    page::PageInit(page, page_size, SIZE_OF_GIN_PAGE_OPAQUE)?;
