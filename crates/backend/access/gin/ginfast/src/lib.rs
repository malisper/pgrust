...
pub fn init_seams() {
    gininsert_seams::gin_get_use_fast_update::set(gin_get_use_fast_update);
    gininsert_seams::gin_fast_insert::set(gin_fast_insert);
...
fn gin_get_use_fast_update(index: &Relation<'_>) -> PgResult<bool> {
    ginutil_seams::gin_get_use_fast_update::call(index)
}
...
fn processPendingPage<'mcx>(
    accum: &mut ginbulk::BuildAccumulator<'mcx>,
    ka: &mut KeyArray<'mcx>,
    ginstate: &GinState<'mcx>,
    mcx: Mcx<'mcx>,
    page: &[u8],
...
pub fn ginInsertCleanup<'mcx>(
    ...
    let work_memory: i32 = if force_cleanup {
        ...
        if am_autovacuum_worker_process()? {
            let avwm = guc_seams::autovacuum_work_mem::call();
            if avwm != -1 {
                avwm
            } else {
    ...
    let mut accum = ginbulk::new_accumulator(ginstate, mcx)?;
...
fn gin_get_pending_list_cleanup_size(index: &Relation<'_>) -> PgResult<i32> {
    ginutil_seams::gin_get_pending_list_cleanup_size::call(index)
}
...
fn check_for_serializable_conflict_in(index_oid: Oid, blkno: BlockNumber) -> PgResult<()> {
    predicate_seams::check_for_serializable_conflict_in_page::call(
        index_oid, blkno,
    )
}
...
fn xlog_ensure_record_space(max_block_id: i32, ndatas: i32) -> PgResult<()> {
    transam_xlog_seams::xlog_ensure_record_space::call(max_block_id, ndatas)
}
