...
//!     [`gininsert_seams::gin_fast_insert`] seam (no ginfast
...
pub fn init_seams() {
    ginutil_seams::gininsert::set(gininsert);
}
...
fn gininsert_with_state<'mcx>(
    ...
    if gininsert_seams::gin_get_use_fast_update::call(index)? {
        ...
        gininsert_seams::gin_fast_insert::call(
            mcx,
            index,
            ginstate.index,
            values[..natts].to_vec(),
