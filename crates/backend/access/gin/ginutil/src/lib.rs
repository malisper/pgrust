...
//! `GinNullCategory` are the canonical GIN carriers from [`gin`]; the
...
//! them through the [`ginutil_seams`] AM-callback seams,
...
fn gin_get_pending_list_cleanup_size_impl<'mcx>(index: &Relation<'mcx>) -> PgResult<i32> {
    ...
    match from_opts {
        ...
        None => Ok(guc_tables::vars::gin_pending_list_limit.read()),
    }
}
...
fn gin_index_getattr_impl<'mcx>(
    ...
    indextuple::nocache_index_getattr_seam(mcx, tuple, attnum as i32, td)
}
...
pub fn initGinState<'mcx>(index: &Relation<'mcx>, mcx: Mcx<'mcx>) -> PgResult<GinState<'mcx>> {
    ...
    for i in 0..natts {
        ...
        if state.oneCol {
            ...
            let mut td = tupdesc::CreateTemplateTupleDesc(mcx, 2)?;
