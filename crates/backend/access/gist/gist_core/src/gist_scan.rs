    ...
    while i <= maxoff {
        ...
        if is_bitmap && page_is_leaf {
            ...
            let tbm_concrete = tbm
                .payload
                .as_mut()
                .and_then(|pl| pl.downcast_mut::<tidbitmap::TIDBitmap>())
                .expect("amgetbitmap TIDBitmap payload is not a tidbitmap::TIDBitmap");
            nodes_core_seams::tbm_add_tuples::call(tbm_concrete, &[tid], recheck)?;
            ntids += 1;
...
pub fn gistbeginscan<'mcx>(
    ...
    let so = GISTScanOpaqueData {
        ...
        queue: pairingheap::pairingheap_allocate(pairingheap_gist_search_item_cmp),
        queueCxt: mcx,
        pageDataCxt: None,
...
pub fn gistrescan<'mcx>(
    ...
    gist(scan).queue =
        pairingheap::pairingheap_allocate(pairingheap_gist_search_item_cmp);
    ...
    if !orderbys.is_empty() && scan.number_of_order_bys > 0 {
        ...
        for skey in orderbys.iter() {
            ...
            let rettype = lsyscache_seams::get_func_rettype::call(
                skey.sk_func.fn_oid,
            )?;
