...
fn install_test_seams() {
    ...
    INSTALL.call_once(|| {
        ...
        lsyscache_seams::get_opclass_input_type::set(|_oc| Ok(23));
        transam_xact_seams::command_counter_increment::set(|| Ok(()));
        ...
        amvalidate_seams::opclass_for_family_datatype::set(
...
fn catalog_projection_accounting_is_exact() {
    ...
    let form = syscache_seams::search_opclass::call(mcx, OC_VALID)
        .unwrap()
        .unwrap();
    assert_eq!(ctx.used(), form.opcname.capacity_bytes());

    let oprs = syscache_seams::search_amop_list::call(mcx, 100).unwrap();
    let procs = syscache_seams::search_amproc_list::call(mcx, 100).unwrap();
    assert_eq!(
        ctx.used(),
