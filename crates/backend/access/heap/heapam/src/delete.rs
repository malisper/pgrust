...
pub fn heap_delete<'mcx>(
    ...
    if relkind != RELKIND_RELATION && relkind != RELKIND_MATVIEW {
        ...
        heaptoast::heap_toast_delete(mcx, relation, &tp, false)?;
    }
...
pub fn ExtractReplicaIdentity<'mcx>(
    ...
    if replident == REPLICA_IDENTITY_FULL {
        ...
        if HeapTupleHasExternal(tp) {
            let flat = heaptoast::toast_flatten_tuple(mcx, tp, desc)?;
            return Ok(Some(flat));
        }
    ...
    let idattrs = relcache_seam::relation_get_index_attr_bitmap::call(
        mcx,
        relation,
        relcache_seams::IndexAttrBitmapKind::Identity,
    )?;

    /*
     * If there's no defined replica identity columns, treat as !key_required.
     */
    if nodes_core_seams::bms_is_empty::call(idattrs.as_deref()) {
        return Ok(None);
    }
    ...
    for i in 0..natts.max(0) as usize {
        if nodes_core_seams::bms_is_member::call(
    ...
    if HeapTupleHasExternal(&key_tuple) {
        let flat = heaptoast::toast_flatten_tuple(mcx, &key_tuple, desc)?;
        return Ok(Some(flat));
    }
