...
pub fn heap_abort_speculative<'mcx>(
    ...
    if HeapTupleHasExternalF(&tp) {
        ...
        heaptoast::heap_toast_delete(mcx, relation, &tp, true)?;
    }
    ...
    pgstat_seams::pgstat_count_heap_delete::call(
        relation.rd_id,
...
pub fn heap_inplace_lock<'mcx>(
    ...
    inval::cache_invalidate::CacheInvalidateHeapTupleInplace(
        relation,
        oldtup,
        &inval_data,
    )?;
    ...
    if !ret {
        ...
        inval::at_eoxact::ForgetInplace_Inval();
...
pub fn heap_inplace_update_and_unlock<'mcx>(
    ...
    let (inval_messages, relcache_init_file_inval, nmsgs) =
        if transam_xlog_seams::xlog_standby_info_active::call() {
            let (msgs, fileinval) =
                inval::at_eoxact::inplaceGetInvalidationMessages()?;
     ...
     */
    inval::at_eoxact::PreInplace_Inval()?;
    ...
    if relcache_seam::relation_needs_wal::call(relation) {
        ...
        let xlrec = xl_heap_inplace {
            ...
            tsId: init_small_seams::my_database_table_space::call(),
     ...
     */
    inval::at_eoxact::AtInplace_Inval()?;
    ...
    inval::local_list::AcceptInvalidationMessages()?;
...
fn is_toast_relation(relation: &RelationData<'_>) -> bool {
    catalog_seams::is_toast_relation::call(relation)
}
...
fn transaction_id_precedes(a: TransactionId, b: TransactionId) -> bool {
    transam_seams::transaction_id_precedes::call(a, b)
}
