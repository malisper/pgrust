...
use init_small_seams as initsmall_seam;
...
pub fn heap_beginscan<'mcx>(
    ...
    let mut sscan = std::boxed::Box::new(TableScanDescData {
        ...
        rs_tbmiterator: tidbitmap::TBMIterator::default(),
        am_private: Some(am),
    });
...
fn exec_clear_tuple(slot: &mut nodes::tuptable::SlotData<'_>) -> PgResult<()> {
    execTuples_seams::exec_clear_tuple_payload::call(slot)
}
...
fn exec_store_buffer_heap_tuple<'mcx>(
    ...
    execTuples_seams::exec_store_buffer_heap_tuple::call(tuple, slot, buffer)
}
