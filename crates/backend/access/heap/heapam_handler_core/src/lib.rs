...
use heapam_handler_dml_seams as dml_seam;
...
fn heapam_parallelscan_estimate(rel: &Relation<'_>) -> usize {
    table_tableam::table_block_parallelscan_estimate(rel)
}
...
fn heapam_parallelscan_initialize(
    ...
    table_tableam::table_block_parallelscan_initialize(rel, pscan)
}
...
fn heapam_parallelscan_reinitialize(
    ...
    table_tableam::table_block_parallelscan_reinitialize(rel, pscan);
    Ok(())
}
...
fn heapam_index_fetch_tuple<'mcx>(
    ...
    if res.found {
        *tid = res.tid;
        ...
        // chain be visible.
        *call_again = !snapshot::snapshot::IsMVCCSnapshot(&*snap);
...
fn heapam_relation_nontransactional_truncate(rel: &Relation<'_>) -> PgResult<()> {
    catalog_storage_seams::relation_truncate::call(rel, 0)
}
...
fn heapam_relation_needs_toast_table(rel: &Relation<'_>) -> PgResult<bool> {
    ...
    for i in 0..tupdesc.natts as usize {
        ...
        if att.attlen > 0 {
            ...
            let maxlen = format_type_seams::type_maximum_size::call(
                att.atttypid,
                att.atttypmod,
            )?;
            if maxlen < 0 {
...
fn table_block_relation_estimate_size(
    ...
    let density: f64 = if reltuples >= 0.0 && relpages > 0 {
        ...
        let mut tuple_width =
            plancat_seams::get_rel_data_width::call(relid, attr_widths)?
                as usize;
        ...
        costsize_seams::clamp_row_est::call(raw as f64)
    };
...
pub fn init_seams() {
    ...
    access_tableam_seams::table_relation_copy_for_cluster::set(
    ...
    plancat_ext_seams::table_relation_estimate_size::set(
    ...
    plancat_ext_seams::table_has_scan_bitmap::set(table_has_scan_bitmap);
    plancat_ext_seams::table_has_tid_range::set(table_has_tid_range);
}
