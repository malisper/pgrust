...
fn itemptr_decode(encoded: i64) -> ItemPointerData {
    ...
    page::ItemPointerSet(&mut tid, block, offset);
    tid
}
...
pub fn heapam_index_validate_scan<'mcx>(
    ...
    let predicate_src: Option<Vec<nodes::primnodes::Expr>> = index_info
        .ii_Predicate
        .as_ref()
    ...
    let mut scan = table_tableam::table_beginscan_strat(
        mcx,
        heap_relation,
        snapshot.clone(),
        0,
    ...
    // pgstat_progress_update_param(PROGRESS_SCAN_BLOCKS_TOTAL, hscan->rs_nblocks).
    {
        ...
        activity_small_seams::pgstat_progress_update_param::call(
    ...
    loop {
        ...
        if cblock != root_blkno {
            ...
            root_offsets =
                pruneheap_seams::heap_get_root_tuples::call(mcx, cbuf)?;
        ...
        let mut root_offnum = page::ItemPointerGetOffsetNumber(&heapcursor);
            ...
            if !offset_number_is_valid(root_offnum) {
                return Err(ereport(ERROR)
                    ...
                    .errmsg_internal(format!(
                        ...
                        page::ItemPointerGetBlockNumber(&heapcursor),
                        page::ItemPointerGetOffsetNumber(&heapcursor),
                        rel_name(heap_relation),
                    ))
                    .into_error());
            }
            page::ItemPointerSetOffsetNumber(&mut root_tuple, root_offnum);
        }
                ...
                ) < 0)
        {
            if let Some(ref ic) = indexcursor {
                ...
                if page::ItemPointerGetBlockNumber(ic) == root_blkno {
                    let off = page::ItemPointerGetOffsetNumber(ic) as usize;
