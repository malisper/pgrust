                    ...
                    if transaction_id_is_valid(fetched.snapshot_xmax) {
                        ...
                        match wait_policy {
                            ...
                            LockWaitPolicy::LockWaitSkip => {
                                if !lmgr_seams::conditional_xact_lock_table_wait::call(
                                    snap_xmax, false,
                                )? {
...
fn heapam_relation_set_new_filelocator(
    ...
    let freeze_xid = snapmgr::RecentXmin();
    let minmulti = multixact_seams::get_oldest_multi_xact_id::call()?;

    catalog_storage_seams::relation_set_new_filelocator_storage::call(
        *newrlocator,
        persistence,
    )?;
...
/// update-chase path).
pub fn init_seams() {
    use heapam_handler_dml_seams as sx;
