...
// lazy_truncate_heap. Single-backend divergences: the lock-retry latch loop
// collapses (ConditionalLockRelation cannot fail without peers), the
// LockHasWaitersRelation poll is skipped, PrefetchBuffer is elided.
fn lazy_truncate_heap(vacrel: &mut LVRelState<'_, '_>) -> PgResult<()> {
    let mut orig_rel_pages = vacrel.rel_pages;
    loop {
        if !lmgr::ConditionalLockRelation(vacrel.rel, types_rel::AccessExclusiveLock)? {
            return Ok(());
        }
        ...
        if new_rel_pages != orig_rel_pages {
            lmgr::UnlockRelation(vacrel.rel, types_rel::AccessExclusiveLock)?;
            return Ok(());
        }

        let (new_rel_pages, lock_waiter_detected) = count_nondeletable_pages(vacrel)?;
...
// count_nondeletable_pages: (last nonempty page + 1, lock_waiter_detected).
fn count_nondeletable_pages(vacrel: &mut LVRelState<'_, '_>) -> PgResult<(BlockNumber, bool)> {
    ...
    while blkno > vacrel.nonempty_pages {
        ...
        for offnum in 1..=maxoff {
            // Any non-unused item (even LP_DEAD) keeps the page: its index
            // entries may not have been cleaned out.
        ...
        if hastup {
            return Ok((blkno + 1, false));
        }
    }
    Ok((vacrel.nonempty_pages, false))
}
