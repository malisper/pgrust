...
pub fn init_seams() {
    visibilitymap_seams::visibilitymap_get_status::set(
    ...
    visibilitymap_seams::visibilitymap_pin::set(
        |rel, heap_blk, vmbuf_in| {
            let mut vmbuf = vmbuf_in;
    ...
    visibilitymap_seams::visibilitymap_pin_ok::set(|heap_blk, vmbuf| {
        visibilitymap_pin_ok(heap_blk, vmbuf)
    });

    // The five `&Relation<'mcx>`-keyed VM seams in vacuumlazy-seams.
    vacuumlazy_seams::visibilitymap_count::set(|rel| {
        visibilitymap_count(rel)
    });
    vacuumlazy_seams::visibilitymap_get_status::set(|rel, heap_blk, vmbuf_in| {
        let mut vmbuf = vmbuf_in;
        let status = visibilitymap_get_status(rel, heap_blk, &mut vmbuf)?;
        Ok((status, vmbuf))
    });
    vacuumlazy_seams::visibilitymap_pin::set(|rel, heap_blk, vmbuf_in| {
        let mut vmbuf = vmbuf_in;
        visibilitymap_pin(rel, heap_blk, &mut vmbuf)?;
        Ok(vmbuf)
    });
    vacuumlazy_seams::visibilitymap_set::set(|rel, args| {
        visibilitymap_set(
            rel,
            args.heap_blk,
            args.heap_buf,
    ...
    vacuumlazy_seams::visibilitymap_clear::set(|rel, heap_blk, vmbuf, flags| {
