...
fn scan_pg_class(
    ...
    let use_index =
        index_ok && relcache_seams::critical_relcaches_built::call();
...
fn scan_pg_attribute(reloid: Oid, _natts: i16) -> PgResult<Vec<seam::ScannedPgAttribute>> {
    ...
    let use_index = relcache_seams::critical_relcaches_built::call();
    let mut scandesc =
...
fn relcache_scan_pg_policy(relid: Oid) -> PgResult<Vec<seam::ScannedPgPolicy>> {
    ...
    while let Some(ntp) = systable_getnext(smcx, scandesc.desc_mut())? {
        ...
        let role_elems = arrayfuncs_seams::deconstruct_array_values_bytes::call(
            smcx,
            &roles_bytes,
            OIDOID,
            4,
...
fn relcache_exclusion_info(
    ...
    for i in 0..indnkeyatts {
        let op = ops[i];
        let proc = lsyscache_seams::get_opcode::call(op)?;
        let opfamily = rd_opfamily
            .get(i)
            .copied()
            .ok_or_else(|| PgError::error("index rd_opfamily shorter than key columns"))?;
        let strat =
            lsyscache_seams::get_op_opfamily_strategy::call(op, opfamily)?;
