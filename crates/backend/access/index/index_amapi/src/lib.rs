...
pub fn amvalidate(mcx: ::mcx::Mcx<'_>, opclassoid: Oid) -> PgResult<bool> {
    ...
    let result = match amoid {
        BTREE_AM_OID => nbt_validate::btvalidate(mcx, opclassoid)?,
        HASH_AM_OID => hashvalidate::hashvalidate(mcx, opclassoid)?,
        GIN_AM_OID => {
            core_probe::ginvalidate::ginvalidate(mcx, opclassoid)?
        }
        BRIN_AM_OID => brin_validate::brinvalidate(mcx, opclassoid)?,
        GIST_AM_OID => gist_validate::gistvalidate(mcx, opclassoid)?,
        SPGIST_AM_OID => spg_validate::spgvalidate(mcx, opclassoid)?,
...
fn am_adjust_members<'mcx>(
    ...
    match amoid {
        ...
        GIST_AM_OID => {
            gist_validate::gistadjustmembers(
                opfamilyoid,
                opclassoid,
                &mut operators,
                &mut procedures,
        ...
        HASH_AM_OID => {
            ...
            hashvalidate::hashadjustmembers(
                opfamilyoid,
                opclassoid,
                &mut ops,
                &mut procs,
        ...
        GIN_AM_OID => {
            let mut ops = to_trimmed(&operators);
            let mut procs = to_trimmed(&procedures);
            core_probe::ginvalidate::ginadjustmembers(
                opfamilyoid,
                opclassoid,
                &mut ops,
                &mut procs,
...
/// `hash::...::OpFamilyMember` — the trimmed per-AM record the hash/GIN
/// `amadjustmembers` callbacks mutate.
type TrimmedMember = hash::backend_access_hash_hashvalidate::OpFamilyMember;
