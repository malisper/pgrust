...
//! [`dedup`]; the page-format reads / scankey comparisons /
...
fn build_index_value_desc<'mcx>(
    ...
    let desc = genam_seams::build_index_value_description::call(
...
fn predicate_lock_page_split<'mcx>(
    ...
    predicate_seams::predicate_lock_page_split::call(rel.rd_id, l, r)
}
    ...
    if heapkeyspace {
        ...
        while (PageGetFreeSpace(&PageRef::new(&page)?) as usize) < insertstate.itemsz {
            ...
            if P_RIGHTMOST(&opaque)
                ...
                || prng::global_prng(|p| p.next_u32()) <= (u32::MAX / 100)
            {
                break;
            }
...
fn _bt_simpledel_pass<'mcx>(
    ...
    nbtree_core_seams::bt_delitems_delete_check::call(
...
pub fn init_seams_insert() {
    nbtree_core_seams::bt_doinsert::set(bt_doinsert);
}
