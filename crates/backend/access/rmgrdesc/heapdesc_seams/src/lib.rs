...
seam_core::seam!(
    ...
    pub fn heap_desc(buf: &mut mcx::PgString<'_>, record: &wal::rmgr::XLogReaderState<'_>) -> types_error::PgResult<()>
);
...
seam_core::seam!(
    ...
    /// `buf` (`rm_desc` slot). Appending allocates, so the C OOM
    /// `ereport(ERROR)` surface is `Err`.
    pub fn heap2_desc(buf: &mut mcx::PgString<'_>, record: &wal::rmgr::XLogReaderState<'_>) -> types_error::PgResult<()>
);
