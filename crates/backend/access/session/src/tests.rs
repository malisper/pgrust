...
fn tls_source_census_and_session_surface_are_pinned() {
    ...
    //      own 464 pin was stale for its tree (its CI cluster unit sweeps did not
    ...
    // 556, re-pinned at the issue #58 crit-section counter unification:
    //   -1 utils/error/elog/src/config.rs — CRIT_SECTION_COUNT: duplicate
    //      per-thread cell DELETED; elog now reads init_small's counter (the
    //      one START/END_CRIT_SECTION actually bump), making errstart's
    ...
    //      (dfmgr FILE_LIST is the module list it mirrors).
        ...
        // 594, 18.6 conformance audit (2026-09-05), bufmgr remediation
    //   (b039-backend-storage-buffer-1): one product source + one test-only
    //   source —
    //   87. storage/buffer/bufmgr/src/read.rs BATCH_CREDIT +
    //      BATCH_CREDIT_CLOCK (one thread_local! block) — Cell<[Option<
    //      BatchCredit>; 4]> per-backend memory of the unconsumed tail of
    //      the last combined reads so the scan's sequential re-pin is
    //      counted once (bufmgr.c StartReadBuffers/PinBufferForBlock count
    //      every block at pin time; read_stream is backend-private). A
    //      transient per-backend accounting aid with no session identity,
    //      never bound or reset — non-session TLS (slot-41 class).
    //   88. storage/buffer/bufmgr/src/tests.rs B039_CAPTURED —
    //      RefCell<Vec<PgError>> per-thread emit_log_hook recorder for the
    ...
    //      (#[cfg(test)] module; same class as 67).
        // 589, 18.6 conformance audit (2026-09-04), b006 gin remediation: one
    //   test-only source —
    //   69. access/gin/gin/src/rem_b006_tests.rs NBLOCKS/NALLOC/WAL/WAL_FAIL/
    //      DIRTY/EXTENDED (one thread_local! block) — fake-buffer + WAL-record
    //      fixture behind the bufmgr/xloginsert seams for the b006 critical-
    ...
    //      fixture; same class as 43/45/46/60).
                // 595, 18.6 conformance audit (2026-09-05), walsender remediation
    //   (b023-backend-replication-walsender-1): two product sources, both
    //   walsender-thread per-process statics of class 21-33 (dedicated
    //   background thread, no envelope capture/restore, no
    //   SESSION_ENVELOPE_MANIFEST rows):
    //   73. replication/walsender/src/lag.rs LAG_TRACKER —
    //      RefCell<Option<Box<LagTracker>>>, the port of walsender.c:249
    //      `static LagTracker *lag_tracker` (allocated by InitWalSender,
    //      lives for the walsender's life) — same class as 31.
    //   74. replication/walsender/src/logical_stream.rs LAG_TRACK_SEND_TIME
    ...
    //      61/90).
    // MultiXact ownership: WAL_SCRATCH joins its existing scratch TLS block (-1);
    // cfg(test) OWNER_CLEANUPS and INJECT_WAL_ERROR add two harness blocks (+2).
    // Both fixtures are non-production TLS; net textual census delta is +1.
    ...
    assert_eq!(count_tree(crates), 602, "TLS census changed; classify the delta in SESSION_ENVELOPE_MANIFEST or document it as non-session TLS");
    let session_sources = [
