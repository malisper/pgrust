...
fn tls_source_census_and_session_surface_are_pinned() {
    ...
    // 481, spi-compile-residual lane (fix/spi-compile-residual, PROCPERF P2):
    //   21. executor/execexpr/src/compile.rs COMPILE_ECONOMY — Cell<bool>
    ...
    //      session collector.
    //   22. pl/plpgsql/src/handler.rs PL_GUC_VALUES — Cell<Option<..>>
    ...
    assert_eq!(count_tree(crates), 481, "TLS census changed; classify the delta in SESSION_ENVELOPE_MANIFEST or document it as non-session TLS");
    let session_sources = [
