...
fn GetIndexInputType(index: &Relation<'_>, indexcol: AttrNumber) -> PgResult<Oid> {
    ...
    if heapcol != 0 {
        // Simple index column.
        let atttype =
            lsyscache_seams::get_atttype::call(rd_index.indrelid, heapcol)?;
        return lsyscache_seams::get_base_type::call(atttype);
    }
    ...
    spg_core_seams::get_index_input_type_expr::call(index.rd_id, indexcol)
}
...
fn fillTypeDesc(type_oid: Oid) -> PgResult<SpGistTypeDesc> {
    let tp = lsyscache_seams::lookup_pg_type::call(type_oid)?
...
pub fn spgGetCache<'mcx>(mcx: Mcx<'mcx>, index: &Relation<'mcx>) -> PgResult<SpGistCache> {
    ...
    let config_proc_oid =
        relcache_seams::index_getprocid::call(index, 1, SPGIST_CONFIG_PROC as u16)?;
    spg_core_seams::spg_config::call(config_proc_oid, &cfgin, &mut cache.config)?;
    ...
    if cache.config.leafType != atttype {
        if !OidIsValid(relcache_seams::index_getprocid::call(
            index,
            1,
    ...
    relcache_seams::set_rd_amcache_spgist::call(index.rd_id, cache)?;
...
pub fn initSpGistState<'mcx>(
    ...
    let redirect_xid = transam_xact_seams::get_top_transaction_id_if_any::call();
...
pub fn SpGistNewBuffer<'mcx>(mcx: Mcx<'mcx>, index: &Relation<'mcx>) -> PgResult<Buffer> {
    ...
    loop {
        ...
        let buffer = bufmgr_seams::read_buffer::call(index, blkno)?;
    ...
    let buffer = bufmgr_seams::extend_buffered_rel::call(
        index,
...
pub fn SpGistUpdateMetaPage<'mcx>(index: &Relation<'mcx>) -> PgResult<()> {
    let cache = match relcache_seams::rd_amcache_spgist::call(index.rd_id)? {
        Some(c) => c,
        None => return Ok(()),
    };
    ...
    if bufmgr_seams::conditional_lock_buffer::call(metabuffer)? {
        bufmgr_seams::with_buffer_page::call(
            metabuffer,
            &mut |metapage: &mut [u8]| {
        ...
        bufmgr_seams::release_buffer::call(metabuffer);
    }
    Ok(())
}
...
fn allocNewBuffer<'mcx>(
    ...
    loop {
        ...
        let blkno = bufmgr_seams::buffer_get_block_number::call(buffer);
    ...
    if cache.lastUsedPages.cachedPage[lup_idx].freeSpace >= need_space {
        ...
        if type_ok && nulls_ok {
            ...
            if free_space >= need_space {
                ...
                *is_new = false;
                relcache_seams::set_rd_amcache_spgist::call(
                    index.rd_id,
                    cache,
                )?;
                return Ok(buffer);
...
pub fn SpGistInitBuffer(b: Buffer, f: u16) -> PgResult<()> {
    bufmgr_seams::with_buffer_page::call(b, &mut |page: &mut [u8]| {
...
pub fn spgoptions(reloptions: Option<&[u8]>, validate: bool) -> PgResult<Option<Vec<u8>>> {
    reloptions_seams::build_reloptions_spgist::call(reloptions, validate)
}
...
pub fn spgproperty<'mcx>(
    ...
    let catlist = syscache_seams::search_amop_list::call(mcx, opfamily)?;

    let mut res = false;

    for amopform in &catlist {
        if amopform.amoppurpose == opclass::AMOP_ORDER
            && (amopform.amoplefttype == opcintype || amopform.amoprighttype == opcintype)
            && amvalidate_seams::opfamily_can_sort_type::call(
                amopform.amopsortfamily,
                lsyscache_seams::get_op_rettype::call(amopform.amopopr)?,
            )?
        {
            res = true;
            break;
...
pub fn init_seams() {
    spg_xlog_seams::spg_redo::set(spgxlog::spg_redo);
    spg_xlog_seams::spg_xlog_startup::set(spgxlog::spg_xlog_startup);
    spg_xlog_seams::spg_xlog_cleanup::set(spgxlog::spg_xlog_cleanup);
    spg_xlog_seams::spg_mask::set(spgxlog::spg_mask);
}
