...
fn doPickSplit<'mcx>(
    ...
    if !is_nulls {
        ...
        spg_core_seams::spg_picksplit::call(mcx, proc_oid, &in_, &mut out)?;
    } else {
...
pub fn spgdoinsert<'mcx>(
    ...
    parallel_rt_seams::check_for_interrupts::call()?;
    ...
    loop {
        ...
        'inner: loop {
            ...
            let mut out = if !isnull {
                ...
                spg_core_seams::spg_choose::call(mcx, choose_proc, &in_, &mut out)?;
                out
            } else {
            ...
            if all_the_same {
                match &out.result {
                    ...
                    spgChooseOutResult::MatchNode(m) => {
                        ...
                        let r =
                            prng::global_prng(|p| p.u64_range(0, (n_nodes - 1) as u64)) as i32;
...
fn interrupt_pending() -> bool {
    init_small_seams::interrupt_pending::call()
}
