...
//! [`pairingheap`] arena heap, so Rust ownership replaces every
...
/// [`pairingheap`] arena (the heap owns the value); the `void
...
pub type SpgScanQueue<'mcx> = pairingheap::PairingHeap<
...
fn resetSpGistScanOpaque<'mcx>(so: &mut SpGistScanOpaqueData<'mcx>) -> PgResult<()> {
    ...
    so.scanQueue = pairingheap::pairingheap_allocate(Box::new(
...
pub fn spgbeginscan<'mcx>(
    ...
    let scanQueue = pairingheap::pairingheap_allocate(Box::new(
...
fn spgLeafTest<'mcx>(
    ...
    if isnull {
        ...
        result = spg_core_seams::spg_leaf_consistent::call(
            mcx,
...
fn spgInnerTest<'mcx>(
    ...
    if !isnull {
        ...
        spg_core_seams::spg_inner_consistent::call(
            mcx,
