...
impl CritSection {
    /// `START_CRIT_SECTION()`.
    fn enter() -> Self {
        let count = utils_error::config::crit_section_count();
        utils_error::config::set_crit_section_count(count + 1);
        CritSection
    }
}
...
pub fn init_seams() {
    generic_xlog_seams::generic_redo::set(generic_redo);
    generic_xlog_seams::generic_mask::set(generic_mask);
}
