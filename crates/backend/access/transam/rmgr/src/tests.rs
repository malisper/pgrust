...
thread_local! {
    ...
    // so we capture the by-value scalar word (`datum::Datum`, the `ByVal`
    // payload) — every emitted column is a by-value scalar here.
    static ROWS: RefCell<Vec<(Vec<datum::Datum>, Vec<bool>)>> =
...
fn setup() {
    ...
    INSTALL.call_once(|| {
        ...
        // the real `gist_core::gistxlog::{gist_xlog_startup,
        ...
        funcapi::InitMaterializedSRF::set(|fcinfo, _flags| {
            ...
            fcinfo.resultinfo = Some(nodes::funcapi::ReturnSetInfo::default());
            Ok(())
        });
        funcapi::materialized_srf_putvalues::set(|_rsinfo, values, nulls| {
            ...
            let words: Vec<datum::Datum> = values
                .iter()
                .map(|d| match d {
                    Datum::ByVal(w) => datum::Datum::from_usize(*w),
                    Datum::ByRef(b) => datum::Datum::from_usize(b.len()),
                    Datum::Cstring(_)
                    | Datum::Composite(_)
                    | Datum::Expanded(_)
                    | Datum::Internal(_) => {
        ...
        varlena::cstring_to_text_v::set(|mcx, s| {
            ...
            Ok(types_tuple::heaptuple::Datum::ByRef(bytes))
        });
    });
...
fn pg_get_wal_resource_managers_emits_one_row_per_existing_rmgr() {
    ...
    ROWS.with(|r| {
        ...
        assert_eq!(values[0], datum::Datum::from_i32(0));
        // col1 = CStringGetTextDatum("XLOG") -> stub encodes len("XLOG") = 4
        assert_eq!(values[1], datum::Datum::from_usize(4));
        // col2 = BoolGetDatum(RmgrIdIsBuiltin(0)) = true
        assert_eq!(values[2], datum::Datum::from_bool(true));

        // Last row is LogicalMessage (id 21), still builtin.
        let (values, _) = &rows[21];
        assert_eq!(values[0], datum::Datum::from_i32(21));
        assert_eq!(values[1], datum::Datum::from_usize("LogicalMessage".len()));
        assert_eq!(values[2], datum::Datum::from_bool(true));
    });
}
