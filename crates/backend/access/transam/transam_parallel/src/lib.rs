...
//! [`parallel_rt_seams`] and panics loudly until each
//! owner lands.
...
pub fn accumulate_shared_index_searches(
    shared_info: &mut nodes::SharedIndexScanInstrumentation,
    nsearches: u64,
) {
...
fn collect_fixed_parallel_state() -> PgResult<FixedParallelState> {
    let (current_user_id, sec_context) =
        matview_deps_seams::get_user_id_and_sec_context::call()?;
    let (temp_namespace_id, temp_toast_namespace_id) =
        namespace_seams::get_temp_namespace_state::call();
    Ok(FixedParallelState {
        database_id: init_small_seams::my_database_id::call(),
        authenticated_user_id: variable_seams::get_authenticated_user_id::call(),
        session_user_id: user_seams::get_session_user_id::call()?,
        outer_user_id: variable_seams::get_current_role_id::call(),
        current_user_id,
        temp_namespace_id,
        temp_toast_namespace_id,
        sec_context,
        session_user_is_superuser:
            variable_seams::get_session_user_is_superuser::call(),
        role_is_superuser: variable_seams::current_role_is_superuser::call(),
        parallel_leader_pgproc: 0,
        parallel_leader_pid: init_small_seams::my_proc_pid::call(),
        parallel_leader_proc_number: lmgr_proc_seams::my_proc_number::call(),
        xact_ts: transam_xact_seams::get_current_transaction_start_timestamp::call(),
        stmt_ts: transam_xact_seams::get_current_statement_start_timestamp::call(),
        serializable_xact_handle:
            predicate_seams::share_serializable_xact::call(),
        last_xlog_end: 0,
    })
}
...
pub fn initialize_parallel_dsm<'mcx>(mcx: Mcx<'mcx>, pcxt: ParallelContextHandle) -> PgResult<()> {
    ...
    if pcxt_nworkers(pcxt) > 0 {
        session_dsm_handle = session_seams::get_session_dsm_handle::call()?;
...
pub fn init_seams() {
    ...
    planner_seams::is_parallel_worker::set(is_parallel_worker);
    ...
    vacuum_seams::is_parallel_worker::set(|| Ok(is_parallel_worker()));
    ...
    execUtils_seams::is_parallel_worker::set(is_parallel_worker);
}
...
fn install_worker_state_seams() {
    ...
    rt::worker_install_signal_handlers::set(|| {
        legacy_pqsignal::pqsignal(
            libc::SIGTERM,
            signal::SigHandler::Handler(
                postgres_seams::die_signal_handler::call(),
            ),
        );
        bgworker_seams::background_worker_unblock_signals::call();
        Ok(())
    });
...
fn install_mmgr_context_seams() {
    ...
    rt::get_current_subtransaction_id::set(|| {
        Ok(transam_xact_seams::get_current_sub_transaction_id::call())
    });
...
fn install_execparallel_support_pcxt_seams() {
    ...
    sup::instr_init_slot::set(|sei, i, instrument_options| {
        ...
        unsafe {
            instrument::instr_init_slot_at(array_base, i, instrument_options)
        }
    });
    ...
    sup::report_instr_to_dsm::set(|sei, plan_node_id, worker, add| {
        ...
        unsafe {
            instrument::instr_agg_node_to_slot_at(array_base, slot, &add);
        }
        Ok(())
    });
    ...
    sup::retrieve_instr_from_dsm::set(|sei, plan_node_id| {
        ...
        for n in 0..header.num_workers {
            ...
            let instr =
                unsafe { instrument::instr_read_slot_at(array_base, base_slot + n) };
            out.push(instr);
        }
        Ok(out)
    });
...
mod dsm_substrate_tests {
    ...
    fn install_top_mcx_once() {
        INSTALL_TOP_MCX
            .call_once(|| mcxt_seams::top_memory_context::set(test_top_mcx));
    }
    ...
    fn install_shm_mq_seam_layer_once() {
        INSTALL_SHM_MQ.call_once(|| {
            ...
            lmgr_proc_seams::proc_latch::set(|procno| {
                LatchHandle::new(procno as usize + 1)
            });
            latch_seams::my_latch::set(|| LatchHandle::new(1));
            latch_seams::set_latch::set(|_latch| {});
            latch_seams::reset_latch::set(|_latch| {});
            latch_seams::wait_latch::set(|_latch, _events, _timeout, _wei| {
