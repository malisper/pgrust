...
pub(crate) use transam_parallel as parallel_seams;
pub(crate) use varsup_seams as varsup_seams;
pub(crate) use transam_xlog_seams as xlog_seams;
pub(crate) use xloginsert_seams as xloginsert_seams;
pub(crate) use spi_seams as spi_seams;
pub(crate) use sinval_seams as sinval_seams;
pub(crate) use lmgr_seams as lmgr_seams;
pub(crate) use predicate_seams as predicate_seams;
pub(crate) use lmgr_proc_seams as proc_seams;
pub(crate) use timestamp_seams as timestamp_seams;
pub(crate) use inval_seams as inval_seams;
pub(crate) use relmapper_seams as relmapper_seams;
pub(crate) use guc_file_seams as guc_seams;
pub(crate) use ::guc_seams as guc_core_seams;
pub(crate) use combocid_seams as combocid_seams;
pub(crate) use snapmgr_seams as snapmgr_seams;
...
pub use crate::wal::{XactLogAbortRecord, XactLogCommitRecord};
...
fn assign_transaction_id_at(idx: usize) -> PgResult<()> {
    ...
    let saved_owner = resowner_seams::set_current_to_cur_transaction_ancestor::call(levels_up);
    let insert_result = lmgr_seams::xact_lock_table_insert::call(full.xid());
    resowner_seams::set_CurrentResourceOwner::call(saved_owner);
    insert_result?;
...
fn seam_xact_redo(record: &mut ::wal::rmgr::XLogReaderState<'_>) -> PgResult<()> {
...
/// into a [`::wal::xact_records::XactLogCommitRecordArgs`]; the in-crate
...
pub fn init_seams() {
    use transam_xact_seams as seams;
    ...
    guc_funcs_seams::is_in_parallel_mode::set(IsInParallelMode);
    ...
    guc_funcs_seams::warn_no_transaction_block::set(|is_top_level, stmt_type| {
    ...
    tablecmds_seams::xact_accessed_temp_namespace::set(
    ...
    seams::save_transaction_characteristics::set(SaveTransactionCharacteristics);
    seams::restore_transaction_characteristics::set(RestoreTransactionCharacteristics);
    ...
    parallel_rt_seams::isolation_uses_xact_snapshot::set(
        IsolationUsesXactSnapshot,
    );
    parallel_rt_seams::start_transaction_command::set(
        StartTransactionCommand,
    );
    parallel_rt_seams::commit_transaction_command::set(
    ...
    parallel_rt_seams::set_parallel_start_timestamps::set(
        |xact_ts, stmt_ts| {
    ...
    matview_deps_seams::command_counter_increment::set(CommandCounterIncrement);
