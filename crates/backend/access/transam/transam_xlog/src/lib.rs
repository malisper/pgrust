...
pub fn GetWALAvailability(target_lsn: XLogRecPtr) -> WALAvailability {
    ...
    let oldest_slot_seg = {
        ...
        retention::KeepLogSeg(
            ...
            guc_tables::vars::max_slot_wal_keep_size_mb.read(),
            // IsBinaryUpgrade
            init_small::globals::IsBinaryUpgrade(),
            // keep = GetOldestUnsummarizedLSN(NULL, NULL);
            walsummarizer_seams::get_oldest_unsummarized_lsn::call()
                .unwrap_or(InvalidXLogRecPtr),
            // wal_keep_size_mb (GUC)
            guc_tables::vars::wal_keep_size_mb.read(),
        )
    };
    ...
    retention::GetWALAvailability(
        ...
        last_removed_segno,
        wal_segment_size,
        guc_tables::vars::max_wal_size_mb.read(),
    )
}
...
pub fn init_seams() {
    ...
    s::wal_level::set(|| {
        let lvl = guc_tables::vars::wal_level.read();
        match lvl {
    ...
    s::xlog_logical_info_active::set(|| {
        guc_tables::vars::wal_level.read() >= WalLevel::Logical as i32
    });
    ...
    s::xlog_standby_info_active::set(|| {
        guc_tables::vars::wal_level.read() >= WalLevel::Replica as i32
    });
    ...
    postmaster_seams::xlog_archiving_active::set(|| {
        guc_tables::vars::XLogArchiveMode.read()
    ...
    postmaster_seams::xlog_archiving_always::set(|| {
        guc_tables::vars::XLogArchiveMode.read()
    ...
    postgres_seams::local_process_control_file::set(shmem::LocalProcessControlFile);
    ...
    postmaster_seams::local_process_control_file::set(|reset| {
    ...
    postgres_seams::initialize_wal_consistency_checking::set(|| {
    ...
    s::in_recovery::set(xlogrecovery_seams::in_recovery::call);
    ...
    s::wal_keep_size_mb::set(|| guc_tables::vars::wal_keep_size_mb.read());
