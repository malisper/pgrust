...
fn cstring_get_text_datum<'mcx>(mcx: Mcx<'mcx>, s: &[u8]) -> PgResult<PgVec<'mcx, u8>> {
    varlena::keystone::cstring_to_text(mcx, s)
}
...
pub fn pg_backup_stop<'mcx>(
    ...
    let backup_label =
        xlogbackup::build_backup_content_default(&stopped_state, false)?;
...
pub fn pg_last_wal_receive_lsn() -> Option<XLogRecPtr> {
    ...
    let recptr = walreceiverfuncs::GetWalRcvFlushRecPtr(None, None);
    if recptr == 0 {
        None
    } else {
        Some(recptr)
...
pub fn pg_walfile_name<'mcx>(
    ...
    varlena::keystone::cstring_to_text(mcx, xlogfilename.as_bytes())
}
...
pub fn pg_split_walfile_name<'mcx>(
    ...
    let segment_number = adt_numeric::io::numeric_in(mcx, &buf, -1)?;
...
pub fn pg_get_wal_replay_pause_state<'mcx>(mcx: Mcx<'mcx>) -> PgResult<PgVec<'mcx, u8>> {
    ...
    varlena::keystone::cstring_to_text(mcx, statestr.as_bytes())
}
...
pub fn pg_wal_lsn_diff<'mcx>(
    ...
    lsn_trigfuncs::pg_lsn::pg_lsn_mi(mcx, lsn1, lsn2)
}
...
pub fn pg_promote(wait: bool, wait_seconds: i32) -> PgResult<bool> {
    ...
    match fd_seams::create_empty_file::call(PROMOTE_SIGNAL_FILE) {
        fd_seams::CreateEmptyFileOutcome::Ok => {}
        fd_seams::CreateEmptyFileOutcome::CreateFailed(errno) => {
        ...
        fd_seams::CreateEmptyFileOutcome::WriteFailed(errno) => {
            return ereport(ERROR)
                .with_saved_errno(errno)
                .errcode_for_file_access()
    ...
    if unsafe { libc::kill(postmaster_pid, libc::SIGUSR1) } != 0 {
        ...
        let _rc = fd_seams::unlink_file::call(PROMOTE_SIGNAL_FILE);
...
fn my_latch() -> types_storage::latch::LatchHandle {
    latch::my_latch().expect("MyLatch is NULL in pg_promote")
}
