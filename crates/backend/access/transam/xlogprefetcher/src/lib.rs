...
pub fn init_seams() {
    ...
    xlogprefetcher_seams::xlog_prefetch_shmem_size::set(
        XLogPrefetchShmemSize,
    );
    xlogprefetcher_seams::xlog_prefetch_shmem_init::set(
        XLogPrefetchShmemInit,
    );
    ...
    guc_tables::hooks::check_recovery_prefetch.install(
        |new_value, _extra, _source| {
            ...
            match check_recovery_prefetch(*new_value) {
                Ok(()) => Ok(true),
                Err(e) => {
                    guc_seams::guc_check_errdetail::call(e.message().to_string());
                    Ok(false)
                }
            }
        },
