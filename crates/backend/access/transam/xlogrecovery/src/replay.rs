...
use ipc_seams as ipc_seam;
use pmsignal_seams as pmsignal_seam;
...
pub fn perform_wal_recovery<'mcx>(
    ...
    if st.redo_start_lsn < st.check_point_loc {
        ...
        let r = reader_state();
        if xlogreader::XLogRecGetRmid(r) != RM_XLOG_ID
            || (xlogreader::XLogRecGetInfo(r) & !XLR_INFO_MASK)
    ...
    if record != RecordRef::default() {
        let ru0 = pg_rusage::pg_rusage_new();
...
pub(crate) fn apply_wal_record(
    ...
    let xl_xid = xlogreader::XLogRecGetXid(reader_state());
    ...
    let xl_rmid = xlogreader::XLogRecGetRmid(reader_state());
    ...
    if xl_rmid == RM_XLOG_ID {
        ...
        let info = xlogreader::XLogRecGetInfo(reader_state()) & !XLR_INFO_MASK;

        if info == XLOG_CHECKPOINT_SHUTDOWN {
            let data = xlogreader::XLogRecGetData(reader_state());
    ...
    if (xlogreader::XLogRecGetInfo(reader_state()) & XLR_CHECK_CONSISTENCY)
        != 0
    {
...
pub(crate) fn xlogrecovery_redo(
    ...
    debug_assert!(xlogreader::XLogRecGetRmid(reader_state()) == RM_XLOG_ID);
...
pub(crate) fn check_time_line_switch(
    ...
    if new_tli < replay_tli
        || !timeline_seams::tli_in_history::call(
            new_tli,
            &st.expected_tles,
        )
    {
...
pub(crate) fn verify_backup_page_consistency(
    ...
    debug_assert!(
        (xlogreader::XLogRecGetInfo(reader_state()) & XLR_CHECK_CONSISTENCY)
            != 0
    );
    ...
    for block_id in 0..=max_block_id as u8 {
        ...
        debug_assert!(xlogreader::xlog_rec_has_block_image(
            reader_state(),
            block_id
        ));

        if xlogreader::xlog_rec_block_image_apply(reader_state(), block_id) {
        ...
        if !xlogreader::restore_block_image_bytes(
            reader_state(),
            block_id,
...
fn read_max_block_id() -> i32 {
    xlogreader::reader_max_block_id(reader_state())
}
