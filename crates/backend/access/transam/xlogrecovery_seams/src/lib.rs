...
seam_core::seam!(
    ...
    pub fn init_wal_recovery<'mcx>(
        control_file: &mut control::ControlFileData,
        mcx: mcx::Mcx<'mcx>,
    ) -> types_error::PgResult<wal::xlogrecovery_carriers::InitWalRecoveryResult>
);
...
seam_core::seam!(
    ...
    pub fn finish_wal_recovery<'mcx>(
        mcx: mcx::Mcx<'mcx>,
    ) -> types_error::PgResult<wal::xlogrecovery_carriers::EndOfWalRecoveryInfo>
);
...
seam_core::seam!(
    /// `recoveryEndCommand` (xlogrecovery.c global) — the configured
    /// `recovery_end_command`, or `None`/empty when unset. Returned charged to
    /// `mcx` (the `pstrdup` analog). Consumed by `CleanupAfterArchiveRecovery`
    /// (xlog.c) at the conclusion of archive recovery.
    pub fn recovery_end_command<'mcx>(
        mcx: mcx::Mcx<'mcx>,
    ) -> Option<mcx::PgString<'mcx>>
);
...
seam_core::seam!(
    ...
    /// spinlock.
    pub fn get_recovery_state() -> types_error::PgResult<wal::RecoveryState>
);
...
seam_core::seam!(
    ...
    pub fn get_recovery_pause_state() -> wal::RecoveryPauseState
);
