...
pub fn CreateFakeRelcacheEntry(
    mcx: mcx::Mcx<'_>,
    rlocator: RelFileLocator,
) -> PgResult<rel::RelationData<'_>> {
    relcache_seams::create_fake_relcache_entry::call(mcx, rlocator)
}

/// Free a fake relation cache entry (`FreeFakeRelcacheEntry`, lines 617-621).
pub fn FreeFakeRelcacheEntry(fakerel: rel::RelationData<'_>) {
    relcache_seams::free_fake_relcache_entry::call(fakerel)
}
...
fn read_local_xlog_page_guts(
    ...
    loop {
        ...
        if reader_seam::reader_curr_tli::call(state) == currTLI {
            ...
            pgsleep_seams::pg_usleep::call(1000);
        } else {
...
pub fn init_seams() {
    ...
    xlogutils_seams::standby_state::set(standby_state);
    xlogutils_seams::set_standby_state::set(set_standby_state);
    xlogutils_seams::in_recovery::set(in_recovery);
    xlogutils_seams::set_in_recovery::set(set_in_recovery);
    xlogutils_seams::ignore_invalid_pages::set(ignore_invalid_pages);
    xlogutils_seams::set_ignore_invalid_pages::set(set_ignore_invalid_pages);
    // `bool ignore_invalid_pages` (xlogutils.c GUC, boot false) — install the
    // guc-tables slot over this crate's backing accessors.
    guc_tables::vars::ignore_invalid_pages.install(
    ...
    xlogutils_seams::xlog_read_buffer_for_redo_extended::set(
        |record, block_id, mode, get_cleanup_lock| {
            XLogReadBufferForRedoExtended(record, block_id, mode, get_cleanup_lock)
        },
    );
    xlogutils_seams::xlog_init_buffer_for_redo::set(|record, block_id| {
    ...
    xlogutils_seams::xlog_read_buffer_extended_fsm::set(|rlocator, blkno| {
