...
pub fn SendBaseBackup<'mcx>(
    ...
    if status == wal::SessionBackupState::Running {
    ...
    match opt.compression {
        PgCompressAlgorithm::Gzip => {
            sink = gzip::bbsink_gzip_new(mcx, sink, &opt.compression_specification);
        }
        PgCompressAlgorithm::Lz4 => {
            sink = lz4::bbsink_lz4_new(mcx, sink, &opt.compression_specification);
        }
        PgCompressAlgorithm::Zstd => {
            sink = zstd::bbsink_zstd_new(mcx, sink, &opt.compression_specification);
        }
...
fn send_wal_files<'mcx>(
    ...
    for de in fd_seams::read_dir_names::call("pg_wal")? {
    ...
    for wal_file_name in &wal_file_list {
        ...
        let fd = fd_seams::open_transient_file::call(&pathbuf, OPEN_FLAGS);
        if fd < 0 {
...
fn pg_strcasecmp(a: &str, b: &str) -> i32 {
    pgstrcasecmp::pg_strcasecmp(a.as_bytes(), b.as_bytes())
}
...
fn sendDir<'mcx>(
    ...
    for d_name in fd_seams::read_dir_names::call(path)? {
...
fn sendFile<'mcx>(
    ...
    let fd = fd_seams::open_transient_file::call(readfilename, OPEN_FLAGS);
    if fd < 0 {
        if fd_seams::last_errno::call() == _ENOENT && missing_ok {
            return Ok(false);
        }
        return Err(ereport(ERROR)
            .errcode_for_file_access()
...
fn basebackup_read_file(
    ...
    let rc = fd_seams::pg_pread::call(fd, buf, offset);
...
pub fn init_seams() {
    replication_basebackup_seams::send_base_backup::set(send_base_backup_entry);
