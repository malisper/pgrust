...
use cryptohash_seams as cryptohash;
use sha2::{PG_SHA256_DIGEST_LENGTH, PG_SHA256_DIGEST_STRING_LENGTH};
...
pub fn InitializeBackupManifest<'mcx>(
    ...
    *manifest = BackupManifestInfo::zeroed();
    ...
    if want_manifest != MANIFEST_OPTION_NO {
        let system_identifier = transam_xlog_seams::get_system_identifier::call();
