...
fn boot_open_stmt(mcx: Mcx<'static>, p: &mut Parser) -> PgResult<()> {
    ...
    do_start();
    bootstrap::boot_openrel(mcx, &name)?;
    do_end()?;
    Ok(())
}
...
fn boot_close_stmt(mcx: Mcx<'static>, p: &mut Parser) -> PgResult<()> {
    ...
    do_start();
    bootstrap::closerel(mcx, Some(&name))?;
    do_end()?;
    Ok(())
}
...
fn boot_create_stmt(mcx: Mcx<'static>, p: &mut Parser) -> PgResult<()> {
    ...
    for i in 0..numattr as usize {
        attrs.push(
            bootstrap::attrtypes(i)
                .expect("attrtypes[i] set by DefineAttr for i < numattr"),
        );
    }
    let tupdesc = tupdesc::CreateTupleDesc(mcx, &attrs)?;
    ...
    if optbootstrap != 0 {
        ...
        bootstrap::set_boot_reldesc(Some(rel));

        ereport(DEBUG4)
            .errmsg_internal("bootstrap relation created")
            .finish(loc(221, "Boot_CreateStmt"))?;
    } else {
        let id = heap_seams::heap_create_with_catalog::call(
            heap_seams::HeapCreateWithCatalogArgs {
...
fn boot_column_def(mcx: Mcx<'static>, p: &mut Parser) -> PgResult<()> {
    ...
    let numattr = bootstrap::numattr() + 1;
    bootstrap::set_numattr(numattr);
    if numattr as usize > bootstrap::MAXATTR {
        return ereport(FATAL)
            .errmsg_internal("too many columns")
            .finish(loc(446, "boot_column_def"));
    }
    /* DefineAttr($1, $3, numattr-1, $4); */
    bootstrap::DefineAttr(mcx, &name, &type_, numattr - 1, nullness)?;
    Ok(())
}
...
fn boot_column_nullness(p: &mut Parser) -> PgResult<i32> {
    if p.peek() == BootTokenKind::Force {
        ...
        if p.peek() == BootTokenKind::Not {
            ...
            Ok(bootstrap::BOOTCOL_NULL_FORCE_NOT_NULL)
        } else {
            p.expect(BootTokenKind::Null)?; // XNULL
            Ok(bootstrap::BOOTCOL_NULL_FORCE_NULL)
        }
    } else {
        Ok(bootstrap::BOOTCOL_NULL_AUTO)
    }
}
...
fn boot_column_val(mcx: Mcx<'static>, p: &mut Parser) -> PgResult<()> {
    if p.peek() == BootTokenKind::NullVal {
        ...
        bootstrap::InsertOneNull(n)?;
    } else {
        let value = p.boot_ident()?;
        let n = num_columns_read();
        set_num_columns_read(n + 1);
        bootstrap::InsertOneValue(mcx, &value, n)?;
    }
    Ok(())
}
...
fn boot_declare_index_stmt(mcx: Mcx<'static>, p: &mut Parser, unique: bool) -> PgResult<()> {
    ...
    let relation_id = namespace_seams::range_var_get_relid::call(
        mcx,
        &lookup_rv,
        types_storage::lock::NoLock,
        false,
    ...
    indexcmds_seams::define_index::call(
        mcx,
        indexcmds_seams::DefineIndexArgs {
...
fn boot_declare_toast_stmt(mcx: Mcx<'static>, p: &mut Parser) -> PgResult<()> {
    ...
    do_start();
    toasting::BootstrapToastTable(mcx, &relname, toast_oid, toast_index_oid)?;
    do_end()?;
    Ok(())
}
...
fn boot_index_params<'mcx>(
    ...
    let mut params: PgVec<'mcx, nodes::nodes::NodePtr<'mcx>> = PgVec::new_in(mcx);
...
fn boot_index_param<'mcx>(
    ...
    let mut opclass: PgVec<'mcx, nodes::nodes::NodePtr<'mcx>> = PgVec::new_in(mcx);
    ...
    let elem = nodes::ddlnodes::IndexElem {
        ...
        ordering: nodes::rawnodes::SortByDir::SORTBY_DEFAULT,
...
/// (`nodes::rawnodes::RangeVar`, the parse-tree shape).
...
pub fn init_seams() {
    bootparse_seams::boot_yylex_init::set(boot_yylex_init);
    bootparse_seams::boot_yyparse::set(boot_yyparse);
}
