...
pub fn BootstrapModeMain(mcx: Mcx<'static>, argv: Vec<String>, check_only: bool) -> PgResult<()> {
    ...
    miscinit_seams::init_standalone_process::call(&argv[0])?;
    ...
    loop {
        ...
        match flag {
            ...
            '-' => {
                ...
                if main_seams::parse_dispatch_option::call(&optarg)
    ...
    if !guc_seams::select_config_files::call(userDoption.as_deref(), &progname)? {
        ipc_seams::proc_exit::call(1);
    }

    /* Validate the DataDir and change into it */
    miscinit_seams::check_data_dir::call()?;
    miscinit_seams::change_to_data_dir::call()?;

    miscinit_seams::create_data_dir_lock_file::call(false)?;

    miscinit_seams::set_processing_mode_bootstrap::call();
    miscinit_seams::set_ignore_system_indexes::call(true);

    postinit_seams::initialize_max_backends::call()?;
     ...
     */
    fd_seams::set_max_safe_fds::call()?;
    ...
    if bootparse_seams::boot_yylex_init::call() != 0 {
        return ereport(ERROR)
     ...
     */
    relmapper_seams::relation_map_finish_bootstrap::call()?;
...
fn handle_c_option(mcx: Mcx<'static>, flag: char, getopt: &Getopt) -> PgResult<()> {
    ...
    let (name, value) = guc_seams::parse_long_option::call(mcx, &optarg)?;
    match value {
        None => {
            if flag == '-' {
                return ereport(ERROR)
...
fn set_config_option_argv(name: &str, value: &str) -> PgResult<()> {
    guc_seams::set_config_option::call(name, value, PGC_POSTMASTER, PGC_S_ARGV)
}
...
pub fn InsertOneTuple(mcx: Mcx<'static>) -> PgResult<()> {
    ...
    with_boot_reldesc_rel_res(|rel| {
        heapam_seams::insert_one_tuple::call(mcx, rel, &attr, &vals, &nulls)
    })?;
...
pub fn InsertOneValue(mcx: Mcx<'static>, value: &str, i: i32) -> PgResult<()> {
    ...
    let datum = fmgr_seams::oid_input_function_call::call(
        mcx,
        io.typinput,
        value,
        io.typioparam,
...
fn populate_typ_list(mcx: Mcx<'static>) -> PgResult<()> {
    ...
    let rows = heapam_seams::read_pg_type::call(mcx)?;
...
pub fn build_indices(mcx: Mcx<'static>) -> PgResult<()> {
    ...
    while let Some(mut entry) = il_head_pop() {
        ...
        let heap = table::table_open(mcx, entry.il_heap, types_storage::lock::NoLock)?;
        let ind = indexam_seams::index_open::call(mcx, entry.il_ind, types_storage::lock::NoLock)?;

        index_seams::index_build::call(mcx, &heap, &ind, &mut entry.il_info)?;
