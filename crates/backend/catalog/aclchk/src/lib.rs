...
use types_core::catalog::{
    BOOTSTRAP_SUPERUSERID, DATABASE_RELATION_ID, EXTENSION_RELATION_ID, LANGUAGE_RELATION_ID,
    NAMESPACE_RELATION_ID, PROCEDURE_RELATION_ID, RELATION_RELATION_ID, TYPE_RELATION_ID,
};
use types_core::Oid;
use types_error::{
...
pub fn pg_class_aclmask_ext(
    ...
    if mask & SYSTEM_WRITE != 0 {
        ...
        let is_system_class =
            catalog::IsCatalogRelationOid(table_oid) || catalog::IsToastNamespace(relnamespace);
...
pub fn aclcheck_error(aclerr: i32, objtype: ObjectType, objectname: &str) -> PgResult<()> {
    match aclerr {
        ...
        ACLCHECK_NO_PRIV => Err(Box::new(
            ...
            // C aclchk.c:2795 (18.6): the ACLCHECK_NO_PRIV ereport
            .with_location("aclchk.c", 2795, "aclcheck_error"),
        )),
        ACLCHECK_NOT_OWNER => {
