...
pub fn object_aclmask_ext(
    ...
    let proj = syscache_seams::object_owner_acl::call(
        mcx,
        cacheid,
        objectid,
        owner_attnum,
...
pub fn pg_attribute_aclmask_ext(
    ...
    let att = syscache_seams::pg_attribute_owner_acl::call(
     ...
     */
    let cls = syscache_seams::pg_class_owner_acl::call(mcx, table_oid)?;
...
pub fn pg_class_aclmask_ext(
    ...
    if (mask & (ACL_INSERT | ACL_UPDATE | ACL_DELETE | ACL_TRUNCATE | ACL_USAGE)) != 0
        && catalog_seams::is_system_class_by_namespace::call(
            table_oid,
            cls.relnamespace,
        )
...
pub fn pg_parameter_aclmask(
    ...
    let parname = misc_guc::convert_guc_name_for_parameter_acl(name);

    /* ... and look it up */
    let proj = syscache_seams::parameter_acl_by_name::call(mcx, &parname)?;
...
pub fn pg_parameter_acl_aclmask(
    ...
    let proj = syscache_seams::parameter_acl_by_oid::call(mcx, acl_oid)?;
...
pub fn pg_largeobject_aclmask_snapshot(
    ...
    let proj = pg_largeobject_seams::largeobject_owner_acl::call(
...
pub fn pg_namespace_aclmask_ext(
     ...
     */
    if namespace_seams::is_temp_namespace::call(nsp_oid)? {
    ...
    let nsp = syscache_seams::pg_namespace_owner_acl::call(mcx, nsp_oid)?;
...
pub fn pg_type_aclmask_ext(
    ...
    let ty = syscache_seams::pg_type_owner_acl::call(mcx, type_oid)?;
    let Some(ty) = ty else {
        if is_missing {
            return Ok((0, true));
        }
...
pub fn pg_attribute_aclcheck_all_ext(
    ...
    let extra = syscache_seams::pg_class_extra::call(table_oid)?;
...
pub fn aclcheck_error_type(mcx: Mcx<'_>, aclerr: AclResult, type_oid: Oid) -> PgResult<()> {
    let element_type = lsyscache_seams::get_element_type::call(type_oid)?;
    let effective = match element_type {
        Some(et) if OidIsValid(et) => et,
        _ => type_oid,
    };
    let formatted = format_type_seams::format_type_be::call(mcx, effective)?;
...
pub fn has_createrole_privilege(roleid: Oid) -> PgResult<bool> {
    ...
    let result = match syscache_seams::lookup_authid_by_oid::call(
        ctx.mcx(),
        roleid,
    )? {
...
pub fn init_seams() {
    ...
    // bits.
    guc_funcs_seams::pg_parameter_aclcheck_ok::set(|name, roleid, mode| {
    ...
    // `aclcheck_error(aclresult, OBJECT_SCHEMA, ...)`).
    {
        ...
        fc::get_func_name::set(|func_oid| {
            ...
            let name = lsyscache_seams::get_func_name::call(ctx.mcx(), func_oid)?;
    ...
    utility_out_seams::execute_grant_stmt_slow::set(|mcx, stmt| {
    ...
    utility_out_seams::execute_grant_stmt::set(|mcx, stmt| {
        grant_exec::execute_grant_stmt(mcx, stmt)
    });
    ...
    utility_out_seams::exec_alter_default_privileges_stmt::set(|mcx, _pstate, stmt| {
    ...
    // OBJECT_COLLATION, ...)`.
    collationcmds_seams::collation_ownercheck::set(|coll_oid, roleid| {
