...
//! the access boundary via [`genam_seams`]); the
...
/// Vec<u64> }` word storage `pull_varattnos` produces (`pathnodes`,
/// distinct from the `nodes` planner-relids set the nodes-core ops use).
...
pub fn init_seams() {
    partition_seams::get_partition_parent::set(get_partition_parent);
    partition_seams::get_partition_ancestors::set(get_partition_ancestors);
    partition_seams::map_partition_varattnos::set(map_partition_varattnos);
    partition_seams::has_partition_attrs::set(has_partition_attrs_seam);
}
