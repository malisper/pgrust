...
seam_core::seam!(
    ...
    pub fn is_catalog_relation(relation: &rel::RelationData<'_>) -> bool
);
...
seam_core::seam!(
    ...
    pub fn is_toast_relation(relation: &rel::RelationData<'_>) -> bool
);
...
seam_core::seam!(
    /// `IsToastNamespace(namespaceId)` (catalog/catalog.c): true iff the
    /// namespace is `pg_toast` (`namespaceId == PG_TOAST_NAMESPACE`) or my
    /// temporary-toast-table namespace. No catalog access — infallible.
...
seam_core::seam!(
    ...
    pub fn is_system_relation(rel: &rel::Relation<'_>) -> PgResult<bool>
);
seam_core::seam!(
