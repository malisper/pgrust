...
use smgr as smgr;
use xloginsert as xloginsert;
use xlogutils as xlogutils;
use freespace as freespace;
use visibilitymap as visibilitymap;
use page as page;
use relpath as relpath;
use table as table;
...
use bulkwrite_seams as bulkwrite_seam;
...
/// `XLR_INFO_MASK`.
const XLR_INFO_MASK: u8 = wal::wal::XLR_INFO_MASK;
...
/// Convert the WAL-record `RelFileLocator` (`wal`) to the storage-layer
/// one (`types_storage`); the two are byte-identical (three `Oid`s) but live in
/// different type crates.
#[inline]
fn from_wal_locator(loc: wal::RelFileLocator) -> RelFileLocator {
    RelFileLocator {
        spcOid: loc.spc_oid(),
        dbOid: loc.db_oid(),
...
fn xlog_is_needed() -> PgResult<bool> {
    Ok(xlog_seam::wal_level::call() >= wal::xlog_consts::WAL_LEVEL_REPLICA)
}
...
pub fn create_and_copy_relation_data(
    ...
    // copy main fork.
    bufmgr_seams::relation_copy_storage_using_buffer::call(
        src_rlocator,
        dst_rlocator,
        MAIN_FORKNUM,
        permanent,
...
fn drop_storage_assume_new_relfilelocator(rel: &rel::Relation<'_>) -> PgResult<()> {
...
pub fn RelationTruncate(rel: &rel::Relation<'_>, nblocks: BlockNumber) -> PgResult<()> {
...
pub fn RelationPreTruncate(rel: &rel::Relation<'_>) -> PgResult<()> {
...
/// consumers use.
pub fn drop_relation_files(rels: &[wal::RelFileLocator]) -> PgResult<()> {
...
pub fn smgr_redo(record: &mut wal::rmgr::XLogReaderState<'_>) -> PgResult<()> {
    ...
    if info == XLOG_SMGR_CREATE {
        ...
        let xlrec = wal::rmgrdesc::xl_smgr_create::from_bytes(data)
        ...
        let xlrec = wal::rmgrdesc::xl_smgr_truncate::from_bytes(data)
...
pub fn init_seams() {
    ...
    vacuumlazy_seams::relation_truncate::set(|rel, nblocks| {
