...
pub fn cookDefault<'mcx>(
    ...
    let mut expr: Expr<'mcx> =
        match parse_expr::transformExpr(pstate, Some(raw_node), expr_kind)? {
    ...
    if attgenerated != 0 {
        ...
        if clauses::deferred::contain_mutable_functions_after_planning(
            mcx,
            expr.clone_in(mcx)?,
        )? {
         ...
         * column references.
         */
        debug_assert!(!vars::var::contain_var_clause(&Node::mk_expr(mcx, 
            expr.clone_in(mcx)?
        )?));
    }
    ...
    if OidIsValid(atttypid) {
        let type_id = nodes_core::nodefuncs::expr_type(Some(&expr))?;
     ...
     */
    parse_collate::assign_expr_collations(Some(pstate), &mut expr)?;
...
fn cookConstraint<'mcx>(
     ...
     */
    let expr = coerce::coerce_to_boolean(mcx, Some(pstate), expr, "CHECK")?;
...
fn check_nested_generated_walker(
    ...
    match node.node_tag() {
        ntag::T_Var => {
            ...
            if attnum > 0 {
                match lsyscache_seams::get_attgenerated::call(relid, attnum) {
                    Ok(g) if g != 0 => {
                        let colname = match lsyscache_seams::get_attname::call(
                            mcx,
                            relid,
                            attnum,
                            false,
        ...
        _ => nodes_core::node_walker::expression_tree_walker(node, &mut |n| {
...
fn check_virtual_generated_security_walker(
    ...
    if !node.is_list() {
        if let Some(expr) = node.as_expr() {
            ...
            match nodes_core::nodefuncs::check_functions_in_node(&mut e_owned, &mut checker) {
                Ok(true) => {
                    let location =
                        nodeFuncs_seams::exprLocation::call(expr);
                    let e = ereport(ERROR)
             ...
             * types.
             */
            match nodes_core::nodefuncs::expr_type(Some(expr)) {
...
fn StoreRelCheck<'mcx>(
     ...
     */
    let var_list = vars::var::pull_var_clause(mcx, expr, 0)?;
     ...
     */
    let constr_oid = pg_constraint::CreateConstraintEntry(
        mcx,
        ccname,
...
fn SetRelationNumChecks<'mcx>(_mcx: Mcx<'mcx>, rel: &Relation<'mcx>, numchecks: i32) -> PgResult<()> {
    ...
    let cur = syscache_seams::fetch_relchecks::call(rel.rd_id)?;
    ...
    if cur as i32 != numchecks {
        ...
        heap_seams::set_relation_num_checks::call(rel.rd_id, numchecks)?;
    } else {
...
fn MergeWithExistingConstraint<'mcx>(
    ...
    pg_constraint::MergeWithExistingConstraint(
        mcx,
        rel.rd_id,
...
pub fn AddRelationNewConstraints<'mcx>(
    ...
    for (attnum, raw_default, generated) in new_col_defaults.iter() {
        ...
        let def_oid =
            pg_attrdef::StoreAttrDefault(mcx, rel.rd_id, attnum, &expr_node, is_internal)?;
    ...
    for cdef_node in new_constraints.iter() {
        ...
        if cdef.contype == ConstrType::CONSTR_CHECK {
            ...
            if let Some(raw) = cdef.raw_expr.as_ref() {
                ...
                expr_node = read_seams::string_to_node::call(mcx, &cooked)?;
            }
            ...
            if let Some(cn) = cdef.conname.as_ref() {
                 ...
                 */
                let vars = vars::var::pull_var_clause(mcx, &expr_node, 0)?;
                ...
                let colname: Option<String> = if uniq_attnos.len() == 1 {
                    lsyscache_seams::get_attname::call(
                        mcx,
                        rel.rd_id,
                        uniq_attnos[0],
                        true,
                ...
                ccname = pg_constraint::ChooseConstraintName(
                    mcx,
                    &rel.rd_rel.relname.as_str().to_string(),
            ...
            let colnum = lsyscache_seams::get_attnum::call(rel.rd_id, &keyname)?;
            ...
            if let Some(cn) = cdef_conname.as_ref() {
                ...
                nnname = pg_constraint::ChooseConstraintName(
                    mcx,
                    &rel.rd_rel.relname.as_str().to_string(),
                    &keyname,
                    "not_null",
...
pub fn AddRelationNotNullConstraints<'mcx>(
    ...
    while outerpos < work.len() {
        let attnum = lsyscache_seams::get_attnum::call(
            rel.rd_id,
        ...
        if let Some(cn) = work[outerpos].conname.clone() {
            ...
            let colname = lsyscache_seams::get_attname::call(
                mcx, rel.rd_id, attnum, false,
            )?
            .map(|s| s.as_str().to_string())
            .unwrap_or_default();
            conname = pg_constraint::ChooseConstraintName(
                mcx,
                &rel.rd_rel.relname.as_str().to_string(),
                &colname,
                "not_null",
...
pub fn RemoveAttributeById<'mcx>(mcx: Mcx<'mcx>, relid: Oid, attnum: AttrNumber) -> PgResult<()> {
    ...
    loop {
        ...
        let (attnum_val, _) = heaptuple::heap_getattr(
            mcx,
            &tuple,
...
pub fn RelationClearMissing<'mcx>(mcx: Mcx<'mcx>, rel: &Relation<'mcx>) -> PgResult<()> {
    ...
    loop {
        ...
        let (hasmissing, _) = heaptuple::heap_getattr(
            mcx,
            &tuple,
...
pub fn SetAttributeHasDefault<'mcx>(
    ...
    loop {
        ...
        let (this_attnum, _) = heaptuple::heap_getattr(
            mcx,
            &tuple,
            Anum_pg_attribute_attnum as i32,
            &attr_rel.rd_att,
        ...
        {
            {
                ...
                let (attgenerated, _) = heaptuple::heap_getattr(
                    mcx,
                    &tuple,
...
pub fn StoreAttrMissingVal<'mcx>(
    ...
    loop {
        ...
        // attStruct = (Form_pg_attribute) GETSTRUCT(atttup);
        let (atttypid_d, _) = heaptuple::heap_getattr(
            mcx,
            &tuple,
            Anum_pg_attribute_atttypid as i32,
            &attr_rel.rd_att,
        )?;
        let (attlen_d, _) = heaptuple::heap_getattr(
            mcx,
            &tuple,
            Anum_pg_attribute_attlen as i32,
            &attr_rel.rd_att,
        )?;
        let (attbyval_d, _) = heaptuple::heap_getattr(
            mcx,
            &tuple,
            Anum_pg_attribute_attbyval as i32,
            &attr_rel.rd_att,
        )?;
        let (attalign_d, _) = heaptuple::heap_getattr(
            mcx,
            &tuple,
        ...
        let arr = arrayfuncs::construct::construct_array_values(
            mcx,
