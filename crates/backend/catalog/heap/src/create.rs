...
pub fn InsertPgClassTuple<'mcx>(
    ...
    indexing_seams::catalog_tuple_insert_pg_class::call(mcx, pg_class_desc, &row)
}
...
pub fn InsertPgAttributeTuples<'mcx>(
    ...
    indexing_seams::catalog_insert_pg_attribute_tuples::call(
        mcx,
        pg_attribute_rel,
        &rows,
    )
...
pub fn AddNewAttributeTuples<'mcx>(
     ...
     */
    let rel = table::table_open(mcx, AttributeRelationId, RowExclusiveLock)?;
    ...
    for i in 0..natts as usize {
        ...
        pg_depend::recordDependencyOn(mcx, &myself, &referenced, DEPENDENCY_NORMAL)?;
    ...
    if relkind != RELKIND_VIEW && relkind != RELKIND_COMPOSITE_TYPE {
        ...
        let sysatts = sys_att();
        let td = tupdesc::CreateTupleDesc(mcx, &sysatts)?;
...
pub fn heap_create_with_catalog<'mcx>(
    ...
    let pg_class_desc =
        table::table_open(mcx, RelationRelationId, RowExclusiveLock)?;
    ...
    if !OidIsValid(relid) {
        ...
        if binary_upgrade_seams::is_binary_upgrade::call() {
            ...
            if relkind == RELKIND_TOASTVALUE {
                ...
                let next =
                    binary_upgrade_seams::consume_next_pg_class_oid::call(true);
                if OidIsValid(next) {
                    relid = next;

                    let next_rfn =
                        binary_upgrade_seams::consume_next_pg_class_relfilenumber::call(true);
                ...
                let next =
                    binary_upgrade_seams::consume_next_pg_class_oid::call(false);
                if !OidIsValid(next) {
                    return Err(ereport(ERROR)
                        .errcode(ERRCODE_INVALID_PARAMETER_VALUE)
                ...
                if RELKIND_HAS_STORAGE(relkind) {
                    let next_rfn =
                        binary_upgrade_seams::consume_next_pg_class_relfilenumber::call(false);
                    if !RelFileNumberIsValid(next_rfn) {
                        return Err(ereport(ERROR)
                            .errcode(ERRCODE_INVALID_PARAMETER_VALUE)
        ...
        if !OidIsValid(relid) {
            relid = catalog_catalog::GetNewRelFileNumber(
                mcx,
                reltablespace,
                Some(&pg_class_desc),
                relpersistence,
    ...
    let relacl: Option<types_tuple::heaptuple::Datum<'mcx>> = if use_user_acl {
        match relkind {
            ...
            {
                aclchk_seams::get_user_default_acl::call(
                    mcx,
                    ObjectType::Table,
                    ownerid,
                    relnamespace,
            ...
            x if x == RELKIND_SEQUENCE => aclchk_seams::get_user_default_acl::call(
                mcx,
                ObjectType::Sequence,
                ownerid,
                relnamespace,
    ...
    let new_rel_desc = common_relation::relation_open(mcx, relid, NoLock)?;
        ...
        || relkind == RELKIND_PARTITIONED_INDEX)
    {
         ...
         */
        let new_array_oid = typecmds::AssignTypeArrayOid()?;
    ...
    if oncommit != OnCommitAction::ONCOMMIT_NOOP {
        tablecmds_seams::register_on_commit_action::call(relid, oncommit)?;
    }
