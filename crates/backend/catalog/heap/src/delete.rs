...
pub fn DeleteRelationTuple<'mcx>(mcx: Mcx<'mcx>, relid: Oid) -> PgResult<()> {
    ...
    let pg_class_desc = table::table_open(
        mcx,
        RelationRelationId,
    ...
    let Some((tid, _form)) =
        syscache_seams::search_syscache_copy_pg_class::call(mcx, relid)?
    else {
    ...
    indexing_seams::catalog_tuple_delete::call(&pg_class_desc, tid)?;
...
pub fn DeleteAttributeTuples<'mcx>(mcx: Mcx<'mcx>, relid: Oid) -> PgResult<()> {
    ...
    loop {
        ...
        indexing_seams::catalog_tuple_delete::call(&attrel, atttup.tuple.t_self)?;
    }
...
pub fn RelationRemoveInheritance<'mcx>(mcx: Mcx<'mcx>, relid: Oid) -> PgResult<()> {
    ...
    let catalog_relation = table::table_open(
        mcx,
        InheritsRelationId,
        types_storage::lock::RowExclusiveLock,
    )?;
    ...
    loop {
        ...
        indexing_seams::catalog_tuple_delete::call(
            &catalog_relation,
            tuple.tuple.t_self,
        )?;
    }
