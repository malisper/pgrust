...
pub fn heap_create<'mcx>(
    ...
    if !allow_system_table_mods
        && ((catalog_catalog::IsCatalogNamespace(relnamespace) && relkind != RELKIND_INDEX)
            || catalog_catalog::IsToastNamespace(relnamespace))
     ...
     */
    if reltablespace == tablespace_globals_seams::MyDatabaseTableSpace::call()? {
        reltablespace = InvalidOid;
    }
     ...
     */
    let rel = relcache::initfile::RelationBuildLocalRelation(
        relname,
        relnamespace,
        tup_desc,
        relid,
    ...
    let rel_data = common_relation::relation_open_prebuilt(mcx, rel)?;
    ...
    if create_storage {
        ...
        if RELKIND_HAS_TABLE_AM(rd_relkind) {
            ...
            let (fx, mm) =
                table_tableam_seams::table_relation_set_new_filelocator::call(
                    &rel_data,
                    rel_data.rd_locator,
                    relpersistence as i8,
                )?;
    ...
    if !create_storage && reltablespace != InvalidOid {
        pg_shdepend_seams::recordDependencyOnTablespace::call(
            RelationRelationId,
            relid,
            reltablespace,
        )?;
    ...
    pgstat_seams::pgstat_create_relation::call(
        rel_data.rd_id,
