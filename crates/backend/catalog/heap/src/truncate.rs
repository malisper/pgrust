...
pub fn heap_truncate_one_rel<'mcx>(mcx: Mcx<'mcx>, rel: &Relation<'mcx>) -> PgResult<()> {
    ...
    table_tableam_seams::table_relation_nontransactional_truncate::call(rel)?;

    /* If the relation has indexes, truncate the indexes too */
    index_seams::relation_truncate_indexes::call(mcx, rel)?;

    /* If there is a toast table, truncate that too */
    let toastrelid = rel.rd_rel.reltoastrelid;
    if OidIsValid(toastrelid) {
        let toastrel =
            table_seams::table_open::call(mcx, toastrelid, AccessExclusiveLock)?;
        table_tableam_seams::table_relation_nontransactional_truncate::call(
            &toastrel,
        )?;
        index_seams::relation_truncate_indexes::call(mcx, &toastrel)?;
        /* keep the lock... */
...
pub fn heap_truncate<'mcx>(mcx: Mcx<'mcx>, relids: &[Oid]) -> PgResult<()> {
    ...
    for &rid in relids {
        let rel = table_seams::table_open::call(mcx, rid, AccessExclusiveLock)?;
        relations.push(rel);
    }
