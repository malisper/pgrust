...
use matview_deps_seams as matview;
use guc_seams as guc;
use backend_progress_seams as progress;
use table_seams as table_am;
use indexing_seams as indexing;
use nodes_core_seams as nodes_seam;
use lsyscache_seams as lsyscache;
use relcache_nodexform_seams as nodexform;
...
///
/// The result element type is the word-model `datum::Datum` (the seam's
...
fn UpdateIndexRelation<'mcx>(
    ...
    let indpred: Option<alloc::string::String> = match &index_info.ii_Predicate {
        Some(pred) if !pred.is_empty() => {
            ...
            let anded = nodes_core::makefuncs::make_ands_explicit(clauses);
...
use pg_depend_seams as depend;
use partition_seams as partition;
use pg_constraint_seams as pg_constraint;
...
use tablespace_globals_seams as tablespace;
...
pub fn index_create<'mcx>(
    ...
    if OidIsValid(parent_index_relid) {
        ...
        lmgr_seams::lock_relation_oid::call(
            parent_index_relid,
...
pub fn validate_index<'mcx>(
    ...
    vacuum_seams::bulk_delete_callback::unregister(handle);
    ...
    let mut counters = table_tableam_seams::ValidateScanCounters::default();
    ...
    let sort_pull = RefCell::new(tuplesort);
    {
        ...
        table_tableam_seams::table_index_validate_scan::call(
            mcx,
            &heap_relation,
            &index_relation,
            &mut index_info,
...
pub fn index_concurrently_swap<'mcx>(
     ...
     */
    comment_seams::move_relation_comment::call(mcx, old_index_id, new_index_id)?;
...
fn ConstructTupleDescriptor<'mcx>(
    ...
    for i in 0..numatts {
        ...
        *index_tup_desc.attr_mut(i) = to;
        tupdesc::populate_compact_attribute(&mut index_tup_desc, i)?;
    }
