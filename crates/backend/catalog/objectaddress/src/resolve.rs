...
pub fn get_object_address<'mcx>(
    ...
    loop {
        ...
        let inval_count = sinval_seams::shared_invalid_message_counter::call();
        ...
        match objtype {
            ...
            OBJECT_DOMCONSTRAINT => {
                ...
                address.objectId =
                    pg_constraint_seams::get_domain_constraint_oid::call(
                        mcx,
                        domaddr.objectId,
                        constrname,
                        missing_ok,
            ...
            OBJECT_AGGREGATE | OBJECT_FUNCTION | OBJECT_PROCEDURE | OBJECT_ROUTINE => {
                ...
                address.objectId =
                    parse_func_seams::lookup_func_with_args_for_objtype::call(
            ...
            OBJECT_OPERATOR => {
                ...
                address.objectId =
                    parse_oper_seams::lookup_oper_with_args_node::call(
                        owa, missing_ok,
                    )?;
                address.objectSubId = 0;
            }
            OBJECT_COLLATION => {
                ...
                address.objectId =
                    namespace_seams::get_collation_oid::call(mcx, &strs, missing_ok)?;
                address.objectSubId = 0;
            }
            OBJECT_CONVERSION => {
                let names = cast_list(object)?;
                ...
                let strs = name_list_strs(names);
                address.objectId = namespace_seams::get_conversion_oid::call(
                    mcx, &strs, missing_ok,
                )?;
                address.objectSubId = 0;
            }
            ...
            OBJECT_LARGEOBJECT => {
                ...
                address.objectId = oid_seams::oidparse::call(object)?;
                address.objectSubId = 0;
                if !pg_largeobject_seams::large_object_exists::call(
                    address.objectId,
                )? && !missing_ok
                {
            ...
            OBJECT_CAST => {
                ...
                let sourcetypeid = parse_type_seams::lookup_type_name_oid::call(
                    sourcetype, missing_ok,
                )?;
                let targettypeid = parse_type_seams::lookup_type_name_oid::call(
                    targettype, missing_ok,
                )?;
                address.classId = CastRelationId;
                address.objectId = lsyscache_seams::get_cast_oid::call(
                    sourcetypeid,
                    targettypeid,
                    missing_ok,
                )?;
            ...
            OBJECT_TRANSFORM => {
                ...
                let type_id = parse_type_seams::lookup_type_name_oid::call(
                    typename, missing_ok,
                )?;
                let lang_id = proclang_seams::get_language_oid::call(
                    langname,
                    missing_ok,
                )?;
                address.classId = TransformRelationId;
                address.objectId = functioncmds_seams::get_transform_oid::call(
            ...
            OBJECT_TSPARSER => {
                ...
                let strs = name_list_strs(names);
                address.objectId =
                    namespace_seams::get_ts_parser_oid::call(mcx, &strs, missing_ok)?;
                address.objectSubId = 0;
            }
            OBJECT_TSDICTIONARY => {
                let names = cast_list(object)?;
                ...
                let strs = name_list_strs(names);
                address.objectId =
                    namespace_seams::get_ts_dict_oid::call(mcx, &strs, missing_ok)?;
                address.objectSubId = 0;
            }
            OBJECT_TSTEMPLATE => {
                let names = cast_list(object)?;
                ...
                let strs = name_list_strs(names);
                address.objectId = namespace_seams::get_ts_template_oid::call(
                    mcx, &strs, missing_ok,
                )?;
                address.objectSubId = 0;
            }
            OBJECT_TSCONFIGURATION => {
                ...
                let strs = name_list_strs(names);
                address.objectId =
                    namespace_seams::get_ts_config_oid::call(&strs, missing_ok)?;
                address.objectSubId = 0;
            }
            OBJECT_USER_MAPPING => {
            ...
            OBJECT_STATISTIC_EXT => {
                ...
                let strs = name_list_strs(names);
                address.objectId =
                    namespace_seams::get_statistics_object_oid::call(
                        mcx, &strs, missing_ok,
                    )?;
                address.objectSubId = 0;
            }
        ...
        if have_old && oid_is_valid(old_address.classId) {
            ...
            if old_address.classId != RelationRelationId {
                if catalog_seams::is_shared_relation::call(old_address.classId) {
        ...
        if address.classId != RelationRelationId {
            if catalog_seams::is_shared_relation::call(address.classId) {
...
pub fn get_object_address_unqualified(
    ...
    match objtype {
        OBJECT_ACCESS_METHOD => {
            ...
            address.objectId = amcmds_seams::get_am_oid::call(name, missing_ok)?;
        }
        OBJECT_DATABASE => {
            address.classId = DatabaseRelationId;
            address.objectId = user_seams::get_database_oid::call(
                name.to_string(),
                missing_ok,
            )?;
        }
        OBJECT_EXTENSION => {
            ...
            address.objectId =
                extension_seams::get_extension_oid::call(name, missing_ok)?;
        }
        OBJECT_TABLESPACE => {
            address.classId = TableSpaceRelationId;
            address.objectId =
                tablespace_seams::get_tablespace_oid::call(name, missing_ok)?;
        }
        OBJECT_ROLE => {
            address.classId = AuthIdRelationId;
            address.objectId =
                user_seams::get_role_oid::call(name.to_string(), missing_ok)?;
        }
        OBJECT_SCHEMA => {
            address.classId = NamespaceRelationId;
            address.objectId =
                namespace_seams::get_namespace_oid::call(name, missing_ok)?;
        }
        OBJECT_LANGUAGE => {
            address.classId = LanguageRelationId;
            address.objectId = proclang_seams::get_language_oid::call(
                name,
                missing_ok,
            )?;
        }
        OBJECT_FDW => {
            ...
            address.objectId =
                foreign_seams::get_foreign_data_wrapper_oid::call(name, missing_ok)?;
        }
        OBJECT_FOREIGN_SERVER => {
            address.classId = ForeignServerRelationId;
            address.objectId =
                foreign_seams::get_foreign_server_oid::call(name, missing_ok)?;
        }
        OBJECT_EVENT_TRIGGER => {
            address.classId = EventTriggerRelationId;
            address.objectId =
                event_trigger_seams::get_event_trigger_oid::call(name, missing_ok)?;
        }
        OBJECT_PARAMETER_ACL => {
            address.classId = ParameterAclRelationId;
            address.objectId =
                pg_parameter_acl_seams::parameter_acl_lookup::call(name, missing_ok)?;
        }
        OBJECT_PUBLICATION => {
            address.classId = PublicationRelationId;
            address.objectId =
                lsyscache_seams::get_publication_oid::call(name, missing_ok)?;
        }
        OBJECT_SUBSCRIPTION => {
            address.classId = SubscriptionRelationId;
            address.objectId =
                lsyscache_seams::get_subscription_oid::call(name, missing_ok)?;
        }
        _ => {
            return Err(PgError::error(format!(
                "unrecognized object type: {}",
...
pub fn get_relation_by_qualified_name<'mcx>(
    ...
    let names = cast_list(object)?;
    let strs = name_list_strs(names);
    let rv = namespace_seams::make_range_var_from_name_list::call(&strs)?;
    let Some(relation) = common_relation_seams::relation_openrv_extended::call(
...
pub fn get_object_address_relobject<'mcx>(
    ...
    let rv = namespace_seams::make_range_var_from_name_list::call(&relname)?;
    let relation = common_relation_seams::relation_openrv_extended::call(
        mcx,
        &rv,
        AccessShareLock,
        missing_ok,
    ...
    match objtype {
        OBJECT_RULE => {
            ...
            address.objectId = if relation.is_some() {
                rewritesupport_seams::get_rewrite_oid::call(
        ...
        OBJECT_TRIGGER => {
            ...
            address.objectId = if relation.is_some() {
                trigger_seams::get_trigger_oid::call(reloid, depname, missing_ok)?
            } else {
                INVALID_OID
            };
        }
        OBJECT_TABCONSTRAINT => {
            address.classId = ConstraintRelationId;
            address.objectId = if relation.is_some() {
                pg_constraint_seams::get_relation_constraint_oid::call(
        ...
        OBJECT_POLICY => {
            ...
            address.objectId = if relation.is_some() {
                policy_seams::get_relation_policy_oid::call(
                    reloid, depname, missing_ok,
                )?
            } else {
                INVALID_OID
...
pub fn get_object_address_attribute<'mcx>(
    ...
    let rv = namespace_seams::make_range_var_from_name_list::call(&relname)?;
    let relation = common_relation_seams::relation_openrv::call(mcx, &rv, lockmode)?;
    let reloid = relation.rd_id;

    let attnum = lsyscache_seams::get_attnum::call(reloid, attname)?;
    if attnum == 0 {
        if !missing_ok {
            return Err(PgError::new(
                ERROR,
...
pub fn get_object_address_attrdef<'mcx>(
    ...
    if attnum != 0 {
        defoid = heap_seams::get_attr_default_oid::call(reloid, attnum)?;
    }
...
pub fn get_object_address_type(
    ...
    let typeoid = parse_type_seams::lookup_type_name_oid::call(tn, missing_ok)?;
    ...
    if objtype == OBJECT_DOMAIN {
        ...
        let typtype = lsyscache_seams::get_typtype::call(typeoid)?;
...
pub fn get_object_address_opcf(
    ...
    let amoid = amcmds_seams::get_index_am_oid::call(amname, false)?;
    ...
    match objtype {
        OBJECT_OPCLASS => {
            ...
            address.objectId = opclasscmds_seams::get_opclass_oid::call(
                mcx, amoid, &tail, missing_ok,
            )?;
        }
        OBJECT_OPFAMILY => {
            address.classId = OperatorFamilyRelationId;
            address.objectId = opclasscmds_seams::get_opfamily_oid::call(
                mcx, amoid, &tail, missing_ok,
            )?;
        }
        _ => {
...
pub fn get_object_address_usermapping(
    ...
    let userid = if username == "public" {
        ...
        let oid = user_seams::get_role_oid::call(username.to_string(), true)?;
        if !oid_is_valid(oid) {
            if !missing_ok {
    ...
    let server =
        foreign_seams::get_foreign_server_by_name::call(mcx, servername, true)?;
    ...
    let umoid = foreign_seams::usermapping_oid::call(userid, server.serverid)?;
...
pub fn get_object_address_publication_rel<'mcx>(
    ...
    let pub_oid =
        lsyscache_seams::get_publication_oid::call(pubname, missing_ok)?;
...
pub fn get_object_address_publication_schema(
    ...
    let schemaid =
        namespace_seams::get_namespace_oid::call(schemaname, missing_ok)?;
...
pub fn get_object_address_defacl(
    ...
    let userid = user_seams::get_role_oid::call(username.to_string(), true)?;
    ...
    let schemaid = if let Some(schema) = schema {
        let sid = namespace_seams::get_namespace_oid::call(schema, true)?;
        if !oid_is_valid(sid) {
    ...
    match objtype {
        ...
        OBJECT_TYPE | OBJECT_DOMAIN | OBJECT_ATTRIBUTE => {
            if !aclchk_seams::object_ownercheck::call(
                ...
                return aclchk_seams::aclcheck_error_type::call(
                    ACLCHECK_NOT_OWNER,
                    address.objectId,
                );
            }
        ...
        OBJECT_DOMCONSTRAINT => {
            let contypid = match pg_constraint_seams::constraint_type_oids::call(
                address.objectId,
            )? {
        ...
        OBJECT_CAST => {
            ...
            let targettype = list[1]
                ...
                .ok_or_else(|| PgError::error("lsecond_node(TypeName)"))?;
            let sourcetypeid =
                parse_type_seams::typename_type_id_node::call(sourcetype)?;
            let targettypeid =
                parse_type_seams::typename_type_id_node::call(targettype)?;
            if !aclchk_seams::object_ownercheck::call(
                TypeRelationId,
                sourcetypeid,
                roleid,
                ...
                let src =
                    format_type_seams::format_type_be_str::call(sourcetypeid)?;
                let tgt =
                    format_type_seams::format_type_be_str::call(targettypeid)?;
                return Err(PgError::new(
                    ERROR,
        ...
        OBJECT_TRANSFORM => {
            ...
            let typename = list[0]
                .as_typename()
                .ok_or_else(|| PgError::error("linitial_node(TypeName)"))?;
            let typeid = parse_type_seams::typename_type_id_node::call(typename)?;
            if !aclchk_seams::object_ownercheck::call(
                TypeRelationId,
                typeid,
                roleid,
        ...
        OBJECT_ROLE => {
            ...
            if user_seams::superuser_arg::call(address.objectId)? {
                if !user_seams::superuser_arg::call(roleid)? {
                    return Err(PgError::new(ERROR, "permission denied".to_string())
                        .with_sqlstate(ERRCODE_INSUFFICIENT_PRIVILEGE)
                        .with_detail("The current user must have the SUPERUSER attribute."));
                }
            } else {
                if !user_seams::has_createrole_privilege::call(roleid)? {
                    return Err(PgError::new(ERROR, "permission denied".to_string())
                        .with_sqlstate(ERRCODE_INSUFFICIENT_PRIVILEGE)
...
pub fn object_ownercheck(classid: Oid, objectid: Oid, roleid: Oid) -> PgResult<bool> {
    aclchk_seams::object_ownercheck::call(classid, objectid, roleid)
}
...
fn cast_attr_oid<'mcx>(
    ...
    let datum: TupleDatum<'mcx> = heaptuple::nocachegetattr(
        mcx,
        &tup.tuple,
        attnum as i32,
        &cast_desc.rd_att,
...
pub fn get_catalog_object_by_oid_extended<'mcx>(
    ...
    indexing_seams::get_catalog_object_by_oid::call(
