...
pub fn init_seams() {
    pg_inherits_seams::find_all_inheritors::set(|mcx, parent_rel_id, lockmode| {
    ...
    pg_inherits_seams::store_single_inheritance::set(StoreSingleInheritance);
    // index_drop (catalog/index.c) partition-index parent-link cleanup.
    pg_inherits_seams::delete_inherits_tuple::set(DeleteInheritsTuple);
    // IndexSetParentIndex (indexcmds.c) + DefineIndex partitioned recursion.
    pg_inherits_seams::has_superclass::set(has_superclass);
}
