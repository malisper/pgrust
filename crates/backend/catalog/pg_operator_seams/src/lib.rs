...
seam_core::seam!(
    /// `OperatorLookup(operatorName, leftTypeId, rightTypeId, &defined)`
    /// (pg_operator.c): resolve a binary operator by qualified name + arg
    /// types, returning `(oid, defined)` where `defined` distinguishes a real
    /// operator from a shell (`InvalidOid` when no row exists at all). Can
    /// `ereport(ERROR)`, carried on `Err`.
    pub fn operator_lookup(
