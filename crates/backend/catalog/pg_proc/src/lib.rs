...
//!     [`pg_proc_seams`]; each loud-panics until its owner
...
pub fn init_seams() {
    functioncmds_seams::procedure_create::set(procedure_create_from_args);
    functioncmds_seams::alter_function_begin::set(alter_function_begin);
    ...
    comp_seams::function_parse_error_transpose::set(|prosrc, err| {
...
fn procedure_create_from_args(
    args: functioncmds_seams::ProcedureCreateArgs,
) -> PgResult<ObjectAddress> {
    let functioncmds_seams::ProcedureCreateArgs {
        procedure_name,
        namespace_id,
        replace,
        returns_set,
    ...
    let functioncmds_seams::CookedParameterDefaults {
...
fn alter_function_begin(
    ...
    let func_oid =
        functioncmds_seams::lookup_func_with_args::call(objtype, func, false)?;
    ...
    if !aclchk_seams::object_ownercheck::call(ProcedureRelationId, func_oid, user_id)? {
        let objtype_e = nodes::parsenodes::ObjectType::from_i32(objtype).ok_or_else(|| {
            elog_error(format!("AlterFunction: invalid object type {objtype}"))
        })?;
        aclchk_seams::aclcheck_error::call(
            ACLCHECK_NOT_OWNER,
            objtype_e,
            Some(functioncmds_seams::name_list_to_string::call(objname.clone())?),
        )?;
    }

    if info.form.prokind == PROKIND_AGGREGATE {
        let name = functioncmds_seams::name_list_to_string::call(objname)?;
        return Err(ereport(ERROR)
            .errcode(ERRCODE_WRONG_OBJECT_TYPE)
    ...
    Ok(functioncmds_seams::AlterFunctionTarget {
        func_oid,
