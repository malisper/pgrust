 ...
 * node vocabulary (`parsenodes::Node`). pg_proc.c's `nodeToString` of
 ...
 * These four seams carry the `parsenodes::Node` the consumer produces; the
 ...
 * `parsenodes::Node` through pg-proc: `interpret_sql_body` (in the
