...
fn make_array_object_name(typeName: &str, pass: Option<i32>) -> PgResult<String> {
    ...
    indexcmds_seams::make_object_name::call("", typeName, &label)
}
...
pub fn makeMultirangeTypeName(rangeTypeName: &str, typeNamespace: Oid) -> PgResult<String> {
    ...
    let clip = mbutils_seams::pg_mbcliplen::call(buf.as_bytes(), buflen, NAMEDATALEN - 1);
...
/// Install this unit's inward seams ([`pg_type_seams`]) and the
...
pub fn init_seams() {
     ...
     * the C function). */
    functioncmds_seams::type_shell_make::set(|typname, nsp, owner| {
