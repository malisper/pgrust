...
pub fn TypeCreate<'mcx>(mcx: Mcx<'mcx>, p: &TypeCreateParams<'_>) -> PgResult<ObjectAddress> {
    ...
    // Post creation hook for new type (pg_type.c:513).
    objectaccess::InvokeObjectPostCreateHook(TYPE_RELATION_ID, typeObjectId, 0)?;
...
pub fn TypeShellMake<'mcx>(
    ...
    // Post creation hook for new shell type (pg_type.c:171).
    objectaccess::InvokeObjectPostCreateHook(TYPE_RELATION_ID, typoid, 0)?;
...
// A derived (array / multirange) name is clipped by pg_mbcliplen under the
// database encoding; under SQL_ASCII (byte semantics) or C's own
// NAMEDATALEN-12 strlcpy in makeMultirangeTypeName the clip can land inside
// a multibyte character.  C stores those bytes as-is; pgrust carries
// identifiers as UTF-8 only (docs/design/carve-ratifications.md §11), so the
// unrepresentable name is a typed refusal instead of a panic.
fn derived_name_utf8<'n>(
    name: &'n [u8],
    source: &str,
    hint: &'static str,
) -> PgResult<&'n str> {
    core::str::from_utf8(name).map_err(|_| {
        Box::new(
            PgError::new(
                ERROR,
                format!(
                    "type name \"{}\" derived from \"{source}\" is not valid UTF-8",
                    String::from_utf8_lossy(name)
                ),
            )
            .with_sqlstate(types_error::ERRCODE_FEATURE_NOT_SUPPORTED)
            .with_detail(
                "pgrust stores only UTF-8 identifiers (UTF-8-only carve, \
                 docs/design/carve-ratifications.md); PostgreSQL would truncate the \
                 derived name inside a multibyte character.",
            )
            .with_hint(hint),
        )
    })
...
const ARRAY_NAME_HINT: &str =
    "Use a shorter name so that the array type name ends on a character boundary.";
const MULTIRANGE_NAME_HINT: &str = "You can manually specify a multirange type name using the \
...
pub fn makeArrayTypeName(typeName: &str, typeNamespace: Oid) -> PgResult<NameData> {
    ...
    loop {
        let candidate = derived_name_utf8(arr_name.name_str(), typeName, ARRAY_NAME_HINT)?;
...
pub fn makeMultirangeTypeName(rangeTypeName: &str, typeNamespace: Oid) -> PgResult<NameData> {
    ...
    let candidate = derived_name_utf8(&buf[..clipped], rangeTypeName, MULTIRANGE_NAME_HINT)?;
    if syscache_seams::lookup_pg_type_oid_by_name::call(candidate, typeNamespace)? != InvalidOid {
...
pub fn RenameTypeInternal<'mcx>(
    ...
    // C order (pg_type.c:779-819): open pg_type, verify typeOid (elog
    // "cache lookup failed"), then check the new name for a conflict.
    ...
    // C: namestrcpy(&typ->typname, newTypeName) on the syscache copy --
    // NAMEDATALEN-1 truncation, never a failure.
    let mut name = NameData::default();
    name.namestrcpy(newTypeName);
    ...
    values[Anum_pg_type_typname - 1] = Datum::from_usize(name.data.as_ptr() as usize);
    replace[Anum_pg_type_typname - 1] = true;
    let mut newtup = heaptuple::heap_modify_tuple(mcx, tup, desc, &values, &nulls, &replace)?;
    let otid = tup.t_self;
    genam::systable_endscan(mcx, scan)?;

    // Check for a conflicting type name.  C dodges a conflicting
    // autogenerated array type by renaming it out of the way; shell types
    ...
    objectaccess::InvokeObjectPostAlterHook(TYPE_RELATION_ID, typeOid, 0)?;
    ...
    if arrayOid != InvalidOid && arrayOid != oldTypeOid {
        ...
        let arr_str = derived_name_utf8(arr.name_str(), newTypeName, ARRAY_NAME_HINT)?;
...
pub fn moveArrayTypeName<'mcx>(
    ...
    let newname_str = derived_name_utf8(newname.name_str(), typeName, ARRAY_NAME_HINT)?;
