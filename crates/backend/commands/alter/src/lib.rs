...
//! `indexing_seams::update_object_owner_tuple` (the same shape
...
fn role_spec_oid(mcx: Mcx<'_>, node: &Node) -> PgResult<Oid> {
    ...
    adt_acl::role_membership::get_rolespec_oid(&view, false)
}
...
fn namespace_name(mcx: Mcx<'_>, nsp_oid: Oid) -> PgResult<String> {
    Ok(
        lsyscache::namespace_range_index_pubsub::get_namespace_name(mcx, nsp_oid)?
            .map(|s| s.as_str().to_string())
            .unwrap_or_default(),
    )
}
...
fn invoke_post_alter_hook(class_id: Oid, object_id: Oid) -> PgResult<()> {
    objectaccess::invoke_object_post_alter_hook(class_id, object_id, 0, InvalidOid, false)
}
...
fn name_nsp_exists(
    ...
    cache_syscache::SearchSysCacheExists(
        ...
        SysCacheKey::Value(datum::Datum::from_oid(nsp_oid)),
        SysCacheKey::UNUSED,
        SysCacheKey::UNUSED,
    )
}
...
fn check_duplicate_name(
    ...
    if class_id == ProcedureRelationId {
        ...
        collationcmds::IsThereCollationInNamespace(new_name, collnamespace)?;
        ...
        opclasscmds::IsThereOpClassInNamespace(
        ...
        opclasscmds::IsThereOpFamilyInNamespace(
...
fn AlterObjectRename_internal(
    ...
    if !more::superuser::superuser()? {
        ...
        if !adt_acl::role_membership::has_privs_of_role(
            miscinit::GetUserId(),
            ownerId,
        )? {
            aclcheck_error(
        ...
        if class_id == SubscriptionRelationId {
            ...
            if aclresult != ACLCHECK_OK {
                let dbname =
                    dbcommands_seams::get_database_name::call(mcx, my_database_id)?;
            ...
            .as_bool();
            if !subpasswordrequired && !more::superuser::superuser()? {
                return Err(ereport(ERROR)
                    .errcode(ERRCODE_INSUFFICIENT_PRIVILEGE)
    ...
    if class_id == SubscriptionRelationId {
        worker_seams::LogicalRepWorkersWakeupAtCommit::call(objectId)?;
    }
    ...
    if class_id == PublicationRelationId {
        ...
        .as_bool();
        publicationcmds_seams::InvalidatePubRelSyncCache::call(puboid, puballtables)?;
    }
...
pub fn ExecRenameStmt(mcx: Mcx<'_>, stmt: &RenameStmt) -> PgResult<ObjectAddress> {
    match stmt.renameType {
        OBJECT_TABCONSTRAINT | OBJECT_DOMCONSTRAINT => {
            tablecmds_seams::RenameConstraint::call(mcx, stmt)
        }

        OBJECT_DATABASE => dbcommands_seams::RenameDatabase::call(
            stmt.subname.as_deref().unwrap_or(""),
            stmt.newname.as_deref().unwrap_or(""),
        ),

        OBJECT_ROLE => user::RenameRole(
            mcx,
            stmt.subname.as_deref().unwrap_or(""),
            stmt.newname.as_deref().unwrap_or(""),
        ),

        OBJECT_SCHEMA => schemacmds::RenameSchema(
            mcx,
            stmt.subname.as_deref().unwrap_or(""),
            stmt.newname.as_deref().unwrap_or(""),
        ),

        OBJECT_TABLESPACE => tablespace::RenameTableSpace(
            mcx,
            stmt.subname.as_deref().unwrap_or(""),
            stmt.newname.as_deref().unwrap_or(""),
        ),
        ...
        | OBJECT_FOREIGN_TABLE => tablecmds_seams::RenameRelation::call(mcx, stmt),

        OBJECT_COLUMN | OBJECT_ATTRIBUTE => {
            tablecmds_seams::renameatt::call(mcx, stmt)
        }
        ...
        OBJECT_RULE => {
            ...
            rewriteDefine::RenameRewriteRule(
                mcx,
                relation,
                stmt.subname.as_deref().unwrap_or(""),
                stmt.newname.as_deref().unwrap_or(""),
        ...
        OBJECT_TRIGGER => trigger_seams::renametrig::call(mcx, stmt),

        OBJECT_POLICY => policy_seams::rename_policy::call(mcx, stmt),

        OBJECT_DOMAIN | OBJECT_TYPE => typecmds_seams::RenameType::call(mcx, stmt),
...
pub fn ExecAlterObjectDependsStmt(
    ...
    objectaddress::resolve::check_object_ownership(
        miscinit::GetUserId(),
        stmt.objectType,
        address,
        object,
    ...
    if stmt.remove {
        ...
        if !currexts.iter().any(|&o| o == refAddr.objectId) {
            pg_depend::recordDependencyOn(mcx, &address, &refAddr, DEPENDENCY_AUTO_EXTENSION)?;
        }
    }
...
pub fn ExecAlterObjectSchemaStmt(
    ...
    match stmt.objectType {
        OBJECT_EXTENSION => {
            ...
            let (addr, old) = extension_seams::AlterExtensionNamespace::call(
        ...
        OBJECT_FOREIGN_TABLE | OBJECT_SEQUENCE | OBJECT_TABLE | OBJECT_VIEW | OBJECT_MATVIEW => {
            let (addr, old) =
                tablecmds_seams::AlterTableNamespace::call(mcx, stmt, want_old)?;
            address = addr;
            oldNspOid = old;
        }
        ...
        OBJECT_DOMAIN | OBJECT_TYPE => {
            ...
            let (addr, old) = typecmds_seams::AlterTypeNamespace::call(
                mcx,
                names,
                newschema,
                stmt.objectType,
        ...
        | OBJECT_TSTEMPLATE => {
            ...
            let nspOid = catalog_namespace::LookupCreationNamespace(mcx, newschema)?;
            oldNspOid =
...
pub fn AlterObjectNamespace_oid(
    ...
    if classId == RelationRelationId {
        let rel = common_relation::relation_open(mcx, objid, AccessExclusiveLock)?;
        oldNspOid = rel.rd_rel.relnamespace;
        tablecmds_seams::AlterTableNamespaceInternal::call(
            mcx, &rel, oldNspOid, nspOid, objsMoved,
        )?;
        table_close(rel, NoLock)?;
    } else if classId == TypeRelationId {
        oldNspOid =
            typecmds_seams::AlterTypeNamespace_oid::call(objid, nspOid, true, objsMoved)?;
...
fn AlterObjectNamespace_internal(
    ...
    catalog_namespace::CheckSetNamespace(mcx, oldNspOid, nspOid)?;
    ...
    if class_id == ProcedureRelationId {
        ...
        collationcmds::IsThereCollationInNamespace(&collname, nspOid)?;
    } else if class_id == OperatorClassRelationId {
        ...
        opclasscmds::IsThereOpClassInNamespace(mcx, &opcname, opcmethod, nspOid)?;
    } else if class_id == OperatorFamilyRelationId {
        ...
        opclasscmds::IsThereOpFamilyInNamespace(mcx, &opfname, opfmethod, nspOid)?;
...
pub fn ExecAlterOwnerStmt(mcx: Mcx<'_>, stmt: &AlterOwnerStmt) -> PgResult<ObjectAddress> {
    ...
    match stmt.objectType {
        OBJECT_DATABASE => {
            dbcommands_seams::AlterDatabaseOwner::call(owner_obj_str(stmt)?, newowner)
        }

        OBJECT_SCHEMA => schemacmds::AlterSchemaOwner(mcx, owner_obj_str(stmt)?, newowner),

        OBJECT_TYPE | OBJECT_DOMAIN => {
            let names = stmt.object.as_deref().ok_or_else(|| {
                PgError::error("ExecAlterOwnerStmt: OWNER TO object must be set")
            })?;
            typecmds_seams::AlterTypeOwner::call(mcx, names, newowner, stmt.objectType)
        }

        OBJECT_FDW => commands_foreigncmds::AlterForeignDataWrapperOwner(
            mcx,
            owner_obj_str(stmt)?,
            newowner,
        ),
        ...
        OBJECT_FOREIGN_SERVER => {
            commands_foreigncmds::AlterForeignServerOwner(mcx, owner_obj_str(stmt)?, newowner)
        }

        OBJECT_EVENT_TRIGGER => {
            event_trigger_seams::AlterEventTriggerOwner::call(owner_obj_str(stmt)?, newowner)
        }

        OBJECT_PUBLICATION => {
            publicationcmds_seams::AlterPublicationOwner::call(owner_obj_str(stmt)?, newowner)
        }

        OBJECT_SUBSCRIPTION => {
            subscriptioncmds_seams::AlterSubscriptionOwner::call(owner_obj_str(stmt)?, newowner)
        }
...
pub fn AlterObjectOwner_internal(
    ...
    if old_ownerId != new_ownerId {
        ...
        indexing_seams::update_object_owner_tuple::call(
...
// ---------------------------------------------------------------------------
// Live (`nodes`) → owned (`parsenodes`) converters for the
...
fn lower_alter_object(
    ...
    match object.as_ref() {
        Some(n) => Ok(Some(Box::new(
            parse_type::rich_node_to_parse(n)?,
        ))),
        None => Ok(None),
    }
}
...
/// ([`utility_out_seams`]): the RENAME dispatch arms in
...
pub fn init_seams() {
    ...
    utility_out_seams::exec_rename_stmt::set(exec_rename_stmt_arm);
    utility_out_seams::exec_rename_stmt_slow::set(exec_rename_stmt_slow_arm);

    // ALTER … SET SCHEMA / [NO] DEPENDS ON EXTENSION / OWNER TO — the remaining
    // ProcessUtilitySlow legs whose owner (alter.c) lives here.
    utility_out_seams::exec_alter_object_schema_stmt_slow::set(
        exec_alter_object_schema_stmt_slow_arm,
    );
    utility_out_seams::exec_alter_object_depends_stmt_slow::set(
        exec_alter_object_depends_stmt_slow_arm,
    );
    utility_out_seams::exec_alter_owner_stmt::set(exec_alter_owner_stmt_arm);
    utility_out_seams::exec_alter_owner_stmt_slow::set(
