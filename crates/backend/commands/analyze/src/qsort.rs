//! lib/sort_template.h (pg_qsort) over Copy elements (gistproc's copy; a
//! shared crate is a follow-up). compute_scalar_stats must use C's qsort,
//! not std's sort: user btree comparators are allowed to be intransitive
//! (contrib cube's cube_cmp over mixed-dimension values is), and std's
//! driver PANICS on such comparators where C's qsort just produces an
//! arbitrary-but-safe order. ANALYZE over any cube column would otherwise
//! die with "user-provided comparison function does not correctly
