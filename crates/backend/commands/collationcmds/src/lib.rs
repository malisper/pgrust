...
//! collversion update) cross [`collationcmds_seams`], each of
...
fn defel_arg<'mcx>(mcx: Mcx<'mcx>, defel: &DefElem) -> PgResult<Option<DefElemArg>> {
    ...
    Ok(Some(match node {
        ...
        Node::TypeName(t) => DefElemArg::TypeName(
            parse_type_seams::typename_to_string_node::call(mcx, t)?
                .as_str()
                .to_string(),
        ),
...
pub fn DefineCollation<'mcx>(
    ...
    if let Some(fromEl) = fromEl {
        ...
        let from_name = define::defGetQualifiedName(fromEl)?;
...
pub fn init_seams() {
    ...
    // VERSION).
    utility_out_seams::alter_collation::set(alter_collation_arm);
