...
seam_core::seam!(
    ...
    /// `defel->location` here for the "attribute not recognized" and
    /// "conflicting or redundant options" syntax errors. `source` is the active
    /// query string (`pstate->p_sourcetext`); the full C body is
