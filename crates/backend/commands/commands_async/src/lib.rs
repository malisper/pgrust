...
use ipc_shmem_seams as ipc_shmem_seams;
use latch_seams as latch_seams;
use procsignal_seams as procsignal_seams;
use lmgr_seams as lmgr_seams;
use timestamp_seams as timestamp_seams;
use init_small_seams as init_seams_decls;
use ps_status_seams as ps_seams;
...
pub fn pg_notify_core(channel: &str, payload: &str) -> PgResult<()> {
    ...
    utility_seams::prevent_command_during_recovery::call("NOTIFY")?;
...
pub fn NotifyMyFrontEnd(channel: &str, payload: &str, srcPid: i32) -> PgResult<()> {
    if postgres_seams::where_to_send_output::call()
        == types_dest::dest::CommandDest::Remote
    {
        ...
        let mut buf = pqformat::pq_beginmessage(ctx.mcx(), PqMsg_NotificationResponse)?;
        pqformat::pq_sendint32(&mut buf, srcPid as u32)?;
        pqformat::pq_sendstring(&mut buf, channel.as_bytes())?;
        pqformat::pq_sendstring(&mut buf, payload.as_bytes())?;
...
fn notification_hash(key: &Notification, keysize: Size) -> u32 {
    ...
    hashfn::hash_bytes(&key.data[..len])
}
...
pub fn init_seams() {
    ...
    async_seams::async_shmem_size::set(AsyncShmemSize);
    async_seams::async_shmem_init::set(AsyncShmemInit);
    async_seams::async_unlisten_all::set(Async_UnlistenAll);
    ...
    utility_out_seams::async_notify::set(async_notify_arm);
    utility_out_seams::async_listen::set(Async_Listen);
    utility_out_seams::async_unlisten::set(Async_Unlisten);
    utility_out_seams::async_unlisten_all::set(Async_UnlistenAll);
    ...
    parallel_rt_seams::notify_my_front_end::set(NotifyMyFrontEnd);
}
