...
fn defGetCopyRejectLimitOption(defel: &DefElem) -> PgResult<i64> {
    let reject_limit = match defel.arg.as_deref() {
        ...
        Some(s) if s.node_tag() == ntag::T_String => numutils::pg_strtoint64(s.expect_string().sval.as_str())?,
...
pub fn ProcessCopyOptions<'mcx>(
    ...
    for node in options.unwrap_or(&[]) {
        ...
        if dn == "format" {
            ...
            opts.file_encoding = extra_encnames_fgram::pg_char_to_encoding(&name) as i32;
            if opts.file_encoding < 0 {
                return Err(PgError::error(format!(
...
pub fn DoCopy<'mcx>(
    ...
    if !pipe {
        ...
        if stmt.is_program {
            if !adt_acl::role_membership::has_privs_of_role(userid, ROLE_PG_EXECUTE_SERVER_PROGRAM)? {
                return Err(PgError::error(
            ...
            if is_from && !adt_acl::role_membership::has_privs_of_role(userid, ROLE_PG_READ_SERVER_FILES)? {
            ...
            if !is_from && !adt_acl::role_membership::has_privs_of_role(userid, ROLE_PG_WRITE_SERVER_FILES)? {
    ...
    let mut rel: Option<rel::Relation<'mcx>> = None;
    ...
    if let Some(relation_node) = stmt.relation.as_deref() {
        ...
        let opened = table::table_openrv(mcx, &access_rv, lockmode)?;
        relid = opened.rd_id;
        ...
        for attno in attnums.iter().copied() {
            ...
            if is_from {
                ...
                pi.selectedCols = Some(nodes_core::bitmapset::bms_add_member(
                    mcx,
        ...
        execMain::exec_check_permissions(
            core::slice::from_ref(&perminfo_snapshot),
            true,
        )?;

        // Row-security check.
        let rls = more::check_enable_rls(mcx, relid, 0, false)?;
    ...
    if is_from {
        ...
        if transam_xact_seams::xact_read_only::call()
            && !relation_is_local_temp(rel)
        {
            transam_xact_seams::prevent_command_if_read_only::call("COPY FROM")?;
        }
        ...
        let mut cstate = copyto::BeginCopyTo(
            mcx,
            Some(pstate),
            rel.take(),
            raw_query,
        ...
        processed = copyto::DoCopyTo(&mut cstate)?;
        copyto::EndCopyTo(cstate)?;
    }
...
fn relation_is_local_temp(rel: &rel::Relation<'_>) -> bool {
    rel.rd_rel.relpersistence == RELPERSISTENCE_TEMP
        && rel.rd_backend == lmgr_proc_seams::my_proc_number::call()
}
...
fn analyze_copy_where<'mcx>(
    ...
    let transformed =
        parse_expr_seams::transformExpr::call(pstate, raw, EXPR_KIND_COPY_WHERE)?;
    ...
    let where_clause =
        coerce::coerce_to_boolean(mcx, Some(pstate), where_clause, "WHERE")?;
    ...
    parse_collate::assign_expr_collations(Some(pstate), &mut where_clause)?;
    ...
    let mut expr_attrs =
        var_seams::pull_varattnos::call(mcx, &where_clause, 1)?;
    let first_low = FirstLowInvalidHeapAttributeNumber as i32;
    let whole_row_member = 0 - first_low;
    if nodes_core::bitmapset::bms_is_member(whole_row_member, expr_attrs.as_deref()) {
        // expand to all real columns, then drop the whole-row marker
        expr_attrs = nodes_core::bitmapset::bms_add_range(
            mcx,
            expr_attrs,
            1 - first_low,
        ...
        expr_attrs =
            nodes_core::bitmapset::bms_del_member(expr_attrs, whole_row_member);
    }

    let mut i = -1;
    loop {
        i = nodes_core::bitmapset::bms_next_member(expr_attrs.as_deref(), i);
        if i < 0 {
            break;
        }
        ...
        if rel.rd_att.attrs[(attno - 1) as usize].attgenerated != 0 {
            let colname =
                lsyscache::attribute::get_attname(mcx, rel.rd_id, attno, false)?
    ...
    let where_clause = clauses::eval_const_expressions(mcx, where_clause)?;
...
fn copy_from_driver<'mcx>(
    ...
    let mut state = copyfrom::BeginCopyFrom(
        mcx,
        rel.alias(),
        opts,
        file_encoding,
    ...
    let processed = copyfrom::CopyFrom(mcx, &mut state)?;
    copyfrom::EndCopyFrom(state)?;
    Ok(processed)
}
...
pub fn init_seams() {
    commands_copy_seams::process_copy_options::set(seam_process_copy_options);
    commands_copy_seams::copy_get_attnums::set(seam_copy_get_attnums);
    utility_out_seams::do_copy::set(seam_do_copy);
}
