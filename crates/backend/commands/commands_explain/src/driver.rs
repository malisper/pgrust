...
pub fn ExplainOneUtility<'mcx>(
    ...
    match utility_stmt.node_tag() {
        ntag::T_CreateTableAsStmt => {
            ...
            if queryjumble_seams::is_query_id_enabled::call() {
                ctas_query.queryId =
                    queryjumble_seams::jumble_query_compute::call(&ctas_query);
            }
        ...
        ntag::T_DeclareCursorStmt => {
            ...
            if queryjumble_seams::is_query_id_enabled::call() {
                dcs_query.queryId =
                    queryjumble_seams::jumble_query_compute::call(&dcs_query);
            }
...
pub fn ExplainQuery<'mcx>(
    ...
    if queryjumble_seams::is_query_id_enabled::call() {
        query_copy.queryId =
            queryjumble_seams::jumble_query_compute::call(&query_copy);
    }

    let rewritten = rewritehandler_seams::query_rewrite_canonical::call(
        mcx, query_copy,
    )?;
    ...
    let mut tstate = execTuples::exectype_tupoutput::begin_tup_output_tupdesc(
        mcx,
        dest,
        tupdesc,
    ...
    if es.format == ExplainFormat::EXPLAIN_FORMAT_TEXT {
        execTuples::exectype_tupoutput::do_text_output_multiline(
            mcx,
            &mut tstate,
            &plan_text,
        )?;
    ...
    execTuples::exectype_tupoutput::end_tup_output(mcx, tstate)?;
...
fn do_text_output_oneline<'mcx>(
    ...
    let values = [datum];
    let isnull = [false];
    execTuples::exectype_tupoutput::do_tup_output(
