...
fn parse_option_list_costs_off_format_json() {
    ...
    define_seams::def_get_boolean::set(|_defname, arg| {
        Ok(match arg {
            None => true,
            Some(define_seams::DefElemArg::Boolean(b)) => b,
            Some(define_seams::DefElemArg::String(s)) => {
                matches!(s.as_str(), "on" | "true" | "1")
            }
            other => panic!("unexpected boolean arg: {other:?}"),
        })
    });
    define_seams::def_get_string::set(|mcx, _defname, arg| {
        let s = match arg {
