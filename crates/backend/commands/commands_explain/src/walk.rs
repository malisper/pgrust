...
pub fn ExplainPrintPlan<'es, 'p>(
    ...
    if es.verbose
        && pstmt.queryId != 0
        && guc_tables::vars::compute_query_id.read()
            != guc_tables::consts::COMPUTE_QUERY_ID_REGRESS
    {
...
fn show_tablesample<'es, 'p>(
    ...
    let method_name = lsyscache::function::get_func_name(mcx, tsc.tsmhandler)?
...
fn show_modifytable_info<'es, 'p>(
    ...
    if let Some(arbiters) = node.arbiterIndexes.as_ref() {
        for &idx_oid in arbiters.iter() {
            ...
            if let Some(name) = lsyscache::relation::get_rel_name(mcx, idx_oid)? {
...
fn explain_scan_target_switch<'es, 'p>(
    ...
    match plan_node.node_tag() {
        ...
        ntag::T_BitmapIndexScan => {
            ...
            if es.format == ExplainFormat::EXPLAIN_FORMAT_TEXT {
                let quoted =
                    ruleutils::quote_identifier(mcx, indexname.as_str())?;
...
fn show_window_def<'es, 'p>(
    ...
    let quoted = ruleutils::quote_identifier(mcx, winname)?;
...
fn show_windowagg_info(es: &mut ExplainState<'_>, planstate: &PlanStateNode<'_>) -> PgResult<()> {
    ...
    let (max_storage_type, max_space_used) =
        sort_storage_seams::tuplestore_get_stats::call(buffer);
...
fn show_material_info(es: &mut ExplainState<'_>, planstate: &PlanStateNode<'_>) -> PgResult<()> {
    ...
    let (max_storage_type, max_space_used) =
        sort_storage_seams::tuplestore_get_stats::call(tupstore);
    crate::details::show_storage_info(es, max_storage_type, max_space_used)?;
    Ok(())
}
...
fn show_sortorder_options<'p>(
    ...
    if collation != InvalidOid
        && collation != lsyscache::type_::get_typcollation(sortcoltype)?
    {
        let collname = lsyscache::collation_constraint_language_cast::get_collation_name(mcx, collation)?
            .unwrap_or_else(|| panic!("cache lookup failed for collation {collation}"));
        let quoted = ruleutils::quote_identifier(mcx, collname.as_str())?;
    ...
    if sort_operator == gt_opr {
        ...
        let opname = lsyscache::opfamily_operator::get_opname(mcx, sort_operator)?
        ...
        if let Some((_eq_op, rev)) =
            lsyscache::opfamily_operator::get_equality_op_for_ordering_op(sort_operator)?
        {
            reverse = rev;
        }
    }
