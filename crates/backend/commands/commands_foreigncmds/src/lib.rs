...
use foreign_seams as foreign_seams;
use postgres_seams as postgres_seams;
use utility_fc_seams as utility_seams;
use parse_func_seams as parse_func_seams;
use parse_type_seams as parse_type_seams;
...
/// Seam body for [`foreigncmds_seams::foreign_table_fdwvalidator`]:
...
/// Seam body for [`foreigncmds_seams::transform_generic_options`]:
...
/* =========================================================================
 * ProcessUtilitySlow dispatch adapters: rich `nodes` -> flat
 * `types_foreigncmds` converters + the dispatch arms.
 *
 * The parser produces the rich `nodes` statement nodes (option lists as
...
pub fn init_seams() {
    ...
    utility_out_seams::create_foreign_data_wrapper::set(create_fdw_seam);
    utility_out_seams::alter_foreign_data_wrapper::set(alter_fdw_seam);
    utility_out_seams::create_foreign_table::set(create_foreign_table_seam);
    utility_out_seams::create_foreign_server::set(create_foreign_server_seam);
    utility_out_seams::alter_foreign_server::set(alter_foreign_server_seam);
    utility_out_seams::create_user_mapping::set(create_user_mapping_seam);
    utility_out_seams::alter_user_mapping::set(alter_user_mapping_seam);
    utility_out_seams::remove_user_mapping::set(remove_user_mapping_seam);
    utility_out_seams::import_foreign_schema::set(import_foreign_schema_seam);
}
