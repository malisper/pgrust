...
pub fn RefreshMatViewByOid(
     ...
     */
    if concurrent {
        let indexoidlist = relcache::derived::RelationGetIndexList(matviewRel.rd_id)?;
     ...
     */
    tablecmds_seams::check_table_not_in_use::call(
        &matviewRel,
        if is_create {
    ...
    if concurrent {
         ...
         */
        pgstat_relation::pgstat_count_truncate(
            matviewRel.rd_id,
            matviewRel.rd_rel.relisshared,
            matviewRel.pgstat_enabled,
        )?;
        if !skipData {
            pgstat_relation::pgstat_count_heap_insert(
                matviewRel.rd_id,
                matviewRel.rd_rel.relisshared,
                matviewRel.pgstat_enabled,
                processed as i64,
...
fn refresh_matview_datafill<'mcx>(
     ...
     */
    let token = tcop_dest::dest_receiver_state_token(dest);
...
pub fn CreateTransientRelDestReceiver(transientoid: Oid) -> PgResult<DestReceiverHandle> {
    ...
    Ok(tcop_dest::register_dest_receiver(
...
fn transientrel_shutdown<'mcx>(mcx: Mcx<'mcx>, state: u64) -> PgResult<()> {
    ...
    if let Some(bistate) = st.bistate.as_deref_mut() {
        heapam::FreeBulkInsertState(bistate);
    }
...
/// see `tcop_dest`), so this is not wired into the vtable — it exists for
...
fn quote_qualified_relname(mcx: Mcx<'_>, rel: &Relation<'_>) -> PgResult<String> {
    ...
    let qualified = ruleutils_seams::quote_qualified_identifier::call(
        mcx,
...
pub fn init_seams() {
    ...
    utility_out_seams::exec_refresh_mat_view::set(exec_refresh_mat_view_utility);
