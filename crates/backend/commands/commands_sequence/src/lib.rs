...
const REGBUF_WILL_INIT: u8 = wal::xloginsert::REGBUF_WILL_INIT;
...
pub fn DefineSequence<'mcx>(
    ...
    if seq.if_not_exists {
        let seqoid = namespace_seams::range_var_get_and_check_creation_namespace::call(
        ...
        if OidIsValid(seqoid) {
            ...
            pg_depend_seams::checkMembershipInCurrentExtension::call(mcx, &address)?;
    ...
    indexing_seams::catalog_insert_pg_sequence::call(&seqform)?;
...
pub fn ResetSequence<'mcx>(mcx: Mcx<'mcx>, seq_relid: Oid) -> PgResult<()> {
    ...
    let startv = match syscache_seams::search_seqrelid::call(seq_relid)? {
        Some(p) => p.seqstart,
        None => {
     ...
     */
    let seq_rel = sequence_seams::sequence_open::call(mcx, seq_relid, NoLock)?;
...
fn fill_seq_with_data<'mcx>(
    ...
    if rel_relpersistence(rel) == RELPERSISTENCE_UNLOGGED {
        ...
        catalog_storage_seams::smgr_create_init_fork_and_log::call(locator)?;

        fill_seq_fork_with_data(mcx, rel, data, ForkNumber::INIT_FORKNUM)?;

        // FlushRelationBuffers(rel);
        bufmgr_seams::flush_relation_buffers::call(rel)?;
...
fn fill_seq_fork_with_data<'mcx>(
    ...
    let buf = bufmgr_seams::extend_buffered_rel::call(rel, fork_num)?;
    debug_assert!(bufmgr_seams::buffer_get_block_number::call(buf) == 0);
    ...
    bufmgr_seams::with_buffer_page::call(buf, &mut |page: &mut [u8]| {
        page::PageInit(page, BLCKSZ, 4)?;
        ...
        let mut pm = page::PageMut::new(page)?;
        let offnum = page::PageAddItemExtended(
            &mut pm,
            &item_for_page,
    ...
    if rel_needs_wal(rel) || fork_num == ForkNumber::INIT_FORKNUM {
        ...
        xloginsert_seams::xlog_register_buffer::call(0, buf, REGBUF_WILL_INIT)?;

        let xlrec = XlSeqRec {
            locator: rel_locator(rel),
        };
        xloginsert_seams::xlog_register_data::call(&serialize_locator(
            &xlrec.locator,
        ))?;
        xloginsert_seams::xlog_register_data::call(&item)?;

        let recptr = xloginsert_seams::xlog_insert_record::call(
            RM_SEQ_ID,
            XLOG_SEQ_LOG,
        )?;
        bufmgr_seams::page_set_lsn::call(buf, recptr)?;
    }
...
fn special_pointer_offset(page: &[u8]) -> PgResult<usize> {
    let pref = page::PageRef::new(page)?;
    let sp = page::PageGetSpecialPointer(&pref)?;
...
pub fn AlterSequence<'mcx>(
    ...
    let relid = namespace_seams::range_var_get_relid_owns_seq::call(
    ...
    let mut seqform = match syscache_seams::search_seqrelid::call(relid)? {
        Some(t) => t,
        None => {
    ...
    if need_seq_rewrite {
         ...
         */
        let seqrel = sequence_seams::sequence_open::call(mcx, relid, NoLock)?;
...
pub fn DeleteSequenceTuple(relid: Oid) -> PgResult<()> {
    let found = indexing_seams::catalog_delete_pg_sequence::call(relid)?;
    if !found {
...
pub fn nextval<'mcx>(fcinfo: &mut FunctionCallInfoBaseData<'mcx>) -> PgResult<ValueDatum<'mcx>> {
    ...
    let relid =
        namespace_seams::range_var_get_relid_from_text::call(mcx, &name, NoLock, false)?;
...
pub fn nextval_internal<'mcx>(mcx: Mcx<'mcx>, relid: Oid, check_permissions: bool) -> PgResult<i64> {
    ...
    let (incby, maxv, minv, cache, cycle) =
        match syscache_seams::search_seqrelid::call(relid)? {
    ...
    let page_lsn = bufmgr_seams::page_get_lsn::call(buf)?;
    ...
    if logit && rel_needs_wal(&seqrel) {
        ...
        xloginsert_seams::xlog_register_data::call(&serialize_locator(&loc))?;
        xloginsert_seams::xlog_register_data::call(&logged_item)?;
        let recptr = xloginsert_seams::xlog_insert_record::call(
            RM_SEQ_ID,
            XLOG_SEQ_LOG,
        )?;
...
fn write_seq_data_into_buffer<'mcx>(
    ...
    bufmgr_seams::with_buffer_page::call(buf, &mut |page: &mut [u8]| {
        let (item_off, item_len, t_hoff) = {
            let pref = page::PageRef::new(page)?;
            let lp = page::PageGetItemId(&pref, FIRST_OFFSET_NUMBER)?;
            let item = page::PageGetItem(&pref, &lp)?;
            let off = item.as_ptr() as usize - page.as_ptr() as usize;
            let len = page::ItemIdGetLength(&lp) as usize;
...
pub fn currval_internal<'mcx>(mcx: Mcx<'mcx>, relid: Oid) -> PgResult<i64> {
    ...
    if aclchk_seams::pg_class_aclcheck::call(
        seqtable_get(relid, |e| e.relid),
        miscinit_seams::get_user_id::call(),
...
pub fn lastval_internal<'mcx>(mcx: Mcx<'mcx>) -> PgResult<i64> {
    ...
    if !syscache_seams::search_syscache_exists_reloid::call(last_relid)? {
        return ereport(ERROR)
            .errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE)
            .errmsg("lastval is not yet defined in this session")
            .finish(here("lastval"))
...
fn do_setval<'mcx>(mcx: Mcx<'mcx>, relid: Oid, next: i64, iscalled: bool) -> PgResult<()> {
    ...
    let (maxv, minv) = match syscache_seams::search_seqrelid::call(relid)? {
...
fn lock_and_open_sequence<'mcx>(mcx: Mcx<'mcx>, relid: Oid) -> PgResult<Relation<'mcx>> {
    let thislxid = lmgr_proc_seams::my_proc_lxid::call();
    ...
    sequence_seams::sequence_open::call(mcx, relid, NoLock)
}
...
fn read_seq_tuple<'mcx>(
    ...
    let buf = bufmgr_seams::read_buffer::call(rel, 0)?;
    bufmgr_seams::lock_buffer_exclusive::call(buf)?;
    ...
    if needs_dirty_hint {
        bufmgr_seams::mark_buffer_dirty_hint::call(buf, true);
    }
    ...
    *need_seq_rewrite = false;
    ...
    if let Some(as_type) = as_type {
        ...
        let as_type_location = as_type
            ...
            .unwrap_or(-1);
        let newtypid =
            parse_type_seams::typename_type_id_from_defelem::call(as_type).map_err(
                |e| {
...
fn process_owned_by<'mcx>(
    ...
    if nnames == 1 {
        ...
        let trel = common_relation_seams::relation_openrv::call(mcx, &rv, AccessShareLock)?;
        ...
        let an = lsyscache_seams::get_attnum::call(rel_relid(&trel), attrname)?;
    ...
    if deptype == DEPENDENCY_AUTO {
        if let Some((table_id, _col_id)) =
            dependency_seams::sequence_is_owned::call(rel_relid(seqrel), DEPENDENCY_INTERNAL)?
        {
            return ereport(ERROR)
    ...
    if let Some(ref trel) = tablerel {
        ...
        dependency_seams::record_dependency_on::call(depobject, refobject, deptype)?;
    }
...
pub fn pg_sequence_parameters_core<'mcx>(
    ...
    let datum =
        funcapi_seams::record_from_values::call(mcx, &coltypes, &values, &isnull)?;
    Ok(datum)
}
...
pub fn seq_redo(record: &mut wal::rmgr::XLogReaderState<'_>) -> PgResult<()> {
    ...
    let buffer = xlogutils_seams::xlog_init_buffer_for_redo::call(record, 0)?;
    ...
    page::PageInit(&mut localpage, BLCKSZ, 4)?;
    {
        // sm->magic = SEQ_MAGIC
        let pref = page::PageRef::new(&localpage)?;
        let off = page::PageGetSpecialPointer(&pref)?.as_ptr() as usize
            - localpage.as_ptr() as usize;
        localpage[off..off + 4].copy_from_slice(&SEQ_MAGIC.to_ne_bytes());
    }
    {
        let mut pm = page::PageMut::new(&mut localpage)?;
        let off = page::PageAddItemExtended(&mut pm, item, FIRST_OFFSET_NUMBER, 0)?;
    ...
    {
        let mut pm = page::PageMut::new(&mut localpage)?;
        page::PageSetLSN(&mut pm, lsn);
    }

    bufmgr_seams::with_buffer_page::call(buffer, &mut |page: &mut [u8]| {
...
fn record_get_data<'a>(record: &'a wal::rmgr::XLogReaderState<'_>) -> &'a [u8] {
    record.record.as_ref().map(|r| r.data()).unwrap_or(&[])
}

fn record_get_info(record: &wal::rmgr::XLogReaderState<'_>) -> u8 {
...
pub fn seq_mask(pagedata: &mut [u8], _blkno: BlockNumber) -> PgResult<()> {
    bufmask_seams::mask_page_lsn_and_checksum::call(pagedata);
    bufmask_seams::mask_unused_space::call(pagedata)?;
    Ok(())
}
...
fn format_type_be(type_oid: Oid) -> PgResult<String> {
    format_type_seams::format_type_be_str::call(type_oid)
}
...
fn text_to_str(t: &datum::varlena::Bytea<'_>) -> String {
...
pub fn init_seams() {
    sequence_seams_2::seq_redo::set(seq_redo);
    sequence_seams_2::seq_mask::set(seq_mask);
    sequence_seams_2::reset_sequence_caches::set(ResetSequenceCaches);
    sequence_seams_2::reset_sequence::set(ResetSequence);
    sequence_seams_2::DeleteSequenceTuple::set(DeleteSequenceTuple);
    sequence_seams_2::nextval_internal::set(nextval_internal);
    sequence_seams_2::sequence_change_persistence::set(SequenceChangePersistence);

    // ProcessUtilitySlow dispatch arms (utility.c CREATE/ALTER SEQUENCE).
    utility_out_seams::define_sequence::set(define_sequence_arm);
    utility_out_seams::alter_sequence::set(alter_sequence_arm);
