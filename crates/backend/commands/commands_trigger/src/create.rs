...
pub fn CreateTriggerFiringOn<'mcx>(
    ...
    let rel = if valid(rel_oid) {
        table_seams::table_open::call(mcx, rel_oid, ShareRowExclusiveLock)?
    } else {
        ...
        let oid =
            catalog_namespace::RangeVarGetRelid(mcx, &arv, ShareRowExclusiveLock, false)?;
        table_seams::table_open::call(mcx, oid, ShareRowExclusiveLock)?
    };
    ...
    if !guc_tables::vars::allowSystemTableMods.read()
        && catalog_catalog::IsSystemRelation(&rel)
    {
        return Err(ereport(ERROR)
    ...
    if stmt.isconstraint {
        if valid(ref_rel_oid) {
            ...
            constrrelid =
                catalog_namespace::RangeVarGetRelid(mcx, &acr, AccessShareLock, false)?;
        }
    }
    ...
    if !is_internal {
        ...
        if r != ACLCHECK_OK {
            aclchk::aclcheck_error::call(
                r,
                objectaddress::resolve::get_relkind_objtype(relkind as u8),
                Some(relname.clone()),
            )?;
        }
        if valid(constrrelid) {
            ...
            if r != ACLCHECK_OK {
                let k = lsyscache::relation::get_rel_relkind(constrrelid)?;
                let nm = lsyscache::relation::get_rel_name(mcx, constrrelid)?
                    .map(|s| s.to_string())
                    .unwrap_or_default();
                aclchk::aclcheck_error::call(
                    r,
                    objectaddress::resolve::get_relkind_objtype(k),
                    Some(nm),
                )?;
            }
        }
    ...
    if partition_recurse {
        pg_inherits::find_all_inheritors(mcx, relid, ShareRowExclusiveLock, false)?;
    }
    ...
    if stmt.isconstraint && !valid(constraint_oid) {
        ...
        let trigname_for_constr = stmt
            ...
            .map(|s| s.as_str())
            .unwrap_or_default();
        constraint_oid = pg_constraint_seams::create_constraint_entry::call(
            mcx,
            pg_constraint_seams::CreateConstraintEntryArgs {
    ...
    if !valid(funcoid) {
        ...
        funcoid = func::LookupFuncName(mcx, &names, 0, &[], false)?;
    }
    if !is_internal {
        let userid = miscinit::GetUserId();
    ...
    if lsyscache::function::get_func_rettype(funcoid)? != TRIGGEROID {
        return Err(ereport(ERROR)
    ...
    if existing.is_none() {
        trigoid = catalog_catalog::GetNewOidWithIndex(
            &tgrel,
    ...
    if partition_recurse {
        ...
        for i in 0..(partdesc.nparts as usize) {
            ...
            let child_tbl = table_seams::table_open::call(
                mcx,
                child_oid,
...
fn map_when_to_partition<'mcx>(
    mcx: Mcx<'mcx>,
    when_clause: &Option<Node<'mcx>>,
    child_tbl: &rel::RelationData<'mcx>,
    parent: &rel::RelationData<'mcx>,
...
fn transform_trigger_transitions<'mcx>(
    ...
    for node in stmt.transitionRels.iter() {
        ...
        //   }
        if pt::TRIGGER_FOR_ROW(tgtype)
            && pg_inherits::has_superclass(rel.rd_id)?
        {
            if rel.rd_rel.relispartition {
                return Err(ereport(ERROR)
                    .errcode(ERRCODE_FEATURE_NOT_SUPPORTED)
...
fn transform_trigger_when<'mcx>(
    ...
    parser_relation::addNSItemToQuery(mcx, &mut pstate, oldnsitem, false, true, true)?;
    ...
    parser_relation::addNSItemToQuery(mcx, &mut pstate, newnsitem, false, true, true)?;
    ...
    let mut when_expr = match clause_seams::transform_where_clause::call(
        mcx,
        &mut pstate,
        Some(clause_copy),
    ...
    if let Some(e) = when_expr.as_mut() {
        parse_collate::assign_expr_collations(Some(&pstate), e)?;
    }
    ...
    for var_expr in vars::pull_var_clause(mcx, &when_node, 0)? {
...
pub fn init_seams() {
    ...
    utility_out_seams::create_trigger::set(
