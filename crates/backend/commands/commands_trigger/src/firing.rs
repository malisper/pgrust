...
pub fn slot_attisnull_impl(slot: types_ri_triggers::TupleTableSlotRef, attnum: i16) -> PgResult<bool> {
    let r = with_slot_tuple(slot.0, |tup, rel| {
        heaptuple::heap_attisnull(&tup.tuple, attnum as i32, Some(&rel.rd_att))
    });
...
pub fn slot_getattr_impl<'mcx>(
    ...
    let r = CURRENT_TRIGGER_SLOTS.with(|cell| -> PgResult<Option<(SlotDatum<'mcx>, bool)>> {
        ...
        let col =
            heaptuple::heap_getattr(mcx, tup, attnum as i32, &s.relation.rd_att)?;
        Ok(Some((col.0, col.1)))
    })?;
    ...
    for &attnum in attnums {
        ...
        let value = if isnull {
            ...
            let (foutoid, _typisvarlena) =
                lsyscache_seams::get_type_output_info::call(atttypid)?;
            Some(
                fmgr_seams::oid_output_function_call_datum::call(
                    mcx, foutoid, datum,
                )?,
            )
        };
...
pub fn pk_datum_image_eq_impl<'mcx>(
    ...
    let r = CURRENT_TRIGGER_SLOTS.with(|cell| -> Option<bool> {
        ...
        datum_seams::datum_image_eq_v::call(
            oldvalue,
            newvalue,
            ca.attbyval,
            ca.attlen,
...
pub fn tg_relation_tuple_satisfies_snapshot_self_impl(
    ...
    let r = CURRENT_TRIGGER_SLOTS.with(|cell| -> PgResult<Option<bool>> {
        ...
        let mut snap = snapshot::SnapshotData::sentinel(snapshot::SnapshotType::SNAPSHOT_SELF);
...
pub fn tg_relation_impl<'mcx>(
    ...
    let r = CURRENT_TRIGGER_SLOTS.with(|cell| {
        cell.borrow().as_ref().map(|s| {
            ...
            let aliased: rel::Relation<'static> = s.relation.alias();
            ...
            unsafe {
                core::mem::transmute::<rel::Relation<'static>, rel::Relation<'mcx>>(
                    aliased,
                )
            }
        })
...
pub fn after_trigger_execute<'mcx>(
    ...
    let tg_relation: rel::Relation<'static> = {
    ...
    let slots_relation: rel::Relation<'static> = {
        let aliased = rel.relation.alias();
        // SAFETY: query-context lifetime extension, as above.
        unsafe { core::mem::transmute(aliased) }
    };
...
fn fetch_trigger_tuple<'mcx>(
    ...
    let rel = table_seams::table_open::call(mcx, relid, NoLock)?;
    let snapshot_any =
        snapshot::SnapshotData::sentinel(snapshot::SnapshotType::SNAPSHOT_ANY);

    let fetched =
        heapam_seams::heap_fetch::call(mcx, &rel, &snapshot_any, *ctid, false)?;
...
fn fetch_trigger_tuple_cp<'mcx>(
    ...
    let leaf_rel = table_seams::table_open::call(mcx, leaf_relid, NoLock)?;
    let map =
        next_seams::convert_tuples_by_name::call(mcx, &leaf_rel.rd_att, root_desc)?;
    ...
    let result = match map.as_ref() {
        ...
        Some(map) => {
            ...
            let converted = next_seams::execute_attr_map_tuple::call(
                mcx,
                &leaf_formed.tuple,
...
pub fn validate_foreign_key_constraint<'mcx>(
    ...
    // only the trigger's tgconstraint) for the duration of the call.
    {
        ...
        if !hasperiod
            && ri_triggers_seams::ri_initial_check::call(
                mcx,
    ...
    let mut scan = heapam_seams::heap_beginscan::call(
        mcx,
        rel.alias(),
        snapshot.clone(),
        flags,
    ...
    let scan_result: PgResult<()> = (|| {
        while let Some(formed) =
            heapam_seams::heap_getnext::call(mcx, &mut scan)?
        {
            ...
            let row_relation: rel::Relation<'static> = {
                let aliased = rel.alias();
                unsafe { core::mem::transmute(aliased) }
            };
            ...
            ri_triggers_seams::ri_fkey_check_ins::call(
                mcx,
    ...
    let end = heapam_seams::heap_endscan::call(scan);
...
pub fn detach_partition_remove_check<'mcx>(
    ...
    ri_triggers_seams::ri_partition_remove_check::call(
        mcx,
        types_ri_triggers::TriggerRef(crate::ri_accessors::CURRENT),
        fk_rel,
        partition,
...
pub fn after_trigger_mark_events(
    ...
    if deferred_found && miscinit::InSecurityRestrictedOperation() {
...
fn exec_before_statement_triggers(
    ...
    let updated_cols = if tg_event_bit == TRIGGER_EVENT_UPDATE {
        let mcx = estate.es_query_cxt;
        execUtils_seams::exec_get_all_updated_cols::call(mcx, estate, relinfo)?
    } else {
        None
    };
...
fn fire_statement_trigger(
    ...
    let tg_relation: rel::Relation<'static> = {
        ...
        unsafe {
            core::mem::transmute::<rel::Relation<'_>, rel::Relation<'static>>(aliased)
        }
    };
    let slots_relation: rel::Relation<'static> = {
...
fn exec_truncate_fire_after_triggers_impl(
    ...
    execUtils_seams::free_executor_state::call(state.estate)?;
    ...
    for i in 0..numtriggers {
        ...
        if newtuple.is_none() {
            let (formed, _should_free) = {
                ...
                execTuples_seams::exec_fetch_slot_heap_tuple::call(mcx, sd, true)?
            };
...
fn partition_move_in_before_trigger_error<'mcx>(
    ...
    let nspname =
        lsyscache_seams::get_namespace_name::call(mcx, nspoid)?
            .map(|s| s.as_str().to_string())
            .unwrap_or_default();
    Ok(ereport(ERROR)
...
pub fn tsv_make_and_install_tsvector_impl(
    ...
    CURRENT_TRIGGER_SLOTS.with(|cell| {
        ...
        let modified = heaptuple::heap_modify_tuple_by_cols(
            mcx,
            rettuple,
            &s.relation.rd_att,
            1,
...
fn bms_to_sorted_vec(modified_cols: Option<&::nodes::Bitmapset<'_>>) -> Option<Vec<i32>> {
    ...
    let mut x = -1;
    loop {
        x = nodes_core::bitmapset::bms_next_member(modified_cols, x);
        if x < 0 {
            break;
        }
        out.push(x);
...
fn trigger_enabled<'mcx>(
    ...
    if has_qual {
        ...
        let pass = execExpr_seams::exec_qual::call(&mut predicate, econtext, estate)?;
...
fn build_trigger_when_predicate<'mcx>(
    ...
    let node = read_seams::string_to_node::call(mcx, &tgqual)?;
    ...
    expr = rewritehandler_seams::expand_generated_columns_in_expr::call(
        mcx,
        Some(expr),
        rel_oid,
        PRS2_OLD_VARNO,
    ...
    execExpr_seams::exec_prepare_qual::call(
...
fn change_var_nodes_expr(
    ...
    fn walk(node: Expr, rt_index: i32, new_index: i32) -> Expr {
        match node {
            ...
            other => nodes_core::nodefuncs::expression_tree_mutator(other, &mut |child| {
...
fn transition_table_add_tuple<'mcx>(
    ...
    let map = execMain_seams::exec_get_child_to_root_map_full::call(
        estate.es_query_cxt,
        estate,
        relinfo,
    )?;
...
impl TransitionEnvGuard {
    fn install(
        ...
        let depth = queryenvironment_home::push_query_env(env);
...
impl Drop for TransitionEnvGuard {
    fn drop(&mut self) {
        ...
        if let Some(mut env) = queryenvironment_home::pop_query_env(self.depth) {
    ...
    {
        {
            ...
            if fired_by_upd_or_del {
                const RI_TRIGGER_PK: i32 = 1;
                const RI_TRIGGER_FK: i32 = 2;
                let kind = ri_triggers_seams::ri_fkey_trigger_type::call(tgfoid);
                match kind {
                    RI_TRIGGER_PK => {
...
fn exec_ar_delete_triggers_impl<'mcx>(
    ...
    let oldslot = {
        ...
        let slot =
            trigger_seams::exec_get_trigger_old_slot::call(estate, relinfo)?;
...
fn exec_ir_delete_triggers_impl<'mcx>(
    ...
    // TupleTableSlot *slot = ExecGetTriggerOldSlot(estate, relinfo);
    let oldslot = trigger_seams::exec_get_trigger_old_slot::call(estate, relinfo)?;
...
fn get_tuple_for_trigger<'mcx>(
    ...
    let epqslot = match epqslot {
        ...
        None => {
            ...
            if !trigger_seams::get_tuple_for_trigger_fetch::call(
    ...
    *epqslot = None;
    ...
    let test = trigger_seams::get_tuple_for_trigger_lock::call(
        estate,
        relinfo,
        tid,
        oldslot,
...
fn exec_ar_update_triggers_impl<'mcx>(
    ...
    let oldslot = if have_old {
        let slot =
            trigger_seams::exec_get_trigger_old_slot::call(estate, tupsrc)?;
    ...
    if is_crosspart_update {
        ...
        let root_old = trigger_seams::exec_get_trigger_old_slot::call(estate, relinfo)?;
        if let Some((attr_map, _outdesc)) =
            execMain_seams::exec_get_child_to_root_map_full::call(mcx, estate, tupsrc)?
        {
            execTuples_seams::execute_attr_map_slot_explicit::call(
                estate, &attr_map, oldslot, root_old,
            )?;
        } else {
            execTuples_seams::exec_copy_slot::call(estate, root_old, oldslot)?;
        }
        ...
        let root_new =
            trigger_seams::exec_get_trigger_new_slot::call(estate, relinfo)?;
        let dst_rri = _dst_partinfo.expect("cross-partition update has a dst partition");
        if let Some((attr_map, _outdesc)) =
            execMain_seams::exec_get_child_to_root_map_full::call(mcx, estate, dst_rri)?
        {
            execTuples_seams::execute_attr_map_slot_explicit::call(
                estate, &attr_map, ns, root_new,
            )?;
        } else {
            execTuples_seams::exec_copy_slot::call(estate, root_new, ns)?;
        }
...
fn has_noncloned_pk_fkey_trigger_impl<'mcx>(
    ...
    for i in 0..numtriggers {
        ...
        if !tgisclone
            && ri_triggers_seams::ri_fkey_trigger_type::call(tgfoid)
                == RI_TRIGGER_PK
        {
            return Ok(true);
        }
...
fn get_trigger_oid_scan<'mcx>(
    ...
    let tgrel =
        table_seams::table_open::call(mcx, pt::TriggerRelationId, AccessShareLock)?;
...
fn fire_global_event_cycle(
    ...
    if snapshot_pushed {
        let pop = snapmgr_seams::pop_active_snapshot::call();
        result?;
        pop?;
    } else {
        result?;
    ...
    // FreeExecutorState(estate);
    execUtils_seams::free_executor_state::call(estate)?;
    Ok(())
}
...
fn after_trigger_set_state<'mcx, 'n>(
    ...
    if !stmt.deferred {
        ...
        let delete_ok =
            !transam_xact_seams::is_sub_transaction::call();
        fire_global_event_cycle(
            mcx,
            /* immediate_only */ true,
            delete_ok,
...
fn resolve_constraint_trigger_oids<'mcx, 'n>(
    ...
    for cnode in stmt.constraints.iter() {
        ...
        let namespacelist: Vec<Oid> = if let Some(sch) = constraint.schemaname.as_ref() {
            let nsoid = namespace_seams::lookup_explicit_namespace::call(
                sch.as_str(),
                false,
            )?;
            vec![nsoid]
        } else {
            namespace_seams::fetch_search_path::call(mcx, true)?
                .iter()
                .copied()
                .collect()
        };
...
pub fn init_seams() {
    ...
    utility_out_seams::after_trigger_set_state::set(after_trigger_set_state_seam);
    ...
    tablecmds_seams::exec_truncate_fire_before_triggers::set(
        exec_truncate_fire_before_triggers_impl,
    );
    tablecmds_seams::exec_truncate_fire_after_triggers::set(
