...
//!     [`TriggerData`](nodes::trigger::TriggerData) to the callee through
...
fn assign_session_replication_role(
    ...
    if changed {
        ...
        let _ = cache_plancache::ResetPlanCache();
    }
}
...
pub fn init_seams() {
    ...
    guc_tables::vars::SessionReplicationRole.install(
    ...
    guc_tables::hooks::assign_session_replication_role
    ...
    tablecmds_seams::after_trigger_pending_on_rel::set(|relid| {
