...
fn range_var_callback_for_rename_trigger(rv: &RangeVar, relid: Oid) -> PgResult<()> {
    ...
    let relkind = match lsyscache::relation::get_rel_relkind(relid) {
        Ok(k) => k,
        Err(_) => return Ok(()),
    };
...
pub fn renametrig<'mcx>(
    ...
    let targetrel = table_seams::table_open::call(mcx, relid, NoLock)?;
    ...
    if let Some(tuple) = genam_seams::systable_getnext::call(mcx, tgscan.desc_mut())? {
        ...
        if valid(tgparentid) {
            let parent =
                partition_seams::get_partition_parent::call(relid, false)?;
            let parent_name = lsyscache::relation::get_rel_name(mcx, parent)?
                .map(|s| s.to_string())
                .unwrap_or_default();
            return Err(ereport(ERROR)
        ...
        if targetrel.rd_rel.relkind == RELKIND_PARTITIONED_TABLE {
            let partdesc =
                partdesc::RelationGetPartitionDesc(mcx, &targetrel, true)?;
...
fn renametrig_partition<'mcx>(
    ...
    while let Some(tuple) = genam_seams::systable_getnext::call(mcx, tgscan.desc_mut())? {
        ...
        let partition_rel =
            table_seams::table_open::call(mcx, partition_id, NoLock)?;
        ...
        if partition_rel.rd_rel.relkind == RELKIND_PARTITIONED_TABLE {
            let partdesc =
                partdesc::RelationGetPartitionDesc(mcx, &partition_rel, true)?;
            for i in 0..partdesc.nparts as usize {
...
pub fn init_seams() {
    trigger_seams::renametrig::set(|mcx, stmt| {
