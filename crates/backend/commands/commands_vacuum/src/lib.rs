...
//! `nodes` enum. The src-idiomatic `vac_context`/`MemoryContextHandle`
...
use define_seams as define;
use vacuum_seams as rt;
...
use async_seams as async_seam;
use cluster_seams as cluster_seam;
...
use pg_inherits_seams as pg_inherits_seam;
...
fn vacuum_tid_is_dead_impl(tid: ItemPointerData, callback_state_handle: u64) -> bool {
    ...
    if vacuum_seams::bulk_delete_callback::is_closure_handle(callback_state_handle)
    {
        return vacuum_seams::bulk_delete_callback::invoke(
