...
pub fn init_seams() {
    ...
    vacuum::interrupt_pending::set(|| {
        Ok(init_small_seams::interrupt_pending::call())
    });
    vacuum::config_reload_pending::set(|| {
        Ok(interrupt::ConfigReloadPending())
    });
    ...
    vacuum::new_guc_nest_level::set(|| {
        Ok(guc_seams::new_guc_nest_level::call())
    });
    vacuum::at_eoxact_guc::set(|is_commit, nestlevel| {
        guc_seams::at_eoxact_guc::call(is_commit, nestlevel)
    });
    ...
    utility_out_seams::exec_vacuum::set(exec_vacuum_arm);
