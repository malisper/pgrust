...
//! [`rel::Relation`], the `systable` index scans
...
pub fn CommentObject<'mcx>(
    ...
    if stmt.objtype == OBJECT_DATABASE {
        ...
        if !OidIsValid(
            dbcommands_seams::get_database_oid::call(&database, true)?,
        ) {
...
pub fn init_seams() {
    comment_seams::DeleteComments::set(DeleteComments);
    comment_seams::move_relation_comment::set(move_relation_comment);
    ...
    utility_out_seams::comment_object::set(comment_object_seam);
    utility_out_seams::comment_object_slow::set(comment_object_slow_seam);
    ...
    tablecmds_seams::get_comment::set(GetComment);
    tablecmds_seams::comment_object::set(re_add_comment_object_seam);
}
