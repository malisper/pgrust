...
enum CopySourceReader {
    ...
    Pipe { stream: fd_seams::PgFileStream },
    ...
    SOURCES.with(|s| {
        ...
        match reader {
            ...
            CopySourceReader::Pipe { stream } => {
                ...
                // zero-byte read.
                let data = fd_seams::copy_pipe_read::call(
                    *stream,
                    maxread as i32,
                )?;
...
pub fn BeginCopyFrom<'mcx>(
    ...
    for attnum in 1..=num_phys_attrs {
        ...
        let (func_oid, typioparam) = if binary {
            lsyscache_seams::get_type_binary_input_info::call(att.atttypid)?
        } else {
            lsyscache_seams::get_type_input_info::call(att.atttypid)?
        };
        let resolved = fmgr_core::fmgr_info(mcx, func_oid)?;
        ...
        if (opts.default_print.is_some() || !in_list) && att.attgenerated == 0 {
            ...
            let built = rewritehandler_seams::build_column_default::call(
                mcx,
                rel.alias(),
                attnum as i32,
            )?;
            if let Some(defexpr) = built {
                ...
                let planned =
                    planner_pc_seams::expression_planner_value::call(
                        mcx, owned,
                    )?
                    .clone_in(mcx)?;
    ...
    let database_encoding = mbutils_seams::get_database_encoding::call();
    let file_encoding = if file_encoding_opt < 0 {
        mbutils_seams::pg_get_client_encoding::call()
    } else {
        file_encoding_opt
    };
    ...
    let conversion_proc = if need_transcoding {
        let proc = mbutils_seams::find_default_conversion_proc::call(
            file_encoding,
            database_encoding,
        )?;
    ...
    copyfrom_seams::pgstat_progress_start_command_copy::call(cstate.rel.rd_id)?;
    cstate.bytes_processed = 0;
    ...
    if let Some(cb) = data_source_cb {
        ...
        if is_program {
            ...
            let stream =
                fd_seams::open_pipe_from_program::call(filename)?;
        ...
        let data =
            fd_seams::read_server_file::call(mcx, filename, 0, -1, false)?;
        let data = match data {
            Some(v) => v.to_vec(),
            None => {
                return Err(PgError::error(format!(
    ...
    copyfrom_seams::pgstat_progress_update_multi_param::call(
        &[
...
pub fn EndCopyFrom(state: CopyFromStateData<'_>) -> PgResult<()> {
    ...
    if let Some(handle) = state.cstate.copy_file {
        if state.is_program {
            ...
            if let Some(stream) = stream {
                ...
                fd_seams::close_pipe_from_program::call(
                    stream,
                    cmd,
    ...
    copyfrom_seams::pgstat_progress_end_command::call()?;
    Ok(())
}
...
pub fn CopyFrom<'mcx>(mcx: Mcx<'mcx>, state: &mut CopyFromStateData<'mcx>) -> PgResult<u64> {
    ...
    if state.freeze {
        ...
        let create_subid =
            tablecmds_seams::relation_get_create_subid::call(&state.cstate.rel)?;
        let new_rfl_subid =
            tablecmds_seams::relation_get_new_relfilelocator_subid::call(
                &state.cstate.rel,
            )?;
    ...
    let processed = {
        ...
        execIndexing_seams::exec_open_indices::call(estate, rri, false)?;
        ...
        let slot = table_tableam::table_slot_create(mcx, &rel_alias)?;
        ...
        // if the column is not copied from input, record it in `defmap`.
        {
            ...
            for (i, maybe) in raw.into_iter().enumerate() {
                ...
                let compiled =
                    execExpr_seams::exec_prepare_expr::call(&defexpr, estate)?;
        ...
        trigger_seams::exec_bs_insert_triggers::call(estate, rri)?;
        ...
        loop {
            ...
            execUtils::ResetPerTupleExprContext(estate);

            // ExecClearTuple(myslot);
            execTuples::slot_store_fetch::ExecClearTuple(estate.slot_data_mut(singleslot))?;
                    ...
                    .is_some_and(|c| c.error_occurred())
            {
                ...
                // (copyfrom.c:1164).
                copyfrom_seams::pgstat_progress_update_param::call(
            ...
            if let Some(qualexpr) = state.cstate.qualexpr.as_mut() {
                ...
                if !execExpr_seams::exec_qual::call(
            ...
            if let Some(proute) = proute.as_mut() {
                ...
                result_rel_info = execPartition_seams::exec_find_partition::call(
                    mcx,
                    &mut mtstate,
                    target_rri,
                    proute,
                ...
                if prev_result_rel_info != Some(result_rel_info) {
                    ...
                    heapam::ReleaseBulkInsertStatePin(&mut bistate);
                ...
                if let Some(attr_map) = map {
                    ...
                    myslot = execTuples_seams::execute_attr_map_slot_explicit::call(
            ...
            if has_before_insert_row_trig
                && !trigger_seams::exec_br_insert_triggers::call(
            ...
            if has_instead_insert_row_trig {
                ...
                trigger_seams::exec_ir_insert_triggers::call(
                    estate, result_rel_info, myslot,
                )?;
            } else {
                ...
                // back into the slot before checking constraints.
                {
                    let has_gen_stored = relation_alias(estate, result_rel_info)
                        ...
                        .unwrap_or(false);
                    if has_gen_stored {
                        nodeModifyTable_seams::exec_compute_stored_generated::call(
                            mcx,
                            estate,
                            result_rel_info,
                            myslot,
        ...
        heapam::FreeBulkInsertState(&mut bistate);
        ...
        execUtils::ExecResetTupleTable(estate, false)?;
        execUtils::ExecCloseResultRelations(estate)?;
        execUtils::ExecCloseRangeTableRelations(estate)?;

        processed
    };

    // FreeExecutorState(estate);
    execUtils::free_executor_state_in(estate_owned)?;
...
fn store_row_into_slot<'mcx>(
    ...
    execTuples::slot_store_fetch::ExecStoreVirtualTuple(s)
}
...
fn input_function_call_safe_impl<'mcx>(
    ...
    // is keyed on fn_oid.
    let resolved = fmgr_core::fmgr_info(mcx, flinfo.fn_oid)?;

    let out = fmgr_core::input_function_call_safe_typed(
        mcx,
        &resolved.resolution,
        resolved.finfo,
        string,
...
fn receive_function_call_impl<'mcx>(
    ...
    let out = fmgr_core::receive_function_call_typed(
        mcx,
        &resolved.resolution,
        resolved.finfo,
        buf,
...
fn copy_limit_printout_length(s: &str) -> String {
    ...
    let slen = s.len();
    let len = mbutils_seams::pg_mbcliplen::call(
        s.as_bytes(),
        slen as i32,
