...
use fd_seams as fd_s;
use backend_progress_seams as progress_s;
...
fn send_copy_begin(cstate: &mut CopyToStateData<'_>) -> PgResult<()> {
    ...
    let mut buf = pqformat::pq_beginmessage(cstate.mcx, PQMSG_COPY_OUT_RESPONSE)?;
    pqformat::pq_sendbyte(&mut buf, format as u8)?; // overall format
    pqformat::pq_sendint16(&mut buf, natts as u16)?;
    for _ in 0..natts {
        pqformat::pq_sendint16(&mut buf, format as u16)?; // per-column formats
    }
...
fn send_copy_end(cstate: &CopyToStateData<'_>) -> PgResult<()> {
    ...
    pqformat::pq_putemptymessage(PQMSG_COPY_DONE)
}
...
fn copy_send_end_of_row(cstate: &mut CopyToStateData<'_>) -> PgResult<()> {
    match cstate.copy_dest {
        ...
        CopyDest::Frontend => {
            ...
            let _eof = pqcomm_seams::pq_putmessage::call(
                PQMSG_COPY_DATA,
                cstate.fe_msgbuf.as_bytes(),
            )?;
        }
...
pub fn BeginCopyTo<'mcx>(
    ...
    if let Some(rel) = cstate.rel.as_ref() {
        ...
        let rewritten = parser_analyze_seams::pg_analyze_and_rewrite_fixedparams::call(
            mcx, raw, source_text,
        )?;
        ...
        snapmgr_seams::push_copied_active_snapshot::call()?;
        snapmgr_seams::update_active_snapshot_command_id::call()?;
        ...
        let receiver = tcop_dest::dest_receiver_state_token(handle);
...
pub fn DoCopyTo(cstate: &mut CopyToStateData<'_>) -> PgResult<u64> {
    ...
    if let Some(rel) = cstate.rel.as_ref() {
        ...
        let mut slot = table_tableam::table_slot_create(cstate.mcx, &rel_alias)?;
...
fn copy_one_row_to<'mcx>(
    ...
    let cols = execTuples_seams::slot_getallattrs::call(cstate.mcx, slot)?;
...
pub fn init_seams() {
    copyto_seams::copy_dest_receive::set(copy_dest_receive);
    copyto_seams::create_copy_dest_receiver::set(CreateCopyDestReceiver);
}
