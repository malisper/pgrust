...
//! are registered into the tcop-dest router ([`tcop_dest`]) the same way
//! `copyto.c`'s `DR_copy` is — a real [`tcop_dest::ReceiverVtable`] keyed
...
fn build_coldef_checked<'mcx>(
    ...
    let col = nodes_core::makefuncs::make_column_def(mcx, colname, type_oid, typmod, collation)?;
    ...
    if !oid_is_valid(col.collOid) {
        ...
        if lsyscache_seams::type_is_collatable::call(typeoid)? {
            let colname = col.colname.as_ref().map(|s| s.as_str()).unwrap_or("");
            let fmt = format_type_seams::format_type_be::call(mcx, typeoid)?;
...
fn create_ctas_internal<'mcx>(
    ...
    createas_seams::create_ctas_relation::call(
        mcx,
        into.clone_in(mcx)?,
        attr_list,
        relkind,
    ...
    for tle in query.targetList.iter() {
        if !tle.resjunk {
            ...
            let info = match tle.expr.as_ref() {
                Some(expr) => nodeFuncs_seams::expr_type_info::call(expr)?,
                None => nodeFuncs_seams::ExprTypeInfo {
                    typid: InvalidOid,
                    typmod: -1,
...
pub fn ExecCreateTableAs<'mcx>(
     ...
     */
    createas_seams::jumble_and_post_analyze::call(mcx, query, query_string)?;
    ...
    if query.commandType == CmdType::CMD_UTILITY && query_is_execute_stmt(query) {
        ...
        qc = createas_seams::execute_query::call(
            mcx,
            estmt,
            into.clone_in(mcx)?,
            query_string,
    ...
    if into.skipData {
         ...
         */
        if do_refresh {
            let (_addr, new_qc) = matview_seams::RefreshMatViewByOid::call(
                address.objectId,
                true,
                false,
                false,
         ...
         * `run_ctas_executor`).
         */
        qc = createas_seams::run_ctas_executor::call(
            mcx,
            query.clone_in(mcx)?,
            into.clone_in(mcx)?,
            query_string,
...
pub fn CreateTableAsRelExists<'mcx>(
    ...
    let nspid = catalog_namespace::RangeVarGetCreationNamespace(mcx, &access_rv)?;

    /* oldrelid = get_relname_relid(into->rel->relname, nspid); */
    let relname = rel_node.relname.as_ref().map(|s| s.as_str()).unwrap_or("");
    let oldrelid = lsyscache_seams::get_relname_relid::call(relname, nspid)?;
...
fn intorel_startup<'mcx>(
    ...
    let into_relation_desc =
        table::table_open(mcx, into_relation_addr.objectId, AccessExclusiveLock)?;
     ...
     */
    if more_seams::check_enable_rls::call(
    ...
    if is_matview && !into.skipData {
        matview_seams::SetMatViewPopulatedState::call(
            into_relation_addr.objectId,
            true,
        )?;
    }
    ...
    let output_cid = transam_xact::GetCurrentCommandId(true)?;
...
fn intorel_shutdown<'mcx>(mcx: Mcx<'mcx>, state: u64) -> PgResult<()> {
    ...
    if !st.into.skipData {
        ...
        createas_seams::table_finish_bulk_insert::call(rel, st.ti_options)?;
    }
...
/// `tcop_dest`), so this is not wired into the vtable — it exists for
...
pub fn init_seams() {
    createas_seams::get_into_rel_eflags::set(get_into_rel_eflags_seam);
    createas_seams::exec_create_table_as::set(exec_create_table_as_seam);
    createas_seams::create_table_as_rel_exists::set(create_table_as_rel_exists_seam);
    createas_seams::create_into_rel_dest_receiver::set(
        create_into_rel_dest_receiver_seam,
    );
    createas_seams::create_into_rel_dest_receiver_setup::set(
    ...
    utility_out_seams::exec_create_table_as::set(exec_create_table_as_utility);
}
