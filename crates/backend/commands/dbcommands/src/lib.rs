...
//! [`dbcommands_seams`] that landed consumers
...
fn recovery_create_dbdir(path: &str, only_tblspc: bool) -> PgResult<()> {
    ...
    if let Some(st) = fd_seams::stat_file::call(path, true)? {
        if st.isdir {
            return Ok(());
        }
    }
    ...
    if reached && !tablespace_globals_seams::allow_in_place_tablespaces::call()? {
        return Err(ereport(PANIC)
    ...
    if let Err(errno) = fd_seams::pg_mkdir_p::call(path) {
        return Err(ereport(PANIC)
            .errmsg(format!(
...
pub fn dbase_redo(record: &mut XLogReaderState<'_>) -> PgResult<()> {
    ...
    if info == XLOG_DBASE_CREATE_FILE_COPY {
        ...
        let src_path =
            relpath::GetDatabasePath(xlrec.src_db_id(), xlrec.src_tablespace_id());
        let dst_path =
            relpath::GetDatabasePath(xlrec.db_id(), xlrec.tablespace_id());

        // Our theory for replaying a CREATE is to forcibly drop the target
        // subdirectory if present, then re-copy the source data.
        if let Some(st) = fd_seams::stat_file::call(&dst_path, true)? {
            if st.isdir && !fd_seams::rmtree::call(&dst_path, true) {
        ...
        if fd_seams::stat_file::call(&parent_path, true)?.is_none() {
        ...
        if fd_seams::stat_file::call(&src_path, true)?.is_none() {
            recovery_create_dbdir(&src_path, false)?;
        }

        // Force dirty buffers out to disk, to ensure source is up-to-date.
        bufmgr_seams::flush_database_buffers::call(xlrec.src_db_id())?;
        ...
        let in_hot_standby = standby_seams::in_hot_standby::call();
        if in_hot_standby {
            ...
            standby_seams::resolve_recovery_conflict_with_database::call(db_id)?;
        }

        // Drop any database-specific replication slots.
        replication_slot::ReplicationSlotsDropDBSlots(
            db_id,
        ...
        for tblspc in xlrec.tablespace_ids() {
            let dst_path = relpath::GetDatabasePath(db_id, tblspc);

            // And remove the physical files.
            if !fd_seams::rmtree::call(&dst_path, true) {
                ereport(WARNING)
                    .errmsg(format!(
                        "some useless files may be left behind in old database directory \"{dst_path}\""
                    ))
...
pub fn init_seams() {
    ...
    catalog_storage_seams::scan_source_database_pg_class::set(
        heavy::scan_source_database_pg_class,
    );

    // --- ProcessUtility dispatch arms (utility.c database globals) -----------
    utility_out_seams::createdb::set(createdb_arm);
    utility_out_seams::alter_database::set(alter_database_arm);
    utility_out_seams::alter_database_refresh_coll::set(alter_database_refresh_coll_arm);
    utility_out_seams::alter_database_set::set(alter_database_set_arm);
    utility_out_seams::drop_database::set(drop_database_arm);
