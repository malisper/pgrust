...
//! owner seam.
//!
//! The `def_get_string` / `def_get_boolean` seams ([`define_seams`])
...
pub fn defGetString<'mcx>(mcx: Mcx<'mcx>, def: &parsenodes::DefElem) -> PgResult<PgString<'mcx>> {
    ...
    match arg {
        ...
        Node::List(cells) => {
            ...
            catalog_namespace::NameListToString(mcx, &names)
        }
...
pub fn defGetTypeName(def: &parsenodes::DefElem) -> PgResult<TypeName> {
    ...
    match arg {
        ...
        Node::String(s) => {
            makefuncs_seams::make_type_name_from_name_list::call(list_make1_string(s))
        }
...
fn oidin(s: &str) -> PgResult<Oid> {
    let (value, _) = numutils::uint32in_subr(s, false, "oid", None)?;
    Ok(value as Oid)
}
