...
pub fn init_seams() {
    dropcmds_seams::remove_objects::set(remove_objects);
    ...
    utility_out_seams::remove_objects::set(remove_objects_seam);
}
...
fn remove_objects_seam(_mcx: Mcx<'_>, stmt: &::nodes::nodes::Node<'_>) -> PgResult<()> {
    ...
    for obj in ds.objects.iter() {
        objects.push(parse_type::rich_node_to_parse(obj)?);
    }
