...
pub fn event_trigger_sql_drop_add_object(
    ...
    let info = objectaddress_seams::event_trigger_describe_dropped_object::call(
        scratch.mcx(),
        object,
    )?;
    ...
    for snap in &snapshots {
        ...
        match &snap.address {
            Some(addr) => {
                ...
                let tag = utility_seams::create_command_tag::call(&snap.parsetree)?;
                let tagname = cmdtag::get_command_tag_name(tag.0);
...
/// promises through the [`event_trigger_get_event`](event_trigger_seams::event_trigger_get_event)
/// / [`event_trigger_get_tag_name`](event_trigger_seams::event_trigger_get_tag_name)
...
pub fn event_trigger_get_tag_name() -> Option<String> {
    CURRENT_EVENT_TRIGGER.with(|c| {
        c.borrow()
            .as_ref()
            .map(|d| cmdtag::get_command_tag_name(d.tag.0).to_string())
    })
}
...
fn event_trigger_get_tag(parsetree: &Node, event: EventTriggerEvent) -> PgResult<CommandTag> {
    if event == EventTriggerEvent::Login {
        ...
        Ok(CommandTag(cmdtag::get_command_tag_enum(b"LOGIN")))
    } else {
        utility_seams::create_command_tag::call(parsetree)
    }
}
...
fn filter_event_trigger(
    ...
    if !nodes_core::bitmapset::bms_is_empty(tagset)
        && !nodes_core::bitmapset::bms_is_member(tag.0, tagset)
    {
        return false;
    }
...
fn event_trigger_invoke(
    ...
    utility_out_seams::check_stack_depth::call()?;
    ...
    for &fnoid in fn_oid_list {
        ...
        let _ctx_guard = fmgr::fmgr::CallContextTagGuard::install(T_EVENT_TRIGGER_DATA);
        fmgr_seams::function_call_invoke::call(fnoid, InvalidOid, &[])?;
    }
...
fn event_trigger_login_setup(mcx: Mcx<'_>) -> PgResult<(Vec<Oid>, CommandTag)> {
    ...
    let tag = CommandTag(cmdtag::get_command_tag_enum(b"LOGIN"));
    ...
    if !runlist.is_empty() {
        ...
        if got {
            ...
            if recheck.is_empty() {
                ...
                pg_database_seams::reset_database_has_login_event_triggers::call(
                    setup_cxt.mcx(),
                )?;
            }
        }
...
pub fn event_trigger_collect_simple_command_opfamily(
    address: ObjectAddress,
    secondary_object: ObjectAddress,
    stmt: &opclass::CreateOpFamilyStmt,
) -> PgResult<()> {
    if !collecting() {
        return Ok(());
    }
...
pub fn event_trigger_collect_create_opclass(
    stmt: &opclass::CreateOpClassStmt,
    opclassoid: Oid,
    _operators: &[opclass::OpFamilyMember],
    _procedures: &[opclass::OpFamilyMember],
) -> PgResult<()> {
    if !collecting() {
        return Ok(());
    }
...
pub fn event_trigger_collect_alter_opfam(
    stmt: &opclass::AlterOpFamilyStmt,
    opfamilyoid: Oid,
    _operators: &[opclass::OpFamilyMember],
    _procedures: &[opclass::OpFamilyMember],
) -> PgResult<()> {
...
fn validate_ddl_tags(filtervar: &str, taglist: &[String]) -> PgResult<()> {
    for tag in taglist {
        let command_tag = cmdtag::get_command_tag_enum(tag.as_bytes());
        ...
        if !cmdtag::command_tag_event_trigger_ok(command_tag) {
            return Err(ereport(ERROR)
                .errcode(ERRCODE_FEATURE_NOT_SUPPORTED)
...
fn validate_table_rewrite_tags(_filtervar: &str, taglist: &[String]) -> PgResult<()> {
    for tag in taglist {
        let command_tag = cmdtag::get_command_tag_enum(tag.as_bytes());
        if !cmdtag::command_tag_table_rewrite_ok(command_tag) {
            return Err(ereport(ERROR)
                .errcode(ERRCODE_FEATURE_NOT_SUPPORTED)
                .errmsg(format!("event triggers are not supported for {tag}"))
                .into_error());
...
pub fn SetDatabaseHasLoginEventTriggers(mcx: Mcx<'_>) -> PgResult<()> {
    pg_database_seams::set_database_has_login_event_triggers::call(mcx)
}
...
pub fn init_seams() {
    ...
    tsearchcmds_seams::event_trigger_collect_alter_ts_config::set(
        event_trigger_collect_alter_ts_config,
    );

    utility_out_seams::event_trigger_begin_complete_query::set(
        event_trigger_begin_complete_query,
    );
    utility_out_seams::event_trigger_end_complete_query::set(
        event_trigger_end_complete_query,
    );
    utility_out_seams::event_trigger_inhibit_command_collection::set(
        event_trigger_inhibit_command_collection,
    );
    utility_out_seams::event_trigger_undo_inhibit_command_collection::set(
        event_trigger_undo_inhibit_command_collection,
    );
    utility_out_seams::event_trigger_ddl_command_start::set(|parsetree| {
        event_trigger_ddl_command_start(parsetree)
    });
    utility_out_seams::event_trigger_ddl_command_end::set(|parsetree| {
        event_trigger_ddl_command_end(parsetree)
    });
    utility_out_seams::event_trigger_sql_drop::set(|parsetree| {
        event_trigger_sql_drop(parsetree)
    });
    utility_out_seams::event_trigger_collect_simple_command::set(
        event_trigger_collect_simple_command,
    );
    utility_out_seams::event_trigger_collect_alter_def_privs::set(
        event_trigger_collect_alter_def_privs,
    );
    utility_out_seams::event_trigger_alter_table_start::set(
        event_trigger_alter_table_start,
    );
    utility_out_seams::event_trigger_alter_table_relid::set(
        event_trigger_alter_table_relid,
    );
    ...
    tablecmds_seams::event_trigger_alter_table_relid::set(|object_id| {
        event_trigger_alter_table_relid(object_id);
        Ok(())
    });
    utility_out_seams::event_trigger_alter_table_end::set(
        event_trigger_alter_table_end,
    );
    utility_out_seams::event_trigger_collect_alter_table_subcmd::set(
        event_trigger_collect_alter_table_subcmd,
    );
    utility_out_seams::event_trigger_supports_object_type::set(
        event_trigger_supports_object_type,
    );
    utility_out_seams::create_event_trigger::set(create_event_trigger_seam);
    utility_out_seams::alter_event_trigger::set(alter_event_trigger_seam);
    ...
    event_trigger_seams::event_trigger_get_event::set(|| {
        Ok(event_trigger_get_event())
    });
    event_trigger_seams::event_trigger_get_tag_name::set(|| {
    ...
    event_trigger_seams::trackDroppedObjectsNeeded::set(
    ...
    event_trigger_seams::EventTriggerSQLDropAddObject::set(
        event_trigger_sql_drop_add_object,
    );
    event_trigger_seams::EventTriggerSupportsObject::set(|object| {
    ...
    event_trigger_seams::get_event_trigger_oid::set(get_event_trigger_oid);
    ...
    event_trigger_seams::AlterEventTriggerOwner::set(|name, new_owner_id| {
        let scratch = MemoryContext::new("AlterEventTriggerOwner");
        AlterEventTriggerOwner(scratch.mcx(), name, new_owner_id)
    });
    event_trigger_seams::alter_event_trigger_owner_oid::set(
    ...
    event_trigger_seams::event_trigger_collect_simple_command_create_schema::set(
        event_trigger_collect_simple_command_create_schema,
    );
    event_trigger_seams::event_trigger_collect_simple_command_reindex::set(
        event_trigger_collect_simple_command_reindex,
    );
    event_trigger_seams::event_trigger_collect_simple_command_publication::set(
        event_trigger_collect_simple_command_publication,
    );
    event_trigger_seams::event_trigger_table_rewrite::set(
        event_trigger_table_rewrite,
    );
    event_trigger_seams::event_trigger_collect_create_opclass::set(
        event_trigger_collect_create_opclass,
    );
    event_trigger_seams::event_trigger_collect_alter_opfam::set(
