...
fn read_control_file_items(
    ...
    let mut variables: Vec<guc_file::ConfigVariable> = Vec::new();
...
pub fn parse_extension_control_file(
    ...
    for (name, value) in &items {
        match name.as_str() {
            ...
            "encoding" => {
                let enc = extra_encnames_fgram::pg_valid_server_encoding(value);
...
fn parse_bool_field(name: &str, value: &str) -> PgResult<bool> {
    match scalar_seams::parse_bool::call(value) {
        Some(b) => Ok(b),
...
pub fn init_seams() {
    use extension_seams as s;

    // ProcessUtilitySlow dispatch arm (utility.c CREATE EXTENSION).
    utility_out_seams::create_extension::set(create_extension_arm);
    ...
    guc_tables::vars::Extension_control_path.install(
    ...
    functioncmds_seams::extension_file_exists::set(|ext_name| {
