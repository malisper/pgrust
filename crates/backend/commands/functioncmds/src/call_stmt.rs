...
//!
//! Both bodies operate on the live, arena-lifetimed `nodes` CALL node:
...
pub fn ExecuteCallStmt<'mcx>(
     ...
     */
    let user_id = functioncmds_seams::get_user_id::call()?;
    let aclresult =
        functioncmds_seams::proc_aclcheck::call(funcid, user_id, ACL_EXECUTE)?;
    if aclresult != ACLCHECK_OK {
        let name =
            functioncmds_seams::get_func_name::call(funcid)?.unwrap_or_default();
        functioncmds_seams::aclcheck_error_procedure::call(aclresult, name)?;
    }
     ...
     */
    let proc = clauses_seams::get_func_form::call(funcid)?;
    ...
    objectaccess_seams::invoke_function_execute_hook::call(funcid)?;
     ...
     */
    let mut estate = execUtils_seams::create_executor_state::call(mcx)?;
    estate.es_param_list_info = params;
    let econtext = execUtils_seams::create_expr_context::call(&mut estate)?;
    ...
    for arg in fexpr.args.iter() {
        let mut exprstate =
            execExpr_seams::exec_prepare_expr::call(arg, &mut estate)?;
        let (val, isnull) =
    ...
    {
        let _ctx_guard =
            fmgr::fmgr::CallContextTagGuard::install_call(T_CALL_CONTEXT, call_atomic);
        let (r, n) = fmgr_seams::function_call_invoke_datum::call(
            mcx,
            funcid,
            inputcollid,
            &argvals,
    ...
    if funcresulttype == VOIDOID {
        ...
        let (tup_type, tup_typmod, values, nulls) =
            execTuples_seams::deform_record_datum::call(mcx, retval)?;
...
pub fn CallStmtResultDesc<'mcx>(mcx: Mcx<'mcx>, stmt_node: &Node<'mcx>) -> PgResult<TupleDesc<'mcx>> {
    ...
    let tupdesc =
        funcapi_seams::build_function_result_tupdesc_t::call(mcx, funcid)?;
    ...
    for i in 0..natts {
        ...
        let typ = nodes_core::nodefuncs::expr_type(outarg_expr)?;
        tupdesc_seams::tuple_desc_init_entry::call(
            &mut td,
            (i + 1) as i16,
            Some(&attname),
            typ,
...
// ===========================================================================
// Seam adapters (utility_out_seams::{execute_call_stmt,
// call_stmt_result_desc}).
// ===========================================================================

/// Seam adapter for `utility_out_seams::execute_call_stmt`. The
...
/// Seam adapter for `utility_out_seams::call_stmt_result_desc`.
