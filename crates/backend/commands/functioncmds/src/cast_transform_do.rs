...
pub fn CreateCast(stmt: &CreateCastStmt) -> PgResult<ObjectAddress> {
    ...
    if aclresult != ACLCHECK_OK {
        aclchk_seams::aclcheck_error_type::call(aclresult, sourcetypeid)?;
    }

    let aclresult = seam::type_aclcheck::call(targettypeid, seam::get_user_id::call()?, ACL_USAGE)?;
    if aclresult != ACLCHECK_OK {
        aclchk_seams::aclcheck_error_type::call(aclresult, targettypeid)?;
    }
...
pub fn CreateTransform(stmt: &CreateTransformStmt) -> PgResult<ObjectAddress> {
    ...
    if !seam::type_ownercheck::call(typeid, seam::get_user_id::call()?)? {
        aclchk_seams::aclcheck_error_type::call(ACLCHECK_NOT_OWNER, typeid)?;
    }

    let aclresult = seam::type_aclcheck::call(typeid, seam::get_user_id::call()?, ACL_USAGE)?;
    if aclresult != ACLCHECK_OK {
        aclchk_seams::aclcheck_error_type::call(aclresult, typeid)?;
    }

    /* Get the language */
    let lang = stmt.lang.clone().unwrap_or_default();
    let langid = proclang_seams::get_language_oid::call(&lang, false)?;
...
// ===========================================================================
// execute_do_stmt seam adapter (utility_out_seams::execute_do_stmt)
// ===========================================================================
//
...
// `parsenodes` shape, then calls the faithful body.
//
...
/// Lower one live `DefElem` node into the `parsenodes` model. DO only ever
...
fn lower_defelem(node: &nodes::nodes::Node<'_>) -> DefElem {
...
/// Seam adapter for `utility_out_seams::execute_do_stmt`. Lowers
/// the live `T_DoStmt` node into the `parsenodes` model and runs the
/// faithful [`ExecuteDoStmt`] body.
pub fn execute_do_stmt_seam<'mcx>(
    _mcx: Mcx<'mcx>,
    _pstate: &mut nodes::parsestmt::ParseState<'mcx>,
