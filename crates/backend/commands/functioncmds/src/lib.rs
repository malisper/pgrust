...
//! [`functioncmds_seams`] crate (NOT the monolithic
...
pub fn init_seams() {
    functioncmds_seams::get_transform_oid::set(
    ...
    // through the `utility_out_seams::execute_do_stmt` inward seam.
    utility_out_seams::execute_do_stmt::set(cast_transform_do::execute_do_stmt_seam);
    ...
    utility_out_seams::create_function::set(create_function_seam);
    utility_out_seams::create_cast::set(create_cast_seam);
    ...
    utility_out_seams::create_transform::set(create_transform_seam);
    ...
    utility_out_seams::alter_function::set(alter_function_seam);
    ...
    functioncmds_seams::aclcheck_error_language::set(aclcheck_error_language_seam);

    // CALL (functioncmds.c): `ExecuteCallStmt` is reached by
    // standard_ProcessUtility's T_CallStmt arm, and `CallStmtResultDesc` by
    // UtilityTupleDescriptor — both through the `utility_out_seams`
    // inward seams, operating on the live `T_CallStmt` node whose `funcexpr` /
    // `outargs` transformCallStmt (analyze.c) already populated.
    utility_out_seams::execute_call_stmt::set(call_stmt::execute_call_stmt_seam);
    utility_out_seams::call_stmt_result_desc::set(
...
/// `AlterFunctionStmt` into the flat [`parsenodes::AlterFunctionStmt`] and
...
fn alter_function_seam<'mcx>(
    ...
    let mut actions: Vec<parsenodes::Node> = Vec::with_capacity(afs.actions.len());
    for n in afs.actions.iter() {
        actions.push(rich_node_to_parse(n)?);
    }

    let pn = parsenodes::AlterFunctionStmt {
...
/// `CreateTransformStmt` into the flat [`parsenodes::CreateTransformStmt`]
...
fn create_transform_seam<'mcx>(
    ...
    let pn = parsenodes::CreateTransformStmt {
        replace: cts.replace,
        type_name,
...
/// `CreateFunctionStmt` into the flat [`parsenodes::CreateFunctionStmt`]
...
fn create_function_seam<'mcx>(
    ...
    let mut funcname: Vec<parsenodes::StringNode> =
        Vec::with_capacity(cfs.funcname.len());
    for n in cfs.funcname.iter() {
        match n.as_string() {
            Some(s) => funcname.push(parsenodes::StringNode {
    ...
    let mut parameters: Vec<parsenodes::Node> =
    ...
    let mut options: Vec<parsenodes::Node> =
    ...
    // the flat `parsenodes` vocabulary cannot represent. It is threaded
    ...
    // rich node instead.
    let sql_body_rich: Option<&nodes::nodes::Node<'mcx>> = cfs.sql_body.as_deref();

    let pn = parsenodes::CreateFunctionStmt {
    ...
    // `parsenodes` vocabulary can't hold); thread them as a side-channel
    // parallel to the flat `parameters`, exactly as `sql_body_rich` is threaded.
    let rich_parameters: Vec<&nodes::nodes::Node<'mcx>> =
...
/// [`parsenodes::CreateCastStmt`] and run the ported
...
fn create_cast_seam<'mcx>(
    ...
    let pn = parsenodes::CreateCastStmt {
        sourcetype,
        targettype,
        func,
        context: ccs.context,
...
use functioncmds_seams as _functioncmds_seams_dep;
#[allow(unused_imports)]
use seam_core as _seam_core_dep;
#[allow(unused_imports)]
use nodes as _types_nodes_dep;
