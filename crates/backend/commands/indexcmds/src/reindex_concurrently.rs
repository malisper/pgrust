...
use dependency_seams as dependency_seam;
use event_trigger_seams as event_trigger_seam;
...
pub(crate) fn ReindexRelationConcurrently<'mcx>(
    ...
    match relkind {
        ...
        RELKIND_INDEX => {
            let heap_id = index::IndexGetRelation(
                relation_oid,
    ...
    for idx in index_ids.iter_mut() {
        ...
        let new_index_id = index::index_concurrently_create_copy(
            mcx,
            &heap_rel,
            idx.index_id,
            tablespaceid,
    ...
    for newidx in new_index_ids.iter() {
        ...
        index::index_concurrently_build(mcx, newidx.table_id, newidx.index_id)?;
    ...
    for oldidx in index_ids.iter() {
        ...
        index::index_concurrently_set_dead(mcx, oldidx.table_id, oldidx.index_id)?;
...
fn get_index_isvalid(_mcx: Mcx<'_>, indexoid: Oid) -> PgResult<bool> {
    Ok(tablecmds_seams::get_index_isvalid::call(indexoid)?.unwrap_or(false))
}
