...
pub fn init_seams() {
    lockcmds_seams::lock_table_command::set(lock_table_command);
    // The tcop/utility.c dispatch routes LOCK TABLE through its own outward
    // seam; install the real arm here (the owner crate).
    utility_out_seams::lock_table_command::set(lock_table_command_arm);
}
...
pub fn lock_table_command(lockstmt: &LockStmt) -> PgResult<()> {
    ...
    for p in lockstmt.relations.iter() {
        ...
        let reloid: Oid = catalog_namespace::RangeVarGetRelidExtended(
            mcx,
            &rv,
            lockstmt.mode,
            flags,
...
/// error.  Because the [`nodes_core`] walker callback can only return
...
/// `bool`-returning [`nodes_core`] walker.
///
