...
use createas_seams as createas_seam;
use explain_seams as explain_seam;
use execExpr_seams as execexpr_seam;
use params_seams as params_seam;
use parser_analyze_seams as analyze_seam;
use parse_expr_seams as parseexpr_seam;
use parse_type_seams as parsetype_seam;
use pquery_seams as pquery_seam;
use utility_seams as utility_seam;
use arrayfuncs_seams as arrayfuncs_seam;
use format_type_seams as formattype_seam;
use varlena_seams as varlena_seam;
use plancache_seams as plancache_seam;
use funcapi_seams as funcapi_seam;
use portalmem_seams as portal_seam;
use resowner_seams as resowner_seam;
...
pub fn ExecuteQuery<'mcx>(
    ...
    // token (`types_core`/`nodes` -> `portal`); same underlying
...
/// `ExecCreateTableAs` through the `createas_seams::execute_query`
...
pub fn init_seams() {
    utility_out_seams::prepare_query::set(prepare_query_arm);
    utility_out_seams::execute_query::set(execute_query_arm);
    utility_out_seams::deallocate_query::set(deallocate_query_arm);
    utility_out_seams::execute_stmt_has_result::set(execute_stmt_has_result_arm);
    utility_out_seams::execute_stmt_result_desc::set(execute_stmt_result_desc_arm);
