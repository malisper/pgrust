...
fn rel_name(rel: &rel::Relation<'_>) -> alloc::string::String {
    rel.name().to_string()
}
...
pub fn CreateStatistics<'mcx>(
    ...
    for rln in stmt.relations.iter() {
            ...
            && relkind != RELKIND_PARTITIONED_TABLE
        {
            ...
            let detail =
                pg_class::errdetail_relkind_not_supported(relkind)?;
            return Err(ereport(ERROR)
...
pub fn AlterStatistics<'mcx>(mcx: Mcx<'mcx>, stmt: &AlterStatsStmt<'_>) -> PgResult<ObjectAddress> {
    ...
    stxoid = catalog_namespace::get_statistics_object_oid(mcx, &defnames, stmt.missing_ok)?;
    ...
    if stxoid == 0 {
        ...
        let (schemaname, statname) =
            catalog_namespace::DeconstructQualifiedName(mcx, &defnames)?;
...
fn range_var_get_relid_share_update_arm<'mcx>(
    ...
    catalog_namespace::RangeVarGetRelid(mcx, &rv, ShareUpdateExclusiveLock, false)
}

/// `statscmds.c` owns the `RemoveStatisticsById` inward seam (dependency.c's
/// `doDeletion` for `OCLASS_STATISTIC_EXT`).  Installed here.
pub fn init_seams() {
    statscmds_seams::RemoveStatisticsById::set(remove_statistics_by_id_seam);

    // ProcessUtilitySlow dispatch arms (utility.c CREATE/ALTER STATISTICS).
    utility_out_seams::alter_statistics::set(alter_statistics_arm);
    utility_out_seams::create_statistics::set(create_statistics_arm);
    utility_out_seams::create_statistics_rebuild::set(create_statistics_rebuild_arm);
    utility_out_seams::range_var_get_relid_share_update::set(
