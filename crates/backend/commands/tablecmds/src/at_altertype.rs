    ...
    loop {
        match cur {
            ...
            Expr::CoerceToDomain(d) => {
                if typcache_seams::domain_has_constraints::call(d.resulttype)? {
                    return Ok(true);
                }
            ...
            Expr::FuncExpr(f) => {
                ...
                match f.funcid {
                    F_TIMESTAMPTZ_TIMESTAMP | F_TIMESTAMP_TIMESTAMPTZ => {
                        if timestamp_seams::timestamp_timestamptz_requires_rewrite::call() {
                            return Ok(true);
                        }
...
pub fn ATPrepAlterColumnType<'mcx>(
    ...
    // Don't alter columns used in the partition key.
    {
        ...
        let (is_part_attr, _is_expr) =
            partition_seams::has_partition_attrs::call(mcx, rel, Some(&singleton))?;
        if is_part_attr {
            return utils_error::ereport(ERROR)
                .errcode(ERRCODE_INVALID_TABLE_DEFINITION)
                .errmsg(format!(
    ...
    if attgenerated == ATTRIBUTE_GENERATED_VIRTUAL {
        ...
        let planned = planner::expression_planner(mcx, transform2)?;
...
fn feature_not_supported<'mcx>(
    ...
    let obj_desc =
        objectaddress_seams::get_object_description::call(mcx, found_object, false)?
            .map(|s| s.to_string())
            .unwrap_or_default();
...
fn RememberReplicaIdentityForRebuilding<'mcx>(
    ...
    let name = lsyscache_seams::get_rel_name::call(mcx, indoid)?;
...
pub(crate) fn ATPostAlterTypeCleanup<'mcx>(
    ...
    for (old_id, defstr) in con_pairs.into_iter() {
        ...
        if OidIsValid(conform.conrelid) {
            ...
            let base = lsyscache_seams::get_base_type::call(conform.contypid)?;
            con_relid = lsyscache_seams::get_typ_typrelid::call(base)?;
        }
    ...
    for (old_id, defstr) in idx_pairs.into_iter() {
        let idx_relid = index::IndexGetRelation(old_id, false)?;
...
fn ATPostAlterTypeParse<'mcx>(
    ...
    for stm in querytree_list.iter() {
        ...
        if stm_tag == ntag::T_IndexStmt {
            ...
            istmt.idxcomment = match tablecmds_seams::get_comment::call(
                mcx,
                old_id,
                RelationRelationId,
                0,
            ...
            for subcmd_node in atstmt.cmds.iter() {
                ...
                match subcmd.subtype {
                    AlterTableType::AT_AddIndex => {
                        ...
                        indstmt.idxcomment =
                            match tablecmds_seams::get_comment::call(
                                mcx,
                                indoid,
                                RelationRelationId,
                                0,
            ...
            let mut stmt = stmt;
            stmt.stxcomment = match tablecmds_seams::get_comment::call(
                mcx,
                old_id,
...
fn RebuildConstraintComment<'mcx>(
    ...
    let comment_str = tablecmds_seams::get_comment::call(
        mcx,
        objid,
...
pub fn ATExecAlterColumnType<'mcx>(
    ...
    let parse_type_name = parse_type::raw_typename_to_parse(type_name)?;
    ...
    if atthasmissing {
        ...
        if !missing_null {
            ...
            let new_arr = arrayfuncs_seams::construct_array_values_bytes::call(
                mcx,
...
fn format_type_be<'mcx>(mcx: Mcx<'mcx>, typid: Oid) -> PgResult<String> {
    Ok(adt_format_type::format_type_be(mcx, typid)?
        .as_str()
        .to_string())
}
...
fn format_type_with_typemod<'mcx>(mcx: Mcx<'mcx>, typid: Oid, typmod: i32) -> PgResult<String> {
    Ok(
        adt_format_type::format_type_with_typemod(mcx, typid, typmod)?
            .as_str()
            .to_string(),
    )
}
...
fn quote_identifier<'mcx>(mcx: Mcx<'mcx>, ident: &str) -> PgResult<String> {
    Ok(ruleutils::quote_identifier(mcx, ident)?
        .as_str()
        .to_string())
}
