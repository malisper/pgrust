...
pub(crate) fn ATExecAttachPartition<'mcx>(
    ...
    let parent_partdesc = partdesc::RelationGetPartitionDesc(mcx, rel, true)?;
    ...
    let attach_reloftype =
        syscache_seams::search_relation_reloftype::call(attachrel.rd_id)?
            .unwrap_or(0);
    ...
    if pg_inherits::has_superclass(attachrel.rd_id)? {
        return Err(ereport(ERROR)
            .errcode(ERRCODE_WRONG_OBJECT_TYPE)
    ...
    if attachrel.rd_rel.relkind == RELKIND_RELATION
        && pg_inherits::has_subclass_scan(attachrel.rd_id)?
    {
        return Err(ereport(ERROR)
            .errcode(ERRCODE_WRONG_OBJECT_TYPE)
    ...
    let (attachrel_children, _) =
        pg_inherits::find_all_inheritors(mcx, attachrel.rd_id, AccessExclusiveLock, false)?;
    ...
    if attachrel.rd_rel.relpersistence == RELPERSISTENCE_TEMP
        && !relcache_seams::rd_islocaltemp::call(&attachrel)?
    {
        return Err(ereport(ERROR)
            .errcode(ERRCODE_WRONG_OBJECT_TYPE)
            .errmsg(
    ...
    heap::StorePartitionBound(mcx, &attachrel, rel, bound)?;
    ...
    let qual_partdesc = if bound.is_default {
        Some(partdesc::RelationGetPartitionDesc(mcx, rel, false)?)
    } else {
        None
    };
    let part_bound_constraint =
        partbounds_seams::get_qual_from_partbound::call(
            mcx,
            rel.rd_id,
            &key,
            bound,
    ...
    let parent_qual = partcache::RelationGetPartitionQual(mcx, rel)?;
    ...
    if !part_constraint.is_empty() {
        ...
        let mapped =
            catalog_partition::map_partition_varattnos(mcx, single, 1, &attachrel, rel)?;
    ...
    if let Some(default_rel) = default_rel.as_ref() {
        ...
        let def_part_constraint =
            catalog_partition::get_proposed_default_constraint(mcx, pbc_copy)?;

        // Map from rel's attnos to defaultrel's.
        let mapped = catalog_partition::map_partition_varattnos(
            mcx,
            def_part_constraint,
            1,
            default_rel,
...
fn eval_const_expressions_list<'mcx>(
    ...
    for n in list.into_iter() {
        ...
        out.push(clauses::eval_const_expressions(mcx, e)?);
    }
    Ok(out)
}
...
fn AttachPartitionEnsureIndexes<'mcx>(
    ...
    let attach_rel_idxs =
        relcache::derived::RelationGetIndexList(attachrel.rd_id)?;
    ...
    for &cld_idx_id in attach_rel_idxs.iter() {
        let cld = indexam_seams::index_open::call(
            mcx,
            cld_idx_id,
            AccessShareLock,
        )?;
        let info = index::BuildIndexInfo(mcx, &cld)?;
    ...
    if attachrel.rd_rel.relkind == RELKIND_FOREIGN_TABLE {
        for &idx in idxes.iter() {
            let idx_rel =
                indexam_seams::index_open::call(mcx, idx, AccessShareLock)?;
    ...
    for &idx in idxes.iter() {
        ...
        let info = index::BuildIndexInfo(mcx, &idx_rel)?;
        ...
        for i in 0..attachrel_idx_rels.len() {
            ...
            if index::CompareIndexInfo(
                ...
                if OidIsValid(constraint_oid) {
                    ...
                    if lsyscache::collation_constraint_language_cast::get_constraint_type(constraint_oid)?
                        != lsyscache::collation_constraint_language_cast::get_constraint_type(cld_constr_oid)?
                    {
                        continue;
                    }
                }
        ...
        if !found {
            let (stmt, con_oid) =
                parse_utilcmd_seams::generateClonedIndexStmt::call(
            ...
            indexcmds_seams::define_index_full::call(mcx, args)?;
        }
...
pub(crate) fn CloneRowTriggersToPartition<'mcx>(
    ...
    let trigs = genam_seams::relcache_scan_pg_trigger::call(parent.rd_id)?;
    ...
    for trig in &trigs {
        ...
        let qual: Option<Node<'mcx>> = match &trig.tgqual {
            ...
            Some(qual_text) => {
                ...
                let node = read_seams::string_to_node::call(mcx, qual_text.as_str())?;
                ...
                let mut exprs = partition_seams::map_partition_varattnos::call(
                    mcx,
                    exprs,
                    PRS2_NEW_VARNO,
                    partition,
        ...
        commands_trigger::create::CreateTriggerFiringOn(
            mcx,
            &trig_stmt,
            "",
            partition.rd_id,
...
fn FindTriggerIncompatibleWithInheritance<'mcx>(
    ...
    let trigs =
        genam_seams::relcache_scan_pg_trigger::call(attachrel.rd_id)?;
    for trig in &trigs {
        if !pt::TRIGGER_FOR_ROW(trig.tgtype) {
            continue;
        }
...
fn CreateInheritance<'mcx>(
    ...
    let inhseqno = pg_inherits::next_inheritance_seqno_checked(
        mcx,
        child_rel.rd_id,
        parent_rel.rd_id,
...
fn StoreCatalogInheritance1<'mcx>(
    ...
    pg_inherits::StoreSingleInheritance(relation_id, parent_oid, seq_number)?;
...
fn MergeAttributesIntoExisting<'mcx>(
    ...
    for parent_attno in 1..=parent_desc.natts {
        ...
        if parent_att.attnotnull && !child_attnotnull {
            let contup =
                pg_constraint::findNotNullConstraintAttnum(mcx, parent_rel.rd_id, parent_att.attnum)?;
            let connoinherit = match &contup {
                Some(t) => pg_constraint::constraint_connoinherit(mcx, t)?,
...
fn MergeConstraintsIntoExisting<'mcx>(
    ...
    // genuine no-op.
    pg_constraint::merge_constraints_into_existing(
        mcx,
        child_rel,
        parent_rel,
    )
...
fn QueuePartitionConstraintValidation<'mcx>(
    ...
    if scanrel.rd_rel.relkind == RELKIND_RELATION {
        ...
        let partdesc = partdesc::RelationGetPartitionDesc(mcx, scanrel, true)?;
        for &part_oid in partdesc.oids.iter() {
            ...
            let this_part_constraint = catalog_partition::map_partition_varattnos(
                mcx,
                this_copy,
                1,
                &part_rel,
...
fn ConstraintImpliedByRelConstraint<'mcx>(
    ...
    if let Some(constr) = scanrel.rd_att.constr.as_ref() {
        for check in constr.check.iter() {
            ...
            let cexpr =
                prepqual_seams::canonicalize_qual::call(mcx, Some(cexpr), true)?;
    ...
    predtest_seams::predicate_implied_by_exprs::call(
        mcx,
        &test_exprs,
        &exist_constraint,
        true,
