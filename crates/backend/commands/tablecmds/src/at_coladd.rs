...
pub fn ATParseTransformCmd<'mcx>(
    ...
    let nspname = lsyscache::namespace_range_index_pubsub::get_namespace_name(
        mcx,
    ...
    for stmt in before_stmts.into_iter() {
        utility_out_seams::process_utility_wrapper::call(
            mcx,
            &stmt,
            query_string,
            -1,
...
pub fn ATExecAddColumn<'mcx>(
    ...
    if col_def.inhcount > 0 {
            ...
            )?
        {
            ...
            if ccollid != childatt_collation {
                let n1 = lsyscache::collation_constraint_language_cast::get_collation_name(mcx, ccollid)?;
                let n2 = lsyscache::collation_constraint_language_cast::get_collation_name(mcx, childatt_collation)?;
    ...
    let Some((reltid, mut relform)) =
        syscache_seams::search_syscache_copy_pg_class::call(mcx, myrelid)?
    else {
        return Err(utils_error::PgError::error(format!(
    ...
    let reltype = lsyscache::relation::get_rel_type_id(myrelid)?;
    ...
    if RELKIND_HAS_STORAGE(relkind) {
        ...
        let mut defval = if col_def.identity != 0 {
            ...
            let seqid = catalog_namespace::RangeVarGetRelid(
                mcx,
                &identity_seq_access,
                NoLock,
                false,
            ...
            let fresh_rel = relation_open(mcx, myrelid, NoLock)?;
            let dv = rewritehandler_seams::build_column_default::call(
                mcx,
                fresh_rel.alias(),
                newattnum as i32,
            )?;
        ...
        let has_domain_constraints =
            typcache_seams::domain_has_constraints::call(attribute_typid)?;
        if defval.is_none() && has_domain_constraints {
            ...
            let (base_type_id, base_type_mod) =
                lsyscache_seams::get_base_type_and_typmod::call(
                    attribute_typid,
                )?;
            let base_type_coll =
                lsyscache_seams::get_typcollation::call(base_type_id)?;
            let null_const = nodes_core::makefuncs::make_null_const(
                mcx,
                base_type_id,
                base_type_mod,
                base_type_coll,
        ...
        if let Some(dv) = defval.take() {
            ...
            if relkind == RELKIND_RELATION
                ...
                && !clauses::contain_volatile_functions(Some(&planned))?
            {
                ...
                //                             &missingIsNull);
                let mut estate =
                    execExpr_seams::create_executor_state::call(mcx)?;
                let mut expr_state =
                    execExpr_seams::exec_prepare_expr::call(&planned, &mut estate)?;
                let econtext =
                    execUtils_seams::get_per_tuple_expr_context::call(
                        &mut estate,
                    )?;
