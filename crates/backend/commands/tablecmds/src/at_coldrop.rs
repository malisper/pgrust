...
/// `bms_make_singleton(x)` (bitmapset.c) in the `pathnodes::Bitmapset`
/// (`bitmapword[]`) layout `has_partition_attrs` consumes.
pub(crate) fn bms_make_singleton(x: i32) -> pathnodes::Bitmapset {
    ...
    if !children.is_empty() {
        ...
        for &childrelid in children.iter() {
            ...
            let tuple = SearchSysCacheAttName(mcx, childrelid, colName)?.ok_or_else(|| {
                utils_error::PgError::error(format!(
