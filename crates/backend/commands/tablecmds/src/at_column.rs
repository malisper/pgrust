...
fn relation_mark_replica_identity<'mcx>(
    ...
    let index_list = relcache_seams::relation_get_index_list::call(mcx, rel)?;
...
pub fn ATExecReplicaIdentity<'mcx>(
    ...
    let index_oid = lsyscache_seams::get_relname_relid::call(
        index_name,
    ...
    let info = relcache_seams::get_replident_index_info::call(index_oid)?;
...
pub fn ATExecColumnDefault<'mcx>(
    ...
    if att.attidentity != 0 {
        ...
        let mut b = utils_error::ereport(ERROR)
...
pub fn ATExecAddOf<'mcx>(
    ...
    let parse_type_name = parse_type::raw_typename_to_parse(of_typename)?;
    let (typeform, _typmod) =
        parse_type::typenameType(mcx, None, &parse_type_name)?;
    ...
    if typeform.typtype == TYPTYPE_COMPOSITE {
        ...
        table::table_close(type_relation, NoLock)?;
        if !type_ok {
            return utils_error::ereport(ERROR)
                .errcode(ERRCODE_WRONG_OBJECT_TYPE)
                .errmsg(format!(
                    "type {} is the row type of another table",
                    format_type_seams::format_type_be::call(mcx, typeid)?
                        .as_str()
                ))
                .errdetail(
    ...
    if pg_inherits_seams::has_superclass::call(relid)? {
        return utils_error::ereport(ERROR)
            .errcode(ERRCODE_WRONG_OBJECT_TYPE)
    ...
    let type_tupdesc =
        typcache_seams::lookup_rowtype_tupdesc::call(mcx, typeid, -1)?;
...
pub fn ATExecSetOptions<'mcx>(
    ...
    let new_options = common_reloptions::transformRelOptionsBytes(
        mcx,
        old_bytes,
        &def_list,
        None,
    ...
    common_reloptions::attribute_reloptions(mcx, new_options.as_deref(), true)?;
...
pub fn ATExecAlterColumnGenericOptions<'mcx>(
    ...
    use foreigncmds_seams as fc_seam;
            ...
            for it in items.iter() {
                if let Some(de) = it.as_defelem() {
                    let name = de.defname.as_ref().map(|s| s.as_str().to_string()).ok_or_else(
                        || utils_error::PgError::error("DefElem has no defname"),
                    )?;
    ...
    let old_options: Vec<(String, Option<String>)> = if cur_isnull {
        ...
        common_reloptions::untransformRelOptions(mcx, Some(cur_datum.as_ref_bytes()))?
    };
...
fn SetIndexStorageProperties<'mcx>(
    ...
    for indexoid in index_list.iter().copied() {
        ...
        let indkey = relcache_seams::rd_index_indkey::call(&indrel)?
            .unwrap_or_default();
        let indnatts =
            relcache_seams::rd_index_indnatts::call(&indrel)?.unwrap_or(0)
                as usize;
...
pub fn ATExecClusterOn<'mcx>(
    ...
    cluster_seams::check_index_is_clusterable::call(mcx, rel, index_oid, lockmode)?;

    // And do the work.
    cluster_seams::mark_index_clustered::call(mcx, rel, index_oid, false)?;
...
fn validate_setrel_options(
    ...
    if relkind == RELKIND_RELATION || relkind == RELKIND_MATVIEW {
        common_reloptions::heap_reloptions(mcx, relkind, new_options, true)?;
    } else if relkind == RELKIND_PARTITIONED_TABLE {
        common_reloptions::partitioned_table_reloptions(new_options, true)?;
    } else if relkind == RELKIND_VIEW {
        common_reloptions::view_reloptions(mcx, new_options, true)?;
    } else if relkind == RELKIND_INDEX || relkind == RELKIND_PARTITIONED_INDEX {
        common_reloptions::index_reloptions(mcx, amhandler, new_options, true)?;
    } else {
...
pub fn ATExecSetRelOptions<'mcx>(
    mcx: Mcx<'mcx>,
    rel: &Relation<'mcx>,
    def_list: Vec<common_reloptions::DefElem>,
    ...
    let old_bytes: Option<Vec<u8>> = if operation == AlterTableType::AT_ReplaceRelOptions {
        ...
        let tok = syscache_seams::fetch_class_reloptions::call(mcx, relid)?;
        if tok.is_null {
            None
        } else {
            Some(tok.bytes)
    ...
    if types_core::OidIsValid(toastid) {
        ...
        let toast_old: Option<Vec<u8>> = if operation == AlterTableType::AT_ReplaceRelOptions {
            ...
            let tok = syscache_seams::fetch_class_reloptions::call(mcx, toastid)?;
            if tok.is_null {
                None
            } else {
                Some(tok.bytes)
        ...
        let toast_new = common_reloptions::transformRelOptionsBytes(
            mcx,
            toast_old.as_deref(),
            &def_list,
            Some("toast"),
