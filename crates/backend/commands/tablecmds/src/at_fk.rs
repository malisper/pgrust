...
fn transformColumnNameList(
    ...
    for col in col_list.iter() {
        ...
        let att = lsyscache::attribute::get_attnum(rel_id, attname)?;
        ...
        if let Some(t) = atttypids.as_deref_mut() {
            t[attnum] = lsyscache::attribute::get_atttype(rel_id, att)?;
        }
        if let Some(c) = attcollids.as_deref_mut() {
            let (_typ, _typmod, coll) =
                lsyscache::attribute::get_atttypetypmodcoll(rel_id, att)?;
            c[attnum] = coll;
        }
        attnum += 1;
    }
...
fn transformFkeyGetPrimaryKey<'mcx>(
    ...
    *index_oid = InvalidOid;
    ...
    let indexoidlist =
        relcache::derived::RelationGetIndexList(pkrel.rd_id)?;

    let mut pk_info: Option<cache::PgIndexInfo<'mcx>> = None;
    for &indexoid in indexoidlist.iter() {
        let info = syscache_seams::search_pg_index_info::call(mcx, indexoid)?
            .ok_or_else(|| {
    ...
    for k in 0..i {
        ...
        attnums[k] = pkattno;
        atttypids[k] = parser_relation::attnumTypeId(pkrel, pkattno as i32)?;
        attcollids[k] = parser_relation::attnumCollationId(pkrel, pkattno as i32)?;
        opclasses[k] = info.indclass[k];
        let name = parser_relation::attnumAttName(mcx, pkrel, pkattno as i32)?;
...
fn transformFkeyCheckAttrs<'mcx>(
    ...
    for &cand in indexoidlist.iter() {
        ...
        let has_pred =
            syscache_seams::pg_index_has_predicate::call(indexoid)?
                .unwrap_or(false);
        let (_tid, has_exprs) =
            syscache_seams::pg_index_tid_and_hasexprs::call(indexoid)?
...
fn checkFkeyPermissions(rel: &Relation<'_>, attnums: &[AttrNumber], natts: i32) -> PgResult<()> {
    ...
    let r = aclchk_seams::pg_class_aclcheck::call(rel.rd_id, roleid, ACL_REFERENCES)?;
    if r == ACLCHECK_OK {
        return Ok(());
    }
    // Else we must have REFERENCES on each column.
    for &att in attnums.iter().take(natts as usize) {
        let r = aclchk_seams::pg_attribute_aclcheck::call(
...
pub fn ATAddForeignKeyConstraint<'mcx>(
    ...
    let pkrel = if OidIsValid(fkconstraint.old_pktable_oid) {
        ...
        table::table_openrv(mcx, &to_access_range_var(rv), ShareRowExclusiveLock)?
    };
    ...
    if !tablespace_globals_seams::allowSystemTableMods::call()?
        && catalog_catalog::IsSystemRelation(&pkrel)
    {
        return Err(utils_error::ereport(ERROR)
    ...
    match rel.rd_rel.relpersistence {
        ...
        RELPERSISTENCE_TEMP => {
            ...
            if tablecmds_seams::relation_is_other_temp::call(&pkrel)?
                || tablecmds_seams::relation_is_other_temp::call(rel)?
            {
    ...
    for i in 0..numpks as usize {
        ...
        let amid = lsyscache::opclass::get_opclass_method(opclasses[i])?;
        let (opfamily, opcintype) =
            lsyscache::opclass::get_opclass_opfamily_and_input_type(
                opclasses[i],
            )?
            .ok_or_else(|| {
                utils_error::ereport(ERROR)
        ...
        let eqstrategy = amapi_seams::index_am_translate_cmptype::call(
            cmptype, amid, opfamily, true,
        )?;
        if eqstrategy == InvalidStrategy {
            let amname = lsyscache::namespace_range_index_pubsub::get_am_name(mcx, amid)?
                .map(|s| s.as_str().to_string())
                .unwrap_or_default();
            let opfname = lsyscache::opclass::get_opfamily_name(mcx, opfamily, false)?
                .map(|s| s.as_str().to_string())
                .unwrap_or_default();
            return Err(utils_error::ereport(ERROR)
                .errcode(ERRCODE_UNDEFINED_OBJECT)
        ...
        // ppeqop: PK = PK.
        let ppeqop = lsyscache_seams::get_opfamily_member::call(
        ...
        let fktyped = lsyscache_seams::get_base_type::call(fktype)?;
        let mut pfeqop = lsyscache_seams::get_opfamily_member::call(
        ...
        if OidIsValid(pfeqop) {
            ...
            ffeqop = lsyscache_seams::get_opfamily_member::call(
        ...
        if OidIsValid(pkcoll) && OidIsValid(fkcoll) {
            let pkcolldet =
                lsyscache::collation_constraint_language_cast::get_collation_isdeterministic(pkcoll)?;
            let fkcolldet =
                lsyscache::collation_constraint_language_cast::get_collation_isdeterministic(fkcoll)?;
    ...
    if with_period {
        let _ = pg_constraint_seams::find_fk_period_opers::call(
...
fn addFkRecurseReferenced<'mcx>(
    ...
    if pkrel.rd_rel.relkind == RELKIND_PARTITIONED_TABLE {
        let pd = partdesc::RelationGetPartitionDesc(mcx, pkrel, true)?;

        for i in 0..pd.nparts as usize {
            // XXX would it be better to acquire these locks beforehand?
            let part_rel = table_seams::table_open::call(
                mcx,
                pd.oids[i],
                ShareRowExclusiveLock,
            )?;
            ...
            let part_index_id =
                catalog_partition::index_get_partition(&part_rel, index_oid)?;
...
fn addFkRecurseReferencing<'mcx>(
    ...
    if rel.rd_rel.relkind == RELKIND_RELATION {
        ...
        if !old_check_ok && !fkconstraint.skip_validation && fkconstraint.is_enforced {
            ...
            let name = lsyscache::collation_constraint_language_cast::get_constraint_name(
                mcx, parent_constr,
            )?;
        ...
        for i in 0..pd.nparts as usize {
            let partition = table_seams::table_open::call(
                mcx,
                pd.oids[i],
                lockmode,
            )?;
            ...
            let part_fks = relcache::derived::RelationGetFKeyList(partition.rd_id)?;
...
fn CreateFKCheckTrigger<'mcx>(
    ...
    let trig_address = commands_trigger::create::CreateTrigger(
        mcx,
        &fk_trigger,
        "",
        my_rel_oid,
...
pub(crate) fn createForeignKeyActionTriggers<'mcx>(
    ...
    let del_addr = commands_trigger::create::CreateTrigger(
    ...
    let upd_addr = commands_trigger::create::CreateTrigger(
...
pub(crate) fn DetachPartitionForeignKeys<'mcx>(
    ...
    let fks = relcache::derived::RelationGetFKeyList(part_rel.rd_id)?;
    if fks.is_empty() {
        return Ok(());
    }
    ...
    for fk in fks.iter() {
        ...
        if fk.conenforced {
            ...
            commands_trigger::set_parent::TriggerSetParentTrigger(
                mcx,
                &trigrel,
                insert_trigger_oid,
                InvalidOid,
        ...
        {
            let tuple = syscache_seams::search_constraint_tuple_by_oid::call(
                mcx, fk.conoid,
            )?
            .ok_or_else(|| {
                utils_error::ereport(ERROR)
            ...
            let arrays = pg_constraint::DeconstructFkConstraintRow(
            ...
            let refd_rel = table_seams::table_open::call(
                mcx,
                fk.confrelid,
                ShareRowExclusiveLock,
            )?;
...
fn tryAttachPartitionForeignKey<'mcx>(
    mcx: Mcx<'mcx>,
    wqueue: &mut PgVec<'mcx, AlteredTableInfo<'mcx>>,
    fk: &relcache::derived::ForeignKeyCacheInfo,
    ...
    let parent_constr =
        syscache_seams::search_constraint_form_by_oid::call(parent_constr_oid)?
            .ok_or_else(|| {
                utils_error::ereport(ERROR)
                    .errmsg_internal(format!(
    ...
    let part_constr =
        syscache_seams::search_constraint_form_by_oid::call(fk.conoid)?
            .ok_or_else(|| {
                utils_error::ereport(ERROR)
                    .errmsg_internal(format!("cache lookup failed for constraint {}", fk.conoid))
                    .into_error()
...
fn AttachPartitionForeignKey<'mcx>(
    ...
    let part_constr =
        syscache_seams::search_constraint_form_by_oid::call(part_constr_oid)?
            .ok_or_else(|| {
                utils_error::ereport(ERROR)
    ...
    // needed.  Remove those.
    if lsyscache_seams::get_rel_relkind::call(part_constr_frelid)?
...
fn CloneFkReferenced<'mcx>(
    ...
    for &constr_oid in clone.iter() {
        ...
        let fk_rel = table_seams::table_open::call(
            mcx,
        ...
        let fk = pg_constraint::DeconstructFkConstraintRow(
            mcx, &tuple, true, true, true, true,
        )?;
        ...
        let part_index_id = catalog_partition::index_get_partition(partition_rel, index_oid)?;
        if !OidIsValid(part_index_id) {
            return Err(utils_error::ereport(ERROR)
                .errmsg_internal(format!(
                    "index for {index_oid} not found in partition {}",
...
fn CloneFkReferencing<'mcx>(
    ...
    let parent_fks =
        relcache::derived::RelationGetFKeyList(parent_rel.rd_id)?;
    let mut clone: Vec<Oid> = Vec::new();
    ...
    let part_fks = relcache::derived::RelationGetFKeyList(part_rel.rd_id)?;
    ...
    for &parent_constr_oid in clone.iter() {
        ...
        // all partitions.
        let pkrel = table_seams::table_open::call(
            mcx,
            constr_form.confrelid,
            ShareRowExclusiveLock,
        )?;
        if pkrel.rd_rel.relkind == RELKIND_PARTITIONED_TABLE {
            let _ = pg_inherits::find_all_inheritors(
                mcx,
                pkrel.rd_id,
                ShareRowExclusiveLock,
                false,
...
fn format_type<'mcx>(mcx: Mcx<'mcx>, oid: Oid) -> PgResult<String> {
    Ok(adt_format_type::format_type_be(mcx, oid)?.as_str().to_string())
}

fn coll_name<'mcx>(mcx: Mcx<'mcx>, oid: Oid) -> PgResult<String> {
    Ok(lsyscache::collation_constraint_language_cast::get_collation_name(mcx, oid)?
        .map(|s| s.as_str().to_string())
        .unwrap_or_default())
}
...
fn collation_is_det(oid: Oid) -> PgResult<bool> {
    ...
    lsyscache::collation_constraint_language_cast::get_collation_isdeterministic(oid)
}
