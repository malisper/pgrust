        ...
        if let Some(items) = def.as_list() {
            for it in items.iter() {
                if let Some(de) = it.as_defelem() {
                    ...
                    out.push(common_reloptions::DefElem::new(
...
pub(crate) fn ATCheckPartitionsNotInUse<'mcx>(
    ...
    if rel.rd_rel.relkind == RELKIND_PARTITIONED_TABLE {
        ...
        let (inh, _) =
            pg_inherits::find_all_inheritors(mcx, rel.rd_id, lockmode, false)?;
...
pub(crate) fn RangeVarCallbackForAlterRelation<'mcx>(
    ...
    if !aclchk_seam::object_ownercheck::call(RelationRelationId, relid, GetUserId())? {
        let actual_kind = lsyscache_seams::get_rel_relkind::call(relid)?;
    ...
    if !tablespace_globals_seams::allowSystemTableMods::call()?
    ...
    for lcmd in cmds.iter() {
        ...
        match cmd.subtype {
            ...
            AT_SetRelOptions | AT_ResetRelOptions => {
                ...
                cmd_lockmode =
                    common_reloptions::AlterTableGetRelOptionsLockLevel(&def_list[..]);
            }
...
pub(crate) fn ATPrepCmd<'mcx>(
    ...
    if rel.rd_rel.relispartition
        ...
        && pg_inherits::PartitionHasPendingDetach(rel.rd_id)?
    {
        return utils_error::ereport(ERROR)
...
fn ATRewriteCatalogs<'mcx>(
    ...
    for ti in 0..wqueue.len() {
        ...
        if needs_toast_check {
            ...
            toasting::AlterTableCreateToastTable(
                mcx,
                wqueue[ti].relid,
...
fn ATExecCmd<'mcx>(
    ...
    match cmd.subtype {
        ...
        AT_ChangeOwner => {
            ...
            let new_owner_id = match newowner.as_rolespec() {
                Some(rs) => {
                    ...
                    acl_seams::get_rolespec_oid::call(&role, false)?
                }
        ...
        AT_ReAddDomainConstraint => {
            ...
            _address = tablecmds_seams::re_add_domain_constraint::call(
                mcx, &names, def,
            )?;
        }
        AT_ReAddComment => {
            ...
            _address =
                tablecmds_seams::comment_object::call(mcx, stmt)?;
        }
        AT_AddIndexConstraint => {
        ...
        | AT_EnableTrigAll | AT_DisableTrigAll | AT_EnableTrigUser | AT_DisableTrigUser => {
            ...
            commands_trigger::enable_disable::ATExecEnableDisableTrigger(
                mcx,
                rel,
                trigname,
                fires_when,
        ...
        AT_EnableRule | AT_EnableAlwaysRule | AT_EnableReplicaRule | AT_DisableRule => {
            ...
            rewriteDefine::EnableDisableRule(mcx, rel, rulename, fires_when)?;
    ...
    utility_out_seams::event_trigger_collect_alter_table_subcmd::call(
        subcmd_node,
        _address,
    )?;
...
fn ATRewriteTables<'mcx>(
    ...
    for ti in 0..wqueue.len() {
        ...
        if !heap::RELKIND_HAS_STORAGE(wqueue[ti].relkind) {
            continue;
        }
        ...
        if wqueue[ti].rewrite > 0 && wqueue[ti].relkind != RELKIND_SEQUENCE {
            ...
            if parsetree.is_some() {
                event_trigger_seams::event_trigger_table_rewrite::call(
                    parsetree,
                    relid,
                    wqueue[ti].rewrite,
                )?;
            ...
            let oid_new_heap = cluster_seams::make_new_heap::call(
                mcx,
                relid,
                new_table_space,
                new_access_method,
            ...
            let frozen_xid = snapmgr_pc_seams::recent_xmin::call();
            let cutoff_multi =
                multixact_seams::read_next_multixact_id::call()?;
            cluster_seams::finish_heap_swap::call(
                mcx,
                relid,
                oid_new_heap,
                false,
            ...
            if wqueue[ti].chgPersistence {
                sequence_seams_2::sequence_change_persistence::call(
                    mcx,
                    wqueue[ti].relid,
                    wqueue[ti].newrelpersistence,
                )?;
        ...
        if wqueue[ti].chgPersistence {
            ...
            let seqlist =
                pg_depend_seams::getOwnedSequences::call(mcx, relid)?;
    ...
    for ti in 0..wqueue.len() {
        ...
        for ci in 0..wqueue[ti].constraints.len() {
            ...
            let hasperiod =
                syscache_seams::search_constraint_form_by_oid::call(conid)?
                    .map(|c| c.form.conperiod)
                    .unwrap_or(false);

            // validateForeignKeyConstraint(conname, rel, refrel, con->refindid,
            //                              con->conid, con->conwithperiod);
            let result = trigger_seams::validate_foreign_key_constraint::call(
                mcx,
                &conname,
...
/// [`execMain_seams::at_rewrite_table_scan`] seam (the scan +
...
fn run_at_rewrite_table_scan<'mcx>(
    ...
    execMain_seams::at_rewrite_table_scan::call(
        mcx,
        tab.relid,
        oid_new_heap,
        &tab.oldDesc,
...
fn ATPrepChangePersistence<'mcx>(
    ...
    if !to_logged
        && !pg_publication_seams::GetRelationPublications::call(mcx, rel.rd_id)?
            .is_empty()
    {
        return utils_error::ereport(ERROR)
            .errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE)
    ...
    let fks = pg_constraint::fk_constraints_for_persistence_check(
