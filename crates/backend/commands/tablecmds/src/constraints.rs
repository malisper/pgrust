...
pub(crate) fn add_relation_not_null_constraints<'mcx>(
    ...
    for outerpos in 0..cons.len() {
        ...
        let colname = cdef
            ...
            .sval;
        let Some((attnum, _)) = crate::alter::attname_lookup(mcx, rel.rd_id, colname, false)?
        else {
            return Err(Box::new(
                PgError::error(format!(
                    "column \"{colname}\" of relation \"{relname}\" does not exist"
...
mod tests {
    ...
    fn collect_column_defaults_splits_raw_and_cooked() {
        ...
        elts.lappend(mcx, coldef(mcx, None, Some(cooked2), 0))
            .unwrap();
        elts.lappend(mcx, coldef(mcx, Some(raw3), None, b's'))
            .unwrap();
