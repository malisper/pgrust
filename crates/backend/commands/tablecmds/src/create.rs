...
fn validate_reloptions(mcx: Mcx<'_>, relkind: u8, reloptions: Option<&[u8]>) -> PgResult<()> {
    if relkind == RELKIND_VIEW {
        common_reloptions::view_reloptions(mcx, reloptions, true)?;
    } else if relkind == RELKIND_PARTITIONED_TABLE {
        common_reloptions::partitioned_table_reloptions(reloptions, true)?;
    } else {
        common_reloptions::heap_reloptions(mcx, relkind, reloptions, true)?;
    }
    Ok(())
}
...
pub fn create_toast_for_relation<'mcx>(
    ...
    toasting::NewRelationCreateToastTable(mcx, relid, toast_options)
}
...
pub fn create_ctas_relation<'mcx>(
    ...
    if is_matview {
        ...
        view_seams::store_view_query::call(
            mcx,
...
pub(crate) fn get_attribute_compression(
    ...
    if !type_is_toastable(atttypid)? {
        let typ = format_type_seams::format_type_be_str::call(atttypid)?;
        return ereport(ERROR)
            .errcode(ERRCODE_FEATURE_NOT_SUPPORTED)
