...
fn partition_colname<'mcx>(
    ...
    if attno != 0 {
        ...
        match lsyscache::attribute::get_attname(mcx, parent_relid, attno, false)?
        {
    ...
    if strategy == PARTITION_STRATEGY_HASH {
        {
            if spec.strategy != PARTITION_STRATEGY_HASH {
                ...
                return ereport(ERROR)
                    ...
                    .errposition(small1::parser_errposition(pstate, spec.location))
                    .finish(here("transformPartitionBound"))
                    .map(|()| unreachable!());
            }
    ...
    } else if strategy == PARTITION_STRATEGY_LIST {
        {
            ...
            for cell in spec.listdatums.iter() {
                ...
                for existing in new_listdatums.iter() {
                    if equalfuncs_seams::equal_node::call(existing, &value_node) {
                        duplicate = true;
                        break;
                    }
                }
...
fn bound_value_location(val: &Node<'_>) -> i32 {
    ...
    nodes_core::nodefuncs::expr_location(val.as_expr())
        .unwrap_or(-1)
}
...
fn transformPartitionBoundValue<'mcx>(
    ...
    let exprtype = nodes_core::nodefuncs::expr_type(Some(&value))?;
    ...
    let mut value: Expr<'mcx> = match coerced {
        ...
        None => {
            let type_name =
                adt_format_type::format_type_be(mcx, col_type)?;
            return ereport(ERROR)
                .errcode(ERRCODE_DATATYPE_MISMATCH)
                .errmsg(format!(
                ...
                .errposition(small1::parser_errposition(pstate, val_location))
    ...
    if !matches!(value, Expr::Const(_)) {
        parse_collate::assign_expr_collations(Some(pstate), &mut value)?;
        let planned = planner::expression_planner(mcx, value)?;
        let evaluated = clauses::evaluate_expr(
            mcx,
            planned,
            col_type,
            col_typmod,
...
pub fn define_relation_partbound<'mcx>(
    ...
    let key = partcache_seams::relation_get_partition_key::call(mcx, parent.alias())?
        .ok_or_else(|| {
            ereport(ERROR)
    ...
    heap::StorePartitionBound(mcx, rel, &parent, bound)?;
...
pub(crate) fn check_default_partition_contents<'mcx, 's>(
    ...
    let def_part_constraints =
        catalog_partition::get_proposed_default_constraint(mcx, new_part_vec)?;
    ...
    //                                                default_rel, parent);
    let def_part_constraints = catalog_partition::map_partition_varattnos(
        mcx,
    ...
    let all_parts: Vec<Oid> =
        if default_rel.rd_rel.relkind == RELKIND_PARTITIONED_TABLE {
            let (oids, _) = pg_inherits::find_all_inheritors(
                mcx,
                default_rel.rd_id,
                AccessExclusiveLock,
                false,
    ...
    for part_relid in all_parts {
        ...
        if part_relid != default_relid {
            ...
            let part_rel = common_relation::relation_open(mcx, part_relid, NoLock)?;
            ...
            let part_constraint = catalog_partition::map_partition_varattnos(
                mcx,
                def_copy,
                1,
                &part_rel,
...
fn check_default_partition_contents_one<'mcx>(
    ...
    execMain_seams::validate_default_partition_contents_scan::call(
        mcx,
        default_name,
        part_rel.rd_id,
        &exprs,
...
pub fn define_relation_clone_partition_objects<'mcx>(
    ...
    let idxlist = relcache::derived::RelationGetIndexList(parent_oid)?;
    // CloneForeignKeyConstraints(NULL, parent, rel): scans the parent's FKs.
    let has_fkeys =
        relcache::derived::relation_has_foreign_keys(parent_oid)?;
    ...
    let has_triggers =
        syscache_seams::rel_relhastriggers::call(parent_oid)?.unwrap_or(false);
    ...
    if !idxlist.is_empty() {
        let rel = common_relation::relation_open(mcx, relation_id, NoLock)?;
