...
fn ComputePartitionAttrs<'mcx>(
    ...
    for (attn, pp) in part_params.iter().enumerate() {
        ...
        if let Some(name) = pelem.name.as_ref().map(|s| s.as_str()) {
            ...
            let atttuple =
                cache_syscache::SearchSysCacheAttName(mcx, relid, name)?;
            if atttuple.is_none() {
                return ereport(ERROR)
                    ...
                    .errposition(small1::parser_errposition(pstate, pelem.location))
            ...
            let attnum = lsyscache::attribute::get_attnum(relid, name)?;
            ...
            let attgenerated =
                lsyscache::attribute::get_attgenerated(relid, attnum)?;
            if attgenerated != 0 {
                return ereport(ERROR)
                    .errcode(ERRCODE_INVALID_OBJECT_DEFINITION)
            ...
            let (typid, _typmod, collid) =
                lsyscache::attribute::get_atttypetypmodcoll(relid, attnum)?;
            ...
            atttype = nodes_core::nodefuncs::expr_type(Some(expr))?;
            attcollation = nodes_core::nodefuncs::expr_collation(Some(expr))?;

            /* The expression must be of a storable type. */
            let partattname = format!("{}", attn + 1);
            let mut containing: Vec<Oid> = Vec::new();
            heap::CheckAttributeType(
                mcx,
                &partattname,
                atttype,
                attcollation,
                &mut containing,
                heap::CHKATYPE_IS_PARTKEY,
            )?;
            ...
            let mut expr_attrs = vars::var::pull_varattnos(
                &stripped_node,
                1,
                None,
            );
            ...
            loop {
                ...
                if attno > 0 {
                    let attgen = lsyscache::attribute::get_attgenerated(
                        relid,
                        attno as AttrNumber,
                    )?;
                    if attgen != 0 {
                        let cname = lsyscache::attribute::get_attname(
                            mcx,
                            relid,
                            attno as AttrNumber,
                            false,
             ...
             * for mutability.
             */
            let planned = planner::expression_planner(mcx, stripped.clone_in(mcx)?)?;

            if clauses::grounded::contain_mutable_functions(Some(&planned))? {
                return ereport(ERROR)
                    .errcode(ERRCODE_INVALID_OBJECT_DEFINITION)
...
fn finish_collation<'mcx>(
    ...
    if !pelem.collation.is_empty() {
        ...
        *attcollation =
            catalog_namespace::get_collation_oid(mcx, &namelist, false)?;
    }

    /* Check we have a collation iff it's a collatable type. */
    if lsyscache::type_::type_is_collatable(atttype)? {
        ...
        return ereport(ERROR)
            ...
            .errmsg(format!(
                ...
                adt_format_type::format_type_be(mcx, atttype)?
            ))
            .finish(here("ComputePartitionAttrs"))
            .map(|()| unreachable!());
    }
...
fn resolve_opclass<'mcx>(
    ...
    if pelem.opclass.is_empty() {
        let opclass = indexcmds::opclass::GetDefaultOpClass(atttype, am_oid)?;
        ...
        indexcmds::opclass::ResolveOpClass(
            mcx,
            &pelem.opclass,
            atttype,
            am_name,
