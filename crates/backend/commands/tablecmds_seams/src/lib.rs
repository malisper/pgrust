...
seam_core::seam!(
    ...
    pub fn define_relation<'mcx>(
        mcx: mcx::Mcx<'mcx>,
        stmt: nodes::ddlnodes::CreateStmt<'mcx>,
        relkind: u8,
        owner_id: Oid,
...
seam_core::seam!(
    ...
    pub fn remove_relations<'mcx>(
        mcx: mcx::Mcx<'mcx>,
        drop: &nodes::ddlnodes::DropStmt<'mcx>,
    ) -> PgResult<()>
);
...
seam_core::seam!(
    ...
    pub fn execute_truncate<'mcx>(
        mcx: mcx::Mcx<'mcx>,
        stmt: &nodes::ddlnodes::TruncateStmt<'mcx>,
    ) -> PgResult<()>
);
...
seam_core::seam!(
    ...
    pub fn build_desc_for_relation<'mcx>(
        mcx: mcx::Mcx<'mcx>,
        columns: &[nodes::rawnodes::ColumnDef<'mcx>],
...
seam_core::seam!(
    ...
    pub fn register_on_commit_action(
        relid: Oid,
        action: nodes::primnodes::OnCommitAction,
    ) -> PgResult<()>
);
...
seam_core::seam!(
    ...
    pub fn check_table_not_in_use(rel: &rel::Relation<'_>, stmt: &str) -> PgResult<()>
);
seam_core::seam!(
...
seam_core::seam!(
    ...
    pub fn define_sequence_relation<'mcx>(
        mcx: mcx::Mcx<'mcx>,
        seq: &nodes::ddlnodes::CreateSeqStmt<'_>,
    ) -> PgResult<types_catalog::catalog_dependency::ObjectAddress>
);
...
seam_core::seam!(
    ...
    pub fn AlterTableNamespace<'mcx>(
        mcx: mcx::Mcx<'mcx>,
        stmt: &parsenodes::AlterObjectSchemaStmt,
        want_oldschema: bool,
...
pub struct MergeAttributesResult<'mcx> {
    ...
    pub columns: mcx::PgVec<'mcx, nodes::rawnodes::ColumnDef<'mcx>>,
    /// `*supconstr` — inherited CHECK constraints (`CookedConstraint` nodes,
    /// carried as raw `Node`s).
    pub old_constraints: mcx::PgVec<'mcx, nodes::nodes::NodePtr<'mcx>>,
    /// `*supnotnulls` — inherited NOT NULL constraints.
    pub old_notnulls: mcx::PgVec<'mcx, nodes::nodes::NodePtr<'mcx>>,
}
...
seam_core::seam!(
    ...
    pub fn merge_attributes<'mcx>(
        mcx: mcx::Mcx<'mcx>,
        columns: mcx::PgVec<'mcx, nodes::rawnodes::ColumnDef<'mcx>>,
        supers: &[Oid],
        relpersistence: u8,
        is_partition: bool,
...
seam_core::seam!(
    ...
    pub fn typename_type_id<'mcx>(
        mcx: mcx::Mcx<'mcx>,
        type_name: &nodes::rawnodes::TypeName<'mcx>,
    ) -> PgResult<Oid>
);
...
seam_core::seam!(
    ...
    pub fn get_column_def_collation<'mcx>(
        mcx: mcx::Mcx<'mcx>,
        coldef: &nodes::rawnodes::ColumnDef<'mcx>,
        type_oid: Oid,
    ) -> PgResult<Oid>
);
...
seam_core::seam!(
    ...
    /// logical decoding?
    pub fn relation_is_logically_logged(rel: &rel::Relation<'_>) -> PgResult<bool>
);

seam_core::seam!(
    /// `RELATION_IS_OTHER_TEMP(rel)` (rel.h): is this a temp table of *another*
    /// backend?
    pub fn relation_is_other_temp(rel: &rel::Relation<'_>) -> PgResult<bool>
);

seam_core::seam!(
    /// `rel->rd_refcnt` (rel.h): the relcache pin count — read by
    /// `CheckTableNotInUse`.
    pub fn relation_get_refcount(rel: &rel::Relation<'_>) -> PgResult<i32>
);

seam_core::seam!(
    /// `rel->rd_isnailed` (rel.h): is this relation nailed in the relcache?
    pub fn relation_is_nailed(rel: &rel::Relation<'_>) -> PgResult<bool>
);

seam_core::seam!(
    /// `rel->rd_createSubid` (rel.h): the sub-transaction that created this
    /// relation (`InvalidSubTransactionId` if not new in this xact).
    pub fn relation_get_create_subid(rel: &rel::Relation<'_>) -> PgResult<SubTransactionId>
);
...
seam_core::seam!(
    ...
    pub fn pgstat_count_truncate(rel: &rel::Relation<'_>) -> PgResult<()>
);
...
/* ---- RENAME / SET SCHEMA outward seams (cross-crate owners) --------------- */

seam_core::seam!(
    /// `AlterTypeNamespaceInternal(typeOid, nspOid, isImplicitArray,
    /// ignoreDependent, errorOnTableType, objsMoved)` (typecmds.c:4091): move a
    /// type (and its autogenerated array type) to a new namespace, fixing the
    /// pg_type row and its pg_depend entry. `AlterTableNamespaceInternal` calls
    /// it for a relation's row type (`rd_rel->reltype`). The owner
    /// (`backend-commands-typecmds`) installs this in its `init_seams()`
    /// (a direct tablecmds->typecmds dependency would cycle). Returns the moved
    /// type's OID. `Err` carries the catalog `ereport(ERROR)`s.
    pub fn alter_type_namespace_internal(
        type_oid: Oid,
        nsp_oid: Oid,
...
seam_core::seam!(
    ...
    /// `SearchSysCache1(TYPEOID, typid)` + `checkDomainOwner(tup)` +
    /// `table_close(NoLock)`. Returns the domain's type OID. The owner crates
    /// (`backend-parser-parse-type` + `backend-commands-typecmds`) install this
    /// when they land (the `tablecmds` crate cannot take a
    /// `parse-type`/`typecmds` dependency without a cycle). `Err` carries the
    /// type-lookup / domain-owner `ereport(ERROR)`s. `names` is the
    /// qualified-name list decoded from `stmt->object` (`castNode(List, ...)` of
    /// `String` value nodes).
...
seam_core::seam!(
    ...
    pub fn re_add_domain_constraint<'mcx>(
        mcx: mcx::Mcx<'mcx>,
        names: &[String],
        new_constraint: &nodes::nodes::Node<'mcx>,
    ) -> PgResult<types_catalog::catalog_dependency::ObjectAddress>
);
