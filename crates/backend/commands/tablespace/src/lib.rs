...
pub fn TablespaceCreateDbspace(spcOid: Oid, _dbOid: Oid, isRedo: bool) -> PgResult<()> {
    ...
    let dir = relpath::GetDatabasePath(_dbOid, spcOid);
...
pub fn DropTableSpace<'mcx>(mcx: Mcx<'mcx>, stmt: &DropTableSpaceStmt<'mcx>) -> PgResult<()> {
    ...
    let (has_deps, detail, detail_log) =
        pg_shdepend_seams::checkSharedDependencies::call(
            mcx,
            TABLE_SPACE_RELATION_ID,
            tablespaceoid,
        )?;
     ...
     */
    comment::DeleteSharedComments(tablespaceoid, TABLE_SPACE_RELATION_ID)?;
    seclabel::DeleteSharedSecurityLabel(
        mcx,
        tablespaceoid,
        TABLE_SPACE_RELATION_ID,
    )?;
...
fn destroy_tablespace_directories(tablespaceoid: Oid, redo: bool) -> PgResult<bool> {
     ...
     */
    let dirdesc = match fd::allocated_desc::AllocateDir(&linkloc_with_version_dir)? {
    ...
    if let Some(dirdesc) = dirdesc {
        loop {
            let name =
                match fd::allocated_desc::ReadDir(Some(dirdesc), &linkloc_with_version_dir)? {
                    Some(de) => de.d_name,
                    None => break,
                };
            if name == "." || name == ".." {
            ...
            if !redo && !directory_is_empty(&subfile)? {
                fd::allocated_desc::FreeDir(Some(dirdesc))?;
                return Ok(false);
            }
...
pub fn directory_is_empty(path: &str) -> PgResult<bool> {
    let dirdesc = fd::allocated_desc::AllocateDir(path)?;

    loop {
        // ReadDir with a None dir mirrors C's NULL DIR* (failed AllocateDir):
        // it reports the open failure at ERROR.
        match fd::allocated_desc::ReadDir(dirdesc, path)? {
            Some(de) => {
                if de.d_name == "." || de.d_name == ".." {
                    continue;
                }
                fd::allocated_desc::FreeDir(dirdesc)?;
                return Ok(false);
            }
            None => break,
        }
...
pub fn check_default_tablespace<'mcx>(
     ...
     */
    if transam_xact::IsTransactionState() && glob::MyDatabaseId::call()? != InvalidOid
    {
        if !newval.is_empty() && !OidIsValid(get_tablespace_oid(mcx, newval, true)?) {
            ...
            if source == PGC_S_TEST {
                ...
                misc_guc::GUC_check_errdetail(format!(
...
pub fn GetDefaultTablespace<'mcx>(
    ...
    if relpersistence as u8 == RELPERSISTENCE_TEMP {
        ...
        return Ok(fd::temp_files::GetNextTempTableSpace());
    }
...
pub fn assign_temp_tablespaces(_newval: &str, extra: Option<&TempTablespacesExtra>) {
    ...
    if let Some(extra) = extra {
        fd::temp_files::SetTempTablespaces(extra.tblSpcs())
    } else {
        fd::temp_files::SetTempTablespaces(&[])
    }
}
...
pub fn PrepareTempTablespaces<'mcx>(mcx: Mcx<'mcx>) -> PgResult<()> {
    ...
    if fd::temp_files::TempTablespacesAreSet() {
        return Ok(());
    }
     ...
     */
    if !transam_xact::IsTransactionState() {
        return Ok(());
    }
    ...
    fd::temp_files::SetTempTablespaces(&table_spaces);
    Ok(())
}
...
pub fn tblspc_redo(record: &mut wal::rmgr::XLogReaderState<'_>) -> PgResult<()> {
    let info = record_get_info(record) & !wal::wal::XLR_INFO_MASK;
    ...
    if info == XLOG_TBLSPC_CREATE {
        ...
        if !destroy_tablespace_directories(ts_id, true)? {
            ...
            standby::ResolveRecoveryConflictWithTablespace(ctx.mcx(), ts_id)?;
...
fn xlog_tblspc_create(tablespaceoid: Oid, location: &str) -> PgResult<()> {
    ...
    xloginsert::XLogRegisterData(&tablespaceoid.to_ne_bytes())?;
    // XLogRegisterData(location, strlen(location) + 1) — include the NUL.
    let mut path_bytes = location.as_bytes().to_vec();
    path_bytes.push(0);
    xloginsert::XLogRegisterData(&path_bytes)?;
    xloginsert::XLogInsert(RM_TBLSPC_ID, XLOG_TBLSPC_CREATE)?;
    Ok(())
}
...
fn xlog_tblspc_drop(tablespaceoid: Oid) -> PgResult<()> {
    xloginsert::XLogBeginInsert()?;
    xloginsert::XLogRegisterData(&tablespaceoid.to_ne_bytes())?;
    xloginsert::XLogInsert(RM_TBLSPC_ID, XLOG_TBLSPC_DROP)?;
    Ok(())
}
...
pub fn init_seams() {
    ...
    tablespace_seams::get_tablespace_oid::set(|name, missing_ok| {
    ...
    tablespace_seams::prepare_temp_tablespaces::set(|| {
    ...
    // (C `char`).
    tablecmds_seams::get_default_tablespace::set(|relpersistence, partitioned| {
    ...
    utility_out_seams::create_table_space::set(create_table_space_arm);
    utility_out_seams::drop_table_space::set(drop_table_space_arm);
    utility_out_seams::alter_table_space_options::set(alter_table_space_options_arm);
