...
//! seam in [`tsearchcmds_seams`] that returns the relevant
...
fn get_ts_parser_func<'mcx>(mcx: Mcx<'mcx>, defel: &DefElem, attnum: i32) -> PgResult<Oid> {
    ...
    let proc_oid = parse_func_seams::lookup_func_name::call(
        &func_name,
        nargs as i32,
        arg_types,
        false,
    )?;
    if lsyscache_seams::get_func_rettype::call(proc_oid)? != ret_type_id {
        let sig = func::func_signature_string(&func_name, nargs as i32, &[], arg_types)?;
        let fmt = format_type_seams::format_type_be::call(mcx, ret_type_id)?;
...
fn makeConfigurationDependencies<'mcx>(
    ...
    if remove_old {
        ...
        pg_shdepend_seams::deleteSharedDependencyRecordsFor::call(
            myself.classId,
            myself.objectId,
            0,
        )?;
    ...
    /* dependency on extension */
    pg_depend_seams::recordDependencyOnCurrentExtension::call(
...
fn namespace_create_aclcheck<'mcx>(mcx: Mcx<'mcx>, namespaceoid: Oid) -> PgResult<()> {
    let aclresult: AclResult = aclchk_seams::object_aclcheck::call(
        NamespaceRelationId,
        namespaceoid,
        miscinit_seams::get_user_id::call(),
        ACL_CREATE,
...
pub fn init_seams() {
    ...
    // CONFIGURATION).
    utility_out_seams::alter_ts_dictionary::set(alter_ts_dictionary_arm);
    utility_out_seams::alter_ts_configuration::set(alter_ts_configuration_arm);
}
