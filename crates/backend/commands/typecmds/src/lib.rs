...
fn parser_errposition_src(source: Option<&str>, location: i32) -> i32 {
    ...
    match source {
        ...
        Some(s) => {
            match mbutils_seams::pg_mbstrlen_with_len::call(s.as_bytes(), location)
            {
                Ok(n) => n + 1,
                Err(_) => 0,
            }
...
/// Project the resolver-facing `opclass::TypeName` consumed by the
...
fn typenameTypeId(tn: &TypeName) -> PgResult<Oid> {
    parse_type_seams::typename_type_id::call(&to_resolver_typename(tn))
}
...
fn typenameTypeIdAndMod(tn: &TypeName) -> PgResult<(Oid, i32)> {
    parse_type_seams::typename_type_id_and_mod::call(tn)
}

/// `TypeNameToString(typeName)` (parse_type.c).
#[allow(dead_code)]
fn TypeNameToString(mcx: Mcx<'_>, tn: &TypeName) -> PgResult<String> {
    Ok(parse_type_seams::typename_to_string::call(mcx, &to_resolver_typename(tn))?
        .to_string())
}
...
pub fn RemoveTypeById(type_oid: Oid) -> PgResult<()> {
     ...
     */
    let typtype = pg_type_seams::remove_type_catalog_row::call(type_oid)?;
...
pub fn AssignTypeArrayOid() -> PgResult<Oid> {
    ...
    if is_binary_upgrade::call() {
        ...
        pg_type_seams::get_new_type_oid::call()
    }
}
...
pub fn DefineCompositeType<'mcx>(
    ...
    let address = tablecmds_seams::define_relation::call(
        mcx,
        create_stmt,
...
fn type_name_from_namelist(names: &[String]) -> opclass::TypeName {
    opclass::TypeName {
        names: names.to_vec(),
        typeOid: InvalidOid,
        setof: false,
...
fn typename_type_id_from_names(names: &[String]) -> PgResult<Oid> {
    parse_type_seams::typename_type_id::call(&type_name_from_namelist(names))
}
...
fn domain_add_check_constraint<'mcx>(
    ...
    let expr = coerce::coerce_to_boolean(mcx, Some(&mut pstate), expr, "CHECK")?;
    ...
    if !pstate.p_rtable.is_empty()
        || var_seams::contain_var_clause::call(&expr)
    {
        return ereport(ERROR)
...
pub fn AlterDomainDefault<'mcx>(
    ...
    pg_type_seams::set_domain_default::call(domainoid, default_value, default_bin)?;
...
pub fn AlterDomainNotNull<'mcx>(
    ...
    if not_null {
        ...
        let conoid = pg_constraint_seams::find_domain_not_null_constraint_oid::call(
            mcx, domainoid,
        )?;
    ...
    pg_type_seams::set_type_not_null::call(domainoid, not_null)?;
...
pub fn AlterDomainDropConstraint<'mcx>(
     ...
     */
    let (found, was_notnull) = pg_constraint_seams::drop_domain_constraint::call(
        mcx,
        domainoid,
    ...
    if was_notnull {
        pg_type_seams::set_type_not_null::call(domainoid, false)?;
    }
     ...
     */
    inval_seams::cache_invalidate_heap_tuple::call(TypeRelationId, domainoid)?;
...
pub fn AlterDomainAddConstraint<'mcx>(
    ...
    if constr.contype == CONSTR_CHECK {
        ...
        inval_seams::cache_invalidate_heap_tuple::call(
            TypeRelationId,
            domainoid,
        )?;
        ...
        pg_type_seams::set_type_not_null::call(domainoid, true)?;
    }
...
pub fn AlterDomainValidateConstraint<'mcx>(
    ...
    let (conoid, conbin) = pg_constraint_seams::find_domain_check_constraint::call(
        mcx,
        domainoid,
        constr_name.to_string(),
    )?;
    ...
    pg_constraint_seams::set_constraint_validated::call(mcx, conoid)?;
...
pub fn RenameType<'mcx>(
     ...
     */
    if typ.typtype == TYPTYPE_COMPOSITE {
        tablecmds_seams::rename_relation_internal::call(
            mcx,
            typ.typrelid,
            new_type_name,
            false,
...
pub fn AlterTypeOwner<'mcx>(
     ...
     */
    let typeOid = parse_type_seams::lookup_type_name_oid_from_names::call(
    ...
    if typ.typowner != new_owner_id {
        ...
        if !superuser::call(mcx)? {
            ...
            acl_seams::check_can_set_role::call(get_user_id::call(), new_owner_id)?;
...
pub fn AlterTypeOwner_oid(
     ...
     */
    if typ.typtype == TYPTYPE_COMPOSITE {
        tablecmds_seams::at_exec_change_owner::call(
            typ.typrelid,
            new_owner_id,
            true,
            AccessExclusiveLock,
    ...
    if has_depend_entry {
        pg_shdepend_seams::changeDependencyOnOwner::call(
            TypeRelationId,
            type_oid,
            new_owner_id,
        )?;
...
pub fn AlterTypeOwnerInternal(type_oid: Oid, new_owner_id: Oid) -> PgResult<()> {
    ...
    pg_type_seams::set_type_owner::call(type_oid, new_owner_id)?;
...
pub fn AlterTypeNamespace<'mcx>(
    ...
    let nspOid = namespace_seams::lookup_creation_namespace::call(newschema)?;

    let mut objsMoved = dependency_seams::new_object_addresses::call()?;
    let oldNspOid = AlterTypeNamespace_oid(typeOid, nspOid, false, &mut objsMoved)?;
    dependency_seams::free_object_addresses::call(objsMoved)?;
...
pub fn AlterTypeNamespaceInternal(
    ...
    if dependency_seams::object_address_present::call(thisobj, &*objs_moved)? {
        return Ok(InvalidOid);
    }
    ...
    if oldNspOid != nsp_oid {
        ...
        pg_type_seams::set_type_namespace::call(type_oid, nsp_oid)?;
    }
     ...
     */
    if isCompositeType {
        tablecmds_seams::alter_relation_namespace_internal::call(
            typform.typrelid,
            oldNspOid,
            nsp_oid,
            false,
        ...
        pg_constraint_seams::alter_constraint_namespaces::call(
            type_relation_mcx(),
            typform.typrelid,
            oldNspOid,
            nsp_oid,
    ...
    dependency_seams::add_exact_object_address::call(thisobj, objs_moved)?;
...
pub fn AlterTypeRecurse(
    ...
    let domains = pg_type_seams::scan_domains_over_basetype::call(type_oid)?;
...
pub fn init_seams() {
    ...
    tablecmds_seams::alter_type_namespace_internal::set(
    ...
    tablecmds_seams::alter_type_owner_internal::set(AlterTypeOwnerInternal);
    ...
    tablecmds_seams::rename_constraint_domain_typid::set(
    ...
    utility_out_seams::define_stmt::set(define_stmt_seam);
    ...
    utility_out_seams::define_enum::set(define_enum_seam);
    utility_out_seams::define_range::set(define_range_seam);
    ...
    // RELKIND_COMPOSITE_TYPE).
    utility_out_seams::define_composite_type::set(define_composite_type_seam);
    ...
    // TYPE … SET (…).
    utility_out_seams::alter_type::set(alter_type_seam);

    // ProcessUtilitySlow `T_AlterEnumStmt` dispatch target (utility.c) — ALTER
    // TYPE … ADD VALUE / RENAME VALUE.
    utility_out_seams::alter_enum::set(alter_enum_seam);
    ...
    tablecmds_seams::re_add_domain_constraint::set(
...
fn alter_type_seam<'mcx>(
    ...
    for n in ats.options.iter() {
        options.push(parse_type::rich_node_to_parse(n)?);
    }
...
fn define_range_seam<'mcx>(
    ...
    for n in crs.params.iter() {
        params.push(parse_type::rich_node_to_parse(n)?);
    }
...
fn define_stmt_seam<'mcx>(
    ...
    for n in ds.definition.iter() {
        definition.push(parse_type::rich_node_to_parse(n)?);
    }
    ...
    match ds.kind {
        ...
        OBJECT_OPERATOR => {
            ...
            operatorcmds::DefineOperator(mcx, &names, &definition)
        }
        OBJECT_AGGREGATE => {
            ...
            for n in ds.args.iter() {
                args.push(parse_type::rich_node_to_parse(n)?);
            }
        ...
        OBJECT_TSPARSER => {
            // Assert(stmt->args == NIL).
            tsearchcmds::DefineTSParser(mcx, &ds.defnames, &ds.definition)
        }
        OBJECT_TSDICTIONARY => {
            // Assert(stmt->args == NIL).
            tsearchcmds::DefineTSDictionary(mcx, &ds.defnames, &ds.definition)
        }
        OBJECT_TSTEMPLATE => {
            // Assert(stmt->args == NIL).
            tsearchcmds::DefineTSTemplate(mcx, &ds.defnames, &ds.definition)
        }
        OBJECT_TSCONFIGURATION => {
            ...
            tsearchcmds::DefineTSConfiguration(
                mcx,
                &ds.defnames,
                &ds.definition,
                &mut copied,
...
fn define_domain_seam<'mcx>(
    ...
    let type_name = parse_type::raw_typename_to_parse(raw_tn)?;
