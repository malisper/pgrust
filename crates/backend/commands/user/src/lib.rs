...
//!
//! Genuine cross-subsystem externals cross the [`user_seams`]
...
pub fn CreateRole<'mcx>(
     ...
     * "pg_" namespace.
     */
    if catalog_seams::is_reserved_name::call(role.clone())? {
        return ereport(ERROR)
...
pub fn RenameRole<'mcx>(
     ...
     */
    if catalog_seams::is_reserved_name::call(authform.rolname.clone())? {
        return ereport(ERROR)
            .errcode(ERRCODE_RESERVED_NAME)
    ...
    if catalog_seams::is_reserved_name::call(newname.to_string())? {
        return ereport(ERROR)
            .errcode(ERRCODE_RESERVED_NAME)
...
pub fn init_seams() {
     ...
     * second copy.
     */
    seam::superuser::set(|| superuser_seams::superuser::call());
    seam::superuser_arg::set(|roleid| {
        superuser_seams::superuser_arg::call(roleid)
    });
    seam::has_privs_of_role::set(|member, role| {
        acl_seams::has_privs_of_role::call(member, role)
    });
    ...
    seam::is_binary_upgrade::set(|| {
        Ok(binary_upgrade_seams::is_binary_upgrade::call())
    });
     ...
     * `parsenodes` role-statement model these drivers consume. */
    utility_out_seams::create_role::set(create_role_arm);
    utility_out_seams::alter_role::set(alter_role_arm);
    utility_out_seams::alter_role_set::set(alter_role_set_arm);
    utility_out_seams::drop_role::set(drop_role_arm);
    utility_out_seams::reassign_owned_objects::set(reassign_owned_objects_arm);
    utility_out_seams::grant_role::set(grant_role_arm);
    utility_out_seams::drop_owned_objects::set(drop_owned_objects_arm);
}
