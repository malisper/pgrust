...
fn check_datestyle_inner(newval: &str, source: GucSource) -> PgResult<Option<DateStyleResult>> {
    ...
    let elemlist = match varlena_seams::split_identifier_string::call(
        scratch.mcx(),
        newval,
        ',',
    )? {
...
/// canonical per-backend store is `adt_datetime::settings`
...
pub fn assign_intervalstyle(newval: i32, _extra: Option<&GucHookExtra>) {
    adt_datetime::settings::set_interval_style(newval);
}
...
pub fn check_client_encoding(
    ...
    let canonical_name = encnames_seams::pg_encoding_to_char::call(encoding).to_string();
    ...
    if !parallel::is_parallel_worker() && own::prepare_client_encoding::call(encoding)? < 0 {
        if xact::is_transaction_state::call() {
            ...
            GUC_check_errcode(ERRCODE_FEATURE_NOT_SUPPORTED);
            let db_enc = mbutils_seams::get_database_encoding_name::call();
...
fn clean_ascii_name(newval: &mut Option<String>) -> PgResult<bool> {
    ...
    let cleaned = match string::pg_clean_ascii(scratch.mcx(), val, 0x02) {
...
fn recompute_io_combine_limit(newval: i32, from_max: bool) {
    ...
    bufmgr_seams::set_io_combine_limit::call(effective);
}
...
mod install {
    ...
    pub fn own_seams() {
        ...
        own::date_style::set(adt_datetime::settings::date_style);
        own::date_order::set(adt_datetime::settings::date_order);
        ...
        own::load_and_install_tz_abbrevs::set(|filename| {
            ...
            match misc_timeout::tzparser::load_tzoffsets(&filename) {
                Ok(table) => {
                    ...
                    adt_datetime::tz_abbrev_install::install_time_zone_abbrevs(
                        table,
                    );
                    Ok(true)
                }
        ...
        own::interval_in_for_timezone::set(|val| {
            ...
            let interval = adt_datetime::interval::interval_in(&val, -1)?;
            Ok((interval.month, interval.day, interval.time))
        });

        // -------- pgtz.c / localtime.c / state-pgtz (ported) — delegate --------
        own::pg_tzset_name::set(|name| {
            Ok(timezone_pgtz::pg_tzset(&name)?
                .map(|tz| localtime::pg_get_timezone_name(&tz).to_string()))
        });
        own::pg_tzset_offset_name::set(|gmtoffset| {
            Ok(timezone_pgtz::pg_tzset_offset(gmtoffset)?
                .map(|tz| localtime::pg_get_timezone_name(&tz).to_string()))
        });
        own::pg_tz_name_acceptable::set(|name| {
            ...
            match timezone_pgtz::pg_tzset(&name)? {
                Some(tz) => Ok(localtime::pg_tz_acceptable(&tz)),
                None => Ok(false),
            }
        });
        own::set_session_timezone_by_name::set(|name| {
            let tz = timezone_pgtz::pg_tzset(&name)?
        ...
        own::show_session_timezone_name::set(|| {
            ...
            let name = localtime::pg_get_timezone_name(&tz);
            if name.is_empty() {
                "unknown".to_string()
            } else {
                name.to_string()
        ...
        own::first_snapshot_set::set(snapmgr::FirstSnapshotSet);

        // -------- setseed (pseudorandomfuncs.c) --------
        // Installed by the real owner crate
        // `pseudorandomfuncs::init_seams()`; not set here.
        ...
        own::pg_valid_client_encoding::set(|name| {
            Ok(extra_encnames_fgram::pg_valid_client_encoding(&name))
        });
        ...
        own::get_session_user_id::set(miscinit::GetSessionUserId);
        own::get_session_user_is_superuser::set(
            miscinit::GetSessionUserIsSuperuser,
        );
        own::get_authenticated_user_id::set(miscinit::GetAuthenticatedUserId);
        own::get_current_role_id::set(miscinit::GetCurrentRoleId);
        ...
        own::lookup_authid_by_name::set(|mcx, name| {
            Ok(
                syscache_seams::lookup_authid_by_name::call(mcx, &name)?
        ...
        own::data_directory_mode::set(init_small::globals::data_directory_mode);
        own::log_file_mode::set(syslogger::config::log_file_mode);
        own::unix_socket_permissions::set(pqcomm::config::unix_socket_permissions);
    }
}
