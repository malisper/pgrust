...
fn setup() -> MutexGuard<'static, ()> {
    ...
    INSTALL.call_once(|| {
        ...
        varlena_seams::split_identifier_string::set(|mcx, raw, sep| {
        ...
        transam_xact_seams::xact_read_only::set(|| XACT_READ_ONLY.with(|c| c.get()));
        transam_xact_seams::xact_iso_level::set(|| XACT_ISO_LEVEL.with(|c| c.get()));
        transam_xact_seams::is_transaction_state::set(|| {
            IS_TRANSACTION_STATE.with(|c| c.get())
        });
        transam_xact_seams::is_sub_transaction::set(|| {
