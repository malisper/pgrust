...
use nodeLimit as nodeLimit;
use nodeMaterial as nodeMaterial;
use nodeSubplan_seams as nodeSubplan;
use nodeAppend as nodeAppend;
use nodeBitmapHeapscan as nodeBitmapHeapscan;
use nodeBitmapIndexscan as nodeBitmapIndexscan;
use nodeBitmapOr as nodeBitmapOr;
use nodeCtescan as nodeCtescan;
use nodeCustom as nodeCustom;
use nodeForeignscan as nodeForeignscan;
use nodeGather as nodeGather;
use nodeGatherMerge as nodeGatherMerge;
use nodeGroup as nodeGroup;
use nodeAgg as nodeAgg;
use nodeHash as nodeHash;
use nodeHashjoin as nodeHashjoin;
use nodeIndexonlyscan as nodeIndexonlyscan;
use nodeIndexscan as nodeIndexscan;
use nodeMemoize as nodeMemoize;
use nodeMergeAppend as nodeMergeAppend;
use nodeMergejoin as nodeMergejoin;
use nodeModifyTable as nodeModifyTable;
use nodeNamedtuplestorescan as nodeNamedtuplestorescan;
use nodeNestloop as nodeNestloop;
use nodeProjectSet as nodeProjectSet;
use nodeRecursiveunion as nodeRecursiveunion;
use nodeResult as nodeResult;
use nodeSeqscan as nodeSeqscan;
use nodeSetOp as nodeSetOp;
use nodeSort as nodeSort;
use nodeIncrementalSort as nodeIncrementalSort;
use nodeSubqueryscan as nodeSubqueryscan;
use nodeTableFuncscan as nodeTableFuncscan;
use nodeFunctionscan as nodeFunctionscan;
use nodeTidscan as nodeTidscan;
use nodeSamplescan as nodeSamplescan;
use nodeTidrangescan as nodeTidrangescan;
use nodeWorktablescan as nodeWorktablescan;
use nodeUnique as nodeUnique;
use nodeValuesscan as nodeValuesscan;
use nodeWindowAgg as nodeWindowAgg;
...
pub fn init_seams() {
    execAmi_seams::exec_re_scan::set(exec_re_scan);
    execAmi_seams::exec_mark_pos::set(exec_mark_pos);
    execAmi_seams::exec_restr_pos::set(exec_restr_pos);
    ...
    init_subselect_ext_seams::exec_materializes_output::set(
    ...
    init_subselect_ext_seams::exec_supports_backward_scan::set(
    ...
    joinpath_seams::exec_materializes_output::set(
        exec_materializes_output,
    );
}
    ...
    if node.tag() == ::nodes::execstate_tags::T_BitmapAndState {
        ...
        match node {
            ...
            PlanStateNode::LockRows(m) => {
                nodeLockRows::ExecReScanLockRows(m, estate)?
            }
