    ...
    match node.expr_tag() {
        ...
        etag::T_CaseExpr => {
            ...
            let case_cell = if let Some(arg) = caseexpr.arg.as_deref() {
                ...
                let argtype = nodeFuncs_seams::expr_type_info::call(arg)?.typid;
                if lsyscache_seams::get_typlen::call(argtype)? == -1 {
        ...
        etag::T_MinMaxExpr => {
            ...
            let cmp_proc = typcache_seams::lookup_element_cmp_proc::call(
        ...
        etag::T_ArrayExpr => {
            ...
            let tlbva = lsyscache_seams::get_typlenbyvalalign::call(
        ...
        etag::T_RowExpr => {
            ...
            let tupdesc: PgBox<'mcx, types_tuple::heaptuple::TupleDescData<'mcx>> =
                if rowexpr.row_typeid == RECORDOID {
                    ...
                    execTuples_seams::bless_tuple_desc::call(mcx, Some(td))?
                        .expect("BlessTupleDesc returned no tupdesc")
                } else {
                    // it's been cast to a named type, use that.
                    typcache_seams::lookup_rowtype_tupdesc_copy::call(
                        mcx,
                        rowexpr.row_typeid,
                        -1,
                    )?
            ...
            for i in 0..nelems {
                ...
                if i < nargs && !att.attisdropped {
                    ...
                    let etype = nodeFuncs_seams::expr_type_info::call(e)?.typid;
                    if etype != att.atttypid {
                        ...
                        let got = format_type_seams::format_type_be_owned::call(
                            etype,
                        )?;
        ...
        etag::T_RowCompareExpr => {
            ...
            for i in 0..nopers {
                ...
                let (_strategy, lefttype, righttype) =
                    lsyscache_seams::get_op_opfamily_properties::call(
                ...
                let proc = lsyscache_seams::get_opfamily_proc::call(
                    opfamily,
                    lefttype,
                    righttype,
                    BTORDER_PROC,
        ...
        etag::T_CoerceViaIO => {
            ...
            let src_type = nodeFuncs_seams::expr_type_info::call(arg)?.typid;
            ...
            let fcinfo_data_out = mcx::alloc_in(
                ...
                nodes::fmgr::FunctionCallInfoBaseData {
                    ...
                    isnull: false,
                    nargs: 1,
                    args: vec![datum::NullableDatum {
                        value: datum::Datum::from_usize(0),
                        isnull: false,
                    }],
            ...
            let (in_func, typioparam) =
                lsyscache_seams::get_type_input_info::call(
                    iocoerce.resulttype,
                )?;
            let finfo_in = fmgr_seams::fmgr_info::call(mcx, in_func)?;
        ...
        etag::T_ConvertRowtypeExpr => {
            ...
            let inputtype = nodeFuncs_seams::expr_type_info::call(arg)?.typid;
            let scratch = ExprEvalStep {
...
pub fn exec_prepare_qual<'a, 'mcx>(
    ...
    for q in qual {
        ...
        let pl = planner_pc_seams::expression_planner_value::call(mcx, owned)?;
...
pub fn exec_prepare_expr<'a, 'mcx>(
    ...
    let planned =
        planner_pc_seams::expression_planner_value::call(mcx, owned)?
            .clone_in(mcx)?;
...
pub fn exec_prepare_check<'mcx>(
    ...
    let anded = nodes_core::makefuncs::make_ands_explicit(planned);
...
pub fn exec_build_projection_info_impl<'mcx>(
    ...
    for (i, tle) in target_list.iter().enumerate() {
        ...
        if is_safe_var {
            ...
            let typlen = lsyscache_seams::get_typlenbyvalalign::call(typid)?
                .typlen;
...
pub fn exec_build_update_projection_impl<'mcx>(
    ...
    for attnum in (1..=natts).rev() {
        ...
        if nodes_core_seams::bms_is_member::call(attnum, assigned_cols_ref) {
            continue;
        }
    ...
    for attnum in 1..=natts {
        ...
        } else if !nodes_core_seams::bms_is_member::call(attnum, assigned_cols_ref) {
...
pub fn exec_eval_expr_switch_context<'mcx>(
    ...
    execExprInterp_seams::exec_eval_expr_switch_context::call(
...
pub fn exec_project_info<'mcx>(
    ...
    execExprInterp_seams::exec_eval_expr_no_return::call(
...
pub fn eval_const_test(
    ...
    let mut test_expr = nodes_core::makefuncs::make_opclause(
        test_op,
    ...
    // fix_opfuncids((Node *) test_expr);
    nodes_core::nodefuncs::fix_opfuncids(&mut test_expr)?;
...
pub fn evaluate_expr_fallback(
    ...
    nodes_core::nodefuncs::fix_opfuncids(&mut expr)?;
    ...
    let (result_typlen, result_typbyval) =
        lsyscache_seams::get_typlenbyval::call(result_type)?;
    ...
    let result = nodes_core::makefuncs::make_const(
...
pub fn eval_exec_param_into_list<'mcx>(
    ...
    let econtext =
        execUtils_seams::get_per_tuple_expr_context::call(estate)?;
