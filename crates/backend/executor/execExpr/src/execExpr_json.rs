...
pub(crate) fn exec_init_json_expr<'mcx>(
    ...
    let returning_domain =
        lsyscache_seams::get_typtype::call(returning.typid)? == TYPTYPE_DOMAIN;
    ...
    for i in 0..nargs {
        ...
        let ti = nodeFuncs_seams::expr_type_info::call(argexpr)?;
...
fn exec_init_json_coercion<'mcx>(
    ...
    let exists_cast_to_int = exists_coerce
        && lsyscache_seams::get_base_type::call(returning.typid)?
...
pub(crate) fn exec_init_json_is_predicate<'mcx>(
    ...
    let arg_type = nodeFuncs_seams::expr_type_info::call(expr)?.typid;
