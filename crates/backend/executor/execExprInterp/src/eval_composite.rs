...
pub fn ExecEvalRowNullInt<'mcx>(
    ...
    for att in 1..=tup_desc.natts {
        ...
        let attisnull = heaptuple::heap_attisnull(
            &tuple.tuple,
            att,
            Some(&tup_desc),
        );
...
pub fn ExecEvalRow<'mcx>(
    ...
    let datum = if has_external {
        let (flattened, _datum) =
            heaptoast::heap_tuple_header_get_datum(mcx, tuple)?;
...
pub fn ExecEvalMinMax<'mcx>(
    ...
    for off in 0..nelems as usize {
        ...
        if cur_isnull {
            ...
            let cmpresult = datum::Datum::from_usize(result.as_usize()).as_i32();
...
pub fn ExecEvalFieldSelect<'mcx>(
    ...
    let (value, fieldisnull) =
        heaptuple::heap_getattr(mcx, &tuple, fieldnum as i32, &tup_desc)?;
...
pub fn ExecEvalConvertRowtype<'mcx>(
    ...
    let indesc =
        typcache_seams::lookup_rowtype_tupdesc::call(mcx, inputtype, -1)?;
    let outdesc =
        typcache_seams::lookup_rowtype_tupdesc::call(mcx, outputtype, -1)?;

    // map = convert_tuples_by_name(indesc, outdesc);
    let map = next_seams::convert_tuples_by_name::call(mcx, &indesc, &outdesc)?;
    ...
    let result = if let Some(map) = map.as_ref() {
        ...
        next_seams::execute_attr_map_tuple::call(
            mcx,
            &in_tuple.tuple,
        ...
        heaptuple::heap_copy_tuple_as_datum(mcx, &in_tuple, &outdesc)?
    };
...
pub fn ExecEvalWholeRowVar<'mcx>(
    ...
    let slot_id = if has_junk_filter {
        ...
        execJunk::ExecFilterJunk(estate, jf, slot_id)?
    } else {
        slot_id
    };
    ...
    if first {
        ...
        if vartype != ::types_tuple::heaptuple::RECORDOID {
            ...
            let var_tupdesc =
                typcache_seams::lookup_rowtype_tupdesc_domain::call(
                    mcx, vartype, -1, false,
                )?
            ...
            let slot_tupdesc =
                execTuples_seams::exec_slot_descriptor::call(mcx, estate, slot_id)?
            ...
            if varno >= 1 && (varno as usize) <= estate.es_range_table_size {
                ...
                if let Some(names) = colnames {
                    ...
                    execTuples_seams::exec_type_set_col_names::call(
                        &mut out, &refs,
                    )?;
                }
            }
        ...
        let blessed =
            execTuples_seams::bless_tuple_desc::call(mcx, Some(output_tupdesc))?;
    ...
    let cols = execTuples_seams::slot_getallattrs_by_id::call(estate, slot_id)?;
    let slot_tupdesc =
        execTuples_seams::exec_slot_descriptor::call(mcx, estate, slot_id)?
            .expect("ExecEvalWholeRowVar: slot has no tuple descriptor");
    ...
    let mut tuple =
        heaptoast::toast_build_flattened_tuple(mcx, &slot_tupdesc, &values, &nulls)?;
