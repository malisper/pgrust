...
/// xml.c value workers in `adt_xml`.
    ...
    let (xexpr, named_arg_cells, named_arg_types, arg_cells) =
        match &state.steps.as_ref().unwrap()[op].d {
            ExprEvalStepData::XmlExpr {
                ...
                for e in &xexpr.args {
                    arg_types.push(nodeFuncs_seams::expr_type_info::call(e)?.typid);
                }
...
fn xml_arg_payload<'a>(v: &'a Datum<'a>) -> &'a [u8] {
    ...
    match img.first() {
        ...
        Some(_) if img.len() >= datum::varlena::VARHDRSZ => {
...
pub fn ExecEvalJsonConstructor<'mcx>(
    ...
    if ctor_type == Ct::JSCTOR_JSON_PARSE {
        ...
        if is_jsonb {
            ...
            adt_json::json_validate(js, true, true)?;
    ...
    let bytes: Vec<u8> = match ctor_type {
        Ct::JSCTOR_JSON_ARRAY => {
            if is_jsonb {
                adt_jsonb::jsonb_build_array_worker(
                    mcx, &arg_values, &arg_nulls, &arg_types, absent_on_null,
                )?
            } else {
                adt_json::json_build_array_worker(
                    mcx, &arg_values, &arg_nulls, &arg_types, absent_on_null,
                )?
            }
            .as_slice()
        ...
        Ct::JSCTOR_JSON_OBJECT => {
            if is_jsonb {
                adt_jsonb::jsonb_build_object_worker(
                    mcx, &arg_values, &arg_nulls, &arg_types, absent_on_null, unique,
                )?
            } else {
                adt_json::json_build_object_worker(
                    mcx, &arg_values, &arg_nulls, &arg_types, absent_on_null, unique,
                )?
            }
            .as_slice()
        ...
        Ct::JSCTOR_JSON_SCALAR => {
            ...
            let (category, outfuncid) =
                adt_jsonfuncs::categorize::json_categorize_type(
                    arg_types[0],
                    is_jsonb,
                )?;
            if is_jsonb {
                adt_jsonb::datum_to_jsonb(mcx, value, category, outfuncid)?
            } else {
                adt_json::datum_to_json(mcx, value, category, outfuncid)?
            }
            .as_slice()
            .to_vec()
        }
...
pub fn ExecEvalJsonIsPredicate<'mcx>(
    ...
    let res: bool = if arg_type == TEXTOID || arg_type == JSONOID {
        ...
        if res && (unique_keys || arg_type == TEXTOID) {
            res = adt_json::json_validate(json, unique_keys, false)?;
        }
        res
...
pub fn ExecEvalJsonExprPath<'mcx>(
                ...
                Some(jbv) => {
                    if !error && !empty {
                        ...
                        if rettypid == JSONOID || rettypid == JSONBOID {
                            ...
                            let s = adt_jsonb::jsonb_out(mcx, &jb)?;
                            ...
                            if !jsexpr.use_io_coercion && !is_null {
                                ...
                                if let Some(vs) = val_string.as_ref() {
                                    let t = varlena_seams::cstring_to_text_v::call(mcx, vs.as_str())?;
    ...
    if !resnull_now && jsexpr.use_io_coercion {
        ...
        if suppress_errors {
            ...
            let coerced = fmgr_seams::input_function_call::call(
                mcx,
                fn_oid,
                Some(&vs),
                typioparam,
...
fn build_path_vars<'mcx>(
    ...
    for (name, typid, typmod, cell) in specs {
        ...
        let (word, value_bytes) = match val {
            Datum::ByVal(w) => (datum::Datum::from_usize(w), None),
            other => (
                datum::Datum::null(),
...
fn exec_get_json_value_item_string<'mcx>(
    ...
    match &item.val {
        ...
        JsonbValueData::Numeric(num) => {
            let s = adt_numeric::io::numeric_out(mcx, num)?;
        ...
        JsonbValueData::Array { .. } | JsonbValueData::Object(_) | JsonbValueData::Binary { .. } => {
            ...
            let jb = jsonb_util::JsonbValueToJsonb(mcx, item)?;
            let s = adt_jsonb::jsonb_out(mcx, &jb)?;
        ...
        JsonbValueData::Datetime(dt) => {
            ...
            let s = fmgr_seams::oid_output_function_call_datum::call(
                mcx,
...
pub fn ExecEvalJsonCoercion<'mcx>(
    ...
    if exists_coerce {
        if exists_cast_to_int {
            ...
            if exists_check_domain {
                ...
                let res = typcache_seams::domain_check_input::call(
    ...
    let result = adt_jsonfuncs::populate::json_populate_type(
        mcx,
        &json_val,
        JSONBOID,
        targettype,
