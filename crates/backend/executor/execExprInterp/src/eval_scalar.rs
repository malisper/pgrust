...
pub fn exec_func_step<'mcx>(
    ...
    let (value, isnull) =
        if execSRF_seams::is_scalar_record_function::call(fn_oid) {
            execSRF_seams::invoke_scalar_record_function::call(
...
pub fn exec_nullif_step<'mcx>(
    ...
    if !nulls[0] && !nulls[1] {
        ...
        if make_ro {
            let mcx = estate.es_query_cxt;
            let ro = misc2_seams::make_expanded_object_read_only_internal_v::call(
                mcx,
                &save_arg0,
            )?;
            args[0] = ro;
...
pub fn ExecEvalFuncExprFusage<'mcx>(
    ...
    let mut fcusage =
        pgstat_function_seams::pgstat_init_function_usage::call(
            fn_stats, fn_oid,
        )?;
    ...
    // pgstat_end_function_usage(&fcusage, true).
    pgstat_function_seams::pgstat_end_function_usage::call(
        &mut fcusage,
        true,
    )
}
...
pub fn ExecEvalNextValueExpr<'mcx>(
    ...
    // int64 newval = nextval_internal(op->d.nextvalueexpr.seqid, false);
    let newval = sequence_seams_2::nextval_internal::call(mcx, seqid, false)?;
...
pub fn ExecEvalConstraintNotNull<'mcx>(
    ...
    if resnull {
        ...
        let typename =
            format_type_seams::format_type_be_owned::call(resulttype)?;
...
pub fn ExecEvalSysVar<'mcx>(
    ...
    let (value, isnull): (DatumV<'mcx>, bool) =
        execTuples_seams::slot_getsysattr::call(mcx, slot_data, attnum as i16)?;
...
pub fn ExecEvalScalarArrayOp<'mcx>(
    ...
    let mcx = estate.es_query_cxt;
    let elemtype =
        arrayfuncs_seams::array_get_elemtype_bytes::call(mcx, arr_bytes)?;
    let tlba = lsyscache_seams::get_typlenbyvalalign::call(elemtype)?;
    ...
    let elements = arrayfuncs_seams::deconstruct_array_values_bytes::call(
        mcx,
        arr_bytes,
        elemtype,
        tlba.typlen,
...
pub fn ExecEvalHashedScalarArrayOp<'mcx>(
    ...
    if !has_built {
        ...
        let elemtype = arrayfuncs_seams::array_get_elemtype_bytes::call(
            mcx,
            arraydatum_v.as_ref_bytes(),
        )?;
        let tlba =
            lsyscache_seams::get_typlenbyvalalign::call(elemtype)?;
        ...
        let elements = arrayfuncs_seams::deconstruct_array_v::call(
            mcx,
            arraydatum_v,
            elemtype,
            tlba.typlen,
