...
pub fn ExecFindPartition<'mcx>(
    ...
    let ecxt = execUtils_seams::get_per_tuple_expr_context::call(estate)?;
    ...
    while let Some(disp) = dispatch {
        ...
        if is_leaf {
            ...
            if idx >= 0 {
                ...
                let found_rri = execMain_seams::exec_lookup_result_rel_by_oid::call(
                    mtstate, estate, part_oid, true, false,
                )?;
                if let Some(found_rri) = found_rri {
                    // Verify this ResultRelInfo allows INSERTs.
                    let on_conflict_action =
                        nodeModifyTable_seams::exec_get_on_conflict_action::call(
                            mtstate,
                        );
            ...
            if proute.partition_dispatch_info[next_dispatch].tupslot.is_some() {
                ...
                cur_slot = tupconvert_seams::execute_attr_map_slot::call(
                    estate, &map_copy, cur_slot, new_my,
                )?;
                if let Some(tempslot) = tempslot {
                    execTuples_seams::exec_clear_tuple::call(estate, tempslot)?;
                }
            }
        ...
        if partidx == default_index {
            ...
            if is_leaf {
                ...
                if let Some(map) = map {
                    ...
                    cur_slot =
                        tupconvert_seams::execute_attr_map_slot::call(
    ...
    if let Some(myslot) = myslot {
        execTuples_seams::exec_clear_tuple::call(estate, myslot)?;
    }
...
pub(crate) fn FormPartitionKeyDatum<'mcx>(
    ...
    if has_partexprs && proute.partition_dispatch_info[dispatch].keystate.is_empty() {
        ...
        let keystate =
            execExpr_seams::exec_prepare_expr_list::call(partexprs, estate)?;
    ...
    for i in 0..partnatts {
        ...
        if keycol != 0 {
            ...
            let (d, n) = execTuples_seams::slot_getattr::call(
                estate,
                slot,
                keycol,
            )?;
...
pub(crate) fn get_partition_for_tuple<'mcx>(
    ...
    match strategy {
        PartitionStrategy::Hash => {
            ...
            let row_hash =
                partbounds_seams::compute_partition_hash_value::call(
                    key, values, isnull,
                )?;
        ...
        PartitionStrategy::List => {
            if isnull[0] {
                ...
                if last_found_count >= PARTITION_CACHED_FIND_THRESHOLD {
                    ...
                    let cmpval =
                        partbounds_seams::partition_list_datum_cmp::call(
                ...
                let (off, equal) =
                    partbounds_seams::partition_list_bsearch::call(
                        key,
                        boundinfo,
                        key_word(&values[0]),
                    )?;
        ...
        PartitionStrategy::Range => {
            ...
            if !range_partkey_has_null {
                ...
                if last_found_count >= PARTITION_CACHED_FIND_THRESHOLD {
                    ...
                    let cmpval =
                        partbounds_seams::partition_rbound_datum_cmp::call(
                            key,
                            &last_datums,
                            &kind,
                            values,
                ...
                let boundinfo = dispatch.partdesc.as_ref().unwrap().boundinfo.as_ref().unwrap();
                let (off, _equal) =
                    partbounds_seams::partition_range_datum_bsearch::call(
                        key,
                        boundinfo,
                        partnatts as i32,
                        values,
...
pub(crate) fn ExecBuildSlotPartitionKeyDescription<'mcx>(
    ...
    if more_seams::check_enable_rls::call(relid, InvalidOid, true)? == RLS_ENABLED
    {
        return Ok(None);
    }
    ...
    let aclresult =
        aclchk_seams::pg_class_aclcheck::call(relid, user_id, ACL_SELECT)?;
    if aclresult != ACLCHECK_OK {
        ...
        for i in 0..partnatts {
            ...
            if attnum == 0
                || aclchk_seams::pg_attribute_aclcheck::call(
    ...
    let cols =
        ruleutils_seams::pg_get_partkeydef_columns::call(mcx, relid, true)?;
    buf.try_push_str("(")?;
    ...
    for i in 0..partnatts {
        ...
        if isnull[i] {
            ...
            let (foutoid, _typisvarlena) =
                lsyscache_seams::get_type_output_info::call(key.parttypid[i])?;
        ...
        if vallen <= maxfieldlen {
            ...
            let clipped = mbutils_seams::pg_mbcliplen::call(val, vallen, maxfieldlen);
