...
pub(crate) fn ExecInitRoutingInfo<'mcx>(
    ...
    let root_to_child =
        execUtils_seams::exec_get_root_to_child_map::call(mcx, estate, part_rel_info)?;
    let partition_tuple_slot = if root_to_child.is_some() {
        ...
        let partrel = estate
            ...
            .alias();
        let slot = table_tableam::table_slot_create(mcx, &partrel)?;
...
pub(crate) fn ExecInitPartitionDispatchInfo<'mcx>(
    ...
    if estate.es_partition_directory.0.is_none() {
        ...
        estate.es_partition_directory =
            partitioning_core_seams::create_partition_directory::call(
                query_cxt,
                omit_detached,
            )?;
    }
    ...
    let (rel_owned, rel): (Option<Relation<'mcx>>, Relation<'mcx>) = if partoid != root_oid {
        let opened = common_relation_seams::relation_open::call(
            mcx,
            partoid,
            RowExclusiveLock,
        )?;
    ...
    let key = partcache_seams::relation_get_partition_key::call(mcx, rel.alias())?;
    ...
    let (tupmap, tupslot) = if let Some(parent) = parent_pd {
        ...
        let tupmap = next_seams::build_attrmap_by_name_if_req::call(
            mcx,
...
pub fn ExecCleanupTupleRouting<'mcx>(
    ...
    for i in 1..proute.num_dispatch as usize {
        ...
        if let Some(tupslot) = pd.tupslot.take() {
            execTuples_seams::exec_drop_single_tuple_table_slot::call(tupslot)?;
        }
    }
    ...
    for i in 0..proute.num_partitions as usize {
        ...
        // ExecCloseIndices(resultRelInfo);
        execIndexing_seams::exec_close_indices::call(estate, rri)?;
        // table_close(resultRelInfo->ri_RelationDesc, NoLock);
