...
/// [`TIDBitmap`](tidbitmap::TIDBitmap), folding that caller-side `IsA`
...
pub fn multi_exec_proc_node<'mcx>(
    ...
    match node {
        ...
        PlanStateNode::BitmapIndexScan(state) => {
            nodeBitmapIndexscan::MultiExecBitmapIndexScan(state, estate)
        }
        // case T_BitmapOrState:  result = MultiExecBitmapOr((BitmapOrState *) node);
        PlanStateNode::BitmapOr(state) => {
            nodeBitmapOr::MultiExecBitmapOr(mcx, state, estate)
        }
...
pub fn exec_end_node<'mcx>(
    ...
    match node {
        ...
        PlanStateNode::TidScan(state) => {
            nodeTidscan::ExecEndTidScan(state)
        }
