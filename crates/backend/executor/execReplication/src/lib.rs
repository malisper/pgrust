...
// Outward seams.
use amapi_seams as amapi_seams;
use table_tableam_seams as tableam_seams;
...
use execIndexing_seams as indexing_seams;
use execMain_seams as execMain_seams;
use execTuples_seams as execTuples_seams;
use nodeModifyTable_seams as modifytable_seams;
...
use relcache_nodexform_seams as relcache_nodexform_seams;
...
pub fn RelationFindReplTupleByIndex<'mcx>(
    ...
    let snap = snapshot::SnapshotData::sentinel(snapshot::snapshot::SnapshotType::SNAPSHOT_DIRTY);
...
fn BuildConflictIndexInfo<'mcx>(
    ...
    for i in 0..n as usize {
        ...
        let result = index_seams::build_speculative_index_info::call(
            &index_relation,
...
pub fn CheckCmdReplicaIdentity<'mcx>(
    ...
    if oid_is_valid(relcache::derived::RelationGetReplicaIndex(rel.rd_id)?) {
        return Ok(());
    }
