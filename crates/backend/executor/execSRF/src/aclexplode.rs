...
//! [`adt_acl::acl_ops::aclexplode`], which hands back a
//! `Vec<AclExplodeRow>`.
//!
...
fn aclexplode<'mcx>(fcinfo: &mut FunctionCallInfoBaseData<'mcx>) -> PgResult<Datum<'mcx>> {
        ...
        for (elem, isnull) in elems.iter() {
            ...
            match elem {
                ArrayElementDatum::ByRef(window) => {
                    items.push(adt_acl::acl_ops::aclitem_from_image(window));
                }
    ...
    let rows = adt_acl::acl_ops::aclexplode(&items);
    ...
    for row in &rows {
        ...
        let priv_text = varlena_seams::cstring_to_text_v::call(
            mcx,
            row.privilege_type,
        )?;
        let values = [
