...
//! registry (`fmgr_core`) holds `fmgr::PGFunction`s whose
...
pub fn init_seams() {
    ...
    register_srf(3401, misc2::admin::pg_hba_file_rules);
    register_srf(6250, misc2::admin::pg_ident_file_mappings);
    ...
    // pg_config() (OID 3400): the `pg_config` view SRF (get_configdata over the
    // running executable's location).
    pg_config::register_pg_config();
...
fn init_sexpr<'mcx>(
    ...
    let fn_retset = lsyscache_seams::get_func_retset::call(foid)?;
    let fn_strict = lsyscache_seams::func_strict::call(foid)?;
    sexpr.func = FmgrInfo {
        fn_addr: 0,
        fn_oid: foid,
    ...
    if let Some(expr) = sexpr.expr.as_deref() {
        fmgr_seams::fmgr_info_set_expr::call(
            estate.es_query_cxt,
            &mut sexpr.func,
            expr,
        )?;
    ...
    if sexpr.func.fn_retset && need_desc_for_srf {
        ...
        match resolved.class {
            ...
            Some(::nodes::funcapi::TypeFuncClass::Scalar) => {
                ...
                let collation =
                    nodes_core::nodefuncs::expr_collation(sexpr.expr.as_deref())?;
                tupdesc::TupleDescInitEntryCollation(&mut td, 1, collation)?;
                sexpr.funcReturnsTuple = false;
...
fn ExecInitTableFunctionResult<'mcx>(
    ...
    if let Some(func) = expr.as_funcexpr() {
        ...
        let es = execExpr_seams::exec_init_expr::call(expr, parent, estate)?;
...
fn materialize_sink_into_rsinfo<'mcx>(
    ...
    let ts = sort_storage_seams::tuplestore_begin_heap::call(
        per_query,
        random_access,
        false,
    ...
    for row in sink.rows.into_iter() {
        ...
        if returns_tuple && natts != 1 && row.len() == 1 {
            ...
            if col.isnull {
                ...
                sort_storage_seams::tuplestore_putvalues::call(
            ...
            let formed =
                heaptuple::DatumGetHeapTupleHeader(per_query, &comp)?;
...
fn ExecMakeTableFunctionResult<'mcx>(
    ...
    let funcrettype =
        nodes_core::nodefuncs::expr_type(setexpr.expr.as_deref())?;
    let returns_tuple =
        lsyscache_seams::type_is_rowtype::call(funcrettype)?;
                        ...
                        if !result_isnull {
                            let formed = match &result {
                                Datum::Composite(_) | Datum::ByRef(_) => {
                                    heaptuple::DatumGetHeapTupleHeader(
                                        per_query, &result,
                                    )?
                                }
                                _ => {
...
fn exec_prepare_tuplestore_result<'mcx>(
    ...
    if sexpr.funcResultSlot.is_none() {
        ...
        let slot = execTuples_seams::exec_init_extra_tuple_slot::call(
            estate,
            Some(slot_desc),
    ...
    loop {
        ...
        if fcache.funcResultStore.is_some() {
            ...
            let found_tup = {
                ...
                sort_storage_seams::tuplestore_gettupleslot::call(
            ...
            if found_tup {
                ...
                if fcache.funcReturnsTuple {
                    ...
                    let d =
                        execTuples_seams::exec_fetch_slot_heap_tuple_datum::call(
                            estate, slot,
                        )?;
                    ...
                    let (d, isnull) = execTuples_seams::slot_getattr::call(
                        estate, slot, 1,
                    )?;
            ...
            if let Some(store) = fcache.funcResultStore.take() {
                sort_storage_seams::tuplestore_end::call(store);
            }
        ...
        if callit {
            ...
            let funcrettype_tl =
                nodes_core::nodefuncs::expr_type(fcache.expr.as_deref())?;
            let returns_tuple_tl =
                lsyscache_seams::type_is_rowtype::call(funcrettype_tl)?;
            let fcinfo = fcache
                .fcinfo
                .as_mut()
                .expect("ExecMakeFunctionResultSet: fcinfo not initialized");
            ...
            let res = match dispatch {
                ...
                srf_registry::SrfDispatch::Materialized(sink) => {
                    ...
                    let exp = match rsinfo.expectedDesc.as_deref() {
                        ...
                        None => tupdesc::CreateTemplateTupleDesc(
                            estate.es_query_cxt,
                            0,
                        )?,
                    };
...
pub fn RestartSetExprState<'mcx>(fcache: &mut SetExprState<'mcx>) -> PgResult<()> {
    ...
    if let Some(fcinfo) = fcache.fcinfo.as_deref_mut() {
        if fcinfo.fn_extra.is_some() {
            funcapi_seams::end_MultiFuncCall::call(fcinfo)?;
        }
    }
...
fn tupledesc_match<'mcx>(
    ...
    for i in 0..dst.natts as usize {
        ...
        if sattr.atttypid == dattr.atttypid
            || coerce_seams::is_binary_coercible::call(
                sattr.atttypid,
                dattr.atttypid,
            )?
        {
        ...
        if !dattr.attisdropped {
            let returned =
                format_type_seams::format_type_be::call(mcx, sattr.atttypid)?;
            let expects =
                format_type_seams::format_type_be::call(mcx, dattr.atttypid)?;
            return Err(ereport(ERROR)
                .errcode(ERRCODE_DATATYPE_MISMATCH)
                .errmsg("function return row and query-specified return row do not match")
