...
//! [`misc::pg_get_catalog_foreign_keys`], which hands back a
...
fn pg_get_catalog_foreign_keys<'mcx>(
    ...
    let rows = misc::pg_get_catalog_foreign_keys()?;
    ...
    for row in &rows {
        ...
        let fkcols_img =
            array_more_seams::construct_text_array::call(&row.fkcols)?;
        let pkcols_img =
            array_more_seams::construct_text_array::call(&row.pkcols)?;
