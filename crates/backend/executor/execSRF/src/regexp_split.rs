...
//! are. It bypasses the by-OID builtin registry (whose `fmgr::PGFunction`
...
fn regexp_split_to_table<'mcx>(
    ...
    if fcinfo.fn_extra.is_none() {
        ...
        let rows: Vec<Vec<u8>> = {
            ...
            let materialized = regexp::regexp_split_to_table(
                mcx,
                &orig_str,
                &pattern,
                flags.as_deref(),
    ...
    if state.next < state.rows.len() {
        ...
        let datum = varlena_seams::bytes_to_varlena_v::call(
            mcx,
