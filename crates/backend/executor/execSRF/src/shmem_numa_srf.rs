...
//! ([`ipc_shmem::pg_get_shmem_allocations_numa`]); it runs the
...
fn huge_pages_status() -> HugePagesStatus {
    match guc_tables::vars::huge_pages_status.read() {
...
fn pg_get_shmem_allocations_numa<'mcx>(
    ...
    ipc_shmem::pg_get_shmem_allocations_numa(mcx, fcinfo, huge_pages_status())
}
