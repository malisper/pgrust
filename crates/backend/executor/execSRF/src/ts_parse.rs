...
//! [`parse::tt_storage_list`] (the parser's `(lexid, alias,
//! descr)` token-type descriptors) and [`parse::prs_tokenize`]
...
fn resolve_parser_byname(mcx: Mcx<'_>, name: &[u8]) -> PgResult<Oid> {
    let parts = varlena_seams::text_to_qualified_name_list::call(mcx, name)?;
    let refs: Vec<&str> = parts.iter().map(|s| s.as_str()).collect();
    namespace_seams::get_ts_parser_oid::call(mcx, &refs, false)
}
...
fn emit_token_type<'mcx>(
    ...
    let rows = parse::tt_storage_list();
    ...
    for row in rows {
        ...
        let alias = varlena_seams::cstring_to_text_v::call(mcx, &row.alias)?;
        let descr = varlena_seams::cstring_to_text_v::call(mcx, &row.descr)?;
...
fn emit_parse<'mcx>(
    ...
    let rows = parse::prs_tokenize(txt)?;
    ...
    for row in rows {
        ...
        let token = varlena_seams::cstring_to_text_v::call(
            mcx,
