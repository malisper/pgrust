...
//! [`tsvector_core::op::tsvector_unnest`], which hands back a
...
fn tsvector_unnest<'mcx>(fcinfo: &mut FunctionCallInfoBaseData<'mcx>) -> PgResult<Datum<'mcx>> {
    ...
    let rows = tsvector_core::op::tsvector_unnest(&image)?;
    ...
    for row in rows {
        ...
        let lexeme = varlena_seams::cstring_to_text_v::call(
            mcx,
        ...
        let (positions, weights, isnull): (Datum<'mcx>, Datum<'mcx>, [bool; 3]) =
            match &row.posweights {
                Some((p, w)) => {
                    ...
                    let pimg = array_more_seams::construct_int2_array::call(p)?;
                    ...
                    let wimg =
                        array_more_seams::construct_text_array::call(&wcols)?;
                    (
