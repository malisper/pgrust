...
fn ExecTypeFromTLInternal<'mcx>(
    ...
    for tle in target_list {
        ...
        let info = nodeFuncs_seams::expr_type_info::call(
            tle.expr
                .as_deref()
...
pub fn ExecTypeFromExprList<'mcx>(
    ...
    for e in expr_list {
        let info = nodeFuncs_seams::expr_type_info::call(e)?;
...
pub fn BuildTupleFromCStrings<'mcx>(
    ...
    for i in 0..natts {
        if !tupdesc.compact_attr(i).attisdropped {
            ...
            let value = fmgr_seams::input_function_call_for_heap_form::call(
                mcx,
...
pub fn HeapTupleHeaderGetDatum<'mcx>(
    ...
    heaptoast_seams::heap_tuple_header_get_datum::call(mcx, tuple)
}
...
pub fn do_tup_output<'mcx>(
    ...
    let _ =
        dest_seams::dest_receive_slot::call(mcx, &mut tstate.slot, tstate.dest)?;
...
pub fn do_text_output_multiline<'mcx>(
    ...
    while !rest.is_empty() {
        ...
        let datum = varlena_seams::cstring_to_text_v::call(mcx, line)?;
        let values = [datum];
...
pub fn end_tup_output<'mcx>(mcx: Mcx<'mcx>, tstate: TupOutputState<'mcx>) -> PgResult<()> {
    ...
    dest_seams::dest_rshutdown::call(mcx, tstate.dest)?;
...
pub fn install_guc_funcs_show_seams() {
    ...
    gseams::do_tup_output::set(|mcx, tstate, values| {
        ...
        for v in values {
            match v {
                Some(s) => {
                    datums.push(varlena_seams::cstring_to_text_v::call(mcx, &s)?);
                    isnull.push(false);
                }
                None => {
...
pub fn install_tup_output_seams() {
    use execTuples_seams as eseams;
...
pub fn deform_record_datum<'mcx>(
    ...
    let formed: FormedTuple<'mcx> =
        heaptuple::DatumGetHeapTupleHeader(mcx, &datum)?;
    let header = formed
        .tuple
        .t_data
        .as_ref()
    ...
    let cols =
        heaptuple::heap_deform_tuple(mcx, &formed.tuple, &retdesc, &formed.data)?;
