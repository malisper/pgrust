...
pub fn ExecForceStoreHeapTuple<'mcx>(
    ...
    match slot {
        ...
        SlotData::BufferHeap(_) => {
            ...
            let copied = heaptuple::heap_copytuple(mcx, Some(&tuple))?;
...
pub fn ExecForceStoreMinimalTuple<'mcx>(
    ...
    let htup = heaptuple::heap_tuple_from_minimal_tuple(mcx, &mtup)?;
...
pub fn ExecClearTuple(slot: &mut SlotData) -> PgResult<()> {
    trace::trace!(trace::Category::Slot, "ExecClearTuple");
    slot_clear(slot);
    Ok(())
}
...
pub fn ExecSetSlotDescriptor<'mcx>(
    ...
    if tupdesc.tdrefcount >= 0 {
        tupdesc::IncrTupleDescRefCount(&mut tupdesc)?;
    }
...
fn deform_into_slot<'mcx>(
    ...
    let columns =
        heaptuple::heap_deform_tuple(mcx, &tuple.tuple, desc, &tuple.data)?;
...
/// (`heaptuple::DatumGetHeapTupleHeader`), which is then
...
fn deform_composite_datum_into_slot<'mcx>(
    ...
    let tuple = heaptuple::DatumGetHeapTupleHeader(mcx, &data)?;
...
/// `heaptuple::HeapTupleGetDatum`, which sets the
...
fn heap_copy_tuple_as_datum_carrier<'mcx>(
    ...
    heaptuple::HeapTupleGetDatum(mcx, &tup, desc)
}
