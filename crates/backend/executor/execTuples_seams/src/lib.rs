...
seam_core::seam!(
    ...
    /// [`nodes::tuptable::SlotData`] and runs the real `clear` op (the
...
seam_core::seam!(
    ...
    /// payload-bearing [`nodes::tuptable::SlotData`]; the `shouldFree`
...
seam_core::seam!(
    ...
    /// [`nodes::tuptable::SlotData`] pair and runs the real `copyslot`
...
seam_core::seam!(
    ...
    pub fn exec_drop_single_tuple_table_slot<'mcx>(
        slot: nodes::tuptable::SlotData<'mcx>,
    ) -> types_error::PgResult<()>
);
...
seam_core::seam!(
    ...
    pub fn slot_natts(estate: &nodes::EStateData<'_>, slot: nodes::SlotId) -> i32
);
...
seam_core::seam!(
    ...
    pub fn exec_get_result_slot_ops<'mcx>(
        planstate: &nodes::execnodes::PlanStateData<'mcx>,
    ) -> nodes::TupleSlotKind
);
...
seam_core::seam!(
    ...
    pub fn exec_copy_slot_standalone<'mcx>(
        mcx: mcx::Mcx<'mcx>,
        dstslot: &mut nodes::SlotData<'mcx>,
        srcslot: &mut nodes::SlotData<'mcx>,
    ) -> types_error::PgResult<()>
);
