...
fn executor_state_create_and_free() {
    ...
    estate.with(|s| assert_eq!(s.es_direction, nodes::execnodes::ForwardScanDirection));
...
fn result_slot_ops_fallbacks() {
    ...
    estate.with_mut(|estate| {
        ...
        let slot = estate
            .make_slot({
                let mut slot = nodes::TupleTableSlot::new_in(estate.es_query_cxt);
...
fn relation_is_target_relation() {
    ...
    estate.with_mut(|estate| {
        ...
        estate.es_plannedstmt = Some(
            alloc_in(
                mcx,
                nodes::nodeindexscan::PlannedStmt {
                    commandType: nodes::nodes::CmdType::default(),
