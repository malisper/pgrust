...
pub(crate) fn init_plan<'mcx>(
    ...
    if operation == CmdType::CMD_SELECT {
        ...
        // is immutable here, so the filter is suppressed instead, for the
        // worker's root statement only. The TLS read hides behind the
        // resjunk scan (junk tlists are the rare case).
        let junk_filter_needed = plan.targetlist.iter().any(|tle_node| {
            ...
            && crate::execparallel::is_worker_root_pstmt(pstmt));
