...
/// (`=0`/`off` → default off, PGC_S_ENV_VAR) and remains the CI-harness /
/// kill-switch path.
#[inline]
pub fn enabled() -> bool {
            ...
            None => {
                ...
                arm_scan_staging(
                    ...
                    ScanFeedShape::RowFeed {
                        ctx: "standalone pgrcolumnar scan",
                        stitch: false,
                    },
                )?;
...
pub fn try_own_agg_over_seq_scan<'mcx>(
    ...
    // per-pull arm_dop cost a 2-col hash-distinct leg ~1.4x on the CI cluster).
...
unsafe fn agg_fold_staged_mm<'mcx>(
    ...
    if sa.is_none() && ::nodeagg::sink::agg_sink_mode(agg) && ::lanefold::plan_has_str_trans(plan) {
...
// route_to=legacy until the GL-MJSORT CI cluster letter flips it.
