...
/// Lane-ownable plan-shape classes. `Join` is reserved for the in-flight
/// hash-join lane (sibling branch) so the gate files don't churn when it
/// lands.
...
pub(super) enum RefuseReason {
    ...
    /// initplan params).
    AggNotDrainable = 17,
    /// Admission economics (perf-agent lane): the legacy fused drive already
    /// owns this shape better than the v2 pipeline.
    #[allow(dead_code)]
    AdmissionEconomicsFusedDrive = 18,
    /// Admission economics (perf-agent lane): no batch-consuming parent, so
    /// v2 ownership is pure overhead.
    #[allow(dead_code)]
