...
// machinery; only if the CI cluster scenario says donation cannot restore the
...
fn helper_drive_stmt(payload: &Arc<StmtTaskShared>) {
    // Pre-BIND stale-cancel hygiene (CI-caught race, armed take 1 job
...
fn run_channel_ladder(
    ...
    // worker claim before the close; witnessed on the CI cluster armed take-1
