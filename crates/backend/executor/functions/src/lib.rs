...
fn capture_receive<'mcx>(mcx: Mcx<'mcx>, state: u64, slot: &mut SlotData<'mcx>) -> PgResult<bool> {
    let (value, isnull) =
        execTuples::slot_deform::slot_getattr(mcx, slot, 1)?;
...
fn composite_receive<'mcx>(
    ...
    let (tuple, _should_free) =
        execTuples::slot_store_fetch::ExecFetchSlotHeapTuple(mcx, slot, false)?;
    ...
    if rettype != RECORDOID {
        let declared = typcache_seams::lookup_rowtype_tupdesc::call(
            mcx, rettype, -1,
        )?;
        ...
        if has_dropped {
            ...
            for attnum in 1..=live_natts {
                let (value, isnull) = execTuples::slot_deform::slot_getattr(
            ...
            let datum = heaptuple::HeapTupleGetDatum(mcx, &formed, &desc)?;
    ...
    let datum = heaptuple::HeapTupleGetDatum(mcx, &tuple, &desc)?;
    let image = match datum {
        CanonDatum::ByRef(b) => b.as_slice().to_vec(),
        CanonDatum::Composite(t) => t.to_datum_image(),
        other => {
...
fn fmgr_sql<'mcx>(
    ...
    if body.num_queries() == 0 && rettype != VOIDOID {
        ...
        return Err(parser_analyze::check_sql_fn_retval(
            mcx,
            &mut empty,
...
fn resolve_call_result_type<'mcx>(
    ...
    let resolved =
        funcapi_seams::get_expr_result_type::call(mcx, Some(&node))?;
...
fn parse_body_queries<'mcx>(
    ...
    if let Some(body) = prosqlbody {
        ...
        for raw in raw_list.iter() {
            ...
            let query = parser_analyze::parse_analyze_sql_function(
                mcx,
                raw,
                prosrc_mcx,
                pinfo.clone(),
...
fn check_sql_function_body(mcx: Mcx<'_>, funcoid: Oid) -> PgResult<()> {
    ...
    let rettupdesc =
        funcapi_seams::build_function_result_tupdesc_t::call(mcx, funcoid)?;
    parser_analyze::check_sql_fn_retval(
        mcx,
        &mut querytrees[..],
        form.prorettype,
        rettupdesc.as_deref(),
...
fn prepare_body_statement<'mcx>(
    ...
    let mut analyzed: Query<'mcx> = match body {
        ...
        BodyQueries::Raw {
            ...
            prosrc_mcx,
            pinfo,
        } => parser_analyze::parse_analyze_sql_function(
            mcx,
            &stmts[qi],
            prosrc_mcx,
            pinfo.clone(),
...
fn run_one_query<'mcx>(
    ...
    let (dest, token) = if is_result {
        ...
        let vtable = tcop_dest::ReceiverVtable {
            rStartup: capture_startup,
            receiveSlot: receive_slot,
            rShutdown: capture_shutdown,
        };
        let handle =
            tcop_dest::register_dest_receiver(CommandDest::SqlFunction, vtable, token);
        (handle, Some(token))
    } else {
        (tcop_dest::none_receiver(), None)
    };
    ...
    let run = (|| -> PgResult<()> {
        ...
        if plan.commandType == CMD_UTILITY {
            ...
            return utility_seams::process_utility::call(
                mcx,
                plan,
                source_text,
...
fn run_one_query_setof<'mcx>(
    ...
    let dest = if is_result {
        ...
        tcop_dest::register_dest_receiver(CommandDest::SqlFunction, vtable, 0)
    } else {
...
pub fn init_seams() {
    functions_seams::fmgr_sql::set(fmgr_sql);
    ...
    pg_proc_seams::run_sql_function_body_check::set(|funcoid| {
