...
pub fn init_seams() {
    instrument_seams::instr_alloc::set(InstrAlloc);
    instrument_seams::instr_start_node::set(InstrStartNode);
    instrument_seams::instr_stop_node::set(InstrStopNode);
    instrument_seams::instr_end_loop::set(InstrEndLoop);
    instrument_seams::instr_update_tuple_count::set(InstrUpdateTupleCount);
    instrument_seams::instr_agg_node::set(|dst, add| InstrAggNode(dst, &add));

    // `pgWalUsage` traffic report from the WAL-insertion core (xlog.c:1105).
    instrument_seams::report_wal_usage::set(|wal_bytes, wal_records, wal_fpi| {
        with_pgWalUsage(|u| {
    ...
    vacuumlazy_seams::pg_buffer_usage::set(|| {
        let u = pgBufferUsage();
        Ok((
            u.shared_blks_hit,
            u.shared_blks_read,
    ...
    vacuumlazy_seams::pg_wal_usage::set(|| {
        let w = pgWalUsage();
