...
/// probe_bytes_batch must be probe_bytes row-for-row: same group identity,
/// same is_new positions, same state pointers — across growth, the salt
/// enable point, the two-level conversion, and short/long key mixes.
#[test]
fn bytes_batch_matches_perrow() {
    // Distinct keys ≈ 200 + 3*card/4 (the class-0 arm collapses to 200):
    // crosses SALT_DISABLE (8192) and TWO_LEVEL (100k).
    let card = 160_000usize;
    let corpus: Vec<Vec<u8>> = (0..card * 3)
        .map(|i| {
            let k = i % card;
            match k % 4 {
                0 => format!("k{}", k % 200).into_bytes(), // <=8 B, hot dups
                1 => format!("key-{k:07}").into_bytes(),   // <=16 B
                2 => format!("key-{k:07}-a-mid-length-tail").into_bytes(),
                _ => format!("key-{k:07}-with-a-definitely-long-suffix-past-24").into_bytes(),
            }
        })
        .collect();
    ...
    for chunk in corpus.chunks(1024) {
        ...
        out.clear();
        new_out.clear();
        b.probe_bytes_batch(&refs, &mut hashes, &mut out, &mut new_out);
        assert_eq!(out.len(), chunk.len());
        let mut ni = 0usize;
        for (i, k) in chunk.iter().enumerate() {
            let pr = a.probe_bytes(k, a.hash_key_bytes(k));
            let batch_new = ni < new_out.len() && new_out[ni] as usize == i;
            if batch_new {
                ni += 1;
            }
            assert_eq!(pr.is_new, batch_new, "row {i} key {:?}", String::from_utf8_lossy(k));
            // SAFETY: zeroed 8-byte states on both sides.
            unsafe {
                *states_i64(pr.states) += 1;
                *states_i64(out[i]) += 1;
            }
        }
        assert_eq!(ni, new_out.len());
    }
    assert_eq!(a.len(), b.len());
    assert_eq!(a.nrows(), b.nrows());
    assert!(b.is_two_level());
    let mut sa = [0u8; 8];
    let mut sb = [0u8; 8];
    for i in 0..a.nrows() {
