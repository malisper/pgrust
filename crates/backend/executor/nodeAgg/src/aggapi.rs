...
pub fn AggRegisterCallback<'mcx>(
    ...
    // pool. (This rich-`nodes`-frame entry mirrors the `agg_register_callback`
...
// (orderedsetaggs.c) calls. They receive the LOW-LEVEL `fmgr` call frame
...
// `AggStateData`, and run the same reads the `nodes`-frame functions above
// do. This is the C `(AggState *) fcinfo->context` recovery, reproduced across
// the by-OID fmgr dispatch the executor actually uses.
// ---------------------------------------------------------------------------

/// Recover the live `AggStateData` from a low-level `fmgr` call frame's
...
fn agg_state_is_shared_shim(fcinfo: &fmgr::FunctionCallInfoBaseData) -> bool {
    if let Some(aggstate) = agg_context_from_raw_frame::<'_, '_>(fcinfo) {
        if aggstate.curperagg >= 0 {
            if let Some(peragg) = aggstate
                .peragg
...
pub fn init_seams() {
    nodeAgg_aggapi_seams::agg_get_aggref::set(agg_get_aggref_shim);
    nodeAgg_aggapi_seams::agg_check_call_context::set(agg_check_call_context_shim);
    nodeAgg_aggapi_seams::agg_state_is_shared::set(agg_state_is_shared_shim);
    nodeAgg_aggapi_seams::agg_register_callback::set(agg_register_callback_shim);
    nodeAgg_pq_seams::exec_agg_estimate::set(exec_agg_estimate_shim);
    nodeAgg_pq_seams::exec_agg_initialize_dsm::set(exec_agg_initialize_dsm_shim);
    nodeAgg_pq_seams::exec_agg_initialize_worker::set(
        exec_agg_initialize_worker_shim,
    );
    nodeAgg_pq_seams::exec_agg_retrieve_instrumentation::set(
        exec_agg_retrieve_instrumentation_shim,
    );
    costsize_seams::hash_agg_entry_size::set(hash_agg_entry_size_shim);
    costsize_seams::hash_agg_set_limits::set(hash_agg_set_limits_shim);
}
