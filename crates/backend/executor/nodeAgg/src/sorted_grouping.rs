...
fn exec_qual_eqfunction<'mcx>(
    ...
    with_phase_eqfunction(aggstate, eqidx, |state| {
        execExpr_seams::exec_qual::call(state, tmpcontext, estate)
    })
}
...
fn exec_qual_and_reset_eqfunction<'mcx>(
    ...
    with_phase_eqfunction(aggstate, eqidx, |state| {
        execExpr_seams::exec_qual_and_reset::call(state, tmpcontext, estate)
    })
}
...
pub fn agg_retrieve_direct<'mcx>(
    ...
    while !aggstate.agg_done {
        ...
        execUtils_seams::re_scan_expr_context::call(estate, econtext_id)?;
        ...
        for i in 0..num_reset {
            ...
            execUtils_seams::re_scan_expr_context::call(estate, aggctx_id)?;
        }
        ...
        if take_new_group_branch {
            ...
            if aggstate.grp_first_tuple.is_none() {
                ...
                if !tup_is_null(outerslot, estate) {
                    ...
                    let copied = execTuples_seams::exec_copy_slot_heap_tuple::call(
                        estate,
                        outerslot.unwrap(),
                    )?;
                ...
                loop {
                    ...
                    if node_aggstrategy != AGG_PLAIN && node_numcols > 0 {
                        ...
                        if !exec_qual_eqfunction(aggstate, (node_numcols - 1) as usize, estate)? {
                            // aggstate->grp_firstTuple = ExecCopySlotHeapTuple(outerslot);
                            let copied =
                                execTuples_seams::exec_copy_slot_heap_tuple::call(
                                    estate,
                                    outerslot.unwrap(),
                                )?;
                            aggstate.grp_first_tuple = Some(copied);
