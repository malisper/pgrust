    ...
    if pertrans.aggsortrequired {
        ...
        if let Some(sortstates) = pertrans.sortstates.as_mut() {
            if let Some(state) = sortstates[current_set].take() {
                tuplesort_seams::tuplesort_end::call(state)?;
            }
        }
        ...
        let new_state = if pertrans.num_inputs == 1 {
            ...
            //   TUPLESORT_NONE)
            tuplesort_seams::tuplesort_begin_datum::call(
                mcx,
                attr.atttypid,
                sort_operators[0],
                sort_collations[0],
            ...
            //   TUPLESORT_NONE)
            tuplesort_seams::tuplesort_begin_heap::call(
                mcx,
                sortdesc,
...
type AggDatum<'mcx> = types_tuple::heaptuple::Datum<'mcx>;
...
fn invoke_transfn<'mcx>(
    ...
    mcx: Mcx<'mcx>,
    estate_link: fmgr::fmgr::RawEStateLink,
...
/// `aggstate->ss.ps.state`) for the [`fmgr::fmgr::EStateCallContextGuard`]
...
pub(crate) fn agg_call_context_guard(
    ...
    Some(fmgr::fmgr::AggCallContextGuard::install(
...
pub fn process_ordered_aggregate_single<'mcx>(
    ...
    let mut old_val: types_tuple::heaptuple::Datum =
    ...
    let mut new_abbrev_val: types_tuple::heaptuple::Datum =
        types_tuple::heaptuple::Datum::null();
    let mut old_abbrev_val: types_tuple::heaptuple::Datum =
        types_tuple::heaptuple::Datum::null();
    ...
    tuplesort_seams::tuplesort_performsort::call(&mut state)?;
    ...
    loop {
        let (found, new_val, new_is_null) =
            tuplesort_seams::tuplesort_getdatum::call(&mut state, true, false)?;
        if !found {
            break;
        }
        ...
        new_abbrev_val = types_tuple::heaptuple::Datum::null();
    ...
    pertrans.distinct_value = types_tuple::heaptuple::Datum::null();
...
pub fn process_ordered_aggregate_multi<'mcx>(
    ...
    execTuples_seams::exec_clear_tuple::call(estate, slot1)?;
    if let Some(s2) = slot2 {
        execTuples_seams::exec_clear_tuple::call(estate, s2)?;
    }
...
fn datum_copy_into_ecxt<'mcx>(
    ...
    let _ = aggcontext;
    let mcx = estate.es_query_cxt;
    datum_seams::datum_copy_v::call(mcx, &value, typ_by_val, typ_len as i32)
}
...
fn work_mem() -> i32 {
    init_small_seams::work_mem::call()
}
...
fn pfree_datum(_d: types_tuple::heaptuple::Datum<'_>) {
...
fn transfn_input_args<'mcx>(
    ...
    alloc::vec::Vec<types_tuple::heaptuple::Datum<'mcx>>,
    alloc::vec::Vec<bool>,
) {
...
///
/// The owned `transfn_fcinfo->args[]` is the bare-word `datum::NullableDatum`
...
fn store_distinct_value<'mcx>(
    slot: &mut types_tuple::heaptuple::Datum<'mcx>,
    value: types_tuple::heaptuple::Datum<'mcx>,
) {
...
fn equalfn_one_call<'mcx>(
    pertrans: &AggStatePerTransData<'mcx>,
    old_val: types_tuple::heaptuple::Datum<'mcx>,
    new_val: types_tuple::heaptuple::Datum<'mcx>,
    estate: &EStateData<'mcx>,
) -> PgResult<bool> {
...
fn equalfn_multi_qual<'mcx>(
    ...
    execExpr_seams::exec_qual::call(equalfn_multi, tmpcontext, estate)
}
...
fn load_transfn_args_from_slot<'mcx>(
    ...
    for i in 1..=num_trans_inputs {
        let (value, isnull) =
            execTuples_seams::slot_getsomeattr::call(estate, slot, i)?;
