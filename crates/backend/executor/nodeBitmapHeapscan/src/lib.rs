...
fn BitmapDoneInitializingSharedState(node: &mut BitmapHeapScanState) -> PgResult<()> {
    ...
    condition_variable::ConditionVariableBroadcast(&pstate.cv);
    Ok(())
}
...
fn BitmapShouldInitializeSharedState(node: &mut BitmapHeapScanState) -> PgResult<bool> {
    ...
    loop {
        ...
        condition_variable::ConditionVariableSleep(
            &pstate.cv,
...
pub fn ExecBitmapHeapReInitializeDSM<'mcx>(
    node: &mut BitmapHeapScanState<'mcx>,
    _pcxt: execparallel::ParallelContextHandle,
    estate: &mut EStateData<'mcx>,
) -> PgResult<()> {
    // dsa_area *dsa = node->ss.ps.state->es_query_dsa;
...
pub fn ExecBitmapHeapInitializeWorker<'mcx>(
    ...
    let seg = execparallel::DsmSegmentHandle(0);
...
/// The `pcxt->seg` handle as the `execparallel::DsmSegmentHandle` the
...
fn chunk_advance(
    cursor: execparallel::SerializeCursor,
    off: usize,
) -> execparallel::SerializeCursor {
    execparallel::SerializeCursor(cursor.0 + off)
}
...
pub fn init_seams() {
    nodeBitmapHeapscan_seams::exec_bitmapheap_estimate::set(
        seam_exec_bitmapheap_estimate,
    );
    nodeBitmapHeapscan_seams::exec_bitmapheap_initialize_dsm::set(
        seam_exec_bitmapheap_initialize_dsm,
    );
    nodeBitmapHeapscan_seams::exec_bitmapheap_reinitialize_dsm::set(
        seam_exec_bitmapheap_reinitialize_dsm,
    );
    nodeBitmapHeapscan_seams::exec_bitmapheap_initialize_worker::set(
        seam_exec_bitmapheap_initialize_worker,
    );
    nodeBitmapHeapscan_seams::exec_bitmapheap_retrieve_instrumentation::set(
...
fn seam_exec_bitmapheap_initialize_worker(
    _node: execparallel::PlanStateHandle,
    _pwcxt: execparallel::ParallelWorkerContextHandle,
) -> PgResult<()> {
    panic!(
