...
use execParallel_seams as execParallel;
...
pub fn ExecInitGatherMerge<'mcx>(
    ...
    execUtils::ExecAssignExprContext(estate, &mut ps)?;
    ...
    execTuples::exec_init_result_type_tl::call(&mut ps, estate)?;
    {
        ...
        execUtils::ExecConditionalAssignProjectionInfo(
...
pub fn ExecGatherMerge<'mcx>(
    ...
    if !node.initialized {
        ...
        if guc_tables::vars::parallel_leader_participation.read()
            || node.nreaders == 0
        {
            node.need_to_scan_locally = true;
        }
