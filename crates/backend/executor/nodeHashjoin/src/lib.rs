...
use sharedfileset_seams as sharedfileset;
...
pub fn init_seams() {
    nodeHashjoin_seams::ExecHashJoinSaveTuple::set(exec_hash_join_save_tuple_seam);
    ...
    // ported entry point.
    nodeHashjoin_pq_seams::exec_hashjoin_estimate::set(
        exec_hashjoin_estimate_shim,
    );
    nodeHashjoin_pq_seams::exec_hashjoin_initialize_dsm::set(
        exec_hashjoin_initialize_dsm_shim,
    );
    nodeHashjoin_pq_seams::exec_hashjoin_reinitialize_dsm::set(
        exec_hashjoin_reinitialize_dsm_shim,
    );
    nodeHashjoin_pq_seams::exec_hashjoin_initialize_worker::set(
...
pub fn ExecHashJoinInitializeDSM(
    ...
    inner_hash_state(node).parallel_state = Some(chunk.0 as execparallel::DsaPointer);
    Ok(())
}
