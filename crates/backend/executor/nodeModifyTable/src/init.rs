...
pub fn ExecInitModifyTable<'mcx>(
...

    {
        ...
        if let Some(rrs) = rrs {
            for idx in 0..rrs.len() {
                ...
                let mut keep_rel = nodes_core_seams::bms_is_member::call(
                    rti as i32,
    ...
    //   }
    if node.rootRelation > 0 {
        debug_assert!(nodes_core_seams::bms_is_member::call(
    ...
    if node.onConflictAction == OnConflictAction::ONCONFLICT_UPDATE {
        ...
        let rel = estate
            ...
            .alias();
        let existing_slot = table_tableam::table_slot_create(mcx, &rel)?;
        ...
        let rel2 = estate
            ...
            .expect("ON CONFLICT result relation is open")
            .alias();
        let proj_slot_raw = table_tableam::table_slot_create(mcx, &rel2)?;
    ...
    execMain_seams::eval_plan_qual_set_plan_with_row_marks::call(
        mcx,
        &mut mtstate.mt_epqstate,
        estate,
        row_marks,
...
fn exec_init_result_relation<'mcx>(
    ...
    execMain_seams::exec_init_result_relation::call(estate, result_rel_info, rti)
}
...
fn eval_plan_qual_init<'mcx>(
    ...
    execMain_seams::eval_plan_qual_init::call(
        mcx,
        epqstate,
        estate,
        epq_param,
...
fn exec_find_junk_attribute_in_tlist(target_list: &[TargetEntry<'_>], attr_name: &str) -> i16 {
    execJunk::ExecFindJunkAttributeInTlist(target_list, attr_name)
}
...
fn exec_init_result_type_tl<'mcx>(
    ...
    execTuples_seams::exec_init_result_type_tl::call(planstate, estate)
}
