...
fn eval_merge_join_condition<'mcx>(
    ...
    let result = match cond.as_mut() {
        Some(state) => execExpr_seams::exec_qual::call(state, econtext, estate),
        None => Ok(true),
    };
        ...
        while idx < n_actions {
            ...
            match command_type {
                CmdType::CMD_UPDATE => {
                    ...
                    let projected =
                        execExpr_seams::exec_project_info::call(proj, estate)?;
                ...
                CmdType::CMD_DELETE => {
                    ...
                    if has_instead_delete_row {
                        if !trigger_seams::exec_ir_delete_triggers::call(
                            estate,
                            result_rel_info,
                            oldtuple.clone(),
                        )? {
            ...
            match result {
                ...
                TM_Result::TM_Updated => {
                    ...
                    let lockmode = execMain_seams::exec_update_lock_mode::call(
                        estate,
                        result_rel_info,
                    )?;
                    ...
                    let lock_result = table_tableam::table_tuple_lock(
                        mcx,
                        &rel,
                        &tid,
                        &snapshot,
                    ...
                    match lock_result {
                        TM_Result::TM_Ok => {
                            ...
                            if was_matched {
                                ...
                                let junk = execTuples_seams::slot_getattr_by_id::call(
                                ...
                                if !*matched {
                                    ...
                                    if have_both_not_matched {
                                        if let Some(outer) = mtstate.ps.lefttree.as_mut() {
                                            ...
                                            {
                                                instrument_seams::instr_update_tuple_count::call(
                                                    instr, 1.0,
                                                );
                                            }
                                        }
...
fn finish<'mcx>(
    ...
    if let Some(tid) = lockedtid {
        ...
        lmgr_seams::unlock_tuple::call(oid, tid, INPLACE_UPDATE_TUPLE_LOCK)?;
    }
    Ok(rslot)
}
