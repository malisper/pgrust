...
pub fn ExecUpdate<'mcx>(
    ...
    if trig_instead {
        ...
        if !trigger_seams::exec_ir_update_triggers::call(
            estate,
            result_rel_info,
            oldtuple.clone(),
            slot,
        ...
        match execMain_seams::exec_foreign_update::call(
            estate,
            result_rel_info,
            slot,
            plan_slot,
        ...
        let result: TM_Result = loop {
            ...
            match r {
                ...
                TM_Result::TM_Updated => {
                    ...
                    let inputslot = execMain_seams::eval_plan_qual_slot::call(
                        estate,
                    ...
                    let r2 = table_tableam::table_tuple_lock(
                        mcx,
                        &rel,
                        &cur_tid,
                        &snapshot,
                    ...
                    match r2 {
                        TM_Result::TM_Ok => {
                            ...
                            let epqslot = execMain_seams::eval_plan_qual::call(
                                estate,
                                &mut mtstate.mt_epqstate,
                                result_rel_info,
                                rti,
...
pub fn ExecUpdatePrologue<'mcx>(
    ...
    if trig_before {
        ...
        return trigger_seams::exec_br_update_triggers::call(
            estate,
            &mut mtstate.mt_epqstate,
            result_rel_info,
            tupleid,
...
pub fn ExecCrossPartitionUpdate<'mcx>(
    ...
    *retry_slot = None;
    ...
    if Some(result_rel_info) == mtstate.rootResultRelInfo {
        execMain_seams::exec_partition_check_emit_error::call(
            estate,
            result_rel_info,
            slot,
        )?;
    ...
    if mtstate.mt_partition_tuple_routing.is_none() {
        ...
        let proute = execPartition_seams::exec_setup_partition_tuple_routing::call(
            qcxt,
            estate,
            root_rel.alias(),
        )?;
        ...
        let root_slot = table_tableam::table_slot_create(qcxt, &root_rel)?;
    ...
    if execMain_seams::exec_get_child_to_root_map::call(estate, result_rel_info)? {
        let root_slot = mtstate
            .mt_root_tuple_slot
            .expect("ExecCrossPartitionUpdate: mt_root_tuple_slot not set");
        slot = execTuples_seams::execute_attr_map_slot::call(
            estate,
            result_rel_info,
            slot,
            root_slot,
...
pub fn ExecCrossPartitionUpdateForeignKey<'mcx>(
    ...
    let ancestor_rels = execMain_seams::exec_get_ancestor_result_rels::call(
        mcx,
        estate,
        source_part_info,
    )?;
    ...
    for r_info in ancestor_rels.iter().copied() {
        ...
        if estate.result_rel(r_info).ri_trig_update_after_row {
            has_noncloned_fkey =
                trigger_seams::has_noncloned_pk_fkey_trigger::call(
                    estate, r_info,
                )?;
        }
...
pub fn ExecGetUpdateNewTuple<'mcx>(
    ...
    execExpr_seams::exec_project_new_tuple::call(estate, relinfo, plan_slot, old_slot)
}
...
pub fn ExecInitUpdateProjection<'mcx>(
    ...
    let rel = relation_alias(estate, result_rel_info);
    let old_tuple_slot = table_tableam::table_slot_create(mcx, &rel)?;
    ...
    execExpr_seams::exec_build_update_projection::call(
        mtstate,
        estate,
        result_rel_info,
        &update_colnos,
