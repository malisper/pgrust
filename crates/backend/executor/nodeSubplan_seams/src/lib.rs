...
seam_core::seam!(
    ...
    pub fn exec_re_scan_set_param_plan<'mcx>(
        node: &mut nodes::SubPlanState<'mcx>,
        parent_chg_param: &mut Option<mcx::PgBox<'mcx, nodes::Bitmapset<'mcx>>>,
...
seam_core::seam!(
    ...
    /// out-of-line [`SubPlanState`](nodes::SubPlanState) for a `SubPlan`
    ...
    pub fn exec_init_sub_plan<'mcx>(
        subplan: mcx::PgBox<'mcx, nodes::primnodes::SubPlan<'mcx>>,
        estate: &mut nodes::EStateData<'mcx>,
    ) -> types_error::PgResult<nodes::SubPlanState<'mcx>>
);
