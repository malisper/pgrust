...
use toastdesc_seams as tupdesc;
...
fn tuple_datum_to_carrier(
    d: &Datum<'_>,
) -> (datum::Datum, Option<alloc::vec::Vec<u8>>) {
    match d {
        Datum::ByVal(w) => (datum::Datum::from_usize(*w), None),
        _ => (datum::Datum::null(), Some(d.as_ref_bytes().to_vec())),
    }
}
...
fn build_json_table_plan(
    ...
    if let Some(scan) = node.as_jsontablepathscan() {
        ...
        Ok(jsonpath_exec::JsonTablePlan::PathScan(
            jsonpath_exec_seams::JsonTablePathScan {
                path,
        ...
        Ok(jsonpath_exec::JsonTablePlan::SiblingJoin(
            jsonpath_exec_seams::JsonTableSiblingJoin { lplan, rplan },
        ))
    } else {
