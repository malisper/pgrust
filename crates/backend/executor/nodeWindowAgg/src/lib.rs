...
fn initialize_peragg<'mcx>(
    ...
    aggtranstype = parse_agg_seams::resolve_aggregate_transtype::call(
        mcx,
        winfnoid,
        aggtranstype,
        &input_types,
    ...
    if oid_is_valid(invtransfn_oid) {
        let mut invtransfn = fmgr_seams::fmgr_info::call(mcx, invtransfn_oid)?;
        let inv = invtransfnexpr.as_ref().expect(
            "build_aggregate_transfn_expr did not return an inverse transfn expr",
        );
        fmgr_seams::fmgr_info_set_expr::call(mcx, &mut invtransfn, inv)?;
    ...
    if oid_is_valid(finalfn_oid) {
        ...
        let mut finalfn = fmgr_seams::fmgr_info::call(mcx, finalfn_oid)?;
        fmgr_seams::fmgr_info_set_expr::call(mcx, &mut finalfn, &finalfnexpr)?;
...
fn contain_volatile_functions_wfunc<'mcx>(
    ...
    let wfunc = pf.wfunc.as_ref().expect("perfunc.wfunc not set");
    for arg in wfunc.args.iter() {
        if path_small_seams::contain_volatile_functions_expr::call(arg) {
            return Ok(true);
        }
    }
    if let Some(filter) = wfunc.aggfilter.as_deref() {
        if path_small_seams::contain_volatile_functions_expr::call(filter) {
            return Ok(true);
        }
    }
    Ok(false)
...
fn contain_subplans_wfunc<'mcx>(
    ...
    let pf = perfunc_ref(winstate, perfuncno);
    let wfunc = pf.wfunc.as_ref().expect("perfunc.wfunc not set");
    if clauses_seams::contain_subplans::call(&wfunc.args) {
        return Ok(true);
    }
    if let Some(filter) = wfunc.aggfilter.as_deref() {
        let one = core::slice::from_ref(filter);
        if clauses_seams::contain_subplans::call(one) {
            return Ok(true);
        }
    }
    Ok(false)
