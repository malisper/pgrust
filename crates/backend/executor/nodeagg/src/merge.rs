...
// Relocated from crate lanefold at its deletion (P7-2 D-10) — the two live
// utilities the classic parallel merge kept using: the memcmp-tier collation
// proof for min/max(text) byref combines, and the no-nulls 1-D int8
// transarray payload offset the avg/sum(int2|int4) combine reads through.
const ARR_OVERHEAD_NONULLS_1: usize = 24;

/// Memcmp-tier collation: C/POSIX, or a deterministic collate-is-C default
/// locale — the grant under which text min/max combine order is invisible.
fn str_collation_safe(collid: ::types_core::Oid) -> bool {
    use ::types_core::catalog::{C_COLLATION_OID, DEFAULT_COLLATION_OID, POSIX_COLLATION_OID};
...
impl AggTableHandoff {
    fn new(kinds: Vec<CombineKind>) -> AggTableHandoff {
        AggTableHandoff { slots: Mutex::new(HandoffSlots::default()), kinds }
    }
...
pub(crate) fn init_finalize_merge<'mcx>(
    ...
    for t in 0..numtrans {
            ...
            && merge_byref_kinds_enabled()
        {
            ...
            && str_collation_safe(aggref.inputcollid)
            && merge_byref_kinds_enabled()
        {
    ...
    if merge_stats_enabled() {
        eprintln!(
            "AGG_MERGE_STATS engage: node={} kinds={:?} par={}",
...
// `entry_state_bytes` over a bare pergroup array (state blocks carrying the
// same `kinds.len()` pergroups).
//
// # Safety
...
fn install_classic_handoff<'mcx>(
    ...
    let handed = export_handed_table(
        ...
        &handoff.kinds,
        id,
        // P7-2 D-8: the Stage-4 armed-pool worker-side pre-partition rode
        // pgrust.lane_parallel_pool (tombstoned, no effect) — the classic
        // leader-side partitioning is the only arm, byte-for-byte the
        // unarmed behavior.
        false,
    );
...
// `synth_state_vals` over a bare pergroup block.
//
// # Safety
...
fn parallel_merge(
    ...
    // the classic engaged path) plus the leader. (P7-2 D-8: the armed-pool
    // DOP clamp rode pgrust.lane_parallel_pool — tombstoned, no effect; the
    // unarmed arm is the only arm.)
