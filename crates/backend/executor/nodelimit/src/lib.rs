...
fn eval_limit_expr<'mcx>(
    ...
    // (nodeSubplan.c ExecSetParamPlan); the suspension driver services both
    // inline SubPlans and pending-initplan $n fetches on demand, so
    // LIMIT (SELECT n) resolves here (#167) at first fetch, C-lazily.
