...
fn descriptor<'m>(mcx: mcx::Mcx<'m>, checks: bool) -> Rc<TupleDescData<'m>> {
    ...
    if checks {
        check.push(types_tuple::ConstrCheck {
            ccname: None,
            ccbin: None,
            ccenforced: true,
            ccvalid: true,
    ...
    Rc::new(TupleDescData {
        ...
        attrs,
        compact_attrs,
        constr: Some(mcx::box_new_in(
            mcx,
            TupleConstr {
                defval: mcx::PgVec::new_in(mcx),
                check,
                missing: mcx::PgVec::new_in(mcx),
                num_defval: 0,
                num_check: u16::from(checks),
                relchecks: i16::from(checks),
                has_not_null: false,
...
fn constant<'m>(mcx: mcx::Mcx<'m>, image: &[u8]) -> PgBox<'m, ExprState<'m>> {
    let node = types_nodes::Node::mk_const(
        mcx,
        25,
        -1,
        0,
    ...
    .unwrap();
    execexpr::exec_init_expr(mcx, Some(node), execexpr::ParamBind::NONE)
        .unwrap()
        .unwrap()
}
...
fn generated_slot_owns_values_after_short_row_drop() {
    ...
    for kind in [
        ...
        exprs.push(GeneratedExpr {
            attnum: 0,
            state: constant(mcx, &image),
        });
        ...
        for _ in 0..3 {
            ...
            exectuples::exec_store_virtual_tuple(&mut slot);
            {
                ...
                exec_compute_stored_generated_in_row(mcx, row.mcx(), &mut exprs, &rel, &mut slot)
                    .unwrap();
            }
            let mut null = true;
            ...
            let bytes = unsafe {
                ...
                let header = if types_tuple::varatt::varatt_is_1b(ptr) {
                    1
                } else {
                    4
                };
...
fn relation<'m>(mcx: ::mcx::Mcx<'m>, desc: Rc<TupleDescData<'m>>) -> Relation<'m> {
...
fn operator<'m>(
    ...
    func: u32,
    ty: u32,
    left: types_nodes::Node<'m>,
    right: types_nodes::Node<'m>,
) -> types_nodes::Node<'m> {
    types_nodes::Node::mk(
        mcx,
        types_nodes::primnodes::OpExpr {
            ...
            opcollid: 0,
            inputcollid: 950,
            args: types_nodes::list::NodeList::make2(mcx, left, right).unwrap(),
            location: -1,
        },
    )
    .unwrap()
...
fn check_allocating_expression_reuses_cache_after_short_row_drop() {
    ...
    let input = types_nodes::Node::mk_const(
        mcx,
        25,
        -1,
        0,
    ...
    let state = execexpr::exec_init_expr(mcx, Some(equality), execexpr::ParamBind::NONE)
        .unwrap()
        .unwrap();
    let mut checks = mcx::PgVec::new_in(mcx);
    checks.push(CheckExpr {
        name: mcx::PgString::from_str_in("same", mcx).unwrap(),
        state: Some(state),
    });
