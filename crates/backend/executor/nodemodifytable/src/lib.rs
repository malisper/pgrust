...
// ===== WAVE-9 APPEND REGION (WS-AG fusion D1a) — DELETED at RB-R1 ==========
// The rowmode-endgame §2.2 accept-seam decomposition lived here: the
// MT_ROWCHAIN_* mask bits, `mt_rowchain_shape_mask`, and the five
// `mt_ins_*` seams (stage / br_triggers / write / epilogue / returning) —
// mirror-decompositions of `mt_accept_row`/`exec_insert` whose SOLE
// consumer was the stitched trigger-INSERT chain host (lanev2/dml.rs,
// deleted in the same commit; grep-proven sole-consumer per the RB-R1
// recipe). `mt_accept_row`/`exec_insert` themselves are UNTOUCHED — the
// production per-row path never called these seams.
