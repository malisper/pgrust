...
use crate::lifecycle::QueryTaskGuard;
...
impl Scheduler {
    ...
    fn run_task(&self, local: &mut WorkerLocal, ts: &Arc<TaskSetRt>) -> bool {
        ...
        // Generation gate (H1): a task of an aborted generation is
        // unconsumable — no guard, no morsel. The exhausted path then drives
        // ordinary invalidate/finalize cleanup.
        let exhausted = match QueryTaskGuard::enter(&ts.rg.lifecycle, ts.rg.generation) {
            None => {
            ...
            Some(guard) => {
                ...
                loop {
                    ...
                    ts.work().run_morsel(local.worker, range);
