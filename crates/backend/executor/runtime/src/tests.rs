...
fn lifecycle_semantics() {
    ...
    // Normal join/leave.
    let guard = QueryTaskGuard::enter(&lc, g0).expect("open generation admits");
    assert_eq!(lc.active(), 1);
    assert!(!guard.aborted());
    // Retire refuses while a participant is inside.
    assert_eq!(lc.retire(g0), None);
    drop(guard);
    assert_eq!(lc.active(), 0);

    // Abort: unconsumable by construction, idempotent.
    assert!(lc.abort(g0));
    assert!(lc.abort(g0));
    assert!(QueryTaskGuard::enter(&lc, g0).is_none(), "aborted generation admits nobody");
    assert!(lc.is_aborted(g0));

    // Retire the aborted generation, open the next.
    let g1 = lc.retire(g0).expect("drained generation retires");
    assert_eq!(g1, lc.current_generation());
    assert_ne!(g0, g1);
    // The dead generation stays unconsumable forever.
    assert!(QueryTaskGuard::enter(&lc, g0).is_none());
    assert!(lc.is_aborted(g0));
    // Aborting a retired generation is a no-op refusal.
    assert!(!lc.abort(g0));
    // The new generation admits.
    let guard = QueryTaskGuard::enter(&lc, g1).expect("new generation admits");
    assert!(!guard.aborted());
    drop(guard);
}
