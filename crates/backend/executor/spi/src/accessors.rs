...
/// `SPI_result`.
///
/// [`SystemAttributeByName`]: heap::SystemAttributeByName
pub fn SPI_fnumber(tupdesc: &TupleDescData<'_>, fname: &[u8]) -> i32 {
    ...
    if let Some(sysatt) = heap::SystemAttributeByName(fname) {
...
/// column).
///
/// [`SystemAttributeDefinition`]: heap::SystemAttributeDefinition
pub fn SPI_gettypeid(tupdesc: &TupleDescData<'_>, fnumber: i32) -> PgResult<Oid> {
    ...
    if fnumber > 0 {
        ...
        Ok(heap::SystemAttributeDefinition(fnumber as i16)?.atttypid)
    }
}
...
pub fn SPI_getvalue<'mcx>(
    ...
    let typoid = if fnumber > 0 {
        ...
        heap::SystemAttributeDefinition(fnumber as i16)?.atttypid
    };

    // getTypeOutputInfo(typoid, &foutoid, &typisvarlena);
    let (foutoid, _typisvarlena) =
        lsyscache_seams::get_type_output_info::call(typoid)?;
    // return OidOutputFunctionCall(foutoid, val);
    let bytes =
        fmgr_seams::oid_output_function_call::call(mcx, foutoid, &val)?;
    Ok(Some(bytes))
}
