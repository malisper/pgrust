...
pub fn spi_eval_expr(
    ...
    utils_error::pop_emit_context_callback(prepare_cb_id);
...
fn prepare_expr_plan<'mcx>(
    ...
    let raw_list = driver::raw_parser(mcx, query, parse_mode(parsemode))?;
    ...
    let query_node = parser_analyze::parse_analyze_plpgsql_expr(
        mcx,
        parsetree,
        query,
        parse_state,
    )?;
    let querytree_list = rewritehandler::QueryRewrite(mcx, query_node)?;
