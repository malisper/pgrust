...
pub(crate) fn prepare_dynexecute_plan<'mcx>(
    ...
    let raw_list = driver::raw_parser(mcx, query, RawParseMode::RAW_PARSE_DEFAULT)?;
    ...
    let query_node = parser_analyze::parse_analyze_fixedparams(
        mcx,
        parsetree,
        query,
        param_types,
...
fn check_must_return_tuples(source: SourceHandle) -> PgResult<()> {
    ...
    let cmdtag = if tag == portal::CMDTAG_SELECT {
        "SELECT INTO"
    } else {
...
pub(crate) fn prepare_execsql_plan<'mcx>(
    ...
    let raw_list = driver::raw_parser(mcx, query, parsemode)?;
    ...
    let query_node =
        parser_analyze::parse_analyze_plpgsql_expr(mcx, parsetree, query, parse_state)?;
    let querytree_list = rewritehandler::QueryRewrite(mcx, query_node)?;
