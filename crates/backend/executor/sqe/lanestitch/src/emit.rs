// AArch64 word emitter: literal pool + label fixups + branch patching,
// trimmed to the encodings the stitched bodies need. Every non-trivial
// encoding is pinned against clang in the test table below — those byte
// assertions run on every host (including macOS, where emitted Tier B
// bodies are never dispatched in production).
...
// mnemonic — the golden byte-emission reference for the stencil encodings.
#[cfg(test)]
mod tests {
    use super::Emitter;
