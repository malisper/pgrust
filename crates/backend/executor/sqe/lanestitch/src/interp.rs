// The loop-inside reference interpreter — the executable Step-IR
// specification (step-ir.md §2, R2). CI PARITY ORACLE ONLY: this module
// compiles solely under the `oracle` feature and is reachable from no
// production path. In production, an erroring stitched body
// refuses-and-replays onto the AOT error-kernel path (production-plan §3,
// a standing P1-3 obligation gated by the error corpus); in CI this
// oracle stands in as the replay target so the parity suites can assert
// error identity and position.
...
use crate::spec::{
    ArithOp, Batch, BoolTestKind, NullTestKind, OutLane, Program, SelVec, Step, MAX_REGS,
    MAX_ROWS,
};
...
// Width-specific out-of-range messages, byte-identical to the C integer
// kernels. Stitched code never fabricates these — the replay path raises
// the exact message on the exact row.
#[track_caller]
#[cold]
#[inline(never)]
...
/// Checked add/sub/mul (width-exact overflow) plus the div traps (zero
/// divisor, MIN/-1). Reads/writes at the op's width; the Datum image
/// stays canonically sign-extended.
#[inline(always)]
...
/// One step at row `i` over the register file — the single copy of the
/// per-row step semantics both walkers share.
#[inline(always)]
fn eval_step(
    prog: &Program,
    batch: &Batch<'_>,
    ...
    match *step {
        ...
        Step::SaopAny { a, out, op, arr } => {
            // Strict-OR three-valued: scan for a non-NULL matching element
            // (short-circuits true); NULL scalar or a NULL element with no
            // match yields NULL.
...
/// [`eval_row`] with projection output lanes; qual programs pass `&mut []`.
#[inline(always)]
pub fn eval_row_outs(
    ...
    for step in &prog.steps {
        if eval_step(prog, batch, i, step, &mut regs, outs)? == Flow::QualFail {
            return Ok(false);
        }
    }
    Ok(true)
...
/// Reference projection over one staged batch: every SELECTED row
/// (ascending) runs the whole program, StoreOut writing the output lanes.
/// Non-selected rows' cells are untouched. On error, outputs for rows
/// before the erroring row are final; the caller discards them.
pub fn eval_project(
    prog: &Program,
    batch: &Batch<'_>,
    sel: &SelVec,
...
/// Reference qual over one staged batch: ascending rows, failing rows'
/// sel bits cleared. On error, bits for rows before the erroring row are
/// final (the parity contract's error-position currency).
