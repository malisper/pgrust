// The stitcher: compiles one Step-IR qual/projection program into ONE
// fused AArch64 body (batch in, selection bits / output lanes out). The
// row loop is INSIDE the body; per-step snippets are straight-line
// stencils. No register allocation across stencils: the virtual register
// file lives in the frame at fixed offsets ([sp + r*16] value,
// [sp + r*16 + 8] isnull). Callee-saved conventions: x19 = params block,
// x20 = row index, x21 = nrows, x23 = sel words, x25-x28 = hoisted lane
// base pointers of the first two used columns.
//
// Clause fusion: whole non-erroring clause shapes ([lane, const, cmp,
// qual], [lane, lane, cmp, qual], ...) emit one fused stencil with no
// register-file round trip — legal only when the window's output
// registers are dead downstream (regs_dead_after).
//
// Semantics: steps execute in program order per row, ascending row order
// — clause order, short-circuit, and refusal positions match the oracle
// by construction. Erroring (Arith) stencils carry refuse-and-replay: an
// overflow or zero divisor branches to the body's refuse exit instead of
// constructing an error — the DRIVER replays the batch on the
// error-owning path (production: the AOT error-kernel path,
// production-plan §3; CI: the oracle interpreter). No Rust helper is
// reachable from a stitched body at all.
//
// Fail-closed classification: `plan_clauses` / `plan_project` are
// exhaustive over Step and CmpOp with no wildcard admission — an
// unclassified shape refuses to compile, a typed refusal at the caller.
// Float comparators fuse only where the fcmp conds / NEON compares are
// provably exact (see float_cond and the FCmpVar masks).
//
// SIMD tier (classify_simd): when every clause is pure and non-erroring,
// the body runs 64-row blocks — NEON stencils build a 64-bit pass word,
// non-vector clauses run per-row over surviving bits in ascending order.
// Legality: the qual is an AND of pure clauses, so evaluation order
// across rows and clauses is unobservable and there are no errors to
// reorder; any Arith clause refuses the tier and the scalar row loop
// keeps exact per-row refuse order.
...
/// Body exit codes (x0): 0 = batch fully consumed; RC_REFUSE = an
/// erroring stencil tripped (overflow / zero divisor) — the driver must
/// replay the batch on the error-owning path (refuse-and-replay).
...
/// stitches and how each clause renders. None = refuse — a typed refusal
/// at the caller. Exhaustive over Step and CmpOp, no wildcard admission.
            ...
            window => {
                ...
                for step in window {
                    match *step {
                        ...
                        // Projection-only step: fail closed in a qual body.
...
// SVE survivor-extraction frame extension: 64 u32 index slots plus
// MAX-VL ST1W overstore slack (only the CNTP-counted prefix is
// meaningful); tier detection refuses VL > SVE_MAX_VL_BYTES so the
// slack bound holds.
...
/// Adaptive crossover: the SVE COMPACT survivor-extraction path engages
/// for a block when the vector pass leaves at least this many survivors
/// of 64 (measured whole-pipeline break-even; below it the NEON
/// rbit/clz bit iteration wins).
pub(crate) const SVE_SURVIVOR_CROSSOVER: u32 = 16;

/// Per-clause cap on SVE2 MATCH IN-list candidates (8 u16 candidates per
/// 128-bit MATCH segment).
...
//
// Boot-time HWCAP gate: the SVE2 tier engages only when the kernel
// reports SVE + SVE2 and the vector length fits the survivor buffer's
// overstore slack. Apple Silicon and non-SVE parts read false and keep
// the NEON tier. Emitted SVE stencils are VL-agnostic (ptrue-pattern
// governance); a wider part only needs SVE_MAX_VL_BYTES raised.
...
/// The active SIMD stencil tier for one compile. `Off` pins NEON on SVE2
/// hardware (A/B arms); `Force` drops the adaptive survivor crossover to
/// 0 for parity coverage of the SVE stencils at any selectivity. Both are
/// inert off-SVE2-hardware — the tier never engages where it cannot run.
...
pub(crate) fn simd_tier(pin: crate::Sve2Pin) -> SimdTier {
    if !sve2_hw() {
        return SimdTier::Neon;
    }
    match pin {
        crate::Sve2Pin::Off => SimdTier::Neon,
        crate::Sve2Pin::Force => SimdTier::Sve2 { force: true },
        crate::Sve2Pin::Auto => SimdTier::Sve2 { force: false },
    }
}
...
/// Admission: Eq relation, 1..=MAX_MATCH_ELEMS non-NULL elements, every
/// element in [0, 0xFFFF] as its canonical Datum image. Exactness: datum
/// == elem iff low16 MATCHes AND the upper 48 datum bits are zero — the
/// stencil ANDs the MATCH mask with that in-domain mask, so out-of-domain
/// values (incl. sign-extended negatives) never false-match.
...
fn emit_step(e: &mut Emitter, ctx: &Ctx<'_>, prog: &Program, step: &Step) {
    match *step {
        ...
        Step::Arith { op, a, b, out } => {
            ...
            // RC_REFUSE — no stitched error construction (the driver's
...
/// True = this clause has a NEON vector-pass arm in emit_simd_blocks;
/// false = inside a SIMD body it runs per-row over the pass word's
/// surviving bits (Generic clauses). SaopQ is vector-pass at every
/// admitted length (measured; no separate SIMD cap exists).
...
pub(crate) fn classify_simd(plan: &Plan) -> bool {
    if plan.has_arith {
        return false;
    }
    let mut nconst = 0u32;
...
fn emit_simd_blocks(e: &mut Emitter, ctx: &Ctx<'_>, prog: &Program, plan: &Plan, tier: SimdTier) {
        ...
        match *c {
            ...
            ClauseShape::SaopQ { col, op, arr } => {
                if !match_regs_of[ix].is_empty() {
                    // SVE2 MATCH stencil: 8 u16 candidates per MATCH vs
                    // the NEON per-candidate CMEQ+ORR below.
    ...
    if !generics.is_empty() {
        if let SimdTier::Sve2 { force } = tier {
            // Adaptive survivor-emit tier: at or above the crossover,
            // extract survivor indices densely via COMPACT/CNTP and
            // iterate the list; below it the NEON rbit/clz bit iteration
...
/// SVE COMPACT survivor extraction: densifies the block's pass
...
/// Compile-time introspection: (the SVE COMPACT survivor path was
/// emitted, number of clauses on the SVE2 MATCH stencil). Both zero on
/// non-SVE2 hardware or with the tier pinned off.
pub(crate) fn plan_sve2_info(
    prog: &Program,
    plan: &Plan,
    simd: bool,
    tier: SimdTier,
...
// staged batch (the qual segment's selection bitmap is the input
// currency): straight-line steps ending in StoreOut, NO Qual steps. One
// scalar row loop — test the sel bit, skip clear rows, run the stencils,
// store outputs. Arith traps take the same refuse-and-replay exit as the
// qual body: RC_REFUSE with no error constructed; the driver replays the
// batch on the error-owning per-row path (outputs written before the trap
// are discarded — the consumer contract only covers an RC_OK batch).
...
/// Fail-closed classification for projection programs: exhaustive over
/// Step, no wildcard admission. Refuses any Qual step, float compares
/// (no NaN-exact generic cond), missing StoreOut, and every bound
/// violation.
