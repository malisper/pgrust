    ...
    for (i, (val, null)) in cols.into_iter().enumerate() {
        ...
        if !attisdropped && attlen == -1 && !null {
            ...
            if is_external {
                ...
                let fetched = detoast_seams::detoast_external_attr::call(
                    mcx,
                    out.as_ref_bytes(),
                )?;
...
fn put_row<'mcx>(
    ...
    sort_storage_seams::tuplestore_putvalues::call(store, tdesc, values, isnull)
}
...
pub fn init_seams() {
    tstorereceiver_seams::create_dest_receiver_tuplestore::set(|| {
        Ok(CreateTuplestoreDestReceiver())
    });
    tstorereceiver_seams::set_tuplestore_dest_receiver_params::set(
        SetTuplestoreDestReceiverParams,
    );
    tstorereceiver_seams::dest_destroy::set(tstore_destroy_receiver);
}
