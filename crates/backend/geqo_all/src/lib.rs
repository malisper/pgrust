...
//! [`geqo_all_seams`]. The clump-merging *algorithm*
...
/// [`prng::PgPrng`], the analogue of `pg_prng_state`).
#[derive(Clone, Debug, Default)]
pub struct GeqoPrivateData {
    pub initial_rels: alloc::vec::Vec<RelId>,
    pub random_state: prng::PgPrng,
}
...
/// `have_join_order_restriction`) live in [`geqo_all_seams`] and are
/// installed by their owners (joinrels/joininfo/allpaths), not here.
///
/// The remaining two seams in [`geqo_all_seams`] —
...
pub fn init_seams() {
    guc_state::install();
    geqo_all_seams::geqo_eval_context_create::set(eval::geqo_eval_context_create);
    geqo_all_seams::geqo_eval_context_delete::set(eval::geqo_eval_context_delete);
}
