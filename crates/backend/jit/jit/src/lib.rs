...
fn pkglib_path() -> String {
    let bytes = init_small::globals::pkglib_path();
...
/// `seams-init::init_all()`.
pub fn init_seams() {
    jit_seams::jit_release_context::set(jit_release_context);
    jit_seams::jit_reset_after_error::set(jit_reset_after_error);
