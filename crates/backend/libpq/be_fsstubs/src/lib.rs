...
//! ([`be_fsstubs_seams`], panic until their owners land): the
...
pub fn be_lo_open(lobjId: Oid, mode: i32) -> PgResult<i32> {
    ...
    if mode & INV_WRITE != 0 {
        transam_xact_seams::prevent_command_if_read_only::call("lo_open(INV_WRITE)")?;
    }
    ...
    lobjDesc.subid = transam_xact_seams::get_current_sub_transaction_id::call();
...
pub fn be_lo_creat() -> PgResult<Oid> {
    ...
    transam_xact_seams::prevent_command_if_read_only::call("lo_creat()")?;
...
pub fn be_lo_create(mut lobjId: Oid) -> PgResult<Oid> {
    transam_xact_seams::prevent_command_if_read_only::call("lo_create()")?;
...
pub fn be_lo_unlink(lobjId: Oid) -> PgResult<i32> {
    transam_xact_seams::prevent_command_if_read_only::call("lo_unlink()")?;
...
pub fn be_lowrite(fd: i32, wbuf: &[u8]) -> PgResult<i32> {
    ...
    transam_xact_seams::prevent_command_if_read_only::call("lowrite()")?;
...
fn lo_import_internal(filename: &[u8], lobjOid: Oid) -> PgResult<Oid> {
    ...
    transam_xact_seams::prevent_command_if_read_only::call("lo_import()")?;
...
pub fn be_lo_truncate(fd: i32, len: i32) -> PgResult<i32> {
    transam_xact_seams::prevent_command_if_read_only::call("lo_truncate()")?;
...
pub fn be_lo_truncate64(fd: i32, len: int64) -> PgResult<i32> {
    transam_xact_seams::prevent_command_if_read_only::call("lo_truncate64()")?;
...
pub fn be_lo_from_bytea(mut loOid: Oid, str: &[u8]) -> PgResult<Oid> {
    ...
    transam_xact_seams::prevent_command_if_read_only::call("lo_from_bytea()")?;
...
pub fn be_lo_put(loOid: Oid, offset: int64, str: &[u8]) -> PgResult<()> {
    ...
    transam_xact_seams::prevent_command_if_read_only::call("lo_put()")?;
...
pub fn init_seams() {
    ...
    be_fsstubs_seams::import_server_file::set(import_server_file);
    be_fsstubs_seams::export_server_file::set(export_server_file);
