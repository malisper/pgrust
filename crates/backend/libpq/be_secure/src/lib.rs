...
//! backends: the `#ifdef USE_SSL` arm into [`be_secure_openssl`]
...
pub fn secure_open_server(port: &mut Port) -> PgResult<i32> {
    if USE_SSL {
        ...
        let len = pqcomm_seams::pq_buffer_remaining_data::call();
        if len > 0 {
            ...
            pqcomm_seams::pq_endmsgread::call();
            port.raw_buf = Some(buf);
            port.raw_buf_remaining = len;
            port.raw_buf_consumed = 0;
        }
        debug_assert_eq!(pqcomm_seams::pq_buffer_remaining_data::call(), 0);
...
pub fn secure_read(port: &mut Port, buf: &mut [u8]) -> PgResult<SockResult> {
    ...
    loop {
        ...
        if n < 0 && !port.noblock && would_block(arm_errno) {
            ...
            pqcomm_seams::modify_fe_be_wait_set_socket::call(waitfor as u32);

            // WaitEventSetWait(FeBeWaitSet, -1, &event, 1, WAIT_EVENT_CLIENT_READ);
            let (_nev, events) =
                pqcomm_seams::wait_event_set_wait_fe_be::call(-1, WAIT_EVENT_CLIENT_READ);
...
pub fn secure_write(port: &mut Port, buf: &[u8]) -> PgResult<SockResult> {
    ...
    loop {
        ...
        if n < 0 && !port.noblock && would_block(arm_errno) {
            ...
            let (_nev, events) = pqcomm_seams::wait_event_set_wait_fe_be::call(
                -1,
...
fn be_tls_open_server(port: &mut Port) -> PgResult<i32> {
    ...
    let res = tls::be_tls_open_server(port, |_waitfor| {
        ...
        let _ = sock;
        pqcomm_seams::modify_fe_be_wait_set_socket::call(_waitfor);
        let _ = pqcomm_seams::wait_event_set_wait_fe_be::call(
            -1,
...
pub fn init_seams() {
    ...
    auth_seams::secure_loaded_verify_locations::set(secure_loaded_verify_locations);
