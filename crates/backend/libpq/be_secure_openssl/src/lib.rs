...
//! [`be_secure_openssl_ffi_seams`] FFI boundary. That seam crate
...
fn port_bio_read(port_token: u64, size: usize) -> (isize, Vec<u8>) {
    ...
    let n = be_secure_seams::secure_raw_read::call(port, &mut buf);
    if n > 0 {
...
fn port_bio_write(port_token: u64, buf: &[u8]) -> isize {
    ...
    let port: &mut Port = unsafe { &mut *p };
    be_secure_seams::secure_raw_write::call(port, buf)
}
...
pub fn init_bio_bridge_seams() {
    be_secure_openssl_ffi_seams::port_bio_read::set(port_bio_read);
    be_secure_openssl_ffi_seams::port_bio_write::set(port_bio_write);
}
...
fn be_tls_init_inner(
    ...
    if !be_secure_common_seams::check_ssl_key_file_permissions::call(
        &ssl_key_file,
        is_server_start,
    )? {
        return Ok(Err(()));
...
pub fn ssl_external_passwd_cb<'mcx>(mcx: Mcx<'mcx>, size: i32) -> PgResult<::mcx::PgVec<'mcx, u8>> {
    ...
    be_secure_common_seams::run_ssl_passphrase_command::call(
        mcx,
        PROMPT,
        is_server_start,
        size,
...
fn get_config_option(name: &str) -> String {
    guc_seams::get_config_option::call(name.to_string(), false, false)
        .unwrap_or_default()
}
...
pub fn init_seams() {
    be_secure_seams::be_tls_get_version::set(be_tls_get_version);
    be_secure_seams::be_tls_get_cipher::set(be_tls_get_cipher);
    be_secure_seams::be_tls_get_cipher_bits::set(be_tls_get_cipher_bits);
