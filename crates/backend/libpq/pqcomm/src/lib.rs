...
//!   the `init_small_seams::with_my_proc_port` callback seam.
...
fn with_my_proc_port(f: &mut dyn FnMut(Option<&mut Port>)) {
    init_small_seams::with_my_proc_port::call(f);
}
...
fn secure_read_my_port(buf: &mut [u8]) -> PgResult<SockResult> {
    ...
    with_my_proc_port(&mut |port| {
        if let Some(port) = port {
            res = Some(be_secure_seams::secure_read::call(port, buf));
        }
    });
...
fn secure_write_my_port(buf: &[u8]) -> PgResult<SockResult> {
    ...
    with_my_proc_port(&mut |port| {
        if let Some(port) = port {
            res = Some(be_secure_seams::secure_write::call(port, buf));
        }
    });
...
pub fn pq_init(client_sock: &ClientSocket, my_latch: LatchHandle) -> PgResult<Port> {
    ...
    dsm_core_seams::on_proc_exit::call(socket_close, Datum::from_usize(0))?;

    // In backends (as soon as forked) we operate the underlying socket in
    // nonblocking mode and use latches to implement blocking semantics if
    // needed. That allows us to provide safely interruptible reads and
    // writes.
    if !noblock_seams::pg_set_noblock::call(port.sock) {
        let e = errno();
        ereport(FATAL)
            .with_saved_errno(e)
...
fn socket_close(_code: i32, _arg: Datum<'static>) -> PgResult<()> {
    ...
    with_my_proc_port(&mut |port| {
        if let Some(port) = port {
            ...
            be_secure_seams::secure_close::call(port);
...
pub fn ListenServerPort(
    ...
    let ret = ip_seams::pg_getaddrinfo_all::call(host_name, Some(&service), &hint, &mut addrs);
    ...
    for addr in &addrs {
        ...
        let addr_desc: String = if addr.family == libc::AF_UNIX {
            ...
            ip_seams::pg_getnameinfo_all::call(
                &addr.addr,
                Some(&mut node),
                None,
                libc::NI_NUMERICHOST,
...
fn Lock_AF_UNIX(unix_socket_dir: &str, unix_socket_path: &str) -> PgResult<i32> {
    ...
    // bind() time.
    miscinit_seams::create_socket_lock_file::call(
        unix_socket_path,
        true,
        unix_socket_dir,
    )?;
...
pub fn AcceptConnection(server_fd: pgsocket, client_sock: &mut ClientSocket) -> i32 {
    ...
    if fd == PGINVALID_SOCKET {
        ...
        pgsleep_seams::pg_usleep::call(100000); // wait 0.1 sec
        return STATUS_ERROR;
    }
...
pub fn pq_getmessage(s: &mut StringInfo<'_>, maxlen: i32) -> PgResult<i32> {
    ...
    if len > 0 {
        ...
        if let Err(oom) = pqformat::enlarge_string_info(s, len) {
    ...
    while *start < *end {
        match secure_write_my_port(&buf[*start..*end])? {
            ...
            other => {
                ...
                *start = 0;
                *end = 0;
                init_small_seams::set_client_connection_lost::call(true);
        ...
        'retry: loop {
            ...
            for event in events.iter().take(rc.max(0) as usize) {
                ...
                if event.events & WL_LATCH_SET != 0 {
                    ...
                    latch_seams::reset_latch::call(latch);
                    continue 'retry;
                }
            }
            return Ok(true);
...
pub fn init_seams() {
    ...
    pqcomm_seams::pq_is_send_pending::set(pq_is_send_pending);
    ...
    pqcomm_seams::pq_putmessage_noblock::set(|msgtype, body| {
        pq_putmessage_noblock(msgtype, body).expect("pq_putmessage_noblock");
    });
    pqcomm_seams::modify_fe_be_wait_set_socket::set(pq_modify_fe_be_wait_set_socket);
    pqcomm_seams::wait_event_set_wait_fe_be::set(pq_wait_event_set_wait_fe_be);
    ...
    parallel_rt_seams::pq_put_terminate::set(pq_put_terminate);
