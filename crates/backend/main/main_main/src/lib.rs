...
pub fn pg_main(mcx: Mcx<'static>, argv: &[&str]) -> PgResult<MainOutcome> {
    ...
    init_small::globals::set_progname(progname.clone());
    ...
    more::ps_status::save_ps_display_args(argv);
    ...
    init_small::globals::SetMyProcPid(unsafe { libc::getpid() });
    ...
    match dispatch_option {
        ...
        DispatchOption::DISPATCH_DESCRIBE_CONFIG => {
            ...
            let text = help_config::GucInfoMain()?;
...
fn run_bootstrap(mcx: Mcx<'static>, argv: &[&str], check_only: bool) -> PgResult<()> {
    ...
    bootstrap::BootstrapModeMain(mcx, argv_owned, check_only)
}
...
fn install_ereport_panic_hook() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        // A recoverable hard error payload: either a structured `PgError` value
        // (the preferred channel — carries hint/detail/all ErrorData fields), a
        // legacy `PGRUST-SQLSTATE:` string, or a generic seam-miss/`panic!`
        // message. The main loop's catch_unwind turns all of these into a clean
