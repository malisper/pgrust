...
//! refers to these items as `::node_support::PgNodeCopy` etc.
//!
...
impl PgNodeOut for f64 {
    ...
    fn out_node(&self, buf: &mut String) {
        ...
        let mut tmp = [0u8; ryu::DOUBLE_SHORTEST_DECIMAL_LEN];
        let len = ryu::double_to_shortest_decimal_bufn(*self, &mut tmp);
...
impl PgNodeOut for f32 {
    ...
    fn out_node(&self, buf: &mut String) {
        ...
        let mut tmp = [0u8; ryu::FLOAT_SHORTEST_DECIMAL_LEN];
        let len = ryu::float_to_shortest_decimal_bufn(*self, &mut tmp);
        buf.push_str(core::str::from_utf8(&tmp[..len]).expect("Ryū output is ASCII"));
    }
}
