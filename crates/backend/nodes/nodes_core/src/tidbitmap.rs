...
//!
//! The opaque carriers in `tidbitmap` (`TIDBitmap`,
...
//
// `tidbitmap::TIDBitmap` is the opaque carrier those consumers hold. It
...
/// [`tidbitmap::TIDBitmap`] carrier.
fn carrier_inner(tbm: &tidbitmap::TIDBitmap) -> PgResult<&TidBitmapInner> {
    tbm.0
        .as_ref()
...
/// [`tidbitmap::TIDBitmap`] carrier.
fn carrier_inner_mut(tbm: &mut tidbitmap::TIDBitmap) -> PgResult<&mut TidBitmapInner> {
    tbm.0
        .as_mut()
...
pub fn tbm_create(maxbytes: usize, dsa: Option<DsaAreaHandle>) -> tidbitmap::TIDBitmap {
    let inner = TidBitmapInner::create(maxbytes, dsa);
    tidbitmap::TIDBitmap(Some(Box::new(inner)))
}

/// `tbm_is_empty(tbm)` (`tidbitmap.c`).
pub fn tbm_is_empty(tbm: &tidbitmap::TIDBitmap) -> PgResult<bool> {
...
pub fn tbm_add_page(tbm: &mut tidbitmap::TIDBitmap, pageno: BlockNumber) -> PgResult<()> {
...
pub fn tbm_union(
    a: &mut tidbitmap::TIDBitmap,
    b: &tidbitmap::TIDBitmap,
) -> PgResult<()> {
...
// `gin::TBMIterateResult` (with an opaque `Box<dyn Any>` `internal_page`),
...
/// `gin::TBMIterateResult`. Returns `false` at end (with `blockno ==
/// InvalidBlockNumber`).
pub fn gin_tbm_private_iterate(
    iterator: &mut tidbitmap::TBMPrivateIterator,
    tbmres: &mut gin::TBMIterateResult,
) -> bool {
...
/// scan, reading the consumer-side `gin::TBMIterateResult`.
pub fn gin_tbm_extract_page_tuple(
    iteritem: &gin::TBMIterateResult,
...
pub fn gin_tbm_end_private_iterate(_iterator: tidbitmap::TBMPrivateIterator) {
...
pub fn tbm_free(tbm: tidbitmap::TIDBitmap) {
...
/// Provider for `nodes_core_seams::tbm_add_tuple`: add one heap TID to
...
/// Provider for `nodes_core_seams::tbm_add_tuples`: add an array of heap
...
/// Provider for `nodes_core_seams::tbm_add_page`: mark the whole heap
/// page `pageno` lossy in the bitmap `tbm` (`tbm_add_page(tbm, pageno)`).
fn provide_tbm_add_page(tbm: &mut tidbitmap::TIDBitmap, pageno: BlockNumber) -> PgResult<()> {
...
fn provide_tbm_free(tbm: &mut tidbitmap::TIDBitmap) {
    tbm.0 = None;
}
...
fn provide_tbm_begin_iterate(
    tbm: Option<&mut tidbitmap::TIDBitmap>,
    dsa: Option<DsaAreaHandle>,
    dsp: tidbitmap::dsa_pointer,
) -> PgResult<tidbitmap::TBMIterator> {
    if dsa_pointer_is_valid(dsp) {
        ...
        let inner_it = begin_private_iterate(inner)?;
        let carrier =
            tidbitmap::TBMPrivateIterator(Some(Box::new(PrivateIterCarrier {
                tbm: inner_ptr,
                iter: inner_it,
            })));
        Ok(tidbitmap::TBMIterator {
...
/// The payload stored in a private [`tidbitmap::TBMPrivateIterator`]
...
fn provide_tbm_end_iterate(iterator: &mut tidbitmap::TBMIterator) {
...
fn provide_tbm_prepare_shared_iterate(
    tbm: &mut tidbitmap::TIDBitmap,
) -> PgResult<tidbitmap::dsa_pointer> {
    let (dsa, layout) = {
...
fn provide_tbm_free_shared_area(dsa: DsaAreaHandle, dp: tidbitmap::dsa_pointer) {
...
fn attach_shared_iterate(
    dsa: DsaAreaHandle,
    dp: tidbitmap::dsa_pointer,
) -> PgResult<tidbitmap::TBMSharedIterator> {
...
fn dsa_shared_tbm_attach(
    ...
    Ok(tidbitmap::TBMSharedIterator(Some(Box::new(carrier))))
}
...
/// The backend-private payload stored in a [`tidbitmap::TBMSharedIterator`]
...
pub fn tbm_shared_iterate(
    iterator: &mut tidbitmap::TBMSharedIterator,
    tbmres: &mut TBMIterateResult,
) -> PgResult<bool> {
    let carrier = iterator
        .0
...
fn provide_tbm_create<'mcx>(
    ...
    maxbytes: usize,
    dsa: Option<DsaAreaHandle>,
) -> PgResult<mcx::PgBox<'mcx, tidbitmap::TIDBitmap>> {
...
fn provide_tbm_iterate(
    iterator: &mut tidbitmap::TBMIterator,
) -> PgResult<Option<tidbitmap::TBMIterateOutcome>> {
    ...
    Ok(Some(tidbitmap::TBMIterateOutcome {
...
pub fn init_seams() {
    nodes_core_seams::tbm_add_tuple::set(provide_tbm_add_tuple);
    nodes_core_seams::tbm_add_tuples::set(provide_tbm_add_tuples);
    nodes_core_seams::tbm_add_page::set(provide_tbm_add_page);

    core_tidbitmap_seams::tbm_create::set(provide_tbm_create);
    core_tidbitmap_seams::tbm_union::set(provide_tbm_union);
    core_tidbitmap_seams::tbm_prepare_shared_iterate::set(
        provide_tbm_prepare_shared_iterate,
    );
    core_tidbitmap_seams::tbm_begin_iterate::set(provide_tbm_begin_iterate);
    core_tidbitmap_seams::tbm_iterate::set(provide_tbm_iterate);
    core_tidbitmap_seams::tbm_end_iterate::set(provide_tbm_end_iterate);
    core_tidbitmap_seams::tbm_free::set(provide_tbm_free);
    core_tidbitmap_seams::tbm_free_shared_area::set(provide_tbm_free_shared_area);
    core_tidbitmap_seams::tbm_intersect::set(tbm_intersect);
    core_tidbitmap_seams::tbm_is_empty::set(tbm_is_empty);
    ...
    costsize_seams::tbm_calculate_entries::set(|maxbytes| {
