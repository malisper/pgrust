...
pub fn set_append_rel_size<'mcx>(
    ...
    for appinfo in &appinfos {
        ...
        for nid in &parent_exprs {
            ...
            let translated = equivclass_ext_seams::adjust_appendrel_attrs::call(
                run,
                root,
                parent_expr,
                alloc::vec![childrel],
        ...
        let need_ec = root.rel(rel).has_eclass_joins
            || pathkeys::has_useful_pathkeys(root, root.rel(rel));
        if need_ec {
            equivclass_seams::add_child_rel_equivalences::call(
                root,
                run,
        ...
        for childpath in child_paths {
            ...
            if !childkeys.is_empty() {
                ...
                for existing in &all_child_pathkeys {
                    if pathkeys::compare_pathkeys(existing, &childkeys)
...
pub fn get_cheapest_parameterized_child_path<'mcx>(
    ...
    let cheapest0 = pathkeys::get_cheapest_path_for_pathkeys(
        root,
...
pub fn generate_orderedappend_paths<'mcx>(
    ...
    {
        let (pk, partial) = pathkeys::build_partition_pathkeys(
            root,
            mcx,
            rel,
        ...
        let (pkd, partial_d) = pathkeys::build_partition_pathkeys(
            root,
            mcx,
            rel,
    ...
    for pathkeys in all_child_pathkeys {
        ...
        while i != end_index {
            ...
            let mut cheapest_startup = pathkeys::get_cheapest_path_for_pathkeys(
                root,
            ...
            if root.tuple_fraction > 0.0 {
                ...
                let cf = pathkeys::get_cheapest_fractional_path_for_pathkeys(
                    root,
                    &root.rel(childrel).pathlist.clone(),
                    pathkeys,
                    &None,
...
fn pathkeys_contained_in(keys1: &[PathKey], keys2: &[PathKey]) -> bool {
    pathkeys::pathkeys_contained_in(keys1, keys2)
}
...
fn get_cheapest_parallel_safe_total_inner(root: &PlannerInfo, rel: RelId) -> Option<PathId> {
    pathkeys::get_cheapest_parallel_safe_total_inner(
        root,
