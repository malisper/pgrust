...
//! [`pathnodes`] arena model (`PlannerInfo`/`RelId`/`PathId`): every
...
fn min_parallel_table_scan_size() -> i32 {
    guc_tables::vars::min_parallel_table_scan_size.read()
}
#[inline]
fn min_parallel_index_scan_size() -> i32 {
    guc_tables::vars::min_parallel_index_scan_size.read()
}
#[inline]
...
fn enable_partitionwise_join() -> bool {
    guc_tables::vars::enable_partitionwise_join.read()
}
#[inline]
fn enable_geqo() -> bool {
    guc_tables::vars::enable_geqo.read()
}
#[inline]
fn geqo_threshold() -> i32 {
    guc_tables::vars::geqo_threshold.read()
}
...
fn check_index_predicates<'mcx>(
    ...
    indxpath::check_index_predicates(mcx, root, run, rel)
}
...
fn create_index_paths<'mcx>(
    ...
    indxpath::create_index_paths(mcx, root, run, rel)
}
...
fn create_tidscan_paths<'mcx>(
    ...
    let enable_tidscan = guc_tables::vars::enable_tidscan.read();
    path_small::create_tidscan_paths(root, run, rel, enable_tidscan)
}

/// `get_rel_persistence(relid)` (lsyscache.c).
fn get_rel_persistence(relid: Oid) -> PgResult<i8> {
    Ok(lsyscache_seams::get_rel_persistence::call(relid)? as i8)
}
...
fn tablesample_get_sample_size<'mcx>(
    ...
    match handler {
        F_TSM_SYSTEM_HANDLER => {
            tablesample_core::system_samplescangetsamplesize(mcx, baserel, &args)
        }
        F_TSM_BERNOULLI_HANDLER => {
            tablesample_core::bernoulli_samplescangetsamplesize(mcx, baserel, &args)
        }
...
fn tablesample_repeatable_across_scans<'mcx>(
    ...
    let handler = rte::rte_tablesample_handler::call(run, root, rti);
    let routine = tablesample_core_seams::get_tsm_routine_oid::call(mcx, handler)?;
...
fn tablesample_is_parallel_safe_impl<'mcx>(
    ...
    let handler = rte::rte_tablesample_handler::call(run, root, rti);
    let proparallel = lsyscache_seams::func_parallel::call(handler)?;
...
pub fn init_seams() {
    ...
    geqo_all_seams::build_and_cost_join_rel::set(build_and_cost_join_rel_seam);
    ...
    setrefs_seams::subroot_final_rel_is_dummy::set(
    ...
    vacuum_seams::min_parallel_index_scan_size::set(|| {
