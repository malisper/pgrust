...
//!
//! All operate over the owned `Query<'mcx>` value model (`nodes`).
...
fn check_output_expressions<'mcx>(
    ...
    for (idx, tle) in tlist.iter().enumerate() {
        ...
        if (safety_info.unsafe_flags[resno] & UNSAFE_HAS_VOLATILE_FUNC) == 0
            && clauses::grounded::contain_volatile_functions(texpr)?
        {
...
pub(crate) fn qual_is_pushdown_safe<'mcx>(
    ...
    if clauses::grounded::contain_subplans(Some(clause))? {
        return Ok(PushdownSafe::Unsafe);
    }

    // Refuse volatile quals if we found they'd be unsafe (point 2).
    if safety_info.unsafe_volatile
        && clauses::grounded::contain_volatile_functions(Some(clause))?
    {
        return Ok(PushdownSafe::Unsafe);
    }
    ...
    if safety_info.unsafe_leaky
        && clauses::grounded::contain_leaked_vars(Some(clause))?
    {
        return Ok(PushdownSafe::Unsafe);
    }
    ...
    let vars = vars::var::pull_var_clause(
        mcx,
        &clause_node,
...
pub(crate) fn subquery_push_qual<'mcx>(
    ...
    if subquery.setOperations.is_some() {
        ...
        rewrite_core::replace::ReplaceVarsFromTargetList(
            &mut qual_node,
            rti as i32,
            0,
            rte,
            ...
            rewrite_core::replace::ReplaceVarsNoMatchOption::ReportError,
            0,
            &mut has_sublinks,
            mcx,
        )?;
...
pub(crate) fn check_and_push_window_quals<'mcx>(
    ...
    if !lsyscache_seams::func_strict::call(opexpr.opfuncid)? {
        return Ok(true);
    }
...
fn find_window_run_conditions<'mcx>(
    ...
    *keep_original = true;
    ...
    if clauses::grounded::contain_subplans(Some(wfunc_node))? {
        return Ok(false);
    }
    ...
    if !clauses::grounded::is_pseudo_constant_clause(Some(otherexpr))? {
        return Ok(false);
    }
    ...
    let opinfos =
        lsyscache_seams::get_op_index_interpretation::call(mcx, opexpr.opno)?;
    ...
    for opinfo in opinfos.iter() {
        ...
        if cmptype == COMPARE_LT || cmptype == COMPARE_LE {
            ...
            *keep_original = true;
            runopexpr = Some(clone_runopexpr(opexpr)?);
            runoperator = lsyscache_seams::get_opfamily_member_for_cmptype::call(
                opinfo.opfamily_id,
                opinfo.oplefttype,
                opinfo.oprighttype,
                newcmptype,
...
pub(crate) fn remove_unused_subquery_outputs<'mcx>(
    ...
    for tle in subquery.targetList.iter_mut() {
        ...
        if clauses::grounded::contain_volatile_functions(texpr)? {
            continue;
        }
...
fn pull_varattnos_relids<'mcx>(
    ...
    Ok(vars::var::pull_varattnos(node, relid, attrs))
}
