...
//! [`IndexPath`](pathnodes) / `BitmapHeapPath` / `BitmapAndPath` /
...
//! `Path *` is a [`PathId`](pathnodes), a `RestrictInfo *` is a
//! [`RinfoId`](pathnodes), a clause `Expr *` is a
//! [`NodeId`](pathnodes), an `EquivalenceClass *` is an `EcId`, and the
