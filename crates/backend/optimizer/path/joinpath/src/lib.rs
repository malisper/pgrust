...
fn param_exprs_member(root: &PlannerInfo, param_exprs: &[NodeId], expr: NodeId) -> bool {
    ...
    param_exprs
        .iter()
        .any(|&existing| equalfuncs_seams::equal_expr::call(root.node(existing), target))
}
...
/// modelled [`FdwRoutine`](nodes::FdwRoutine) carries only the
...
fn clause_is_const(root: &PlannerInfo, rinfo: RinfoId) -> bool {
    ...
    matches!(root.node(clause), nodes::primnodes::Expr::Const(_))
}
...
fn clause_opexpr_opno(root: &PlannerInfo, rinfo: RinfoId) -> Oid {
    ...
    match root.node(clause) {
        nodes::primnodes::Expr::OpExpr(o) => o.opno,
        _ => panic!(
...
fn clause_is_opexpr_with_two_args(root: &PlannerInfo, rinfo: RinfoId) -> bool {
    let clause = root.rinfo(rinfo).clause;
    match root.node(clause) {
        nodes::primnodes::Expr::OpExpr(o) => o.args.len() == 2,
        _ => false,
    }
}
...
fn opexpr_arg<'mcx>(mcx: ::mcx::Mcx<'mcx>, root: &mut PlannerInfo, rinfo: RinfoId, n: i32) -> PgResult<NodeId> {
    ...
    let arg = match root.node(clause) {
        nodes::primnodes::Expr::OpExpr(o) => o.args[n as usize].clone_in(mcx)?,
