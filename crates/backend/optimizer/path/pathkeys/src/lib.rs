...
//! ([`equivclass_seams`], panic-until-owner).
//!
...
// COMPARE_GT / COMPARE_EQ; `CompareType` is the `i32` alias in `pathnodes`.
...
pub fn matches_boolean_partition_clause(
    ...
    let partexpr: &nodes::primnodes::Expr = root.node(partexpr_id);
...
pub fn find_var_for_subquery_tle(
    ...
    rel: RelId,
    tle: NodeId,
) -> Option<nodes::primnodes::Expr<'static>> {
...
pub fn make_pathkeys_for_sortclauses_extended(
    ...
    for idx in 0..sortclauses.len() {
        ...
        if remove_group_rtindex {
            ...
            sortkey = nf::remove_nulling_relids::call(mcx, sortkey, &singleton, &None);
        }
