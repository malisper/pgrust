...
pub fn join_is_removable<'mcx>(
    ...
    for ph_id in ph_ids {
        ...
        let varnos = joininfo_ext_seams::pull_varnos_expr::call(root, phexpr);
    ...
    for restrictinfo in joininfo {
        ...
        if !joinpath_seams::clause_sides_match_join::call(
            root,
            restrictinfo,
            &min_lefthand,
            &innerrel_relids,
...
pub fn rel_is_distinct_for<'mcx>(
    ...
    if rtekind == RTE_RELATION {
         ...
         */
        indxpath::relation_has_unique_index_ext(
            root,
            rel,
            clause_list,
            &[],
...
pub fn init_seams() {
    init_subselect_seams::reduce_unique_semijoins::set(
        |root, run| {
    ...
    init_subselect_seams::remove_useless_joins::set(
        |root, run, joinlist| {
    ...
    init_subselect_seams::remove_useless_self_joins::set(
        |root, run, joinlist| remove_joins::remove_useless_self_joins(root, run, joinlist),
    );

    joinpath_seams::innerrel_is_unique::set(
    ...
    pathnode_seams::subquery_is_distinct_for::set(
    ...
    guc_tables::vars::enable_self_join_elimination.install(
