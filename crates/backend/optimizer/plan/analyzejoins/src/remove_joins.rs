...
fn remove_leftjoinrel_from_query<'mcx>(
    ...
    for rinfo in joininfos {
        ...
        joininfo::remove_join_clause_from_rels(
            run,
            root,
            rinfo,
            &required_relids,
        ...
        if rinfo_is_pushed_down(root, rinfo, &join_plus_commute) {
            ...
            equivclass_ext_seams::distribute_restrictinfo_to_rels::call(
                run, root, rinfo,
            )?;
        }
    }
     ...
     */
    joininfo::rebuild_placeholder_attr_needed(run.mcx(), root)?;
    plan_small_seams::rebuild_joinclause_attr_needed::call(root, run);
    equivclass::rebuild_eclass_attr_needed(run.mcx(), root)?;
    plan_small_seams::rebuild_lateral_attr_needed::call(root, run);
    Ok(())
}
...
fn remove_rel_from_eclass<'mcx>(
    ...
    equivclass::ec_clear_derived_clauses(root, ec_id);
    Ok(())
}
...
fn split_selfjoin_quals<'mcx>(
    ...
    for &rinfo_id in joinquals {
        ...
        if equivclass_ext_seams::equal::call(&leftexpr, &rightexpr) {
...
fn match_unique_clauses<'mcx>(
    ...
    for &rinfo_id in uclauses {
        ...
        for &orinfo_id in &root.rel(outer).baserestrictinfo {
            ...
            if equivclass_ext_seams::equal::call(&iclause, &oclause)
                && equivclass_ext_seams::equal::call(&c1, &c2)
            {
                matched = true;
                break;
            }
...
fn restrict_infos_logically_equal(root: &PlannerInfo, a: RinfoId, b: RinfoId) -> bool {
    ...
    ra.is_pushed_down == rb.is_pushed_down
        ...
        && equivclass_ext_seams::equal::call(
            root.node(ra.clause),
            root.node(rb.clause),
        )
}
...
fn update_eclasses<'mcx>(
    ...
    for em_id in members {
        ...
        for &other_id in &new_members {
            ...
            if equivclass_ext_seams::equal::call(
    ...
    equivclass::derives::ec_clear_derived_clauses(root, ec);
...
fn remove_self_join_rel<'mcx>(
    ...
    for rinfo_id in joininfos {
        ...
        joininfo::remove_join_clause_from_rels(run, root, rinfo_id, &req);
    ...
    joininfo::placeholder::rebuild_placeholder_attr_needed(run.mcx(), root)
