...
pub fn remove_useless_groupby_columns(root: &mut PlannerInfo, run: &PlannerRun<'_>) {
    ...
    for &sgc in &group_clause {
        ...
        let expr: &nodes::primnodes::Expr = root.node(te_info.expr);

        // Ignore non-Vars and Vars from other query levels.
        let var = match expr {
            nodes::primnodes::Expr::Var(v) if v.varlevelsup == 0 => v.clone(),
            _ => continue,
        };
    ...
    if let Some(surplusvars) = surplusvars {
        ...
        for &sgc in &group_clause {
            ...
            let keep = match expr {
                nodes::primnodes::Expr::Var(v) if v.varlevelsup == 0 => {
                    !is_member(
