...
// clamp on 15-CPU CI cluster pods must clear the floor (a 16 floor would flap
// on cores-1 boxes).
// ---------------------------------------------------------------------------
...
fn class_guard(class: CoverClass) -> FloorGuard {
    match class {
        ...
        // sort-merge-redesign; measured 2026-07-21 @ 0296033fd, CI cluster jobs
...
fn textdistinct_enabled() -> bool {
    ...
    *ON.get_or_init(|| {
        ...
        // vs knob-OFF (doc above); this flip is validated on the CI cluster
...
/// switch. HISTORY: night/planner-fix-forced held this OFF because the CI cluster
...
/// GL-TEXTDIST CI cluster letters own re-measuring each shape's real economics).
...
/// band (letter of record: scratchpad/night/GL-LOWDIST-1-letter.md; CI cluster
...
/// of record scratchpad/night/CI-ab-parallelism.md 2026-07-21, campaign
...
/// CI-ab-parallelism.md 2026-07-21): the numeric evaltrans win is
...
/// CI-ab-parallelism.md 2026-07-21): heap-side joins engage 2.1-3.1x
...
/// CI-ab-parallelism.md 2026-07-21): varchar/text canonical-bytes join
...
/// CI-ab-parallelism.md 2026-07-21): the K-text unlock on skeletons —
...
/// CI-ab-parallelism.md 2026-07-21): the first FILTERED grouped-join
...
/// CI cluster letter 2026-07-21, 10M unforced pair jobs -6edd/-533a: the
...
/// FLIP-RECOMMENDED — CI cluster letter 2026-07-21: the no-qual three-int-agg
...
///
/// DEFAULT ON (GL-INTCASE-1, CI-ab-parallelism.md 2026-07-21, @
...
/// keyed only under the AGG_POLY knob until its CI cluster letter lands).
...
///
/// Default RAISED 1e6 → 4e6 (night/routing-floor-fixes, CI cluster letter in the
...
fn classify_covered(run: &mut PlannerRun<'_>) -> PgResult<bool> {
    ...
    for e in &passengers {
        ...
        // exclusions: NO QUALS (CI cluster containment, GL letter
        // CI-ab-parallelism.md: the qualed target shape suppressed but the arm
...
// nlx-ab-CI-full.tsv) — but a census-shape query never reaches it: the
...
/// letter, 2026-07-21 — scratchpad/night/CI-ab-parallelism.md; the t35
...
/// leg (c) verifies the CI cluster fixtures key). Any key without one keeps
/// Gather.
...
pub mod serial_shadow {
    ...
    //! pick, and the enforced verdict (the e2e/CI cluster census vehicle).
...
/// engagement shape). The CI cluster letter owns re-measuring.
...
/// width), affine ±Const only (Mul/Div refuse). CAVEATS of record (CI cluster
...
/// confirmation is CI cluster work; the arm's admission-time canonical-domain
...
/// (open-rows flip train, GL-OPENROWS-EXPRKEY-TOPN — CI cluster letter
...
/// count is large (CI cluster letter, two measured points: ~9x win at the
