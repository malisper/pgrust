...
/// identical vs C + vs knob-OFF; the DEFAULT flip is CI-gated future work
...
/// unforced win (q11/q12 8-9x, matching the mt16 rt arm), but the CI cluster A/B
...
/// suppress Gather and land on serial). CI default-flip owed
/// (GL-MKTEXT-1).
...
/// DEFAULT OFF (`PGRUST_LANE_V2_EXPRKEY_EXTRACT=1|on`); CI cluster default-flip
...
fn classify_covered(run: &mut PlannerRun<'_>) -> PgResult<bool> {
    ...
    if parse.groupClause.is_nil() {
        ...
        if is_cb {
            ...
            // engagement — the CI cluster runtime posture arms it; GL-TEXTDIST-2
            ...
            // default OFF). The CI cluster unforced-vs-mt16 A/B measured the plain
    ...
    // vocabulary); the TEXT is the GROUP key. CI default-flip owed
    // (GL-TEXTDIST-1).
