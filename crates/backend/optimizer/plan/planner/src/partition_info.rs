...
fn get_stat_ext_keys_exprs<'run>(
    ...
    for el in elems.into_iter() {
        ...
        nodes_core::nodefuncs::fix_opfuncids(&mut folded)?;
...
fn get_stat_ext_data_kinds(
    ...
    let Some((stxdinherit, built)) =
        syscache_seams::statext_data_built_kinds::call(stat_oid, inh)?
    else {
        return Ok(None);
    };
    ...
    let kind_chars = [
        statistics::STATS_EXT_NDISTINCT,
        statistics::STATS_EXT_DEPENDENCIES,
        statistics::STATS_EXT_MCV,
        statistics::STATS_EXT_EXPRESSIONS,
    ];
...
fn process_check_constraint<'run>(
    ...
    let items = nodes_core::makefuncs::make_ands_implicit(canon);
...
fn set_relation_partition_info<'run>(
    ...
    let relation =
        common_relation_seams::relation_open::call(mcx, relid, NoLock)?;
    ...
    //           CreatePartitionDirectory(CurrentMemoryContext, true);
    {
        ...
        if glob.partition_directory.0.is_none() {
            glob.partition_directory =
                partitioning_core_seams::create_partition_directory::call(mcx, true)?;
        }
    }
...
fn find_partition_scheme<'mcx>(
    ...
    let partkey = partcache::RelationGetPartitionKey(mcx, relation)?
        .ok_or_else(|| {
...
fn set_baserel_partition_constraint_inner<'mcx, 'run>(
    ...
    let partconstr = partcache::RelationGetPartitionQual(mcx, relation)?;
