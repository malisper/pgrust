...
pub fn find_placeholder_info<'mcx>(
    ...
    // placeholder.c:128 pull_varnos(root, phexpr): a nested PlaceHolderVar
    // with a PlaceHolderInfo contributes its ph_eval_at (translated for an
    // appendrel-child copy) plus phnullingrels, not its syntactic phrels
    // (var.c:197-260).
