...
pub fn amcostestimate(
    ...
    match types_relscan::IndexAmKind::from_relam(relam) {
        ...
        types_relscan::IndexAmKind::Objkv => objkvcostestimate(run, path_id, loop_count),
        #[allow(unreachable_patterns)]
        other => panic!("amcostestimate (selfuncs.c): {other:?}; M2 index-AM lane"),
    }
}

// objkv: the generic estimate, not btree's — btcostestimate reads correlation
// off a sort order objkv does not publish. Optimistic, but enough to prefer a
// seek over reading the whole table.
fn objkvcostestimate(
    run: &mut PlannerRun<'_>,
    path_id: types_pathnodes::PathId,
    loop_count: f64,
) -> PgResult<AmCostEstimate> {
