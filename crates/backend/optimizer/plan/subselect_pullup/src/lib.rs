...
pub fn init_seams() {
    subselect_pullup_seams::convert_ANY_sublink_to_join::set(
        convert_ANY_sublink_to_join,
    );
    subselect_pullup_seams::convert_EXISTS_sublink_to_join::set(
        convert_EXISTS_sublink_to_join,
    );
    subselect_pullup_seams::convert_VALUES_to_ANY::set(
...
fn convert_VALUES_to_ANY<'mcx>(
    ...
    for row in rte.values_lists.iter() {
        if let Some(elems) = row.as_ref().as_list() {
            for value in elems.iter() {
                if clauses::grounded::contain_volatile_functions(
                    value.as_expr(),
                )? {
                    return Ok(None);
                }
    ...
    for row in rte.values_lists.iter() {
        ...
        let folded =
            clauses::fold::eval_const_expressions_with_params(
                mcx,
                converted_expr,
                bound_params.clone(),
            )?;
...
pub fn convert_ANY_sublink_to_join<'mcx>(
    ...
    let sub_ref_outer_relids =
        vars::var::pull_varnos_of_level(None, &subselect_node, 1);
    ...
    let upper_varnos =
        vars::var::pull_varnos(Some(root), &testexpr_node);
    ...
    if clauses::grounded::contain_volatile_functions(Some(testexpr_expr))?
    {
        return Ok(None);
    }
...
pub fn convert_EXISTS_sublink_to_join<'mcx>(
    ...
    // Re-wrap the (mutated) subselect Query as a Node for the level walkers.
    {
        ...
        if vars::var::contain_vars_of_level(&subselect_as_node, 1) {
            return Ok(None);
        }
        ...
        match where_clause.as_deref() {
            Some(wc) => {
                if !vars::var::contain_vars_of_level(wc, 1) {
                    return Ok(None);
                }
    ...
    let rtoffset = parse.rtable.len() as i32;
    {
        ...
        rewrite_core::offset::OffsetVarNodes(&mut subselect_node2, rtoffset, 0, mcx);
        // IncrementVarSublevelsUp((Node *) subselect, -1, 1)
        rewrite_core::increment::IncrementVarSublevelsUp(&mut subselect_node2, -1, 1, mcx)?;
    ...
    if let Some(wc) = where_clause.as_deref_mut() {
        rewrite_core::offset::OffsetVarNodes(wc, rtoffset, 0, mcx);
        rewrite_core::increment::IncrementVarSublevelsUp(wc, -1, 1, mcx)?;
    }
    ...
    let clause_varnos = match where_clause.as_deref() {
        Some(wc) => vars::var::pull_varnos(Some(root), wc),
        None => None,
    };
...
fn simplify_EXISTS_query<'mcx>(
    ...
    if query.limitCount.is_some() {
        ...
        let folded: Option<Expr> =
            Some(clauses::fold::eval_const_expressions(mcx, limit_expr)?);
