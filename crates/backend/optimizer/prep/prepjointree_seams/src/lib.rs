...
//! [`PlannerRun`](pathnodes::planner_run::PlannerRun) store behind the
//! `PlannerInfo.parse` [`QueryId`](pathnodes::QueryId) handle. The caller
...
seam_core::seam!(
    ...
    /// caller from the [`PlannerRun`](pathnodes::planner_run::PlannerRun)
...
seam_core::seam!(
    ...
    pub fn build_virtual_generated_columns_tlist<'mcx>(
        ...
        Option<mcx::PgVec<'mcx, nodes::primnodes::TargetEntry<'mcx>>>,
    >
);
...
seam_core::seam!(
    ...
    pub fn get_relids_for_join<'mcx>(
        ...
        joinrelid: i32,
    ) -> types_error::PgResult<nodes::primnodes::ExprRelids>
);
