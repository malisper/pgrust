...
//!   [`preptlist_seams::preprocess_targetlist`] and
...
//!   `restrictinfo_seams::has_plan_rowmark` (used by
...
pub fn preprocess_targetlist<'mcx>(
    ...
    let target_relation: Option<Relation<'mcx>> = if result_relation != 0 {
        ...
        Some(table::table_open(mcx, relid, 0 /* NoLock */)?)
    } else {
    ...
    if !parse.returningList.is_empty() && parse.rtable.len() > 1 {
        let flags = vars::PVC_RECURSE_AGGREGATES
        ...
        for var in vars.into_iter() {
            ...
            let already = tlist.iter().any(|&id| {
                ...
                equalfuncs_seams::equal_expr::call(&var, root.node(expr_id))
            });
            if already {
        ...
        let new_tle = match matched {
            ...
            None => {
                ...
                let new_expr: Expr = if att_tup.attisdropped {
                    ...
                    coerce_seams::coerce_null_to_domain::call(
                        mcx,
                        att_tup.atttypid,
                        att_tup.atttypmod,
                        att_tup.attcollation,
        ...
        let new_tle: nodes::primnodes::TargetEntry<'mcx> = match matched {
            ...
            None => {
                ...
                let resname =
                    alloc::string::String::from_utf8_lossy(att_tup.attname.name_str())
                        .into_owned();
                nodes_core::makefuncs::make_target_entry(
                    mcx,
                    new_expr,
                    attrno,
                    Some(resname.as_str()),
...
/// Backs `restrictinfo_seams::has_plan_rowmark`: does the
...
pub fn init_seams() {
    preptlist_seams::preprocess_targetlist::set(preprocess_targetlist);
    restrictinfo_seams::has_plan_rowmark::set(seam_has_plan_rowmark);
}
