...
pub fn init_seams() {
    ...
    plancat_ext_seams::subquery_physical_tlist::set(|run, root, rti| {
        ...
        let vars: alloc::vec::Vec<(
            ...
            for tle in subquery.targetList.iter() {
                let var = nodes_core::makefuncs::make_var_from_target_entry(
                    rti as i32, tle,
                )?;
    ...
    init_subselect_ext_seams::rte_kind_inh_relkind::set(|run, root, rti| {
    ...
    relnode_ext_seams::rte_eref_colnames_len::set(|run, root, rti| {
    ...
    costsize_seams::rte_relid::set(|run, root, rel| {
    ...
    // owned RTE store.
    costsize_seams::rte_enrtuples::set(|run, root, rel| {
        let rti = root.rel(rel).relid;
    ...
    costsize_seams::rte_tablesample_tsmhandler::set(|run, root, rel| {
        let rti = root.rel(rel).relid;
        match planner_rt_fetch(run, root, rti)
            .tablesample
            .as_deref()
    ...
    costsize_seams::rte_cte_self_reference::set(|run, root, rel| {
        let rti = root.rel(rel).relid;
    ...
    costsize_seams::rte_function_max_set_rows::set(|run, root, rel| {
        let rti = root.rel(rel).relid;
        let rte = planner_rt_fetch(run, root, rti);
        ...
        for f in rte.functions.iter() {
            if let Some(rtf) = (&**f).as_rangetblfunction() {
                ...
                let ntup =
                    clauses::expression_returns_set_rows(funcexpr)
    ...
    costsize_seams::rte_functions_exprcost::set(|run, root, rel| {
        let rti = root.rel(rel).relid;
        let rte = planner_rt_fetch(run, root, rti);
    ...
    costsize_seams::rte_tablefunc_exprcost::set(|run, root, rel| {
        let rti = root.rel(rel).relid;
        let rte = planner_rt_fetch(run, root, rti);
        let mut startup = 0.0_f64;
        let mut per_tuple = 0.0_f64;
                ...
                let arbiter_where: Vec<Expr> = match oc.arbiterWhere.as_deref() {
                    ...
                    Some(other) => match other.as_expr() {
                        Some(e) => {
                            ...
                            nodes_core::makefuncs::make_ands_implicit(Some(cloned))
                        }
                        None => {
                            return Err(types_error::PgError::error(format!(
