...
use appendinfo_ext_seams as fdw;
use relnode_ext_seams as relnode_ext;
use relnode_seams as bms;
use plan_small_seams as initsplan;
...
type Relation<'mcx> = rel::RelationData<'mcx>;
...
pub fn distribute_row_identity_vars(
    ...
    if root.row_identity_vars.is_empty() {
        let target_relation =
            table_seams::table_open::call(mcx, target_rte_relid, NO_LOCK)?;
...
fn make_null_const<'mcx>(consttype: Oid, consttypmod: i32, constcollid: Oid) -> PgResult<Const<'mcx>> {
    ...
    Ok(Const {
        ...
        constvalue: types_tuple::heaptuple::Datum::ByVal(0),
        constisnull: true,
...
fn relids_del_member(relids: Relids, x: i32) -> Relids {
    ...
    pathnode_seams::relids_del_members::call(relids, &singleton)
}
...
pub fn init_seams() {
    ...
    ai::add_row_identity_var::set(add_row_identity_var);
