...
fn add_nullingrels_if_needed<'mcx>(
    ...
    if is_standard_join_alias_expression(&newnode, oldvar) {
        ...
        if expr_relids::is_empty(&phrels) {
            ...
            let join_relids =
                prepjointree_seams::get_relids_for_join::call(
                    mcx,
                    context.query,
                    oldvar.varno,
                )?;
        ...
        let mut newphv = placeholder_seams::make_placeholder_expr::call(
            root_mut,
...
fn alias_relid_set<'mcx>(
    ...
    while let Some(next) = expr_relids::next_member(relids, rtindex) {
        ...
        if rte.rtekind == RTEKind::RTE_JOIN {
            let join_relids = prepjointree_seams::get_relids_for_join::call(
                mcx, query, rtindex,
            )?;
...
/// `ruleutils_seams::flatten_group_exprs`.
...
pub(crate) fn init_seams() {
    ruleutils_seams::flatten_group_exprs::set(seam_flatten_group_exprs);
}
...
fn mark_nullable_by_grouping<'mcx>(
    ...
    if !expr_relids::is_empty(&relids) {
            ...
            && !equivclass_ext_seam::expression_returns_set::call(expr)
        {
            ...
            //                                 true, false);
            let phrels_expr =
                prepjointree_seams::get_relids_in_query_jointree::call(
                    mcx, query,
                )?;
            ...
            let mut newphv =
                placeholder_seams::make_placeholder_expr::call(
                    root,
                    expr_val.erase_lifetime(),
                    phrels,
                )
