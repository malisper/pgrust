...
fn parser_errposition(pstate: &ParseState, location: i32) -> i32 {
    small1_seams::parser_errposition::call(pstate, location).unwrap_or(0)
}
...
pub fn transformAggregateCall<'mcx>(
    ...
    if AGGKIND_IS_ORDERED_SET(agg.aggkind) {
        ...
        torder = clause::transformSortClause(
            mcx,
            pstate,
            &orderlist,
            &mut tlist,
        ...
        if agg_distinct {
            tdistinct = clause::transformDistinctClause(
            ...
            for sortcl in tdistinct.iter() {
                if !OidIsValid(sortcl.sortop) {
                    let expr = vars::tlist::get_sortgroupclause_expr(
                        sortcl, &tlist,
                    )?;
                    ...
                    let tyname = format_type_seams::format_type_be::call(mcx, typ)?;
...
fn check_agglevels_and_constraints<'mcx, 'n>(
    ...
    if errkind {
        let name = parse_expr_seams::parse_expr_kind_name::call(kind);
        let msg = if is_agg {
...
fn check_agg_arguments<'mcx>(
    ...
    if agglevel == context.min_agglevel {
        ...
        if aggloc < 0 {
            if let Some(f) = filter {
                aggloc = rewritemanip_seams::locate_agg_of_level::call(f, agglevel);
            }
        }
        return Err(ereport(ERROR)
            .errcode(ERRCODE_GROUPING_ERROR)
...
fn locate_agg_of_level_list(list: &[Node], levelsup: i32) -> i32 {
    for n in list {
        let loc = rewritemanip_seams::locate_agg_of_level::call(n, levelsup);
        if loc >= 0 {
            return loc;
        }
    }
...
fn locate_var_of_level_list(list: &[Node], levelsup: i32) -> i32 {
    for n in list {
        let loc = vars::locate_var_of_level(n, levelsup);
        if loc >= 0 {
            return loc;
        }
    }
...
fn contain_windowfuncs_exprs<'mcx>(mcx: Mcx<'mcx>, args: &[Expr]) -> types_error::PgResult<bool> {
    for e in args {
        ...
        if rewritemanip_seams::contain_windowfuncs::call(&n) {
            return Ok(true);
        }
    }
    Ok(false)
...
fn eq_node_lists(a: &[NodePtr], b: &[NodePtr]) -> bool {
    ...
    a.iter()
        .zip(b.iter())
        .all(|(x, y)| equalfuncs_seams::equal_node::call(x, y))
}
...
fn eq_opt_nodes(a: &Option<NodePtr>, b: &Option<NodePtr>) -> bool {
    match (a.as_deref(), b.as_deref()) {
        (None, None) => true,
        (Some(x), Some(y)) => equalfuncs_seams::equal_node::call(x, y),
        _ => false,
    }
}
...
pub fn parseCheckAggregates<'mcx>(
    ...
    for grpcl_node in qry.groupClause.iter() {
        ...
        let tle = vars::tlist::get_sortgroupclause_tle(
            grpcl,
            &qry.targetList,
        )?;
    ...
    if !group_clauses.is_empty() {
        ...
        let nsitem = parser_relation::addRangeTableEntryForGroup(
            mcx,
            pstate,
            &group_clauses,
        )?;
    ...
    if let Some(having) = qry.havingQual.take() {
        ...
        if has_join_rtes {
            clause = rewritemanip_seams::flatten_join_alias_vars::call(
                mcx,
    ...
    if pstate.p_hasAggs && has_self_ref_rtes {
        let qry_node = Node::mk_query(mcx, qry.clone_in(mcx)?)?;
        let loc = rewritemanip_seams::locate_agg_of_level::call(&qry_node, 0);
        return Err(ereport(ERROR)
...
fn flatten_node_list<'mcx>(
    ...
    for n in list {
        out.push(rewritemanip_seams::flatten_join_alias_vars::call(
            mcx, None, qry_node, n,
        )?);
    }
    Ok(out)
...
fn substitute_grouped_columns_mutator<'mcx>(
    ...
    if context.have_non_var_grouping && context.sublevels_up == 0 {
        if let Some(this_expr) = node.as_expr() {
            ...
            for tle in context.group_clauses {
                attnum += 1;
                if let Some(e) = tle.expr.as_deref() {
                    if equalfuncs_seams::equal_expr::call(this_expr, e) {
                        let mcx = context.mcx;
    ...
    if let Some(var) = node.as_var() {
        ...
        if rte.rtekind == RTEKind::RTE_RELATION {
            ...
            match pg_constraint::check_functional_grouping(
                context.mcx,
                rte.relid,
                varno as u32,
                0,
        ...
        let attname = match parser_relation::get_rte_attribute_name(
            context.mcx,
            rte,
            varattno,
        ) {
...
fn compute_grouping_refs(
    ...
    for expr in &grp.args {
        ...
        let expr_node: Node = if context.has_join_rtes {
            flat_node = rewritemanip_seams::flatten_join_alias_vars::call(
                context.mcx,
                None, // C parse_agg.c passes NULL root
                context.qry,
        ...
        if let Some(var) = cur_expr.as_var() {
            ...
            for tle in context.group_clauses {
                if let Some(e) = tle.expr.as_deref() {
                    if equalfuncs_seams::equal_expr::call(cur_expr, e) {
                        ref_ = tle.ressortgroupref;
                        break;
                    }
                }
...
pub fn resolve_aggregate_transtype<'mcx>(
    ...
    if IsPolymorphicType(aggtranstype) {
        ...
        let sig =
            lsyscache_seams::get_func_signature::call(mcx, aggfuncid)?;
        ...
        aggtranstype = coerce::enforce_generic_type_consistency(
            input_types,
...
pub fn agg_args_support_sendreceive(aggref: &Aggref) -> PgResult<bool> {
    for tle in &aggref.args {
        ...
        let row =
            lsyscache_seams::get_type_sendreceive_byval::call(typ)?;
...
pub fn init_seams() {
    parse_agg_seams::transform_grouping_func::set(transformGroupingFunc);
    parse_agg_seams::transform_aggregate_call::set(transformAggregateCall);
    parse_agg_seams::transform_window_func_call::set(transformWindowFuncCall);
    parse_agg_seams::parse_check_aggregates::set(parseCheckAggregates);
    parse_agg_seams::expand_grouping_sets::set(expand_grouping_sets);
    parse_agg_seams::get_aggregate_argtypes::set(get_aggregate_argtypes);
    parse_agg_seams::resolve_aggregate_transtype::set(resolve_aggregate_transtype);
    parse_agg_seams::agg_args_support_sendreceive::set(agg_args_support_sendreceive);
    parse_agg_seams::build_aggregate_transfn_expr::set(build_aggregate_transfn_expr);
    parse_agg_seams::build_aggregate_serialfn_expr::set(build_aggregate_serialfn_expr);
    parse_agg_seams::build_aggregate_deserialfn_expr::set(build_aggregate_deserialfn_expr);
    parse_agg_seams::build_aggregate_finalfn_expr::set(build_aggregate_finalfn_expr);
}
