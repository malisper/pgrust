...
pub fn transformWhereClause<'mcx>(
    ...
    let qual =
        coerce::coerce_to_boolean(mcx, Some(pstate), qual, constructName)?;
    Ok(Some(qual))
}
        ...
        if cref.fields.len() == 1 {
            if let Some(field0_name) = str_val(&cref.fields[0]) {
                ...
                if exprKind == EXPR_KIND_GROUP_BY {
                    ...
                    if parser_relation::colNameToVar(mcx, pstate, n, true, location)?
                        .is_some()
                    {
                        name = None;
                    }
...
pub fn init_seams() {
    clause_seams::transform_where_clause::set(transformWhereClause);
}
