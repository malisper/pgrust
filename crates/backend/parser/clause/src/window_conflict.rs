...
fn resolve_unique_index_expr<'mcx>(
    ...
    for ielem_node in infer.indexElems.iter() {
        ...
        let infercollid = if ielem.collation.is_empty() {
            ...
            parse_type::LookupCollation(mcx, Some(pstate), &collnames, loc)?
        };
        ...
        let inferopclass = if ielem.opclass.is_empty() {
            ...
            opclasscmds::get_opclass_oid(mcx, BTREE_AM_OID, &opclassname, false)?
        };
...
pub fn transformOnConflictArbiter<'mcx>(
     ...
     */
    if catalog_catalog::IsCatalogRelation(target_rel) {
        return Err(ereport(ERROR)
            .errcode(ERRCODE_FEATURE_NOT_SUPPORTED)
    ...
    if let Some(infer) = infer {
        ...
        if let Some(conname) = infer.conname.as_ref() {
            ...
            let (constraint_oid, conattnos) =
                pg_constraint::get_relation_constraint_attnos(
                    mcx,
                    relid,
                    conname.as_str(),
                    false,
...
fn RelationIsUsedAsCatalogTable(relation: &rel::RelationData<'_>) -> bool {
