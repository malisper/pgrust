...
fn format_type_with_typemod(mcx: Mcx<'_>, type_oid: Oid, typemod: i32) -> PgResult<String> {
    let s = adt_format_type::format_type_with_typemod(mcx, type_oid, typemod)?;
...
fn analyzeCTE<'mcx>(
    ...
    if let Some(cycle_box) = cte.cycle_clause.take() {
        ...
        cycle_clause.cycle_mark_type = {
            ...
            coerce::select_common_type(Some(pstate), &exprs, Some("CYCLE"))?
        };
        ...
        cycle_clause.cycle_mark_typmod = {
            let exprs = list_make2_exprs(&mark_value, &mark_default);
            coerce::select_common_typmod(&exprs, cycle_clause.cycle_mark_type)?
        };
        ...
        cycle_clause.cycle_mark_collation = {
            ...
            let coll =
                parse_collate::select_common_collation(Some(pstate), &mut exprs, true)?;
        ...
        let eq_opr = typcache_seams::lookup_type_cache_eq_opr::call(
            cycle_clause.cycle_mark_type,
        )?;
        ...
        let op = lsyscache::opfamily_operator::get_negator(eq_opr)?;
        if op == InvalidOid {
            return Err(ereport(ERROR)
                .errcode(ERRCODE_UNDEFINED_FUNCTION)
                .errmsg(format!(
    ...
    let query_node =
        parser_analyze_seams::parse_sub_analyze::call(mcx, &ctequery, pstate, Some(&*cte), false, true)?;
...
fn transform_cycle_expr<'mcx>(
    ...
    parse_expr::transformExpr(pstate, input, ParseExprKind::EXPR_KIND_CYCLE_MARK)
}
