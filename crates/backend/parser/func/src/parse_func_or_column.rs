...
struct FuncDetail<'mcx> {
    ...
    argdefaults: PgVec<'mcx, nodes::nodes::NodePtr<'mcx>>,
...
fn clone_node_ptr_vec<'mcx>(
    mcx: Mcx<'mcx>,
    src: Option<&PgVec<'mcx, nodes::nodes::NodePtr<'mcx>>>,
) -> PgResult<PgVec<'mcx, nodes::nodes::NodePtr<'mcx>>> {
...
fn transform_where_clause_filter<'mcx>(
    ...
    Ok(clause_seams::transform_where_clause::call(
        mcx,
        pstate,
        Some(clause),
    ...
    if let Some(best_candidate) = best_candidate {
        ...
        if with_argdefaults && best_candidate.ndargs > 0 {
            ...
            if !best_candidate.argnumbers.is_empty() {
                ...
                let mut newdefaults: PgVec<'mcx, nodes::nodes::NodePtr<'mcx>> =
                    PgVec::new_in(mcx);
...
fn unify_hypothetical_args<'mcx>(
    ...
    while hargpos < num_direct_args {
        ...
        let commontypmod =
            coerce_seams::select_common_typmod::call(&pair, commontype)?;
