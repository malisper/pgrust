...
fn conv_value_node<'mcx>(mcx: Mcx<'mcx>, n: *mut RawNode) -> PgResult<Node<'mcx>> {
    ...
    match tag {
        tags::T_Integer => {
            let v = unsafe { &*n.cast::<pg_ffi_fgram::nodes::Integer>() };
            Ok(Node::mk_integer(mcx, tn_val::Integer { ival: v.ival })?)
        }
        tags::T_Float => {
            let v = unsafe { &*n.cast::<pg_ffi_fgram::nodes::Float>() };
            Ok(Node::mk_float(mcx, tn_val::Float {
                fval: cstr(mcx, v.fval)?,
            })?)
        }
        tags::T_Boolean => {
            let v = unsafe { &*n.cast::<pg_ffi_fgram::nodes::Boolean>() };
            Ok(Node::mk_boolean(mcx, tn_val::Boolean { boolval: v.boolval })?)
        }
        tags::T_String => {
            let v = unsafe { &*n.cast::<pg_ffi_fgram::nodes::StringNode>() };
            Ok(Node::mk_string(mcx, tn_val::StringNode {
                sval: cstr(mcx, v.sval)?,
            })?)
        }
        tags::T_BitString => {
            let v = unsafe { &*n.cast::<pg_ffi_fgram::nodes::BitString>() };
