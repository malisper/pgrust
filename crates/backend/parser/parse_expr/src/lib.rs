    ...
    match expr.node_tag() {
        ...
        NodeTag::T_A_Expr => {
            ...
            match a.kind {
                ...
                A_Expr_Kind::AEXPR_OP_ANY => transformAExprOpAnyAll(mcx, pstate, a, true),
                A_Expr_Kind::AEXPR_OP_ALL => transformAExprOpAnyAll(mcx, pstate, a, false),
...
fn transformAExprOpAnyAll<'mcx>(
    mcx: Mcx<'mcx>,
    pstate: &mut ParseState<'_, 'mcx>,
    a: &A_Expr<'mcx>,
    use_or: bool,
) -> PgResult<Node<'mcx>> {
    let lexpr = transformExprRecurse(mcx, pstate, a.lexpr.expect("ANY/ALL lexpr"))?;
    let rexpr = transformExprRecurse(mcx, pstate, a.rexpr.expect("ANY/ALL rexpr"))?;
