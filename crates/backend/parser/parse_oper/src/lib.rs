...
fn type_name_arg(objargs: &[opclass::TypeName], i: usize) -> Option<&opclass::TypeName> {
    objargs.get(i)
}
...
fn format_type_be(type_oid: Oid) -> PgResult<String> {
    adt_format_type::format_type_be_owned(type_oid)
}
...
fn make_oper_cache_key(
    ...
    let (schemaname, opername) =
        catalog_namespace::DeconstructQualifiedName(cx.mcx(), &name_list)?;
    ...
    *key = OprCacheKey::default();
    ...
    if let Some(schemaname) = schemaname {
        ...
        let count = catalog_namespace::fetch_search_path_array(cx.mcx(), &mut path)?;
...
/// Install the seams this crate owns. Wired into `init::init_all()`.
...
fn generate_operator_name<'mcx>(
    ...
    if need_qual {
        ...
        let qnsp =
            ruleutils_seams::quote_identifier::call(mcx, nspname.as_str())?;
...
pub fn init_seams() {
    parse_oper_seams::lookup_oper_with_args::set(seam_lookup_oper_with_args);
    parse_oper_seams::lookup_oper_with_args_node::set(
        seam_lookup_oper_with_args_node,
    );
    parse_oper_seams::lookup_oper_name::set(seam_lookup_oper_name);

    // ruleutils' `generate_operator_name` (the deparser's operator-name
    // generator) — owned here because its body needs `oper`/`left_oper`.
    ruleutils_seams::generate_operator_name::set(generate_operator_name);
}
