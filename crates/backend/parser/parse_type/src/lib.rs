...
fn parser_errposition(pstate: Option<&types_cluster::ParseState<'_>>, location: i32) -> PgResult<i32> {
    match pstate {
        Some(ps) => small1_seams::parser_errposition::call(ps, location),
        None => Ok(0),
    }
}
...
pub fn typeStringToTypeName(
    ...
    let typeName = driver_seams::raw_parse_type_name::call(s.to_string())
...
pub fn rich_functionparameter_to_parse(
    ...
    // TypeCast / FuncCall / ...) that the flat `parsenodes` vocabulary
...
/// `TypeNameToString(typeName)` over the trimmed `opclass::TypeName`.
...
/// opclasscmds/function `opclass::TypeName` carrier — the `ObjectWithArgs`
