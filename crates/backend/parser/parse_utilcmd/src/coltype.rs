...
pub fn transformColumnType<'mcx>(
    ...
    if let Some(coll_clause) = column.collClause.as_deref() {
        ...
        for n in coll_clause.collname.iter() {
            match n.node_tag() {
                ntag::T_String => {
                    collname_pn.push(parsenodes::Node::String(
...
pub(crate) fn raw_typename_to_parse(
    ...
    for n in tn.names.iter() {
        match n.node_tag() {
            ntag::T_String => names.push(parsenodes::Node::String(
                parsenodes::StringNode {
    ...
    for n in tn.arrayBounds.iter() {
        match n.as_integer() {
            Some(i) => array_bounds.push(parsenodes::Node::Integer(
                parsenodes::Integer { ival: i.ival },
            )),
            None => array_bounds.push(parsenodes::Node::Integer(parsenodes::Integer {
                ival: -1,
            })),
        }
    }
