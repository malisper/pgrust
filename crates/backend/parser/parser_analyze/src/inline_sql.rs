...
pub fn inline_sql_function<'mcx>(
    ...
    let fn_typtype = if is_void {
        ...
        lsyscache_seams::get_typtype::call(result_type)?
    };
    ...
    if !is_void && !is_scalar && !is_rowtype {
        let tyname =
            format_type_seams::format_type_be::call(mcx, result_type)?;
        return Err(PgError::error(format!(
    ...
    if !is_void && src_type != result_type && result_type != RECORDOID {
        ...
        match coerced {
            Some(c) => {
                ...
                parse_collate::assign_expr_collations_in(mcx, &mut c)?;
                newexpr = c;
            }
            None => {
                return Err(PgError::error(format!(
    ...
    if form.provolatile == PROVOLATILE_IMMUTABLE
        && clauses::contain_mutable_functions(Some(&newexpr))?
    {
        return Ok(None);
    } else if form.provolatile == PROVOLATILE_STABLE
        && clauses::contain_volatile_functions(Some(&newexpr))?
    {
        return Ok(None);
    }
    if form.proisstrict
        && clauses::contain_nonstrict_functions(Some(&newexpr))?
    {
        return Ok(None);
    }
    ...
    for a in args {
        if clauses::contain_context_dependent_node(Some(a))? {
            return Ok(None);
        }
    }
    ...
    for (i, param) in args.iter().enumerate() {
        ...
        if count == 0 {
            ...
            if clauses::contain_subplans(Some(param))? {
                return Ok(None);
            }
            ...
            if clauses::contain_volatile_functions(Some(param))? {
                return Ok(None);
            }
        }
    }
