...
pub fn inline_set_returning_function_sql_body<'mcx>(
    mcx: Mcx<'mcx>,
    _root: &mut pathnodes::PlannerInfo,
    ...
    let mut querytree: Query<'mcx> = if let Some(body) = prosqlbody.as_ref() {
        ...
        // AcquireRewriteLocks(querytree, true, false).
        let query = rewritehandler_seams::acquire_rewrite_locks::call(
        ...
        let raw_list =
            driver::raw_parser(mcx, prosrc_mcx, RawParseMode::RAW_PARSE_DEFAULT)?;
        if raw_list.len() != 1 {
            return Ok(None);
        }
        ...
        let rewritten =
            rewritehandler_seams::query_rewrite_canonical::call(mcx, analyzed)?;
        if rewritten.len() != 1 {
            return Ok(None);
        }
        rewritten.into_iter().next().unwrap()
    ...
    if !rtfunc.funccolnames.is_empty() {
        ...
        let desc = tupdesc::BuildDescFromLists(
            mcx,
            &names,
            &types,
            &typmods,
        ...
        let resolved =
            funcapi_seams::get_expr_result_type::call(mcx, Some(&fexpr_node))?;
...
fn check_sql_stmt_retval<'mcx>(
    ...
    let fn_typtype = lsyscache_seams::get_typtype::call(rettype)?;
...
fn fmt_type(oid: Oid) -> String {
    match format_type_seams::format_type_be_str::call(oid) {
        Ok(name) => name,
...
fn srf_param_mutator<'mcx>(
    ...
    if let Some(param) = node.as_expr().and_then(|e| e.as_param()) {
        if param.paramkind == ParamKind::PARAM_EXTERN {
            ...
            if let Err(e) =
                rewrite_core::increment::IncrementVarSublevelsUp(&mut new_node, *sublevels_up, 0, mcx)
            {
                *err = Some(e);
                return true;
            }
