...
pub fn transformInsertStmt<'mcx>(
    ...
    let (icolumns, attrnos) =
        parse_target::checkInsertTargets(mcx, pstate, copy_cols(mcx, &stmt.cols)?)?;
    ...
    if stmt.onConflictClause.is_some() || stmt.returningClause.is_some() {
        ...
        parser_relation::addNSItemToQuery(mcx, pstate, target, false, true, true)?;
    }
...
fn transformInsertSelect<'mcx>(
    ...
    let mut sub_pstate = small1::make_parsestate(mcx, Some(pstate))?;
    ...
    small1::free_parsestate(sub_pstate)?;
    ...
    let alias = nodes_core::makefuncs::make_alias(mcx, "*SELECT*", PgVec::new_in(mcx))?;
    ...
    for tle in sub_target_list.iter() {
        ...
        ) && nodes_core::nodefuncs::expr_type(tle.expr.as_deref())? == UNKNOWNOID;
        ...
        let expr = if copy_up {
            ...
            let mut var =
                nodes_core::makefuncs::make_var_from_target_entry(rtindex, tle)?;
            var.location = nodes_core::nodefuncs::expr_location(tle.expr.as_deref())?;
            Expr::Var(var)
        };
...
fn transformInsertMultiRowValues<'mcx>(
    ...
    for row_node in select_stmt.valuesLists.iter() {
        ...
        parse_collate::assign_list_collations(Some(pstate), &mut row[..])?;
    ...
    let lateral = if pstate.p_rtable.len() != 1 {
        ...
        vars::contain_vars_of_level(&probe, 0)
    } else {
        false
    };

    // Generate the VALUES RTE.
    let nsitem = parser_relation::addRangeTableEntryForValues(
        mcx,
        pstate,
        exprs_lists,
        coltypes,
    ...
    let var_nodes = parser_relation::expandNSItemVars(mcx, pstate, &nsitem, 0, -1, None)?;
...
pub fn transformInsertRow<'mcx>(
    ...
    if exprlist.len() > icolumns.len() {
        ...
        let loc = nodes_core::nodefuncs::expr_location(Some(&exprlist[icolumns.len()]))?;
        return Err(ereport(ERROR)
            .errcode(ERRCODE_SYNTAX_ERROR)
    ...
    for (idx, expr) in exprlist.into_iter().enumerate() {
        ...
        let mut transformed = parse_target::transformAssignedExpr(
            mcx,
            pstate,
            Some(expr),
    ...
    match expr {
        ...
        Expr::Var(var) => {
            ...
            if attnum > 0 && var.vartype == RECORDOID {
                let rte = parser_relation::GetRTEByRangeTablePosn(
                    pstate,
                    var.varno,
...
fn transformOnConflictClause<'mcx>(
    ...
    if on_conflict_clause.action == OnConflictAction::ONCONFLICT_UPDATE {
        let alias = nodes_core::makefuncs::make_alias(mcx, "excluded", PgVec::new_in(mcx))?;
        ...
        let mut excl_nsitem = parser_relation::addRangeTableEntryForRelation(
            mcx,
            pstate,
            &targetrel,
    ...
    let (arbiter_exprs, arbiter_where_expr, arbiter_constraint) =
        clause::transformOnConflictArbiter(
            mcx,
            pstate,
            on_conflict_clause,
        )?;
    ...
    if on_conflict_clause.action == OnConflictAction::ONCONFLICT_UPDATE {
        ...
        parser_relation::addNSItemToQuery(mcx, pstate, excl_nsitem, false, true, true)?;
        ...
        let where_expr = clause::transformWhereClause(
            mcx,
            pstate,
            where_clause,
...
pub fn build_on_conflict_excluded_targetlist<'mcx>(
    mcx: Mcx<'mcx>,
    targetrel: &rel::RelationData<'mcx>,
    excl_rel_index: i32,
    ...
    for attno in 0..natts {
        ...
        let (var, name): (Expr, Option<&str>) = if attr.attisdropped {
            ...
            let c = nodes_core::makefuncs::make_null_const(
                mcx,
            ...
            let v = nodes_core::makefuncs::make_var(
                excl_rel_index,
                (attno + 1) as i16,
                attr.atttypid,
                attr.atttypmod,
