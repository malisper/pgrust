...
fn set_query_env_from_home<'mcx>(mcx: Mcx<'mcx>, pstate: &mut ParseState<'mcx>) -> PgResult<()> {
    let child = queryenvironment_home::with_top_query_env(|env| match env {
...
pub fn parse_analyze_fixedparams<'mcx>(
    ...
    if !param_types.is_empty() {
        small1::setup_parse_fixed_parameters(&mut pstate, param_types);
    }
...
pub fn parse_analyze_plpgsql_expr<'mcx>(
    ...
    // ref-hook state.
    parse_expr::setup_parse_plpgsql_expr(&mut pstate, state);
...
fn transform_one_body_stmt<'mcx>(
    ...
    if q.commandType == CmdType::CMD_UTILITY {
        ...
        let tagname = match &q.utilityStmt {
            Some(ustmt) => {
                let tag = utility_seams::create_command_tag::call(ustmt.as_ref())?;
                cmdtag::get_command_tag_name(tag.0)
            }
            None => "statement",
        };
...
pub fn interpret_sql_body<'mcx>(
    ...
    // path (`nodes_core::read::string_to_node`) consumes.
    let text = outfuncs::nodeToString(mcx, &cooked)?
        .as_str()
        .to_string();
    ...
    // there.
    let mut refs_ctx =
        dependency::find_expr::FindExprReferencesContext::new(mcx);
    dependency::find_expr::find_expr_references_walker(&cooked, &mut refs_ctx)?;
    if let Some(e) = refs_ctx.err.take() {
        return Err(e);
    }
    let body_refs = refs_ctx.addrs.refs;

    Ok(functioncmds_seams::InterpretedSqlBody { text, body_refs })
}
...
fn cook_parameter_defaults<'mcx>(
...
) -> PgResult<functioncmds_seams::CookedParameterDefaults> {
    let nargdefaults = defaults.len() as i32;
    if defaults.is_empty() {
        return Ok(functioncmds_seams::CookedParameterDefaults::default());
    }
    ...
    let text = outfuncs::nodeToString(mcx, &list)?
        .as_str()
        .to_string();
    ...
    let mut refs_ctx =
        dependency::find_expr::FindExprReferencesContext::new(mcx);
    dependency::find_expr::find_expr_references_walker(&list, &mut refs_ctx)?;
    if let Some(e) = refs_ctx.err.take() {
        return Err(e);
    }
    let refs = refs_ctx.addrs.refs;

    Ok(functioncmds_seams::CookedParameterDefaults {
        text: Some(text),
        nargdefaults,
        refs,
    })
...
fn check_defaults_compatible(
    ...
    let new_node = nodes_core::read::string_to_node(mcx, &new_proargdefaults)?;
    ...
    for (oldlc, newlc) in old_defaults.iter().zip(new_defaults.iter().skip(skip)) {
        let old_ty = nodes_core::nodefuncs::expr_type((**oldlc).as_expr())?;
        let new_ty = nodes_core::nodefuncs::expr_type((**newlc).as_expr())?;
        if old_ty != new_ty {
...
fn transformExplainStmt<'mcx>(
    ...
    // make sure all is well with parameter types.
    if generic_plan {
        small1::check_variable_parameters(pstate, &transformed)?;
    }
...
fn transformCreateTableAsStmt<'mcx>(
    ...
    if stmt.objtype == nodes::parsenodes::ObjectType::Matview {
         ...
         * creation query.
         */
        if parser_relation::isQueryUsingTempRelation(mcx, &query)? {
            return Err(ereport(ERROR)
                .errcode(ERRCODE_FEATURE_NOT_SUPPORTED)
         ...
         */
        if small1::query_contains_extern_params(&query) {
            return Err(ereport(ERROR)
                .errcode(ERRCODE_FEATURE_NOT_SUPPORTED)
...
pub fn init_seams() {
    parser_analyze_seams::parse_sub_analyze::set(parse_sub_analyze);
    ...
    parser_analyze_seams::stmt_requires_parse_analysis_value::set(
        stmt_requires_parse_analysis_value_impl,
    );
    parser_analyze_seams::analyze_requires_snapshot_value::set(
        analyze_requires_snapshot_value_impl,
    );
    parser_analyze_seams::query_requires_rewrite_plan_value::set(
    ...
    functioncmds_seams::interpret_sql_body::set(interpret_sql_body);
    ...
    functioncmds_seams::transform_parameter_default::set(
        transform_parameter_default,
    );
    functioncmds_seams::cook_parameter_defaults::set(cook_parameter_defaults);
    ...
    parser_analyze_seams::run_post_parse_analyze_hook::set(
    ...
    createas_seams::jumble_and_post_analyze::set(jumble_and_post_analyze_impl);
    ...
    clauses_seams::inline_sql_function::set(
    ...
    clauses_seams::inline_set_returning_function_sql_body::set(
