...
pub fn transformSelectStmt<'mcx>(
    ...
    /* mark column origins */
    parse_target::markTargetListOrigins(mcx, pstate, &mut tlist)?;
...
pub fn transformValuesClause<'mcx>(
    ...
    for i in 0..(sublist_length.max(0) as usize) {
        ...
        for col in col_take {
            let c = coerce::coerce_to_common_type(
                mcx,
                Some(&mut *pstate),
        ...
        let coltypmod = coerce::select_common_typmod(&coerced, coltype)?;
        let mut coerced_mut = coerced;
        let colcoll = parse_collate::select_common_collation(
            Some(&*pstate),
            &mut coerced_mut,
            true,
        )?;
    ...
    if !pstate.p_rtable.is_empty() {
        ...
        if vars::var::contain_vars_of_level(&probe, 0) {
            lateral = true;
        }
    }
    ...
    parser_relation::addNSItemToQuery(mcx, pstate, nsitem, true, true, true)?;

    /* Generate a targetlist as though expanding "*" */
    let mut tlist_vec: Vec<_> =
        parser_relation::expandNSItemAttrs(mcx, pstate, ns_index, 0, true, -1)?
            .into_iter()
            .collect();
