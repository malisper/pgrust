...
pub fn makeSortGroupClauseForSetOp(
    ...
    let ops =
        parse_oper::get_sort_group_operators(rescoltype, false, true, false, true)?;
...
pub fn transformSetOperationStmt<'mcx>(
        ...
        leftq
            ...
            .map(|tle| {
                let loc = tle
                    .expr
                    .as_deref()
                    .and_then(|e| nodes_core::nodefuncs::expr_location(Some(e)).ok())
                    .unwrap_or(-1);
                (
                    tle.resno,
    ...
    for i in 0..col_types.len() {
        ...
        let mut var = nodes_core::makefuncs::make_var(
            leftmost_rti,
            *resno,
            col_type,
            col_typmod,
    ...
    let jnsitem = parser_relation::addRangeTableEntryForJoin(
        mcx,
        pstate,
        &targetnames,
        sortnscolumns,
    ...
    parser_relation::addNSItemToQuery(mcx, pstate, jnsitem, false, false, true)?;
    ...
    let transformed_sort = clause::transformSortClause(
        mcx,
        pstate,
        &sort_input,
        &mut tlist,
    ...
    let lo = clause::transformLimitClause(
        mcx,
        pstate,
    ...
    let lc = clause::transformLimitClause(
        mcx,
        pstate,
...
fn transformSetOperationTree<'mcx>(
    ...
    if is_leaf {
        ...
        if !pstate.p_namespace.is_empty()
            && vars::var::contain_vars_of_level(&select_query_node, 1)
        {
            return Err(elog_error(
        ...
        let alias = nodes_core::makefuncs::make_alias(mcx, &select_name, PgVec::new_in(mcx))?;
        let selectq_owned = select_query.clone_in(mcx)?;
        let nsitem = parser_relation::addRangeTableEntryForSubquery(
            mcx,
            pstate,
            selectq_owned,
            Some(alias),
    ...
    for (ltle, rtle) in ltargetlist.iter().zip(rtargetlist.iter()) {
        ...
        let rescoltype =
            coerce::select_common_type(Some(pstate), &exprs, Some(context))?;
        ...
        let lcolnode2: Expr<'static> = if lcoltype != UNKNOWNOID {
            coerce::coerce_to_common_type(mcx, Some(pstate), lcolnode.clone_in(mcx)?.erase_lifetime(), rescoltype, context)?
        ...
        let rcolnode2: Expr<'static> = if rcoltype != UNKNOWNOID {
            coerce::coerce_to_common_type(mcx, Some(pstate), rcolnode.clone_in(mcx)?.erase_lifetime(), rescoltype, context)?
        ...
        let rescoltypmod = coerce::select_common_typmod(&coerced, rescoltype)?;
        ...
        let restle = nodes_core::makefuncs::make_target_entry(
            mcx,
...
fn determineRecursiveColTypes<'mcx>(
    ...
    let r = cte::analyzeCTETargetList(mcx, pstate, &mut cte, &target_list);
...
fn exprtype(e: &Expr) -> PgResult<Oid> {
    nodes_core::nodefuncs::expr_type(Some(e))
}
