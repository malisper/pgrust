...
pub fn transformCallStmt<'mcx>(
    ...
    let last_srf = parse_expr::last_srf_expr(pstate);
    let funcname = parse_expr::clone_namelist_pgstrings(&funccall.funcname, mcx)?;
    let node = func::ParseFuncOrColumn(
        pstate,
        &funcname,
        targs,
        last_srf.as_ref(),
    ...
    parse_collate::assign_expr_collations(Some(pstate), &mut node)?;
     ...
     */
    let proc = clauses_seams::get_func_form::call(fexpr.funcid)?;

    fexpr.args = clauses::expand_function_arguments(
        mcx,
