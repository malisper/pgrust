...
pub fn transformDeleteStmt<'mcx>(
     ...
     */
    clause::transformFromClause(mcx, pstate, &stmt.usingClause)?;
...
pub fn transformUpdateStmt<'mcx>(
     ...
     */
    clause::transformFromClause(mcx, pstate, &stmt.fromClause)?;
    ...
    for tle in tlist.iter_mut() {
        ...
        let attrno = {
            let rel = pstate
                ...
                .as_deref()
                .ok_or_else(|| elog_error("UPDATE: no target relation"))?;
            parser_relation::attnameAttNum(rel, &colname, true)?
        };
        ...
        if perminfoindex > 0 {
            ...
            pi.updatedCols = Some(nodes_core::bitmapset::bms_add_member(
                mcx,
                updated,
...
fn addNSItemForReturning<'mcx>(
    ...
    parser_relation::addNSItemToQuery(mcx, pstate, nsitem, false, true, false)
}
...
pub(crate) fn transformReturningClause<'mcx>(
    ...
    for opt_node in rc.options.iter() {
        ...
        if parser_relation::refnameNamespaceItem(
            pstate,
            None,
            value.as_str(),
            -1,
    ...
    let mut returning_list =
        parse_target::transformTargetList(mcx, pstate, res_targets, expr_kind)?;
    ...
    if returning_list.is_empty() {
        ...
        return Err(ereport(ERROR)
            ...
            .errposition(small1::parser_errposition(pstate, loc))
            .into_error());
    }

    // mark column origins
    parse_target::markTargetListOrigins(mcx, pstate, &mut returning_list)?;

    // resolve any still-unresolved output columns as being type text
    if pstate.p_resolve_unknowns {
        parse_target::resolveTargetListUnknowns(mcx, pstate, &mut returning_list)?;
    }
