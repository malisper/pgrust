...
pub fn refnameNamespaceItem(
    ...
    if let Some(schemaname) = schemaname {
        ...
        let namespace_id = catalog_namespace::LookupNamespaceNoError(schemaname)?;
...
pub fn scanNameSpaceForENR(pstate: &ParseState<'_>, refname: &str) -> bool {
    small1::name_matches_visible_ENR(pstate, refname)
}
...
fn searchRangeTableForRel<'mcx>(
    ...
    if cte.is_none() && !isenr {
        rel_id = catalog_namespace::RangeVarGetRelid(mcx, &to_access_range_var(relation), NoLock, true)?;
    }
...
fn updateFuzzyAttrMatchState(
    ...
    let mut columndistance = varlena::misc_encoding::varstr_levenshtein_less_equal(
        actual.as_bytes(),
        actual.len() as i32,
        match_.as_bytes(),
        matchlen,
    ...
    for depth in 0..=max_depth {
        ...
        for (rte_index, rte) in ps.p_rtable.iter().enumerate() {
            ...
            if let Some(alias) = alias {
                ...
                fuzzy_rte_penalty = varlena::misc_encoding::varstr_levenshtein_less_equal(
                    alias.as_bytes(),
                    alias.len() as i32,
                    aliasname.as_bytes(),
...
pub fn parserOpenTable<'mcx>(
    ...
    let rel = table::table_openrv_extended(mcx, &to_access_range_var(relation), lockmode, true)?;
    match rel {
        Some(rel) => Ok(rel),
        None => {
...
pub fn addRangeTableEntryForFunction<'mcx>(
    ...
    for funcno in 0..nfuncs as usize {
        ...
        let resolved =
            funcapi::result_type::get_expr_result_type(mcx, Some(funcexpr_node))?;
        ...
        let final_tupdesc: TupleDescData<'mcx> = match functypclass {
            ...
            Some(TypeFuncClass::Record) => {
                ...
                let mut td =
                    tupdesc::CreateTemplateTupleDesc(mcx, coldeflist.len() as i32)?;
                ...
                for col in coldeflist.iter() {
                    ...
                    let (attrtype, attrtypmod) =
                        tablecmds_seams::typename_type_id_and_mod::call(
                            mcx, type_name,
                        )?;
                    // C: attrcollation = GetColumnDefCollation(pstate, n, attrtype);
                    let attrcollation =
                        tablecmds_seams::get_column_def_collation::call(
                            mcx, n, attrtype,
                        )?;
                ...
                heap_seams::check_attribute_names_types::call(
                    mcx,
                    &td,
            ...
            _ => {
                ...
                return Err(ereport(ERROR)
                    ...
                    .errmsg(format!(
                        ...
                        funcname.as_str(),
                        format_type_seams::format_type_be_str::call(funcrettype)?
                    ))
                    .errposition(parser_errposition(
                        pstate,
                        ::nodes_core::nodefuncs::expr_location(funcexpr_expr)?,
    ...
    let tupdesc: TupleDescData<'mcx> = if nfuncs > 1 || rangefunc.ordinality {
        ...
        let mut merged = tupdesc::CreateTemplateTupleDesc(mcx, totalatts)?;
...
pub fn addRangeTableEntryForENR<'mcx>(
    ...
    let (enrtype, reliddesc, enrname, enrtuples, tupdesc_owned) = {
        let enrmd = small1::get_visible_ENR(pstate, rv_relname)
            .expect("addRangeTableEntryForENR: ENR must be visible");
        let td = queryenvironment::ENRMetadataGetTupDesc(mcx, enrmd)?
            ...
            for func_node in rte.functions.iter() {
                ...
                if attnum_i32 > atts_done && attnum_i32 <= atts_done + rtfunc.funccolcount {
                    ...
                    let funcexpr_node: Option<&Node<'mcx>> =
                        rtfunc.funcexpr.as_deref();
                    let tupdesc = funcapi::result_type::get_expr_result_tupdesc(
                        mcx,
                        funcexpr_node,
                        true,
                    )?;
...
pub fn attnumTypeId(rd: &rel::RelationData<'_>, attid: i32) -> PgResult<Oid> {
    if attid <= 0 {
...
pub fn attnumCollationId(rd: &rel::RelationData<'_>, attid: i32) -> PgResult<Oid> {
    if attid <= 0 {
...
fn isQueryUsingTempRelation_walker_query(mcx: Mcx<'_>, query: &Query<'_>) -> PgResult<bool> {
    for rte in query.rtable.iter() {
        if rte.rtekind == RTE_RELATION {
            let rel = table::table_open(mcx, rte.relid, AccessShareLock)?;
...
pub fn init_seams() {
    parser_relation_seams::get_rte_permission_info::set(getRTEPermissionInfo);
    parser_relation_seams::scan_ns_item_for_column_by_posn::set(
    ...
    ruleutils_seams::get_rte_attribute_name::set(get_rte_attribute_name);
    ...
    ruleutils_seams::ruleutils_expand_function_rte_colnames::set(
    ...
    plancat_ext_seams::expand_function_rte_colvars::set(
