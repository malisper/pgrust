    ...
    fn lex_initial(&mut self) -> Result<Option<Token>, LexError> {
        ...
        match rule {
            ...
            InitialRule::Xnstart => {
                ...
                self.set_yylloc();
                self.yyless(1);
                let kwnum = keywords_fgram::ScanKeywordLookup("nchar", &keywords_fgram::ScanKeywords);
                if kwnum >= 0 {
                    let kw = keywords_fgram::GetScanKeyword(kwnum as usize, &keywords_fgram::ScanKeywords)
            ...
            InitialRule::Param => {
                ...
                let val = numutils_fgram::pg_strtoint32_safe(s, Some(&mut escontext));
                match val {
            ...
            InitialRule::Identifier => {
                ...
                let kwnum = {
                    ...
                    keywords_fgram::ScanKeywordLookup(text, &keywords_fgram::ScanKeywords)
                };
                if kwnum >= 0 {
                    let kw = keywords_fgram::GetScanKeyword(
                        kwnum as usize,
                        &keywords_fgram::ScanKeywords,
                    )
