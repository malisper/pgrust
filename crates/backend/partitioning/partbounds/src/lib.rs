...
fn datum_copy<'mcx>(
    ...
    scalar_seams::datum_copy::call(mcx, value, typbyval, typlen)
}
...
pub fn check_new_partition_bound<'mcx, 'k, 'd, 's>(
    ...
    let errpos = |location: i32| -> i32 {
        match pstate {
            Some(ps) if location >= 0 => {
                small1_seams::parser_errposition::call(ps, location).unwrap_or(0)
            }
            _ => 0,
        }
    };
    ...
    match key.strategy {
        ...
        PartitionStrategy::Range => {
            ...
            if cmpval > 0 {
                let lower_str =
                    partbounds_seams::get_range_partbound_string::call(
                        mcx,
                        &spec.lowerdatums,
                    )?;
                let upper_str =
