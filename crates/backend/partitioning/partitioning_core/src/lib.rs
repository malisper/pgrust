...
fn enable_partition_pruning() -> bool {
    (guc_tables::vars::enable_partition_pruning
        .get()
        .get)()
}
...
fn collect_prune_inputs_with_clauses<'mcx>(
    ...
    let relation = table_seams::table_open::call(
        mcx,
        reloid_oid,
    ...
    let partkey =
        partcache_seams::relation_get_partition_key::call(mcx, relation.alias())?
            .ok_or_else(|| {
    ...
    let mut partdesc =
        partdesc::RelationGetPartitionDesc(mcx, &relation, true)?;
...
fn gen_partprune_steps_internal<'mcx>(
    ...
    if context.has_default && context.has_partition_qual {
        if predtest_seams::predicate_refuted_by_exprs::call(
            context.mcx,
...
fn match_opexpr_to_partition_key<'mcx>(
    ...
    if !matches!(expr, Expr::Const(_)) {
        ...
        if var_seams::contain_var_clause::call(&expr) {
            return Ok(PartClauseMatchStatus::Unsupported);
        }
        // Reject anything containing a volatile function (stable is OK).
        if path_small_seams::contain_volatile_functions_expr::call(&expr) {
            return Ok(PartClauseMatchStatus::Unsupported);
        }
...
fn match_saop_to_partition_key<'mcx>(
    ...
    if !matches!(rightop, Expr::Const(_)) {
        ...
        // We can never prune using an expression that contains Vars.
        if var_seams::contain_var_clause::call(rightop) {
            return Ok(PartClauseMatchStatus::Unsupported);
        }
        // Reject anything containing a volatile function (stable is OK).
        if path_small_seams::contain_volatile_functions_expr::call(rightop) {
            return Ok(PartClauseMatchStatus::Unsupported);
        }
        // See if there are any exec Params. If so, usable only at per-scan time.
...
fn partkey_datum_from_expr_exec<'mcx>(
    ...
    execExpr_seams::exec_eval_expr_switch_context::call(exprstate, ectx, estate)
}
...
fn get_matching_hash_bounds(
    ...
    if nvalues + nullkeys.len() as i32 == partnatts {
        ...
        let _ = base_stateidx;
        let row_hash = partbounds_seams::compute_partition_hash_value::call(
            context.partkey,
            values,
            &isnull,
        )?;
...
fn get_commutator(opno: Oid) -> PgResult<Oid> {
    lsyscache_seams::get_commutator::call(opno)
}
...
fn op_in_opfamily(opno: Oid, opfamily: Oid) -> PgResult<bool> {
    lsyscache_seams::op_in_opfamily::call(opno, opfamily)
}
fn op_strict(opno: Oid) -> PgResult<bool> {
    lsyscache_seams::op_strict::call(opno)
}
fn op_volatile(opno: Oid) -> PgResult<u8> {
    lsyscache_seams::op_volatile::call(opno)
}
fn get_opfamily_proc(opfamily: Oid, lefttype: Oid, righttype: Oid, procnum: i16) -> PgResult<Oid> {
    lsyscache_seams::get_opfamily_proc::call(opfamily, lefttype, righttype, procnum)
}
...
fn get_op_opfamily_properties(opno: Oid, opfamily: Oid, ordering_op: bool) -> PgResult<(i32, Oid, Oid)> {
    lsyscache_seams::get_op_opfamily_properties::call(opno, opfamily, ordering_op, false)?
        .ok_or_else(|| PgError::error("operator is not a member of opfamily"))
}

fn node_equal(a: &Expr, b: &Expr) -> PgResult<bool> {
    Ok(nodeFuncs_seams::equal::call(a, b))
}
fn is_notclause(clause: &Expr) -> bool {
    nodeFuncs_seams::is_notclause::call(clause)
}
fn get_notclausearg<'a, 'mcx>(clause: &'a Expr<'mcx>) -> &'a Expr<'mcx> {
    nodeFuncs_seams::get_notclausearg::call(clause)
}
...
fn deconstruct_const_array<'mcx>(
    ...
    let elemtype =
        match lsyscache_seams::get_element_type::call(arr.consttype)? {
            Some(t) => t,
    ...
    let s = lsyscache_seams::get_typlenbyvalalign::call(elemtype)?;

    // deconstruct_array(DatumGetArrayTypeP(arr->constvalue), ARR_ELEMTYPE,
    //                   elemlen, elembyval, elemalign,
    //                   &elem_values, &elem_nulls, &num_elems);
    let deconstructed = arrayfuncs_seams::deconstruct_array_v::call(
        mcx,
