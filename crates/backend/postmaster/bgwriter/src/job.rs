...
struct EnvelopeBind {
    ...
    /// out of range)", CI cluster job ...-261e). Released + deleted at unbind;
...
impl EnvelopeBind {
    fn bind(pid: pid_t, procno: ProcNumber, overlay: &Overlay) -> EnvelopeBind {
        ...
        // ResourceOwnerEnlarge on the NULL owner, CI cluster job ...-24fb).
...
impl BgJob for BgWriterJob {
    ...
    fn startup(&self) -> Result<(), Box<PgError>> {
        ...
        // crash-loops hot (observed: CI cluster job ...-42c3).
    ...
    fn run_cycle(&self, reason: CycleReason) -> CycleOutcome {
        ...
        // AbortBufferIO arm; CI cluster job ...-0f92). Needs the identity bind
