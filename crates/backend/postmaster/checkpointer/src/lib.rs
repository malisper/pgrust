...
use pmsignal_seams as pmsignal;
...
fn pending_stats_inc(which: CheckpointerStatField) {
    activity_small::pgstat_checkpointer::with_pending_checkpointer_stats(|p| {
        match which {
...
fn pgstat_report_checkpointer() -> PgResult<()> {
    activity_small::pgstat_checkpointer::pgstat_report_checkpointer()
}
...
pub fn init_seams() {
    checkpointer_seams::checkpointer_main::set(checkpointer_main_entry);
    checkpointer_seams::absorb_sync_requests::set(absorb_sync_requests_seam);
    checkpointer_seams::forward_sync_request::set(forward_sync_request_seam);
    checkpointer_seams::request_checkpoint::set(request_checkpoint_seam);
    checkpointer_seams::checkpoint_stats_set::set(checkpoint_stats_set_seam);
    checkpointer_seams::checkpointer_shmem_size::set(checkpointer_shmem_size_seam);
    checkpointer_seams::checkpointer_shmem_init::set(checkpointer_shmem_init_seam);

    // --- bufmgr BufferSync write-rate throttle (bufmgr.c BufferSync →
    //     CheckpointWriteDelay, checkpointer.c) ---
    bufmgr_seams::checkpoint_write_delay::set(CheckpointWriteDelay);

    // --- ProcessUtility dispatch arm (utility.c CHECKPOINT → RequestCheckpoint) ---
    utility_out_seams::request_checkpoint::set(RequestCheckpoint);
    ...
    guc_tables::vars::CheckPointTimeout.install(
        guc_tables::GucVarAccessors {
            get: CheckPointTimeout,
            set: set_CheckPointTimeout,
        },
    );
    guc_tables::vars::CheckPointWarning.install(
        guc_tables::GucVarAccessors {
