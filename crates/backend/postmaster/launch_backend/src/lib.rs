...
static CHILD_PROCESS_KINDS: [ChildProcessKind; BACKEND_NUM_TYPES] = [
    ...
    ChildProcessKind {
        name: "backend",
        main_fn: Some(backend_startup_seams::backend_main::call),
        shmem_attach: true,
    },
    ...
    ChildProcessKind {
        ...
        main_fn: Some(autovacuum_seams::auto_vac_launcher_main::call),
        shmem_attach: true,
    },
    // [B_AUTOVAC_WORKER] = {"autovacuum worker", AutoVacWorkerMain, true},
    ChildProcessKind {
        name: "autovacuum worker",
        main_fn: Some(autovacuum_seams::auto_vac_worker_main::call),
        shmem_attach: true,
    },
    // [B_BG_WORKER] = {"bgworker", BackgroundWorkerMain, true},
    ChildProcessKind {
        name: "bgworker",
        main_fn: Some(bgworker_seams::background_worker_main::call),
        shmem_attach: true,
    },
    ...
    ChildProcessKind {
        ...
        main_fn: Some(slotsync_seams::repl_slot_sync_worker_main::call),
        shmem_attach: true,
    },
    ...
    ChildProcessKind {
        name: "archiver",
        main_fn: Some(pgarch_seams::pg_archiver_main::call),
        shmem_attach: true,
    },
    // [B_BG_WRITER] = {"bgwriter", BackgroundWriterMain, true},
    ChildProcessKind {
        name: "bgwriter",
        main_fn: Some(bgwriter_seams::background_writer_main::call),
        shmem_attach: true,
    },
    // [B_CHECKPOINTER] = {"checkpointer", CheckpointerMain, true},
    ChildProcessKind {
        name: "checkpointer",
        main_fn: Some(checkpointer_seams::checkpointer_main::call),
        shmem_attach: true,
    },
    // [B_IO_WORKER] = {"io_worker", IoWorkerMain, true},
    ChildProcessKind {
        name: "io_worker",
        main_fn: Some(methods_seams::io_worker_main::call),
        shmem_attach: true,
    },
    // [B_STARTUP] = {"startup", StartupProcessMain, true},
    ChildProcessKind {
        name: "startup",
        main_fn: Some(startup_seams::startup_process_main::call),
        shmem_attach: true,
    },
    // [B_WAL_RECEIVER] = {"wal_receiver", WalReceiverMain, true},
    ChildProcessKind {
        name: "wal_receiver",
        main_fn: Some(walreceiver_seams::wal_receiver_main::call),
        shmem_attach: true,
    },
    // [B_WAL_SUMMARIZER] = {"wal_summarizer", WalSummarizerMain, true},
    ChildProcessKind {
        name: "wal_summarizer",
        main_fn: Some(walsummarizer_seams::wal_summarizer_main::call),
        shmem_attach: true,
    },
    // [B_WAL_WRITER] = {"wal_writer", WalWriterMain, true},
    ChildProcessKind {
        name: "wal_writer",
        main_fn: Some(walwriter_seams::wal_writer_main::call),
        shmem_attach: true,
    },
    // [B_LOGGER] = {"syslogger", SysLoggerMain, false},
    ChildProcessKind {
        name: "syslogger",
        main_fn: Some(syslogger_seams::sys_logger_main::call),
        shmem_attach: false,
    },
];
...
pub fn postmaster_child_launch(
    ...
    let pid = fork_process_seams::fork_process::call();
    if pid == 0 {
        // child
        ...
        if is_external_connection_backend(child_type) {
            ...
            let fork_end = timestamp_seams::get_current_timestamp::call();
            backend_startup_seams::set_conn_timing_child::call(
                socket_created,
                fork_started,
                fork_end,
            );
        ...
        postmaster_seams::close_postmaster_ports::call(
        ...
        miscinit_seams::init_postmaster_child::call()
        ...
        if !CHILD_PROCESS_KINDS[child_type as usize].shmem_attach {
            dsm_core_seams::dsm_detach_all::call();
            sysv_shmem_seams::pg_shared_memory_detach::call();
        }
        ...
        // data.
        mcxt_seams::switch_to_top_memory_context::call();

        // MyPMChildSlot = child_slot;
        init_small_seams::set_my_pm_child_slot::call(child_slot);
        if let Some(client_sock) = client_sock {
            // MyClientSocket = palloc(sizeof(ClientSocket)); memcpy(...);
            init_small_seams::set_my_client_socket::call(*client_sock);
        }
...
pub fn init_seams() {
    launch_backend_seams::postmaster_child_launch::set(
        postmaster_child_launch_seam_adapter,
    );
    launch_backend_seams::postmaster_child_name::set(postmaster_child_name);
}
