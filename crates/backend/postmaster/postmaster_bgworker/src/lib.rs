...
pub fn BackgroundWorkerShmemInit() -> PgResult<()> {
    ...
    let (addr, _found) =
        ipc_shmem_seams::shmem_init_struct::call("Background Worker Data", size)?;
    ...
    while slotno < max_worker_processes {
        ...
        if let Some(rw_index) = rw_index {
            ...
            if slot_terminate && !rw_terminate {
                ...
                if rw_pid != 0 {
                    postmaster_seams::signal_child_sigterm::call(rw_pid);
                } else {
        ...
        if slot_terminate {
            ...
            if notify_pid != 0 {
                postmaster_seams::signal_child_sigusr1::call(notify_pid);
            }
            slotno += 1;
            continue;
        }
        ...
        if !postmaster_seams::postmaster_mark_pid_for_worker_notify::call(
...
pub fn bgworker_die(bgw_type: &[u8]) -> PgResult<()> {
    libpq_pqsignal_seams::block_signals::call();
    Err(PgError::new(
        FATAL,
        format!(
...
fn float_exception_handler_wrapper(signo: i32) {
    postgres::interrupt::FloatExceptionHandler(signo)
...
fn install_bgworker_signal_handlers(db_connection: bool) {
    ...
    if db_connection {
        ...
        pqsignal_be(
            libc::SIGINT,
            SigHandler::Handler(postgres::interrupt::StatementCancelHandler),
        );
        pqsignal_be(
            libc::SIGUSR1,
    ...
    misc_timeout::InitializeTimeouts();
...
pub fn BackgroundWorkerMain(startup_data: &StartupData) -> ! {
    ...
    ps_status_seams::init_ps_display::call(&worker.bgw_name);

    // Assert(GetProcessingMode() == InitProcessing);

    // Apply PostAuthDelay.
    let post_auth_delay = init_small_seams::post_auth_delay::call();
    ...
    match run_worker_body(worker) {
        Ok(()) => {
            ...
            dsm_core_seams::proc_exit::call(0, my_pid)
        }
        Err(_edata) => {
...
fn run_worker_body(worker: BackgroundWorker) -> PgResult<()> {
    ...
    // wrap it back into the bare-word `datum::Datum` the loader hands
    // straight to the C-ABI `bgworker_main_type` entry point.
    let main_arg = datum::Datum::from_usize(worker.bgw_main_arg);
    fmgr_seams::call_bgworker_entrypoint::call(worker, main_arg)?;
...
pub fn BackgroundWorkerInitializeConnection(
    ...
    postinit_seams::init_postgres_by_name::call(dbname, username, init_flags)?;
...
pub fn BackgroundWorkerInitializeConnectionByOid(
    ...
    postinit_seams::init_postgres_by_oid::call(dboid, useroid, init_flags)?;
...
pub fn RegisterBackgroundWorker(worker: &BackgroundWorker) -> PgResult<()> {
        ...
        || !init_small_seams::is_postmaster_environment::call()
    {
        ...
        if miscinit_seams::process_shared_preload_libraries_in_progress::call() {
            return Ok(());
        }
        ereport::call(
            PgError::new(
...
pub fn RegisterDynamicBackgroundWorker(
    ...
    let mut slotno = 0;
    {
        ...
        if parallel
            && d.parallel_register_count().wrapping_sub(d.parallel_terminate_count())
                >= init_small_seams::max_parallel_workers::call() as uint32
        {
            debug_assert!(
                d.parallel_register_count().wrapping_sub(d.parallel_terminate_count())
    ...
    if success {
        pmsignal_seams::send_postmaster_signal_bgworker_change::call();
    }
...
fn call_bgworker_entrypoint(
    ...
    match fname {
        ...
        // The bgworker loader carries the arg as the bare-word `datum::Datum`;
...
pub fn init_seams() {
    bgworker_seams::background_worker_main::set(background_worker_main_seam);
    bgworker_seams::get_background_worker_pid::set(get_background_worker_pid_seam);
    bgworker_seams::register_background_worker::set(register_background_worker_seam);
    bgworker_seams::register_dynamic_background_worker::set(
        register_dynamic_background_worker_seam,
    );
    bgworker_seams::background_worker_initialize_connection::set(
        background_worker_initialize_connection_seam,
    );
    bgworker_seams::background_worker_unblock_signals::set(
        background_worker_unblock_signals_seam,
    );
    // Pure-wiring install (assemble/seam-wiring-guard): owner body matches.
    bgworker_seams::background_worker_shmem_init::set(BackgroundWorkerShmemInit);
    ...
    bgworker_seams::background_worker_shmem_size::set(BackgroundWorkerShmemSize);
    ...
    parallel_rt_seams::background_worker_initialize_connection_by_oid::set(
    ...
    parallel_rt_seams::worker_number_from_bgw_extra::set(|| {
    ...
    parallel_rt_seams::register_dynamic_background_worker::set(
        |seg_handle, worker_index| register_dynamic_parallel_worker_seam(seg_handle, worker_index),
    );
    parallel_rt_seams::get_background_worker_pid::set(|handle| {
        Ok(GetBackgroundWorkerPid(&handle))
    });
    parallel_rt_seams::wait_for_background_worker_shutdown::set(|handle| {
        WaitForBackgroundWorkerShutdown(&handle)
    });
    parallel_rt_seams::terminate_background_worker::set(|handle| {
    ...
    parallel_rt_seams::terminate_background_worker_handle_free::set(
        |_handle| Ok(()),
    );
    ...
    postgres_seams::my_bgworker_type::set(|| {
        MY_BGWORKER_ENTRY
...
fn pm_registry_init_seams() {
    ...
    fmgr_seams::call_bgworker_entrypoint::set(call_bgworker_entrypoint);
...
fn register_dynamic_parallel_worker_seam(
    ...
    // `usize`).
    worker.bgw_main_arg = datum::Datum::from_u32(seg_handle).as_usize();
