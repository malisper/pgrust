...
fn pg_archiver_main_inner(startup_data: &types_startup::StartupData) -> PgResult<()> {
    ...
    init_small_seams::set_my_backend_type::call(BackendType::Archiver);
    ...
    dsm_core_seams::on_shmem_exit::call(pgarch_die, types_tuple::Datum::from_i32(0))?;
    ...
    let ctx = mcxt_seams::create_archiver_memcxt::call();
...
pub fn PgArchWakeup() {
    ...
    if arch_pgprocno != INVALID_PROC_NUMBER {
        lmgr_proc_seams::set_proc_latch::call(arch_pgprocno);
    }
}
    ...
    while let Some(xlog) = pgarch_readyXlog()? {
        ...
        loop {
            ...
            if !path_exists(&pathname)? {
                ...
                pgsleep_seams::pg_usleep::call(1_000_000);
                continue;
            }
            ...
            if pgarch_archiveXlog(&xlog)? {
                ...
                pgsleep_seams::pg_usleep::call(1_000_000); // wait a bit before retrying
            }
        }
    }
    Ok(())
...
fn report_archiver(xlog: &str, failed: bool) {
    ...
    activity_small::pgstat_archiver::pgstat_report_archiver(&buf, failed);
}
...
fn pgarch_archiveXlog(xlog: &str) -> PgResult<bool> {
    ...
    let ret = match result {
        Ok(r) => {
            ...
            mcxt_seams::MemoryContextReset::call(ctx);
            r
        }
        Err(_err) => {
...
fn archive_error_cleanup(
    ...
    aio_core_seams::pgaio_error_cleanup::call();
    all_seams::release_aux_process_resources::call(false);
    fd_seams::at_eoxact_files::call(false);
    dynahash_seams::at_eoxact_hash_tables::call(false);
...
fn pgarch_readyXlog() -> PgResult<Option<String>> {
    ...
    file_seams::with_allocated_dir::call(&xlog_archive_status_dir, &mut |d_name| {
...
fn ProcessPgArchInterrupts() -> PgResult<()> {
    ...
    if interrupt::ConfigReloadPending() {
        ...
        if !transam_xlog_seams::xlog_archive_library::call().is_empty()
            && !transam_xlog_seams::xlog_archive_command::call().is_empty()
        {
            return Err(ereport(ERROR)
        ...
        let archive_lib_changed =
            transam_xlog_seams::xlog_archive_library::call() != archive_lib;
...
pub fn init_seams() {
    pgarch_seams::pg_archiver_main::set(PgArchiverMain);
    ...
    pgarch_seams::pg_arch_shmem_size::set(PgArchShmemSize);
    pgarch_seams::pg_arch_shmem_init::set(PgArchShmemInit);
    pgarch_seams::pg_arch_wakeup::set(PgArchWakeup);
    pgarch_seams::pg_arch_force_dir_scan::set(PgArchForceDirScan);

    // `PgArchCanRestart()` (pgarch.c) — the postmaster's
    // LaunchMissingBackgroundProcesses restart-throttle check before relaunching
    // the archiver.
    postmaster_seams::pgarch_can_restart::set(PgArchCanRestart);
    ...
    guc_tables::vars::XLogArchiveLibrary.install(
    ...
    transam_xlog_seams::xlog_archive_library::set(xlog_archive_library_get);
}
