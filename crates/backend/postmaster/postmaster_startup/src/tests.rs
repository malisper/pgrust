...
struct MockState {
    ...
    standby_state: wal::HotStandbyState,
    now: i64,
}
...
fn install_mocks() -> MutexGuard<'static, ()> {
    ...
    *MOCK.lock().unwrap() = Some(MockState {
        ...
        standby_state: wal::STANDBY_DISABLED,
        now: 0,
    });

    static INSTALL: Once = Once::new();
    INSTALL.call_once(|| {
        xlogrecovery_seams::wakeup_recovery::set(|| {
            record(Call::WakeupRecovery)
        });
        xlogrecovery_seams::primary_conninfo::set(|mcx| {
            with_mock(|m| mcx::PgString::from_str_in(&m.conninfo, mcx))
        });
        xlogrecovery_seams::primary_slot_name::set(|mcx| {
            with_mock(|m| mcx::PgString::from_str_in(&m.slotname, mcx))
        });
        xlogrecovery_seams::wal_receiver_create_temp_slot::set(|| {
            with_mock(|m| m.temp_slot)
        });
        xlogrecovery_seams::startup_request_wal_receiver_restart::set(
            || record(Call::RequestWalRcvRestart),
        );
        guc_file_seams::process_config_file::set(|_context| {
            with_mock(|m| {
        ...
        dsm_core_seams::proc_exit::set(mock_proc_exit);
        init_small_seams::my_proc_pid::set(|| 12345);
        dsm_core_seams::on_shmem_exit::set(|_f, _arg| {
            record(Call::OnShmemExit);
            Ok(())
        });
        init_small_seams::is_under_postmaster::set(|| {
            with_mock(|m| m.is_under_postmaster)
        });
        init_small_seams::set_my_backend_type::set(|t| {
            record(Call::SetBackendType(t))
        });
        pmsignal_seams::postmaster_is_alive::set(|| {
        ...
        procsignal_seams::procsignal_sigusr1_handler::set(|_signo| {});
        ...
        xlogutils_seams::standby_state::set(|| {
        ...
        standby_seams::standby_dead_lock_handler::set(|| {});
        standby_seams::standby_timeout_handler::set(|| {});
        standby_seams::standby_lock_timeout_handler::set(|| {});
        timestamp_seams::get_current_timestamp::set(|| with_mock(|m| m.now));
        ...
        timeout_seams::register_timeout::set(|id, _handler| {
            record(Call::RegisterTimeout { id });
            id
        });
        timeout_seams::disable_timeout::set(|id, keep_indicator| {
            record(Call::DisableTimeout { id, keep_indicator })
        });
        timeout_seams::enable_timeout_every::set(|id, fin_time, delay_ms| {
        ...
        transam_xlog_seams::startup_xlog::set(|| {
...
fn startup_proc_exit_shuts_down_recovery_env_only_when_standby_active() {
    let _guard = install_mocks();
    StartupProcExit(0, types_tuple::heaptuple::Datum::null());
    assert_eq!(calls(), vec![]);

    with_mock(|m| m.standby_state = wal::STANDBY_SNAPSHOT_READY);
    StartupProcExit(0, types_tuple::heaptuple::Datum::null());
