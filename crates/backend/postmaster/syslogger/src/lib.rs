...
pub fn SysLoggerMain(
    ...
    init_small_seams::set_my_backend_type::call(BackendType::Logger);
    ...
    loop {
        ...
        latch_seams::reset_latch::call(my_latch);
        ...
        if ConfigReloadPending() {
            ...
            if config::log_directory() != current_log_dir {
                ...
                fd_seams::make_pg_directory::call(&current_log_dir);
            }
...
pub fn SysLogger_Start(child_slot: i32) -> PgResult<i32> {
    ...
    fd_seams::make_pg_directory::call(&config::log_directory());
    ...
    let syslogger_pid = launch_backend_seams::postmaster_child_launch::call(
        BackendType::Logger,
        child_slot,
        &[],
    );
...
fn logfile_getname(timestamp: pg_time_t, suffix: Option<&str>) -> String {
    ...
    let tm = pgtz_seams::pg_localtime::call(timestamp, &tz)
...
fn set_next_rotation_time() {
    ...
    let tz = config::log_timezone();
    let tm = pgtz_seams::pg_localtime::call(now, &tz)
        .expect("pg_localtime returned NULL for a valid wall-clock time");
    now += tm.tm_gmtoff;
    now -= now % rotinterval;
    now += rotinterval;
...
/// `init_small::globals`, and `MyLatch` from the latch unit's
...
fn sys_logger_main_entry(startup_data: &types_startup::StartupData) -> ! {
    ...
    let start_time = init_small::globals::MyStartTime();
    let mode_mask = init_small::globals::data_directory_mode() as u32;
    let my_latch = latch::my_latch()
...
pub fn init_seams() {
    ...
    // metafile.
    postmaster_seams::remove_logrotate_signal_files::set(RemoveLogrotateSignalFiles);
    postmaster_seams::remove_log_metainfo_datafile::set(remove_log_metainfo_datafile);
    postmaster_seams::check_logrotate_signal::set(CheckLogrotateSignal);
    ...
    syslogger_seams::logging_collector::set(config::logging_collector);
