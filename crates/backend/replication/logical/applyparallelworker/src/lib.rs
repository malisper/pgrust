...
fn pa_can_start() -> PgResult<bool> {
     ...
     */
    if !tablesync_seams::all_tablesyncs_ready::call()? {
        return Ok(false);
    }
...
fn pa_launch_parallel_worker() -> PgResult<Option<WorkerHandle>> {
    ...
    let launched = launcher::logicalrep_worker_launch::call(
        replication_launcher::LogicalRepWorkerType::ParallelApply,
...
fn pa_free_worker(winfo_index: WorkerHandle) -> PgResult<()> {
    ...
    if serialize_changes
        || pool_len
            > (guc_tables::vars::max_parallel_apply_workers_per_subscription
                .read()
                / 2)
    {
        /*
...
pub fn HandleParallelApplyMessageInterrupt() {
    init_small_seams::set_interrupt_pending::call(true); /* InterruptPending = true; */
    set_parallel_apply_message_pending(true);
    latch_seams::set_latch_my_latch::call(); /* SetLatch(MyLatch); */
}
...
fn ProcessParallelApplyMessage(msg: &[u8]) -> PgResult<()> {
    ...
    match msgtype {
        b'E' => {
             ...
             */
            let edata = pqmq_seams::pq_parse_errornotice::call(&msg[1..])?;
...
pub fn pa_send_data(winfo_index: WorkerHandle, data: &[u8]) -> PgResult<bool> {
    ...
    debug_assert!(!transam_xact_seams::is_transaction_state::call());
    ...
    loop {
        ...
        if start_time == 0 {
            start_time = timestamp_seams::get_current_timestamp::call();
        } else if timestamp_seams::timestamp_difference_exceeds::call(
            start_time,
...
pub fn pa_stream_abort(abort_data: &LogicalRepStreamAbortData) -> PgResult<()> {
     ...
     * case of crash.
     */
    origin_seams::set_replorigin_session_origin_lsn::call(
    ...
    if subxid == xid {
        ...
        if transam_xact_seams::is_transaction_block::call() {
            transam_xact_seams::end_transaction_block::call(false)?;
            transam_xact_seams::commit_transaction_command::call()?;
        }
        ...
        while i >= 0 {
            ...
            if xid_tmp == subxid {
                transam_xact_seams::rollback_to_savepoint::call(&spname)?;
                transam_xact_seams::commit_transaction_command::call()?;
...
pub fn pa_lock_stream(xid: TransactionId, lockmode: LOCKMODE) -> PgResult<()> {
    lmgr_seams::lock_apply_transaction_for_session::call(
...
pub fn pa_unlock_stream(xid: TransactionId, lockmode: LOCKMODE) -> PgResult<()> {
    lmgr_seams::unlock_apply_transaction_for_session::call(
        worker::my_worker_subid::call(),
        xid,
        PARALLEL_APPLY_LOCK_STREAM,
        lockmode,
...
fn pub_winfo_snapshot(
    winfo_index: WorkerHandle,
) -> PgResult<replication_applyparallel::ParallelApplyWorkerInfo> {
    ...
    Ok(replication_applyparallel::ParallelApplyWorkerInfo {
        has_error_mq_handle: w.error_mq_handle != 0,
        in_use: w.in_use,
        shared: replication_applyparallel::ParallelApplyWorkerShared {
            xid: s.xid,
            xact_state: match s.xact_state {
                ParallelTransState::PARALLEL_TRANS_UNKNOWN => {
                    replication_applyparallel::ParallelTransState::Unknown
                }
                ParallelTransState::PARALLEL_TRANS_STARTED => {
                    replication_applyparallel::ParallelTransState::Started
                }
                ParallelTransState::PARALLEL_TRANS_FINISHED => {
                    replication_applyparallel::ParallelTransState::Finished
                }
            },
...
pub fn init_seams() {
    use applyparallelworker_seams as s;
