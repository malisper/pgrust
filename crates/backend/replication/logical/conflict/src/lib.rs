...
use index_seams as catalog_index_seams;
...
fn build_tuple_value_details<'mcx>(
    ...
    if let Some(remoteslot) = remoteslot {
        ...
        let inserted_cols =
            execUtils::ExecGetInsertedCols(estate, relinfo, mcx)?;
        let updated_cols = execUtils::ExecGetUpdatedCols(estate, relinfo, mcx)?;
        let modified_cols =
...
fn build_index_value_desc<'b, 'mcx>(
    ...
    if estate.slot(slot).tts_ops == TupleSlotKind::Virtual {
        ...
        let new_slot = table_tableam::table_slot_create(query_cxt, localrel)?;
    ...
    let per_tuple_ecxt = execUtils::MakePerTupleExprContext(estate)?;
    ...
    let mut values_canon: Vec<types_tuple::heaptuple::Datum<'b>> =
