...
// transitional bare-word shim `datum::Datum` is kept ONLY at the one
...
// `datum::Datum` at that storage/ABI edge (the `types-bgworker` model
// owns the `bgw_main_arg` field and is not part of this migration), so the
// parameter stays `datum::Datum` until that owner migrates. The
...
use bgworker_seams as bgworker;
use applyparallelworker_seams as pa;
...
use guc_file_seams as guc_file;
use dshash_seams as dshash;
use dsa_seams as dsa;
use pg_subscription_seams as subscription;
...
/// launcher process.
pub fn ApplyLauncherMain(_main_arg: datum::Datum) -> PgResult<()> {
    ereport(DEBUG1)
    ...
    legacy_pqsignal::pqsignal(SIGHUP, SigHandler::Handler(config_reload_handler));
