...
//! reaches through [`origin_extern_seams`]. The
...
pub fn create_catalog_insert(roname: &str) -> PgResult<Option<Oid>> {
    ...
    let dirty = snapshot::SnapshotData::sentinel(snapshot::SnapshotType::SNAPSHOT_DIRTY);
    ...
    while roident < PG_UINT16_MAX as Oid {
        ...
        vacuum_seams::check_for_interrupts::call()?;
        ...
        if !collides {
            ...
            values[(ANUM_PG_REPLICATION_ORIGIN_RONAME - 1) as usize] =
                comment_seams::cstring_get_text_datum::call(mcx, roname)?;
    ...
    let result: Option<String> =
        match SearchSysCache1(mcx, REPLORIGIDENT, SysCacheKey::Value(scalar_oid(roident)))? {
            Some(tuple) => {
                ...
                let s = varlena_seams::text_to_cstring_v::call(mcx, &val)?;
...
/// `ObjectIdGetDatum(roident)` as the syscache key word (`datum::Datum`).
fn scalar_oid(roident: Oid) -> datum::Datum {
    datum::Datum::from_oid(roident)
}

/// `RecoveryInProgress()` (xlog.c) — delegate to the ported xlog owner.
pub fn recovery_in_progress() -> PgResult<bool> {
    Ok(transam_xlog_seams::recovery_in_progress::call())
}

/// `IsTransactionState()` (xact.c) — delegate to the ported xact owner.
pub fn is_transaction_state() -> PgResult<bool> {
    Ok(transam_xact_seams::is_transaction_state::call())
}
...
pub fn lock_relation_oid_origin() -> PgResult<()> {
    lmgr::LockRelationOid(REPLICATION_ORIGIN_RELATION_ID, RowExclusiveLock)
}

/// `UnlockRelationOid(ReplicationOriginRelationId, RowExclusiveLock)` (origin.c).
pub fn unlock_relation_oid_origin() -> PgResult<()> {
    lmgr::UnlockRelationOid(REPLICATION_ORIGIN_RELATION_ID, RowExclusiveLock)
}
...
pub fn lwlock_release_replication_origin_lock() -> PgResult<()> {
    lwlock_seams::lwlock_release_main::call(REPLICATION_ORIGIN_LOCK)
}
