...
pub(crate) struct IterTxnState {
    ...
    heap: binaryheap::BinaryHeap<IterNode, IterCompare>,
}
...
/// our [`binaryheap`] is a faithful port of the same API, so we
...
impl ReorderBuffer {
    ...
    fn iter_txn_init(&mut self, txn_xid: TransactionId) -> IterTxnState {
        ...
        let mut heap = binaryheap::BinaryHeap::allocate(nr_txns as i32, cmp)
    ...
    fn process_txn_body(
        ...
        if using_subtxn {
            transam_xact_seams::begin_internal_sub_transaction::call(Some(
        ...
        if transam_xact_seams::get_current_transaction_id_if_any::call()
        ...
        // cleanup.
        snapmgr_seams::teardown_historic_snapshot::call(false);
        ...
        if streaming || prepared {
            ...
            // Reset CheckXidAlive.
            transam_xact_seams::set_check_xid_alive::call(InvalidTransactionId);
        } else {
            ...
            *prev_lsn = change.lsn;
            ...
            if streaming || change_prepared {
                *curtxn = Some(change_txn);
                ...
                transam_xact_seams::set_check_xid_alive::call(cxid);
            }
    ...
    fn process_tp_change(
        ...
        let reloid = relfilenumbermap::RelidByRelfilenumber(
            rlocator.spcOid,
            rlocator.relNumber,
    ...
    fn tp_change_done(
        ...
        if let Some(reloid) = reloid {
            relcache_seams::relation_close::call(reloid)?;
        }
        Ok(())
    }
    ...
    /// [`ResolvedChange`](reorderbuffer_seams::ResolvedChange)
    ...
    fn process_txn_catch(
        ...
        snapmgr_seams::teardown_historic_snapshot::call(true);
    ...
    fn process_txn_apply_inval(&mut self, txn_xid: TransactionId) -> Result<(), types_error::PgError> {
        if self.with_txn_pub(txn_xid, |t| t.distr_inval_overflowed()) {
            ...
            inval_seams::invalidate_system_caches::call()?;
        } else {
