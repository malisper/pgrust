...
//! named through `replication` handles carried by value through the
...
/// `postgres_signal_arg`; the ported `interrupt`
...
fn setup_signal_handlers() -> PgResult<()> {
    ...
    pqsignal(
        libc::SIGUSR1,
        SigHandler::Handler(procsignal_seams::procsignal_sigusr1_handler::call),
    );
...
pub fn SlotSyncShmemInit() -> PgResult<()> {
    ...
    let (ptr, found) = ipc_shmem_seams::shmem_init_struct::call("Slot Sync Data", size)?;
...
pub fn init_seams() {
    ...
    postmaster_seams::validate_slot_sync_params::set(|elevel| {
        ValidateSlotSyncParams(elevel)
            .expect("ValidateSlotSyncParams errored at LOG elevel (postmaster seam)")
    });
    postmaster_seams::slot_sync_worker_can_restart::set(|| {
        SlotSyncWorkerCanRestart()
