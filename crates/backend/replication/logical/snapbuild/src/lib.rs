...
thread_local! {
    static SAVED_RESOURCE_OWNER_DURING_EXPORT: RefCell<Option<nodes::parsestmt::ResourceOwnerHandle>> =
...
pub fn snap_build_initial_snapshot(builder: &mut SnapBuild) -> PgResult<SnapshotData> {
    ...
    let max_xid_count = procarray_seams::get_max_snapshot_xid_count::call();
...
fn waitevent_start(event: u32) {
    waitevent_seams::pgstat_report_wait_start::call(event);
}
fn waitevent_end() {
...
fn recovery_in_progress() -> PgResult<bool> {
    origin_extern_seams::RecoveryInProgress::call()
}
