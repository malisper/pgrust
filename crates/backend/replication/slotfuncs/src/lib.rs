...
//!   ([`replication_slot`]): `MyReplicationSlot`, the slot array under
...
//!   ([`walsender::PhysicalWakeupLogicalWalSnd`]);
...
use init_small_seams as smallinit;
...
fn create_logical_replication_slot(
     ...
     */
    let mut ctx = logical_logical::CreateInitDecodingContext(
        Some(plugin),
     ...
     */
    if find_startpoint {
        logical_logical::DecodingContextFindStartpoint(&mut ctx)?;
    }

    /* don't need the decoding context anymore */
    logical_logical::FreeDecodingContext(&mut ctx)?;
...
pub fn pg_create_logical_replication_slot(
    ...
    logical_logical::CheckLogicalDecodingRequirements(
        wal_level_logical(),
        smallinit::my_database_id::call(),
    )?;
...
fn pg_logical_replication_slot_advance(moveto: XLogRecPtr) -> PgResult<XLogRecPtr> {
    logical_logical::LogicalSlotAdvanceAndCheckSnapState(
        moveto,
        None,
        xlog::wal_segment_size::call(),
        smallinit::my_database_id::call(),
