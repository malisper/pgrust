...
fn InsertRule<'mcx>(
    ...
    let evqual: String = match event_qual {
        Some(node) => outfuncs::nodeToString(mcx, node)?.as_str().to_string(),
        None => "<>".to_string(),
    };
    // char *actiontree = nodeToString((Node *) action);
    let action_list_node = action_as_list_node(mcx, action)?;
    let actiontree = outfuncs::nodeToString(mcx, &action_list_node)?;

    // pg_rewrite_desc = table_open(RewriteRelationId, RowExclusiveLock);
    let pg_rewrite_desc =
        table::table_open(mcx, RewriteRelationId, RowExclusiveLock)?;

    let rewriteObjectId: Oid;
    let is_update: bool;

    // oldtup = SearchSysCache2(RULERELNAME, eventrel_oid, rulname);
    let oldtup = syscache_seams::rule_tuple_by_relname::call(
        mcx,
        eventrel_oid,
        rulname,
    )?;
    ...
    if let Some((oldtuple, _oldform)) = oldtup {
        ...
        rewriteObjectId = indexing_seams::catalog_tuple_update_pg_rewrite::call(
            mcx,
            &pg_rewrite_desc,
            &oldtuple,
        ...
        rewriteObjectId = indexing_seams::catalog_tuple_insert_pg_rewrite::call(
            mcx,
            &pg_rewrite_desc,
            rulname,
            eventrel_oid,
    ...
    pg_depend::recordDependencyOn(mcx, &myself, &referenced, rel_behavior)?;

    /*
     * Also install dependencies on objects referenced in action and qual.
     */
    // recordDependencyOnExpr(&myself, (Node *) action, NIL, DEPENDENCY_NORMAL);
    dependency::recordDependencyOnExpr(
        &myself,
        &action_list_node,
        &[],
        DEPENDENCY_NORMAL,
...
fn get_rel_name_or_empty<'mcx>(mcx: Mcx<'mcx>, relid: Oid) -> PgResult<String> {
    match lsyscache_seams::get_rel_name::call(mcx, relid)? {
...
pub fn DefineRule<'mcx>(
    ...
    let (actions, where_clause) =
        parse_utilcmd_seams::transformRuleStmt::call(mcx, stmt, query_string)?;
    ...
    let rel_id = catalog_namespace::RangeVarGetRelid(
        mcx,
        &range_var,
        AccessExclusiveLock,
        false,
...
pub fn DefineQueryRewrite<'mcx>(
     ...
     */
    let event_relation =
        table::table_open(mcx, event_relid, AccessExclusiveLock)?;
    ...
    if !init_small::globals::allowSystemTableMods()
        && catalog_catalog::IsSystemRelation(&event_relation)
    {
        return Err(PgError::new(
            ERROR,
            format!(
    ...
    if !aclchk_seams::object_ownercheck::call(
        ...
        let objtype = objectaddress::resolve::get_relkind_objtype(relkind);
    ...
    if !action.is_empty() || is_instead {
         ...
         */
        rewritesupport_seams::SetRelationRuleStatus::call(event_relid, true)?;
    }
...
fn checkRuleResultList<'mcx>(
    ...
    for tle in target_list.iter() {
        ...
        /* Check type match. */
        let tletypid = nodes_core::nodefuncs::expr_type(tle.expr.as_deref())?;
        if attr.atttypid != tletypid {
            ...
            let tle_ty = adt_format_type::format_type_be_owned(tletypid)?;
            let col_ty = adt_format_type::format_type_be_owned(attr.atttypid)?;
         ...
         */
        let tletypmod = nodes_core::nodefuncs::expr_typmod(tle.expr.as_deref())?;
        if attr.atttypmod != tletypmod && attr.atttypmod != -1 && tletypmod != -1 {
            ...
            let tle_ty =
                adt_format_type::format_type_with_typemod(mcx, tletypid, tletypmod)?
                    .as_str()
                    .to_string();
            let col_ty = adt_format_type::format_type_with_typemod(
                mcx,
                attr.atttypid,
                attr.atttypmod,
            )?
...
fn setRuleCheckAsUser_Query<'mcx>(mcx: Mcx<'mcx>, qry: &mut Query<'mcx>, userid: Oid) {
    ...
    if qry.hasSubLinks {
        ...
        let flags = nodes_core::node_walker::QTW_IGNORE_RT_SUBQUERIES
...
pub fn EnableDisableRule<'mcx>(
    ...
    let ruletup =
        syscache_seams::rule_tuple_by_relname::call(mcx, owning_rel, rulename)?;
    ...
    if !aclchk_seams::object_ownercheck::call(
        ...
        let relkind = lsyscache_seams::get_rel_relkind::call(event_relation_oid)?;
        let objtype = objectaddress::resolve::get_relkind_objtype(relkind);
        let name = get_rel_name_or_empty(mcx, event_relation_oid)?;
        aclchk_seams::aclcheck_error::call(ACLCHECK_NOT_OWNER, objtype, Some(name))?;
    }

    /* Change ev_enabled if it is different from the desired new state. */
    let mut changed = false;
    if ruleform.ev_enabled != fires_when {
        indexing_seams::catalog_tuple_update_pg_rewrite_enabled::call(
            mcx,
            &pg_rewrite_desc,
            &ruletup,
            fires_when,
...
fn RangeVarCallbackForRenameRule(mcx: Mcx<'_>, rv: &types_tuple::access::RangeVar, relid: Oid) -> PgResult<()> {
    ...
    let user_id = miscinit::GetUserId();
    if !aclchk_seams::object_ownercheck::call(RelationRelationId, relid, user_id)? {
        let objtype = objectaddress::resolve::get_relkind_objtype(relkind);
        aclchk_seams::aclcheck_error::call(
            ACLCHECK_NOT_OWNER,
            objtype,
...
pub fn RenameRewriteRule<'mcx>(
    ...
    let targetrel = common_relation::relation_open(mcx, relid, NoLock)?;
    ...
    let ruletup =
        syscache_seams::rule_tuple_by_relname::call(mcx, relid, old_name)?;
    let Some((ruletup, ruleform)) = ruletup else {
        return Err(PgError::new(
            ERROR,
            format!(
    ...
    if rewritesupport_seams::IsDefinedRewriteRule::call(relid, new_name)? {
        return Err(PgError::new(
            ERROR,
            format!(
    ...
    indexing_seams::catalog_tuple_update_pg_rewrite_name::call(
        mcx,
        &pg_rewrite_desc,
        &ruletup,
        new_name,
    ...
    inval::cache_invalidate::CacheInvalidateRelcache(&targetrel)?;
...
pub fn init_seams() {
    utility_out_seams::define_rule::set(define_rule_seam);
    rewriteDefine_seams::set_rule_check_as_user::set(set_rule_check_as_user_seam);
    rewriteDefine_seams::set_rule_check_as_user_node::set(
