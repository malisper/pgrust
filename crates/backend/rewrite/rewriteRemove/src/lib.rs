...
pub fn RemoveRewriteRuleById<'mcx>(mcx: Mcx<'mcx>, ruleOid: Oid) -> PgResult<()> {
     ...
     */
    let name_evclass =
        syscache_seams::rewrite_name_evclass::call(mcx, ruleOid)?;
    ...
    let ruletup = syscache_seams::rule_tuple_by_relname::call(
        mcx,
        event_relation_oid,
        rulename.as_str(),
    )?;
     ...
     */
    let event_relation =
        table::table_open(mcx, event_relation_oid, AccessExclusiveLock)?;
     ...
     */
    inval::cache_invalidate::CacheInvalidateRelcache(&event_relation)?;
...
pub fn init_seams() {
    ...
    rewriteRemove_seams::RemoveRewriteRuleById::set(|rule_oid| {
