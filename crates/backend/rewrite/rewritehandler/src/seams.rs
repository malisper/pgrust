...
pub fn init_seams() {
    rewritehandler_seams::build_column_default::set(seam_build_column_default);
    rewritehandler_seams::build_generation_expression::set(
        seam_build_generation_expression,
    );
    rewritehandler_seams::expand_generated_columns_in_expr::set(
    ...
    plancat_ext_seams::expand_generated_columns_in_expr::set(
        seam_expand_generated_columns_in_expr_arena,
    );
    view_seams::view_query_is_auto_updatable::set(
        seam_view_query_is_auto_updatable,
    );
    rewritehandler_seams::get_view_query::set(seam_get_view_query);
    rewritehandler_seams::relation_is_updatable::set(relation_is_updatable);
    rewritehandler_seams::query_rewrite::set(seam_query_rewrite_legacy);
    rewritehandler_seams::query_rewrite_canonical::set(
        seam_query_rewrite_canonical,
    );
    rewritehandler_seams::acquire_rewrite_locks::set(seam_acquire_rewrite_locks);
    ...
    ruleutils_seams::acquire_rewrite_locks::set(
        seam_acquire_rewrite_locks_inplace,
    );
    rewritehandler_seams::relation_has_security_invoker::set(
        seam_relation_has_security_invoker,
    );
    rewritehandler_seams::relation_is_security_view::set(
        seam_relation_is_security_view,
    );
    rewritehandler_seams::relation_has_check_option::set(
        seam_relation_has_check_option,
    );
    rewritehandler_seams::relation_has_cascaded_check_option::set(
...
fn view_options_of(view: &rel::Relation<'_>) -> types_reloptions::relopts::ViewOptions {
    ...
    match common_reloptions::view_reloptions(smcx, Some(&token.bytes), false) {
        Ok(Some(common_reloptions::RelOptStruct::View(v))) => v,
        _ => types_reloptions::relopts::ViewOptions::default(),
    }
}

fn seam_relation_has_security_invoker(view: &rel::Relation<'_>) -> bool {
    view_options_of(view).security_invoker
}
fn seam_relation_is_security_view(view: &rel::Relation<'_>) -> bool {
    view_options_of(view).security_barrier
}
fn seam_relation_has_check_option(view: &rel::Relation<'_>) -> bool {
    view_options_of(view).check_option
        != types_reloptions::relopts::VIEW_OPTION_CHECK_OPTION_NOT_SET
}
fn seam_relation_has_cascaded_check_option(view: &rel::Relation<'_>) -> bool {
    view_options_of(view).check_option
...
fn seam_expand_generated_columns_in_expr<'mcx>(
    ...
    let rel = table::table_open(mcx, rel_oid, NoLock)?;
