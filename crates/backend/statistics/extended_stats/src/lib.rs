...
fn fetch_statentries_for_relation<'mcx>(
    ...
    while let Some(htup) = genam::systable_getnext::call(smcx, scan.desc_mut())? {
        ...
        let schema = lsyscache_seams::get_namespace_name::call(smcx, stxnamespace)?
    ...
    for stat in &statslist {
        ...
        let stats = match lookup_var_attr_stats(mcx, onerel, stat, natts, vacattrstats)? {
            ...
            None => {
                ...
                let relschema =
                    lsyscache_seams::get_namespace_name::call(
                        nmcx,
        ...
        for &t in &stat.types {
            if t == STATS_EXT_NDISTINCT {
                let n = mvdistinct::statext_ndistinct_build(
                    mcx,
                    totalrows,
                    &data,
                    data.nattnums,
                ...
                ndistinct_bytes =
                    Some(mvdistinct::statext_ndistinct_serialize(mcx, &n)?);
            } else if t == STATS_EXT_DEPENDENCIES {
                let deps = dependencies::statext_dependencies_build(
                    mcx,
                    &data,
                    data.nattnums,
                    &data.attnums,
                ...
                if let Some(deps) = deps {
                    dependencies_bytes = Some(
                        dependencies::statext_dependencies_serialize(mcx, &deps)?,
                    );
                }
            } else if t == STATS_EXT_MCV {
                // mcvlist = statext_mcv_build(data, totalrows, stattarget);
                // statext_store(... mcvlist ...)
                let mcvlist = mcv::statext_mcv_build(
                    &data,
                    totalrows,
                    stattarget,
                )?;
                ...
                if let Some(mcvlist) = mcvlist {
                    ...
                    let mut dimstats: Vec<mcv::McvDimStats> =
                        Vec::with_capacity(stats.len());
                    for s in &stats {
                        let (typlen, typbyval) =
                            lsyscache_seams::get_typlenbyval::call(
                                s.attrtypid,
                            )?;
                        dimstats.push(mcv::McvDimStats {
                    ...
                    mcv_bytes = Some(mcv::statext_mcv_serialize(
...
fn statext_store<'mcx>(
    ...
    statscmds::RemoveStatisticsDataById(mcx, stat_oid, inh)?;

    // form and insert a new tuple
    let mut stup = heaptuple::heap_form_tuple(
        mcx,
        &pg_stextdata.rd_att,
        &values,
        &nulls,
    ...
    indexing::keystone::CatalogTupleInsert(mcx, &pg_stextdata, &mut stup)?;
...
pub fn init_seams() {
    ...
    path_small_seams::statext_clauselist_selectivity::set(
