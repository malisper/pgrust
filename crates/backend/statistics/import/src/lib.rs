...
fn extract_variadic_args<'mcx>(
    ...
    if variadic {
        ...
        for i in 0..nargs {
            ...
            let mut typ = fmgr_core::get_fn_expr_argtype(fcinfo.flinfo.as_deref(), i as i32);
...
fn range_var_callback_for_stats(
    ...
    if rel_id != old_rel_id && table_oid != rel_id {
        ...
        let guard = lmgr_seams::lock_relation_oid::call(
            table_oid,
            ShareUpdateExclusiveLock,
        )?;
...
fn rangevar_relname_from_rel(rel: &rel::Relation<'_>) -> String {
...
fn get_attr_stat_type(mcx: Mcx<'_>, reloid: Oid, attnum: AttrNumber) -> PgResult<StatType> {
    ...
    let tce = typcache_seams::lookup_type_cache::call(atttypid, 0)?;
    let atttyptype = tce.typtype;
    let eq_opr = typcache_seams::lookup_type_cache_eq_opr::call(atttypid)?;
    let lt_opr = typcache_seams::lookup_type_cache_lt_opr::call(atttypid)?;
...
fn get_elem_stat_type(atttypid: Oid, _atttyptype: i8) -> PgResult<Option<(Oid, Oid)>> {
    ...
    let elem_eq_opr =
        typcache_seams::lookup_type_cache_eq_opr::call(elemtypid)?;
...
fn text_datum_to_string(d: &Datum) -> PgResult<String> {
    match d {
        ...
        _ => {
            let payload = varlena::vardata_any_slice(d.as_ref_bytes());
...
fn direct_text_datum_to_string(d: &Datum) -> PgResult<String> {
    match d {
        ...
        _ => {
            ...
            let payload = varlena::vardata_any_slice(image);
            Ok(String::from_utf8_lossy(payload).into_owned())
        }
    }
}
