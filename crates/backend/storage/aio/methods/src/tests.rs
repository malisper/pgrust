...
fn setup(max_backends: i32, nbuffers: i32, my_proc_number: i32) -> std::sync::MutexGuard<'static, ()> {
    ...
    if !init_small_seams::max_backends::is_installed() {
        init_small_seams::max_backends::set(|| T_MAX_BACKENDS.load(Ordering::SeqCst));
    }
    if !init_small_seams::nbuffers::is_installed() {
        init_small_seams::nbuffers::set(|| T_NBUFFERS.load(Ordering::SeqCst));
    }
    if !init_small_seams::my_proc_number::is_installed() {
        init_small_seams::my_proc_number::set(|| T_MY_PROC_NUMBER.load(Ordering::SeqCst));
    }
    if !init_small_seams::my_backend_type::is_installed() {
        init_small_seams::my_backend_type::set(|| types_core::init::BackendType::Backend);
    }
    if !ipc_shmem_seams::add_size::is_installed() {
        ipc_shmem_seams::add_size::set(|a, b| {
            a.checked_add(b).ok_or_else(|| oom_error("add_size"))
        });
    }
    if !ipc_shmem_seams::mul_size::is_installed() {
