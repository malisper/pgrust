...
//! advice.
//!
//! This crate sits directly above [`bufmgr`]: it calls
...
fn read_stream_begin_impl<'mcx>(
    ...
    let mut max_ios: i32 = if !oid_is_valid
        || is_catalog_rel
        || catalog_catalog::IsCatalogRelationOid(rel_number)
    {
        ...
        spccache::get_tablespace_io_concurrency(
            tablespace_id,
            my_database_tablespace,
            bufmgr_seams::effective_io_concurrency::call(),
        )?
...
fn bm_get_access_strategy_pin_limit(strategy: &BufferAccessStrategy) -> i32 {
    match strategy {
        None => init_small_seams::nbuffers::call(),
