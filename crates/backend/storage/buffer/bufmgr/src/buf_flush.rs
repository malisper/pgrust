...
//! write rides the landed [`smgr::smgrwrite`] /
...
impl BufferManager {
    ...
    pub(crate) fn flush_buffer(
        ...
        let recptr = self.with_block(buf_id, |block| {
            let page = page::PageRef::new(block).expect("buffer block is BLCKSZ");
        ...
        {
            let mut p = page::PageMut::new(&mut page).expect("page is BLCKSZ");
            page::PageSetChecksumInplace(&mut p, tag.blockNum);
        }
