...
/// Crash-cycle reset. Must not take dynahash freelist spinlocks — a crashed
/// backend may have died holding one.
pub(crate) fn BufTableResetAfterCrash() {
    // SAFETY: crash choreography — children dead, postmaster thread only;
    // shared fixed-size table (InitBufTable).
    unsafe { dynahash::hash_reset_after_crash(htab()) }
}
