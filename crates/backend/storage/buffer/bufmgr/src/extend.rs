...
//!     DIRECT calls into [`smgr`] (no seam; the smgr crate
//!     is a direct dependency).
//!   * the relation-extension lock — the lmgr
//!     [`lmgr_seams::lock_relation_for_extension`] RAII
...
//!     [`init_small_seams::max_backends`] + the
...
impl BufferManager {
    ...
    fn max_proportional_pins(&self) -> u32 {
        let divisor = init_small_seams::max_backends::call() + NUM_AUXILIARY_PROCS;
        if divisor <= 0 {
            return 0;
        }
...
mod tests {
    ...
    fn with_divisor(divisor: i32) -> std::sync::MutexGuard<'static, ()> {
        ...
        INSTALL.call_once(|| {
            init_small_seams::max_backends::set(read_max_backends);
        });
        guard
    }
