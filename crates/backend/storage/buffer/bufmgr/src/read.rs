...
impl BufferManager {
    ...
    pub fn ReadBufferExtended(
        ...
        if bmr.relpersistence == RELPERSISTENCE_TEMP
            && bmr.rlocator.backend != lmgr_proc_seams::my_proc_number::call()
        {
    ...
    pub fn PrefetchBuffer(
        ...
        if bmr.relpersistence == RELPERSISTENCE_TEMP {
            ...
            if bmr.rlocator.backend != lmgr_proc_seams::my_proc_number::call() {
                return Err(PgError::error(
                    "cannot access temporary tables of other sessions",
                ));
            }
    ...
    fn read_local_buffer_miss(
        ...
        use buffer_support_seams as lb;
    ...
    fn read_buffer_state_any(&self, buffer: Buffer) -> u32 {
        if buffer_is_local(buffer) {
            buffer_support_seams::local_buffer_state::call(buffer)
        } else {
    ...
    pub fn RelationCopyStorageUsingBuffer(
        ...
        // than the init fork.
        let use_wal = transam_xlog_seams::wal_level::call()
            >= wal::xlog_consts::WAL_LEVEL_REPLICA
        ...
        for blkno in 0..nblocks {
            ...
            if use_wal {
                xloginsert_seams::log_newpage_buffer::call(
                    dst_buf, true,
                )?;
            }
            // END_CRIT_SECTION().
