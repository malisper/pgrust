...
//! [`s_lock::Spinlock`] (a direct dep). The in-crate
...
pub fn get_access_strategy(btype: BufferAccessStrategyType) -> PgResult<BufferAccessStrategy> {
    let nbuffers_total = init_small_seams::nbuffers::call();
...
fn local_mgr_get_or_create() -> &'static localbuf::LocalBufferManager {
    ...
    let num_temp_buffers = guc_tables::vars::num_temp_buffers.read();
...
fn local_buffer_get_lsn(
    ...
    local_mgr_get_or_create().with_block(buffer, |bytes| {
        let page = page::PageRef::new(bytes)?;
        Ok(page::PageGetLSN(&page))
    })
}
...
pub fn init_seams() {
    bufmgr_seams::get_access_strategy::set(get_access_strategy);
    bufmgr_seams::free_access_strategy::set(free_access_strategy);
    ...
    // cleanup legs).
    bufmgr_seams::local_ref_count::set(local_ref_count);
    bufmgr_seams::mark_local_buffer_dirty::set(mark_local_buffer_dirty);
    bufmgr_seams::local_buffer_alloc::set(local_buffer_alloc);
    bufmgr_seams::prefetch_local_buffer::set(prefetch_local_buffer);
    bufmgr_seams::at_eoxact_local_buffers::set(at_eoxact_local_buffers);
    bufmgr_seams::at_proc_exit_local_buffers::set(at_proc_exit_local_buffers);
    bufmgr_seams::extend_buffered_rel_local::set(extend_buffered_rel_local);
    bufmgr_seams::drop_relation_local_buffers::set(
        drop_relation_local_buffers,
    );
    bufmgr_seams::drop_relation_all_local_buffers::set(
        drop_relation_all_local_buffers,
    );
    bufmgr_seams::flush_relation_local_buffers::set(
    ...
    buffer_support_seams::mark_local_buffer_dirty::set(mark_local_buffer_dirty);
    buffer_support_seams::unpin_local_buffer::set(unpin_local_buffer);
    buffer_support_seams::unpin_local_buffer_no_owner::set(
        unpin_local_buffer_no_owner,
    );
    buffer_support_seams::incr_local_buffer_ref_count::set(
        incr_local_buffer_ref_count,
    );
    buffer_support_seams::local_buffer_block_number::set(local_buffer_block_number);
    buffer_support_seams::local_buffer_state::set(local_buffer_state);
    buffer_support_seams::local_buffer_get_tag::set(local_buffer_get_tag);
    buffer_support_seams::local_buffer_get_lsn::set(local_buffer_get_lsn);
    buffer_support_seams::local_buffer_with_page::set(local_buffer_with_page);
    buffer_support_seams::local_buffer_page_owned::set(local_buffer_page_owned);
    buffer_support_seams::start_local_buffer_io::set(start_local_buffer_io);
    buffer_support_seams::terminate_local_buffer_io::set(terminate_local_buffer_io);
}
