...
fn install_seams_once() {
    ...
    bufmgr_seams::lock_buf_hdr::set(|buf_id| {
        let mut s = header_state().lock().unwrap();
        s.locked += 1;
    ...
    bufmgr_seams::unlock_buf_hdr::set(|buf_id, buf_state| {
        let mut s = header_state().lock().unwrap();
        s.locked -= 1;
        s.states[buf_id as usize] = buf_state & !BM_LOCKED;
    });
    bufmgr_seams::buf_free_next::set(|buf_id| {
        header_state().lock().unwrap().free_next[buf_id as usize]
    });
    bufmgr_seams::set_buf_free_next::set(|buf_id, value| {
    ...
    bufmgr_seams::io_combine_limit::set(|| {
    ...
    bufmgr_seams::io_direct_data::set(|| false);
    ...
    smgr_seams::smgr_prefetch::set(
        |_r: RelFileLocator, _bk: ProcNumber, _f: ForkNumber, _b: BlockNumber| Ok(false),
    );

    // --- localbuf diagnostics. ---
    init_small_seams::my_proc_number::set(|| 3 as ProcNumber);
    init_small_seams::nbuffers::set(|| 1024);
