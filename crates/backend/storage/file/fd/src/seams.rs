...
fn file_access_error(elevel: ErrorLevel, errno: i32, message: String) -> PgError {
    let sqlstate = error_seams::sqlstate_for_file_access::call(errno);
...
pub fn tablespace_location<'mcx>(
    ...
    if tablespace_oid == types_core::primitive::InvalidOid {
        tablespace_oid = init_small_seams::my_database_table_space::call();
    }
...
pub fn tablespace_databases(tablespace_oid: types_core::Oid) -> PgResult<Option<Vec<types_core::Oid>>> {
    ...
    if tablespace_oid == GLOBALTABLESPACE_OID {
        let _ = error_seams::ereport::call(PgError::warning(
    ...
    if let Ok(cpath) = std::ffi::CString::new(location.as_str()) {
        ...
        if sret < 0 {
            ...
            let _ = error_seams::ereport::call(PgError::warning(format!(
