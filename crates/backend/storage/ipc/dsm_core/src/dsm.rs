...
fn make_main_region_dsm_handle(slot: i32) -> dsm_handle {
    ...
    handle |= prng::global_prng(|prng| prng.next_u32())
...
pub fn dsm_postmaster_startup(shim: *mut PGShmemHeader, max_backends: i32) -> PgResult<()> {
    ...
    loop {
        ...
        control_handle = prng::global_prng(|prng| prng.next_u32()) << 1;
...
fn dsm_cleanup_for_mmap() -> PgResult<()> {
    ...
    file_seams::with_allocated_dir::call(PG_DYNSHMEM_DIR, &mut |d_name| {
...
pub fn dsm_shmem_init() -> PgResult<()> {
    ...
    let (begin, found) =
        ipc_shmem_seams::shmem_init_struct::call("Preallocated DSM", size)?;
...
pub fn dsm_create(size: Size, flags: i32, mcx: Mcx<'static>) -> PgResult<Option<DsmSegment>> {
    ...
    if !using_main_dsm_region {
        ...
        loop {
            ...
            let handle: dsm_handle = prng::global_prng(|prng| prng.next_u32()) << 1;
