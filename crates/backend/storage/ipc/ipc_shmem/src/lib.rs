...
pub fn pg_get_shmem_allocations<'mcx>(
    ...
    loop {
        ...
        let ent = unsafe { &*ent };
        values[0] =
            varlena_seams::cstring_to_text_v::call(mcx, &key_str(&ent.key))?;
    ...
    values[0] = varlena_seams::cstring_to_text_v::call(mcx, "<anonymous>")?;
    nulls[1] = true;
...
pub fn pg_get_shmem_allocations_numa<'mcx>(
    ...
    if pg_numa_seams::pg_numa_init::call() == -1 {
        elog(
            ERROR,
    ...
    loop {
        ...
        if pg_numa_seams::pg_numa_query_pages::call(
            0,
...
pub fn pg_get_shmem_pagesize(huge_pages_status: HugePagesStatus) -> Size {
    ...
    if huge_pages_status == HugePagesStatus::HUGE_PAGES_ON {
        let (hugepagesize, _mmap_flags) =
            sysv_shmem_seams::get_huge_page_size::call();
...
pub fn pg_numa_available() -> types_tuple::Datum<'static> {
    types_tuple::Datum::from_bool(pg_numa_seams::pg_numa_init::call() != -1)
}
...
pub fn init_seams() {
    ipc_shmem_seams::shmem_init_struct::set(|name, size| {
        ShmemInitStruct(name, size).map(|(p, found)| (p.as_ptr(), found))
    });
    ipc_shmem_seams::add_size::set(add_size);
    ipc_shmem_seams::mul_size::set(mul_size);
    ipc_shmem_seams::shmem_lock_acquire::set(shmem_lock_acquire_impl);
    ipc_shmem_seams::shmem_lock_release::set(shmem_lock_release_impl);
    ipc_shmem_seams::init_shmem_access::set(|seghdr| unsafe {
        InitShmemAccess(seghdr)
    });
    ipc_shmem_seams::init_shmem_allocation::set(|| {
        InitShmemAllocation().expect("InitShmemAllocation failed")
    });
    ipc_shmem_seams::init_shmem_index::set(InitShmemIndex);
    ipc_shmem_seams::shmem_alloc_unlocked::set(|size| {
