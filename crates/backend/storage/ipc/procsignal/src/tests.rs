...
pub(crate) struct Env {
    ...
    shmem_exit_callback: RefCell<
        Option<(
            fn(i32, types_tuple::heaptuple::Datum<'static>) -> types_error::PgResult<()>,
            types_tuple::heaptuple::Datum<'static>,
        )>,
    >,
}
...
fn install_seams_once() {
    ...
    ONCE.call_once(|| {
        ...
        init_small_seams::max_backends::set(|| TEST_MAX_BACKENDS);
        init_small_seams::set_interrupt_pending::set(|v| {
            ENV.with(|e| e.interrupt_pending.set(v))
        });

        ipc_shmem_seams::mul_size::set(|a, b| Ok(a.checked_mul(b).unwrap()));
        ipc_shmem_seams::add_size::set(|a, b| Ok(a.checked_add(b).unwrap()));
        ...
        ipc_shmem_seams::shmem_init_struct::set(|_name, size| {
        ...
        dsm_core_seams::on_shmem_exit::set(|f, arg| {
        ...
        condition_variable_seams::condition_variable_timed_sleep::set(
        ...
        condition_variable_seams::condition_variable_cancel_sleep::set(
            || false,
        );
        condition_variable_seams::condition_variable_broadcast::set(|_cv| {
        ...
        sinval_seams::handle_catchup_interrupt::set(|| {
            ENV.with(|e| e.catchup_calls.set(e.catchup_calls.get() + 1))
        });
        async_seams::handle_notify_interrupt::set(|| {
            ENV.with(|e| e.notify_calls.set(e.notify_calls.get() + 1))
        });
        walsender_seams::handle_wal_snd_init_stopping::set(|| {});
        mcxt_seams::handle_log_memory_context_interrupt::set(|| {});
        applyparallelworker_seams::handle_parallel_apply_message_interrupt::set(|| {});
        postgres_seams::handle_recovery_conflict_interrupt::set(|reason| {
