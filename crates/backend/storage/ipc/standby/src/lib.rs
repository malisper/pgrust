...
fn WaitExceedsMaxStandbyDelay(wait_event_info: u32) -> PgResult<bool> {
    ...
    if ltime != 0 && timestamp_seams::get_current_timestamp::call() >= ltime {
        return Ok(true);
    }

    // Sleep a bit (this is essential to avoid busy-waiting).
    waitevent_seams::pgstat_report_wait_start::call(wait_event_info);
    pgsleep_seams::pg_usleep::call(STANDBY_WAIT_US.get() as i64);
...
pub fn LogRecoveryConflict(
    ...
    let (secs, mut usecs) =
        timestamp_seams::timestamp_difference::call(wait_start, now);
    ...
    while idx < waitlist.len() && waitlist[idx].is_valid() {
        ...
        while !lock::virtual_xact_lock::call(vxid, false)? {
            ...
            if WaitExceedsMaxStandbyDelay(wait_event_info)? {
                ...
                if pid != 0 {
                    pgsleep_seams::pg_usleep::call(5000);
                }
            }
            ...
            if wait_start != 0 && (!logged_recovery_conflict || !waiting) {
                ...
                if maybe_update_title
                    && timestamp_seams::timestamp_difference_exceeds::call(
                        wait_start, now, 500,
                    )
                {
...
pub fn ResolveRecoveryConflictWithSnapshotFullXid(
    ...
    let next_xid = varsup_seams::read_next_full_transaction_id::call();
...
pub fn ResolveRecoveryConflictWithDatabase(dbid: Oid) -> PgResult<()> {
    while procarray::count_db_backends::call(dbid)? > 0 {
        ...
        pgsleep_seams::pg_usleep::call(10000);
    }
    Ok(())
}
...
pub fn ResolveRecoveryConflictWithLock(
    ...
    if now >= ltime && ltime != 0 {
        ...
        timeout_seams::enable_timeouts::call(enabled)?;
    }
    ...
    timeout_seams::disable_all_timeouts::call(false)?;
    GOT_STANDBY_LOCK_TIMEOUT.set(false);
    GOT_STANDBY_DEADLOCK_TIMEOUT.set(false);
    Ok(())
}
...
pub fn ResolveRecoveryConflictWithBufferPin() -> PgResult<()> {
    ...
    if timestamp_seams::get_current_timestamp::call() >= ltime && ltime != 0 {
        // We're already behind, so clear a path as quickly as possible.
...
pub fn CheckRecoveryConflictDeadlock() -> PgResult<()> {
    ...
    if !bufmgr_seams::holding_buffer_pin_that_delays_recovery::call() {
        return Ok(());
    }
...
fn standby_redo_seam(state: &mut ::wal::rmgr::XLogReaderState<'_>) -> PgResult<()> {
    ...
    let rec = state
        .record
        .as_ref()
        .expect("standby_redo called without a decoded record");
    let redo = RedoRecord {
        info: rec.info(),
        data: rec.data(),
        has_any_block_refs: !rec.blocks().is_empty(),
    };
    standby_redo(ctx.mcx(), redo)
}
...
/// `transam_xlog_seams::log_standby_snapshot` seam (consumed by
...
pub fn init_seams() {
    ...
    bufmgr_seams::in_hot_standby::set(in_hot_standby);
    bufmgr_seams::lock_buffer_for_cleanup_recovery_wait_park::set(
        lock_buffer_for_cleanup_recovery_wait_park,
    );
    bufmgr_seams::lock_buffer_for_cleanup_recovery_wait::set(
