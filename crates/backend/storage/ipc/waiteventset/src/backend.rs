...
mod imp {
    ...
    pub fn initialize_wait_event_support() -> PgResult<()> {
        port_pqsignal_seams::pqsignal::call(libc::SIGURG, signal::SigHandler::Ignore);
        Ok(())
    }

    /// `CreateWaitEventSet` kqueue setup.
    pub fn create(_nevents: i32) -> PgResult<Backend> {
        if !file_seams::acquire_external_fd::call() {
    ...
    fn wait_event_adjust_kqueue(
        ...
        if event.events == WL_POSTMASTER_DEATH {
            ...
            let pm_pid = init_small_seams::postmaster_pid::call();
            kq_add(
                &mut k_ev[count],
                pm_pid as usize,
                libc::EVFILT_PROC,
        ...
        if rc < 0 {
            ...
            if pm_pid != ppid && !pmsignal_seams::postmaster_is_alive::call() {
                set.backend.report_postmaster_not_running = true;
            }
        }
...
mod imp {
    ...
    pub fn initialize_wait_event_support() -> PgResult<()> {
        ...
        libpq_pqsignal_seams::add_unblock_sig::call(libc::SIGURG);
    ...
    pub fn postmaster_death_fd() -> i32 {
        postmaster_seams::postmaster_death_watch_fd::call()
    }
    ...
    pub fn wait_block(
        ...
        for cur_epoll_event in ret_events.iter().take(rc as usize) {
                ...
                && ev_bits & (libc::EPOLLIN | libc::EPOLLERR | libc::EPOLLHUP) as u32 != 0
            {
                ...
                && ev_bits & (libc::EPOLLIN | libc::EPOLLERR | libc::EPOLLHUP) as u32 != 0
            {
                if !pmsignal_seams::postmaster_is_alive::call() {
                    if exit_on_pm_death {
                        ipc_seams::proc_exit::call(1);
                    }
                    out.fd = PGINVALID_SOCKET;
