...
//! performed inside the [`indexing_seams::deform_lo_page`] seam
...
pub(crate) fn install_lo_compat_privileges_guc() {
    guc_tables::vars::lo_compat_privileges.install(
...
pub fn inv_create(lobjId: Oid) -> PgResult<Oid> {
     ...
     */
    lobjId_new = pg_largeobject::LargeObjectCreate(lobjId)?;
...
fn InvokeObjectPostCreateHook(classId: Oid, objectId: Oid, subId: i32) -> PgResult<()> {
    if objectaccess::object_access_hook_present() {
        objectaccess::run_object_post_create_hook(classId, objectId, subId, false)?;
    }
    Ok(())
}
...
pub fn inv_open(lobjId: Oid, flags: i32) -> PgResult<Box<LargeObjectDesc>> {
    let snapshot: Option<Rc<snapshot::SnapshotData>>;
    ...
    if descflags & IFS_WRLOCK != 0 {
        ...
        snapshot = snapmgr_seams::get_active_snapshot::call()?;
    }

    /* Can't use LargeObjectExists here because we need to specify snapshot */
    if !pg_largeobject::LargeObjectExistsWithSnapshot(lobjId, snapshot.clone())? {
        return Err(ereport(ERROR)
    ...
    if (descflags & IFS_RDLOCK) != 0
        ...
        && aclchk_seams::pg_largeobject_aclcheck_snapshot::call(
            lobjId,
            miscinit_seams::get_user_id::call(),
            ACL_SELECT,
            snapshot.clone(),
