...
use lmgr_seams as inward;
use lmgr_pc_seams as inward_pc;
...
pub fn WaitForLockersMultiple(
    ...
    if progress {
        ...
        let values = [0i64, 0, 0];
        activity_small::backend_progress::pgstat_progress_update_multi_param(
            &index, &values,
        );
    }
...
pub fn init_seams() {
    ...
    vx::unlock_relation::set(|rel, lockmode| {
        let lri = relcache_seams::rel_lock_relid::call(rel.rd_id)?;
