...
pub(crate) fn am_autovacuum_worker_process() -> bool {
    init_small_seams::my_backend_type::call() == BackendType::AutovacWorker
}
...
pub(crate) fn autovacuum_launcher_pid() -> i32 {
    autovacuum_ext_seams::get_launcher_pid::call()
}
...
pub(crate) fn on_shmem_exit(callback: fn(i32, Datum<'static>) -> PgResult<()>, arg: Datum<'static>) {
    ...
    dsm_core_seams::on_shmem_exit::call(callback, arg)
...
pub(crate) fn disown_latch(procno: ProcNumber) {
    latch_seams::disown_latch::call(crate::proc_shmem::proc_latch_handle(procno));
}
...
pub(crate) fn pgstat_set_wait_event_storage(procno: ProcNumber) {
    pgstat_seams::pgstat_set_wait_event_storage_for_proc::call(procno);
}

/// `pgstat_reset_wait_event_storage()`.
pub(crate) fn pgstat_reset_wait_event_storage() {
    pgstat_seams::pgstat_reset_wait_event_storage::call();
}
...
pub(crate) fn init_lwlock_access() {
    lwlock_seams::init_lwlock_access::call();
}
...
pub(crate) fn lwlock_release(lock: LWLockHandle) {
    lwlock_seams::lwlock_release_main::call(lock)
...
pub(crate) fn deadlock_check(procno: ProcNumber) -> DeadLockState {
    let (mut space, my_id) =
        lock_seams::build_dead_lock_space::call(procno);
...
pub(crate) fn remember_simple_deadlock(
    ...
    deadlock_seams::remember_simple_dead_lock::call(
        &space,
...
pub(crate) fn proc_array_add(procno: ProcNumber) {
    ...
    procarray_seams::proc_array_add::call(procno).expect("ProcArrayAdd");
}
...
pub(crate) fn proc_array_remove(procno: ProcNumber, latest_xid: TransactionId) {
    procarray_seams::proc_array_remove::call(procno, latest_xid)
