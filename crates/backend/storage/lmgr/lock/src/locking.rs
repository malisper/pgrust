...
pub fn LockAcquireExtended(
    ...
    if wait_result == PROC_WAIT_STATUS_ERROR {
        ...
        if dont_wait {
            if log_lock_failure {
                let buf = lmgr_seams::describe_lock_tag::call(*locktag);
            ...
            return Err(deadlock_seams::dead_lock_report::call());
        }
    }
...
pub(crate) fn get_lock_holders_and_waiters_seam(
    locktag: LOCKTAG,
) -> lock_seams::LockHoldersAndWaiters {
...
fn get_lock_holders_and_waiters(
    ...
    state::with_shared(|s| {
        if !s.lock_exists(locktag) {
            return lock_seams::LockHoldersAndWaiters::default();
        }
