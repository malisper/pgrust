...
fn lwlock_acquire_guard<'l>(
    ...
    mode: LWLockMode,
    my_proc_number: ProcNumber,
) -> PgResult<lwlock_seams::LWLockGuard<'l>> {
    let was_free = LWLockAcquire(lock, mode, my_proc_number)?;
    Ok(lwlock_seams::LWLockGuard::new(
        lock, was_free,
    ))
}
...
fn lwlock_conditional_acquire_guard<'l>(
    lock: &'l LWLock,
    mode: LWLockMode,
) -> PgResult<Option<lwlock_seams::LWLockGuard<'l>>> {
    if LWLockConditionalAcquire(lock, mode)? {
        Ok(Some(lwlock_seams::LWLockGuard::new(
            lock, true,
        )))
    } else {
        Ok(None)
...
fn lwlock_acquire_main_seam(
    offset: usize,
    mode: LWLockMode,
) -> PgResult<lwlock_seams::MainLWLockGuard> {
    let lock = main_lock(offset);
    let was_free = LWLockAcquire(lock, mode, globals::my_proc_number::call())?;
    Ok(lwlock_seams::MainLWLockGuard::new(
        offset, was_free,
    ))
}
...
pub fn init_seams() {
    lwlock_seams::lwlock_initialize::set(LWLockInitialize);
    lwlock_seams::get_lwlock_identifier::set(get_lwlock_identifier_seam);
    lwlock_seams::lwlock_acquire::set(lwlock_acquire_guard);
    lwlock_seams::lwlock_conditional_acquire::set(
        lwlock_conditional_acquire_guard,
    );
    lwlock_seams::lwlock_release::set(LWLockRelease);
    lwlock_seams::lwlock_acquire_main::set(lwlock_acquire_main_seam);
    lwlock_seams::lwlock_release_main::set(lwlock_release_main_seam);
    lwlock_seams::lwlock_release_all::set(lwlock_release_all_seam);
    lwlock_seams::lock_twophase_state::set(lock_twophase_state_seam);
    lwlock_seams::unlock_twophase_state::set(unlock_twophase_state_seam);
    lwlock_seams::twophase_state_held_exclusive::set(
        twophase_state_held_exclusive_seam,
    );
    lwlock_seams::lock_relation_mapping::set(lock_relation_mapping_seam);
    lwlock_seams::unlock_relation_mapping::set(unlock_relation_mapping_seam);
    ...
    tablespace_globals_seams::lwlock_acquire_tablespace_create::set(
        lwlock_acquire_tablespace_create_seam,
    );
    tablespace_globals_seams::lwlock_release_tablespace_create::set(
        lwlock_release_tablespace_create_seam,
    );
    lwlock_seams::relation_mapping_lock_held_by_me_exclusive::set(
        relation_mapping_lock_held_by_me_exclusive_seam,
    );
    lwlock_seams::lwlock_acquire_proc_array::set(
        lwlock_acquire_proc_array_seam,
    );
    lwlock_seams::lwlock_release_proc_array::set(
        lwlock_release_proc_array_seam,
    );
    lwlock_seams::lwlock_conditional_acquire_proc_array::set(
        lwlock_conditional_acquire_proc_array_seam,
    );
    lwlock_seams::lwlock_held_by_me_proc_array::set(
        lwlock_held_by_me_proc_array_seam,
    );
    lwlock_seams::lwlock_acquire_xid_gen::set(lwlock_acquire_xid_gen_seam);
    lwlock_seams::lwlock_release_xid_gen::set(lwlock_release_xid_gen_seam);
    lwlock_seams::lwlock_acquire_wrap_limits_vacuum::set(
        lwlock_acquire_wrap_limits_vacuum_seam,
    );
    lwlock_seams::lwlock_release_wrap_limits_vacuum::set(
        lwlock_release_wrap_limits_vacuum_seam,
    );
    lwlock_seams::lwlock_held_by_me_main::set(lwlock_held_by_me_main_seam);
    lwlock_seams::lwlock_held_by_me_in_mode_main::set(
