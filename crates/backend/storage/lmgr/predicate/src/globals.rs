...
pub fn proc_send_signal(procno: ProcNumber) {
    lmgr_proc_seams::set_proc_latch::call(procno);
}

/// `ProcWaitForSignal(WAIT_EVENT_SAFE_SNAPSHOT)`.
#[inline]
pub fn proc_wait_for_signal_safe_snapshot() -> types_error::PgResult<()> {
    lmgr_proc_seams::proc_wait_for_signal::call(WAIT_EVENT_SAFE_SNAPSHOT)
}
...
pub fn is_in_parallel_mode() -> bool {
    transam_xact_seams::is_in_parallel_mode::call()
}
...
#[inline]
pub fn get_transaction_snapshot() -> types_error::PgResult<snapshot::SnapshotData> {
    snapmgr_seams::get_transaction_snapshot::call()
}

#[inline]
pub fn get_snapshot_data() -> types_error::PgResult<snapshot::SnapshotData> {
    procarray_seams::get_snapshot_data::call()
}
...
pub fn isolation_is_serializable() -> bool {
    ...
    // XACT_SERIALIZABLE == 3.
    transam_xact_seams::xact_iso_level::call() == XACT_SERIALIZABLE
}
...
pub fn xact_read_only() -> bool {
    transam_xact_seams::xact_read_only::call()
}
...
pub fn max_prepared_xacts() -> i32 {
    init_small_seams::max_prepared_xacts::call()
}

#[inline]
pub fn register_two_phase_record(rmid: u8, info: u16, data: &[u8]) -> types_error::PgResult<()> {
    twophase_seams::register_two_phase_record::call(rmid, info, data)
}
...
pub fn xact_deferrable() -> bool {
    transam_xact_seams::xact_deferrable::call()
}
...
pub fn parallel_context_active() -> bool {
    transam_parallel::parallel_context_active()
}
