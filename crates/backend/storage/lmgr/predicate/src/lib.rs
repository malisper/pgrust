...
fn rel_fields(oid: Oid) -> PgResult<RelFields> {
    let cell = relcache_seams::relation_id_get_relation_shared::call(oid)?;
    ...
    relcache_seams::relation_close::call(oid)?;
    Ok(fields)
}
...
pub fn check_serial_buffers(newval: i32) -> (bool, Option<String>) {
    slru::check_slru_buffers("serial_buffers", newval)
}

pub fn init_seams() {
    use predicate_seams as seams;
    ...
    parallel_rt_seams::attach_serializable_xact::set(|handle: usize| {
    ...
    lock_seams::safe_snapshot_blocking_pids::set(|mcx, blocked_pid| {
...
fn relation_fields_from_handle(rel: &rel::Relation<'_>) -> PgResult<(Oid, Oid, bool)> {
    relcache_seams::relation_with_entry(rel, |e| {
