...
pub fn PageSetChecksumCopy(page: &PageRef<'_>, blkno: BlockNumber) -> PgResult<PageTemp> {
    ...
    if !PageIsNew(page) && transam_xlog_seams::data_checksums_enabled::call() {
...
pub fn PageSetChecksumInplace(page: &mut PageMut<'_>, blkno: BlockNumber) {
    if PageIsNew(&page.as_ref()) || !transam_xlog_seams::data_checksums_enabled::call() {
        return;
    }
...
pub fn init_seams() {
    ...
    // read/write it.
    guc_tables::vars::ignore_checksum_failure.install(
    ...
    page_seams::ignore_checksum_failure::set(|| {
        guc_tables::vars::ignore_checksum_failure.read()
    });
}
