...
fn sync_filetag(handler: SyncRequestHandler, ftag: FileTag) -> PgResult<FileTagOpResult> {
    match handler {
        SyncRequestHandler::SYNC_HANDLER_MD => {
            md_seams::mdsyncfiletag::call(ftag)
        }
        SyncRequestHandler::SYNC_HANDLER_CLOG => {
            clog_seams::clogsyncfiletag::call(ftag)
        }
        SyncRequestHandler::SYNC_HANDLER_COMMIT_TS => {
            commit_ts_seams::committssyncfiletag::call(ftag)
        }
        SyncRequestHandler::SYNC_HANDLER_MULTIXACT_OFFSET => {
            multixact_seams::multixactoffsetssyncfiletag::call(ftag)
        }
        SyncRequestHandler::SYNC_HANDLER_MULTIXACT_MEMBER => {
            multixact_seams::multixactmemberssyncfiletag::call(ftag)
        }
...
fn unlink_filetag(handler: SyncRequestHandler, ftag: FileTag) -> PgResult<FileTagOpResult> {
    match handler {
        SyncRequestHandler::SYNC_HANDLER_MD => {
            md_seams::mdunlinkfiletag::call(ftag)
        }
...
fn filetag_matches(
    ...
    match handler {
        SyncRequestHandler::SYNC_HANDLER_MD => {
            md_seams::mdfiletagmatches::call(ftag, candidate)
        }
...
fn process_sync_requests(enable_fsync: bool, log_checkpoints: bool) -> PgResult<()> {
        ...
        if enable_fsync {
            ...
            loop {
                ...
                if !file_possibly_deleted(op.errno) || failures > 0 {
                    let elevel = file_seams::data_sync_elevel::call(ERROR);
    ...
    checkpointer_seams::checkpoint_stats_set::call(
        processed,
        longest,
        total_elapsed,
    );
...
fn register_sync_request(
    ...
    loop {
        ...
        let ret = checkpointer_seams::forward_sync_request::call(
            *ftag,
            request_type,
        )?;
        ...
        latch_seams::wait_latch_register_sync_request::call()?;
    }
}
...
pub fn init_seams() {
    sync_seams::init_sync::set(init_sync_seam);
    sync_seams::sync_pre_checkpoint::set(sync_pre_checkpoint_seam);
    sync_seams::sync_post_checkpoint::set(sync_post_checkpoint_seam);
    sync_seams::process_sync_requests::set(process_sync_requests_seam);
    sync_seams::register_sync_request::set(register_sync_request_seam);
    sync_seams::remember_sync_request::set(remember_sync_request_seam);
}
