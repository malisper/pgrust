...
pub fn backend_main(startup_data: &StartupData) -> ! {
    ...
    let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        let client_sock = init_small_seams::my_client_socket::call()
        ...
        postgres_seams::postgres_main::call(
            dbname.as_deref(),
            username.as_deref(),
        )
    }));
...
fn backend_initialize_inner(
    ...
    let pre_auth_delay = postmaster_seams::pre_auth_delay::call();
    if pre_auth_delay > 0 {
        pgsleep_seams::pg_usleep::call(pre_auth_delay as i64 * 1_000_000);
    }
    ...
    let port = pqcomm_seams::pq_init::call(&client_sock, my_latch)?;
    ...
    let log_hostname = postmaster_seams::log_hostname::call();
    ...
    let ret = ip_seams::pg_getnameinfo_all::call(
        &raddr,
    ...
    let auth_timeout = postmaster_seams::authentication_timeout::call();
    ...
    dsm_core_seams::check_on_shmem_exit_lists_are_empty::call()?;
...
fn reject_for_cac_state(cac: CacState) -> PgResult<()> {
    let err = match cac {
        ...
        CacState::NotHotStandby => {
            let enable_hot_standby = transam_xlog_seams::enable_hot_standby::call();
...
fn build_ps_title() {
    ...
    if am_walsender {
        let desc = miscinit_seams::get_backend_type_desc::call(
    ...
    more_seams::init_ps_display::call(Some(&ps_data));
    ps_status_seams::set_ps_display::call("initializing".to_string());
}
...
fn process_ssl_startup() -> PgResult<i32> {
    ...
    let firstbyte = pqcomm_seams::pq_peekbyte::call()?;
    ...
    if be_secure_seams::ssl_supported::call() {
        ...
        let opened = with_proc_port(be_secure_seams::secure_open_server::call);
        if opened == -1 {
            return reject_direct_ssl();
        }
        // Assert(port->ssl_in_use);
...
fn process_startup_packet(mcx: Mcx<'_>, ssl_done: bool, gss_done: bool) -> PgResult<i32> {
    ...
    if proto == NEGOTIATE_SSL_CODE && !ssl_done {
        let ssl_ok: u8 = if be_secure_seams::ssl_supported::call() {
        ...
        if be_secure_seams::ssl_supported::call()
            && ssl_ok == b'S'
            && with_proc_port(be_secure_seams::secure_open_server::call) == -1
        {
            return Ok(STATUS_ERROR);
        }

        // if (pq_buffer_remaining_data() > 0) ereport(FATAL, ...);
        if pqcomm_seams::pq_buffer_remaining_data::call() > 0 {
        ...
        let gss_ok: u8 = if be_secure_seams::gss_supported::call()
        ...
        if be_secure_seams::gss_supported::call()
            && gss_ok == b'G'
            && with_proc_port(be_secure_seams::secure_open_gssapi::call) == -1
        {
            return Ok(STATUS_ERROR);
        }
        ...
        while offset < len {
            ...
            if cstr_eq(&buf, name_off, b"database") {
                ...
                if cstr_eq(&buf, val_off, b"database") {
                    walsender_seams::set_am_walsender::call(true);
                    walsender_seams::set_am_db_walsender::call(true);
                } else {
                    // else if (!parse_bool(valptr, &am_walsender))
                    match scalar_seams::parse_bool::call(&valstr) {
                        Some(b) => {
                            walsender_seams::set_am_walsender::call(b)
                        }
                        None => {
                            return Err(ereport(FATAL)
                ...
                if cstr_eq(&buf, name_off, b"application_name") {
                    ...
                    let cleaned = string_seams::pg_clean_ascii::call(mcx, &vl, 0)?;
    ...
    let am_walsender = walsender_seams::am_walsender::call();
    if am_walsender {
        init_small_seams::set_my_backend_type::call(BackendType::WalSender);
    } else {
        init_small_seams::set_my_backend_type::call(BackendType::Backend);
    }

    // Normal (non-db) walsenders are not connected to a particular database.
    if am_walsender && !walsender_seams::am_db_walsender::call() {
...
fn write_negotiation_byte(byte: u8, which: &str) -> PgResult<bool> {
    ...
    loop {
        let result =
            with_proc_port(|port| be_secure_seams::secure_write::call(port, &buf));
...
fn process_cancel_request_packet(pkt: &[u8], pktlen: i32) -> PgResult<()> {
    ...
    procsignal::SendCancelRequest(backend_pid, cancel_auth_code);
    Ok(())
}
...
pub fn check_log_connections(mcx: Mcx<'_>, newval: &str) -> PgResult<Result<u32, String>> {
    ...
    let elemlist =
        match varlena_seams::split_identifier_string::call(mcx, newval, ',')? {
            Some(list) => list,
            None => {
...
pub fn init_seams() {
    backend_startup_seams::backend_main::set(backend_main);
...
fn arm_startup_packet_signals() {
    port_pqsignal_seams::pqsignal::call(
        libc::SIGTERM,
        signal::SigHandler::Handler(process_startup_packet_die_signal),
    );
}
...
fn ssl_negotiation_disabled() -> bool {
    with_proc_port(be_secure_seams::ssl_negotiation_disabled::call)
}

/// `port->laddr.addr.ss_family == AF_UNIX` (the GSS-on-Unix-socket guard).
fn gss_negotiation_disabled() -> bool {
    with_proc_port(be_secure_seams::gss_negotiation_disabled::call)
}
