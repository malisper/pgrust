...
fn lock_error_cleanup() {
    lmgr_proc_seams::lock_error_cleanup::call();
}
...
pub fn quickdie(_postgres_signal_arg: i32) -> ! {
    ...
    libpq_pqsignal::block_sig_add(libc_sigquit());
    ...
    // the client.
    match pmsignal::GetQuitSignalReason() {
...
pub fn die(_postgres_signal_arg: i32) {
    ...
    postgres_seams::set_session_end_cause_killed::call();
...
fn awaited_lock_is_set() -> bool {
    lock_seams::get_awaited_lock_hashcode::call() != -1
}
...
fn ProcessRecoveryConflictInterrupt(reason: ProcSignalReason) -> PgResult<()> {
    ...
    if do_fall_lock {
        ...
        if !transam_xact_seams::is_transaction_or_transaction_block::call() {
    ...
    if entered_at_logicalslot {
            ...
            || !transam_xact_seams::is_sub_transaction::call()
        {
            ...
            if transam_xact_seams::is_aborted_transaction_block_state::call() {
                return Ok(());
            }
            ...
            if !globals::doing_command_read() {
                ...
                lock_error_cleanup();
                postgres_seams::pgstat_report_recovery_conflict::call(reason);
                if errstart(ERROR, None) {
...
fn fall_bufferpin(reason: ProcSignalReason, do_fall_lock: &mut bool) -> PgResult<()> {
    ...
    if !bufmgr_seams::holding_buffer_pin_that_delays_recovery::call() {
        if reason == ProcSignalReason::PROCSIG_RECOVERY_CONFLICT_STARTUP_DEADLOCK
            && lmgr_proc::proc_lifecycle::GetStartupBufferPinWaitBufId() < 0
        {
    ...
    lmgr_proc::proc_misc::set_my_proc_recovery_conflict_pending(true);
...
pub fn ProcessInterrupts() -> PgResult<()> {
    ...
    if g::ProcDiePending() {
        ...
        if ::utils_error::config::client_auth_in_progress() {
        ...
        } else if postgres_seams::is_logical_worker::call() {
            return die_fatal(
                ERRCODE_ADMIN_SHUTDOWN,
                "terminating logical replication worker due to administrator command".to_string(),
            );
        } else if launcher_seams::IsLogicalLauncher::call() {
            ...
            let bgw_type = postgres_seams::my_bgworker_type::call();
            return die_fatal(
                ERRCODE_ADMIN_SHUTDOWN,
                alloc::format!(
    ...
    if g::CheckClientConnectionPending() {
        ...
        if !globals::doing_command_read() && client_connection_check_interval() > 0 {
            if !pqcomm::pq_check_connection()? {
                g::SetClientConnectionLost(true);
            } else {
                misc_timeout::enable_timeout_after(
    ...
    if g::QueryCancelPending() && g::QueryCancelHoldoffCount() != 0 {
        ...
        let mut lock_timeout_occurred =
            misc_timeout::get_timeout_indicator(TimeoutId::LOCK_TIMEOUT, true);
        let stmt_timeout_occurred =
            misc_timeout::get_timeout_indicator(TimeoutId::STATEMENT_TIMEOUT, true);
        ...
        if lock_timeout_occurred
            ...
            && misc_timeout::get_timeout_finish_time(TimeoutId::STATEMENT_TIMEOUT)
                < misc_timeout::get_timeout_finish_time(TimeoutId::LOCK_TIMEOUT)
        {
    ...
    if procsignal::ProcSignalBarrierPending() {
        procsignal::ProcessProcSignalBarrier()?;
    }

    if transam_parallel::parallel_message_pending() {
        transam_parallel::process_parallel_messages()?;
    }
    ...
    if applyparallelworker::parallel_apply_message_pending() {
        applyparallelworker::ProcessParallelApplyMessages()?;
    }
...
pub fn errdetail_abort() -> PgResult<()> {
    if lmgr_proc::proc_misc::my_proc_recovery_conflict_pending() {
