...
pub fn pg_analyze_and_rewrite_varparams<'mcx>(
    ...
    Ok(parser_analyze_seams::AnalyzedVarparams {
...
pub fn exec_simple_query<'mcx>(mcx: Mcx<'mcx>, query_string: &'mcx str) -> PgResult<()> {
    ...
    for (idx, parsetree) in parsetree_list.iter().enumerate() {
        ...
        let (cmdtagname, _cmdtaglen) =
            cmdtag::get_command_tag_name_and_len(portal_tag(command_tag));

        // set_ps_display_with_len(cmdtagname, cmdtaglen) — the activity title.
        more_seams::set_ps_display::call(cmdtagname);
        ...
        if use_implicit_block {
            transam_xact::BeginImplicitTransactionBlock();
        }
        ...
        if is_last {
            ...
            if use_implicit_block {
                transam_xact::EndImplicitTransactionBlock();
            }
            finish_xact_command()?;
...
fn portal_tag(tag: CommandTag) -> portal::CommandTag {
    tag.0
}
