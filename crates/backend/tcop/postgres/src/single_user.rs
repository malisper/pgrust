...
fn run(argv: &[&str], username: &str) -> PgResult<()> {
    ...
    miscinit_seams::init_standalone_process::call(
    ...
    // its `SplitDirectoriesString` parse needs.
    {
        ...
        miscinit::process_shared_preload_libraries(spl_cx.mcx())?;
    }
    ...
    // seam.
    s::set_pg_start_time::call(timestamp_seams::get_current_timestamp::call());

    // Create a per-backend PGPROC in shared memory; required before LWLocks.
    miscinit_seams::init_process::call()?;
