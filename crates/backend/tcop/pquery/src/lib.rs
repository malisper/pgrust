...
use tstorereceiver_seams as tstore_seam;
use dest_seams as dest_seam;
use postgres_seams as postgres_seam;
use utility_seams as utility_seam;
use portalmem_seams as portalmem_seam;
use sort_storage_seams as sortstore_seam;
...
fn run_ctas_executor<'mcx>(
     ...
     */
    let mut rewritten =
        rewritehandler_seams::query_rewrite_canonical::call(mcx, query)?;
...
fn run_matview_datafill<'mcx>(
     ...
     */
    let locked =
        rewritehandler_seams::acquire_rewrite_locks::call(mcx, query, true, false)?;
    let mut rewritten =
        rewritehandler_seams::query_rewrite_canonical::call(mcx, locked)?;
...
fn do_portal_run_fetch(
        ...
        FETCH_ABSOLUTE => {
            if count > 0 {
                ...
                if (count - 1) as u64 <= portal_pos / 2 || portal_pos >= i64::MAX as u64 {
                    ...
                    if count > 1 {
                        portal_run_select(portal, true, count - 1, tcop_dest::none_receiver())?;
                    }
                } else {
                 ...
                 * prior row (if any). ...
                 */
                portal_run_select(portal, true, FETCH_ALL, tcop_dest::none_receiver())?;
                if count < -1 {
                    portal_run_select(portal, false, -count - 1, tcop_dest::none_receiver())?;
                }
    ...
    if count == 0 {
        ...
        if dest_seam::dest_get_mydest::call(dest) == CommandDest::None {
             ...
             */
            if on_row {
                portal_run_select(portal, false, 1, tcop_dest::none_receiver())?;
...
pub fn init_seams() {
    pquery_seams::portal_start::set(|portal, params, eflags, snapshot| {
        portal_start(portal, params, eflags, snapshot)
    });
    pquery_seams::portal_run::set(|portal, count, is_top_level, dest, altdest, qc| {
        portal_run(portal, count, is_top_level, dest, altdest, qc)
    });
    pquery_seams::portal_run_fetch::set(|portal, fdirection, count, dest| {
        portal_run_fetch(portal, fdirection, count, dest)
    });
    pquery_seams::choose_portal_strategy_queries::set(choose_portal_strategy_queries);
    pquery_seams::ensure_portal_snapshot_exists::set(ensure_portal_snapshot_exists);
    ...
    createas_seams::run_ctas_executor::set(run_ctas_executor);
    ...
    matview_deps_seams::run_matview_datafill::set(run_matview_datafill);
    ...
    pg_proc_seams::active_portal_source_text::set(|| {
