...
//! [`utility_out_seams`] (xact-control, portal verbs, every
...
pub fn ProcessUtility<'mcx>(
    ...
    if utility_seams::process_utility_hook_present() {
        utility_seams::call_process_utility_hook(
            mcx,
            pstmt,
            query_string,
            read_only_tree,
    ...
    match parsetree.node_tag() {
        ...
        t if t == ntag::T_CheckPointStmt => {
            ...
            rt::request_checkpoint::call(
                wal::xlog_consts::CHECKPOINT_IMMEDIATE
                    | wal::xlog_consts::CHECKPOINT_WAIT
                    | (if rt::checkpoint_recovery_in_progress::call() {
                        0
                    } else {
                        wal::xlog_consts::CHECKPOINT_FORCE
                    }),
            )?;
        }
