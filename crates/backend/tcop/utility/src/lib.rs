...
//! repo's owned [`nodes::nodes::Node`] tree — the read-only classification,
...
//! classifiers consult cross [`utility_out_seams`], defaulting to a
...
//! over the owned [`nodes::nodes::Node`] tree (the `readOnlyTree`
...
//! `utility_seams::process_utility` seam now carries the per-utility
...
//! reached through a forwarding seam in [`utility_out_seams`]
...
pub fn init_seams() {
    ...
    utility_seams::command_is_read_only::set(CommandIsReadOnly);
    utility_seams::planned_stmt_command_tag::set(CreateCommandTagForPlannedStmt);
    ...
    // uninstalled seam.
    utility_out_seams::recovery_in_progress::set(|| {
        transam_xlog_seams::recovery_in_progress::call()
    });
    utility_out_seams::checkpoint_recovery_in_progress::set(|| {
        transam_xlog_seams::recovery_in_progress::call()
    });
    ...
    utility_out_seams::process_utility_slow::set(slow::process_utility_slow);
    ...
    utility_out_seams::process_utility_wrapper::set(dispatch::process_utility_wrapper);
    ...
    utility_fc_seams::process_utility_create_schema_subcommand::set(
