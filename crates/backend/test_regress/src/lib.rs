...
//! ([`dfmgr_seams::builtin_library_present`] /
//! [`dfmgr_seams::resolve_builtin_library_function`]). When
...
fn fc_binary_coercible(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    match coerce_seams::is_binary_coercible::call(srctype, targettype) {
...
fn fc_pt_in_widget(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    match geo_ops::point_distance(&point, &center) {
...
fn fc_test_valid_server_encoding(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    let valid = extra_encnames_fgram::pg_valid_server_encoding(&name) >= 0;
...
fn fc_is_catalog_text_unique_index_oid(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    Datum::from_bool(catalog_catalog::IsCatalogTextUniqueIndexOid(relid))
}
...
fn fc_interpt_pp(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    let p1 = geo_ops::Path::from_datum_image(arg_bytes(fcinfo, 0));
    let p2 = geo_ops::Path::from_datum_image(arg_bytes(fcinfo, 1));
    ...
    while i + 1 < npts1 {
        ...
        while j + 1 < npts2 {
            ...
            match geo_ops::lseg_intersect(&seg1, &seg2) {
                Ok(true) => {
...
fn fc_regress_setenv(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    match superuser_seams::superuser::call() {
        Ok(true) => {}
...
fn char_to_encoding(name: &[u8]) -> i32 {
    ...
    extra_encnames_fgram::pg_char_to_encoding(&String::from_utf8_lossy(&name[..end]))
}
...
fn fc_test_enc_setup(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    for i in 0..PG_LAST_ENCODING {
        ...
        let name = extra_encnames_fgram::pg_encoding_to_char(i);
        if len != 2 {
            warn(&format!(
...
fn fc_test_enc_conversion(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    if src_encoding == dest_encoding {
        ...
        if oklen as usize == srclen {
            ...
            match mbutils_seams::report_invalid_encoding::call(
                src_encoding,
                &src[oklen as usize..],
            ) {
        ...
        let proc = match mbutils_seams::find_default_conversion_proc::call(
            src_encoding,
            dest_encoding,
        ) {
            Ok(proc) => proc,
        ...
        let (consumed, dst) = match mbutils_seams::pg_do_encoding_conversion_buf::call(
            m.mcx(),
            proc,
            src_encoding,
            dest_encoding,
...
fn build_conv_record(
    ...
    let datum =
        match funcapi_seams::record_from_values::call(mcx, coltypes, &values, &nulls)
        {
            Ok(d) => d,
            Err(err) => raise(err),
        };
...
fn fc_test_mblen_func(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    let result: i32 = match func.as_str() {
        "pg_mblen_unbounded" => mbutils::pg_mblen_unbounded(at_offset),
        "pg_mblen_cstr" => match mbutils::pg_mblen_cstr(at_offset) {
            Ok(n) => n,
            Err(err) => raise(err),
        },
        "pg_mblen_with_len" => {
            match mbutils::pg_mblen_with_len(at_offset, (size - offset) as i32) {
                Ok(n) => n,
                Err(err) => raise(err),
            }
        }
        "pg_mblen_range" => match mbutils::pg_mblen_range(at_offset) {
            Ok(n) => n,
            Err(err) => raise(err),
        },
        "pg_encoding_mblen" => {
...
fn fc_test_text_to_wchars(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    let datums: Vec<Datum> = if size > 0 {
        let wchars = match mbutils::pg_encoding_mb2wchar_with_len(
            m.mcx(),
            encoding,
            &data,
            size as i32,
...
fn fc_test_wchars_to_text(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    let bytes: Vec<u8> = if !elems.is_empty() {
        ...
        match mbutils::pg_encoding_wchar2mb_with_len(
            m.mcx(),
            encoding,
            &wchars,
            wlen,
...
fn fc_get_environ(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    let image = match arrayfuncs::construct::construct_text_array_bytes_str(m.mcx(), &refs) {
...
fn fc_overpaid(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    // salary = DatumGetInt32(GetAttributeByName(tuple, "salary", &isnull));
    let (value, isnull) =
        match execUtils::GetAttributeByName(m.mcx(), Some(&tuple), "salary") {
            Ok(pair) => pair,
            Err(err) => raise(err),
        };
    if isnull {
...
fn fc_trigger_return_old(fcinfo: &mut FunctionCallInfoBaseData) -> Datum {
    ...
    let called_as_trigger =
        commands_trigger::firing::with_current_trigger_data(|td| td.is_some());
    ...
    commands_trigger::firing::set_before_trigger_result_to_trigtuple();
...
fn make_tuple_indirect_impl(
    ...
    for i in 0..ncolumns {
        ...
        let token = toast_internals_seams::register_indirect_target(&target);
...
fn test_support_func_cost(
    _funcid: types_core::Oid,
    _node: Option<&nodes::primnodes::Expr>,
...
pub fn init_seams() {
    ...
    clauses::support_rows::register_support_rows_by_symbol(
        "test_support_func",
        test_support_func_rows,
    );
    clauses::support_cost::register_support_cost_by_symbol(
        "test_support_func",
