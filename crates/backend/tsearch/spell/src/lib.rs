...
pub enum AffixReg<'mcx> {
    ...
    /// carried as the [`regex::RegexCompiled`] value.
...
impl Drop for AffixReg<'_> {
    fn drop(&mut self) {
        match self {
            ...
            AffixReg::Regex(h) => regex_core_seams::pg_regfree::call(h.clone()),
        }
    }
}
...
fn pg_mblen(s: &[u8]) -> usize {
    ...
    mbutils_seams::pg_mblen_range::call(s).unwrap_or(s.len() as i32) as usize
}
...
fn t_isalpha(s: &[u8]) -> bool {
    ts_locale_seams::t_isalpha::call(s)
}
...
fn str_tolower(mcx: Mcx<'_>, src: &[u8]) -> PgResult<Vec<u8>> {
    let folded = formatting_seams::str_tolower::call(mcx, src, DEFAULT_COLLATION_OID)?;
