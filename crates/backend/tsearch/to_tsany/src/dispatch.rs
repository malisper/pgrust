...
fn lexize_by_oid(
    ...
    match name.as_str() {
        "dsimple_lexize" => {
            let d = dict::dict_simple::dsimple_init(mcx, &options)?;
            let r = dict::dict_simple::dsimple_lexize(mcx, &d, input, len)?;
            Ok(convert_pgvec(r))
        }
        "dsnowball_lexize" => {
            let d = dict_snowball::dsnowball_init(mcx, &options)?;
            let r = dict_snowball::dsnowball_lexize(mcx, &d, input, len)?;
            Ok(convert_pgvec(r))
        }
        "dsynonym_lexize" => {
            let d = dict::dict_synonym::dsynonym_init(mcx, &options)?;
            let r = dict::dict_synonym::dsynonym_lexize(mcx, &d, input, len)?;
            Ok(convert_pgvec(r))
        }
        "dispell_lexize" => {
            let d = ispell_regis::dispell_init(mcx, &options)?;
            let r = ispell_regis::dispell_lexize(mcx, &d, input, len)?;
            Ok(convert_pgvec(r))
        }
        "thesaurus_lexize" => {
            ...
            let (out, getnext_out, stored_out) =
                thesaurus_cache::with_dict(dict_id, &options, |cmcx, d| {
                    ...
                    let r = dict::dict_thesaurus::thesaurus_lexize(
        ...
        STATE.with(|s| {
            ...
            slot.as_mut().unwrap().with_mut(|cache| {
                if !cache.dicts.contains_key(&dict_id) {
                    let d = dict::dict_thesaurus::thesaurus_init(
                        cache.mcx, options,
                    )?;
