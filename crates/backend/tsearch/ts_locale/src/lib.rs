...
//! seam declared in [`ts_locale_seams`]).
//!
...
fn readfile(filename: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let raw = match fd::seams::allocate_file_read(path) {
        Ok(Some(bytes)) => bytes,
...
pub fn init_seams() {
    ts_locale_seams::t_isalpha::set(t_isalpha);
    ts_locale_seams::t_isalnum::set(t_isalnum);
    ts_locale_seams::readfile::set(readfile);
}
