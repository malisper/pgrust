...
pub fn pgstat_count_backend_io_op_time(
    ...
    debug_assert!(
        guc_tables::vars::track_io_timing.read()
...
fn pgstat_backend_pid_lookup(pid: i32) -> Option<(ProcNumber, BackendType, i32)> {
    ...
    let proc_number =
        match procarray_seams::backend_pid_get_proc_role::call(pid) {
            Some((_role, procno)) => procno,
            None => match lmgr_proc_seams::auxiliary_pid_get_proc::call(pid) {
                Some(procno) => procno,
                None => return None,
            },
        };
    ...
    let (st_backend_type, st_procpid) =
        status_seams::beentry_backend_type_and_pid::call(proc_number)?;
...
pub fn init_seams() {
    ...
    pgstat_io_seams::pgstat_count_backend_io_op::set(
        pgstat_count_backend_io_op,
    );
    pgstat_io_seams::pgstat_count_backend_io_op_time::set(
    ...
    pgstat_io_seams::pgstat_flush_backend_io::set(|nowait| {
    ...
    pgstat_wal_seams::pgstat_flush_backend_wal::set(|nowait| {
