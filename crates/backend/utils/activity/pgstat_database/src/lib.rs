...
pub fn init_seams() {
    ...
    postgres_seams::pgstat_report_recovery_conflict::set(|reason| {
    ...
    pgstat_database_seams::pgstat_count_conn_active_time::set(|usecs| {
        pgstat_count_conn_active_time(usecs as PgStat_Counter);
    });
    pgstat_database_seams::pgstat_count_conn_txn_idle_time::set(|usecs| {
    ...
    pgstat_database_seams::pgstat_update_parallel_workers_stats::set(
        |to_launch, launched| {
    ...
    stat_seams::at_eoxact_pgstat_database::set(AtEOXact_PgStat_Database);
    stat_seams::pgstat_set_session_end_cause_fatal::set(|| {
    ...
    postgres_seams::set_session_end_cause_killed::set(|| {
        set_pgstat_session_end_cause(SessionEndType::DISCONNECT_KILLED);
    });

    pgstat_seams::pgstat_drop_database::set(pgstat_drop_database);
    pgstat_seams::pgstat_prepare_report_checksum_failure::set(
        pgstat_prepare_report_checksum_failure,
    );
    pgstat_seams::pgstat_report_checksum_failures_in_db::set(
        pgstat_report_checksum_failures_in_db,
    );
    pgstat_seams::pgstat_report_connect::set(pgstat_report_connect);
    ...
    autovacuum_ext_seams::pgstat_track_counts::set(pgstat_track_counts);
    autovacuum_ext_seams::pgstat_report_autovac::set(|dbid| {
        pgstat_report_autovac(dbid).expect("pgstat_report_autovac failed");
    });
    autovacuum_ext_seams::pgstat_fetch_stat_dbentry::set(|datid| {
    ...
    vacuumlazy_seams::pgstat_block_read_time::set(|| {
        Ok(PG_STAT_BLOCK_READ_TIME.with(|c| c.get()))
    });
    vacuumlazy_seams::pgstat_block_write_time::set(|| {
