...
fn pgstat_track_counts() -> bool {
    guc_tables::vars::pgstat_track_counts.read()
}
...
fn timestamp_difference_milliseconds(start: TimestampTz, stop: TimestampTz) -> PgStat_Counter {
    timestamp_seams::timestamp_difference_milliseconds::call(start, stop)
}

/// `GetCurrentTransactionStopTimestamp()`.
fn get_current_transaction_stop_timestamp() -> TimestampTz {
    transam_xact_seams::get_current_transaction_stop_timestamp::call()
}

/// `GetCurrentTransactionNestLevel()`.
fn get_current_transaction_nest_level() -> i32 {
    transam_xact_seams::get_current_transaction_nest_level::call()
}
...
pub fn pgstat_report_vacuum(
    ...
    stat_seams::pgstat_flush_io::call(false)?;
...
pub fn init_seams() {
    ...
    bufmgr_seams::count_buffer_read::set(|relid, relisshared, en| {
        pgstat_count_buffer_read(relid, relisshared, en)
            .expect("pgstat_count_buffer_read failed");
    });
    bufmgr_seams::count_buffer_hit::set(|relid, relisshared, en| {
    ...
    // `rel->pgstat_enabled`.
    tablecmds_seams::pgstat_count_truncate::set(|rel| {
    ...
    vacuumlazy_seams::pgstat_report_vacuum::set(
    ...
    analyze_rt_seams::pgstat_report_analyze::set(
