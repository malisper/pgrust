...
fn NumBackendStatSlots() -> i32 {
    init_small::globals::MaxBackends() + NUM_AUXILIARY_PROCS
}
...
pub fn pgstat_beinit() -> PgResult<()> {
    let procno = init_small::globals::MyProcNumber();
...
pub fn pgstat_bestart_initial() -> PgResult<()> {
    ...
    let my_proc_pid = init_small::globals::MyProcPid();
    let my_backend_type = init_small::globals::MyBackendType();
    let my_start_timestamp = init_small::globals::MyStartTimestamp();
    let qsize = pgstat_track_activity_query_size() as usize;
...
pub fn pgstat_bestart_security() -> PgResult<()> {
    ...
    let ssl = init_small::globals::WithMyProcPort(|port| -> PgResult<bool> {
        if !port.ssl_in_use {
            return Ok(false);
        }
        lsslstatus.ssl_bits = be_secure_seams::be_tls_get_cipher_bits::call(port);
        // SAFETY: the destination arrays are NAMEDATALEN bytes; strlcpy never
        // overruns and always NUL-terminates.
        unsafe {
            let v = be_secure_seams::be_tls_get_version::call(mcx, port)?;
            strlcpy(lsslstatus.ssl_version.as_mut_ptr(), v.as_str().as_bytes(), NAMEDATALEN);
            let c = be_secure_seams::be_tls_get_cipher::call(mcx, port)?;
            strlcpy(lsslstatus.ssl_cipher.as_mut_ptr(), c.as_str().as_bytes(), NAMEDATALEN);
            let dn = be_secure_seams::be_tls_get_peer_subject_name::call(mcx, port)?;
            strlcpy(lsslstatus.ssl_client_dn.as_mut_ptr(), dn.as_str().as_bytes(), NAMEDATALEN);
            let serial = be_secure_seams::be_tls_get_peer_serial::call(mcx, port)?;
            strlcpy(lsslstatus.ssl_client_serial.as_mut_ptr(), serial.as_str().as_bytes(), NAMEDATALEN);
            let issuer = be_secure_seams::be_tls_get_peer_issuer_name::call(mcx, port)?;
...
pub fn pgstat_bestart_final() -> PgResult<()> {
    ...
    let mybt = init_small::globals::MyBackendType();
    ...
    if pgstat_backend::pgstat_tracks_backend_bktype(mybt) {
        pgstat_backend::pgstat_create_backend(
...
pub fn pgstat_report_activity(state: BackendState, cmd_str: Option<&[u8]>) {
    ...
    let start_timestamp =
        transam_xact_seams::get_current_statement_start_timestamp::call();
    let mut len: usize = 0;
    ...
    let current_timestamp = timestamp_seams::get_current_timestamp::call();
    ...
    if (cur_state == STATE_RUNNING
        ...
        && state != cur_state
    {
        let (secs, usecs) = timestamp_seams::timestamp_difference::call(
            unsafe { (*beentry).st_state_start_timestamp },
            current_timestamp,
        );

        if cur_state == STATE_RUNNING || cur_state == STATE_FASTPATH {
            pgstat_database_seams::pgstat_count_conn_active_time::call(
                secs * 1_000_000 + usecs as i64,
            );
        } else {
            pgstat_database_seams::pgstat_count_conn_txn_idle_time::call(
                secs * 1_000_000 + usecs as i64,
            );
        }
    }
    ...
    for procNumber in 0..(n as ProcNumber) {
        ...
        if fields.st_procpid > 0 {
            let (xid, xmin, subxact_count, overflowed) =
                procarray_seams::proc_number_get_transaction_ids::call(
                    procNumber,
                );
...
pub fn pgstat_get_backend_current_activity(pid: i32, checkUser: bool) -> Vec<u8> {
    let beentry_base = BackendStatusArray.load(Ordering::Relaxed);
    let max_backends = init_small::globals::MaxBackends();
...
fn current_user_is_superuser() -> bool {
    ...
    miscinit_seams::superuser::call(ctx.mcx()).unwrap_or(false)
}
...
pub fn init_seams() {
    use status_seams as seams;

    // GUC variables owned by backend_status.c.
    guc_tables::vars::pgstat_track_activities.install(
        guc_tables::GucVarAccessors {
            get: pgstat_track_activities,
            set: set_pgstat_track_activities,
        },
    );
    guc_tables::vars::pgstat_track_activity_query_size.install(
        guc_tables::GucVarAccessors {
    ...
    pgstat_seams::pgstat_clear_backend_activity_snapshot::set(
    ...
    execParallel_support_seams::pgstat_report_activity_running::set(
    ...
    variable_seams::pgstat_report_appname::set(|newval| {
        pgstat_report_appname(newval.as_bytes());
    });

    postmaster_seams::pgstat_get_crashed_backend_activity::set(|pid| {
