...
fn convert_function_priv_string(priv_type: &str) -> PgResult<u64> {
    convert_any_priv_string(priv_type, FUNCTION_PRIV_MAP)
}

fn convert_function_name(fcinfo: &Fcinfo, i: usize) -> PgResult<Oid> {
    let funcname = arg_text_str(fcinfo, i)?;
    let oid = adt_regproc::regprocedurein(fcinfo.result_mcx(), funcname, None)?
        .unwrap_or(types_core::InvalidOid);
    if !types_core::OidIsValid(oid) {
        return Err(Box::new(
            PgError::error(format!("function \"{funcname}\" does not exist"))
                .with_sqlstate(types_error::ERRCODE_UNDEFINED_FUNCTION),
        ));
    }
    Ok(oid)
...
fn function_priv_check(roleid: Oid, functionoid: Oid, mode: u64) -> PgResult<Datum> {
...
// object_aclcheck_ext(ProcedureRelationId, ..) is_missing lane: NULL when the
// function OID has no pg_proc tuple (C SearchSysCache miss).
fn function_priv_check_ext(roleid: Oid, functionoid: Oid, mode: u64) -> PgResult<Datum> {
    if syscache_seams::lookup_pg_proc_shape::call(functionoid)?.is_none() {
        return Ok(Datum::null());
    }
    function_priv_check(roleid, functionoid, mode)
}

fn fc_has_function_privilege_name_name(
    _f: Option<&mut FmgrInfo>,
    fcinfo: &mut Fcinfo,
) -> PgResult<Datum> {
    let roleid = get_role_oid_or_public(arg_name_str(fcinfo, 0)?)?;
    let functionoid = convert_function_name(fcinfo, 1)?;
    let mode = convert_function_priv_string(arg_text_str(fcinfo, 2)?)?;
    function_priv_check(roleid, functionoid, mode)
}

fn fc_has_function_privilege_name(
    _f: Option<&mut FmgrInfo>,
    fcinfo: &mut Fcinfo,
) -> PgResult<Datum> {
    let roleid = miscinit_seams::get_user_id::call();
    let functionoid = convert_function_name(fcinfo, 0)?;
    let mode = convert_function_priv_string(arg_text_str(fcinfo, 1)?)?;
    function_priv_check(roleid, functionoid, mode)
}

fn fc_has_function_privilege_name_id(
    _f: Option<&mut FmgrInfo>,
    fcinfo: &mut Fcinfo,
) -> PgResult<Datum> {
    let roleid = get_role_oid_or_public(arg_name_str(fcinfo, 0)?)?;
    let functionoid = fcinfo.arg_oid(1);
    let mode = convert_function_priv_string(arg_text_str(fcinfo, 2)?)?;
    function_priv_check_ext(roleid, functionoid, mode)
}

fn fc_has_function_privilege_id(_f: Option<&mut FmgrInfo>, fcinfo: &mut Fcinfo) -> PgResult<Datum> {
    let roleid = miscinit_seams::get_user_id::call();
    let functionoid = fcinfo.arg_oid(0);
    let mode = convert_function_priv_string(arg_text_str(fcinfo, 1)?)?;
    function_priv_check_ext(roleid, functionoid, mode)
}

fn fc_has_function_privilege_id_name(
    _f: Option<&mut FmgrInfo>,
    fcinfo: &mut Fcinfo,
) -> PgResult<Datum> {
    let roleid = fcinfo.arg_oid(0);
...
fn fc_has_function_privilege_id_id(
    _f: Option<&mut FmgrInfo>,
    fcinfo: &mut Fcinfo,
) -> PgResult<Datum> {
    let roleid = fcinfo.arg_oid(0);
