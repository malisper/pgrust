...
fn authid_by_name_seam<'mcx>(mcx: Mcx<'mcx>, rolename: String) -> PgResult<Option<AuthIdForm>> {
    Ok(
        syscache_seams::lookup_authid_by_name::call(mcx, &rolename)?
            .as_ref()
...
fn authid_exists_by_name_seam(rolename: String) -> PgResult<bool> {
    ...
    let exists = syscache_seams::lookup_authid_by_name::call(
        scratch.mcx(),
        &rolename,
    )?
    .is_some();
...
fn authmem_by_keys_seam<'mcx>(
    ...
    Ok(
        syscache_seams::lookup_authmem_by_keys::call(
...
fn authmem_list_by_role_seam<'mcx>(mcx: Mcx<'mcx>, roleid: Oid) -> PgResult<Vec<AuthMemForm>> {
    let rows = syscache_seams::lookup_authmem_list_by_role::call(mcx, roleid)?;
