...
/// this model; fuzz witness p1-laney). ERANGE per glibc strtod (the
/// oracle platform of record): overflow to ±inf, or underflow from a
/// nonzero mantissa, tininess detected before rounding. No leading-whitespace skip (the caller's first-byte guard
/// already excludes it).
