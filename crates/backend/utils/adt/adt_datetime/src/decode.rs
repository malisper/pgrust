...
//! timezone engine (`localtime`).  All public decode entry
...
//!
//! Faithfulness notes:
//! * `pg_tm` is reused from `localtime`; datetime uses
...
pub(crate) fn GetCurrentTimeUsec(tm: &mut pg_tm, fsec: &mut fsec_t, tzp: Option<&mut i32>) {
    let cur_ts: i64 = transam_xact::GetCurrentTransactionStartTimestamp();
    let stz = session_timezone();
