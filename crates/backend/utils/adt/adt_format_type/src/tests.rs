...
fn install_mocks() {
    ...
    ONCE.call_once(|| {
        syscache_seams::type_form::set(mock_type_form);
        namespace_seams::type_is_visible::set(mock_type_is_visible);
        lsyscache_seams::get_namespace_name_or_temp::set(
            mock_namespace_name_or_temp,
        );
        ruleutils_seams::quote_qualified_identifier::set(
            mock_quote_qualified_identifier,
        );
        fmgr_seams::typmod_out::set(mock_typmod_out);
        mbutils_seams::pg_database_encoding_max_length::set(
            mock_database_encoding_max_length,
        );
        numeric_seams::numeric_maximum_size::set(mock_numeric_maximum_size);
    });
}
