...
fn install_hashfn() {
    HASHFN_INIT.call_once(|| {
        ...
        hashfn_seams::hash_bytes_uint32::set(|k: u32| k.wrapping_mul(2654435761));
        hashfn_seams::tag_hash::set(|key: &[u8], _sz: usize| {
            let mut h: u32 = 0;
            for &b in key {
...
fn install_catalog() {
    CATALOG_INIT.call_once(|| {
        ...
        jsonfuncs_seams::categorize_type::set(|_typoid| {
            Ok((Cat::JSONTYPE_NUMERIC, 42))
        });
        jsonfuncs_seams::output_function_call::set(|_mcx, _oid, val: &Datum| {
...
fn install_jsonapi() {
    JSONAPI_INIT.call_once(|| {
        ...
        jsonapi_seams::parse_validate::set(|json: &[u8]| {
            Ok(if json.is_empty() {
        ...
        jsonapi_seams::parse_validate_unique::set(|json: &[u8]| {
            Ok((
                if json.is_empty() { PErr::JSON_INVALID_TOKEN } else { PErr::JSON_SUCCESS },
                true,
            ))
        });
        jsonapi_seams::lex_first_token::set(|json: &[u8]| {
        ...
        jsonapi_seams::errsave_error::set(|_e, _json, _need_escapes, escontext| {
