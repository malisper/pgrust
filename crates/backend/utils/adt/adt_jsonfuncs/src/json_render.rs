...
/// onto `jsonb_seams::extract_variadic_array`.
pub fn extract_variadic_array<'mcx>(
    ...
    let v = detoast_seams::detoast_attr::call(mcx, array_image.as_ref_bytes())?;
...
/// `jsonb_seams::deconstruct_text_array_with_dims`.
...
pub fn deconstruct_array<'mcx>(mcx: Mcx<'mcx>, array: &Datum<'mcx>) -> PgResult<ArrayForJson<'mcx>> {
    ...
    let v = detoast_seams::detoast_attr::call(mcx, array.as_ref_bytes())?;
