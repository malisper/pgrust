...
fn jsonPathFromCstring<'mcx>(
    ...
    let jsonpath =
        jsonpath_gram_seams::parse::call(input, escontext.as_deref_mut())?;
...
fn escape_json_with_len(mcx: Mcx<'_>, buf: &mut PgVec<'_, u8>, str: &[u8]) -> PgResult<()> {
    ...
    json_seams::escape_json_with_len::call(&mut scratch, str)?;
        ...
        match jpi.typ {
            ...
            jpiVariable => {
                ...
                for (varname, varexpr) in cxt.varnames.iter().zip(cxt.varexprs.iter()) {
                    ...
                    let oid = nodeFuncs_seams::expr_type::call(varexpr.clone());
...
fn datetime_format_has_tz(template: &[u8]) -> PgResult<bool> {
    formatting::datetime_format_has_tz(template)
}
