...
//! [`network_seams`] (called here, installed by the unported
...
fn inet_port(endpoint: SessionEndpoint) -> PgResult<Option<i32>> {
    ...
    Ok(Some(numutils::pg_strtoint32(&s)?))
}
...
fn datum_get_inet_pp<'mcx>(
    ...
    let detoasted = detoast::pg_detoast_datum_packed(mcx, bytes)?;
...
/// `planner` slots in [`network_seams`] are OUTWARD seams it
