...
//! `datum::Datum` is a genuinely-sanctioned ABI edge (per the
...
//!   type is the bare-word `datum::Datum` *in the out-of-scope*
...
fn install_range_planner_support_seams() {
    ...
    rps::make_bool_const::set(nodes_core::makefuncs::make_bool_const);
    rps::make_andclause::set(|a, b| nodes_core::makefuncs::make_andclause(vec![a, b]));
    ...
    rps::make_const::set(|mcx, consttype, constcollid, constlen, constvalue, constbyval| {
        nodes_core::makefuncs::make_const(
    ...
    rps::contain_subplans::set(|node| {
        clauses::contain_subplans(Some(node))
    });

    // costsize.c cost estimation (the free-standing `&Expr` form).
    rps::cost_qual_eval_expr::set(costsize::qualcost::cost_qual_eval_expr);
    rps::cpu_operator_cost::set(costsize::cpu_operator_cost);

    // lsyscache.c catalog lookups.
    rps::get_opfamily_member::set(
        lsyscache::opfamily_operator::get_opfamily_member,
    );
    rps::get_typcollation::set(lsyscache::type_::get_typcollation);
    ...
    rps::range_opfamily::set(|rngtypid| {
        let pg_range = lsyscache::type_::lookup_pg_range(rngtypid)?
            .ok_or_else(|| {
                types_error::PgError::error(format!(
                    "cache lookup failed for range type {rngtypid}"
                ))
            })?;
        lsyscache::opclass::get_opclass_family(pg_range.rngsubopc)
    });
    ...
    clauses::support_simplify::register_support_simplify(
