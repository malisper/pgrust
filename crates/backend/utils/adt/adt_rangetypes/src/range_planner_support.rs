...
pub fn find_simplified_clause<'mcx>(
    ...
    // range-deserialize seam takes the bare-word `datum::Datum`, so lift the
    // pointer word across.
    let constvalue =
        datum::datum::Datum::from_usize(range_const.constvalue.as_byref_word());
    let range =
        rangetypes_seams::datum_get_range_type_p::call(mcx, constvalue)?;
    ...
    let rangetypcache =
        rangetypes_seams::range_get_typcache::call(rngtypid)?;
    ...
    let (lower, upper, empty) =
        rangetypes_seams::range_deserialize::call(&rangetypcache, range)?;
