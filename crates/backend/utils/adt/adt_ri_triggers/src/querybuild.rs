...
pub(crate) fn relation_namespace_name(mcx: Mcx<'_>, nspid: Oid) -> PgResult<Vec<u8>> {
    match lsyscache_seams::get_namespace_name::call(mcx, nspid)? {
...
pub(crate) fn ri_generate_qual(
    ...
    try_extend(mcx, buf, b" ")?;
    let clause = ruleutils_seams::generate_operator_clause::call(
        mcx,
        leftop,
        leftoptype,
        opoid,
...
pub(crate) fn ri_generate_qual_collation(
    ...
    let (nspname, collname) =
        match syscache_seams::collation_qualified_name::call(mcx, collation)? {
