...
pub fn uuid_recv(buffer: &mut StringInfo<'_>) -> PgResult<pg_uuid_t> {
    let bytes = pqformat::pq_getmsgbytes(buffer, UUID_LEN)?;
    let mut data = [0u8; UUID_LEN];
...
pub fn uuid_send<'mcx>(mcx: Mcx<'mcx>, uuid: &pg_uuid_t) -> PgResult<Bytea<'mcx>> {
    let mut buffer = pqformat::pq_begintypsend(mcx)?;
    pqformat::pq_sendbytes(&mut buffer, &uuid.data)?;
    Ok(pqformat::pq_endtypsend(buffer))
}
...
pub fn uuid_sortsupport<'mcx>(
    ...
    if abbreviate {
        ...
        let abbr_card = hyperloglog::initHyperLogLog(mcx, 10)?;
...
pub fn uuid_abbrev_abort(memtupcount: i32, uss: &mut uuid_sortsupport_state<'_>) -> bool {
    ...
    let abbr_card = hyperloglog::estimateHyperLogLog(&uss.abbr_card);
...
pub fn uuid_abbrev_convert(authoritative: &pg_uuid_t, uss: &mut uuid_sortsupport_state<'_>) -> usize {
    ...
    if uss.estimating {
        ...
        hyperloglog::addHyperLogLog(&mut uss.abbr_card, hash_bytes_uint32(tmp));
    }
