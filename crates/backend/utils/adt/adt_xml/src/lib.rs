...
pub fn init_seams() {
    xml_seams::escape_xml::set(escape_xml_seam);
    xml_seams::map_xml_name_to_sql_identifier::set(
...
pub fn xmlChar_to_encoding(encoding_name: &str) -> PgResult<i32> {
    let encoding = encnames_seams::pg_char_to_encoding::call(encoding_name);
    if encoding < 0 {
        return Err(
    ...
    if version_nondefault || (encoding != 0 && encoding != PG_UTF8) || standalone != -1 {
        ...
        if encoding != 0 && encoding != PG_UTF8 {
            ...
            buf.extend_from_slice(
                encnames_seams::pg_encoding_to_char::call(encoding).as_bytes(),
            );
            buf.push(b'"');
        }
...
pub fn map_sql_value_to_xml_value_v<'mcx>(
    ...
    let str =
        fmgr_seams::oid_output_function_call_datum::call(mcx, typeout, value)?;
...
fn deconstruct_xml_namespaces(
    ...
    let (ndim, dims, elems) =
        arrayfuncs_seams::deconstruct_text_array_with_dims::call(arr)?;
