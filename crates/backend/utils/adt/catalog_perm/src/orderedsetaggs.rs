...
pub(crate) type LiveSort = PgBox<'static, nodes::Tuplesortstate<'static>>;
...
pub(crate) fn with_sortstate_mut<R>(
    id: SortStateId,
    f: impl FnOnce(&mut nodes::Tuplesortstate<'static>) -> R,
) -> R {
    SORTSTATES.with(|t| {
        let mut t = t.borrow_mut();
        let slot = t
...
pub(crate) struct OSAPerGroupState {
    ...
    pub(crate) tupslot: Option<Box<nodes::tuptable::SlotData<'static>>>,
}
...
fn build_or_get_qstate(fcinfo: &mut FunctionCallInfoBaseData) -> OSAPerQueryState {
    ...
    if sortlist.len() != 1 || aggref.aggkind == parsenodes::AGGKIND_HYPOTHETICAL {
        raise(PgError::error(
    ...
    let tlbva = ok(lsyscache_seams::get_typlenbyvalalign::call(
        sort_col_type,
    ));
...
fn interval_lerp_cdatum(lo: CDatum<'_>, hi: CDatum<'_>, pct: f64) -> CDatum<'static> {
    ...
    let result = ok(timestamp_seams::interval_lerp::call(
        lo_iv, hi_iv, pct,
    ));
