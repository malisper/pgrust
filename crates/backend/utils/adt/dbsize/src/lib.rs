...
fn calculate_relation_size(
    ...
    let relationpath = relpath::relpathbackend(
...
pub fn pg_relation_size(rel_oid: Oid, fork_name: &str) -> PgResult<Option<i64>> {
    ...
    let forknum = relpath::forkname_to_number(fork_name)? as i32;
...
fn numeric_half_rounded<'mcx>(mcx: Mcx<'mcx>, n: &[u8]) -> PgResult<PgVec<'mcx, u8>> {
    ...
    let d = if adt_numeric::ops_sql::numeric_ge(n, &zero) {
        adt_numeric::ops_sql::numeric_add(mcx, n, &one)?
    } else {
        adt_numeric::ops_sql::numeric_sub(mcx, n, &one)?
    };

    adt_numeric::ops_sql::numeric_div_trunc(mcx, &d, &two)
}
...
fn numeric_truncated_divide<'mcx>(
    ...
    adt_numeric::ops_sql::numeric_div_trunc(mcx, n, &divisor_numeric)
}
...
fn int64_to_numeric<'mcx>(mcx: Mcx<'mcx>, value: i64) -> PgResult<PgVec<'mcx, u8>> {
    adt_numeric::convert::int64_to_numeric(mcx, value)
}
...
fn numeric_int8(mcx: Mcx<'_>, num: &[u8]) -> PgResult<i64> {
    ...
    let x = adt_numeric::convert::set_var_from_num(mcx, num)?;

    match adt_numeric::kernel_transcendental::numericvar_to_int64(&x)? {
...
pub fn pg_size_bytes(mcx: Mcx<'_>, arg: &[u8]) -> PgResult<i64> {
    ...
    let mut num: Vec<u8> = adt_numeric::io::numeric_in(mcx, number_str, -1)?.to_vec();
    ...
    if strptr < str_bytes.len() && str_bytes[strptr] != 0 {
        ...
        if multiplier > 1 {
            ...
            num = adt_numeric::ops_sql::numeric_mul(mcx, &mul_num, &num)?.to_vec();
        }
    }
...
pub fn pg_relation_filepath(relid: Oid) -> PgResult<Option<String>> {
    ...
    let path = relpath::relpathbackend(
        RelFileLocator { spcOid: spc_oid, dbOid: db_oid, relNumber: rel_number },
        backend,
...
fn project_open_relation(rel: &rel::Relation<'_>) -> OpenRelation {
    OpenRelation {
...
pub fn init_seams() {
    ...
    my_database_id::set(init_small::globals::MyDatabaseId);
    my_database_tablespace::set(init_small::globals::MyDatabaseTableSpace);
    get_user_id::set(|| Ok(miscinit::GetUserId()));
    has_privs_of_role::set(adt_acl::role_membership::has_privs_of_role);
    is_temp_or_temp_toast_namespace::set(
        catalog_namespace::isTempOrTempToastNamespace,
    );
    ...
    get_temp_namespace_proc_number::set(
        catalog_namespace::get_temp_namespace_proc_number_no_mcx,
    );
    proc_number_for_temp_relations::set(
        catalog_storage::proc_number_for_temp_relations,
    );
    check_for_interrupts::set(postgres::interrupt::check_for_interrupts);
    ...
    relation_get_index_list::set(relcache::derived::RelationGetIndexList);
    ...
    pg_class_form::set(|relid| {
        Ok(cache_syscache::pg_class_form_dbsize(relid)?.map(
    ...
    relation_map_oid_to_filenumber::set(|relid, relisshared| {
        Ok(relmapper::RelationMapOidToFilenumber(relid, relisshared))
    });
    ...
    relid_by_relfilenumber::set(|reltablespace, relfilenumber| {
        relfilenumbermap::RelidByRelfilenumber(
            reltablespace,
            relfilenumber,
    ...
    tablespace_exists::set(syscache_seams::tablespace_exists::call);
    database_exists::set(syscache_seams::database_exists::call);
    ...
    get_tablespace_oid::set(|name| {
        tablespace_seams::get_tablespace_oid::call(name, false)
    });
    get_database_oid::set(|name| {
        dbcommands_seams::get_database_oid::call(name, false)
    });
    ...
    object_aclcheck::set(|class_id, obj_id, role_id, mode| {
        Ok(aclchk_seams::object_aclcheck::call(class_id, obj_id, role_id, mode)?
...
fn aclcheck_error_provider(objtype: AclObjectType, obj_id: Oid) -> PgError {
    ...
    let (class_id, mode, obj_type, name) = match objtype {
        AclObjectType::Database => {
            let name = dbcommands_seams::get_database_name::call(mcx, obj_id)
                .ok()
                .flatten()
                .map(|s| s.to_string());
            (
                DatabaseRelationId,
                ACL_CONNECT,
                nodes::parsenodes::ObjectType::Database,
                name,
            )
        }
        AclObjectType::Tablespace => {
            let name = tablespace_seams::get_tablespace_name::call(mcx, obj_id)
                .ok()
                .flatten()
                .map(|s| s.to_string());
            (
                TableSpaceRelationId,
                ACL_CREATE,
                nodes::parsenodes::ObjectType::Tablespace,
                name,
            )
        }
    };
    ...
    let aclresult = match aclchk_seams::object_aclcheck::call(
