...
pub fn init_seams() {
    float_seams::erf::set(erf_seam);
    float_seams::erfc::set(erfc_seam);
    float_seams::tgamma::set(tgamma_seam);
    float_seams::lgamma::set(lgamma_seam);
}
...
mod tests {
    ...
    fn binds_and_matches_known_values() {
        init_seams();
        // erf(0) = 0, erfc(0) = 1.
        assert!((float_seams::erf::call(0.0) - 0.0).abs() < 1e-15);
        assert!((float_seams::erfc::call(0.0) - 1.0).abs() < 1e-15);
        // tgamma(5) = 4! = 24.
        assert!((float_seams::tgamma::call(5.0) - 24.0).abs() < 1e-10);
        // lgamma(1) = ln(0!) = 0.
        assert!((float_seams::lgamma::call(1.0) - 0.0).abs() < 1e-12);
        // lgamma(5) = ln(24).
        assert!(
            (float_seams::lgamma::call(5.0) - 24.0_f64.ln()).abs() < 1e-10
        );
    }
}
