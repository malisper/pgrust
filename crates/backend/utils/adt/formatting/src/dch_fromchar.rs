...
//! centralized `seams::formatting` slots:
...
//! (`localtime`, a separate sibling port). The idiomatic
//! surface does not name that pointer; instead the timezone seam returns an
//! owned [`types::formatting::TzHandle`] id, which the DCH
...
fn decode_timezone_abbrev_prefix(s: &[u8]) -> TzAbbrevMatch {
    adt_datetime::seam_impls::seam_decode_timezone_abbrev_prefix(s)
}
