...
//! [`jsonpath_exec_seams`]. The datetime substrate
...
fn jsonb_path_query_array_internal<'mcx>(
    ...
    JsonbValueToJsonb(mcx, &wrapItemsInArray(mcx, &found)?)
}
...
fn executeJsonPath<'mcx>(
    ...
    if !JsonbExtractScalar(mcx, root_container, &mut jbv)? {
...
fn executeLikeRegex<'mcx>(
    ...
    let matched = regexp_seams::RE_compile_and_execute::call(
        mcx,
...
fn JsonItemFromDatum<'mcx>(mcx: Mcx<'mcx>, var: &JsonPathVariable) -> PgResult<JsonbValue<'mcx>> {
    ...
    match typid {
        ...
        NUMERICOID => Ok(JsonbValue {
            ...
            typ: jbvType::jbvNumeric,
            val: JsonbValueData::Numeric(::mcx::slice_borrow_in(mcx, var_bytes()?)?),
        }),
        INT2OID => Ok(JsonbValue {
            ...
            val: JsonbValueData::Numeric(::mcx::slice_borrow_in(
                mcx,
                &int64_to_numeric_bytes(mcx, var.value.as_i16() as i64)?,
            )?),
        }),
        INT4OID => Ok(JsonbValue {
            ...
            val: JsonbValueData::Numeric(::mcx::slice_borrow_in(
                mcx,
                &int64_to_numeric_bytes(mcx, var.value.as_i32() as i64)?,
            )?),
        }),
        INT8OID => Ok(JsonbValue {
            ...
            val: JsonbValueData::Numeric(::mcx::slice_borrow_in(
                mcx,
                &int64_to_numeric_bytes(mcx, var.value.as_i64())?,
            )?),
        }),
        FLOAT4OID => Ok(JsonbValue {
            ...
            val: JsonbValueData::Numeric(::mcx::slice_borrow_in(
                mcx,
                &float8_to_numeric_bytes(mcx, var.value.as_f32() as f64)?,
            )?),
        }),
        FLOAT8OID => Ok(JsonbValue {
            ...
            val: JsonbValueData::Numeric(::mcx::slice_borrow_in(
                mcx,
                &float8_to_numeric_bytes(mcx, var.value.as_f64())?,
            )?),
        }),
        TEXTOID | VARCHAROID => {
        ...
        JSONOID => {
            ...
            let jb: &'mcx [u8] = ::mcx::slice_borrow_in(mcx, &jsonb_in_bytes(mcx, txt)?)?;
            let root = jsonb_root(jb);
            let mut res = JsonbValue::null();
            if json_container_is_scalar(container_header(root)) {
                JsonbExtractScalar(mcx, root, &mut res)?;
        ...
        _ => Err(ereport(ERROR)
            ...
            .errmsg(format!(
                ...
                format_type_seams::format_type_be::call(mcx, typid)?
                    .as_str()
            ))
            .into_error()),
    }
...
fn compareStrings(mcx: Mcx<'_>, mbstr1: &[u8], mbstr2: &[u8]) -> PgResult<i32> {
    ...
    let utf8str1 = mbutils_seams::pg_server_to_any::call(mcx, mbstr1, PG_UTF8)?;
    let utf8str2 = mbutils_seams::pg_server_to_any::call(mcx, mbstr2, PG_UTF8)?;
...
pub fn JsonPathQuery(
    ...
    if wrap {
        return Ok(JsonPathQueryResult {
            value: Some(JsonbValueToJsonb(mcx, &wrapItemsInArray(mcx, &found)?)?),
            empty: false,
            error: false,
        });
    }
...
#[derive(Clone, Debug)]
pub struct JsonPathValueResult<'mcx> {
    /// The single scalar `JsonbValue`, or `None` for SQL NULL / empty.
    pub value: Option<JsonbValue<'mcx>>,
    /// Whether no match was found (C `*empty`).
    pub empty: bool,
    /// Whether an error was suppressed (C `*error`).
    pub error: bool,
...
pub fn JsonPathValue<'mcx>(
    ...
    vars: &'mcx JsonPathVars,
    column_name: Option<&str>,
) -> PgResult<JsonPathValueResult<'mcx>> {
    let mut found = JsonValueList::default();
...
pub(crate) fn JsonValueListInitIteratorPub<'mcx>(
    jvl: &JsonValueList<'mcx>,
) -> JsonValueListIterator<'mcx> {
    JsonValueListInitIterator(jvl)
}

/// `crate`-visible alias of [`JsonValueListNext`].
pub(crate) fn JsonValueListNextPub<'mcx>(
    it: &mut JsonValueListIterator<'mcx>,
) -> Option<JsonbValue<'mcx>> {
...
pub(crate) fn GetJsonPathVarPub<'mcx>(
    mcx: Mcx<'mcx>,
    vars: &'mcx JsonPathVars,
    var_name: &[u8],
    base_object: &mut JsonbValue<'mcx>,
