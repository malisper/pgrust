...
fn fc_macaddr_send(fcinfo: &mut FunctionCallInfoBaseData) -> types_error::PgResult<Datum> {
    ...
    let mut buf = pqformat::pq_begintypsend(scratch.mcx())?;
    pqformat::pq_sendbytes(&mut buf, &wire)?;
...
mod tests {
    ...
    fn in_macaddr(s: &str) -> Vec<u8> {
        ...
        let native = fmgr_core::native_builtin(436).expect("macaddr_in registered");
    ...
    fn out_macaddr(image: &[u8]) -> String {
        ...
        let native = fmgr_core::native_builtin(437).expect("macaddr_out registered");
        native(&mut fcinfo).unwrap();
    ...
    fn call_cmp(oid: u32, a: &[u8], b: &[u8]) -> bool {
        ...
        let native = fmgr_core::native_builtin(oid).expect("op registered");
    ...
    fn byref_macaddr_bitwise_and_trunc() {
        ...
        let native = fmgr_core::native_builtin(753).unwrap();
        native(&mut fcinfo).unwrap();
    ...
    fn byref_hashmacaddr_through_registry() {
        ...
        let native = fmgr_core::native_builtin(399).unwrap();
