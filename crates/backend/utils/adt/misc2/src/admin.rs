...
use binary_upgrade_seams as binup;
use backend_common_path_seams as path;
use hba_seams as hba;
use fd_seams as fd;
...
fn read_text_file<'mcx>(
    ...
    match read_binary_file(mcx, filename, seek_offset, bytes_to_read, missing_ok)? {
        Some(buf) => {
            ...
            mbutils_seams::pg_verifymbstr::call(&buf[..], false)?;
            Ok(Some(buf))
        }
        None => Ok(None),
    }
...
pub fn pg_isolation_test_session_is_blocked<'mcx>(
    ...
    let wait_event_info = lmgr_proc_seams::proc_wait_event_info::call(blocked_procno);
    if let Some(t) = waitevent::pgstat_get_wait_event_type(wait_event_info) {
        if t == "InjectionPoint" {
            return Ok(true);
        }
    }
...
fn construct_int4_array<'mcx>(mcx: Mcx<'mcx>, pids: &[i32]) -> PgResult<Datum<'mcx>> {
    ...
    Ok(Datum::ByVal(
        arrayfuncs_seams::construct_int4_array::call(mcx, pids)?.as_usize(),
    ))
}
