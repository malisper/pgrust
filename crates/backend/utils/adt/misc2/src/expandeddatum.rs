...
/// is the [`eom_get_flat_size`](expanded_methods_seams::eom_get_flat_size)
/// seam — exactly mirroring the C indirection through `eoh_methods`.
///
/// This is the implementation installed into
/// `misc2_seams::eoh_get_flat_size` so external flatteners
...
pub fn eoh_get_flat_size(eoh: ExpandedObjectRef<'_>) -> PgResult<usize> {
    expanded_methods_seams::eom_get_flat_size::call(eoh)
}
...
/// as the [`eom_flatten_into`](expanded_methods_seams::eom_flatten_into)
/// seam.
///
/// Installed into `misc2_seams::eoh_flatten_into`.
pub fn eoh_flatten_into(eoh: ExpandedObjectRef<'_>, dest: &mut [u8]) -> PgResult<()> {
    expanded_methods_seams::eom_flatten_into::call(eoh, dest)
}
