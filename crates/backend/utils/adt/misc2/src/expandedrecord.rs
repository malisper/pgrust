...
fn assign_record_type_identifier(type_id: Oid, typmod: i32) -> PgResult<u64> {
    typcache_seams::assign_record_type_identifier::call(type_id, typmod)
}
...
fn assign_record_type_typmod(tupdesc: &mut TupleDescData<'_>) -> PgResult<()> {
    typcache_seams::assign_record_type_typmod::call(tupdesc)
}
...
fn detoast_external_attr<'mcx>(mcx: Mcx<'mcx>, attr: &[u8]) -> PgResult<PgVec<'mcx, u8>> {
    detoast_seams::detoast_external_attr::call(mcx, attr)
}
...
fn toast_flatten_tuple<'mcx>(
    ...
    heaptoast_seams::toast_flatten_tuple::call(mcx, tup, tupdesc)
}
...
fn system_attribute_by_name(fieldname: &str) -> Option<(i32, Oid, i32, Oid)> {
    heap_seams::system_attribute_by_name::call(fieldname)
}
...
fn type_is_not_composite(mcx: Mcx<'_>, type_oid: Oid) -> PgError {
    let name = match format_type_seams::format_type_be::call(mcx, type_oid) {
...
pub fn make_expanded_record_from_typeid<'mcx>(
    ...
    if type_id != RECORDOID {
        ...
        let typentry = typcache_seams::lookup_type_cache_expanded_record::call(
            mcx, type_id,
        )?;
...
pub fn make_expanded_record_from_tupdesc<'mcx>(
    ...
    if tupdesc.tdtypeid != RECORDOID {
        ...
        let typentry = typcache_seams::lookup_type_cache_tupdesc_view::call(
            mcx,
            tupdesc.tdtypeid,
        )?;
        let Some(td) = typentry.tup_desc else {
