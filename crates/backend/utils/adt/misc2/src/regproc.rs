...
pub fn regcollationin(
    ...
    if !OidIsValid(result) {
        let enc = mbutils_seams::get_database_encoding_name::call();
        return ereturn_oid(
            escontext,
...
pub fn regrolein(
    ...
    let result = acl_seams::get_role_oid::call(&names[0], true)?;
...
pub fn regroleout<'mcx>(mcx: Mcx<'mcx>, roleoid: Oid) -> PgResult<PgString<'mcx>> {
    ...
    match miscinit_seams::get_user_name_from_id::call(mcx, roleoid, true)? {
        Some(name) => {
...
pub fn stringToQualifiedNameList(
    ...
    let namelist = varlena_seams::split_identifier_string::call(mcx, string, '.')?;
