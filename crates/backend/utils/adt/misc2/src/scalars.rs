...
pub fn tidrecv<'mcx>(mcx: Mcx<'mcx>, buf: &[u8]) -> PgResult<Datum<'mcx>> {
    ...
    let offset_number = pqformat::pq_getmsgint(&mut msg, 2)? as OffsetNumber;
    let result = ItemPointer::set(block_number, offset_number);
    unported::return_itempointer(mcx, result)
}
...
pub fn tidsend<'mcx>(mcx: Mcx<'mcx>, item_ptr: Datum) -> PgResult<Datum<'mcx>> {
    ...
    pqformat::pq_sendint32(&mut buf, item_ptr.block_number_no_check())?;
    pqformat::pq_sendint16(&mut buf, item_ptr.offset_number_no_check())?;
...
fn currtid_internal<'mcx>(
    ...
    if aclresult != types_acl::acl::ACLCHECK_OK {
        aclchk_seams::aclcheck_error::call(
            aclresult,
            objectaddress_seams::get_relkind_objtype::call(relkind),
    ...
    if !has_storage {
        let nspname = lsyscache_seams::get_namespace_name::call(
            mcx,
    ...
    let mut scan = table_tableam::table_beginscan_tid(
        mcx,
        rel,
        Some(snapshot.clone()),
    )?;
    table_tableam::table_tuple_get_latest_tid(mcx, &mut scan, &mut result)?;
...
fn currtid_for_view<'mcx>(
    ...
    let rulelock = relcache_seams::relation_rules::call(mcx, viewrel.rd_id)?;
    ...
    for rewrite in rulelock.rules.iter() {
        if rewrite.event == nodes::nodes::CmdType::CMD_SELECT {
            ...
            let tle = parser_relation::get_tle_by_resno(
                &query.targetList,
                (tididx + 1) as i16,
            );
            if let Some(tle) = tle {
                if let Some(expr) = tle.expr.as_deref() {
                    if let nodes::primnodes::Expr::Var(var) = expr {
...
pub fn currtid_byrelname<'mcx>(
    ...
    let parts =
        varlena_seams::text_to_qualified_name_list::call(mcx, relname.as_bytes())?;
    let parts: alloc::vec::Vec<&str> = parts.iter().map(|p| p.as_str()).collect();
    let relrv = namespace_seams::make_range_var_from_name_list::call(&parts)?;

    // rel = table_openrv(relrv, AccessShareLock);
    let rel =
        table::table_openrv(mcx, &relrv, types_storage::lock::AccessShareLock)?;
    ...
    table::table_close(rel, types_storage::lock::AccessShareLock)?;
