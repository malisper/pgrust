...
//!
//! The PRNG itself is the in-process [`prng`] crate: the C primitives map
...
//! exactly as `pg_prng_int64_range` in `common/prng.c`).
//!
//! `numeric_random` delegates to `random_numeric` in the sibling
//! [`adt_numeric`] crate, threading this crate's shared
...
//! in-repo [`prng`] crate provides no strong-seed (OS-entropy) primitive, so
...
/// above.
pub fn init_seams() {
    variable_seams::setseed::set(setseed);
