...
fn expr_type(expr: &Expr) -> PgResult<Oid> {
    Ok(nodeFuncs_seams::expr_type_info::call(expr)?.typid)
}

/// `exprTypmod((const Node *) node)` over an owned `Expr`.
fn expr_typmod(expr: &Expr) -> PgResult<i32> {
    Ok(nodeFuncs_seams::expr_type_info::call(expr)?.typmod)
}
...
pub(crate) fn expr_collation_of_node(node: &Node<'_>) -> PgResult<Oid> {
    match node.as_expr() {
        Some(e) => Ok(nodeFuncs_seams::expr_type_info::call(e)?.collation),
        None => Ok(Oid::default()),
    }
}
...
fn format_type_with_typemod<'mcx>(
    ...
    match format_type_seams::format_type_extended::call(
        mcx,
        type_oid,
        typemod,
        format_type_seams::FORMAT_TYPE_TYPEMOD_GIVEN,
    )? {
        Some(s) => Ok(s),
...
fn generate_operator_name<'mcx>(
    ...
    ruleutils_seams::generate_operator_name::call(mcx, operid, arg1, arg2)
}
...
fn generate_collation_name<'mcx>(mcx: Mcx<'mcx>, collid: Oid) -> PgResult<PgString<'mcx>> {
    ruleutils_seams::generate_collation_name::call(mcx, collid)
}
        ...
        if colexpr.op == JSON_EXISTS_OP {
            ...
            if colexpr.op == JSON_QUERY_OP {
                let (typcategory, _typispreferred) =
                    lsyscache_seams::get_type_category_preferred::call(typid)?;
...
fn get_xmltable(
    ...
    if let Some(colexprs) = tf.colexprs.as_ref().filter(|v| !v.is_empty()) {
        ...
        for colnum in 0..colexprs.len() {
            ...
            let notnull = nodes_core_seams::bms_is_member::call(
                colnum as i32,
    ...
    match expr {
        ...
        Expr::RowExpr(rowexpr) => {
            ...
            let tupdesc = if rowexpr.row_typeid != RECORDOID {
                let td = typcache_seams::lookup_rowtype_tupdesc::call(
                    mcx,
                    rowexpr.row_typeid,
                    -1,
                )?;
        ...
        Expr::NullTest(nt) => {
            ...
            if nt.argisrow || !lsyscache_seams::type_is_rowtype::call(expr_type(arg)?)? {
        ...
        Expr::XmlExpr(xexpr) => {
            ...
            if let Some(name) = xexpr.name.as_deref() {
                let mapped =
                    xml_seams::map_xml_name_to_sql_identifier::call(mcx, name)?;
            ...
            if !xexpr.named_args.is_empty() {
                ...
                for (e, argname) in xexpr.named_args.iter().zip(xexpr.arg_names.iter()) {
                    ...
                    let mapped = xml_seams::map_xml_name_to_sql_identifier::call(
                        mcx, argname,
                    )?;
                    let q = quote_identifier(mcx, mapped.as_str())?;
                    str_(context, " AS ")?;
        ...
        Expr::InferenceElem(iexpr) => {
            ...
            if oid_is_valid(iexpr.inferopclass) {
                ...
                let inferopcinputtype =
                    lsyscache_seams::get_opclass_input_type::call(
                        iexpr.inferopclass,
                    )?;
...
fn get_scalararrayop_expr(
    ...
    let opname = generate_operator_name(
        ...
        s.opno,
        expr_type(arg1)?,
        lsyscache_seams::get_base_element_type::call(expr_type(arg2)?)?,
    )?;
    str_(context, " ")?;
    str_(context, opname.as_str())?;
...
fn get_func_expr_inner(
        ...
        || funcformat == CoercionForm::COERCE_IMPLICIT_CAST
    {
        ...
        let coerced_typmod = match enclosing {
            Some(e) => {
                nodeFuncs_seams::expr_is_length_coercion::call(e)?.1
            }
            None => {
                ...
                nodeFuncs_seams::expr_is_length_coercion::call(&f_expr)?.1
            }
        };
    ...
    let (funcname, use_variadic) = ruleutils_seams::generate_function_name::call(
        mcx,
        f.funcid,
        args.len() as i32,
        argnames,
...
fn text_const_cstring<'mcx>(
    ...
    let (typoutput, _typ_is_varlena) =
        lsyscache_seams::get_type_output_info::call(con.consttype)?;
    let datum = con.constvalue.clone_in(mcx)?;
    fmgr_seams::oid_output_function_call_datum::call(mcx, typoutput, datum)
}
...
fn get_agg_expr_helper(
    ...
    let funcname_str: &str = if let Some(fname) = funcname {
        ...
        let (name, uv) = ruleutils_seams::generate_function_name::call(
            mcx,
            a.aggfnoid,
            nargs,
            PgVec::new_in(mcx),
...
fn get_windowfunc_expr_helper(
    ...
    let funcname_str: &str = if let Some(fname) = funcname {
        ...
        let (name, _uv) = ruleutils_seams::generate_function_name::call(
            mcx,
            w.winfnoid,
            argtypes.len() as i32,
            argnames,
    ...
    for cw in c.args.iter() {
        ...
        if c.arg.is_some() {
            ...
            if let Expr::OpExpr(op) = w {
                if op.args.len() == 2
                    && matches!(
                        nodes_core::nodefuncs::strip_implicit_coercions(&op.args[0]),
                        Expr::CaseTestExpr(_)
                    )
                {
                    w = &op.args[1];
...
fn get_const_expr_inner(
    ...
    let (typoutput, _typ_is_varlena) =
        lsyscache_seams::get_type_output_info::call(consttype)?;
    let datum = c.constvalue.clone_in(mcx)?;
    let extval = fmgr_seams::oid_output_function_call_datum::call(mcx, typoutput, datum)?;
...
fn get_const_collation(c: &Const, context: &mut DeparseContext<'_>) -> PgResult<()> {
    ...
    if oid_is_valid(c.constcollid) {
        let typcollation = lsyscache_seams::get_typcollation::call(c.consttype)?;
...
fn simple_quote_literal(context: &mut DeparseContext<'_>, val: &str) -> PgResult<()> {
    let scs = guc_seams::standard_conforming_strings::call();
    ...
    if var.varno == OUTER_VAR && !context.namespaces[dpns_idx].outer_tlist.is_empty() {
        ...
        };
        {
            ...
            if let Some(ar) = apprelids {
                let unioned = nodes_core_seams::bms_union::call(
                    mcx,
...
pub fn get_variable<'mcx>(
    ...
    let attname: Option<PgString> = if attnum == InvalidAttrNumber {
        ...
        Some(ruleutils_seams::get_rte_attribute_name::call(mcx, &rte_clone, attnum)?)
    };
...
fn get_name_for_var_field<'mcx>(
    ...
    let v = match var {
        ...
        _ => {
            ...
            let tupdesc =
                funcapi_seams::get_expr_result_tupdesc::call(mcx, Some(&node), false)?;
    ...
    if attnum == InvalidAttrNumber {
        let rte_clone = context.namespaces[dpns_idx].rtable[(varno - 1) as usize].clone_in(mcx)?;
        return ruleutils_seams::get_rte_attribute_name::call(mcx, &rte_clone, fieldno as i16);
    }
        ...
        if let Some(cn) = colname {
            if cn == attname {
                ...
                let equal = match tle_expr {
                    Some(e) => nodeFuncs_seams::equal::call(&var_expr, e),
                    None => false,
                };
                if !equal {
                    return Ok(true);
...
fn op_is_first_arg(op: &OpExpr, parent_op: &OpExpr) -> bool {
    match parent_op.args.first() {
        Some(first) => nodeFuncs_seams::equal::call(&Expr::OpExpr(op.clone()), first),
        None => false,
    }
}
...
fn get_simple_binary_op_name(op: &OpExpr) -> Option<u8> {
    ...
    match lsyscache_seams::get_op_name_single_byte::call(op.opno) {
        Ok(b) => b,
        Err(_) => None,
    }
}
