...
fn get_attname<'mcx>(mcx: Mcx<'mcx>, relid: Oid, attnum: i16) -> PgResult<PgString<'mcx>> {
    match lsyscache_seams::get_attname::call(mcx, relid, attnum, false)? {
        Some(s) => Ok(s),
...
fn generate_relation_name<'mcx>(
    ...
    ruleutils_seams::generate_relation_name::call(mcx, relid, need_qual)
}
...
pub fn get_query_def<'mcx>(
    ...
    wrap_column: i32,
    start_indent: i32,
) -> PgResult<stringinfo::StringInfo<'mcx>> {
    ...
    ruleutils_seams::acquire_rewrite_locks::call(mcx, &mut query, false, false)?;
...
fn get_target_list<'mcx>(
    ...
    for tle in target_list.iter() {
        ...
        let saved = core::mem::replace(&mut context.buf, stringinfo::StringInfo::new_in(mcx));
...
pub fn get_rule_orderby<'mcx>(
    ...
    for srt in order_list.iter() {
        ...
        let sortcoltype = match sortexpr.as_ref() {
            Some(e) => nodeFuncs_seams::expr_type_info::call(e)?.typid,
            None => Oid::default(),
        };
        let (lt_opr, gt_opr) =
            ruleutils_seams::lookup_type_cache_lt_gt_opr::call(sortcoltype)?;
...
fn get_insert_query_def<'mcx>(
    ...
    if let Some(confl) = query.onConflict.as_deref() {
        ...
        if !confl.arbiterElems.is_empty() {
            ...
            let name = lsyscache_seams::get_constraint_name::call(mcx, confl.constraint)?
...
fn get_from_clause_coldeflist<'mcx>(
    ...
    for i in 0..n {
        ...
        if attcollation != 0 {
            let typcoll = lsyscache_seams::get_typcollation::call(atttypid)?;
    ...
    loop {
        ...
        match e {
            Expr::FieldStore(fstore) => {
                ...
                let typrelid = lsyscache_seams::get_typ_typrelid::call(
                    fstore.resulttype,
                )?;
                if typrelid == 0 {
                    let ty = format_type_seams::format_type_be::call(
                        mcx,
                        fstore.resulttype,
                    )?;
                    return Err(elog_error(format!(
...
fn expr_is_multiexpr_param(expr: Option<&Expr>) -> bool {
    ...
    let stripped = nodes_core::nodefuncs::strip_implicit_coercions(cur);
