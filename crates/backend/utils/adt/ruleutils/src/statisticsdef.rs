...
pub fn pg_get_statisticsobj_worker<'mcx>(
    ...
    let fields =
        syscache_seams::statext_objdef_fields::call(mcx, statextid)?;
    ...
    let exprs = match &stxexprs_text {
        Some(s) => Some(read_seams::string_to_node::call(mcx, s.as_str())?),
        None => None,
    };
    ...
    if !columns_only {
        ...
        let nsp =
            lsyscache_seams::get_namespace_name_or_temp::call(mcx, stxnamespace)?;
    ...
    for &attnum in stxkeys.iter() {
        ...
        let q = ruleutils_seams::quote_identifier::call(mcx, attname.as_str())?;
    ...
    //                               statextrec->stxrelid);
    let relname = lsyscache_seams::get_rel_name::call(mcx, stxrelid)?
...
pub fn pg_get_statisticsobjdef_expressions<'mcx>(
    ...
    let exprs_node = match &stxexprs_text {
        Some(s) => read_seams::string_to_node::call(mcx, s.as_str())?,
        None => return Ok(None),
    };
