...
pub fn datum_image_hash_bytes(
    ...
    if typ_byval {
        ...
        Ok(hashfn::hash_bytes(&d.to_ne_bytes()))
    } else if typ_len > 0 {
        let b = value.as_ref_bytes();
        Ok(hashfn::hash_bytes(&b[..typ_len as usize]))
    } else if typ_len == -1 {
...
pub fn init_seams() {
    ...
    scalar_seams::datum_copy::set(datum_copy);

    // Migration-target value-enum (`*_v`) lane.
    datum_seams::datum_copy_v::set(datum_copy_v);
    datum_seams::datum_estimate_space_v::set(datum_estimate_space_v);
    datum_seams::datum_serialize_v::set(datum_serialize_v);
    datum_seams::datum_image_hash_v::set(datum_image_hash_v);
    datum_seams::datum_image_eq_v::set(datum_image_eq_v);
    ...
    datum_seams::datum_serialize::set(datum_serialize);
    datum_seams::datum_restore::set(datum_restore);
