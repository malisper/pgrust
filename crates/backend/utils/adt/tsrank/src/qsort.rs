...
//! algorithm is a parity requirement here: SortAndUniqItems dedups
//! same-lexeme QueryOperands keyed ONLY on the operand string, so WHICH
//! operand survives a tie is decided by pg_qsort's equal-key output order —
//! and the survivor's weight/prefix flags change the ts_rank scalar
//! (tsrank_diff DIVERGENCE-2, 2026-07-31).
