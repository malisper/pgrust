...
pub fn ts_phrase_execute<'mcx>(
    ...
    // C tsvector_op.c TS_phrase_execute(): check_stack_depth() + CHECK_FOR_INTERRUPTS()
    // (one frame per tree level; phrase matching can burn CPU for a long time,
    // so without the CFI the match is uncancellable by statement_timeout).
...
fn ts_execute_recurse<'mcx>(
    ...
    // C tsvector_op.c TS_execute_recurse(): check_stack_depth() + CHECK_FOR_INTERRUPTS()
    // (one frame per tree level; phrase matching can burn CPU for a long time,
    // so without the CFI the match is uncancellable by statement_timeout).
    ::stack_depth::check_stack_depth()?;
    ::postgres_seams::check_for_interrupts::call()?;
