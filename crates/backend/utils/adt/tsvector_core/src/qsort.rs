//! lib/sort_template.h (pg_qsort) over Copy elements — verbatim port copy
//! (crates/backend/access/gist/gistproc/src/qsort.rs convention). The exact
//! algorithm is a parity requirement here: uniquePos breaks at a 16383
//! position / MAXNUMPOS immediately after advancing, so the kept weight at
//! the break is decided by qsort's equal-position output order; uniqueentry
//! merges duplicate-lexeme position lists in tie order (feeding that break
//! path); tsvectorrecv's needSort orders duplicate-lexeme entries by tie
//! (tsrank_diff DIVERGENCE-2 + tsvector_core re-floor divergence, 2026-07-31).
