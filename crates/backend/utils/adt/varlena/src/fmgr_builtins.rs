...
fn detoast_varlena_args(fcinfo: &mut FunctionCallInfoBaseData) {
    ...
    for i in 0..n {
        ...
        let detoasted: Vec<u8> = {
            ...
            match detoast_seams::detoast_attr::call(m.mcx(), image) {
...
fn arg_array_datum<'mcx>(
    ...
    i: usize,
) -> types_error::PgResult<types_tuple::heaptuple::Datum<'mcx>> {
    let bytes = arg_array_image(fcinfo, i);
    let v = mcx::slice_in(mcx, bytes)?;
    Ok(types_tuple::heaptuple::Datum::ByRef(v))
}
...
fn format_arg<'mcx>(mcx: mcx::Mcx<'mcx>, fcinfo: &FunctionCallInfoBaseData, i: usize) -> types_error::PgResult<FormatArg<'mcx>> {
    ...
    let typid = fmgr_core::get_fn_expr_argtype(fcinfo.flinfo.as_deref(), i as i32);
...
fn variadic_array<'mcx>(
    ...
    if !fmgr_core::get_fn_expr_variadic(fcinfo.flinfo.as_deref()) {
        return Ok(None);
    }
    ...
    let arrtype = fmgr_core::get_fn_expr_argtype(fcinfo.flinfo.as_deref(), argidx as i32);
    let elemtype = lsyscache_seams::get_base_element_type::call(arrtype)?;
...
fn fc_text_format(fcinfo: &mut FunctionCallInfoBaseData) -> types_error::PgResult<Datum> {
    ...
    let result: Option<Vec<u8>> = if fmgr_core::get_fn_expr_variadic(fcinfo.flinfo.as_deref())
        && fcinfo.nargs() == 2
    {
        ...
        if fcinfo.arg(1).map(|d| d.isnull).unwrap_or(true) {
            ...
            let arrtype = fmgr_core::get_fn_expr_argtype(fcinfo.flinfo.as_deref(), 1);
            let elemtype = (lsyscache_seams::get_base_element_type::call(arrtype))?;
...
fn pg_column_arg<'mcx>(
    ...
    let argtypeid = fmgr_core::get_fn_expr_argtype(fcinfo.flinfo.as_deref(), 0);
    let typlen = lsyscache_seams::get_typlen::call(argtypeid)?;
    if typlen == 0 {
...
/// `FIN_TRADITIONAL_CRC32` (^0xFFFFFFFF). `crc32c::traditional_crc32`
...
fn fc_crc32_bytea(fcinfo: &mut FunctionCallInfoBaseData) -> types_error::PgResult<Datum> {
    detoast_varlena_args(fcinfo);
    let crc = crc32c::traditional_crc32(arg_bytes(fcinfo, 0));
...
fn fc_crc32c_bytea(fcinfo: &mut FunctionCallInfoBaseData) -> types_error::PgResult<Datum> {
    ...
    crc = crc32c::pg_comp_crc32c_sb8(crc, arg_bytes(fcinfo, 0));
    crc ^= 0xFFFF_FFFF;
    Ok(ret_i64(crc as u64 as i64))
}
...
mod tests {
    ...
    fn call_varlena_result(
        ...
        fmgr_core::fmgr_isbuiltin(oid).expect("builtin registered");
    ...
    fn byref_unknown_io_round_trip_through_registry() {
        ...
        fmgr_core::fmgr_isbuiltin(109).expect("unknownin registered");
        ...
        fmgr_core::fmgr_isbuiltin(110).expect("unknownout registered");
