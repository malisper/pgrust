...
pub fn xidin(s: &str, escontext: Option<&mut SoftErrorContext>) -> PgResult<TransactionId> {
    let (result, _rest) = numutils::uint32in_subr(s, false, "xid", escontext)?;
...
pub fn xidrecv(buf: &mut stringinfo::StringInfo<'_>) -> PgResult<TransactionId> {
    let v = pqformat::pq_getmsgint(buf, core::mem::size_of::<TransactionId>() as i32)?;
...
pub fn xid_age(xid: TransactionId) -> PgResult<i32> {
    let now = transam_xact_seams::get_stable_latest_transaction_id::call()?;
...
pub fn mxid_age(xid: TransactionId) -> PgResult<i32> {
    let now = multixact_seams::read_next_multixact_id::call()?;
...
pub fn xid8in(s: &str, escontext: Option<&mut SoftErrorContext>) -> PgResult<FullTransactionId> {
    let (result, _rest) = numutils::uint64in_subr(s, false, "xid8", escontext)?;
...
pub fn xid8recv(buf: &mut stringinfo::StringInfo<'_>) -> PgResult<FullTransactionId> {
    let value = pqformat::pq_getmsgint64(buf)? as u64;
...
pub fn xid8send<'mcx>(mcx: mcx::Mcx<'mcx>, arg1: FullTransactionId) -> PgResult<Bytea<'mcx>> {
    let mut buf = pqformat::pq_begintypsend(mcx)?;
    pqformat::pq_sendint64(&mut buf, U64FromFullTransactionId(arg1))?;
    Ok(pqformat::pq_endtypsend(buf))
}
...
pub fn cidin(s: &str, escontext: Option<&mut SoftErrorContext>) -> PgResult<CommandId> {
    let (result, _rest) = numutils::uint32in_subr(s, false, "cid", escontext)?;
...
pub fn cidrecv(buf: &mut stringinfo::StringInfo<'_>) -> PgResult<CommandId> {
    let v = pqformat::pq_getmsgint(buf, core::mem::size_of::<CommandId>() as i32)?;
    Ok(v as CommandId)
}
