...
// (`cache::catcache::{CatCacheData,CatCTup,CatCList}.
...
pub fn namehashfast(key: &[u8]) -> u32 {
    ...
    // until it lands).
    hashfn_seams::hash_bytes::call(&key[..len])
}
...
pub fn texthashfast(payload: &[u8]) -> u32 {
    ...
    hashfn_seams::hash_bytes::call(payload)
}
...
pub fn oidvectorhashfast(v: &[Oid]) -> PgResult<u32> {
    ...
    Ok(hashfn_seams::hash_bytes::call(&bytes))
}
...
pub fn item_pointer_equals(
    a: cache::catcache::ItemPointer,
    b: cache::catcache::ItemPointer,
) -> bool {
    a == b
}
