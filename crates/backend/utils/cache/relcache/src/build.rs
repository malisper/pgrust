...
/// [`rel::RelationData`] value-slice, copied into `mcx` (the C "copy the
...
/// Build the cross-unit [`rel::RelationData`] slice from the owned entry.
...
fn project_entry<'mcx>(
    ...
    let rd_index = r.rd_index.as_ref().map(|ix| rel::FormData_pg_index {
...
/// cross-unit value-slice (`rel::RelationData.rd_trigdesc`). Mirrors
...
fn project_form_pg_class<'mcx>(
    mcx: Mcx<'mcx>,
    f: &FormPgClass,
) -> PgResult<rel::FormData_pg_class<'mcx>> {
    Ok(rel::FormData_pg_class {
    ...
    let mut relation: Box<RelationData> = loop {
        ...
        match relation.rd_rel.relpersistence {
            ...
            PERSIST_TEMP => {
                ...
                if namespace_seams::is_temp_or_temp_toast_namespace::call(
                    relnamespace,
                )? {
                    ...
                    relation.rd_backend =
                        namespace_seams::get_temp_namespace_proc_number::call(
                            relnamespace,
                        )?;
...
pub fn ScanPgRelation(
    ...
    let scanned = match genam_seams::scan_pg_class::call(
        targetRelId,
        indexOK,
        force_non_historic,
    )? {
...
pub fn RelationBuildTupleDesc(relation: &mut RelationData) -> PgResult<()> {
    ...
    let mut constr = relcache_entry::OwnedTupleConstr::default();
    let mut ndef: i32 = 0;
    ...
    let rows =
        genam_seams::scan_pg_attribute::call(relid, natts as i16)?;
    ...
    let mut attrmiss: Option<Vec<relcache_entry::OwnedAttrMissing>> = None;
        ...
        if attp.atthasmissing {
            if let Some(image) = &attp.attmissingval {
                let arr = attrmiss
                    .get_or_insert_with(|| {
                        vec![
                            relcache_entry::OwnedAttrMissing::default();
                            natts as usize
                        ]
                    });
                arr[(attnum - 1) as usize] = relcache_entry::OwnedAttrMissing {
                    am_present: true,
...
pub fn RelationParseRelOptions(
    ...
    // precisely.
    relation.rd_options =
        reloptions_seams::extract_rel_options::call(
...
pub fn formrdesc(
    ...
    isshared: bool,
    natts: i32,
    schema: &relcache_entry::BootstrapCatalogSchema,
) -> PgResult<Oid> {
