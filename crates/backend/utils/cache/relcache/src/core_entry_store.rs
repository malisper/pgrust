...
pub fn RelationIncrementReferenceCount(rel: Oid) -> PgResult<()> {
    relcache_seams::resource_owner_enlarge_relation::call()?;
    with_relation_mut(rel, |rd| rd.rd_refcnt += 1)?;
    if !miscinit_seams::is_bootstrap_processing_mode::call() {
        relcache_seams::resource_owner_remember_relation::call(rel);
    }
    Ok(())
}
...
pub fn RelationDecrementReferenceCount(rel: Oid) -> PgResult<()> {
    ...
    if !miscinit_seams::is_bootstrap_processing_mode::call() {
        relcache_seams::resource_owner_forget_relation::call(rel);
    }
    Ok(())
}
...
pub(crate) fn set_partkey(
    ...
    let owned = OwnedPartitionKey {
        ...
        partexprs: {
            ...
            let mut v: Vec<nodes::primnodes::Expr<'static>> =
...
pub(crate) fn set_partcheck(
    relid: Oid,
    partcheck: &mcx::PgVec<'_, nodes::nodes::Node<'_>>,
) -> PgResult<()> {
    let cache_mcx = crate::derived::cache_memory_context();
    let mut owned: Vec<nodes::primnodes::Expr<'static>> = Vec::with_capacity(partcheck.len());
...
pub(crate) fn get_partcheck<'mcx>(
    mcx: mcx::Mcx<'mcx>,
    relid: Oid,
) -> PgResult<(bool, mcx::PgVec<'mcx, nodes::nodes::Node<'mcx>>)> {
    with_state(|st| {
        let mut out = mcx::PgVec::new_in(mcx);
        match st.partcheck.get(&relid) {
            Some((valid, exprs)) => {
                for e in exprs {
                    ...
                    out.push(nodes::nodes::Node::mk_expr(mcx, e.clone_in(mcx)?)?);
                }
                Ok((*valid, out))
            }
            None => Ok((false, out)),
