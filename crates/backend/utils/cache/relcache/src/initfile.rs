...
pub fn RelationSetNewRelfilenumber(relation: Oid, persistence: i8) -> PgResult<()> {
    ...
    let is_binary_upgrade = binary_upgrade_seams::is_binary_upgrade::call();
    ...
    let newrelfilenumber: Oid = if !is_binary_upgrade {
        ...
        catalog_seams::get_new_relfilenumber::call(reltablespace, persistence)?
    } else if relkind == RELKIND_INDEX as i8 {
        // C: binary_upgrade_next_index_pg_class_relfilenumber (consume).
        let n = binary_upgrade_seams::consume_next_relfilenumber::call(true);
        if n == InvalidOid {
            return Err(ereport(ERROR)
        ...
        let n = binary_upgrade_seams::consume_next_relfilenumber::call(false);
        if n == InvalidOid {
            return Err(ereport(ERROR)
                .errcode(ERRCODE_INVALID_PARAMETER_VALUE)
    ...
    if is_binary_upgrade {
        ...
        catalog_storage_seams::smgr_unlink_relation_now::call(rd_locator, rd_backend)?;
    } else {
        // C: RelationDropStorage(relation);
        catalog_storage_seams::relation_drop_storage::call(rd_locator, rd_backend)?;
    }
    ...
    if relkind_has_table_am(relkind) {
        ...
        let (fx, mm) = table_tableam_seams::table_relation_set_new_filelocator::call(
            &rel,
            newrlocator,
            persistence,
        )?;
        ...
        //    smgrclose(srel);
        catalog_storage_seams::relation_create_storage_main_fork::call(
            newrlocator,
            persistence,
        )?;
    } else {
    ...
    if is_mapped {
        ...
        transam_xact_seams::get_current_transaction_id::call()?;
        ...
        // catalog owner's.
        catalog_storage_seams::update_pg_class_relfilenumber::call(
            relid,
            newrelfilenumber,
            persistence,
            relkind,
...
mod xunit {
    ...
    pub(super) fn database_path() -> String {
        ...
        // `try_database_path`.
        miscinit_seams::get_database_path::call()
    ...
    pub(super) fn RelationSupportsSysCache(relid: Oid) -> bool {
        ...
        syscache_seams::relation_supports_syscache::call(relid)
    }
    pub(super) fn SearchSysCacheRelOid(relid: Oid) -> PgResult<FormPgClass> {
        ...
        let form = syscache_seams::search_pg_class_full_form::call(
            scratch.mcx(),
            relid,
        )?;
        match form {
    ...
    pub(super) fn LockRelationOid(oid: Oid) -> PgResult<()> {
        ...
        let guard =
            lmgr_seams::lock_relation_oid::call(oid, AccessShareLock)?;
        guard.keep();
        Ok(())
    }
    pub(super) fn UnlockRelationOid(oid: Oid) -> PgResult<()> {
        // C: UnlockRelationOid(oid, AccessShareLock).
        lmgr_seams::unlock_relation_oid::call(oid, AccessShareLock)
    }
    ...
    pub(super) fn lwlock_release_relcache_init() {
        // C: LWLockRelease(RelCacheInitLock).
        lwlock_seams::lwlock_release_main::call(REL_CACHE_INIT_LOCK)
    ...
    pub(super) fn rename_file(from: &str, to: &str) -> Result<(), ()> {
        ...
        if fd_seams::rename_file::call(from, to) == 0 {
            Ok(())
        } else {
            Err(())
        }
    ...
    pub(super) fn read_dir_numeric(dir: &str) -> PgResult<Vec<String>> {
        ...
        let names = fd_seams::read_dir_names_logged::call(dir);
        Ok(names
            .into_iter()
    ...
    pub(super) fn catalog_schema_attrs(
        ...
        relcache_seams::catalog_schema_attrs::call(reltype)
    }
}
