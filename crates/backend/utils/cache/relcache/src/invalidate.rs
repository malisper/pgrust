...
mod xunit {
    ...
    pub(super) fn smgrreleaseall() {
        smgr_seams::smgrreleaseall::call()
    }
    ...
    pub(super) fn relation_close_smgr(relation: types_core::primitive::Oid) {
        ...
        smgr_seams::relation_close_smgr::call(rlocator)
    }
    ...
    pub(super) fn historic_snapshot_active() -> bool {
        snapmgr_seams::historic_snapshot_active::call()
    }
...
fn equal_rule_locks(
    rl1: &Option<mcx::PgBox<'static, relcache_entry::RuleLock>>,
    rl2: &Option<mcx::PgBox<'static, relcache_entry::RuleLock>>,
) -> bool {
    match (rl1, rl2) {
        (None, None) => true,
        (Some(l1), Some(l2)) => {
            ...
            for (r1, r2) in l1.rules.iter().zip(l2.rules.iter()) {
                ...
                for (a1, a2) in r1.actions.iter().zip(r2.actions.iter()) {
                    if !equalfuncs_seams::equal_query::call(a1, a2) {
                        return false;
                    }
                }
            }
...
fn equal_rsdesc(
    rs1: &Option<mcx::PgBox<'static, relcache_entry::RowSecurityDesc>>,
    rs2: &Option<mcx::PgBox<'static, relcache_entry::RowSecurityDesc>>,
) -> bool {
    match (rs1, rs2) {
        (None, None) => true,
        (Some(d1), Some(d2)) => {
...
fn equal_policy(
    p1: &relcache_entry::RowSecurityPolicy,
    p2: &relcache_entry::RowSecurityPolicy,
) -> bool {
...
fn equal_opt_node(
    a: &Option<mcx::PgBox<'static, nodes::nodes::Node<'static>>>,
    b: &Option<mcx::PgBox<'static, nodes::nodes::Node<'static>>>,
) -> bool {
    match (a, b) {
        (None, None) => true,
        (Some(x), Some(y)) => {
            equalfuncs_seams::equal_node::call(x, y)
        }
        _ => false,
    }
}
