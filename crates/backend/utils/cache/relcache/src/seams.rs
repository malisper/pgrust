...
//! bridge the cross-unit value-slice (`rel::RelationData`/`Relation`,
...
pub fn init_seams() {
    ...
    sx::set_relcache_attmissing::set(|relid, attnum, image| {
        crate::core_entry_store::with_relation_mut(relid, |rd| {
            ...
            let constr = rd
                .rd_att
                .constr
                .get_or_insert_with(relcache_entry::OwnedTupleConstr::default);
            if constr.missing.len() < natts {
                constr
                    .missing
                    .resize(natts, relcache_entry::OwnedAttrMissing::default());
            }
            constr.missing[idx] = relcache_entry::OwnedAttrMissing {
                am_present: true,
                am_value: Some(image),
            };
        })
    ...
    matview_deps_seams::matview_rule_info::set(matview_rule_info);
    matview_deps_seams::matview_data_query::set(matview_data_query);
    ...
    // entry.
    matview_deps_seams::index_usability_info::set(index_usability_info);
    ...
    matview_deps_seams::index_match_merge_quals::set(index_match_merge_quals);
    ...
    hx::visibilitymap_pin_ok::set(|heap_blk, vmbuf| {
        Ok(visibilitymap_seams::visibilitymap_pin_ok::call(heap_blk, vmbuf))
    });
    ...
    ruleutils_seams::ruleutils_relation_real_colnames::set(
...
fn tc_relation_is_nailed(rel: &rel::Relation<'_>) -> PgResult<bool> {
...
fn tc_relation_get_refcount(rel: &rel::Relation<'_>) -> PgResult<i32> {
...
fn project_open(
    ...
    Ok(rel::Relation::open(data, None))
}
...
fn relation_extension_lock_waiter_count(rel: Oid) -> PgResult<u32> {
    ...
    let n = lmgr_seams::relation_extension_lock_waiter_count::call(
        lock_rel_id,
    )?;
    Ok(n as u32)
}
...
fn get_page_with_free_space(
    ...
    freespace_seams::get_page_with_free_space::call(&rel, len)
}
...
fn relation_set_target_block(
    ...
    let (locator, backend) =
        crate::core_entry_store::with_relation(rel, |rd| (rd.rd_locator, rd.rd_backend))?;
    smgr_seams::smgrsettargblock::call(locator, backend, target_block)
}
...
fn vac_relation_get_relation_name(rel: &rel::Relation<'_>) -> PgResult<String> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.relname.to_string())
}

/// `rel->rd_rel->reltuples` (float4) widened to f64.
fn vac_relation_get_reltuples(rel: &rel::Relation<'_>) -> PgResult<f64> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.reltuples as f64)
}

/// `RelationNeedsWAL(rel)`.
fn vac_relation_needs_wal(rel: &rel::Relation<'_>) -> PgResult<bool> {
    Ok(relation_needs_wal(rel))
}

/// `RelationUsesLocalBuffers(rel)` = `rel->rd_rel->relpersistence ==
/// RELPERSISTENCE_TEMP`.
fn vac_relation_uses_local_buffers(rel: &rel::Relation<'_>) -> PgResult<bool> {
...
fn relation_get_index_predicate<'mcx>(
    ...
    // "no predicate" (NIL).
    let has_predicate =
        syscache_seams::pg_index_has_predicate::call(rel.rd_id)?
            .unwrap_or(false);
    if !has_predicate {
...
fn relation_get_dummy_index_expressions<'mcx>(
    ...
    match relcache_nodexform_seams::dummy_index_expressions::call(
        mcx,
        index.rd_id,
    )? {
        None => Ok(None),
...
fn rd_indcollation(index: &rel::Relation<'_>, attno: AttrNumber) -> PgResult<Oid> {
 ...
 * the cross-unit `rel::RelationData<'mcx>` value-slice (the build family
 * owns the full projection). The pure scalar reads off a passed
 * `rel::RelationData`/`Relation` value-slice read its inline fields via
...
fn relation_needs_wal(rel: &rel::RelationData<'_>) -> bool {
...
fn relation_is_accessible_in_logical_decoding(
    ...
    crate::core_entry_store::with_relation(rel.rd_id, |rd| {
        ...
        xlog_logical_info_active
            && relation_needs_wal
            && (catalog_seams::is_catalog_relation_oid::call(rd.rd_id)
...
fn is_toast_relation(relation_id: Oid) -> PgResult<bool> {
    ...
    Ok(catalog_seams::is_toast_namespace::call(relnamespace))
}
...
fn relation_rd_indam(index_oid: Oid) -> Option<types_tableam::amapi::IndexAmRoutine> {
    ...
    amapi_seams::get_index_am_routine::call(amhandler).ok()
}
...
) -> PgResult<rel::RelationData<'mcx>> {
    crate::build::create_fake_relcache_entry(mcx, rlocator)
}

fn free_fake_relcache_entry(fakerel: rel::RelationData<'_>) {
...
fn rd_rel_relam(rel: &rel::Relation<'_>) -> PgResult<Oid> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.relam)
}
fn rd_rel_reltablespace(rel: &rel::Relation<'_>) -> PgResult<Oid> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.reltablespace)
}
fn rd_rel_relowner(rel: &rel::Relation<'_>) -> PgResult<Oid> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.relowner)
}
fn rd_rel_relisshared(rel: &rel::Relation<'_>) -> PgResult<bool> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.relisshared)
}
fn rd_rel_relnamespace(rel: &rel::Relation<'_>) -> PgResult<Oid> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.relnamespace)
}
fn rd_rel_relfrozenxid(rel: &rel::Relation<'_>) -> PgResult<TransactionId> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.relfrozenxid)
}
fn rd_rel_relminmxid(rel: &rel::Relation<'_>) -> PgResult<MultiXactId> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.relminmxid)
}
fn rd_islocaltemp(rel: &rel::Relation<'_>) -> PgResult<bool> {
...
fn rd_index_indrelid(index: &rel::Relation<'_>) -> PgResult<Option<Oid>> {
    with_entry(index.rd_id, |rd| rd.rd_index.as_ref().map(|i| i.indrelid))
}
fn rd_index_indisvalid(index: &rel::Relation<'_>) -> PgResult<bool> {
    with_entry(index.rd_id, |rd| {
        rd.rd_index.as_ref().is_some_and(|i| i.indisvalid)
    })
}
fn rd_index_has_indpred(index: &rel::Relation<'_>) -> PgResult<bool> {
...
fn rd_index_indnatts(index: &rel::Relation<'_>) -> PgResult<Option<i16>> {
    with_entry(index.rd_id, |rd| rd.rd_index.as_ref().map(|i| i.indnatts))
}
fn rd_index_indnkeyatts(index: &rel::Relation<'_>) -> PgResult<Option<i16>> {
    with_entry(index.rd_id, |rd| {
        rd.rd_index.as_ref().map(|i| i.indnkeyatts)
    })
}
fn rd_index_indisunique(index: &rel::Relation<'_>) -> PgResult<bool> {
    with_entry(index.rd_id, |rd| {
        rd.rd_index.as_ref().is_some_and(|i| i.indisunique)
    })
}
fn rd_index_indisprimary(index: &rel::Relation<'_>) -> PgResult<bool> {
    with_entry(index.rd_id, |rd| {
        rd.rd_index.as_ref().is_some_and(|i| i.indisprimary)
    })
}
fn rd_index_indisexclusion(index: &rel::Relation<'_>) -> PgResult<bool> {
    with_entry(index.rd_id, |rd| {
        rd.rd_index.as_ref().is_some_and(|i| i.indisexclusion)
    })
}
fn rd_index_indisready(index: &rel::Relation<'_>) -> PgResult<bool> {
    with_entry(index.rd_id, |rd| {
        rd.rd_index.as_ref().is_some_and(|i| i.indisready)
    })
}
fn rd_index_indnullsnotdistinct(index: &rel::Relation<'_>) -> PgResult<bool> {
    with_entry(index.rd_id, |rd| {
...
fn index_match_merge_quals(
    ...
    for (i, attr) in attrs.iter().enumerate() {
        ...
        // COMPARE_EQ).
        let op = lsyscache_seams::get_opfamily_member_for_cmptype::call(
            opfamily,
            opcintype,
            opcintype,
            COMPARE_EQ,
        ...
        let leftop =
            ruleutils_seams::quote_qualified_identifier::call(
                mcx,
                Some("newdata"),
                &attr.attname,
            )?
...
fn rd_rel_relpersistence(rel: &rel::Relation<'_>) -> PgResult<i8> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.relpersistence as i8)
}
fn rd_rel_relkind(rel: &rel::Relation<'_>) -> PgResult<i8> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.relkind as i8)
}
fn rd_rel_relnatts(rel: &rel::Relation<'_>) -> PgResult<i16> {
    with_entry(rel.rd_id, |rd| rd.rd_rel.relnatts)
}
fn rd_rel_relispartition(rel: &rel::Relation<'_>) -> PgResult<bool> {
...
fn rd_indam_amclusterable(index: &rel::Relation<'_>) -> PgResult<bool> {
    ...
    amapi_seams::index_am_clusterable::call(relam)
}
fn relation_is_mapped(rel: &rel::Relation<'_>) -> PgResult<bool> {
...
fn relation_get_number_of_blocks(rel: &rel::Relation<'_>) -> PgResult<u32> {
...
fn rd_opfamily(index: &rel::Relation<'_>, attno: AttrNumber) -> PgResult<Oid> {
...
fn rd_opcintype(index: &rel::Relation<'_>, attno: AttrNumber) -> PgResult<Oid> {
    // `index->rd_opcintype[attno - 1]` off the cached index entry (attno is
    // 1-based, as in C).
    with_entry(index.rd_id, |rd| rd.rd_opcintype[(attno - 1) as usize])
}
fn rd_indam_amcanorder(index: &rel::Relation<'_>) -> PgResult<bool> {
    ...
    // AM OID (`rd_rel->relam`).
    let relam = with_entry(index.rd_id, |rd| rd.rd_rel.relam)?;
    amapi_seams::index_am_canorder::call(relam)
}

fn rd_indam_amsearcharray(index: &rel::Relation<'_>) -> PgResult<bool> {
    // `index->rd_indam->amsearcharray`: as for amcanorder, projected off the
    // AM's untrimmed routine by AM OID.
    let relam = with_entry(index.rd_id, |rd| rd.rd_rel.relam)?;
    amapi_seams::index_am_searcharray::call(relam)
}
...
struct HashMetaAmcache(hash::hashpage::HashMetaPageData);
...
fn rd_amcache_hashmeta(
    index_oid: Oid,
) -> PgResult<Option<hash::hashpage::HashMetaPageData>> {
...
fn set_rd_amcache_hashmeta(
    index_oid: Oid,
    metap: hash::hashpage::HashMetaPageData,
) -> PgResult<()> {
...
struct SpGistCacheAmcache(spgist::SpGistCache);
...
fn rd_amcache_spgist(index_oid: Oid) -> PgResult<Option<spgist::SpGistCache>> {
...
fn set_rd_amcache_spgist(index_oid: Oid, cache: spgist::SpGistCache) -> PgResult<()> {
