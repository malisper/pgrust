...
fn fill_entry(e: &TypeCacheEntry, flags: &mut i32) -> PgResult<()> {
    ...
    if (*flags & TYPECACHE_DOMAIN_BASE_INFO) != 0 && e.typtype.get() == TYPTYPE_DOMAIN {
        lane_unported("DOMAIN_BASE_INFO", type_id);
    }
    if (*flags & TYPECACHE_DOMAIN_CONSTR_INFO) != 0 && e.typtype.get() == TYPTYPE_DOMAIN {
        lane_unported("DOMAIN_CONSTR_INFO", type_id);
    }
