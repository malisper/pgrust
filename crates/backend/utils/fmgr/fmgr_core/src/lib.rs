...
// executor `nodes` frame and every dispatch semantic (collation, secdef,
...
pub fn function_call_invoke_with_expr(
    ...
    match res {
        ...
        FmgrResolution::Sql { fn_oid } => {
            functions_seams::fmgr_sql::call(mcx, *fn_oid, fcinfo)
        }
    }
}
...
pub fn fmgr_info_cxt_security(
    ...
    let proc = syscache_seams::lookup_proc::call(mcx, function_id)?
...
/// [`load_external_function`](dfmgr_seams::load_external_function)
...
fn fmgr_info_c_lang(function_id: Oid, proc: &ProcInfo) -> PgResult<LoadedCFunc> {
    ...
    let (user_fn, api_version) = if let Some(entry) = lookup_c_func(function_id, proc) {
        ...
        let loaded =
            dfmgr_seams::load_external_function::call(probin, prosrc, function_id)?;
...
/// `pg_language` cache (the [`lookup_language`](syscache_seams::lookup_language)
...
fn fmgr_info_other_lang(
    ...
    let lang = syscache_seams::lookup_language::call(mcx, language)?
        .ok_or_else(|| {
...
fn build_cache<'mcx>(
    ...
    let proc = syscache_seams::lookup_proc::call(mcx, fn_oid)?
...
fn fmgr_security_definer_body(
    ...
    if has_config {
        for (name, value) in fcache.config_names.iter().zip(fcache.config_values.iter()) {
            ...
            let context = if miscinit_seams::superuser::call(mcx)? {
            ...
            guc_seams::set_config_with_handle::call(
                name.as_str(),
                value.as_str(),
                context,
                srole,
        ...
        Some(f) => match &f.fn_expr {
            ...
            Some(node) => match node.as_ref() {
                ...
                FnExpr::External(ext) => match external_expr(ext) {
                    Some(expr) => {
                        nodeFuncs_seams::expr_type_info::call(expr)
                            .map_or(InvalidOid, |t| t.typid)
                    }
                    None => nodeFuncs_seams::expr_type::call(ext.clone()),
                },
            },
        },
        None => InvalidOid,
...
/// [`call_expr_argtype`](nodeFuncs_seams::call_expr_argtype) seam.
...
pub fn get_call_expr_argtype(expr: Option<&FnExpr>, argnum: i32) -> Oid {
    match expr {
        ...
        Some(FnExpr::External(ext)) => match external_expr(ext) {
            Some(e) => nodeFuncs_seams::get_call_expr_argtype_expr::call(e, argnum)
                .unwrap_or(InvalidOid),
            None => nodeFuncs_seams::call_expr_argtype::call(ext.clone(), argnum),
        },
    }
}
...
pub fn get_call_expr_arg_stable(expr: Option<&FnExpr>, argnum: i32) -> bool {
    match expr {
        ...
        Some(FnExpr::External(ext)) => match external_expr(ext) {
            Some(e) => nodeFuncs_seams::call_expr_arg_stable_expr::call(e, argnum),
            None => nodeFuncs_seams::call_expr_arg_stable::call(ext.clone(), argnum),
        },
    }
}
    ...
    match flinfo {
        Some(f) => match &f.fn_expr {
            ...
            Some(node) => match node.as_ref() {
                ...
                FnExpr::External(ext) => match external_expr(ext) {
                    Some(e) => nodeFuncs_seams::expr_variadic_expr::call(e),
                    None => nodeFuncs_seams::expr_variadic::call(ext.clone()),
                },
            },
        },
        None => false,
...
pub fn check_function_validator_access(
    ...
    let proc = syscache_seams::lookup_proc::call(mcx, function_oid)?
        .ok_or_else(|| {
    ...
    let lang: LangInfo = syscache_seams::lookup_language::call(mcx, proc.prolang)?
        .ok_or_else(|| {
...
/// seam over the `nodes` call frame. C: `if (!flinfo || !flinfo->fn_expr)
...
fn get_fn_expr_argtype_seam(
    ...
    match fcinfo_fn_expr(fcinfo) {
        None => Ok(InvalidOid),
        Some(expr) => {
            nodeFuncs_seams::get_call_expr_argtype_expr::call(expr, argnum)
        }
    }
}
...
fn typmodin_seam(typmodin: Oid, cstrings: &[String], cursorpos: i32) -> PgResult<i32> {
    ...
    let arr = arrayfuncs_seams::build_cstring_array::call(mcx, &elem_refs)?;
...
fn input_is_valid_by_type_seam(
    ...
    let (typiofunc, typioparam) =
        lsyscache_seams::get_type_input_info::call(typoid)?;
...
// mirror of the `fmgr` ABI frame's `ref_args`): a by-reference `text` /
...
// as `datum_to_ref_arg` does for the `fmgr` frame). The `'mcx` allocation
...
fn pg_getarg_text_pp_seam<'mcx>(
    ...
    let detoasted = if detoasted.first().is_some_and(|&b| b != 0x01 && (b & 0x01) == 0x01) {
        ...
        detoast_seams::detoast_attr::call(mcx, detoasted.as_slice())?
    } else {
        detoasted
    };
...
fn get_fn_expr_variadic_seam(
    ...
    match fcinfo_fn_expr(fcinfo) {
        None => false,
        Some(expr) => nodeFuncs_seams::expr_variadic_expr::call(expr),
    }
}
...
fn get_fn_expr_arg_stable_seam(
    ...
    match fcinfo_fn_expr(fcinfo) {
        None => false,
        Some(expr) => {
            nodeFuncs_seams::call_expr_arg_stable_expr::call(expr, argnum)
        }
    }
}
...
fn record_column_receive_seam<'mcx>(
    ...
    let (typreceive, typioparam) =
        lsyscache_seams::get_type_binary_input_info::call(coltype)?;
    let resolved = fmgr_info(mcx, typreceive)?;
    let out = receive_function_call_typed(
        mcx,
        &resolved.resolution,
...
fn elem_to_arg(e: array::ArrayElementDatum<'_>) -> (NullableDatum, Option<RefPayload>) {
    match e {
        array::ArrayElementDatum::ByValue(d) => (NullableDatum::value(d), None),
        array::ArrayElementDatum::ByRef(bytes) => (
            NullableDatum::value(Datum::null()),
...
fn element_eq_seam(
    function_id: Oid,
    collation: Oid,
    a: array::ArrayElementDatum<'_>,
    b: array::ArrayElementDatum<'_>,
) -> PgResult<bool> {
...
fn element_hash_seam(
    function_id: Oid,
    collation: Oid,
    value: array::ArrayElementDatum<'_>,
) -> PgResult<u32> {
...
fn array_output_function_call_seam<'mcx>(
    ...
    let resolved = fmgr_info(mcx, function_id)?;
    let s = match value {
        array::ArrayElementDatum::ByValue(d) => {
            output_function_call_typed(
                mcx,
                &resolved.resolution,
                resolved.finfo,
...
fn array_send_function_call_seam<'mcx>(
    ...
    let image = match value {
        array::ArrayElementDatum::ByValue(d) => send_function_call_typed(
            mcx,
            &resolved.resolution,
            resolved.finfo,
            FmgrArg::ByVal(canon_byval(d)),
...
/// `typmodin` stay UNINSTALLED: the executor `nodes` frame carries
...
pub fn init_seams() {
    fmgr_seams::fmgr_info_check::set(fmgr_info_check);
    fmgr_seams::fmgr_info::set(fmgr_info_resolve);
    // `FmgrHookIsNeeded(functionId)` (fmgr.c) — the fmgr entry/exit hook gate
    // read by `inline_set_returning_function` (clauses.c).
    clauses_seams::fmgr_hook_is_needed::set(fmgr_hook_is_needed);
    fmgr_seams::fmgr_info_set_expr::set(fmgr_info_set_expr_seam);
    fmgr_seams::get_fn_expr_argtype::set(get_fn_expr_argtype_seam);
    fmgr_seams::get_fn_expr_rettype::set(get_fn_expr_rettype_seam);
    fmgr_seams::oid_function_call_1_deflist::set(oid_function_call_1_deflist);
    fmgr_seams::index_options_function_call::set(
        index_options_function_call_seam,
    );
    fmgr_seams::typmodin::set(typmodin_seam);
    fmgr_seams::oid_send_function_call::set(oid_send_function_call_seam);
    fmgr_seams::oid_output_function_call::set(oid_output_function_call_seam);
    fmgr_seams::function_call1_coll::set(function_call1_coll_seam);
    fmgr_seams::function_call2_coll::set(function_call2_coll_seam);
    fmgr_seams::function_call1_coll_datum::set(function_call1_coll_datum_seam);
    fmgr_seams::function_call2_coll_datum::set(function_call2_coll_datum_seam);
    fmgr_seams::function_call3::set(function_call3_seam);
    fmgr_seams::function_call3_coll_datum::set(function_call3_coll_datum_seam);
    fmgr_seams::output_function_call::set(output_function_call_seam);
    fmgr_seams::send_function_call::set(send_function_call_seam);
    fmgr_seams::oid_input_function_call::set(oid_input_function_call_seam);
    fmgr_seams::oid_receive_function_call::set(oid_receive_function_call_seam);
    fmgr_seams::input_function_call_for_heap_form::set(
        input_function_call_for_heap_form_seam,
    );
    fmgr_seams::oid_output_function_call_datum::set(
        oid_output_function_call_datum_seam,
    );
    fmgr_seams::input_function_call::set(input_function_call_seam);
    fmgr_seams::receive_function_call::set(receive_function_call_seam);
    fmgr_seams::input_function_call_safe::set(input_function_call_safe_seam);
    fmgr_seams::input_is_valid_by_type::set(input_is_valid_by_type_seam);
    fmgr_seams::oid_function_call0::set(oid_function_call0_seam);
    fmgr_seams::function_call_invoke::set(function_call_invoke_seam);
    ...
    fmgr_seams::fastpath_input_function_call::set(
        fastpath_input_function_call_seam,
    );
    fmgr_seams::fastpath_receive_function_call::set(
        fastpath_receive_function_call_seam,
    );
    fmgr_seams::fastpath_output_function_call::set(
        fastpath_output_function_call_seam,
    );
    fmgr_seams::fastpath_send_function_call::set(
        fastpath_send_function_call_seam,
    );
    fmgr_seams::fastpath_function_call_invoke::set(
    ...
    fmgr_seams::function_call_finalfn_owned::set(
    ...
    clauses_seams::fmgr_call::set(fmgr_call_seam);
    fmgr_seams::conversion_proc_empty_input_test::set(
        conversion_proc_empty_input_test_seam,
    );
    fmgr_seams::convert_via_proc::set(convert_via_proc_seam);
    fmgr_seams::convert_via_proc_counted::set(convert_via_proc_counted_seam);

    // Frame-widening (PG_GETARG_* / PG_RETURN_* / PG_NARGS / PG_ARGISNULL /
    // call-mcx / fn_expr readers) over the executor `nodes` frame.
    fmgr_seams::pg_nargs::set(pg_nargs_seam);
    fmgr_seams::pg_argisnull::set(pg_argisnull_seam);
    fmgr_seams::pg_getarg_oid::set(pg_getarg_oid_seam);
    fmgr_seams::pg_getarg_int16::set(pg_getarg_int16_seam);
    fmgr_seams::pg_getarg_int64::set(pg_getarg_int64_seam);
    fmgr_seams::pg_getarg_bool::set(pg_getarg_bool_seam);
    fmgr_seams::pg_getarg_datum::set(pg_getarg_datum_seam);
    ...
    fmgr_seams::pg_getarg_text_pp::set(pg_getarg_text_pp_seam);
    fmgr_seams::pg_getarg_varlena_pp::set(pg_getarg_varlena_pp_seam);
    fmgr_seams::pg_getarg_name::set(pg_getarg_name_seam);
    fmgr_seams::pg_getarg_cstring::set(pg_getarg_cstring_seam);
    fmgr_seams::pg_return_int64::set(pg_return_int64_seam);
    fmgr_seams::pg_return_datum::set(pg_return_datum_seam);
    fmgr_seams::pg_return_bool::set(pg_return_bool_seam);
    fmgr_seams::pg_return_null::set(pg_return_null_seam);
    fmgr_seams::pg_call_mcx::set(pg_call_mcx_seam);
    fmgr_seams::get_fn_expr_variadic::set(get_fn_expr_variadic_seam);
    fmgr_seams::get_fn_expr_arg_stable::set(get_fn_expr_arg_stable_seam);
    fmgr_seams::fn_oid_and_expr::set(fn_oid_and_expr_seam);
    fmgr_seams::fn_oid_and_fn_expr_erased::set(
        fn_oid_and_fn_expr_erased_seam,
    );

    // Re-resolve I/O over typed inputs (`input_is_valid_by_type` installed above).
    fmgr_seams::record_column_input::set(record_column_input_seam);
    fmgr_seams::record_column_receive::set(record_column_receive_seam);
    fmgr_seams::record_column_output::set(record_column_output_seam);
    fmgr_seams::record_column_send::set(record_column_send_seam);
    fmgr_seams::typmod_out::set(typmod_out_seam);

    // Element-type dispatch (arrayfuncs.c).
    fmgr_seams::element_eq::set(element_eq_seam);
    fmgr_seams::element_cmp::set(element_cmp_seam);
    fmgr_seams::element_hash::set(element_hash_seam);
    fmgr_seams::element_hash_extended::set(element_hash_extended_seam);
    fmgr_seams::array_output_function_call::set(array_output_function_call_seam);
    fmgr_seams::array_receive_function_call::set(
        array_receive_function_call_seam,
    );
    fmgr_seams::array_send_function_call::set(array_send_function_call_seam);
    ...
    // context.
    pg_proc_seams::check_function_validator_access::set(
    ...
    pg_proc_seams::validate_internal_function::set(|funcoid| {
        ...
        let proc = syscache_seams::lookup_proc::call(mcx, funcoid)?
    ...
    pg_proc_seams::validate_c_function::set(|funcoid| {
        ...
        dfmgr_seams::load_external_function::call(probin, prosrc, funcoid)?;
        Ok(())
    });
}
