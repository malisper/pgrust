...
fn loader_pg_file_exists(name: &str) -> PgResult<bool> {
    fd_seams::pg_file_exists::call(name)
}
...
/// [`dfmgr_seams::resolve_builtin_library_function`]). Strips
...
/// Installer for `dfmgr_seams::load_external_function`:
...
/// Installer for `dfmgr_seams::load_output_plugin`:
...
fn install_load_output_plugin(plugin: String) -> PgResult<u32> {
    if let Some(builtin) =
        dfmgr_seams::resolve_builtin_output_plugin(&plugin)
    {
...
/// Installer for `dfmgr_seams::invoke_builtin_output_plugin_callback`:
...
fn install_invoke_builtin_output_plugin_callback(
    ...
    if let Some(plugin) = ctx.builtin_plugin {
        if let Some(result) =
            dfmgr_seams::registry_invoke_builtin_output_plugin(plugin, ctx, inv)
        {
            return result;
        }
    }
...
/// Installer for `dfmgr_seams::load_file`: supply the
...
pub fn init_seams() {
    dfmgr_seams::load_file::set(install_load_file);
    dfmgr_seams::load_external_function::set(install_load_external_function);
    ...
    utility_out_seams::load_file::set(|filename, restricted| {
    ...
    dfmgr_seams::builtin_library_present::set(
        dfmgr_seams::registry_library_present,
    );
    dfmgr_seams::resolve_builtin_library_function::set(|library, function| {
        Ok(dfmgr_seams::registry_resolve(library, function))
    });

    dfmgr_seams::load_output_plugin::set(install_load_output_plugin);
    dfmgr_seams::invoke_output_plugin_callback::set(
    ...
    dfmgr_seams::builtin_output_plugin_name::set(|plugin| {
        dfmgr_seams::resolve_builtin_output_plugin(plugin).map(|e| e.name)
    });
    dfmgr_seams::invoke_builtin_output_plugin_callback::set(
    ...
    // entry, so it is self-delimiting on the restore side.
    {
        ...
        // address across `function_exists` (dynloader_seams: "The resolved
    ...
    guc_tables::vars::Dynamic_library_path.install(
