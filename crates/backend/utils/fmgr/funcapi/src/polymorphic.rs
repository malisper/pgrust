...
use nodeFuncs_seams as node_funcs;
...
/// erased field-bearing `Expr` ([`fmgr::ExternalFnExpr`]):
///
...
impl CallExpr {
    ...
    pub fn from_node<'mcx>(mcx: mcx::Mcx<'mcx>, node: &Node<'mcx>) -> PgResult<Self> {
        ...
        Ok(Self {
            external: fmgr::ExternalFnExpr { tag, node: erased },
        })
    }
    ...
    for i in 0..natts {
        ...
        match atttypid {
            ANYELEMENTOID | ANYNONARRAYOID | ANYENUMOID => {
                ...
                tupdesc::TupleDescInitEntryCollation(td, attno, anycollation)?;
            }
            ANYARRAYOID => {
                tupdesc::TupleDescInitEntry(
                    td,
