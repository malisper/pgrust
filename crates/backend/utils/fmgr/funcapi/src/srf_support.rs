...
pub fn InitMaterializedSRF<'mcx>(
    ...
    let mcx: Mcx<'mcx> = fmgr_seams::pg_call_mcx::call(fcinfo);
    ...
    let (fn_oid, fn_expr_erased) =
        fmgr_seams::fn_oid_and_fn_expr_erased::call(fcinfo_ref);
    ...
    let tupstore = sort_storage_seams::tuplestore_begin_heap::call(
        mcx,
        random_access,
        false,
...
pub fn srf_arg_varlena_bytes<'mcx>(
    ...
    detoast_seams::pg_detoast_datum_packed::call(mcx, image)
}
...
mod srf_protocol_tests {
    ...
    //! / `end_MultiFuncCall` over a real `nodes` call frame, faithfully
