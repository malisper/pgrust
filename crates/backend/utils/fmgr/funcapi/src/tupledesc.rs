...
pub fn RelationNameGetTupleDesc<'mcx>(mcx: Mcx<'mcx>, relname: &str) -> PgResult<TupleDesc<'mcx>> {
    ...
    let relname_list = regproc_seams::string_to_qualified_name_list::call(
        mcx, relname, false,
    )?
    ...
    let relvar = namespace_seams::make_range_var_from_name_list::call(&parts)?;

    // rel = relation_openrv(relvar, AccessShareLock);
    let rel = common_relation_seams::relation_openrv::call(
        mcx,
        &relvar,
        AccessShareLock,
    )?;
    ...
    let tupdesc =
        tupdesc_seams::create_tupledesc_copy::call(mcx, &rel.rd_att)?;
...
pub fn TypeGetTupleDesc<'mcx>(
    ...
    let tupdesc: TupleDesc<'mcx> = match functypclass {
        TypeFuncClass::Composite => {
            ...
            let mut td = typcache_seams::lookup_rowtype_tupdesc_copy::call(
                mcx,
                base_typeoid,
                -1,
            )?;
...
pub fn extract_variadic_args<'mcx>(
    ...
    let variadic = fmgr_seams::get_fn_expr_variadic::call(fcinfo);
    ...
    if variadic {
        ...
        debug_assert_eq!(
            fmgr_seams::pg_nargs::call(fcinfo),
            variadic_start + 1
        );

        // if (PG_ARGISNULL(variadic_start)) return -1;
        if fmgr_seams::pg_argisnull::call(fcinfo, variadic_start as usize) {
            return Ok(None);
        }
        ...
        let array_in =
            fmgr_seams::pg_getarg_datum::call(fcinfo, variadic_start as usize);

        // element_type = ARR_ELEMTYPE(array_in);
        let element_type =
            arrayfuncs_seams::array_get_elemtype::call(mcx, array_in)?;

        // get_typlenbyvalalign(element_type, &typlen, &typbyval, &typalign);
        let tlbva = lsyscache_seams::get_typlenbyvalalign::call(element_type)?;
        ...
        let total = fmgr_seams::pg_nargs::call(fcinfo);
        ...
        for i in 0..nargs {
            ...
            let isnull =
                fmgr_seams::pg_argisnull::call(fcinfo, argnum as usize);
            // types_res[i] = get_fn_expr_argtype(fcinfo->flinfo, i + variadic_start);
            let mut argtype =
                fmgr_seams::get_fn_expr_argtype::call(fcinfo, argnum)?;
            ...
            if convert_unknown
                && argtype == UNKNOWNOID
                && fmgr_seams::get_fn_expr_arg_stable::call(fcinfo, argnum)
            {
                argtype = TEXTOID;

                if fmgr_seams::pg_argisnull::call(fcinfo, argnum as usize) {
                    // args_res[i] = (Datum) 0;
                    value = DatumV::null();
                } else {
                    ...
                    let s = fmgr_seams::pg_getarg_cstring::call(
                        fcinfo,
                        argnum as usize,
                    );
                    value = funcapi_seams::cstring_get_text_datum::call(mcx, s)?;
                }
            } else {
                ...
                value = DatumV::ByVal(
                    fmgr_seams::pg_getarg_datum::call(fcinfo, argnum as usize)
                        .as_usize(),
                );
            }
...
/// [`heaptuple::HeapTupleGetDatum`] (NO new Datum variant,
...
pub fn record_from_values<'mcx>(
    ...
    heaptuple::HeapTupleGetDatum(mcx, &formed, &td)
}
