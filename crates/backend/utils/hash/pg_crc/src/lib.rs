...
//!   [`crc32c::legacy::traditional_crc32`] (same table, same normal-read
...
//!   [`crc32c::pg_comp_crc32c_sb8`]; INIT and FIN are performed here at the
...
pub fn crc32_bytea(in_bytes: &[u8]) -> u32 {
    crc32c::legacy::traditional_crc32(in_bytes)
}
...
pub fn crc32c_bytea(in_bytes: &[u8]) -> u32 {
    ...
    let crc = crc32c::pg_comp_crc32c_sb8(crc, in_bytes);
    /* FIN_CRC32C(crc) */
    crc ^ 0xFFFF_FFFF
}
