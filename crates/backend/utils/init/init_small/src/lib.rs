...
pub fn InitProcessGlobals() -> types_error::PgResult<()> {
    let my_start_timestamp = timestamp_seams::get_current_timestamp::call();
    globals::SetMyStartTimestamp(my_start_timestamp);
    globals::SetMyStartTime(
        timestamp_seams::timestamptz_to_time_t::call(my_start_timestamp),
    );
    ...
    let strong_ok = if pg_strong_random_seams::pg_strong_random::is_installed() {
        let mut bytes = [0u8; 16];
        if pg_strong_random_seams::pg_strong_random::call(&mut bytes) {
            let s0 = u64::from_ne_bytes(bytes[0..8].try_into().unwrap());
            let s1 = u64::from_ne_bytes(bytes[8..16].try_into().unwrap());
            prng::global_prng(|state| {
                *state = prng::PgPrng::from_raw(s0, s1);
    ...
    if !strong_ok {
        ...
        prng::global_prng(|state| state.seed(rseed));
    }
...
pub fn init_seams() {
    init_small_seams::init_process_globals::set(InitProcessGlobals);
    init_small_seams::work_mem::set(globals::work_mem);
    init_small_seams::hash_mem_multiplier::set(globals::hash_mem_multiplier);
    init_small_seams::max_worker_processes::set(globals::max_worker_processes);
    init_small_seams::max_parallel_workers::set(globals::max_parallel_workers);
    init_small_seams::fast_path_lock_groups_per_backend::set(
        globals::fast_path_lock_groups_per_backend,
    );
    init_small_seams::set_fast_path_lock_groups_per_backend::set(
        globals::set_fast_path_lock_groups_per_backend,
    );
    init_small_seams::my_proc_pid::set(globals::MyProcPid);
    init_small_seams::my_proc_number::set(globals::MyProcNumber);
    init_small_seams::set_my_proc_number::set(globals::SetMyProcNumber);
    init_small_seams::my_cancel_key::set(my_cancel_key);
    init_small_seams::is_under_postmaster::set(globals::IsUnderPostmaster);
    init_small_seams::postmaster_pid::set(globals::PostmasterPid);
    init_small_seams::my_pm_child_slot::set(globals::MyPMChildSlot);
    init_small_seams::max_backends::set(globals::MaxBackends);
    init_small_seams::with_my_proc_port::set(globals::with_my_proc_port_seam);
    init_small_seams::set_my_proc_port::set(|port| {
        globals::SetMyProcPort(Some(Box::new(port)))
    });
    init_small_seams::set_client_connection_lost::set(globals::SetClientConnectionLost);
    init_small_seams::set_interrupt_pending::set(globals::SetInterruptPending);
    init_small_seams::interrupt_pending::set(globals::InterruptPending);
    init_small_seams::set_proc_die_pending::set(globals::SetProcDiePending);
    init_small_seams::proc_die_pending::set(globals::ProcDiePending);
    init_small_seams::set_query_cancel_pending::set(globals::SetQueryCancelPending);
    init_small_seams::query_cancel_pending::set(globals::QueryCancelPending);
    init_small_seams::set_interrupt_holdoff_count::set(globals::SetInterruptHoldoffCount);
    init_small_seams::interrupt_holdoff_count::set(globals::InterruptHoldoffCount);
    init_small_seams::interrupts_can_be_processed::set(
        globals::InterruptsCanBeProcessed,
    );
    init_small_seams::hold_interrupts::set(globals::HoldInterrupts);
    init_small_seams::resume_interrupts::set(globals::ResumeInterrupts);
    init_small_seams::set_my_backend_type::set(globals::SetMyBackendType);
    init_small_seams::nbuffers::set(globals::NBuffers);
    init_small_seams::nloc_buffer::set(globals::NLocBuffer);
    init_small_seams::my_database_id::set(globals::MyDatabaseId);
    // collationcmds.c re-declares `MyDatabaseId` in its own seam crate.
    collationcmds_seams::my_database_id::set(globals::MyDatabaseId);
    init_small_seams::my_database_table_space::set(globals::MyDatabaseTableSpace);
    init_small_seams::my_start_timestamp::set(globals::MyStartTimestamp);
    init_small_seams::is_postmaster_environment::set(globals::IsPostmasterEnvironment);
    init_small_seams::set_my_pm_child_slot::set(globals::SetMyPMChildSlot);
    init_small_seams::set_my_client_socket::set(|cs| globals::SetMyClientSocket(Some(cs)));
    init_small_seams::my_client_socket::set(globals::MyClientSocket);
    init_small_seams::start_critical_section::set(globals::StartCriticalSection);
    init_small_seams::end_critical_section::set(globals::EndCriticalSection);
    init_small_seams::exit_on_any_error::set(globals::ExitOnAnyError);
    init_small_seams::set_exit_on_any_error::set(globals::SetExitOnAnyError);
    init_small_seams::my_backend_type::set(globals::MyBackendType);
    // Pure-wiring installs (assemble/seam-wiring-guard): owner globals match.
    init_small_seams::max_connections::set(globals::MaxConnections);
    init_small_seams::set_max_backends::set(globals::SetMaxBackends);
    init_small_seams::is_binary_upgrade::set(globals::IsBinaryUpgrade);
    ...
    postmaster_seams::is_binary_upgrade::set(globals::IsBinaryUpgrade);
    ...
    binary_upgrade_seams::is_binary_upgrade::set(globals::IsBinaryUpgrade);
    tablecmds_seams::is_binary_upgrade::set(|| Ok(globals::IsBinaryUpgrade()));
    tablespace_globals_seams::IsBinaryUpgrade::set(|| {
    ...
    tablespace_globals_seams::MyDatabaseId::set(|| Ok(globals::MyDatabaseId()));
    tablespace_globals_seams::MyDatabaseTableSpace::set(|| {
        Ok(globals::MyDatabaseTableSpace())
    });
    init_small_seams::set_my_database_id::set(globals::SetMyDatabaseId);
    init_small_seams::set_my_database_table_space::set(globals::SetMyDatabaseTableSpace);
    init_small_seams::set_my_database_has_login_event_triggers::set(
        globals::SetMyDatabaseHasLoginEventTriggers,
    );
    init_small_seams::my_database_has_login_event_triggers::set(
        globals::MyDatabaseHasLoginEventTriggers,
    );
    init_small_seams::has_my_proc_port::set(globals::MyProcPortIsSet);
    ...
    // bgworker.
    init_small_seams::post_auth_delay::set(globals::post_auth_delay);
    init_small_seams::reserved_connections::set(globals::reserved_connections);
    init_small_seams::superuser_reserved_connections::set(
        globals::superuser_reserved_connections,
    );
    init_small_seams::my_proc_port_user_name::set(globals::my_proc_port_user_name);
    init_small_seams::my_proc_port_database_name::set(
        globals::my_proc_port_database_name,
    );
    init_small_seams::my_proc_port_application_name::set(
        globals::my_proc_port_application_name,
    );
    init_small_seams::my_proc_port_cmdline_options::set(
        globals::my_proc_port_cmdline_options,
    );
    init_small_seams::my_proc_port_guc_options::set(
        globals::my_proc_port_guc_options,
    );
    init_small_seams::data_dir::set(globals::DataDir);
    ...
    init_small_seams::max_prepared_xacts::set(globals::max_prepared_xacts);
    init_small_seams::autovacuum_worker_slots::set(
        globals::autovacuum_worker_slots,
    );
    // `DatabasePath` read, reconciled to the seam's `PgResult<String>` contract.
    init_small_seams::database_path::set(globals::database_path_seam);
    ...
    postgres_seams::set_pg_start_time::set(globals::SetPgStartTime);
