...
pub fn SetMyBackendType(backend_type: BackendType) {
    init_small_seams::set_my_backend_type::call(backend_type);
}
...
pub fn SetDatabasePath(path: &str) {
    ...
    init_small::globals::SetDatabasePath(Some(path.to_owned()));
}
...
pub fn SetDataDir(dir: &str) -> PgResult<()> {
    ...
    init_small::globals::SetDataDir(Some(new));
    Ok(())
}
...
pub fn SetAuthenticatedUserId(userid: Oid) -> PgResult<()> {
    ...
    lmgr_proc_seams::set_my_proc_role_id::call(userid);
    Ok(())
}
...
pub fn has_rolreplication(mcx: Mcx<'_>, roleid: Oid) -> PgResult<bool> {
    ...
    Ok(
        syscache_seams::lookup_authid_by_oid::call(mcx, roleid)?
...
pub fn InitializeSessionUserId(
    ...
    if transam_parallel::initializing_parallel_worker() {
    ...
    let role = if let Some(name) = rolename {
        syscache_seams::lookup_authid_by_name::call(mcx, name)?.ok_or_else(
            || {
        ...
        syscache_seams::lookup_authid_by_oid::call(mcx, roleid)?.ok_or_else(
            || {
                PgError::new(
                    FATAL,
    ...
    guc_seams::set_config_option_backend_override::call(
    ...
    if init_small::globals::IsUnderPostmaster() {
        ...
        if role.rolconnlimit >= 0
            ...
            && !is_superuser
            && procarray_seams::count_user_backends::call(resolved_roleid)?
                > role.rolconnlimit
        {
            return Err(PgError::new(
                FATAL,
...
pub fn GetUserNameFromId<'mcx>(
    ...
    match syscache_seams::lookup_authid_by_oid::call(mcx, roleid)? {
...
fn load_libraries(mcx: Mcx<'_>, libraries: Option<&str>, gucname: &str, restricted: bool) -> PgResult<()> {
    ...
    let elemlist = match varlena_seams::split_directories_string::call(mcx, libraries)? {
        Some(list) => list,
        None => {
    ...
    for filename in elemlist {
        ...
        dfmgr_seams::load_file::call(filename, restricted)?;
...
pub fn process_shmem_requests() -> PgResult<()> {
    ...
    let result = if dfmgr_seams::shmem_request_hook_present::call() {
        dfmgr_seams::shmem_request_hook::call()
    } else {
        Ok(())
    };
...
pub fn init_seams() {
    ...
    config_info_seams::get_configdata::set(|| {
    ...
    postgres_seams::process_shmem_requests::set(process_shmem_requests);
    ...
    dfmgr_seams::shmem_request_hook_present::set(shmem_request_hook_present);
    dfmgr_seams::shmem_request_hook::set(shmem_request_hook);
    ...
    utility_out_seams::in_security_restricted_operation::set(
    ...
    utility_out_seams::my_backend_type::set(GetMyBackendType);
    ...
    utility_out_seams::get_user_id::set(GetUserId);
    ...
    postmaster_seams::recheck_data_dir_lock_file::set(|| {
    ...
    // brackets below).
    s::is_binary_upgrade::set(init_small::globals::IsBinaryUpgrade);
    ...
    // above.)
    postmaster_seams::touch_socket_lock_files::set(
    ...
    s::start_crit_section::set(|| {
        init_small::globals::SetCritSectionCount(
            init_small::globals::CritSectionCount() + 1,
        )
    });
    ...
    s::hold_interrupts::set(|| {
        init_small::globals::SetInterruptHoldoffCount(
            init_small::globals::InterruptHoldoffCount() + 1,
        )
    });
    s::resume_interrupts::set(|| {
        debug_assert!(init_small::globals::InterruptHoldoffCount() > 0);
        init_small::globals::SetInterruptHoldoffCount(
            init_small::globals::InterruptHoldoffCount() - 1,
        )
    });
    ...
    guc_funcs_seams::get_user_id::set(GetUserId);
    guc_funcs_seams::superuser::set(|| {
    ...
    functioncmds_seams::get_user_id::set(|| Ok(GetUserId()));
    functioncmds_seams::superuser::set(|| {
    ...
    s::clear_database_path::set(|| init_small::globals::SetDatabasePath(None));
    ...
    s::get_database_path::set(|| init_small::globals::DatabasePath());
    // MyProcPid getter (relcache's per-backend init-file temp name).
    s::my_proc_pid::set(|| init_small::globals::MyProcPid());
    ...
    s::in_critical_section::set(|| init_small::globals::CritSectionCount() > 0);
    ...
    // `void` in C — wrap in `Ok`.
    {
        ...
        // not just parallel.c.
        rt::check_for_interrupts::set(|| postgres_seams::check_for_interrupts::call());
    }
    ...
    s::init_process::set(|| lmgr_proc_seams::init_process::call());
    // `BaseInit()` (slotsync.c:1479) — postinit.c owner.
    s::base_init::set(|| postinit_seams::base_init::call());
    ...
    s::init_postgres::set(|dbname| {
        postinit_seams::init_postgres_by_name::call(Some(dbname), None, 0)
    });
    // `CHECK_FOR_INTERRUPTS()` (slotsync.c) — postgres.c / interrupt owner.
    s::check_for_interrupts::set(|| postgres_seams::check_for_interrupts::call());
    ...
    common_exec_seams::set_pglocale_pgservice::set(
    ...
    common_exec_seams::resolve_standalone_paths::set(
    ...
    tablespace_globals_seams::canonicalize_path::set(
        boot_paths::canonicalize_path_pub,
    );
    tablespace_globals_seams::is_absolute_path::set(
        boot_paths::is_absolute_path_pub,
    );
    tablespace_globals_seams::path_is_prefix_of_path::set(
        boot_paths::path_is_prefix_of_path_pub,
    );
    tablespace_globals_seams::get_parent_directory::set(
        boot_paths::get_parent_directory_pub,
    );
    username_seams::get_user_name_or_exit::set(startup_paths::get_user_name_or_exit);
