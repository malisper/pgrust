...
fn file_err(context: impl Into<String>, e: &std::io::Error) -> PgError {
    ...
    PgError::new(FATAL, format!("{}: {e}", context.into()))
        .with_sqlstate(utils_error::errno::sqlstate_for_file_access(errno))
}

/// `DataDir` (globals.c).
fn data_dir() -> String {
    init_small::globals::DataDir().unwrap_or_default()
}
...
fn register_lock_file(filename: &str) -> PgResult<()> {
    UNLINK_HOOK_REGISTERED.with(|reg| -> PgResult<()> {
        if !reg.get() {
            dsm_core_seams::on_proc_exit::call(
    ...
    loop {
        ...
        if is_dd_lock {
            if let Some((id1, id2)) = scan_shmem_key_line(&buffer) {
                if sysv_shmem_seams::pg_shared_memory_is_in_use::call(id1, id2)? {
                    return Err(PgError::new(
                        FATAL,
                        format!(
