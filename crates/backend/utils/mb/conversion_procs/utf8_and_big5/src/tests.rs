use super::*;
use ::types_wchar::encoding::{PG_BIG5, PG_UTF8};
...
fn ascii_passes_through() {
    install_check_seam();
    let r = utf8_to_big5(PG_UTF8, PG_BIG5, b"ascii", false).unwrap();
    assert_eq!(&*r.bytes, b"ascii");
    assert_eq!(r.converted, 5);
    let back = big5_to_utf8(PG_BIG5, PG_UTF8, b"ascii", false).unwrap();
...
fn multibyte_round_trips() {
    ...
    let local = utf8_to_big5(PG_UTF8, PG_BIG5, utf8, false).unwrap();
    assert_eq!(local.converted, utf8.len() as i32);
    assert!(!local.bytes.is_empty());
    let back = big5_to_utf8(PG_BIG5, PG_UTF8, &local.bytes, false).unwrap();
...
fn wrong_source_encoding_is_rejected() {
    install_check_seam();
    assert!(utf8_to_big5(PG_BIG5, PG_BIG5, b"x", false).is_err());
}
