...
//! GUC variable machinery (the W1 core, [`misc_guc`]).
//!
...
//! called on the W1 core ([`misc_guc`]) directly. The remaining
...
use misc_guc::model::config_generic;
use misc_guc::registry::{get_config_option_by_name, show_guc_option, GucRegistry};
...
fn replace_auto_config_value(
    list: &mut Vec<guc_file::ConfigVariable>,
    name: &str,
    value: Option<&str>,
) {
    use guc_file::ConfigVariable;
...
fn render_auto_conf_file(list: &[guc_file::ConfigVariable]) -> String {
    let mut buf = String::new();
    ...
    for item in list {
        ...
        buf.push_str(name);
        buf.push_str(" = '");
        buf.push_str(&misc_guc::escape_single_quotes_ascii(value));
        buf.push_str("'\n");
    }
    buf
}
...
pub fn AlterSystemSetConfigFile(stmt: &VariableSetStmt) -> PgResult<()> {
    ...
    if !guc_tables::backing::AllowAlterSystem() {
        return Err(ereport(ERROR)
            .errcode(ERRCODE_FEATURE_NOT_SUPPORTED)
    ...
    if !resetall {
        misc_guc::validate_auto_config_value(name, value.as_deref())?;
    }
    ...
    let mut head: Vec<guc_file::ConfigVariable> = Vec::new();
    ...
    let write_result = file_seams::write_auto_conf_atomic::call(
...
pub fn GetPGVariableResultDesc<'mcx>(mcx: Mcx<'mcx>, name: &str) -> PgResult<TupleDesc<'mcx>> {
    if guc_name_compare(name, "all") == Ordering::Equal {
        ...
        let mut tupdesc = tupdesc::CreateTemplateTupleDesc(mcx, 1)?;
        tupdesc::TupleDescInitEntry(
...
fn as_def_elem(node: &Node) -> PgResult<&parsenodes::DefElem> {
    node.as_defelem()
...
pub fn InitializeShmemGUCs() -> PgResult<()> {
    ...
    let (size_b, _num_semaphores) =
        ipci_seams::calculate_shmem_size::call()?;
    ...
    let (hp_size, _mmap_flags) = sysv_shmem_seams::get_huge_page_size::call();
    if hp_size != 0 {
...
/// `parsenodes::Node` value (the SET-args flattener only inspects these
...
fn set_arg_from_nodes(arg: &nodes::nodes::Node<'_>) -> PgResult<Node> {
    ...
    match val_node.node_tag() {
        ntag::T_Integer => {
            ...
            Ok(Node::Integer(parsenodes::Integer { ival: i.ival }))
        }
        ntag::T_Float => {
            let f = val_node.expect_float();
            Ok(Node::Float(parsenodes::Float {
                fval: Some(f.fval.to_string()),
            }))
        }
        ntag::T_Boolean => {
            let b = val_node.expect_boolean();
            Ok(Node::Boolean(parsenodes::Boolean { boolval: b.boolval }))
        }
        ntag::T_String => {
            let st = val_node.expect_string();
            Ok(Node::String(parsenodes::StringNode {
...
fn variable_set_stmt_from_nodes(
    ...
    for arg in s.args.iter() {
        ...
        let projected = match val_node.node_tag() {
            ntag::T_Integer => {
                let i = val_node.expect_integer();
                Node::Integer(parsenodes::Integer { ival: i.ival })
            }
            ntag::T_Float => {
                let f = val_node.expect_float();
                Node::Float(parsenodes::Float {
                    fval: Some(f.fval.to_string()),
                })
            }
            ntag::T_Boolean => {
                let b = val_node.expect_boolean();
                Node::Boolean(parsenodes::Boolean { boolval: b.boolval })
            }
            ntag::T_String => {
                let st = val_node.expect_string();
            ...
            ntag::T_DefElem => {
                ...
                Node::DefElem(parsenodes::DefElem {
...
pub fn init_seams() {
    ...
    guc_funcs_seams::extract_set_variable_args::set(|sstmt| {
    ...
    guc_seams::set_pg_variable_session_authorization_reset::set(|| {
    ...
    guc_funcs_seams::get_config_option_flags::set(|name, missing_ok| {
    ...
    // option-value `Node` arrives in the post-analyze `nodes` universe and
    // is projected into the `parsenodes` node `SetPGVariable` consumes.
    utility_out_seams::set_pg_variable::set(|name, arg, is_local| {
    ...
    utility_out_seams::get_pg_variable::set(|mcx, name, dest| {
    ...
    utility_out_seams::get_pg_variable_result_desc::set(|mcx, name| {
        let name = name.ok_or_else(|| {
            types_error::PgError::error(
    ...
    postgres_seams::initialize_shmem_gucs::set(InitializeShmemGUCs);
    ...
    explain_seams::get_explain_guc_options::set(|| {
    ...
    explain_seams::explain_get_config_option_by_name::set(|name| {
