...
crate::session_guc_cluster!(BackingSessionGucs, BACKING_SESSION_GUCS:
    ...
    // selector (consts::PARALLEL_ENGINE_*). Default legacy = zero behavior
    // change — the M5-1 router engages only under runtime; the per-arm bench
    // pool GUCs layer BENEATH the switch and keep working verbatim either way.
    (pgrust_parallel_engine_cell, i32, pgrust_parallel_engine, set_pgrust_parallel_engine, 0),
