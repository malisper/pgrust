...
// USE_LZ4 for TOAST is implemented in Rust (lz4_flex), so
// default_toast_compression carries C's #ifdef USE_LZ4 entry. USE_ZSTD stays
// undefined, and wal_compression's lz4/zstd arms are NOT implemented — those
// options follow the C #else branches. USE_SSL / USE_OPENSSL are DEFINED here
// (be_secure_openssl): ssl_library, ssl_ciphers, ssl_groups carry the OpenSSL
// boot values.
...
pub static ConfigureNamesBool: &[GucBoolSetting] = &[
    ...
    GucBoolSetting { name: "jwt_validator.require_scopes", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Requires every HBA scope to be present in the token's scope claim (jwt_validator OAuth validator)."), long_desc: None, flags: 0, variable: &vars::jwt_validator_require_scopes, boot_val: GucDefaultValue::Bool(true), check_hook: None, assign_hook: None, show_hook: None },
    GucBoolSetting { name: "jwt_validator.allow_insecure_http", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Allows plain-HTTP JWKS/introspection endpoints for the jwt_validator OAuth validator (testing only)."), long_desc: None, flags: 0, variable: &vars::jwt_validator_allow_insecure_http, boot_val: GucDefaultValue::Bool(false), check_hook: None, assign_hook: None, show_hook: None },
    #[cfg(feature = "oauth-test-validator")]
    GucBoolSetting { name: "oauth_validator.authorize_tokens", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Should tokens be marked valid?"), long_desc: None, flags: 0, variable: &vars::oauth_validator_authorize_tokens, boot_val: GucDefaultValue::Bool(true), check_hook: None, assign_hook: None, show_hook: None },
    ...
    // pgrust-only (docs/design/connection-scaling.md D3.2 + wave 3a): the
    // process-global generation-keyed L2 of immutable catalog-cache bodies,
    // including the init-file install-or-adopt routing (folded in — there is
    ...
    // isn custom GUC (contrib/isn/isn.c:939 _PG_init DefineCustomBoolVariable),
    // statically defined like auto_explain.*; the cell lives in crates/contrib/isn.
    GucBoolSetting { name: "isn.weak", context: PGC_USERSET, group: CUSTOM_OPTIONS, short_desc: Some("Accept input with invalid ISN check digits."), long_desc: None, flags: 0, variable: &vars::isn_weak, boot_val: GucDefaultValue::Bool(false), check_hook: None, assign_hook: None, show_hook: None },
...
pub static ConfigureNamesInt: &[GucIntSetting] = &[
    ...
    // pg_prewarm custom GUC (contrib/pg_prewarm/autoprewarm.c:128-138 _PG_init
    // DefineCustomIntVariable; defined before the shared_preload_libraries
    // check, so every backend that loads the library has it). Statically
    // defined like auto_explain.*.
    GucIntSetting { name: "pg_prewarm.autoprewarm_interval", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Sets the interval between dumps of shared buffers"), long_desc: Some("If set to zero, time-based dumping is disabled."), flags: GUC_UNIT_S, variable: &vars::autoprewarm_interval, boot_val: GucDefaultValue::Int(300), min: 0, max: i32::MAX / 1000, check_hook: None, assign_hook: None, show_hook: None },
    GucIntSetting { name: "passwordcheck.min_password_length", context: PGC_SUSET, group: CUSTOM_OPTIONS, short_desc: Some("Minimum allowed password length."), long_desc: None, flags: GUC_UNIT_BYTE, variable: &vars::passwordcheck_min_password_length, boot_val: GucDefaultValue::Int(8), min: 0, max: i32::MAX, check_hook: None, assign_hook: None, show_hook: None },
    ...
    GucIntSetting { name: "jwt_validator.clock_skew", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Clock skew tolerated when checking exp/nbf/iat claims (jwt_validator OAuth validator)."), long_desc: None, flags: GUC_UNIT_S, variable: &vars::jwt_validator_clock_skew, boot_val: GucDefaultValue::Int(60), min: 0, max: 3600, check_hook: None, assign_hook: None, show_hook: None },
    GucIntSetting { name: "jwt_validator.jwks_cache_ttl", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("How long a fetched JWKS document is reused before re-fetching (jwt_validator OAuth validator)."), long_desc: None, flags: GUC_UNIT_S, variable: &vars::jwt_validator_jwks_cache_ttl, boot_val: GucDefaultValue::Int(300), min: 0, max: 86400, check_hook: None, assign_hook: None, show_hook: None },
    GucIntSetting { name: "jwt_validator.http_timeout", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Connect/read timeout for JWKS and introspection requests (jwt_validator OAuth validator)."), long_desc: None, flags: GUC_UNIT_S, variable: &vars::jwt_validator_http_timeout, boot_val: GucDefaultValue::Int(10), min: 1, max: 300, check_hook: None, assign_hook: None, show_hook: None },
    ...
    // pgrust-only (docs/design/connection-scaling.md D1): counting-semaphore
    // admission gate at statement-execution start; 0 disables.
    GucIntSetting { name: "max_active_queries", context: PGC_SIGHUP, group: CONN_AUTH_SETTINGS, short_desc: Some("Sets the maximum number of concurrently admitted transactions."), long_desc: Some("0 disables the gate. Admission is at transaction grain: a transaction's first statement queues FIFO before taking a snapshot or any locks, and the slot is held to transaction end, so statements inside an admitted transaction never wait behind the gate. Superusers and replication connections are exempt."), flags: 0, variable: &vars::max_active_queries, boot_val: GucDefaultValue::Int(0), min: 0, max: i32::MAX, check_hook: None, assign_hook: None, show_hook: None },
    // pgrust-only (docs/design/connection-scaling.md D6): park over-limit
    // connections on a bounded FIFO instead of an immediate 53300.
    // PGC_POSTMASTER: it adds headroom to the pmchild backend pool at boot.
    GucIntSetting { name: "connection_queue_size", context: PGC_POSTMASTER, group: CONN_AUTH_SETTINGS, short_desc: Some("Sets the number of over-limit connections that queue for a slot instead of failing."), long_desc: Some("0 disables the queue (an over-limit connection fails immediately with SQLSTATE 53300, as stock PostgreSQL does). Waiters beyond the bound also fail immediately."), flags: 0, variable: &vars::connection_queue_size, boot_val: GucDefaultValue::Int(0), min: 0, max: MAX_BACKENDS, check_hook: None, assign_hook: None, show_hook: None },
    // pgrust-only (docs/design/connection-scaling.md D3.4): after this long
    // idle outside a transaction, a backend passivates — drops its L1 caches
    // (relcache/catcache/plan bodies; rebuilt from the shared L2 on the next
    // query) and returns retained allocator memory.
    GucIntSetting { name: "idle_passivate_timeout", context: PGC_SIGHUP, group: CONN_AUTH_SETTINGS, short_desc: Some("Sets the idle time after which a session passivates its caches."), long_desc: Some("0 disables passivation. A passivated session rebuilds its caches from the shared cache on its next query."), flags: GUC_UNIT_S, variable: &vars::idle_passivate_timeout, boot_val: GucDefaultValue::Int(60), min: 0, max: i32::MAX / 1000, check_hook: None, assign_hook: None, show_hook: None },
    GucIntSetting { name: "connection_queue_timeout", context: PGC_SIGHUP, group: CONN_AUTH_SETTINGS, short_desc: Some("Sets the maximum time a connection waits in the admission queue before failing."), long_desc: Some("0 means wait indefinitely. On expiry the connection fails with the same SQLSTATE 53300 error an unqueued over-limit connection receives."), flags: GUC_UNIT_MS, variable: &vars::connection_queue_timeout, boot_val: GucDefaultValue::Int(5000), min: 0, max: i32::MAX, check_hook: None, assign_hook: None, show_hook: None },
    // pgrust-only (docs/design/connection-scaling.md D3.1): hard entry caps on
    // the per-session L1 caches (clock/LRU eviction, pinned/nailed exempt).
    // PGC_SIGHUP: the eviction points read the cell on every cap check.
    GucIntSetting { name: "catcache_size_limit", context: PGC_SIGHUP, group: RESOURCES_MEM, short_desc: Some("Sets the maximum number of catalog cache tuples kept per session."), long_desc: Some("0 disables the limit. Above it, unpinned tuples are evicted clock-wise on insertion; an evicted tuple is re-read from the system catalogs on its next lookup."), flags: 0, variable: &vars::catcache_size_limit, boot_val: GucDefaultValue::Int(2048), min: 0, max: i32::MAX, check_hook: None, assign_hook: None, show_hook: None },
    GucIntSetting { name: "relcache_size_limit", context: PGC_SIGHUP, group: RESOURCES_MEM, short_desc: Some("Sets the maximum number of relation cache entries kept per session."), long_desc: Some("0 disables the limit. Above it, unpinned entries are evicted in LRU order on insertion; an evicted entry is rebuilt from the system catalogs (or adopted from the shared catalog cache) on its next open."), flags: 0, variable: &vars::relcache_size_limit, boot_val: GucDefaultValue::Int(512), min: 0, max: i32::MAX, check_hook: None, assign_hook: None, show_hook: None },
    GucIntSetting { name: "min_dynamic_shared_memory", context: PGC_POSTMASTER, group: RESOURCES_MEM, short_desc: Some("Amount of dynamic shared memory reserved at startup."), long_desc: None, flags: GUC_UNIT_MB, variable: &vars::min_dynamic_shared_memory, boot_val: GucDefaultValue::Int(0), min: 0, max: 2147483647, check_hook: Some(&hooks::check_min_dynamic_shared_memory), assign_hook: None, show_hook: None },
    ...
    GucIntSetting { name: "pgrust.ephemeral_db_pool_size", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Number of pre-minted spare clones the janitor keeps warm per template minted-from (0 = off)."), long_desc: Some("Usage-keyed: the first mint of a template registers it for pooling (that mint is served cold) and the janitor thereafter keeps up to this many spares of it warm. A mint request is satisfied by a catalog-only RENAME of a spare of the requested template instead of a file copy; spares are named <prefix>spare_<seq> (a namespace reserved from minting), owned by the janitor and not connectable until handout, and replenished in background janitor ticks. A global 4096-spare ceiling is shared fairly (round-robin) when pooled-templates times pool_size exceeds it. Spares are invalidated per template on rebuild/drop/unseal and on any observed change of that template's ALLOW_CONNECTIONS; a permanently connectable template keeps its spares, so handouts may serve content as old as each spare's mint — reseal the template (or rebuild it under a new name) to force freshness."), flags: 0, variable: &vars::pgrust_ephemeral_db_pool_size, boot_val: GucDefaultValue::Int(0), min: 0, max: 4096, check_hook: None, assign_hook: None, show_hook: None },
...
pub static ConfigureNamesReal: &[GucRealSetting] = &[
    ...
    // pg_trgm custom GUCs (contrib/pg_trgm/trgm_op.c:145-190 _PG_init, three
    // DefineCustomRealVariable: bootValue is the float literal 0.3f/0.6f/0.5f
    // widened to double, range 0.0 .. 1.0, PGC_USERSET, no hooks). Statically
    // defined like auto_explain.*.
    GucRealSetting { name: "pg_trgm.similarity_threshold", context: PGC_USERSET, group: CUSTOM_OPTIONS, short_desc: Some("Sets the threshold used by the % operator."), long_desc: Some("Valid range is 0.0 .. 1.0."), flags: 0, variable: &vars::pg_trgm_similarity_threshold, boot_val: GucDefaultValue::Real(0.3f32 as f64), min: 0.0f64, max: 1.0f64, check_hook: None, assign_hook: None, show_hook: None },
    GucRealSetting { name: "pg_trgm.word_similarity_threshold", context: PGC_USERSET, group: CUSTOM_OPTIONS, short_desc: Some("Sets the threshold used by the <% operator."), long_desc: Some("Valid range is 0.0 .. 1.0."), flags: 0, variable: &vars::pg_trgm_word_similarity_threshold, boot_val: GucDefaultValue::Real(0.6f32 as f64), min: 0.0f64, max: 1.0f64, check_hook: None, assign_hook: None, show_hook: None },
    GucRealSetting { name: "pg_trgm.strict_word_similarity_threshold", context: PGC_USERSET, group: CUSTOM_OPTIONS, short_desc: Some("Sets the threshold used by the <<% operator."), long_desc: Some("Valid range is 0.0 .. 1.0."), flags: 0, variable: &vars::pg_trgm_strict_word_similarity_threshold, boot_val: GucDefaultValue::Real(0.5f32 as f64), min: 0.0f64, max: 1.0f64, check_hook: None, assign_hook: None, show_hook: None },
...
pub static ConfigureNamesString: &[GucStringSetting] = &[
    ...
    GucStringSetting { name: "jwt_validator.jwks_uri", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("JWKS endpoint for the jwt_validator OAuth validator (empty: discovered from the HBA issuer)."), long_desc: None, flags: 0, variable: &vars::jwt_validator_jwks_uri, boot_val: GucDefaultValue::String(Some("")), check_hook: None, assign_hook: None, show_hook: None },
    GucStringSetting { name: "jwt_validator.audience", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Audience (aud claim) the jwt_validator OAuth validator requires in every token."), long_desc: None, flags: 0, variable: &vars::jwt_validator_audience, boot_val: GucDefaultValue::String(Some("")), check_hook: None, assign_hook: None, show_hook: None },
    GucStringSetting { name: "jwt_validator.identity_claim", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Token claim the jwt_validator OAuth validator maps to the authenticated identity."), long_desc: None, flags: 0, variable: &vars::jwt_validator_identity_claim, boot_val: GucDefaultValue::String(Some("sub")), check_hook: None, assign_hook: None, show_hook: None },
    GucStringSetting { name: "jwt_validator.introspection_uri", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("RFC 7662 token introspection endpoint for opaque (non-JWT) bearer tokens (empty: opaque tokens are rejected)."), long_desc: None, flags: 0, variable: &vars::jwt_validator_introspection_uri, boot_val: GucDefaultValue::String(Some("")), check_hook: None, assign_hook: None, show_hook: None },
    GucStringSetting { name: "jwt_validator.introspection_client_id", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Client ID the jwt_validator OAuth validator authenticates to the introspection endpoint with."), long_desc: None, flags: 0, variable: &vars::jwt_validator_introspection_client_id, boot_val: GucDefaultValue::String(Some("")), check_hook: None, assign_hook: None, show_hook: None },
    GucStringSetting { name: "jwt_validator.introspection_client_secret", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Client secret the jwt_validator OAuth validator authenticates to the introspection endpoint with."), long_desc: None, flags: GUC_SUPERUSER_ONLY, variable: &vars::jwt_validator_introspection_client_secret, boot_val: GucDefaultValue::String(Some("")), check_hook: None, assign_hook: None, show_hook: None },
    GucStringSetting { name: "jwt_validator.ca_file", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("CA certificate bundle the jwt_validator OAuth validator verifies HTTPS peers against (empty: OpenSSL defaults)."), long_desc: None, flags: 0, variable: &vars::jwt_validator_ca_file, boot_val: GucDefaultValue::String(Some("")), check_hook: None, assign_hook: None, show_hook: None },
    #[cfg(feature = "oauth-test-validator")]
    GucStringSetting { name: "oauth_validator.authn_id", context: PGC_SIGHUP, group: CUSTOM_OPTIONS, short_desc: Some("Authenticated identity to use for future connections"), long_desc: None, flags: 0, variable: &vars::oauth_validator_authn_id, boot_val: GucDefaultValue::String(None), check_hook: None, assign_hook: None, show_hook: None },
