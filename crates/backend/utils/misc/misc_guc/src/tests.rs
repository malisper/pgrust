...
fn install_once() {
    ...
    ONCE.call_once(|| {
        ...
        transam_xact_seams::is_in_parallel_mode::set(|| false);
        miscinit_seams::in_local_user_id_change::set(|| false);
        miscinit_seams::in_security_restricted_operation::set(|| false);
        aclchk_seams::pg_parameter_aclcheck::set(|_name, _role, _mode| {
        ...
        miscinit_seams::get_user_id::set(|| BOOTSTRAP_SUPERUSERID);
        scalar_seams::parse_bool::set(default_parse_bool);
        ...
        scansup_seams::truncate_identifier::set(|mcx, ident, _warn| {
            mcx::slice_in(mcx, ident)
        });
        // The pq sink for ReportGUCOption (no-op accept).
        pqcomm_seams::pq_putmessage::set(|_msgtype, _body| Ok(0));
