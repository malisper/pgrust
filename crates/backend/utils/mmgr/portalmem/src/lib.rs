...
use portalcmds_seams as portalcmds_seam;
use plancache_portal_seams as plancache_seam;
use resowner_seams_2 as resowner_seam;
use resowner_seams as resowner_cur_seam;
use tuplestore_hold_seams as tuplestore_seam;
...
pub fn AtAbort_Portals() -> PgResult<()> {
    ...
    for portal in portals {
        ...
        if status == PORTAL_ACTIVE && dsm_core::ipc::shmem_exit_inprogress() {
...
pub fn pg_cursor<'mcx>(
    ...
    for row in &rows {
        ...
        values[0] = varlena_seams::cstring_to_text_v::call(mcx, &row.name)?;
        // C: values[1] = CStringGetTextDatum(portal->sourceText);
        values[1] = varlena_seams::cstring_to_text_v::call(mcx, &row.statement)?;
