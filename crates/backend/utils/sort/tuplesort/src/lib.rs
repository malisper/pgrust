...
use execUtils_seams as executils_seam;
use execTuples_seams as exectuples_seam;
...
fn apply_sort_comparator<'mcx>(
    ...
    if isnull1 {
        ...
        let mut c = sortsupport_seams::apply_sort_comparator::call(
            datum1, datum2, ssup,
        )?;
...
fn apply_sort_abbrev_full_comparator<'mcx>(
    ...
    if isnull1 {
        ...
        let mut c = sortsupport_seams::apply_sort_abbrev_full_comparator::call(
            datum1, datum2, ssup,
        )?;
        if ssup.ssup_reverse {
            c = -c;
...
pub fn tuplesort_puttuple_common<'mcx>(
    ...
    if !use_abbrev {
        ...
        let abbrev = sortsupport_seams::apply_sort_abbrev_converter::call(
...
fn index_getattr<'mcx>(
    ...
    indextuple_seams::nocache_index_getattr::call(mcx, itup, attnum, itupdesc)
}
...
fn comparetup_index_btree_tiebreak<'mcx>(
    ...
    if enforce_unique && !(!unique_nulls_not_distinct && equal_hasnull) {
        ...
        let cols =
            indextuple_seams::index_deform_tuple::call(mcx, tuple1, tup_des)?;
        ...
        let key_desc = genam_seams::build_index_value_description::call(
            mcx,
            &arg.indexRel,
            &values,
            &isnull,
...
fn consider_abort_common<'mcx>(state: &mut TuplesortStateImpl<'mcx>) -> PgResult<bool> {
    ...
    if state.status == TupSortStatus::Initial && state.memtupcount as i64 >= state.abbrevNext {
        ...
        let abort = sortsupport_seams::apply_sort_abbrev_abort::call(
            memtupcount,
...
fn tuplesort_begin_heap_state(
    ...
    OwnedSort::try_new(sort_context(), move |sx| {
        ...
        for i in 0..nkeys as usize {
            ...
            sortsupport_seams::prepare_sort_support_from_ordering_op::call(
                sort_operators[i],
                &mut sort_key,
            )?;
...
fn tuplesort_begin_datum_state(
    ...
    let (typlen, typbyval) =
        lsyscache_seams::get_typlenbyval::call(datum_type)?;
...
fn tuplesort_begin_index_btree_state(
    ...
    let index_scan_key = nbtree_core_seams::bt_mkscankey::call(index_rel, None)?
    ...
    for i in 0..nkeys as usize {
        ...
        sortsupport_seams::prepare_sort_support_from_index_rel::call(
            index_rel,
            reverse,
            &mut sort_key,
        )?;
...
fn tuplesort_begin_index_gist_state(
    ...
    for i in 0..nkeys as usize {
        ...
        sortsupport_seams::prepare_sort_support_from_gist_index_rel::call(
            index_rel,
            &mut sort_key,
        )?;
        sort_key_snaps.push(SortSupportSnapshot::capture(&sort_key));
...
fn tuplesort_putindextuplevalues_impl<'mcx>(
    ...
    let tuple: PgVec<'mcx, u8> = indextuple_seams::index_form_tuple::call(
...
pub fn init_seams() {
    ...
    costsize_seams::tuplesort_merge_order::set(
