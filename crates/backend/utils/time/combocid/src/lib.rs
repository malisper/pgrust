...
pub fn init_seams() {
    combocid_seams::heap_tuple_header_get_cmin::set(|tuple| {
        with_state(|s| HeapTupleHeaderGetCmin(s, tuple))
    });
    combocid_seams::heap_tuple_header_get_cmax::set(|tuple| {
        with_state(|s| HeapTupleHeaderGetCmax(s, tuple))
    });
    combocid_seams::heap_tuple_header_adjust_cmax::set(|tuple, cmax| {
        with_state(|s| HeapTupleHeaderAdjustCmax(s, tuple, cmax))
    });
    combocid_seams::at_eoxact_combocid::set(|| with_state(AtEOXact_ComboCid));
...
pub fn HeapTupleHeaderGetCmin(state: &ComboCidState, tup: &HeapTupleHeaderData<'_>) -> CommandId {
    ...
    debug_assert!(
        transam_xact_seams::transaction_id_is_current_transaction_id::call(
