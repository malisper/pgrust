...
fn register_snapshot_on(snapshot: &SnapHandle, on_top: bool) -> SnapHandle {
    ...
    let _ = snapmgr_seams::resource_owner_enlarge_for_snapshot::call(on_top);
    ...
    if snap_.borrow().regd_count == 1 {
        ...
        snapmgr_seams::resource_owner_remember_snapshot::call(reg_id, on_top);
    }
...
pub fn UnregisterSnapshotFromOwner(snapshot: &SnapHandle) -> PgResult<()> {
    ...
    if snapshot.borrow().regd_count == 1 {
        ...
        snapmgr_seams::resource_owner_forget_snapshot::call(reg_id);
    }
...
pub fn init_seams() {
    ...
    // is infallible in C.
    use snapmgr_pc_seams as pc_seams;
    ...
    use parallel_rt_seams as rt_seams;
