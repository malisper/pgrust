//! ERROR-ARM drain module, round 3 (lane W4-ERR; line-drain queue chunks
//! `ddl-cmds-residue` + `parser-arms` + `catalog-residue` + `ddl-tablecmds`,
//! the residue LD6's `earm` and LD8's `earm2` passes left — see
//! docs/fuzzing/findings-w4err.md).
//!
//! Same charter as `earm`/`earm2`: the unhit mass is ereport(ERROR)/
//! validation arms only invalid input reaches, so probes deliberately
//! error and the differential bar is the ERROR IDENTITY (SQLSTATE via
//! diff::classify, message drift carried in the detail). Round-3 surface:
//! FK-bearing partition trees through attach/detach/alter cycles
//! (CloneFkReferencing/CloneFkReferenced/DetachPartitionFinalize/
//! CloneRowTriggersToPartition), DefineIndex partitioned-index +
//! am-capability arms, LIKE INCLUDING variants + inheritance merge
//! conflicts (MergeAttributes/expandTableLikeClause), PK/UNIQUE USING
//! INDEX + WITHOUT OVERLAPS residue (transformIndexConstraint),
//! anycompatible/anyrange residue (enforce_generic_type_consistency),
//! GRANT/REVOKE ... ALL ... IN SCHEMA + default-ACL + NO_PRIV arms
//! (objectsInSchemaToOids/RemoveRoleFromObjectACL/aclcheck_error),
//! CREATEROLE-without-SUPERUSER grantor arms, ExecuteTruncateGuts,
//! inherited-constraint drop/rename, ALTER COLUMN TYPE re-derivation,
//! wrong-relkind ALTER arms (alter_table_type_to_string), add/drop
//! column recursion, CreateCast/AlterOperator/opclass proc arms,
//! recursive-CTE misuse (checkWellFormedRecursionWalker), default
//! output-column names (FigureColnameInternal), column-ref/function
//! resolution residue (transformColumnRef/ParseFuncOrColumn/
//! func_get_detail), shared-dependency + dependency-walker fuel
//! (checkSharedDependencies/findDependentObjects), and tsearch
//! deserialize_deflist arms.
//!
//! Rules (inherited from earm/earm2):
//!   - every group is SELF-CONTAINED and emitted VERBATIM from the
//!     hand-verified deck (fuzz/deck-w4err-earm3.sql, applied
//!     per-statement to the A/B pair before encoding — identical
//!     SQLSTATE everywhere; the residual transcript diff is exactly the
//!     already-banked LD6-F1/LD6-F2/LD8-F1 identity classes);
//!   - fixtures live under fixed `ea3_` names, created and dropped
//!     inside the section (or via the section's schema CASCADE drop);
//!     erroring probes cannot create state;
//!   - XML output-column-name forms are deliberately absent: the
//!     in-lane A side is built --without-libxml and raises 0A000 where
//!     pgrust's native XML succeeds (rig-config divergence, not a
//!     conformance surface);
//!   - roles are cluster-global: every CREATE ROLE has its DROP in the
//!     same section past the probes that need it; SET ROLE spans end
//!     with RESET ROLE before any DDL the superuser must perform;
...
fn body(shape: &str) -> Vec<StmtKind> {
    let pool: &[&str] = match shape {
        ...
        "earm3:likei" => LIKEI,
        "earm3:idxcon" => IDXCON,
        "earm3:gentype" => GENTYPE,
        "earm3:aclres" => ACLRES,
        "earm3:roles" => ROLES,
        "earm3:trunc" => TRUNC,
        "earm3:dropcon" => DROPCON,
        "earm3:altcol" => ALTCOL,
        "earm3:wrongk" => WRONGK,
        "earm3:addcol" => ADDCOL,
        "earm3:castop" => CASTOP,
        "earm3:rcte" => RCTE,
        "earm3:colname" => COLNAME,
        "earm3:colref" => COLREF,
        "earm3:shdep2" => SHDEP2,
        "earm3:tsdes" => TSDES,
        other => unreachable!("earm3 shape {other}"),
    };
    raws(pool)
}

pub fn gen_earm3_module(g: &mut Gen) -> Vec<StmtKind> {
    g.fire("earm3");
...
mod tests {
    ...
    fn sections_are_self_contained() {
        // Successful creates must have a later matching drop in the SAME
        // section (schema-CASCADE drops count via their pair entry).
        const PAIRS: &[(&str, &str)] = &[
            ("CREATE TABLE ea3_ppk ", "DROP TABLE ea3_ppk;"),
            ("CREATE TABLE ea3_pfk ", "DROP TABLE ea3_pfk;"),
            ("CREATE TABLE ea3_pfk_c ", "DROP TABLE ea3_pfk_c;"),
            ("CREATE TABLE ea3_pfk_d ", "DROP TABLE ea3_pfk_d;"),
            ("CREATE FUNCTION ea3_tgf(", "DROP FUNCTION ea3_tgf();"),
            ("CREATE TABLE ea3_ip ", "DROP TABLE ea3_ip;"),
            ("CREATE TABLE ea3_plain ", "DROP TABLE ea3_plain;"),
            ("CREATE TABLE ea3_src ", "DROP TABLE ea3_src;"),
            ("CREATE VIEW ea3_srcv ", "DROP VIEW ea3_srcv;"),
            ("CREATE SEQUENCE ea3_seq;", "DROP SEQUENCE ea3_seq;"),
            ("CREATE TYPE ea3_comp ", "DROP TYPE ea3_comp;"),
            ("CREATE TABLE ea3_ui ", "DROP TABLE ea3_ui;"),
            ("CREATE ROLE ea3_u1;", "DROP ROLE ea3_u1;"),
            ("CREATE SCHEMA ea3_s;", "DROP SCHEMA ea3_s CASCADE;"),
            ("CREATE ROLE ea3_cr ", "DROP ROLE ea3_cr;"),
            ("CREATE ROLE ea3_m1;", "DROP ROLE ea3_m1;"),
            ("CREATE ROLE ea3_m2 ", "DROP ROLE ea3_m2;"),
            ("CREATE TABLE ea3_tp ", "DROP TABLE ea3_tp;"),
            ("CREATE TABLE ea3_tc ", "DROP TABLE ea3_tc;"),
            ("CREATE TABLE ea3_ti ", "DROP TABLE ea3_ti;"),
            ("CREATE TABLE ea3_tpart ", "DROP TABLE ea3_tpart;"),
            ("CREATE VIEW ea3_tv ", "DROP VIEW ea3_tv;"),
            ("CREATE TABLE ea3_dp ", "DROP TABLE ea3_dp;"),
            ("CREATE TABLE ea3_dc ", "DROP TABLE ea3_dc;"),
            ("CREATE TABLE ea3_rp ", "DROP TABLE ea3_rp;"),
            ("CREATE TABLE ea3_rc ", "DROP TABLE ea3_rc;"),
            ("CREATE TABLE ea3_nn ", "DROP TABLE ea3_nn;"),
            ("CREATE TABLE ea3_ac ", "DROP TABLE ea3_ac;"),
            ("CREATE TABLE ea3_acr2 ", "DROP TABLE ea3_acr2;"),
            ("CREATE VIEW ea3_acv ", "DROP VIEW ea3_acv;"),
            ("CREATE FUNCTION ea3_wf(", "DROP FUNCTION ea3_wf();"),
            ("CREATE VIEW ea3_wv ", "DROP VIEW ea3_wv;"),
            ("CREATE SEQUENCE ea3_ws;", "DROP SEQUENCE ea3_ws;"),
            ("CREATE MATERIALIZED VIEW ea3_wm ", "DROP MATERIALIZED VIEW ea3_wm;"),
            ("CREATE TABLE ea3_wt ", "DROP TABLE ea3_wt;"),
            ("CREATE ROLE ea3_wown;", "DROP ROLE ea3_wown;"),
            ("CREATE TABLE ea3_ap ", "DROP TABLE ea3_ap;"),
            ("CREATE TABLE ea3_ak ", "DROP TABLE ea3_ak;"),
            ("CREATE DOMAIN ea3_dompos ", "DROP DOMAIN ea3_dompos;"),
            ("CREATE TABLE ea3_dtab ", "DROP TABLE ea3_dtab;"),
            ("CREATE FUNCTION ea3_c1(", "DROP FUNCTION ea3_c1(int);"),
            ("CREATE FUNCTION ea3_c2(", "DROP FUNCTION ea3_c2(text);"),
            ("CREATE FUNCTION ea3_c3(", "DROP FUNCTION ea3_c3(int, int, boolean);"),
            ("CREATE FUNCTION ea3_opf(", "DROP FUNCTION ea3_opf(int, int);"),
            ("CREATE FUNCTION ea3_cmp(", "DROP FUNCTION ea3_cmp(int, int);"),
            ("CREATE FUNCTION ea3_cmp_wrong(", "DROP FUNCTION ea3_cmp_wrong(text, text);"),
            ("CREATE TABLE ea3_r ", "DROP TABLE ea3_r;"),
            ("CREATE TABLE ea3_x ", "DROP TABLE ea3_x;"),
            ("CREATE TABLE ea3_y ", "DROP TABLE ea3_y;"),
            ("CREATE FUNCTION ea3_getb(", "DROP FUNCTION ea3_getb(ea3_x);"),
            ("CREATE ROLE ea3_own1;", "DROP ROLE ea3_own1;"),
            ("CREATE ROLE ea3_own2;", "DROP ROLE ea3_own2;"),
            ("CREATE SCHEMA ea3_ds ", "DROP SCHEMA ea3_ds CASCADE;"),
            ("CREATE TABLE ea3_ddt ", "DROP TABLE ea3_ddt;"),
            ("CREATE TABLE ea3_dft ", "DROP TABLE ea3_dft;"),
            ("CREATE TABLE ea3_det ", "DROP TABLE ea3_det;"),
            ("CREATE TABLE ea3_dst ", "DROP TABLE ea3_dst;"),
            ("CREATE TEXT SEARCH DICTIONARY ea3_d1 ", "DROP TEXT SEARCH DICTIONARY ea3_d1;"),
            ("CREATE TEXT SEARCH DICTIONARY ea3_d2 ", "DROP TEXT SEARCH DICTIONARY ea3_d2;"),
            ("CREATE TEXT SEARCH CONFIGURATION ea3_cfg ", "DROP TEXT SEARCH CONFIGURATION ea3_cfg;"),
            ("CREATE TEXT SEARCH PARSER ea3_prs ", "DROP TEXT SEARCH PARSER ea3_prs;"),
        ];
        for (name, pool) in all_sections() {
            ...
            for (create, drop) in PAIRS {
                if let Some(c) = joined.find(create) {
                    ...
                    assert!(
                        ...
                        "{name}: create {create:?} without later {drop:?}"
                    );
                }
            }
            // Unprivileged spans return to the superuser.
            ...
            // No transaction spans in this module.
            assert!(!joined.contains("BEGIN;"), "{name}: unexpected BEGIN span");
            // No GUC pollution.
            assert!(!joined.contains("SET search_path"), "{name}: search_path touched");
        }
    }
