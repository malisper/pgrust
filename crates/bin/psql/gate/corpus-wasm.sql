-- Increment-2 corpus: corpus.sql adapted for the wasm arm's environment
-- (single pre-minted cluster: user postgres, database postgres; \c targets
-- a database the corpus itself creates; the \i include file is injected
-- into the VFS by the harness). Run through the Rust psql NATIVELY against
-- a native pgrust server AND as psql.wasm cross-piped to postgres.wasm;
-- the two outputs are diffed byte-for-byte (normalizations: Time/PID and
-- the per-respawn "database system was shut down" boot notice — see
-- run-wasm-gate.sh).
...
listen chan1;
notify chan1, 'hello';
-- 9. \c to another database and back (the issue-#46 witness: under wasm
-- this respawns the server instance on the same VFS)
create database gate2;
\c gate2
