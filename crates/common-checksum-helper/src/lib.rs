...
//!   site, and `COMP_CRC32C` calls [`port_crc32c::pg_comp_crc32c_sb8`].
...
//!   so it crosses [`common_cryptohash_seams`]. Each call panics loudly until
...
use types_crypto::{pg_cryptohash_ctx, pg_cryptohash_type};
...
fn strcaseeq(a: &[u8], b: &[u8]) -> bool {
    port_pgstrcasecmp::pg_strcasecmp(a, b) == 0
}
...
fn create_sha2(type_: pg_cryptohash_type) -> Result<*mut pg_cryptohash_ctx, ChecksumError> {
    let ctx = common_cryptohash_seams::pg_cryptohash_create::call(type_);
    if ctx.is_null() {
        return Err(ChecksumError::Cryptohash);
    }
    if common_cryptohash_seams::pg_cryptohash_init::call(ctx) < 0 {
...
pub fn pg_checksum_update(
    ...
    match &mut context.raw_context {
        ...
        RawContext::Crc32c(crc) => {
            ...
            *crc = port_crc32c::pg_comp_crc32c_sb8(*crc, input);
            Ok(())
        }
        RawContext::Sha2(ctx) => {
            // if (pg_cryptohash_update(context->raw_context.c_sha2, input, len) < 0)
            //     return -1;
            if common_cryptohash_seams::pg_cryptohash_update::call(*ctx, input.as_ptr(), input.len())
                < 0
            {
...
pub fn pg_checksum_final(
    ...
    match context.raw_context {
        ...
        RawContext::Sha2(ctx) => {
            ...
            //     return -1;
            if common_cryptohash_seams::pg_cryptohash_final::call(
                ctx,
                output.as_mut_ptr(),
                retval,
            ) < 0
