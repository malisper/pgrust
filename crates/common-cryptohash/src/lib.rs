...
//! `pg_cryptohash_*` seams declared in [`common_cryptohash_seams`].
//!
...
//!
//! * MD5 — [`common_md5`] (port of `common/md5.c`),
...
//! * SHA-224/256/384/512 — [`common_sha2`] (port of `common/sha2.c`).
//!
...
use common_md5::{md5_ctxt, md5_init, md5_loop, md5_pad, md5_result, MD5_DIGEST_LENGTH};
use common_sha2::{
...
/// pointer (`types_crypto::pg_cryptohash_ctx`); this concrete struct is the
...
#[inline]
fn as_ctx<'a>(ctx: *mut types_crypto::pg_cryptohash_ctx) -> Option<&'a mut CryptoHashCtx> {
    if ctx.is_null() {
        None
    } else {
...
pub fn pg_cryptohash_create(type_: pg_cryptohash_type) -> *mut types_crypto::pg_cryptohash_ctx {
    let ctx = Box::new(CryptoHashCtx {
        type_,
        error: pg_cryptohash_errno::PG_CRYPTOHASH_ERROR_NONE,
        data: CryptoHashData::Uninit,
    });
    Box::into_raw(ctx) as *mut types_crypto::pg_cryptohash_ctx
}
...
/// (`ctx == NULL`).
pub fn pg_cryptohash_init(ctx: *mut types_crypto::pg_cryptohash_ctx) -> i32 {
...
pub fn pg_cryptohash_update(
    ctx: *mut types_crypto::pg_cryptohash_ctx,
    data: *const uint8,
    len: Size,
) -> i32 {
    let Some(ctx) = as_ctx(ctx) else {
...
pub fn pg_cryptohash_free(ctx: *mut types_crypto::pg_cryptohash_ctx) {
    if ctx.is_null() {
        return;
    }
...
/// [`common_cryptohash_seams`]. Called from the startup aggregator
/// (`seams-init`).
pub fn init_seams() {
    common_cryptohash_seams::pg_cryptohash_create::set(pg_cryptohash_create);
    common_cryptohash_seams::pg_cryptohash_init::set(pg_cryptohash_init);
    common_cryptohash_seams::pg_cryptohash_update::set(pg_cryptohash_update);
    common_cryptohash_seams::pg_cryptohash_final::set(pg_cryptohash_final);
    common_cryptohash_seams::pg_cryptohash_free::set(pg_cryptohash_free);
}
