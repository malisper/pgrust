...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    output.push_str("pub static SCAN_KEYWORD_CATEGORIES: [pgrust_pg_ffi::KeywordCategory; ");
    output.push_str(&entries.len().to_string());
    output.push_str("] = [");
    for (idx, entry) in entries.iter().enumerate() {
        if idx > 0 {
...
impl KeywordCategoryToken {
    ...
    fn rust_variant(&self) -> &'static str {
        match self {
            Self::Unreserved => "pgrust_pg_ffi::KeywordCategory::Unreserved",
            Self::ColumnName => "pgrust_pg_ffi::KeywordCategory::ColumnName",
            Self::TypeOrFunctionName => "pgrust_pg_ffi::KeywordCategory::TypeOrFunctionName",
            Self::Reserved => "pgrust_pg_ffi::KeywordCategory::Reserved",
        }
    }
}
