...
///
/// The C version's `hash` function pointer is not stored: lookup is performed
/// directly against the generated keyword text (the perfect hash in the
/// faithful port was itself just a scan over that text).
