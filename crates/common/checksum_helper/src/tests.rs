...
fn install_mock() {
    INSTALL.call_once(|| {
        cryptohash_seams::pg_cryptohash_create::set(mock_create);
        cryptohash_seams::pg_cryptohash_init::set(mock_init);
        cryptohash_seams::pg_cryptohash_update::set(mock_update);
        cryptohash_seams::pg_cryptohash_final::set(mock_final);
        cryptohash_seams::pg_cryptohash_free::set(mock_free);
    });
}
...
fn sha2_final_rejects_undersized_buffer() {
    ...
    cryptohash_seams::pg_cryptohash_free::call(match ctx.raw_context {
        RawContext::Sha2(p) => p,
        _ => unreachable!(),
    });
}
