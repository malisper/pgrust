...
fn json_errdetail_tj(error: TjErr, lex: &TjLexContext) -> Vec<u8> {
    ...
    match error {
        ...
        TjErr::JSON_UNICODE_UNTRANSLATABLE => {
            ...
            buf.extend_from_slice(mbutils::GetDatabaseEncodingName().as_bytes());
            buf.push(b'.');
        }
...
fn check_stack_depth() -> Result<(), ()> {
    match stack_depth_seams::check_stack_depth::call() {
        Ok(()) => Ok(()),
        Err(e) => {
...
impl<'m, 'mcx> SaxSink for JsonbSink<'m, 'mcx> {
    ...
    fn scalar(
        ...
        let r = adt_jsonb::jsonb_in_scalar(
            self.mcx,
            self.state,
            token,
            tj_token(tokentype),
...
fn run_parse_to_jsonb<'mcx>(
    ...
    let mut state = adt_jsonb::JsonbInState {
        unique_keys,
        ..Default::default()
    };
    ...
    if result != JsonParseErrorType::Success {
        ...
        let snap = snapshot(&lex);
        adt_jsonfuncs::lex::json_errsave_error(tj_err(result), &snap, escontext)?;
        if had_escontext {
            return Ok(None);
        }
    ...
    Ok(Some(jsonb_util::JsonbValueToJsonb(mcx, &res)?))
}
...
impl<'m> SaxSink for UniqueSink<'m> {
    fn object_start(&mut self, _lex: &JsonLexContext) -> JsonParseErrorType {
        tj_to_internal_err(adt_json::json_unique_object_start(self.state))
    }
    fn object_end(&mut self, _lex: &JsonLexContext) -> JsonParseErrorType {
        tj_to_internal_err(adt_json::json_unique_object_end(self.state))
    }
    fn object_field_start(
        &mut self,
        _lex: &JsonLexContext,
        ...
        let name = fname.unwrap_or(&[]);
        tj_to_internal_err(adt_json::json_unique_object_field_start(
...
fn run_parse_validate_unique(json: &[u8]) -> PgResult<(TjErr, bool)> {
    ...
    let mut lex = make_json_lex_context_cstring_len(json, encoding, true);
    let mut state = adt_json::JsonUniqueParsingState::new();
    let result = {
...
fn run_errsave_error(
    ...
    let snap = snapshot(&lex);
    adt_jsonfuncs::lex::json_errsave_error(error, &snap, escontext)
}
...
pub fn init_seams() {
    jsonapi_seams::parse_validate::set(run_parse_validate);
    jsonapi_seams::parse_validate_unique::set(run_parse_validate_unique);
    jsonapi_seams::lex_first_token::set(run_lex_first_token);
    jsonapi_seams::errsave_error::set(run_errsave_error);
    jsonapi_seams::pg_parse_json::set(run_pg_parse_json);
    jsonapi_seams::json_lex_first::set(run_json_lex_first);
    jsonapi_seams::json_errdetail::set(run_json_errdetail);
    jsonapi_seams::json_count_array_elements::set(run_json_count_array_elements);
    jsonapi_seams::get_database_encoding::set(|| {
        mbutils::GetDatabaseEncoding()
    });
    jsonapi_seams::pg_mblen::set(|s: &[u8]| mbutils::pg_mblen(s) as usize);

    // parse_to_jsonb is owned by jsonb-seams but driven by this lexer unit.
    jsonb_seams::parse_to_jsonb::set(run_parse_to_jsonb);
}
