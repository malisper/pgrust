...
//! software SHA-256 ([`sha2`]) — bit-identical to the non-OpenSSL
//! `cryptohash.c`/`hmac.c` build path SCRAM uses — and base64 over the ported
//! [`prng_base64`]. The folded HMAC-SHA256 also backs the
...
pub fn init_seams() {
    scram_seams::scram_h::set(|input| scram_h(&input));
    scram_seams::scram_salted_password::set(|password, salt, iterations| {
        scram_salted_password(&password, &salt, iterations)
    });
    scram_seams::scram_server_key::set(|salted_password| {
        scram_server_key(&salted_password)
    });
    scram_seams::scram_build_secret::set(|salt, iterations, password| {
        scram_build_secret(&salt, iterations, &password)
    });
    scram_seams::pg_hmac_sha256::set(|key, msg| Ok(hmac_sha256(&key, &msg)));
}
