...
use backend_storage_buffer_bufmgr::BufferManager;
...
fn pg_prewarm_impl(fcinfo: &mut FunctionCallInfoBaseData) -> Result<Datum, PgError> {
    ...
    let fork_number = backend_common_relpath::forkname_to_number(&fork_string)?;
    ...
    if relkind == RELKIND_INDEX || relkind == RELKIND_PARTITIONED_INDEX {
        priv_oid = backend_catalog_index::IndexGetRelation(rel_oid, true)?;
    ...
    let mcx = scratch.mcx();
    let rel: Relation = backend_access_common_relation::relation_open(mcx, rel_oid, AccessShareLock)?;
