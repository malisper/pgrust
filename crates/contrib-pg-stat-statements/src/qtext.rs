        ...
        match qry {
            ...
            Some(qry) => {
                ...
                if qfile.write_all(&chunk).is_err() {
                    let _ = backend_utils_hash_dynahash::hash_seq_term(&mut hash_seq);
                    return gc_fail();
                }
...
fn pgss_shmem_shutdown(code: i32, _arg: Datum<'static>) -> PgResult<()> {
    ...
    let num_entries = backend_utils_hash_dynahash::hash_get_num_entries(pgss_hash) as i32;
...
pub(crate) fn register_shutdown_dump() {
    let _ = backend_storage_ipc_dsm_core::ipc::on_shmem_exit(
        pgss_shmem_shutdown,
        Datum::from_usize(0),
    );
}
