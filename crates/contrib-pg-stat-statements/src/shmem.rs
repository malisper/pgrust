...
use types_hash::hsearch::{HASHCTL, HASH_BLOBS, HASH_ELEM, HTAB};
...
pub(crate) fn pgss_memsize() -> PgResult<usize> {
    ...
    let hash =
        backend_utils_hash_dynahash::hash_estimate_size(crate::pgss_max() as i64, core::mem::size_of::<PgssEntry>());
    size = backend_storage_ipc_shmem::add_size(size, hash)?;
    Ok(size)
}
...
pub(crate) fn install_hooks() {
    backend_utils_init_miscinit::set_shmem_request_hook(pgss_shmem_request);
    backend_storage_ipc::ipci_core::set_shmem_startup_hook(Some(pgss_shmem_startup));
}
...
fn pgss_shmem_request() -> PgResult<()> {
    ...
    let in_progress = backend_utils_init_miscinit::process_shmem_requests_in_progress();
    backend_storage_ipc::ipci_core::request_addin_shmem_space(pgss_memsize()?, in_progress)?;

    let scratch = mcx::MemoryContext::new("pgss_shmem_request");
    backend_storage_lmgr_lwlock::RequestNamedLWLockTranche(
        scratch.mcx(),
        PGSS_SHMEM_NAME,
        1,
        in_progress,
...
fn pgss_shmem_startup() -> PgResult<()> {
    ...
    let (loc, found) = backend_storage_ipc_shmem::ShmemInitStruct(
        PGSS_SHMEM_NAME,
    ...
    if !found {
        ...
        unsafe {
            let lock = backend_storage_lmgr_lwlock::named_tranche_first_lock(PGSS_SHMEM_NAME)?
            ...
            core::ptr::write(
                ...
                PgssSharedState {
                    ...
                    stats: PgssGlobalStats {
                        dealloc: 0,
                        stats_reset: backend_utils_adt_datetime::timestamp::GetCurrentTimestamp(),
                    },
                },
            );
        }
    ...
    let pgss_hash = backend_storage_ipc_shmem::ShmemInitHash(
