...
//! runs in **materialize mode** via the `types_fmgr::mat_srf` sink (the
...
use types_fmgr::mat_srf::{self, MatCell};
...
pub(crate) fn pg_stat_statements_internal(
    ...
    let is_allowed_role =
        backend_utils_adt_acl_seams::has_privs_of_role::call(userid, ROLE_PG_READ_ALL_STATS)?;
    ...
    backend_storage_lmgr_lwlock::LWLockRelease(lock_ref)?;
    Ok(())
}
...
fn push_text(row: &mut Vec<MatCell>, mcx: mcx::Mcx<'_>, s: &str) -> PgResult<()> {
    let image = backend_utils_adt_varlena::keystone::cstring_to_text(mcx, s.as_bytes())?;
    row.push(MatCell {
        value: 0,
...
fn push_numeric(row: &mut Vec<MatCell>, mcx: mcx::Mcx<'_>, s: &str) -> PgResult<()> {
    let image = backend_parser_small1_seams::numeric_in::call(mcx, s)?;
    row.push(MatCell {
        value: 0,
        ref_payload: Some(RefPayload::Varlena(image.as_slice().to_vec())),
        isnull: false,
