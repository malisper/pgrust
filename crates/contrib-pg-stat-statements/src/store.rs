...
use types_hash::hsearch::{HASHACTION, HASH_ENTER, HASH_FIND, HASH_REMOVE};
...
pub(crate) fn install_exec_hooks() {
    backend_executor_execMain_seams::set_executor_start_hook(Some(pgss_executor_start));
    backend_executor_execMain_seams::set_executor_run_hook(Some(pgss_executor_run));
    backend_executor_execMain_seams::set_executor_finish_hook(Some(pgss_executor_finish));
    backend_executor_execMain_seams::set_executor_end_hook(Some(pgss_executor_end));
}
...
fn pgss_executor_start(query_desc: &mut QueryDesc, eflags: i32) -> PgResult<()> {
    ...
    backend_executor_execMain::standard_ExecutorStart(query_desc, eflags)?;
    ...
    if pgss_enabled(nesting_level()) && query_desc.query_id() != 0 {
        ...
        EXEC_BRACKET.with(|b| {
            b.borrow_mut().push(ExecBracket {
                start_time: now(),
                start_buf: backend_executor_instrument::pgBufferUsage(),
                start_wal: backend_executor_instrument::pgWalUsage(),
                active: true,
            });
        });
    } else {
...
fn pgss_executor_run(
    ...
    let result = backend_executor_execMain::standard_ExecutorRun(query_desc, direction, count);
    NESTING_LEVEL.with(|c| c.set(c.get() - 1));
    result
}
...
fn pgss_executor_finish(query_desc: &mut QueryDesc) -> PgResult<()> {
    NESTING_LEVEL.with(|c| c.set(c.get() + 1));
    let result = backend_executor_execMain::standard_ExecutorFinish(query_desc);
    NESTING_LEVEL.with(|c| c.set(c.get() - 1));
    result
}
...
fn pgss_executor_end(query_desc: &mut QueryDesc) -> PgResult<()> {
    ...
    if let Some(bracket) = bracket {
        if query_id != 0 && bracket.active && pgss_enabled(nesting_level()) {
            ...
            backend_executor_instrument::BufferUsageAccumDiff(
                &mut bufusage,
                &backend_executor_instrument::pgBufferUsage(),
                &bracket.start_buf,
            );
            let mut walusage = WalUsage::default();
            backend_executor_instrument::WalUsageAccumDiff(
                &mut walusage,
                &backend_executor_instrument::pgWalUsage(),
                &bracket.start_wal,
            );
    ...
    backend_executor_execMain::standard_ExecutorEnd(query_desc)
}
...
pub(crate) fn pgss_store(
    ...
    let encoding = backend_utils_mb::GetDatabaseEncoding() as i32;
    ...
    let mut key = PgssHashKey {
        userid: backend_utils_init_miscinit::GetUserId(),
...
pub(crate) fn entry_alloc(
    ...
    while backend_utils_hash_dynahash::hash_get_num_entries(pgss_hash) >= pgss_max() as i64 {
        entry_dealloc();
    }
    ...
    if !found {
        ...
        e.encoding = encoding;
        e.stats_since = backend_utils_adt_datetime::timestamp::GetCurrentTimestamp();
...
fn entry_dealloc() {
    ...
    let n = backend_utils_hash_dynahash::hash_get_num_entries(pgss_hash);
    ...
    let mut hash_seq = types_hash::hsearch::HASH_SEQ_STATUS::new();
    backend_utils_hash_dynahash::hash_seq_init(&mut hash_seq, pgss_hash);
    loop {
        let ptr = match backend_utils_hash_dynahash::hash_seq_search(&mut hash_seq) {
            Ok(p) => p,
            Err(_) => break,
        };
        if ptr.is_null() {
...
pub(crate) fn entry_reset(
    ...
    let num_entries = backend_utils_hash_dynahash::hash_get_num_entries(pgss_hash);
    let stats_reset = backend_utils_adt_datetime::timestamp::GetCurrentTimestamp();
...
fn hash_search_raw(key: &PgssHashKey, action: HASHACTION) -> PgResult<(*mut u8, bool)> {
    backend_utils_hash_dynahash::hash_search(pgss_hash(), key_bytes(key), action)
}
...
fn lwlock_acquire(lock: *const types_storage::storage::LWLock, exclusive: bool) -> PgResult<()> {
    use backend_storage_lmgr_lwlock::LWLockAcquire; use types_storage::storage::LWLockMode;
    ...
    LWLockAcquire(lock, mode, backend_utils_init_small_seams::my_proc_number::call())?;
    Ok(())
}
...
fn lwlock_release(lock: *const types_storage::storage::LWLock) {
    // SAFETY: `lock` points into the live MAP_SHARED LWLock array.
    let lock = unsafe { &*lock };
    let _ = backend_storage_lmgr_lwlock::LWLockRelease(lock);
}
