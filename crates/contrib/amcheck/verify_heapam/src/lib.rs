...
use toast_internals_seams as toast_internals;
...
use visibilitymap_seams as visibilitymap;
...
pub fn verify_heapam<'mcx>(
    ...
    let rel = common_relation_seams::relation_open::call(mcx, relid, AccessShareLock)?;
    ...
    let nblocks = hio_seams::relation_get_number_of_blocks::call(relid)?;
    if nblocks == 0 {
        rel.close(AccessShareLock)?;
        return Ok(());
    }
    ...
    let (toast_rel, toast_indexes_guard): (
        ...
        let tr = table_seams::table_open::call(
            mcx,
