...
fn bt_check_every_level_inner<'mcx, 'a: 'mcx>(
    ...
    if state.heapallindexed {
        ...
        state.filter = Some(bloomfilter_seams::bloom_create::call(
            total_elems,
            work_mem,
            seed,
        )?);
        ...
        let snap = snapmgr_seams::register_snapshot::call(
            snapmgr_seams::get_transaction_snapshot::call()?,
        )?;
    ...
    if state.checkunique {
        let indexinfo = index_seams::build_index_info::call(state.mcx, rel)?;
...
pub fn heap_entry_is_visible<'mcx>(
    ...
    let mut slot =
        table_tableam::table_slot_create(state.mcx, &state.heaprel)?;
    let snapshot = state.snapshot.clone();
    let tid_visible = table_tableam::table_tuple_fetch_row_version(
        state.mcx,
        &state.heaprel,
        tid,
        &snapshot,
