...
pub(crate) fn btree_tuple_get_nkeyatts(item: &[u8], rel: &rel::Relation<'_>) -> i32 {
...
fn btree_tuple_get_natts<'mcx>(item: &[u8], rel: &rel::Relation<'mcx>) -> u16 {
    let itup = index_tuple_header(item);
    if nbtcore::tuple_is_pivot::call(item) {
...
fn index_info_is_unique(ii: &nodes::execnodes::IndexInfo<'_>) -> bool {
    ii.ii_Unique
}
...
fn bloom_add_index_tuple<'mcx>(
    ...
    bloomfilter_seams::bloom_add_element::call(filter, &bytes);
    Ok(())
}
