...
fn loc(func: &'static str) -> ErrorLocation {
    ErrorLocation::new("contrib/dblink/dblink.c", 0, func)
}
...
// the CI-gate r2/r3 abort at dblink_fetch(...,false) / dblink_get_result
// after cancel.
...
fn fc_dblink_get_connections(_flinfo: Option<&mut FmgrInfo>, fcinfo: &mut Fcinfo) -> PgResult<Datum> {
    ...
    if names.is_empty() {
        ...
        // detoasting it aborted the server mid-corpus (CI cluster gate
...
fn fc_dblink_open(flinfo: Option<&mut FmgrInfo>, fcinfo: &mut Fcinfo) -> PgResult<Datum> {
    ...
    let ok = cursor_op(&conname, |rc| {
        ...
        // (CI cluster gate r2: the myconn cursor-transactions corpus section).
