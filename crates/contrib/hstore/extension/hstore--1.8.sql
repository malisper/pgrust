/* contrib/hstore/hstore--1.8.sql */
/*
 * pgrust packaging divergence: this is a SQUASHED base install script that
 * creates the final 1.8 objects directly (mirroring pg_stat_statements). The
 * `default_version = '1.8'` control selects this base, so the upstream
 * 1.4-base + 1.4->...->1.8 upgrade chain (which uses ALTER EXTENSION ADD/DROP,
 * ALTER OPERATOR FAMILY ADD, etc. — routed to pgrust's unported
 * process_utility_slow arm) is not run. The resulting catalog state and the
 * referenced C function bodies are byte-faithful to 1.8.
 */
...
AS
	...
	OPERATOR        11      ?&(hstore,text[]) ,
        ...
        FUNCTION        10      (hstore) ghstore_options (internal),
        STORAGE         ghstore;
