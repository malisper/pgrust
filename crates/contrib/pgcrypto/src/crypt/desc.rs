//! Traditional DES crypt and BSDI extended DES (`_`, xdes), ported faithfully
//! from pgcrypto's bundled `contrib/pgcrypto/crypt-des.c` (FreeSec / David
//! Burren). This is a self-contained native Rust port — no `pwhash` crate — so
//! the salt-perturbed DES matches the C byte-for-byte, including the adversarial
//! masking behavior of `ascii_to_bin` for chars outside `./0-9A-Za-z`.
