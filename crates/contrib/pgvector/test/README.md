...
up to it. On the current tree that means `vector_type` and `hnsw_vector`
pass, while `halfvec`, `hnsw_halfvec`, `sparsevec`, `hnsw_sparsevec`, `btree`,
`cast` and `copy` fail, because `halfvec`/`sparsevec` and the b-tree/cast/copy
paths that depend on them are not ported yet — those failing tests are the
acceptance criteria for the follow-up halfvec/sparsevec PRs, not a harness
bug. So a full, argument-less run currently exits non-zero by design; pass
explicit test names (as above) to check only what should already pass.
