...
//! ([`dfmgr_seams::register_builtin_output_plugin`]) keyed by
...
use reorderbuffer_seams as rb;
use format_type_seams as fmttype;
...
fn tuple_to_stringinfo<'mcx>(
    ...
    for natt in 0..natts {
        ...
        let (origval, isnull) =
            heaptuple::heap_getattr(mcx, &formed, natt + 1, tupdesc)?;
...
fn datum_is_external_ondisk(d: &heaptuple::Datum<'_>) -> bool {
    match d {
        heaptuple::Datum::ByRef(bytes) => {
...
fn detoast_datum<'mcx>(
    mcx: mcx::Mcx<'mcx>,
    d: &heaptuple::Datum<'mcx>,
) -> PgResult<heaptuple::Datum<'mcx>> {
    match d {
        heaptuple::Datum::ByRef(bytes) => {
            let flat = detoast::detoast_attr::call(mcx, bytes)?;
            Ok(heaptuple::Datum::ByRef(flat))
        }
...
pub fn init_seams() {
    dfmgr_seams::register_builtin_output_plugin(
        dfmgr_seams::BuiltinOutputPlugin {
            name: "test_decoding",
            init,
            invoke,
        },
...
mod tests {
    ...
    fn test_decoding_builtin_resolves() {
        super::init_seams();
        let entry = dfmgr_seams::resolve_builtin_output_plugin("test_decoding")
        ...
        assert!(dfmgr_seams::resolve_builtin_output_plugin("nope").is_none());
    }
}
