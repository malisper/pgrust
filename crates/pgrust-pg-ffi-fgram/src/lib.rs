...
// so it is named explicitly as `pgrust_pg_ffi::commands_vacuumparallel::*` to
...
// via the explicit `pgrust_pg_ffi::datum` path.
pub mod datum;
...
// modules, so it is named explicitly as `pgrust_pg_ffi::dbcommands_abi::*` to
// avoid the ambiguous-glob trap.
...
// explicitly as `pgrust_pg_ffi::extension_abi::*` to avoid the ambiguous-glob
// trap.
...
// `pgrust_pg_ffi::tablecmds_abi::*` to avoid the ambiguous-glob trap.
pub mod datetime;
pub mod dynahash;
...
// `pgrust_pg_ffi::foreign_publication_parsenodes::*` to avoid the ambiguous-glob
...
// other modules.  Named explicitly as `pgrust_pg_ffi::foreign_catalog::*`.
pub mod encoding;
pub mod error;
...
// (`pgrust_pg_ffi::matview::DR_transientrel`); deliberately NOT in the
...
// path (`pgrust_pg_ffi::policy::*`); deliberately NOT in the crate-root glob to
...
// (`pgrust_pg_ffi::publication::*`); deliberately NOT in the crate-root glob to
...
// is referenced by path (`pgrust_pg_ffi::ruleutils::*`); deliberately NOT in
...
// (`pgrust_pg_ffi::selfuncs::*`); deliberately NOT in the crate-root glob to
...
// e.g. `pgrust_pg_ffi::smgr_md_abi::MdfdVec`.
pub mod smgr_md_abi;
pub mod snapshot;
pub mod sortsupport;
// `spi` (SPI + SQL-language function executor ABI) is referenced by path
// (`pgrust_pg_ffi::spi::*`); deliberately NOT in the crate-root glob to avoid
