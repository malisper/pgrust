...
impl From<crate::access::itup::IndexTupleError> for BtPageError {
...
pub fn bt_max_item_size() -> usize {
    let page_overhead = max_align(
        pgrust_storage::page::bufpage::SIZE_OF_PAGE_HEADER_DATA
            + 3 * pgrust_storage::page::bufpage::ITEM_ID_SIZE,
    ) + max_align(BTPageOpaqueData::SIZE);
    let third = (BLCKSZ - page_overhead) / 3;
    max_align(third) - max_align(std::mem::size_of::<crate::access::itemptr::ItemPointerData>())
}
...
pub fn bt_page_get_meta(page: &[u8; BLCKSZ]) -> Result<BTMetaPageData, BtPageError> {
    BTMetaPageData::decode(
        &page[pgrust_storage::page::bufpage::max_align(
            pgrust_storage::page::bufpage::SIZE_OF_PAGE_HEADER_DATA,
        )
            ..pgrust_storage::page::bufpage::max_align(
                pgrust_storage::page::bufpage::SIZE_OF_PAGE_HEADER_DATA,
            ) + BTMetaPageData::SIZE],
    )
}
...
pub fn bt_page_set_meta(page: &mut [u8; BLCKSZ], meta: BTMetaPageData) -> Result<(), BtPageError> {
    let data_start = pgrust_storage::page::bufpage::max_align(
        pgrust_storage::page::bufpage::SIZE_OF_PAGE_HEADER_DATA,
    );
...
pub fn bt_page_items(page: &[u8; BLCKSZ]) -> Result<Vec<IndexTupleData>, BtPageError> {
    let max_offset = pgrust_storage::page::bufpage::page_get_max_offset_number(page)?;
    let mut items = Vec::with_capacity(max_offset as usize);
    for offset in 1..=max_offset {
        let bytes = pgrust_storage::page::bufpage::page_get_item(page, offset)?;
...
pub fn bt_page_set_deleted(
    ...
    bt_page_set_opaque(page, opaque)?;
    let start = pgrust_storage::page::bufpage::max_align(
        pgrust_storage::page::bufpage::SIZE_OF_PAGE_HEADER_DATA,
    );
...
fn super_write_page_header(
    page: &mut [u8; BLCKSZ],
    header: pgrust_storage::page::bufpage::PageHeaderData,
) {
