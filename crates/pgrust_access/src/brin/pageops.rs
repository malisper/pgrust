...
pub fn brin_metapage_data(page: &[u8; BLCKSZ]) -> AccessResult<BrinMetaPageData> {
    ...
    let bytes = page
        ...
        .ok_or(AccessError::Corrupt("truncated BRIN metapage"))?;
    Ok(BrinMetaPageData {
...
pub fn brin_set_metapage_last_revmap_page(
    page: &mut [u8; BLCKSZ],
...
pub fn brin_page_init(page: &mut [u8; BLCKSZ], page_type: u16) -> AccessResult<()> {
    page_init(page, crate::access::brin_page::BRIN_SPECIAL_SIZE);
...
pub fn brin_page_get_exact_freespace(page: &[u8; BLCKSZ]) -> AccessResult<usize> {
    Ok(page_header(page).map_err(page_error)?.free_space())
}

pub fn brin_page_get_freespace(page: &[u8; BLCKSZ]) -> AccessResult<usize> {
...
pub fn brin_can_do_samepage_update(
    page: &[u8; BLCKSZ],
    original_size: usize,
    new_size: usize,
) -> AccessResult<bool> {
...
pub fn brin_regular_page_add_item(
    page: &mut [u8; BLCKSZ],
    item: &[u8],
) -> AccessResult<OffsetNumber> {
...
pub fn page_index_tuple_delete_no_compact(
    page: &mut [u8; BLCKSZ],
    offnum: OffsetNumber,
) -> AccessResult<()> {
    ...
    if offnum == 0 || offnum > max_offset {
        return Err(AccessError::Corrupt("invalid BRIN tuple offset"));
    }
    ...
    {
        return Err(AccessError::Corrupt("corrupted BRIN line pointer"));
    }

    if offnum < max_offset {
        item_id = ItemIdData::unused();
        let idx = max_align(pgrust_storage::page::bufpage::SIZE_OF_PAGE_HEADER_DATA)
...
pub fn page_index_tuple_overwrite(
    page: &mut [u8; BLCKSZ],
    offnum: OffsetNumber,
    new_tuple: &[u8],
) -> AccessResult<bool> {
    ...
    if size_diff != 0 {
        ...
        let target = upper
            ...
            .ok_or_else(|| AccessError::Corrupt("BRIN overwrite moved page upper out of range"))?;
        ...
        write_page_upper(
            ...
            header
                ...
                .ok_or(AccessError::Corrupt("invalid BRIN page upper adjustment"))?,
        );
        ...
        for index in 1..=max_offset {
            ...
            if existing.has_storage() && usize::from(existing.lp_off) <= offset {
                existing.lp_off = existing
                    .lp_off
                    .checked_add_signed(size_diff as i16)
                    .ok_or(AccessError::Corrupt("invalid BRIN tuple offset adjustment"))?;
                let idx = max_align(pgrust_storage::page::bufpage::SIZE_OF_PAGE_HEADER_DATA)
                    + (usize::from(index) - 1) * ITEM_ID_SIZE;
                page[idx..idx + ITEM_ID_SIZE].copy_from_slice(&existing.encode());
            }
    ...
    let new_offset = (offset as u16)
        .checked_add_signed(size_diff as i16)
        .ok_or(AccessError::Corrupt("invalid BRIN overwrite tuple offset"))?;
...
pub fn brin_page_start_evacuating(page: &mut [u8; BLCKSZ]) -> AccessResult<bool> {
...
fn page_error(err: PageError) -> AccessError {
    AccessError::Scalar(format!("BRIN page error: {err:?}"))
}
