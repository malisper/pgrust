use crate::access::brin_internal::BrinTupleLocation;
...
pub fn brin_revmap_initialize(index_pages: &[[u8; BLCKSZ]]) -> AccessResult<BrinRevmap> {
    let metapage = index_pages
        .get(BRIN_METAPAGE_BLKNO as usize)
        .ok_or(AccessError::Corrupt("missing BRIN metapage"))?;
    let metadata = brin_metapage_data(metapage)?;
    if metadata.brin_magic != BRIN_META_MAGIC {
        return Err(AccessError::Corrupt("invalid BRIN metapage magic"));
    }
...
pub fn brin_revmap_extend(
    ...
    while target_block > revmap.last_revmap_page {
        ...
        if page_header(&index_pages[map_block as usize]).is_ok()
            && crate::access::brin_page::brin_page_type(&index_pages[map_block as usize])
                .map_err(page_error)?
...
pub fn brin_revmap_get_location(
    index_pages: &[[u8; BLCKSZ]],
    revmap: &BrinRevmap,
    heap_blk: u32,
) -> AccessResult<BrinTupleLocation> {
    let heap_blk = normalize_range_start(revmap.pages_per_range, heap_blk);
    ...
    let page = index_pages
        ...
        .ok_or(AccessError::Corrupt("missing BRIN revmap page"))?;
    if crate::access::brin_page::brin_page_type(page).map_err(page_error)? != BRIN_PAGETYPE_REVMAP {
        return Err(AccessError::Corrupt("BRIN revmap page has wrong page type"));
    }
...
pub fn brin_revmap_set_location(
    index_pages: &mut Vec<[u8; BLCKSZ]>,
    revmap: &mut BrinRevmap,
    heap_blk: u32,
...
pub fn brin_revmap_desummarize_range(
    index_pages: &mut Vec<[u8; BLCKSZ]>,
    revmap: &mut BrinRevmap,
    heap_blk: u32,
) -> AccessResult<bool> {
    ...
    let page = index_pages
        ...
        .ok_or(AccessError::Corrupt("BRIN tuple block missing"))?;
    if crate::access::brin_page::brin_page_type(page).map_err(page_error)? != BRIN_PAGETYPE_REGULAR
    {
        return Err(AccessError::Corrupt(
    ...
    if location.offset == 0 || location.offset > max_offset {
        return Err(AccessError::Corrupt("corrupted BRIN revmap tuple offset"));
    }
    ...
    if item_id.lp_flags == ItemIdFlags::Unused || !item_id.has_storage() {
        return Err(AccessError::Corrupt("corrupted BRIN revmap tuple pointer"));
    }
...
pub fn brin_revmap_get_tuple_bytes<'a>(
    index_pages: &'a [[u8; BLCKSZ]],
    revmap: &BrinRevmap,
    heap_blk: u32,
) -> AccessResult<Option<(BrinTupleLocation, &'a [u8])>> {
    let location = brin_revmap_get_location(index_pages, revmap, heap_blk)?;
    if !location.is_valid() {
        return Ok(None);
    }
...
fn revmap_read_entry(page: &[u8; BLCKSZ], index: usize) -> AccessResult<BrinTupleLocation> {
    if index >= REVMAP_PAGE_MAXITEMS {
        return Err(AccessError::Corrupt("BRIN revmap index out of range"));
    }
...
fn evacuate_regular_page(
    index_pages: &mut Vec<[u8; BLCKSZ]>,
    source_block: u32,
) -> AccessResult<Vec<(u32, BrinTupleLocation)>> {
...
fn insert_regular_tuple(
    ...
    let required_size = pgrust_storage::page::bufpage::max_align(bytes.len());
    let mut block = minimum_block;
    loop {
        ensure_page_slot(index_pages, block);
        if let Err(pgrust_storage::page::bufpage::PageError::NotInitialized) =
            page_header(&index_pages[block as usize])
        {
            brin_page_init(&mut index_pages[block as usize], BRIN_PAGETYPE_REGULAR)?;
        }

        if crate::access::brin_page::brin_page_type(&index_pages[block as usize])
            .map_err(page_error)?
            != BRIN_PAGETYPE_REGULAR
        {
            block += 1;
...
fn page_error(err: pgrust_storage::page::bufpage::PageError) -> AccessError {
