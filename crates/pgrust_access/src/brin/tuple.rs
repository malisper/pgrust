use pgrust_catalog_data::pg_opfamily::{
...
use crate::access::brin_internal::{BrinDesc, BrinMemTuple, BrinOpcInfo, BrinTupleBytes};
use crate::access::brin_tuple::{
...
use crate::access::htup::{HeapTuple, TupleValue};
use crate::varatt::{TOAST_POINTER_SIZE, is_compressed_inline_datum, is_ondisk_toast_pointer};
...
pub fn brin_opfamily_is_minmax_multi(opfamily_oid: Option<u32>) -> bool {
    matches!(
        opfamily_oid,
        Some(
...
pub fn brin_build_desc(index_desc: &RelationDesc) -> BrinDesc {
    brin_build_desc_with_meta(index_desc, None)
}

pub fn brin_build_desc_with_meta(
...
pub fn brin_disk_tupdesc(desc: &BrinDesc) -> RelationDesc {
...
pub fn brin_form_tuple(
    ...
    for (column_index, column) in tuple.columns.iter().enumerate() {
        ...
        if column.values.len() != info.nstored {
            return Err(AccessError::Corrupt("BRIN value count mismatch"));
        }
        for value in &column.values {
            values.push(services.encode_value(disk_column, value)?);
        }
    }
    ...
    let heap_tuple = HeapTuple::from_values(&disk_desc.attribute_descs(), &values)
        .map_err(|err| AccessError::Unsupported(format!("{err:?}")))?;
    ...
    let total_len = max_align(header_size + heap_tuple.data.len());
...
pub fn brin_form_placeholder_tuple(
    ...
    brin_form_tuple(desc, &tuple, services)
}
    ...
    for (index, attr) in attr_descs.iter().enumerate() {
        ...
        match attr.attlen {
            len if len > 0 => {
                ...
                let datum = data
                    .get(raw_offset..end)
                    .ok_or(AccessError::Corrupt("truncated BRIN fixed-width datum"))?;
            ...
            -1 => {
                let slice = data
                    .get(raw_offset..)
                    .ok_or(AccessError::Corrupt("truncated BRIN varlena datum"))?;
                if is_ondisk_toast_pointer(slice) {
                    let end = raw_offset + TOAST_POINTER_SIZE;
                    let datum = data
                        .get(raw_offset..end)
                        .ok_or(AccessError::Corrupt("truncated BRIN toast pointer"))?;
                    values.push(Some(datum));
                    raw_offset = end;
                    ...
                    let datum = data
                        ...
                        .ok_or(AccessError::Corrupt("truncated BRIN short varlena datum"))?;
                    values.push(Some(datum));
                    raw_offset = end;
                } else {
                    raw_offset = attr.attalign.align_offset(raw_offset);
                    let header = data
                        ...
                        .ok_or(AccessError::Corrupt("truncated BRIN varlena header"))?;
            ...
            -2 => {
                ...
                while data
                    .get(raw_offset)
                    .copied()
                    .ok_or(AccessError::Corrupt("unterminated BRIN cstring datum"))?
                    != 0
                {
                    raw_offset += 1;
                }
...
pub fn brin_deform_tuple(
    ...
    tuple_bytes: &[u8],
    services: &dyn AccessScalarServices,
) -> AccessResult<BrinMemTuple> {
    if tuple_bytes.len() < BrinTuple::SIZE {
        return Err(AccessError::Corrupt("truncated BRIN tuple"));
    }
    ...
    if header_size < BrinTuple::SIZE || header_size > tuple_bytes.len() {
        return Err(AccessError::Corrupt("invalid BRIN tuple data offset"));
    }
    let null_bitmap = if has_nulls {
        let len = brin_null_bitmap_len(natts);
        tuple_bytes
            .get(BrinTuple::SIZE..BrinTuple::SIZE + len)
            .ok_or(AccessError::Corrupt("truncated BRIN null bitmap"))?
    } else {
        &[][..]
    };
    ...
    for attno in 0..natts {
        ...
        for value_index in 0..count {
            let value = services
                .decode_value(&disk_desc.columns[stored_index], raw_values[stored_index])?;
...
pub fn brin_tuple_bytes_equal(left: &[u8], right: &[u8]) -> bool {
    left == right
}
