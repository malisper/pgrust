...
fn validate_brin_catalog(
    ...
    for opclass in opclasses.iter().filter(|row| row.opcmethod == BRIN_AM_OID) {
        ...
        // generic minmax support path. Other PostgreSQL-compatible BRIN
        // operator classes are catalog-visible before their runtimes exist.
        if !opclass.opcname.ends_with("_minmax_ops") {
            continue;
        }
...
mod tests {
    use super::*;
    use pgrust_catalog_data::{BRIN_INTEGER_MINMAX_FAMILY_OID, INT4_TYPE_OID};
    ...
    fn validate_brin_rejects_missing_required_proc() {
        ...
        amprocs.retain(|row| {
            !(row.amprocnum == BRIN_PROCNUM_STRATEGY_LT
                && row.amprocfamily == BRIN_INTEGER_MINMAX_FAMILY_OID
                && row.amproclefttype == INT4_TYPE_OID)
        });
