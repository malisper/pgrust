...
use pgrust_core::{XLOG_GIST_INSERT, XLOG_GIST_PAGE_UPDATE, XLOG_GIST_SPLIT};
...
fn gist_insert_mutex() -> &'static Mutex<()> {
    static GIST_INSERT_MUTEX: OnceLock<Mutex<()>> = OnceLock::new();
    GIST_INSERT_MUTEX.get_or_init(|| Mutex::new(()))
}
...
impl From<AccessError> for GistWriteError {
...
pub fn gistinsert(
    ...
    let _guard = gist_insert_mutex()
        .lock()
        .map_err(|_| AccessError::Scalar("gist insert mutex poisoned".into()))?;
    ...
    let new_entry = GistTupleEntry {
        tuple: make_leaf_tuple(&ctx.index_desc, &ctx.values, ctx.heap_tid, scalar)?,
...
pub(super) fn load_page_entries(
    ...
    scalar: &dyn AccessScalarServices,
) -> Result<Vec<GistTupleEntry>, AccessError> {
    gist_page_items(page)
        .map_err(|err| AccessError::Scalar(format!("gist tuple parse failed: {err:?}")))?
        .into_iter()
        .map(|tuple| {
            let values = decode_tuple_values(desc, &tuple, scalar)?;
...
pub(super) fn choose_child(
    ...
    for (index, item) in items.iter().enumerate() {
        ...
        let merged_size = tuple_storage_size(desc, &merged, state.scalar_services())?;
        if best
            .as_ref()
...
fn try_append_leaf_entries(
    ...
    mode: GistPageWriteMode,
    wal: &dyn AccessWalServices,
) -> Result<Option<InsertOutcome>, AccessError> {
    ...
    for entry in new_entries {
        match gist_page_append_tuple(&mut appended, &entry.tuple) {
            Ok(_) => {}
            Err(crate::access::gist::GistPageError::Page(PageError::NoSpace)) => {
                return Ok(None);
            }
            Err(err) => {
...
fn try_write_page(
    ...
    rel: pgrust_storage::RelFileLocator,
    block: u32,
    opaque: crate::access::gist::GistPageOpaqueData,
    items: &[GistTupleEntry],
    wal_info: u8,
    mode: GistPageWriteMode,
    wal: &dyn AccessWalServices,
    ...
    gist_page_replace_items(&mut rebuilt, &tuples, opaque).map_err(|err| match err {
        ...
        other => GistWriteError::Catalog(AccessError::Scalar(format!(
            "gist page rebuild failed: {other:?}"
        ))),
    })?;
    write_buffered_page_with_mode(
        pool, client_id, xid, rel, block, &rebuilt, wal_info, mode, wal,
    )
    .map_err(Into::into)
}
...
fn split_page_entries(
    items: &[GistTupleEntry],
    split: &GistPageSplit,
) -> Result<(Vec<GistTupleEntry>, Vec<GistTupleEntry>), AccessError> {
...
fn page_entries_can_fit(items: &[GistTupleEntry], flags: u16) -> Result<bool, AccessError> {
    let tuples = items
        .iter()
        .map(|item| item.tuple.clone())
        .collect::<Vec<_>>();
    ...
    match try_write_page(
        ...
        Err(GistWriteError::NoSpace) => {
            ...
            if is_root {
                ...
                let mut left_page = [0u8; pgrust_storage::BLCKSZ];
                let mut right_page = [0u8; pgrust_storage::BLCKSZ];
                let mut root_page = [0u8; pgrust_storage::BLCKSZ];
                let root_items = vec![
                    GistTupleEntry {
                ...
                .map_err(|err| {
                    AccessError::Scalar(format!("gist split rebuild failed: {err:?}"))
                })?;
                gist_page_replace_items(
                    &mut right_page,
                    &right_items
...
pub(super) fn insert_entries_into_block(
    ...
    if new_entries.is_empty() {
        ...
        let items = load_page_entries(desc, &page, state.scalar_services())?;
        let union = state.union_all(
            &items
                .iter()
                .map(|item| item.values.clone())
    ...
    if opaque.is_leaf() {
        ...
        let mut items = load_page_entries(desc, &page, state.scalar_services())?;
        items.extend(new_entries);
        return write_or_split_page(
            pool, client_id, xid, rel, desc, state, block, opaque, items, is_root, mode, wal,
        );
    }
