...
use crate::access::relscan::{
    ...
    IndexScanDescData, IndexScanOpaque, ScanDirection,
};
...
fn reset_scan(scan: &mut IndexScanDesc) -> Result<(), AccessError> {
    let IndexScanOpaque::Gist(state) = &mut scan.opaque else {
        return Err(AccessError::Corrupt("GiST scan state missing opaque"));
    };
    ...
    state.search_queue.push(GistSearchItem {
        kind: GistSearchItemKind::Page {
            block: crate::access::gist::GIST_ROOT_BLKNO,
            parent_lsn: 0,
        },
        distances: Vec::new(),
        ordinal,
...
pub fn gistbeginscan(ctx: &IndexBeginScanContext) -> Result<IndexScanDesc, AccessError> {
...
fn tuple_matches_scan_keys(
    state: &GistState,
    tuple_values: &[pgrust_nodes::datum::Value],
    keys: &[ScanKeyData],
    is_leaf: bool,
) -> Result<(bool, bool), AccessError> {
...
fn tuple_order_distances(
    ...
    keys: &[ScanKeyData],
    is_leaf: bool,
) -> Result<(Vec<IndexOrderByDistance>, bool), AccessError> {
...
fn scan_page(
    ...
    let gist_state = GistState::new(&scan.index_desc, &scan.index_meta, scalar)?;
    ...
    for (_, tuple) in tuples.into_iter().rev() {
        ...
        if opaque.is_leaf() {
            ...
            let child_block = gist_downlink_block(&tuple).ok_or(AccessError::Corrupt(
...
pub fn gistgettuple(
    ...
    loop {
        ...
        match item.kind {
            GistSearchItemKind::Page { block, parent_lsn } => {
                scan_page(scan, block, parent_lsn, scalar)?
            }
...
pub fn gistgetbitmap(
    ...
    let mut count = 0_i64;
    while gistgettuple(scan, scalar)? {
...
pub fn gistendscan(_scan: IndexScanDesc) -> Result<(), AccessError> {
    Ok(())
}
