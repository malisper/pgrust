use pgrust_nodes::datum::{GeoBox, GeoCircle, GeoPoint, GeoPolygon, Value};
...
fn bounding_box(value: &Value) -> AccessResult<GeoBox> {
    match value {
...
fn exact_polygon_consistent(
    ...
    Some(match strategy {
        3 => services.polygon_overlap(key, query),
        6 => services.polygon_same(key, query),
        7 => services.polygon_contains_polygon(key, query),
        8 => services.polygon_contains_polygon(query, key),
        _ => return None,
    })
}
...
pub fn consistent(
    ...
    if !is_leaf {
        return Ok(GistConsistentResult {
            matches: box_consistent(strategy, &key_box, &query_box, false, services),
            recheck: false,
        });
    }
    ...
    let exact = match (kind, key, query) {
        (GeometryKind::Polygon, Value::Polygon(key), Value::Polygon(query)) => {
            exact_polygon_consistent(strategy, key, query, services)
        }
    ...
    let matches =
        exact.unwrap_or_else(|| box_consistent(strategy, &key_box, &query_box, true, services));
...
pub fn union(
    ...
    for value in iter {
        out = services.bound_box(&out, &bounding_box(value)?);
    }
    ...
    if right.is_empty() {
        ...
        left_union_box = left
            ...
            .fold(None, |acc: Option<GeoBox>, index| {
                ...
                Some(match acc {
                    Some(acc) => services.bound_box(&acc, &current),
                    None => current,
                })
            })
...
pub fn same(
    ...
    match (left, right) {
        ...
        (Value::Polygon(left), Value::Polygon(right)) => Ok(services.polygon_same(left, right)),
        (Value::Circle(left), Value::Circle(right)) => Ok(circle_same(left, right)),
        _ => Ok(services.box_same(&bounding_box(left)?, &bounding_box(right)?)),
    }
}
