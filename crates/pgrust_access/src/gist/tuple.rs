...
use crate::access::gist::{gist_make_downlink_tid, gist_make_leaf_tid};
...
pub fn decode_key_payload(
    ...
    if payload.len() < 2 {
        return Err(AccessError::Corrupt("gist tuple payload too short"));
    }
    ...
    for column in desc.columns.iter().take(count) {
        if offset + 5 > payload.len() {
            return Err(AccessError::Corrupt("gist tuple payload truncated"));
        }
        ...
        let len = u32::from_le_bytes(
            payload[offset..offset + 4]
                .try_into()
                .map_err(|_| AccessError::Corrupt("gist tuple payload length"))?,
        ) as usize;
        offset += 4;
        if is_null {
        ...
        if offset + len > payload.len() {
            return Err(AccessError::Corrupt("gist tuple payload overflow"));
        }
...
pub fn make_leaf_tuple(
    ...
    make_index_tuple(desc, values, gist_make_leaf_tid(heap_tid), services)
}

pub fn make_downlink_tuple(
    desc: &RelationDesc,
    values: &[Value],
    child_block: u32,
    services: &dyn AccessScalarServices,
) -> AccessResult<IndexTupleData> {
    make_index_tuple(desc, values, gist_make_downlink_tid(child_block), services)
}
