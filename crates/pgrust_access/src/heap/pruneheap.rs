...
use pgrust_core::{
    FROZEN_TRANSACTION_ID, TransactionId, TransactionStatus, transaction_id_is_normal,
};
...
pub fn classify_tuple(
    ...
    match txns.transaction_status(tuple.header.xmax) {
...
fn tuple_xmin_committed(tuple: &HeapTuple, txns: &dyn AccessTransactionServices) -> bool {
    tuple.header.xmin == FROZEN_TRANSACTION_ID
        || tuple.header.infomask & HEAP_XMIN_COMMITTED != 0
        || matches!(
            txns.transaction_status(tuple.header.xmin),
            Some(TransactionStatus::Committed)
        )
}

fn tuple_xmin_aborted(tuple: &HeapTuple, txns: &dyn AccessTransactionServices) -> bool {
...
fn tuple_xmax_invalid(tuple: &HeapTuple, txns: &dyn AccessTransactionServices) -> bool {
    tuple.header.xmax == 0
        || tuple.header.infomask & HEAP_XMAX_INVALID != 0
        || matches!(
            txns.transaction_status(tuple.header.xmax),
            Some(TransactionStatus::Aborted)
        )
}
