...
use crate::index::unique::keys_contain_null;
use crate::nbtree::tuple as access_tuple;
use crate::{
    AccessError, AccessHeapServices, AccessInterruptServices, AccessResult, AccessScalarServices,
    AccessTransactionServices, AccessWalBlockRef, AccessWalRecord, AccessWalServices,
};
...
use crate::index::unique::{
    UniqueCandidateContext, UniqueCandidateResult, classify_unique_candidate,
};
use crate::nbtree::nbtcompare::{
    BT_DESC_FLAG, compare_bt_values_with_options_and_services, compare_item_pointers,
};
use crate::nbtree::nbtpreprocesskeys::preprocess_scan_keys;
use crate::nbtree::nbtsplitloc::choose_split_index;
use crate::nbtree::nbtutils::BtSortTuple;
use pgrust_core::{
    PAGE_SIZE, REGBUF_FORCE_IMAGE, REGBUF_STANDARD, REGBUF_WILL_INIT, RM_BTREE_ID, TransactionId,
    TransactionStatus, XLOG_BTREE_INSERT_LEAF, XLOG_BTREE_INSERT_META, XLOG_BTREE_INSERT_UPPER,
...
use pgrust_storage::fsm::get_free_index_page;
use pgrust_storage::include::storage::buf_internals::Page;
...
use pgrust_storage::{
    BufferPool, BufferTag, ClientId, OwnedBufferPin, PinnedBuffer, SmgrStorageBackend,
};

type CatalogError = AccessError;

fn check_insert_split_interrupts(
...
pub(crate) fn log_btree_record(
    ...
    let mut wal_blocks = Vec::with_capacity(blocks.len());
    for block in blocks {
        ...
        wal_blocks.push(AccessWalBlockRef {
            tag: block.tag,
            flags,
            data: if block.force_image {
                block.page.to_vec()
            } else {
                Vec::new()
            },
            buffer_data: block.data.to_vec(),
        });
    }
...
pub fn encode_key_payload(
    ...
    access_tuple::encode_key_payload(desc, values, default_toast_compression, scalar)
}
...
pub fn decode_key_payload(
    ...
    access_tuple::decode_key_payload(desc, payload, scalar)
}
...
fn compare_bt_values_for_type(
    left: &Value,
    right: &Value,
    ty: pgrust_nodes::SqlType,
    scalar: &dyn AccessScalarServices,
) -> Ordering {
    compare_bt_values_for_type_with_option(left, right, ty, 0, scalar)
}
...
fn compare_bt_values_for_type_with_option(
    ...
    if matches!(ty.kind, pgrust_nodes::SqlTypeKind::InternalChar)
        && !ty.is_array
            ...
            (internal_char_cmp_key(left), internal_char_cmp_key(right))
    {
        ...
        if option & crate::nbtree::nbtcompare::BT_DESC_FLAG != 0 {
            ord = ord.reverse();
        }
        return ord;
    }
    if matches!(
        ty.kind,
        pgrust_nodes::SqlTypeKind::Int2Vector | pgrust_nodes::SqlTypeKind::OidVector
    ...
    if matches!(ty.kind, pgrust_nodes::SqlTypeKind::Char)
        && !ty.is_array
    ...
    compare_bt_values_with_options_and_services(left, right, option, scalar)
        .expect("btree comparison should be scalar-service compatible")
}
...
fn compare_key_arrays_with_columns_and_options(
    ...
    for (idx, (left, right)) in left.iter().zip(right.iter()).enumerate() {
        ...
        let ord = columns
            .get(idx)
            .map(|column| {
                compare_bt_values_for_type_with_option(left, right, column.sql_type, option, scalar)
            })
            .unwrap_or_else(|| {
                compare_bt_values_with_options_and_services(left, right, option, scalar)
                    .expect("btree comparison should be scalar-service compatible")
...
fn btree_key_count(index_meta: &IndexRelCacheEntry) -> usize {
...
fn compare_key_arrays_with_columns(
    ...
    compare_key_arrays_with_columns_and_options(columns, left, right, &[], scalar)
}
...
fn compare_bt_keyspace_with_columns_and_options(
    ...
    let ord = compare_key_arrays_with_columns_and_options(
        columns, left_keys, right_keys, indoption, scalar,
    );
    if ord != Ordering::Equal {
        return ord;
    }
...
fn tuple_key_values(
    ...
    decode_key_payload(desc, &tuple.payload, scalar)
}
...
fn tuple_key_prefix_values(
    ...
    let mut values = tuple_key_values(desc, tuple, scalar)?;
...
fn pivot_tuple(
    ...
    Ok(IndexTupleData::new_raw(
        ...
        false,
        false,
        encode_key_payload(desc, key_values, default_toast_compression, scalar)?,
    ))
}
...
fn tuple_matches_scan_keys(
    ...
    let values = decode_key_payload(desc, &tuple.payload, scalar)?;
    for key in keys {
        if key.attribute_number == 0 {
            if !tuple_matches_row_scan_key(desc, &values, key, scalar)? {
                return Ok(false);
            }
            continue;
        }
...
fn value_matches_scan_key_strategy(
        ...
        && let Some(array) = argument.as_array_value()
    {
        ...
        return array.elements.iter().any(|element| {
            ...
            let ord =
                compare_bt_values_for_type_with_option(value, element, sql_type, option, scalar);
            ord == Ordering::Equal
        });
    }
    ...
    let ord = compare_bt_values_for_type_with_option(value, argument, sql_type, option, scalar);
...
fn tuple_matches_row_scan_key(
    ...
    for (idx, right) in record.fields.iter().enumerate() {
        ...
        let ord = compare_bt_values_for_type(left, right, column.sql_type, scalar);
        if ord == Ordering::Equal {
            continue;
        }
...
fn page_fillfactor_reserve(fillfactor: u16) -> usize {
    pgrust_storage::smgr::BLCKSZ * (100usize.saturating_sub(fillfactor as usize)) / 100
}
...
fn append_page_or_finish(
    ...
    let mut page = [0u8; pgrust_storage::smgr::BLCKSZ];
...
fn read_buffered_page(
    ...
    rel: RelFileLocator,
    block: u32,
) -> Result<[u8; pgrust_storage::smgr::BLCKSZ], CatalogError> {
...
fn write_buffered_btree_page(
    ...
    rel: RelFileLocator,
    block: u32,
    page: &[u8; pgrust_storage::smgr::BLCKSZ],
    wal_info: u8,
    wal: &dyn AccessWalServices,
) -> Result<(), CatalogError> {
    write_buffered_btree_page_with_init(
        pool, client_id, xid, rel, block, page, wal_info, false, wal,
    )
}
...
fn write_buffered_btree_page_with_init(
    ...
    let lsn = log_btree_record(
        ...
        &[LoggedBtreeBlock {
            block_id: 0,
            tag: pgrust_storage::buffer::BufferTag {
                rel,
...
fn write_meta_page(
    ...
    let mut metapage = [0u8; pgrust_storage::smgr::BLCKSZ];
...
fn page_lower_bound(
    ...
    tuple_key_prefix_values(desc, tuple, key_count, scalar)
}
...
fn page_first_key_values(
    ...
    tuple_key_prefix_values(desc, tuple, key_count, scalar).map(Some)
}
...
pub(crate) fn ensure_empty_btree(
    ...
    write_meta_page(pool, client_id, xid, rel, 1, 0, wal)?;
    ...
    write_buffered_btree_page_with_init(pool, client_id, xid, rel, 1, &root, XLOG_FPI, true, wal)?;
    Ok(())
}
...
fn write_page_header_fast(page: &mut [u8; pgrust_storage::smgr::BLCKSZ], header: PageHeaderData) {
...
fn write_index_tuple_at(
    page: &mut [u8; pgrust_storage::smgr::BLCKSZ],
    offset: usize,
    tuple: &IndexTupleData,
) {
...
fn mark_root_block(
    ...
    bt_page_set_opaque(&mut page, opaque)
        .map_err(|err| CatalogError::Io(format!("btree opaque write failed: {err:?}")))?;
    write_buffered_btree_page(pool, client_id, xid, rel, block, &page, XLOG_FPI, wal)
}
...
fn build_btree_pages(
    ...
    let mut current = build_leaf_pages(ctx, tuples, &mut next_block, scalar, wal)?;
    while current.len() > 1 {
        current = build_internal_level(ctx, current, &mut next_block, scalar, wal)?;
    }
    let root = current
        .first()
...
fn check_unique_build(
    ...
    for tuple in tuples {
        ...
        if last.is_some_and(|prev| {
            compare_key_arrays_with_columns(columns, prev, tuple_keys, scalar) == Ordering::Equal
        }) {
...
fn format_unique_build_key_detail(
    columns: &[ColumnDesc],
    values: &[Value],
    scalar: &dyn AccessScalarServices,
) -> String {
    scalar
        ...
        .unwrap_or_else(|| scalar.format_unique_key_detail(columns, values))
}

pub fn btbuild_projected(
    ctx: &IndexBuildContext,
    heap_tuples: u64,
    projected: Vec<(ItemPointerData, Vec<Value>)>,
    scalar: &dyn AccessScalarServices,
    wal: &dyn AccessWalServices,
) -> Result<IndexBuildResult, CatalogError> {
    let mut spool = crate::nbtree::nbtsort::BtSpool::default();
    ...
    for (tid, all_values) in projected {
    ...
    build_btree_pages(ctx, tuples, scalar, wal)?;
    Ok(result)
}
...
fn choose_child_slot(
    ...
    for (idx, tuple) in items.iter().enumerate() {
        let key = tuple_key_prefix_values(desc, tuple, key_count, scalar)?;
            ...
            match direction {
                ScanDirection::Forward if matches!(candidate.strategy, 4 | 5) => {
                    ...
                    if chosen.as_ref().is_none_or(|current| {
                        compare_bt_values_with_options_and_services(
                            current,
                            &candidate.argument,
                            0,
                            scalar,
...
fn tuple_prefix_cmp(
    ...
    let values = tuple_key_values(desc, tuple, scalar)?;
...
fn empty_leaf_exhausts_exact_equality_scan(
    ...
    match scan.direction {
        ScanDirection::Forward => {
            ...
            Ok(
                tuple_prefix_cmp(&scan.index_desc, last, &target, &scan.indoption, scalar)?
                    != Ordering::Less,
            )
        }
        ScanDirection::Backward => {
            let first = items
                ...
                .expect("items is not empty when checking equality scan bounds");
            Ok(
                tuple_prefix_cmp(&scan.index_desc, first, &target, &scan.indoption, scalar)?
                    != Ordering::Greater,
            )
        }
    }
...
fn read_page_items(
    ...
    rel: RelFileLocator,
    block: u32,
) -> Result<(Vec<IndexTupleData>, crate::access::nbtree::BTPageOpaqueData), CatalogError> {
    let page = read_page(pool, rel, block)?;
...
fn find_leaf_with_ancestors(
    ...
    while level > 0 {
        ...
        let slot = choose_child_slot(
            desc, &items, target, direction, key_count, indoption, scalar,
        )?;
...
fn leaf_upper_bound(
    ctx: &IndexInsertContext,
    page: &[u8; pgrust_storage::smgr::BLCKSZ],
    opaque: crate::access::nbtree::BTPageOpaqueData,
    scalar: &dyn AccessScalarServices,
) -> Result<Option<Vec<Value>>, CatalogError> {
...
type LiveRightLeaf = (
    u32,
    Page,
    crate::access::nbtree::BTPageOpaqueData,
...
fn left_sibling_may_contain_key(
    ctx: &IndexInsertContext,
    left_page: &Page,
    left_opaque: crate::access::nbtree::BTPageOpaqueData,
    right_block: u32,
...
fn find_parent_from_stack(
    ...
    loop {
        ...
        if opaque.btpo_flags & BTP_INCOMPLETE_SPLIT != 0 {
            ...
            ancestor_stack.pop();
            finish_incomplete_split(ctx, entry.block, &ancestor_stack, runtime, scalar, wal)?;
            continue;
        }
    ...
    loop {
        ...
        while level > 0 {
            ...
            let opaque = bt_page_get_opaque(&page)
                .map_err(|err| CatalogError::Io(format!("btree opaque read failed: {err:?}")))?;
            if opaque.btpo_flags & BTP_INCOMPLETE_SPLIT != 0 {
                finish_incomplete_split(ctx, block, &parent_stack, runtime, scalar, wal)?;
                break;
            }
            let items = bt_page_data_items(&page)
                .map_err(|err| CatalogError::Io(format!("btree page parse failed: {err:?}")))?;
        ...
        loop {
            ...
            let Some(upper_bound) = leaf_upper_bound(ctx, &page, opaque, scalar)? else {
...
fn find_locked_unique_insert_path<'a>(
    ...
    'search: loop {
        let search = find_leaf_for_insert(ctx, key_values, runtime, scalar, wal)?;
        ...
        loop {
            ...
            if opaque.btpo_flags & BTP_INCOMPLETE_SPLIT != 0 {
                drop(guard);
                drop(pin);
                finish_incomplete_split(ctx, block, &[], runtime, scalar, wal)?;
                continue 'search;
            }
            if opaque.btpo_flags & BTP_DELETED != 0 {
            ...
            if let Some(upper_bound) = leaf_upper_bound(ctx, &page, opaque, scalar)?
            ...
            if opaque.btpo_prev != P_NONE {
                ...
                if left_opaque.btpo_flags & BTP_INCOMPLETE_SPLIT != 0 {
                    drop(guard);
                    drop(pin);
                    finish_incomplete_split(ctx, left_block, &[], runtime, scalar, wal)?;
                    continue 'search;
                }
...
fn initial_scan_block(
    ...
    let target = scan_positioning_prefix(&scan.key_data, scan.direction, scalar);
    ...
    loop {
        ...
        if neighbor == P_NONE || !leaf_has_match(scan, neighbor, scalar)? {
            break;
        }
        block = neighbor;
    }
...
fn load_leaf_items(
    ...
    let items = bt_page_data_items(&guard)
        .map_err(|err| CatalogError::Io(format!("btree page parse failed: {err:?}")))?;
    drop(guard);
    let stop_after_empty = empty_leaf_exhausts_exact_equality_scan(scan, &items, scalar)?;
    let filtered = items
        .into_iter()
        .filter(|tuple| {
            tuple_matches_scan_keys(
                &scan.index_desc,
                tuple,
                &scan.key_data,
                &scan.indoption,
...
pub fn btbeginscan(
    ...
    let mut scan = crate::index::genam::index_beginscan_stub(ctx)?;
    let start_block = initial_scan_block(&scan, scalar)?;
...
pub fn btrescan(
    ...
    crate::index::genam::index_rescan_stub(scan, keys, direction)?;
    let start_block = initial_scan_block(scan, scalar)?;
    if let IndexScanOpaque::Btree(state) = &mut scan.opaque {
        state.current_block = start_block;
        state.current_pin = None;
...
pub fn btgettuple(
    ...
    loop {
        ...
        if needs_load {
            if !load_leaf_items(scan, scalar)? {
                return Ok(false);
            }
            continue;
        }
...
pub fn btendscan(scan: IndexScanDesc) -> Result<(), CatalogError> {
    crate::index::genam::index_endscan_stub(scan)
}

fn allocate_btree_block(
    ctx: &IndexInsertContext,
...
fn write_split_pages_locked<'a>(
    ...
    old_opaque: crate::access::nbtree::BTPageOpaqueData,
    is_leaf: bool,
    scalar: &dyn AccessScalarServices,
    wal: &dyn AccessWalServices,
    ...
    left_opaque.btpo_flags |= crate::access::nbtree::BTP_INCOMPLETE_SPLIT;
...
fn clear_incomplete_split_locked(
    ...
    let mut opaque = bt_page_get_opaque(&page)
        .map_err(|err| CatalogError::Io(format!("btree opaque read failed: {err:?}")))?;
    if opaque.btpo_flags & crate::access::nbtree::BTP_INCOMPLETE_SPLIT == 0 {
        return Ok(());
    }
    opaque.btpo_flags &= !crate::access::nbtree::BTP_INCOMPLETE_SPLIT;
    bt_page_set_opaque(&mut page, opaque)
        .map_err(|err| CatalogError::Io(format!("btree opaque write failed: {err:?}")))?;
    let lsn = log_btree_record(
        wal,
...
fn insert_tuple_into_locked_page<'a>(
    ...
    let insert_at = if is_leaf {
        items.partition_point(|item| {
            let existing = tuple_key_prefix_values(&ctx.index_desc, item, key_count, scalar)
                .unwrap_or_default();
    ...
    let rebuilt =
        match build_insert_page_image(old_opaque, existing_high_key.as_ref(), &items, is_leaf) {
            Ok(rebuilt) => rebuilt,
            Err(_) if is_leaf && prune_aborted_leaf_items(ctx, runtime, &mut items) > 0 => {
...
fn build_insert_page_image(
    ...
    items: &[IndexTupleData],
    is_leaf: bool,
) -> Result<Page, crate::access::nbtree::BtPageError> {
    let mut rebuilt = [0u8; pgrust_storage::smgr::BLCKSZ];
    bt_page_init(
        &mut rebuilt,
...
fn prune_aborted_leaf_items(
    ctx: &IndexInsertContext,
    runtime: &(impl AccessHeapServices + AccessTransactionServices),
    items: &mut Vec<IndexTupleData>,
) -> usize {
    let before = items.len();
    items.retain(|item| {
        let Ok(tuple) = runtime.fetch_heap_tuple(ctx.heap_relation, item.t_tid) else {
            return true;
        };
        !matches!(
            runtime.transaction_status(tuple.header.xmin),
...
fn bt_check_unique_locked(
    ...
    loop {
        let items = bt_page_data_items(&page)
            .map_err(|err| CatalogError::Io(format!("btree page parse failed: {err:?}")))?;
        for tuple in items {
            let tuple_keys = tuple_key_prefix_values(
                &ctx.index_desc,
                &tuple,
                btree_key_count(&ctx.index_meta),
                scalar,
            ...
            match compare_key_prefixes_with_columns_and_options(
                ...
                Ordering::Equal => {
                    match classify_unique_candidate(&unique_ctx, tuple.t_tid, runtime)? {
    ...
    R: AccessHeapServices + AccessTransactionServices + AccessInterruptServices,
{
    ...
    let check_unique = matches!(
        ctx.unique_check,
        crate::access::amapi::IndexUniqueCheck::Yes
    ...
    if check_unique {
        ...
        loop {
            let locked = find_locked_unique_insert_path(ctx, &key_values, runtime, scalar, wal)?;
            let page = *locked.guard;
            let opaque = bt_page_get_opaque(&page)
                .map_err(|err| CatalogError::Io(format!("btree opaque read failed: {err:?}")))?;
            match bt_check_unique_locked(ctx, runtime, &page, opaque, &key_values, scalar)? {
                ...
                LockedUniqueCheckResult::Restart { split_block } => {
                    ...
                    if let Some(split_block) = split_block {
                        finish_incomplete_split(ctx, split_block, &[], runtime, scalar, wal)?;
                    }
                }
            }
        }
    ...
    let search = find_leaf_for_insert(ctx, &key_values, runtime, scalar, wal)?;
    if let Some(mut split) = insert_tuple_into_page(
        ctx,
        runtime,
        search.leaf_block,
        new_tuple,
...
pub fn btbuildempty(
    ...
    ensure_empty_btree(&ctx.pool, ctx.client_id, ctx.xid, ctx.index_relation, wal)
}
