...
use crate::crc32c;
use crate::transam::CheckpointConfig;
use crate::transam::xact::TransactionId;
use crate::transam::xlog::{Lsn, WAL_SEG_SIZE_BYTES};
...
impl ControlFile {
    pub fn bootstrap(next_xid: TransactionId, config: &CheckpointConfig) -> Self {
        Self {
            ...
            redo_lsn: 0,
            next_xid,
            checkpoint_timestamp_usecs:
                crate::transam::checkpoint::current_postgres_timestamp_usecs(),
...
impl ControlFileStore {
    ...
    pub fn update(
        ...
        f(&mut control);
        control.checkpoint_timestamp_usecs =
            crate::transam::checkpoint::current_postgres_timestamp_usecs();
...
fn persist_control_file(path: &Path, control: &ControlFile) -> Result<(), ControlFileError> {
    ...
    pgrust_storage::sync_file_data(&file).map_err(|err| ControlFileError::Io(err.to_string()))?;
