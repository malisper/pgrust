...
use crate::{AccessError, AccessResult, AccessTransactionServices};
use parking_lot::Mutex;
pub use pgrust_core::{
    BOOTSTRAP_TRANSACTION_ID, CommandId, FIRST_NORMAL_TRANSACTION_ID, FROZEN_TRANSACTION_ID,
    INVALID_TRANSACTION_ID, MvccError, Snapshot, TransactionId, TransactionStatus,
...
impl TransactionManager {
    ...
    fn write_initial_status_file(file: &mut File, clog_buf: &[u8]) -> Result<(), MvccError> {
        file.write_all(clog_buf)
            .map_err(|e| MvccError::Io(e.to_string()))?;
        pgrust_storage::sync_file_data(file).map_err(|e| MvccError::Io(e.to_string()))?;
...
mod tests {
    ...
    fn hint_fast_path_xmin_committed_xmax_invalid_is_visible() {
        ...
        let bytes = make_tuple_bytes(
            ...
            0,
            0,
            crate::access::htup::HEAP_XMIN_COMMITTED | crate::access::htup::HEAP_XMAX_INVALID,
        );
    ...
    fn hint_fast_path_xmin_invalid_is_not_visible() {
        ...
        let bytes = make_tuple_bytes(
            ...
            0,
            0,
            crate::access::htup::HEAP_XMIN_INVALID | crate::access::htup::HEAP_XMAX_INVALID,
        );
        let (visible, hints) = snapshot.tuple_bytes_visible_with_hints(&txns, &bytes);
        assert!(!visible);
        assert_eq!(hints, 0);
    ...
    fn hint_fast_path_xmin_committed_xmax_committed_is_not_visible() {
        ...
        let bytes = make_tuple_bytes(
            ...
            2,
            0,
            crate::access::htup::HEAP_XMIN_COMMITTED | crate::access::htup::HEAP_XMAX_COMMITTED,
        );
        let (visible, hints) = snapshot.tuple_bytes_visible_with_hints(&txns, &bytes);
    ...
    fn hints_set_xmin_committed_for_committed_insert() {
        use crate::access::htup::{HEAP_XMAX_INVALID, HEAP_XMIN_COMMITTED};
        let mut txns = TransactionManager::default();
        let inserter = txns.begin();
        txns.commit(inserter).unwrap();
        let snapshot = txns.snapshot(INVALID_TRANSACTION_ID).unwrap();
    ...
    fn hints_set_xmin_invalid_for_aborted_insert() {
        use crate::access::htup::HEAP_XMIN_INVALID;
        let mut txns = TransactionManager::default();
        let inserter = txns.begin();
    ...
    fn hints_set_xmax_committed_for_committed_delete() {
        use crate::access::htup::{HEAP_XMAX_COMMITTED, HEAP_XMIN_COMMITTED};
        let mut txns = TransactionManager::default();
        let inserter = txns.begin();
        txns.commit(inserter).unwrap();
        let deleter = txns.begin();
    ...
    fn hints_not_set_for_in_progress_xmax() {
        use crate::access::htup::HEAP_XMIN_COMMITTED;
        let mut txns = TransactionManager::default();
        let inserter = txns.begin();
        txns.commit(inserter).unwrap();
        let deleter = txns.begin();
    ...
    fn hint_roundtrip_update_in_progress_old_still_visible() {
        ...
        assert!(old_hints2 & crate::access::htup::HEAP_XMAX_COMMITTED != 0);
    }
    ...
    fn hint_bits_cleared_on_delete_old_version_becomes_invisible() {
        ...
        use crate::access::htup::{HEAP_XMAX_COMMITTED, HEAP_XMAX_INVALID, HEAP_XMIN_COMMITTED};
        let mut txns = TransactionManager::default();
        let inserter = txns.begin();
        txns.commit(inserter).unwrap();
