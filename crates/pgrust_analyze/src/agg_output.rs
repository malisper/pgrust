...
use super::*;
use pgrust_catalog_data::CONSTRAINT_PRIMARY;
...
pub fn with_functional_grouping_constraint_tracking<T>(
...
fn expr_references_local_cte(expr: &Expr, local_ctes: &HashMap<usize, String>) -> Option<String> {
    match expr {
        ...
        Expr::GroupingKey(grouping_key) => {
            expr_references_local_cte(&grouping_key.expr, local_ctes)
        }
        ...
        bound_args
            ...
            .map(|((arg, actual_type), declared_type)| {
                coerce_bound_expr(
                    arg,
                    actual_type,
                    aggregate_arg_type_for_coercion(Some(func), actual_type, declared_type),
                )
            })
            .collect()
    } else {
...
fn grouped_output_key_expr(group_key_exprs: &[Expr], index: usize) -> Expr {
    grouped_key_expr(group_key_exprs, index)
}

fn grouping_key_ref(expr: &Expr, fallback: usize) -> usize {
    match expr {
...
fn grouping_key_inner(expr: &Expr) -> &Expr {
    match expr {
...
fn grouping_key_matches_arg(group_key: &Expr, bound_arg: &Expr) -> bool {
    if grouping_key_inner(group_key) == bound_arg {
        return true;
    }
    matches!(
        grouping_key_inner(group_key),
        Expr::Row { fields, .. } if fields.iter().any(|(_, field)| field == bound_arg)
    )
}

fn qualified_column_matches_merged_join_group(
    expr: &SqlExpr,
    group_expr: &SqlExpr,
    input_scope: &BoundScope,
) -> bool {
    let SqlExpr::Column(expr_name) = expr else {
        return false;
    };
    let Some((relation_name, column_name)) = expr_name.rsplit_once('.') else {
        return false;
    };
    let SqlExpr::Column(group_name) = group_expr else {
        return false;
    };
    if group_name.contains('.') || !group_name.eq_ignore_ascii_case(column_name) {
        return false;
    }
    let Ok(group_index) = resolve_column(input_scope, group_name) else {
        return false;
    };
    let Some(column) = input_scope.columns.get(group_index) else {
        return false;
    };
    column.source_columns.len() > 1
        && column
            .relation_names
            .iter()
...
fn grouping_arg_expr(group_key: &Expr, bound_arg: &Expr) -> Expr {
    if grouping_key_inner(group_key) == bound_arg {
        group_key.clone()
    } else {
        bound_arg.clone()
...
fn bind_grouping_func_call(
    ...
    let mut grouping_args = Vec::new();
    let mut refs = Vec::new();
    for (index, arg) in args.iter().enumerate() {
        let (group_index, key_expr) = if let Some(group_index) = group_by_exprs
            .iter()
            .position(|group_expr| group_expr == &arg.value)
        {
            (group_index, grouped_key_expr(group_key_exprs, group_index))
        } else {
            ...
            let Some(group_index) = group_key_exprs
                .iter()
                .position(|group_key| grouping_key_matches_arg(group_key, &bound))
            else {
            ...
            let key_expr = grouped_key_expr(group_key_exprs, group_index);
            let arg_expr = grouping_arg_expr(&key_expr, &bound);
            (group_index, arg_expr)
        };
        let _ = index;
        refs.push(grouping_key_ref(
            &grouped_key_expr(group_key_exprs, group_index),
            group_index + 1,
        ));
        grouping_args.push(key_expr);
    }
    Ok(Expr::GroupingFunc(Box::new(GroupingFuncExpr {
        args: grouping_args,
        refs,
        agglevelsup: 0,
    })))
...
pub(super) fn bind_visible_grouping_func_call(
    ...
    let visible_scopes = current_visible_aggregate_scopes();
    ...
    for visible_scope in visible_scopes {
        ...
        for bound in &bound_args {
            let Some((group_index, key_expr)) = raised_group_keys
                .iter()
                .enumerate()
                .find(|(_, group_key)| grouping_key_matches_arg(group_key, bound))
                .map(|(index, group_key)| (index, group_key.clone()))
            else {
                matched = false;
                break;
            };
            refs.push(grouping_key_ref(&key_expr, group_index + 1));
            grouping_args.push(grouping_arg_expr(&key_expr, bound));
        }
        if matched {
            return Ok(Some(Expr::GroupingFunc(Box::new(GroupingFuncExpr {
                args: grouping_args,
                refs,
                agglevelsup: visible_scope.levelsup,
            }))));
...
pub(super) fn bind_agg_output_expr_in_clause(
    ...
    for (i, gk) in group_by_exprs.iter().enumerate() {
        if gk == expr {
            return Ok(grouped_output_key_expr(group_key_exprs, i));
        }
    }
    if let Ok(bound_expr) =
        ...
        && let Some(i) = group_key_exprs
            .iter()
            .position(|group_key| grouping_key_matches_arg(group_key, &bound_expr))
        && !qualified_column_matches_merged_join_group(expr, &group_by_exprs[i], input_scope)
    {
        return Ok(grouped_output_key_expr(group_key_exprs, i));
    }
    ...
    match expr {
        ...
        SqlExpr::FuncCall {
            ...
            let within_group_aggkind = within_group.as_ref().and_then(|_| {
            ...
            if let Some((i, _)) = agg_list.iter().enumerate().find(|(_, agg)| {
                ...
                let within_group_aggkind = aggregate_call_kind_matches_catalog(
                    catalog,
                    name,
                    args,
                ...
                let builtin_hypothetical = resolve_builtin_hypothetical_aggregate(name).is_some();
                let builtin_ordered_set = resolve_builtin_ordered_set_aggregate(name).is_some();
                let hypothetical = (builtin_hypothetical || within_group_aggkind == Some('h'))
                    && !direct_args.is_empty();
                let ordered_set = (builtin_ordered_set || within_group_aggkind == Some('o'))
                    && !aggregate_order_by.is_empty();
                ...
                let custom_within_group_resolved = if hypothetical && !builtin_hypothetical {
                    Some(
                    ...
                    && builtin_hypothetical
                {
                ...
                } else if let Some(resolved) = custom_within_group_resolved.as_ref() {
                    coerce_catalog_within_group_aggregate_inputs(
                        &direct_args,
                        &direct_arg_types,
                        bound_direct_args,
                    ...
                    if resolved.builtin_impl == Some(AggFunc::StringAgg) && coerced_args.len() == 2
                    {
                        let left_type =
                            expr_sql_type_hint(&coerced_args[0]).unwrap_or(arg_types[0]);
                        let right_type =
                            expr_sql_type_hint(&coerced_args[1]).unwrap_or(arg_types[1]);
                        let _ = derive_consumer_collation_from_exprs(
                            catalog,
                            CollationConsumer::StringComparison,
                            &[
                                (&coerced_args[0], left_type, None),
                                (&coerced_args[1], right_type, None),
                            ],
                        )?;
                    }
                    (Vec::new(), coerced_args, bound_order_by)
            ...
            } else if name.eq_ignore_ascii_case("pg_collation_for")
                && !*func_variadic
                && args.args().len() == 1
                && args.args()[0].name.is_none()
            {
                ...
                Ok(Expr::Const(Value::Text(
                    explicit_collation_display_name(&args.args()[0].value, catalog)
                        .unwrap_or_default()
                        .into(),
                )))
            } else {
        ...
        SqlExpr::Case {
            ...
            let (bound_arg, arg_type) = if let Some(arg) = arg {
                ...
                let arg_type = expr_sql_type_hint(&bound).unwrap_or_else(|| {
                    grouped_infer_sql_expr_type(
                        arg,
                        input_scope,
                        catalog,
            ...
            for arm in args {
                let condition = if let Some(arg_type) = arg_type {
                    ...
                    bind_lowered_comparison_expr(
                        "=",
                        OpExprKind::Eq,
                        Expr::CaseTest(Box::new(pgrust_nodes::primnodes::CaseTestExpr {
                            type_id: arg_type,
                        })),
                        arg_type,
                        arg_type,
                ...
                bound_arms.push(pgrust_nodes::primnodes::CaseWhen {
                    expr: condition,
            ...
            Ok(Expr::Case(Box::new(pgrust_nodes::primnodes::CaseExpr {
    ...
    let step = if args.len() >= 3 {
        ...
        match common.kind {
            ...
            SqlTypeKind::Numeric => Expr::Const(Value::Numeric(
                pgrust_nodes::datum::NumericValue::from_i64(1),
            )),
