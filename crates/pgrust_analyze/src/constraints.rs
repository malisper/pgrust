...
use pgrust_catalog_data::{PgConstraintRow, bootstrap_pg_cast_rows};
use pgrust_nodes::parsenodes::{
    ColumnConstraint, ConstraintAttributes, CreateTableStatement, ForeignKeyAction,
    ForeignKeyMatchType, IndexColumnDef, SqlExpr, SqlType, SqlTypeKind, TableConstraint,
    TablePersistence,
};
...
pub struct BoundForeignKeyConstraint {
    ...
    pub referenced_relation_oid: u32,
    pub referenced_rel: pgrust_core::RelFileLocator,
    pub referenced_toast: Option<pgrust_nodes::primnodes::ToastRelationRef>,
...
pub struct BoundReferencedByForeignKey {
    ...
    pub child_rel: pgrust_core::RelFileLocator,
    pub child_toast: Option<pgrust_nodes::primnodes::ToastRelationRef>,
...
fn exclusion_index_columns(
    elements: &[pgrust_nodes::parsenodes::ExclusionElement],
...
pub fn bind_relation_constraints(
    ...
    for row in rows {
        match row.contype {
            pgrust_catalog_data::CONSTRAINT_NOTNULL => {
                let Some(attnum) = row
                    .conkey
                    .as_ref()
            ...
            pgrust_catalog_data::CONSTRAINT_CHECK => {
            ...
            pgrust_catalog_data::CONSTRAINT_FOREIGN => {
            ...
            | pgrust_catalog_data::CONSTRAINT_EXCLUSION
                if row.conperiod =>
            {
...
pub fn bind_temporal_constraint(
    row: PgConstraintRow,
    desc: &RelationDesc,
    ...
    Ok(BoundTemporalConstraint {
        ...
        column_indexes,
        period_column_index,
        primary: row.contype == pgrust_catalog_data::CONSTRAINT_PRIMARY,
        enforced: row.conenforced,
    })
}

pub fn bind_exclusion_constraint(
    row: PgConstraintRow,
    desc: &RelationDesc,
    catalog: &dyn super::CatalogLookup,
...
fn is_referenced_side_foreign_key_clone(
    ...
    if row.contype != pgrust_catalog_data::CONSTRAINT_FOREIGN || row.conparentid == 0 {
        return false;
    }
    catalog
...
pub fn bind_referenced_by_foreign_keys(
    ...
    for row in catalog.constraint_rows().into_iter().filter(|row| {
        row.contype == pgrust_catalog_data::CONSTRAINT_FOREIGN && row.confrelid == relation_oid
    }) {
...
pub fn infer_relation_expr_sql_type(
    expr_sql: &str,
    relation_name: Option<&str>,
    desc: &RelationDesc,
    catalog: &dyn super::CatalogLookup,
...
fn resolve_pending_self_referenced_key(
    table_name: &str,
    column_defs: &[pgrust_nodes::parsenodes::ColumnDef],
...
fn resolve_create_table_column_type_for_foreign_key(
    raw: &pgrust_nodes::parsenodes::RawTypeName,
    catalog: &dyn super::CatalogLookup,
) -> Result<SqlType, ParseError> {
    match raw {
        pgrust_nodes::parsenodes::RawTypeName::Serial(_) => Ok(super::raw_type_name_hint(raw)),
    ...
    let (columns, referenced_attnums, index_oid, referenced_period, temporal_key, primary_key) =
        if let Some(referenced_columns) = referenced_columns {
            ...
            let primary = catalog
                ...
                .into_iter()
                .filter(|row| row.contype == pgrust_catalog_data::CONSTRAINT_PRIMARY)
                .collect::<Vec<_>>();
            if primary.len() != 1 {
                return Err(ParseError::UnexpectedToken {
                    expected: "referenced PRIMARY KEY",
