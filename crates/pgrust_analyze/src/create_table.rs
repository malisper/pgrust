...
use pgrust_nodes::primnodes::expr_contains_set_returning;
        ...
        columns: columns
            ...
            .map(|(index, column)| {
                ...
                let sql_type = match column.ty {
                    pgrust_nodes::parsenodes::RawTypeName::Serial(_) => {
                ...
                if matches!(sql_type.kind, SqlTypeKind::Cstring)
                    || sql_type.type_oid == pgrust_catalog_data::UNKNOWN_TYPE_OID
                {
                    return Err(ParseError::DetailedError {
                        message: format!(
                ...
                if matches!(sql_type.kind, SqlTypeKind::Shell) {
                    let type_name = match &column.ty {
                        pgrust_nodes::parsenodes::RawTypeName::Named { name, .. } => name.as_str(),
                        _ => &column.name,
                    };
                    return Err(ParseError::DetailedError {
                ...
                let serial_kind = match column.ty {
                    pgrust_nodes::parsenodes::RawTypeName::Serial(kind) => Some(kind),
                    _ => None,
                };
...
fn expand_create_table_of_type(
    ...
    let mut typed_columns = type_relation
        ...
        .map(|column| {
            CreateTableElement::Column(pgrust_nodes::parsenodes::ColumnDef {
        ...
        .map(|column| {
            ...
            let generated = if options.generated {
                column.generated.and_then(|kind| {
                    column.default_expr.clone().map(|expr_sql| {
                        pgrust_nodes::parsenodes::ColumnGeneratedDef { expr_sql, kind }
                    })
                })
            } else {
                None
            ...
            CreateTableElement::Column(pgrust_nodes::parsenodes::ColumnDef {
                ...
                identity: options
                    ...
                    .flatten()
                    .map(|kind| pgrust_nodes::parsenodes::ColumnIdentityDef {
                        kind,
                        options: SequenceOptionsSpec::default(),
                    }),
...
pub fn validate_column_default_expr(
    default_sql: &str,
    catalog: &dyn CatalogLookup,
) -> Result<(), ParseError> {
    let parsed = pgrust_parser::parse_expr(default_sql)?;
...
fn ensure_attribute_compression_supported(
    compression: AttributeCompression,
) -> Result<(), ParseError> {
    if compression == AttributeCompression::Lz4 {
        #[cfg(not(feature = "lz4"))]
        {
            return Err(ParseError::DetailedError {
...
mod tests {
    use super::*;
    use pgrust_catalog_data::PgConstraintRow;
    use pgrust_nodes::parsenodes::{
    ...
    #[derive(Default)]
    struct LikeCatalog {
        relation: Option<crate::BoundRelation>,
        constraints: Vec<PgConstraintRow>,
    }

    impl crate::CatalogLookup for LikeCatalog {
        fn lookup_any_relation(&self, name: &str) -> Option<crate::BoundRelation> {
            self.relation
                .as_ref()
    ...
    fn lower_create_table_rejects_anyarray_columns() {
        ...
        assert_eq!(
            lower_create_table(&stmt, &crate::LiteralDefaultCatalog),
    ...
    fn lower_create_table_materializes_not_null_metadata() {
        let stmt = CreateTableStatement {
            ...
            elements: vec![
                CreateTableElement::Column(ColumnDef {
                    ...
                    storage: None,
                    compression: None,
                    constraints: vec![pgrust_nodes::parsenodes::ColumnConstraint::PrimaryKey {
                ...
                CreateTableElement::Column(ColumnDef {
                    ...
                    storage: None,
                    compression: None,
                    constraints: vec![pgrust_nodes::parsenodes::ColumnConstraint::NotNull {
                        attributes: ConstraintAttributes {
        ...
        let lowered = lower_create_table(&stmt, &crate::LiteralDefaultCatalog).unwrap();
    ...
    fn lower_create_table_like_including_all_expands_columns_and_constraints() {
        ...
        let catalog = LikeCatalog {
            relation: Some(crate::BoundRelation {
                ...
                relation_oid: 42,
                toast: None,
                namespace_oid: pgrust_catalog_data::PUBLIC_NAMESPACE_OID,
            ...
            constraints: vec![
                PgConstraintRow {
                    ...
                    connamespace: pgrust_catalog_data::PUBLIC_NAMESPACE_OID,
    ...
    fn lower_create_table_rejects_conflicting_not_null_no_inherit() {
        let stmt = CreateTableStatement {
            ...
            elements: vec![
                ...
                CreateTableElement::Constraint(
                    pgrust_nodes::parsenodes::TableConstraint::NotNull {
                        attributes: ConstraintAttributes {
                            no_inherit: true,
                            ..ConstraintAttributes::default()
                        },
