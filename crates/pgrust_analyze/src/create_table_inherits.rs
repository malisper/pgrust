        ...
        let parent_has_primary_key = parents.iter().any(|parent| {
            parent.relkind == 'p'
                && catalog
                    ...
                    .into_iter()
                    .any(|row| row.contype == pgrust_catalog_data::CONSTRAINT_PRIMARY)
        });
...
fn inherited_table_constraints(
    ...
    for parent in parents {
        ...
        {
            match row.contype {
                pgrust_catalog_data::CONSTRAINT_CHECK if !row.connoinherit => {
                ...
                pgrust_catalog_data::CONSTRAINT_PRIMARY
                | pgrust_catalog_data::CONSTRAINT_UNIQUE
                    if parent.relkind == 'p' =>
                {
                    ...
                    if row.contype == pgrust_catalog_data::CONSTRAINT_PRIMARY {
        ...
        for column in &parent.desc.columns {
            ...
            let generated = column.generated.and_then(|kind| {
                column
                    .default_expr
                    .clone()
                    .map(|expr_sql| pgrust_nodes::parsenodes::ColumnGeneratedDef { expr_sql, kind })
            });
            let inherited = pgrust_nodes::parsenodes::ColumnDef {
...
fn merge_parent_column(
    merged: &mut MergedColumnSpec,
    parent: &pgrust_nodes::parsenodes::ColumnDef,
) -> Result<(), ParseError> {
...
fn merge_local_column(
    merged: &mut MergedColumnSpec,
    local: &pgrust_nodes::parsenodes::ColumnDef,
    local_index: usize,
...
fn inherited_not_null_count(column: &pgrust_nodes::primnodes::ColumnDesc) -> i16 {
...
fn mark_inherited_check_actions(
    ...
    let inherited_checks = parents
        ...
        .filter(|row| row.contype == pgrust_catalog_data::CONSTRAINT_CHECK && !row.connoinherit)
        .collect::<Vec<_>>();
...
fn ensure_not_null_constraint(column: &mut pgrust_nodes::parsenodes::ColumnDef) {
    if !column.nullable() {
        return;
    }
...
fn replace_not_null_constraint(
    column: &mut pgrust_nodes::parsenodes::ColumnDef,
    attributes: ConstraintAttributes,
) {
    column
        .constraints
...
fn ensure_primary_key_constraint(column: &mut pgrust_nodes::parsenodes::ColumnDef) {
...
fn ensure_unique_constraint(column: &mut pgrust_nodes::parsenodes::ColumnDef) {
    if column.unique() {
        return;
    }
...
fn ensure_matching_column_compression(
    name: &str,
    left: Option<pgrust_core::AttributeCompression>,
    right: Option<pgrust_core::AttributeCompression>,
) -> Result<(), ParseError> {
    if left == right {
        return Ok(());
    }
...
fn ensure_matching_column_generated_kind(
    name: &str,
    left: Option<&pgrust_nodes::parsenodes::ColumnGeneratedDef>,
    right: Option<&pgrust_nodes::parsenodes::ColumnGeneratedDef>,
) -> Result<(), ParseError> {
    match (left, right) {
...
fn generated_kind_name(kind: pgrust_nodes::parsenodes::ColumnGeneratedKind) -> &'static str {
    match kind {
        pgrust_nodes::parsenodes::ColumnGeneratedKind::Virtual => "VIRTUAL",
        pgrust_nodes::parsenodes::ColumnGeneratedKind::Stored => "STORED",
    }
}
...
fn merge_parent_column_compression(
    ...
    left: Option<pgrust_core::AttributeCompression>,
    right: Option<pgrust_core::AttributeCompression>,
) -> Result<Option<pgrust_core::AttributeCompression>, ParseError> {
    if left == right {
        return Ok(left);
    }
...
fn is_default_column_compression(compression: Option<pgrust_core::AttributeCompression>) -> bool {
    matches!(
        compression,
        None | Some(pgrust_core::AttributeCompression::Default)
    )
}

fn format_column_compression(
    compression: Option<pgrust_core::AttributeCompression>,
) -> &'static str {
    compression
        .map(compression_name)
        .unwrap_or(compression_name(pgrust_core::AttributeCompression::Default))
}

fn compression_name(compression: pgrust_core::AttributeCompression) -> &'static str {
    match compression {
        pgrust_core::AttributeCompression::Default => "default",
        pgrust_core::AttributeCompression::Pglz => "pglz",
        pgrust_core::AttributeCompression::Lz4 => "lz4",
    }
}
...
fn default_exprs_equal(left: &str, right: &str) -> bool {
    ...
    match (
        pgrust_parser::parse_expr(left),
        pgrust_parser::parse_expr(right),
    ) {
...
mod tests {
    use super::*;
    use pgrust_core::AttributeCompression;
