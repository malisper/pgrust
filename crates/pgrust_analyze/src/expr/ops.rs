use super::*;
use crate::multiranges::bind_maybe_multirange_overlap_or_adjacent;
use crate::ranges::bind_maybe_range_overlap_or_adjacent;
...
use pgrust_nodes::primnodes::OpExpr;

pub(super) fn bind_arithmetic_expr(
    op: &'static str,
    make: pgrust_nodes::primnodes::OpExprKind,
    left: &SqlExpr,
    right: &SqlExpr,
    scope: &BoundScope,
...
fn text_pattern_operator_metadata(
    op: &'static str,
) -> Option<(pgrust_nodes::primnodes::OpExprKind, u32, u32)> {
    Some(match op {
        "~<~" => (
...
pub fn bind_lowered_comparison_expr(
    op: &'static str,
    make: pgrust_nodes::primnodes::OpExprKind,
    left_bound: Expr,
    raw_left_type: SqlType,
...
pub fn supports_comparison_operator(
    ...
    if !left.is_array
        && !right.is_array
        && left == right
        && matches!(left.kind, SqlTypeKind::Composite | SqlTypeKind::Record)
        && matches!(op, "=" | "<>" | "<" | "<=" | ">" | ">=")
    {
        return true;
    }
        ...
        "@@" => {
            if left_type.kind == SqlTypeKind::Jsonb && right_type.kind == SqlTypeKind::JsonPath {
                return Ok(Expr::Op(Box::new(OpExpr {
                    ...
                    op: pgrust_nodes::primnodes::OpExprKind::JsonbPathMatch,
                    opresulttype: SqlType::new(SqlTypeKind::Bool),
                    args: vec![
                        coerce_bound_expr(
                            left_bound,
...
pub(super) fn bind_catalog_equality_operator_expr(
    op: &'static str,
    left: &SqlExpr,
    right: &SqlExpr,
    scope: &BoundScope,
    ...
    Ok(Expr::Op(Box::new(OpExpr {
        ...
        // :HACK: The partition_prune regression defines === as an equality
        // operator backed by int4eq so equivclass reordering does not hide
        // contradiction handling. Keep this path explicit until catalog
        // operator strategy metadata is threaded into OpExpr binding.
        op: OpExprKind::Eq,
        opresulttype: SqlType::new(SqlTypeKind::Bool),
        args: vec![
            coerce_bound_expr(left_bound, raw_left_type, left_type),
...
) -> Option<pgrust_nodes::primnodes::BuiltinScalarFunction> {
    pgrust_catalog_data::builtin_scalar_function_for_proc_row(row).or_else(|| {
        [row.prosrc.as_str(), row.proname.as_str()]
            ...
            .then_some(pgrust_nodes::primnodes::BuiltinScalarFunction::PgRustTestPtInWidget)
    })
}
...
fn resolve_prefix_operator(
    ...
    op: &str,
    actual_type: SqlType,
) -> Option<(pgrust_catalog_data::PgOperatorRow, SqlType, SqlType)> {
    let mut best: Option<(pgrust_catalog_data::PgOperatorRow, SqlType, SqlType, usize)> = None;
