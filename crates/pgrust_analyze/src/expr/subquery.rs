...
use pgrust_nodes::primnodes::{SubLink, SubLinkType, SubqueryComparison, TargetEntry};
...
pub fn exists_subquery_query(mut query: Query) -> Query {
...
) -> Option<(&'static str, pgrust_nodes::primnodes::OpExprKind)> {
...
fn resolve_subquery_comparison_metadata(
    catalog: &dyn CatalogLookup,
    op_text: &'static str,
    op_kind: pgrust_nodes::primnodes::OpExprKind,
    left: &Expr,
    right: &Expr,
    ...
    {
        pgrust_catalog_data::RECORD_TYPE_OID
    } else {
        left_oid
    };
