...
fn outer_scope_has_rule_pseudo_relation(scope: &BoundScope, relation: &str) -> bool {
    let varno = if relation.eq_ignore_ascii_case("old") {
        pgrust_nodes::primnodes::RULE_OLD_VAR
    } else if relation.eq_ignore_ascii_case("new") {
        pgrust_nodes::primnodes::RULE_NEW_VAR
    } else {
        return false;
    };
        ...
        other => {
            if let Some(kind) = resolve_json_table_function(other) {
                ...
                if let Some(kind) = resolve_regex_table_function(other) {
                    ...
                    let output_columns = match kind {
                        pgrust_nodes::primnodes::RegexTableFunction::Matches => {
                            vec![QueryColumn {
                        ...
                        pgrust_nodes::primnodes::RegexTableFunction::SplitToTable => {
