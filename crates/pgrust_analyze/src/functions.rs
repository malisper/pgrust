...
use pgrust_parser::gram::{SQL_JSON_ARRAYAGG_FUNC, SQL_JSON_OBJECTAGG_FUNC};
...
pub struct ResolvedFunctionCall {
    ...
    pub(crate) srf_impl: Option<ResolvedSrfImpl>,
    ...
    pub(crate) row_shape: ResolvedFunctionRowShape,
}
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ResolvedFunctionCallWithArgs {
...
pub fn resolve_function_call_with_arg_defaults(
    catalog: &dyn CatalogLookup,
    name: &str,
    args: &[SqlFunctionArg],
    actual_types: &[SqlType],
...
pub fn decode_proc_arg_defaults(
    row: &pgrust_catalog_data::PgProcRow,
    input_count: usize,
) -> Vec<Option<String>> {
...
pub fn sql_function_anyarray_return_resolution_error(
    catalog: &dyn CatalogLookup,
    name: &str,
    actual_types: &[SqlType],
) -> Option<ParseError> {
...
mod tests {
    ...
    fn resolve_stats_import_calls_as_builtin_proc_rows() {
        ...
        let stmt = pgrust_parser::parse_select(
    ...
    fn resolve_function_call_prefers_range_lower_for_range_arguments() {
        let resolved = resolve_function_call(
            ...
            .with_identity(pgrust_catalog_data::INT4RANGE_TYPE_OID, 0)
            .with_range_metadata(
                pgrust_catalog_data::INT4_TYPE_OID,
                pgrust_catalog_data::INT4MULTIRANGE_TYPE_OID,
                true,
            )],
            false,
        )
    ...
    fn resolve_function_call_supports_zero_arg_builtin_multirange_constructors() {
        ...
        assert_eq!(
            ...
            .with_identity(pgrust_catalog_data::INT4MULTIRANGE_TYPE_OID, 0)
            .with_range_metadata(
                pgrust_catalog_data::INT4_TYPE_OID,
                pgrust_catalog_data::INT4MULTIRANGE_TYPE_OID,
                true,
            )
            .with_multirange_range_oid(pgrust_catalog_data::INT4RANGE_TYPE_OID)
        );
        assert_eq!(
            resolved.scalar_impl,
    ...
    fn resolve_function_call_supports_explicit_variadic_builtin_multirange_constructors() {
        ...
        let range_array_type = catalog
            .type_by_oid(pgrust_catalog_data::NUMRANGE_ARRAY_TYPE_OID)
            .unwrap()
            .sql_type;
        let resolved =
        ...
        assert_eq!(
            resolved.result_type,
            catalog
                .type_by_oid(pgrust_catalog_data::NUMMULTIRANGE_TYPE_OID)
                .unwrap()
                .sql_type
        );
        assert!(resolved.func_variadic);
        assert_eq!(resolved.vatype_oid, pgrust_catalog_data::NUMRANGE_TYPE_OID);
