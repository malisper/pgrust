...
use super::*;
use pgrust_catalog_data::{UNKNOWN_TYPE_OID, range_type_ref_for_sql_type};
...
pub(super) fn infer_sql_expr_type_with_ctes(
    ...
    match expr {
        ...
        SqlExpr::Const(Value::IndirectVarlena(indirect)) => indirect.sql_type,
        SqlExpr::Const(Value::Text(_))
        | SqlExpr::Const(Value::TextRef(_, _))
        ...
        SqlExpr::Xml(xml) if xml.op == pgrust_nodes::parsenodes::RawXmlExprOp::Serialize => xml
            .target_type
            .as_ref()
