...
pub use system_views::{pg_column_is_updatable, pg_relation_is_updatable_events};
...
pub use rewrite::{
    ...
    RlsWriteCheckSource, TargetRlsState, ViewCheck, ViewDmlEvent, ViewDmlRewriteError,
    ViewPrivilegeContext, ViewRuleEventClassification,
};
pub use runtime::{AnalyzeServices, FormattedTriggerDefinition, with_analyze_services};
use runtime::{cast_value, pg_rewrite_query, planner_with_config};
...
pub use query::analyze_select_query_with_outer;
use query::{
    AnalyzedFrom, analyze_values_query_with_outer, identity_target_list, normalize_target_list,
    query_from_from_projection,
};
pub use query::{rewrite_local_vars_for_output_exprs, rewrite_planned_local_vars_for_output_exprs};
pub use rules::{
...
use scope::*;
pub use scope::{
    BoundCte, BoundModifyingCte, BoundScope, BoundWritableCte, GroupedOuterScope,
...
pub fn with_external_param_types<T>(types: &[(usize, SqlType)], f: impl FnOnce() -> T) -> T {
...
pub fn external_param_type(paramid: usize) -> Option<SqlType> {
    EXTERNAL_PARAM_TYPES.with(|cell| {
        cell.borrow()
            .iter()
            .rev()
...
pub fn sql_expr_mentions_system_column(sql: &str) -> bool {
...
pub struct BoundIndexRelation {
    ...
    pub index_meta: pgrust_nodes::relcache::IndexRelCacheEntry,
...
pub fn relation_get_index_expressions(
    index_meta: &mut pgrust_nodes::relcache::IndexRelCacheEntry,
    heap_desc: &RelationDesc,
    catalog: &dyn CatalogLookup,
) -> Result<Vec<Expr>, ParseError> {
    if let Some(exprs) = &index_meta.rd_indexprs {
...
pub fn RelationGetIndexExpressions(
    index_meta: &mut pgrust_nodes::relcache::IndexRelCacheEntry,
    heap_desc: &RelationDesc,
    catalog: &dyn CatalogLookup,
) -> Result<Vec<Expr>, ParseError> {
...
pub fn relation_get_index_predicate(
    index_meta: &mut pgrust_nodes::relcache::IndexRelCacheEntry,
    heap_desc: &RelationDesc,
    catalog: &dyn CatalogLookup,
) -> Result<Option<Expr>, ParseError> {
...
#[allow(non_snake_case)]
pub fn RelationGetIndexPredicate(
    index_meta: &mut pgrust_nodes::relcache::IndexRelCacheEntry,
    heap_desc: &RelationDesc,
    catalog: &dyn CatalogLookup,
) -> Result<Option<Expr>, ParseError> {
...
pub trait CatalogLookup: Send + Sync {
    ...
    fn type_oid_for_sql_type(&self, sql_type: SqlType) -> Option<u32> {
        ...
        for row in self.type_rows() {
            if row.oid == pgrust_catalog_data::UNKNOWN_TYPE_OID {
                continue;
            }
    ...
    fn trigger_rows_for_relation(
        &self,
        _relation_oid: u32,
    ) -> Vec<pgrust_catalog_data::PgTriggerRow> {
        Vec::new()
    }

    fn trigger_rows(&self) -> Vec<pgrust_catalog_data::PgTriggerRow> {
        Vec::new()
    }
    ...
    fn policy_rows_for_relation(
        &self,
        _relation_oid: u32,
    ) -> Vec<pgrust_catalog_data::PgPolicyRow> {
        Vec::new()
    }
    ...
    fn foreign_key_constraint_rows_referencing_index(
        ...
        self.constraint_rows()
            .into_iter()
            .filter(|row| {
                row.contype == pgrust_catalog_data::CONSTRAINT_FOREIGN && row.conindid == index_oid
            })
            .collect()
    }
    ...
    fn attribute_rows_for_relation(
        &self,
        _relation_oid: u32,
    ) -> Vec<pgrust_catalog_data::PgAttributeRow> {
        Vec::new()
    }
    ...
    fn pg_publication_tables_rows(
        &self,
        _publications: Vec<PgPublicationRow>,
        _publication_rels: Vec<PgPublicationRelRow>,
        _publication_namespaces: Vec<PgPublicationNamespaceRow>,
        _namespaces: Vec<PgNamespaceRow>,
        _classes: Vec<PgClassRow>,
        _attributes: Vec<PgAttributeRow>,
        _inherits: Vec<PgInheritsRow>,
    ) -> Vec<Vec<Value>> {
        Vec::new()
    }
    ...
    fn information_schema_foreign_data_wrappers_rows(
        &self,
        _authids: Vec<PgAuthIdRow>,
        _auth_members: Vec<PgAuthMembersRow>,
        _wrappers: Vec<PgForeignDataWrapperRow>,
        _current_user_oid: u32,
    ) -> Vec<Vec<Value>> {
        Vec::new()
    }
    ...
    fn information_schema_foreign_servers_rows(
        ...
        _auth_members: Vec<PgAuthMembersRow>,
        _wrappers: Vec<PgForeignDataWrapperRow>,
        _servers: Vec<PgForeignServerRow>,
        _current_user_oid: u32,
    ) -> Vec<Vec<Value>> {
        Vec::new()
    }
    ...
    fn information_schema_user_mappings_rows(
        ...
        _auth_members: Vec<PgAuthMembersRow>,
        _servers: Vec<PgForeignServerRow>,
        _mappings: Vec<PgUserMappingRow>,
        _current_user_oid: u32,
    ) -> Vec<Vec<Value>> {
        Vec::new()
    }
...
pub fn bound_index_relation_from_relcache_entry(
    name: String,
    entry: &pgrust_nodes::relcache::RelCacheEntry,
    catalog: &dyn CatalogLookup,
) -> Option<BoundIndexRelation> {
    bound_index_relation_from_relcache_entry_with_heap(name, entry, catalog, None)
}

pub fn bound_index_relation_from_relcache_entry_with_heap(
    name: String,
    entry: &pgrust_nodes::relcache::RelCacheEntry,
    catalog: &dyn CatalogLookup,
...
pub fn bound_index_relation_from_relcache_entry_with_heap_and_cache(
    name: String,
    entry: &pgrust_nodes::relcache::RelCacheEntry,
    catalog: &dyn CatalogLookup,
    heap_relation: Option<&BoundRelation>,
...
fn expand_group_by_with_primary_key_dependencies(
    ...
    for relation in &scope.relations {
        ...
        let Some(primary) = catalog
            ...
            .into_iter()
            .find(|row| row.contype == pgrust_catalog_data::CONSTRAINT_PRIMARY)
        else {
            continue;
        };
...
pub fn raw_type_name_hint(raw: &RawTypeName) -> SqlType {
    match raw {
...
pub fn raw_type_name_is_unknown(raw: &RawTypeName) -> bool {
    match raw {
...
#[derive(Default)]
pub struct LiteralDefaultCatalog;
pub type Catalog = LiteralDefaultCatalog;
...
pub fn domain_lookup_sql_type(domain: &DomainLookup) -> SqlType {
    let identity_arg =
...
pub fn is_array_of_domain_over_array_type(sql_type: SqlType, catalog: &dyn CatalogLookup) -> bool {
...
pub fn domain_over_array_element_type(
    sql_type: SqlType,
    catalog: &dyn CatalogLookup,
) -> Option<SqlType> {
    if !sql_type.is_array || sql_type.typrelid == 0 {
...
pub fn array_of_resolved_type(sql_type: SqlType, catalog: &dyn CatalogLookup) -> SqlType {
    if sql_type.is_array
...
pub fn split_named_type_typmod(name: &str) -> (&str, Option<Vec<i32>>) {
    let trimmed = name.trim();
...
fn builtin_named_type_alias(name: &str) -> Option<SqlType> {
    ...
    if name.eq_ignore_ascii_case("float") {
        ...
        Some(SqlType::new(SqlTypeKind::Int8).with_identity(pgrust_catalog_data::XID8_TYPE_OID, 0))
    } else if name.eq_ignore_ascii_case("any") {
        Some(SqlType::new(SqlTypeKind::AnyElement).with_identity(pgrust_catalog_data::ANYOID, 0))
    } else if name.eq_ignore_ascii_case("anyenum") {
        Some(
            SqlType::new(SqlTypeKind::AnyElement).with_identity(pgrust_catalog_data::ANYENUMOID, 0),
        )
    } else if name.eq_ignore_ascii_case("anynonarray") {
        Some(
            SqlType::new(SqlTypeKind::AnyElement)
                .with_identity(pgrust_catalog_data::ANYNONARRAYOID, 0),
        )
    } else if name.eq_ignore_ascii_case("anycompatiblenonarray") {
        Some(
            SqlType::new(SqlTypeKind::AnyCompatible)
                .with_identity(pgrust_catalog_data::ANYCOMPATIBLENONARRAYOID, 0),
        )
        ...
        Some(
            SqlType::new(SqlTypeKind::FdwHandler)
                .with_identity(pgrust_catalog_data::INDEX_AM_HANDLER_TYPE_OID, 0),
        )
    } else if name.eq_ignore_ascii_case("table_am_handler") {
        Some(
            SqlType::new(SqlTypeKind::FdwHandler)
                .with_identity(pgrust_catalog_data::TABLE_AM_HANDLER_TYPE_OID, 0),
        )
        ...
        Some(
            SqlType::new(SqlTypeKind::Text).with_identity(pgrust_catalog_data::UNKNOWN_TYPE_OID, 0),
        )
    } else {
        None
    }
...
pub fn bind_scalar_expr_in_scope(
    expr: &SqlExpr,
...
pub fn bind_scalar_expr_in_named_relation_scope(
    expr: &SqlExpr,
...
pub fn bind_policy_expr_in_named_relation_scope(
    expr: &SqlExpr,
    relation_scopes: &[(&str, &RelationDesc)],
    columns: &[(String, SqlType)],
    catalog: &dyn CatalogLookup,
...
pub fn cte_body_is_modifying(body: &CteBody) -> bool {
    match body {
...
pub fn cte_body_references_table(body: &CteBody, table_name: &str) -> bool {
    match body {
...
pub fn select_statement_references_table(stmt: &SelectStatement, table_name: &str) -> bool {
    if stmt
        .with
        .iter()
...
pub fn insert_statement_references_table(stmt: &InsertStatement, table_name: &str) -> bool {
    stmt.with
        .iter()
        .any(|cte| cte_body_references_table(&cte.body, table_name))
        || match &stmt.source {
...
pub fn update_statement_references_table(stmt: &UpdateStatement, table_name: &str) -> bool {
    stmt.with
        .iter()
        .any(|cte| cte_body_references_table(&cte.body, table_name))
        || stmt
...
pub fn delete_statement_references_table(stmt: &DeleteStatement, table_name: &str) -> bool {
    stmt.with
        .iter()
        .any(|cte| cte_body_references_table(&cte.body, table_name))
...
pub fn merge_statement_references_table(stmt: &MergeStatement, table_name: &str) -> bool {
    stmt.with
        .iter()
        .any(|cte| cte_body_references_table(&cte.body, table_name))
...
pub fn bound_cte_from_materialized_rows(
    name: String,
    desc: &RelationDesc,
    rows: &[Vec<Value>],
) -> BoundCte {
...
pub fn bound_cte_from_query_rows(
    name: String,
...
pub fn bound_cte_from_query_columns(name: String, output_columns: Vec<QueryColumn>) -> BoundCte {
...
fn build_values_plan_with_outer(
    ...
    let query = if config.fold_constants {
        crate::runtime::fold_query_constants(query)?
    } else {
        query
    };
                    ...
                    with_grouped_agg_cte_context(&visible_ctes, &local_ctes, || {
                        let accumulators: Vec<AggAccum> = aggs
                            ...
                            .map(|agg| {
                                ...
                                let (coerced_direct_args, coerced_args, bound_order_by) =
                                    if hypothetical && builtin_hypothetical {
                                        ...
                                        let bound_order_by = bound_order_exprs
                                            ...
                                            .map(|(bound_expr, item)| {
                                                build_bound_order_by_entry(
                                                    item, bound_expr, 0, catalog,
                                                )
                                            })
                                ...
                                {
                                    let resolved = resolve_hypothetical_aggregate_call(&agg.name)
                                        .ok_or_else(|| ParseError::UnexpectedToken {
                                        expected: "supported aggregate",
                                        actual: agg.name.clone(),
                                    })?;
