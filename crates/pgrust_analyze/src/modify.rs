...
use pgrust_nodes::record::lookup_anonymous_record_descriptor;

use crate::rewrite::{RlsWriteCheck, ViewDmlEvent, ViewDmlRewriteError};
use crate::runtime::{
...
pub struct BoundInsertStatement {
    ...
    pub raw_on_conflict: Option<pgrust_nodes::parsenodes::OnConflictClause>,
    pub returning: Vec<TargetEntry>,
    pub rls_write_checks: Vec<RlsWriteCheck>,
...
pub struct BoundUpdateTarget {
    ...
    pub parent_rls_write_checks: Vec<RlsWriteCheck>,
    pub rls_write_checks: Vec<RlsWriteCheck>,
}
...
pub struct BoundMergeStatement {
    ...
    pub returning: Vec<TargetEntry>,
    pub required_privileges: Vec<RelationPrivilegeRequirement>,
    pub input_plan: pgrust_nodes::plannodes::PlannedStmt,
}
...
fn view_check_as_user_oid(view_oid: u32, catalog: &dyn CatalogLookup) -> Option<u32> {
    if relation_has_security_invoker(view_oid, catalog) {
        None
    } else {
...
fn mapped_view_context_columns(context: &crate::rewrite::ViewPrivilegeContext) -> Vec<usize> {
...
fn resolve_update_assignment_column(
    scope: &BoundScope,
    target: &pgrust_nodes::parsenodes::AssignmentTarget,
    relation_name: &str,
...
fn resolve_assignment_indirection_sql_type(
    column_type: SqlType,
    indirection: &[pgrust_nodes::parsenodes::AssignmentTargetIndirection],
    catalog: &dyn CatalogLookup,
) -> Result<SqlType, ParseError> {
    let mut current = column_type;
    for step in indirection {
        current = assignment_navigation_sql_type(current, catalog);
        match step {
            pgrust_nodes::parsenodes::AssignmentTargetIndirection::Subscript(subscript) => {
                if current.kind == SqlTypeKind::Jsonb && !current.is_array {
                    if subscript.is_slice {
                        return Err(ParseError::DetailedError {
                            message: "jsonb subscript does not support slices".into(),
            ...
            pgrust_nodes::parsenodes::AssignmentTargetIndirection::Field(field) => {
                current = resolve_assignment_field_type(current, field, catalog)?;
            }
        }
    }
...
fn query_from_projection_with_target_security_quals(
    ...
    if let Some(rte) = input.rtable.first_mut() {
        match &mut rte.kind {
            pgrust_nodes::parsenodes::RangeTblEntryKind::Subquery { query } => {
...
fn build_visibility_write_checks(
    ...
    source: crate::rewrite::RlsWriteCheckSource,
) -> Vec<RlsWriteCheck> {
    visibility_quals
        .into_iter()
...
fn build_conflict_visibility_checks(visibility_quals: Vec<Expr>) -> Vec<RlsWriteCheck> {
    build_visibility_write_checks(
        visibility_quals,
        crate::rewrite::RlsWriteCheckSource::ConflictUpdateVisibility,
    )
}

fn auto_view_base_rls_user_oid(
    resolved: &crate::rewrite::ResolvedAutoViewTarget,
    catalog: &dyn CatalogLookup,
) -> u32 {
    resolved
        .privilege_contexts
...
fn build_auto_view_base_row_security(
    ...
    catalog: &dyn CatalogLookup,
) -> Result<crate::rewrite::TargetRlsState, ViewDmlRewriteError> {
...
fn on_conflict_update_view_privilege_requirement(
    clause: &pgrust_nodes::parsenodes::OnConflictClause,
    ...
    if !matches!(
        clause.action,
        pgrust_nodes::parsenodes::OnConflictAction::Update
    ) {
        return Ok(None);
    }
...
fn rewrite_merge_action_select_item(
    item: &pgrust_nodes::parsenodes::SelectItem,
) -> (pgrust_nodes::parsenodes::SelectItem, bool) {
    let (expr, changed) = rewrite_merge_action_expr(&item.expr);
    (
        pgrust_nodes::parsenodes::SelectItem {
...
fn bind_merge_returning_targets(
    targets: &[pgrust_nodes::parsenodes::SelectItem],
    scope: &BoundScope,
...
pub fn plan_merge_with_outer_scopes_and_ctes(
    ...
    if let Some(resolved) = auto_view_target.as_ref() {
        let view_checks = resolved
            ...
            .map(|check| RlsWriteCheck {
                ...
                policy_name: None,
                source: crate::rewrite::RlsWriteCheckSource::ViewCheckOption(check.view_name),
            })
            .collect::<Vec<_>>();
    ...
    Ok(BoundMergeStatement {
        ...
        merge_update_visibility_checks: build_visibility_write_checks(
            ...
            crate::rewrite::RlsWriteCheckSource::MergeUpdateVisibility,
        ),
        merge_delete_visibility_checks: build_visibility_write_checks(
            merge_delete_rls.visibility_quals,
            crate::rewrite::RlsWriteCheckSource::MergeDeleteVisibility,
        ),
        ...
        returning,
        required_privileges,
        input_plan: crate::runtime::fold_query_constants(query).map(|query| {
...
fn map_auto_view_column_index(
    view_desc: &RelationDesc,
    updatable_column_map: &[Option<usize>],
    non_updatable_column_reasons: &[Option<crate::rewrite::NonUpdatableViewColumn>],
    column_index: usize,
    ...
    updatable_column_map
        ...
        .ok_or_else(|| {
            ...
            let reason = non_updatable_column_reasons
                ...
                .cloned()
                .flatten()
                .unwrap_or_else(|| crate::rewrite::NonUpdatableViewColumn {
                    relation_name: "<unknown>".into(),
                    reason: crate::rewrite::NonUpdatableViewColumnReason::NotBaseRelationColumn,
                });
            ViewDmlRewriteError::NonUpdatableColumn {
...
fn bind_auto_view_on_conflict_clause(
    ...
    .map_err(|err| ViewDmlRewriteError::UnsupportedViewShape(err.to_string()))?;
    let action = match clause.action {
        pgrust_nodes::parsenodes::OnConflictAction::Nothing => BoundOnConflictAction::Nothing,
        pgrust_nodes::parsenodes::OnConflictAction::Update => {
            ...
            let predicate = clause
                ...
                .map(|expr| {
                    ...
                    let expr = rewrite_local_vars_for_output_exprs(expr, 1, &old_view_output_exprs);
                    Ok::<Expr, ViewDmlRewriteError>(rewrite_local_vars_for_output_exprs(
                        expr,
                        2,
                        &new_view_output_exprs,
                    ))
            ...
            .chain(
                resolved
                    ...
                    .map(|check| RlsWriteCheck {
                        ...
                        display_exprs: resolved.visible_output_exprs.clone(),
                        policy_name: None,
                        source: crate::rewrite::RlsWriteCheckSource::ViewCheckOption(
                            check.view_name,
                        ),
                    }),
            )
    ...
    desc.columns
        ...
        .map(|column| {
            ...
            default_expr
                .as_ref()
                .map(|sql| {
                    let expr = pgrust_parser::parse_expr(sql)?;
                    bind_expr_with_outer_and_ctes(
                        &expr,
                        &empty_scope(),
                        catalog,
...
fn column_has_stored_default(column: &pgrust_nodes::primnodes::ColumnDesc) -> bool {
        ...
        let conflict_quals = update_rls
            ...
            .filter(|qual| {
                !update_rls.write_checks.iter().any(|check| {
                    matches!(check.source, crate::rewrite::RlsWriteCheckSource::Update)
                        && check.expr == *qual
                })
            })
            .collect();
...
fn bind_update_from(
    ...
    let [mut query] = crate::runtime::pg_rewrite_query(query, catalog)?
        .try_into()
        .expect("UPDATE FROM input rewrite should return a single query");
    query.command_type = CommandType::Update;
    let input_plan = crate::runtime::fold_query_constants(query)
        .map(|query| crate::runtime::planner(query, catalog))??;
