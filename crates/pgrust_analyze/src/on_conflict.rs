...
use pgrust_nodes::parsenodes::{OnConflictAction, OnConflictClause, OnConflictTarget};
use pgrust_nodes::primnodes::{AttrNumber, INNER_VAR, OUTER_VAR, Var, user_attrno};
...
pub enum BoundOnConflictAction {
    ...
    Update {
        ...
        conflict_visibility_checks: Vec<crate::rewrite::RlsWriteCheck>,
        update_write_checks: Vec<crate::rewrite::RlsWriteCheck>,
    },
}
...
pub(super) fn resolve_arbiters_with_inference_scope(
    ...
    match clause.target.as_ref() {
        ...
        Some(OnConflictTarget::Constraint(name)) => {
            ...
            if row.contype == pgrust_catalog_data::CONSTRAINT_EXCLUSION {
...
fn bind_inference_element(
    element: &pgrust_nodes::parsenodes::OnConflictInferenceElem,
    scope: &BoundScope,
    catalog: &dyn CatalogLookup,
...
fn bound_opclass_spec_from_row(row: pgrust_catalog_data::PgOpclassRow) -> BoundOpclassSpec {
    BoundOpclassSpec {
