...
use crate::runtime::cast_value_with_source_type;
...
use pgrust_nodes::datum::{
    ArrayDimension, ArrayValue, MultirangeTypeRef, MultirangeValue, NumericValue, RangeBound,
    RangeValue, RecordDescriptor, RecordValue,
};
use pgrust_nodes::partition::{
    LoweredPartitionClause, LoweredPartitionSpec, PartitionBoundSpec, PartitionRangeDatumValue,
    SerializedPartitionArrayValue, SerializedPartitionMultirangeValue,
    SerializedPartitionRangeBound, SerializedPartitionRangeValue, SerializedPartitionRecordField,
    SerializedPartitionRecordValue, SerializedPartitionValue,
};
...
use pgrust_parser::parse_expr;

pub fn lower_partition_clause(
...
pub fn validate_partitioned_index_backed_constraints(
    relation_name: &str,
...
pub fn validate_partitioned_check_constraints(
    relation_name: &str,
    partition_spec: Option<&LoweredPartitionSpec>,
...
pub fn validate_partitioned_not_null_constraints(
    partition_spec: Option<&LoweredPartitionSpec>,
...
pub fn relation_partition_spec(
...
pub fn lower_relation_partition_spec_uncached(
    relation: &BoundRelation,
) -> Result<LoweredPartitionSpec, ParseError> {
...
pub fn pg_partitioned_table_row(
    relation_oid: u32,
...
pub fn serialize_partition_bound(bound: &PartitionBoundSpec) -> Result<String, ParseError> {
...
pub fn deserialize_partition_bound(text: &str) -> Result<PartitionBoundSpec, ParseError> {
...
pub fn partition_value_to_value(value: &SerializedPartitionValue) -> Value {
    match value {
        ...
        SerializedPartitionValue::Numeric(v) => Value::Numeric(NumericValue::from(v.clone())),
        ...
        SerializedPartitionValue::Date(v) => Value::Date(pgrust_nodes::datetime::DateADT(*v)),
        SerializedPartitionValue::Time(v) => Value::Time(pgrust_nodes::datetime::TimeADT(*v)),
        SerializedPartitionValue::TimeTz {
            time,
            offset_seconds,
        } => Value::TimeTz(pgrust_nodes::datetime::TimeTzADT {
            time: pgrust_nodes::datetime::TimeADT(*time),
            offset_seconds: *offset_seconds,
        }),
        SerializedPartitionValue::Timestamp(v) => {
            Value::Timestamp(pgrust_nodes::datetime::TimestampADT(*v))
        }
        SerializedPartitionValue::TimestampTz(v) => {
            Value::TimestampTz(pgrust_nodes::datetime::TimestampTzADT(*v))
        }
...
pub fn value_to_partition_value(value: &Value) -> Result<SerializedPartitionValue, ParseError> {
    Ok(match value {
        ...
        other => {
            return Err(ParseError::FeatureNotSupported(format!(
                ...
                other
                    .sql_type_hint()
                    .unwrap_or(SqlType::new(pgrust_nodes::parsenodes::SqlTypeKind::Text))
            )));
        }
    })
}
...
fn lower_partition_spec(
    ...
    for key in &spec.keys {
        ...
        let planned = crate::runtime::fold_expr_constants(bound.clone())?;
...
fn partition_key_pseudotype_name(key_type: SqlType, raw_expr: &SqlExpr) -> Option<&'static str> {
    if key_type.type_oid == pgrust_catalog_data::UNKNOWN_TYPE_OID
        || matches!(
            raw_expr,
...
fn partition_func_is_mutable(func: &FuncExpr, catalog: &dyn CatalogLookup) -> bool {
    ...
    match func.implementation {
        ScalarFunctionImpl::Builtin(builtin) => matches!(
            builtin,
            pgrust_nodes::primnodes::BuiltinScalarFunction::Random
                | pgrust_nodes::primnodes::BuiltinScalarFunction::RandomNormal
                | pgrust_nodes::primnodes::BuiltinScalarFunction::Now
                | pgrust_nodes::primnodes::BuiltinScalarFunction::ClockTimestamp
                | pgrust_nodes::primnodes::BuiltinScalarFunction::StatementTimestamp
                | pgrust_nodes::primnodes::BuiltinScalarFunction::TransactionTimestamp
        ),
...
fn evaluate_partition_bound_expr(
    ...
    let folded = crate::runtime::fold_expr_constants(bound)?;
    ...
    cast_value_with_source_type(value, None, target, Some(catalog))
...
fn partition_bound_const_expr_value(
    ...
    match expr {
        ...
        Expr::CurrentTimestamp { precision } => Ok(Some(crate::runtime::current_timestamp_value(
            *precision, true,
        ))),
        Expr::LocalTimestamp { precision } => Ok(Some(crate::runtime::current_timestamp_value(
            *precision, false,
        ))),
        ...
        Expr::Cast(inner, target) => {
            ...
            let value = cast_value_with_source_type(value, source_type, *target, Some(catalog))
            ...
            ) =>
        {
            ...
            crate::runtime::eval_to_char_function(&values)
                .map(Some)
                .map_err(|_| partition_bound_error("partition bound values must be constant"))
        }
        _ => Ok(None),
