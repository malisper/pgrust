...
use super::*;
use crate::runtime::cast_value;
use pgrust_catalog_data::{
    ANYELEMENTOID, ANYMULTIRANGEOID, ANYOID, ANYRANGEOID, BIT_TYPE_OID, BTREE_AM_OID,
    CIDR_TYPE_OID, GIST_AM_OID, GIST_MULTIRANGE_FAMILY_OID, GIST_RANGE_FAMILY_OID, HASH_AM_OID,
    INET_TYPE_OID, SPGIST_AM_OID, SPGIST_TEXT_FAMILY_OID, TIMESTAMP_TYPE_OID, TIMESTAMPTZ_TYPE_OID,
    VARBIT_TYPE_OID, bootstrap_pg_operator_rows, builtin_range_spec_by_multirange_oid,
    builtin_range_spec_by_oid, builtin_scalar_function_for_proc_oid,
    proc_oid_for_builtin_scalar_function, sql_type_oid,
};
use pgrust_nodes::primnodes::{BuiltinScalarFunction, OpExprKind, expr_sql_type_hint};
...
pub enum BoundModifyRowSource {
    ...
    Index {
        ...
        keys: Vec<pgrust_nodes::access::ScanKeyData>,
    },
}
...
fn simple_column_index(expr: &Expr) -> Option<usize> {
    ...
    match strip_casts(expr) {
        Expr::Var(var)
            if var.varlevelsup == 0 && !pgrust_nodes::primnodes::is_system_attr(var.varattno) =>
        {
...
fn build_btree_scan_keys(
...
) -> (Vec<pgrust_nodes::access::ScanKeyData>, usize) {
    ...
    for index_pos in 0..index_key_count(index) {
            ...
            })
        {
            used[qual_idx] = true;
            equality_prefix += 1;
            keys.push(pgrust_nodes::access::ScanKeyData {
...
fn build_gist_scan_keys(
    index: &BoundIndexRelation,
    parsed_quals: &[IndexableQual],
) -> Vec<pgrust_nodes::access::ScanKeyData> {
    parsed_quals
        .iter()
        .filter_map(|qual| {
            ...
            Some(pgrust_nodes::access::ScanKeyData {
                attribute_number: (index_pos + 1) as i16,
                strategy,
...
fn build_hash_scan_keys(
    ...
    parsed_quals
        ...
        .find_map(|qual| {
            ...
            (strategy == 1).then_some(pgrust_nodes::access::ScanKeyData {
                attribute_number: 1,
                strategy,
                argument: qual.argument.clone(),
            })
...
fn index_order_match(
    ...
    for (idx, item) in items.iter().enumerate() {
        ...
        let item_direction = if item.descending {
            pgrust_nodes::access::ScanDirection::Backward
        } else {
            pgrust_nodes::access::ScanDirection::Forward
        };
...
) -> Result<Vec<OrderByEntry>, ParseError> {
    items
        .iter()
        .map(|item| {
...
fn index_amop_strategy_for_operator(
    index: &IndexRelCacheEntry,
    desc: &RelationDesc,
    column_index: usize,
    operator_oid: u32,
...
fn index_amop_strategy_for_proc(
    index: &IndexRelCacheEntry,
    desc: &RelationDesc,
    column_index: usize,
