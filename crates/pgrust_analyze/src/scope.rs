...
pub struct ScopeColumn {
    ...
    pub relation_output_exprs: Vec<(String, Expr)>,
    pub hidden_invalid_relation_names: Vec<String>,
    pub hidden_missing_relation_names: Vec<String>,
    pub source_relation_oid: Option<u32>,
    pub source_attno: Option<AttrNumber>,
    pub source_columns: Vec<(u32, AttrNumber)>,
}
...
#[derive(Debug, Clone)]
pub struct GroupedOuterScope {
    pub scope: BoundScope,
    pub group_by_exprs: Vec<SqlExpr>,
}
...
#[derive(Debug, Clone)]
pub struct BoundWritableCte {
    pub cte: BoundCte,
    pub source: BoundModifyingCte,
}

#[derive(Debug, Clone)]
pub enum BoundModifyingCte {
    ...
    let mut matches = scope
        ...
        .filter_map(|entry| {
            ...
            (!matches!(varno, RULE_OLD_VAR | RULE_NEW_VAR)
                && varno != pgrust_nodes::primnodes::OUTER_VAR
                && varno != pgrust_nodes::primnodes::INNER_VAR)
                .then_some(varno)
        })
...
fn passthrough_cte_target<'a>(cte: &'a BoundCte, ctes: &'a [BoundCte]) -> Option<&'a BoundCte> {
    if cte.plan.command_type != pgrust_nodes::CommandType::Select
    ...
    let pgrust_nodes::parsenodes::RangeTblEntryKind::Cte { cte_id, .. } = rte.kind else {
        return None;
    };
    if !matches!(
        cte.plan.jointree,
        Some(pgrust_nodes::parsenodes::JoinTreeNode::RangeTblRef(1))
    ) {
        return None;
    }
    ...
    Ok(TableSampleClause {
        ...
        args: vec![Expr::Cast(
            ...
            SqlType::new(SqlTypeKind::Float4)
                .with_identity(pgrust_catalog_data::FLOAT4_TYPE_OID, 0),
        )],
        repeatable: repeatable.map(|expr| {
            Expr::Cast(
                Box::new(expr),
                SqlType::new(SqlTypeKind::Float8)
                    .with_identity(pgrust_catalog_data::FLOAT8_TYPE_OID, 0),
            )
        }),
    })
}
...
fn retarget_output_var_type(expr: Expr, output_columns: &[QueryColumn]) -> Expr {
    match expr {
        Expr::Var(mut var)
            if var.varno == 1
                && var.varlevelsup == 0
                && let Some(index) = pgrust_nodes::primnodes::attrno_index(var.varattno)
                ...
                sql_type: match &definition.ty {
                    ...
                    RawTypeName::Serial(kind) => {
                        return Err(ParseError::FeatureNotSupported(format!(
                            ...
                            match kind {
                                pgrust_nodes::parsenodes::SerialKind::Small => "smallserial",
                                pgrust_nodes::parsenodes::SerialKind::Regular => "serial",
                                pgrust_nodes::parsenodes::SerialKind::Big => "bigserial",
                            }
                        )));
                    }
...
pub fn scope_for_relation(relation_name: Option<&str>, desc: &RelationDesc) -> BoundScope {
    BoundScope {
        desc: desc.clone(),
...
pub fn scope_for_base_relation_with_optional_name(
...
pub fn shift_scope_rtindexes(mut scope: BoundScope, offset: usize) -> BoundScope {
    if offset == 0 {
        return scope;
    }
