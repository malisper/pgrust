...
use pgrust_nodes::parsenodes::{JoinTreeNode, RangeTblEntryKind};
use pgrust_nodes::primnodes::{
    SetReturningCall, attrno_index, is_system_attr, set_returning_call_exprs,
};

use crate::rewrite::ViewDmlEvent;
use crate::runtime::{
    classify_view_dml_rules, format_trigger_definition, load_view_return_query,
    load_view_return_select, render_relation_expr_sql_for_information_schema,
};
use crate::system_view_registry::{SyntheticSystemViewKind, synthetic_system_view};
...
pub(super) fn bind_builtin_system_view(
    ...
    let rows = match view.kind {
        ...
        SyntheticSystemViewKind::PgPublicationTables => catalog.pg_publication_tables_rows(
        ...
        SyntheticSystemViewKind::PgStatRecoveryPrefetch => catalog.pg_stat_recovery_prefetch_rows(),
        SyntheticSystemViewKind::PgStatSubscriptionStats => {
            catalog.pg_stat_subscription_stats_rows()
        }
        ...
        SyntheticSystemViewKind::InformationSchemaRoutines => {
            information_schema_routine_rows(catalog)
        }
        ...
        SyntheticSystemViewKind::InformationSchemaDomains => {
            information_schema_domain_rows(catalog)
        }
        ...
        SyntheticSystemViewKind::InformationSchemaForeignDataWrappers => catalog
            .information_schema_foreign_data_wrappers_rows(
                catalog.authid_rows(),
                catalog.auth_members_rows(),
                catalog.foreign_data_wrapper_rows(),
                catalog.current_user_oid(),
            ),
        SyntheticSystemViewKind::InformationSchemaForeignDataWrapperOptions => catalog
            .information_schema_foreign_data_wrapper_options_rows(
                catalog.authid_rows(),
                catalog.auth_members_rows(),
                catalog.foreign_data_wrapper_rows(),
                catalog.current_user_oid(),
            ),
        SyntheticSystemViewKind::InformationSchemaForeignServers => catalog
            .information_schema_foreign_servers_rows(
                catalog.authid_rows(),
                catalog.auth_members_rows(),
                catalog.foreign_data_wrapper_rows(),
        ...
        SyntheticSystemViewKind::InformationSchemaForeignServerOptions => catalog
            .information_schema_foreign_server_options_rows(
                catalog.authid_rows(),
                catalog.auth_members_rows(),
                catalog.foreign_server_rows(),
                catalog.current_user_oid(),
            ),
        SyntheticSystemViewKind::InformationSchemaUserMappings => catalog
            .information_schema_user_mappings_rows(
                catalog.authid_rows(),
                catalog.auth_members_rows(),
                catalog.foreign_server_rows(),
        ...
        SyntheticSystemViewKind::InformationSchemaUserMappingOptions => catalog
            .information_schema_user_mapping_options_rows(
                catalog.authid_rows(),
                catalog.auth_members_rows(),
                catalog.foreign_server_rows(),
                catalog.user_mapping_rows(),
        ...
        | SyntheticSystemViewKind::InformationSchemaRoleUsageGrants => catalog
            .information_schema_usage_privileges_rows(
                catalog.authid_rows(),
                catalog.auth_members_rows(),
                catalog.foreign_data_wrapper_rows(),
                catalog.foreign_server_rows(),
        ...
        SyntheticSystemViewKind::InformationSchemaForeignTables => catalog
            .information_schema_foreign_tables_rows(
                catalog.namespace_rows(),
                catalog.class_rows(),
                catalog.foreign_server_rows(),
                catalog.foreign_table_rows(),
            ),
        SyntheticSystemViewKind::InformationSchemaForeignTableOptions => catalog
            .information_schema_foreign_table_options_rows(
                catalog.namespace_rows(),
                catalog.class_rows(),
                catalog.foreign_table_rows(),
            ),
        ...
        for (index, column) in relation.desc.columns.iter().enumerate() {
            ...
            rows.push(vec![
                ...
                column
                    .identity
                    .map(|kind| match kind {
                        pgrust_nodes::parsenodes::ColumnIdentityKind::Always => {
                            Value::Text("ALWAYS".into())
                        }
                        pgrust_nodes::parsenodes::ColumnIdentityKind::ByDefault => {
...
fn routine_depend_rows(
    ...
    pgrust_catalog_data::PgProcRow,
    pgrust_catalog_data::PgDependRow,
)> {
...
fn user_visible_routines(catalog: &dyn CatalogLookup) -> Vec<pgrust_catalog_data::PgProcRow> {
    catalog
        .proc_rows()
        .into_iter()
        .filter(|row| row.prokind == 'f')
...
fn information_schema_relation_check_clause(
    catalog: &dyn CatalogLookup,
    row: &pgrust_catalog_data::PgConstraintRow,
) -> Option<String> {
...
fn information_schema_domain_check_clause(
    constraint: &pgrust_catalog_data::PgConstraintRow,
) -> String {
...
fn format_generation_expression_sql(sql: &str) -> String {
    ...
    match pgrust_parser::parse_expr(trimmed) {
        Ok(SqlExpr::Column(_))
        | Ok(SqlExpr::Const(_))
...
fn information_schema_trigger_rows(catalog: &dyn CatalogLookup) -> Vec<Vec<Value>> {
    ...
    for trigger in catalog.trigger_rows() {
        ...
        let Some(formatted) = format_trigger_definition(&trigger, None, catalog) else {
            continue;
        };
...
fn view_definition_and_check_option(
    ...
    if let Some(relation) = catalog.relation_by_oid(relation_oid)
        && let Ok(definition) =
            crate::runtime::format_view_definition(relation_oid, &relation.desc, catalog)
    {
        let sql = catalog
            ...
            .unwrap_or_default();
        let (_, check_option) = crate::runtime::split_stored_view_definition_sql(&sql);
        return (
            definition,
            reloption_check_option.unwrap_or(match check_option {
                pgrust_nodes::parsenodes::ViewCheckOption::None => "NONE",
                pgrust_nodes::parsenodes::ViewCheckOption::Local => "LOCAL",
                pgrust_nodes::parsenodes::ViewCheckOption::Cascaded => "CASCADED",
            }),
        );
    }
    let sql = catalog
        ...
        .map(|row| row.ev_action)
        .unwrap_or_default();
    let (definition, check_option) = crate::runtime::split_stored_view_definition_sql(&sql);
    (
...
pub fn pg_relation_is_updatable_events(
    catalog: &dyn CatalogLookup,
    relation_oid: u32,
