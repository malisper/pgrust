...
use pgrust_nodes::primnodes::AggFunc;
...
pub fn builtin_range_rows() -> Vec<PgRangeRow> {
    builtin_range_specs()
        ...
        .map(|spec| PgRangeRow {
            ...
            rngcollation: 0,
            rngsubopc: crate::default_btree_opclass_oid(spec.range_type.subtype_oid())
                .unwrap_or_else(|| {
                    if spec.range_type.subtype.is_array {
                        crate::ARRAY_BTREE_OPCLASS_OID
                    } else {
                        0
                    }
                }),
...
fn range_subtype_ref_for_sql_type(sql_type: SqlType) -> Option<SqlType> {
    if let Some(row) = crate::builtin_type_rows()
        .into_iter()
