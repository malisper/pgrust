...
fn build_bootstrap_pg_amop_rows() -> Vec<PgAmopRow> {
    ...
    for (strategy, righttype) in [
        (48_i16, crate::POLYGON_TYPE_OID),
        (68, crate::CIRCLE_TYPE_OID),
    ] {
        rows.push(PgAmopRow {
            oid,
            amopfamily: GIST_POINT_FAMILY_OID,
...
fn operator_oid(rows: &[crate::PgOperatorRow], name: &str, left: u32, right: u32) -> u32 {
    rows.iter()
