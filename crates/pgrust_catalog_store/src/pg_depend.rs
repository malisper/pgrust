use crate::catalog::{CatalogEntry, CatalogIndexMeta};
...
pub fn collect_sql_expr_column_names(expr: &SqlExpr, out: &mut BTreeSet<String>) {
    match expr {
        SqlExpr::Column(name) => {
...
pub fn operator_depend_rows(row: &pgrust_catalog_data::PgOperatorRow) -> Vec<PgDependRow> {
    let mut rows = vec![PgDependRow {
...
fn is_pinned_depend_object(classid: u32, objid: u32) -> bool {
    match classid {
        PG_TYPE_RELATION_OID => pgrust_catalog_data::builtin_type_row_by_oid(objid).is_some(),
        PG_PROC_RELATION_OID => pgrust_catalog_data::bootstrap_pg_proc_row_by_oid(objid).is_some(),
        _ => false,
    }
}
...
pub fn conversion_depend_rows(row: &pgrust_catalog_data::PgConversionRow) -> Vec<PgDependRow> {
    let mut rows = vec![PgDependRow {
...
pub fn collation_depend_rows(row: &pgrust_catalog_data::PgCollationRow) -> Vec<PgDependRow> {
    let mut rows = vec![PgDependRow {
