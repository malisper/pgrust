...
use crate::bootstrap::{bootstrap_catalog_entry, bootstrap_catalog_kinds};
use crate::catalog::allocate_relation_object_oids;
use crate::catalog::column_desc;
use crate::catcache::sql_type_oid;
use crate::indexing::insert_bootstrap_system_indexes;
use crate::pg_constraint::{derived_pg_constraint_rows, sort_pg_constraint_rows};
use crate::pg_depend::{
...
use crate::pg_partitioned_table::sort_pg_partitioned_table_rows;
...
use pgrust_nodes::access::BrinOptions;
use pgrust_nodes::access::BtreeOptions;
use pgrust_nodes::access::GinOptions;
use pgrust_nodes::access::GistOptions;
use pgrust_nodes::access::HashOptions;
...
fn build_catalog_index_entry(
    ...
    unique: bool,
    primary: bool,
    columns: &[pgrust_nodes::parsenodes::IndexColumnDef],
...
fn index_column_opckey_sql_type(
    ...
    if opclass.opcmethod != pgrust_catalog_data::GIST_AM_OID || opclass.opckeytype == 0 {
        return None;
    }
...
fn explicit_opclass_oid(am_oid: u32, opclass_name: &str, type_oid: u32) -> Option<u32> {
    ...
    pgrust_catalog_data::bootstrap_pg_opclass_rows()
        .into_iter()
        .find(|row| {
...
fn default_index_build_options_for_entry(
    ...
    for column_name in columns {
        ...
        let opclass_oid = if let Some(opclass_name) = column_name.opclass.as_deref() {
            ...
            pgrust_catalog_data::default_opclass_oid_for_am(am_oid, type_oid, sql_type)
    ...
    Ok(CatalogIndexBuildOptions {
        ...
        indimmediate: true,
        btree_options: None,
        brin_options: (am_oid == pgrust_catalog_data::BRIN_AM_OID)
            .then(pgrust_nodes::access::BrinOptions::default),
        gist_options: (am_oid == pgrust_catalog_data::GIST_AM_OID)
            .then(pgrust_nodes::access::GistOptions::default),
        gin_options: (am_oid == pgrust_catalog_data::GIN_AM_OID)
            .then(pgrust_nodes::access::GinOptions::default),
        hash_options: (am_oid == pgrust_catalog_data::HASH_AM_OID)
            .then(pgrust_nodes::access::HashOptions::default),
    })
}

fn push_unique_index_column(
    columns: &mut Vec<pgrust_nodes::primnodes::ColumnDesc>,
    used_names: &mut BTreeSet<String>,
    mut column: pgrust_nodes::primnodes::ColumnDesc,
) {
...
#[derive(Debug, Clone, PartialEq)]
pub struct Catalog {
    pub tables: BTreeMap<String, CatalogEntry>,
    ...
    pub triggers: Vec<pgrust_catalog_data::PgTriggerRow>,
...
impl Catalog {
    ...
    pub fn create_table_with_relkind(
        &mut self,
        name: impl Into<String>,
        ...
        let entry = CatalogEntry {
            ...
            relpersistence,
            relkind,
            am_oid: pgrust_catalog_data::relam_for_relkind(relkind),
            relhassubclass: false,
            relhastriggers: false,
            relispartition: false,
            relispopulated: true,
    ...
    pub fn create_check_constraint(
        ...
        let row = PgConstraintRow {
            ...
            contype: pgrust_catalog_data::CONSTRAINT_CHECK,
            condeferrable: false,
            condeferred: false,
            conenforced,
            convalidated,
    ...
    pub fn create_foreign_key_constraint(
        ...
        let row = PgConstraintRow {
            ...
            conname,
            connamespace: table.namespace_oid,
            contype: pgrust_catalog_data::CONSTRAINT_FOREIGN,
            condeferrable: deferrable,
            condeferred: initially_deferred,
            conenforced,
            convalidated,
    ...
    pub fn set_column_not_null(
        ...
        let old_entry = self
            ...
            .cloned()
            .ok_or_else(|| CatalogError::UnknownTable(relation_oid.to_string()))?;
        if !matches!(old_entry.relkind, 'r' | 'p' | 'v') {
            return Err(CatalogError::UnknownTable(relation_oid.to_string()));
        }
    ...
    pub fn validate_check_constraint(
        ...
        let row = self
            ...
            .find(|row| {
                row.conrelid == relation_oid
                    && row.contype == pgrust_catalog_data::CONSTRAINT_CHECK
    ...
    pub fn validate_foreign_key_constraint(
        ...
        let row = self
            ...
            .find(|row| {
                row.conrelid == relation_oid
                    && row.contype == pgrust_catalog_data::CONSTRAINT_FOREIGN
                    && row.conname.eq_ignore_ascii_case(constraint_name)
            })
            .ok_or_else(|| CatalogError::UnknownTable(constraint_name.to_string()))?;
        let old_row = row.clone();
    ...
    pub fn default_index_build_options(
        ...
        for column_name in columns {
            ...
            let opclass_oid = if let Some(opclass_name) = column_name.opclass.as_deref() {
                explicit_opclass_oid(pgrust_catalog_data::BTREE_AM_OID, opclass_name, type_oid)
                    .ok_or_else(|| CatalogError::UnknownType("index operator class".into()))?
            } else {
                pgrust_catalog_data::default_opclass_oid_for_am(
                    pgrust_catalog_data::BTREE_AM_OID,
                    type_oid,
                    column.sql_type,
                )
        ...
        Ok(CatalogIndexBuildOptions {
            am_oid: pgrust_catalog_data::BTREE_AM_OID,
            indclass,
            indclass_options,
            indcollation,
            indoption,
    ...
    pub fn alter_table_add_column(
        &mut self,
        relation_oid: u32,
        column: pgrust_nodes::primnodes::ColumnDesc,
    ) -> Result<(String, CatalogEntry, CatalogEntry), CatalogError> {
        let name = self
            .tables
            .iter()
    ...
    pub fn alter_table_set_column_generation(
        ...
        column_name: &str,
        default_expr: Option<String>,
        generated: Option<pgrust_nodes::parsenodes::ColumnGeneratedKind>,
    ) -> Result<(String, CatalogEntry, CatalogEntry), CatalogError> {
        let name = self
            .tables
            .iter()
    ...
    pub fn alter_table_set_column_storage(
        ...
        relation_oid: u32,
        column_name: &str,
        storage: pgrust_core::AttributeStorage,
    ) -> Result<(String, CatalogEntry, CatalogEntry), CatalogError> {
        let name = self
            .tables
            .iter()
    ...
    pub fn alter_table_alter_column_type(
        ...
        relation_oid: u32,
        column_name: &str,
        new_column: pgrust_nodes::primnodes::ColumnDesc,
    ) -> Result<(String, CatalogEntry, CatalogEntry), CatalogError> {
        let name = self
            .tables
            .iter()
    ...
    pub fn create_trigger(&mut self, mut row: PgTriggerRow) -> Result<PgTriggerRow, CatalogError> {
        ...
        pgrust_catalog_data::sort_pg_trigger_rows(&mut self.triggers);
...
fn foreign_key_equality_operators(indclass: &[u32]) -> Option<Vec<u32>> {
    let opclasses = pgrust_catalog_data::bootstrap_pg_opclass_rows();
    let amops = pgrust_catalog_data::bootstrap_pg_amop_rows();
    indclass
        .iter()
        .map(|opclass_oid| {
...
pub fn validate_builtin_type_rows(desc: &RelationDesc) -> Result<(), CatalogError> {
...
fn format_sql_type_name(sql_type: SqlType) -> String {
    ...
    if sql_type.is_multirange() {
        return pgrust_catalog_data::builtin_multirange_name_for_sql_type(sql_type)
