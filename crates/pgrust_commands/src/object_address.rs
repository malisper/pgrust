use pgrust_analyze::{BoundRelation, CatalogLookup, resolve_raw_type_name};
use pgrust_catalog_data::*;
use pgrust_expr::expr_reg;
use pgrust_nodes::datetime::TimestampTzADT;
use pgrust_nodes::parsenodes::RawTypeName;
...
fn relation_by_names(
    catalog: &dyn CatalogLookup,
    names: &[String],
) -> Result<BoundRelation, ObjectAddressError> {
...
fn type_oid_from_name(catalog: &dyn CatalogLookup, name: &str) -> Result<u32, ObjectAddressError> {
    ...
    let raw = pgrust_parser::parse_type_name(name).unwrap_or_else(|_| RawTypeName::Named {
        name: name.to_string(),
        array_bounds: 0,
    });
...
impl pgrust_expr::ExprCatalogLookup for ExprCatalogAdapter<'_> {
    ...
    fn type_oid_for_sql_type(&self, sql_type: pgrust_nodes::parsenodes::SqlType) -> Option<u32> {
        self.0.type_oid_for_sql_type(sql_type)
    }
}

fn type_identity_text(catalog: &dyn CatalogLookup, type_oid: u32) -> String {
    let expr_catalog = ExprCatalogAdapter(catalog);
    catalog
        ...
        .map(|row| {
            ...
            if row.typnamespace == PG_CATALOG_NAMESPACE_OID {
                expr_reg::format_type_text(type_oid, None, &expr_catalog)
            } else {
                qualified_namespace_name(catalog, row.typnamespace, &row.typname)
            }
        })
        .unwrap_or_else(|| expr_reg::format_type_text(type_oid, None, &expr_catalog))
}
