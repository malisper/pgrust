...
pub fn format_trigger_definition(
    ...
    let action_timing = format_trigger_timing(row.tgtype);
    let action_orientation = if trigger_is_row(row.tgtype) {
        "ROW"
    } else {
        "STATEMENT"
    };
...
fn trigger_event_list(row: &PgTriggerRow, columns: &[ColumnDesc]) -> String {
    let mut events = Vec::new();
...
fn format_trigger_timing(tgtype: i16) -> &'static str {
    if trigger_is_instead(tgtype) {
        "INSTEAD OF"
    } else if trigger_is_before(tgtype) {
        "BEFORE"
