...
use pgrust_analyze::bound_index_relation_from_relcache_entry_with_heap_and_cache;
use pgrust_analyze::{BoundRelation, CatalogLookup, DomainLookup};
...
use pgrust_catalog_store::catcache::CatCache;
use pgrust_catalog_store::pg_constraint::derived_pg_constraint_rows;
use pgrust_catalog_store::relcache::RelCache;
use pgrust_executor::DatabaseStatsStore;
use pgrust_nodes::pathnodes::PlannerIndexExprCacheEntry;
use pgrust_rewrite::{format_stored_rule_definition_with_catalog, format_view_definition};
...
impl VisibleCatalog {
    ...
    pub fn foreign_server_rows(&self) -> Vec<pgrust_catalog_data::PgForeignServerRow> {
        self.catcache
            .as_ref()
    ...
    pub fn foreign_table_rows(&self) -> Vec<pgrust_catalog_data::PgForeignTableRow> {
        self.catcache
            .as_ref()
    ...
    pub fn user_mapping_rows(&self) -> Vec<pgrust_catalog_data::PgUserMappingRow> {
        self.catcache
            .as_ref()
...
impl CatalogLookup for VisibleCatalog {
    ...
    fn index_relations_for_heap(
        &self,
        relation_oid: u32,
    ) -> Vec<pgrust_analyze::BoundIndexRelation> {
    ...
    fn index_row_by_oid(&self, index_oid: u32) -> Option<pgrust_catalog_data::PgIndexRow> {
    ...
    fn proc_row_by_oid(&self, oid: u32) -> Option<PgProcRow> {
        self.catcache
            ...
            .or_else(|| {
                pgrust_catalog_data::synthetic_range_proc_row_by_oid(
                    oid,
                    &self.type_rows(),
                    &self.range_rows(),
                )
    ...
    fn attribute_rows(&self) -> Vec<pgrust_catalog_data::PgAttributeRow> {
        self.catcache
            .as_ref()
    ...
    fn foreign_server_rows(&self) -> Vec<pgrust_catalog_data::PgForeignServerRow> {
        VisibleCatalog::foreign_server_rows(self)
    }

    fn foreign_table_rows(&self) -> Vec<pgrust_catalog_data::PgForeignTableRow> {
        VisibleCatalog::foreign_table_rows(self)
    }

    fn user_mapping_rows(&self) -> Vec<pgrust_catalog_data::PgUserMappingRow> {
    ...
    ) -> Vec<Vec<pgrust_nodes::Value>> {
        crate::system_views::build_information_schema_foreign_data_wrappers_rows(
            authids,
            auth_members,
            wrappers,
            current_user_oid,
    ...
    fn information_schema_foreign_data_wrapper_options_rows(
        ...
        crate::system_views::build_information_schema_foreign_data_wrapper_options_rows(
            authids,
            auth_members,
            wrappers,
            current_user_oid,
    ...
    fn information_schema_foreign_servers_rows(
        ...
        auth_members: Vec<PgAuthMembersRow>,
        wrappers: Vec<PgForeignDataWrapperRow>,
        servers: Vec<pgrust_catalog_data::PgForeignServerRow>,
        current_user_oid: u32,
    ) -> Vec<Vec<pgrust_nodes::Value>> {
        crate::system_views::build_information_schema_foreign_servers_rows(
            authids,
            auth_members,
            wrappers,
            servers,
    ...
    fn information_schema_foreign_server_options_rows(
        ...
        crate::system_views::build_information_schema_foreign_server_options_rows(
            authids,
            auth_members,
            servers,
            current_user_oid,
    ...
    fn information_schema_user_mappings_rows(
        ...
        auth_members: Vec<PgAuthMembersRow>,
        servers: Vec<pgrust_catalog_data::PgForeignServerRow>,
        mappings: Vec<pgrust_catalog_data::PgUserMappingRow>,
        current_user_oid: u32,
    ) -> Vec<Vec<pgrust_nodes::Value>> {
        crate::system_views::build_information_schema_user_mappings_rows(
            authids,
            auth_members,
            servers,
            mappings,
    ...
    fn information_schema_user_mapping_options_rows(
        ...
        crate::system_views::build_information_schema_user_mapping_options_rows(
            authids,
            auth_members,
            servers,
            mappings,
    ...
    fn information_schema_usage_privileges_rows(
        ...
        crate::system_views::build_information_schema_usage_privileges_rows(
            authids,
            auth_members,
            wrappers,
            servers,
    ...
    fn information_schema_foreign_tables_rows(
        ...
        servers: Vec<pgrust_catalog_data::PgForeignServerRow>,
        tables: Vec<pgrust_catalog_data::PgForeignTableRow>,
    ) -> Vec<Vec<pgrust_nodes::Value>> {
        crate::system_views::build_information_schema_foreign_tables_rows(
    ...
    fn information_schema_foreign_table_options_rows(
        ...
        crate::system_views::build_information_schema_foreign_table_options_rows(
    ...
    fn pg_tables_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
    ...
    fn pg_views_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
        build_pg_views_rows_with_definition_formatter(
    ...
    fn pg_matviews_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
    ...
    fn pg_indexes_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
    ...
    fn pg_policies_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
    ...
    fn pg_rules_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
    ...
    fn pg_stats_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
        build_pg_stats_rows(
    ...
    fn pg_stats_ext_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
        build_pg_stats_ext_rows(
            ...
            catcache.class_rows(),
            catcache.attribute_rows(),
            catcache.statistic_ext_rows(),
            catcache.statistic_ext_data_rows(),
            self.current_user_oid(),
        )
    }

    fn pg_stats_ext_exprs_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
    ...
    fn pg_user_mappings_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
    ...
    fn pg_publication_tables_rows(
        ...
        namespaces: Vec<PgNamespaceRow>,
        classes: Vec<PgClassRow>,
        attributes: Vec<pgrust_catalog_data::PgAttributeRow>,
    ...
    fn pg_stat_activity_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        build_pg_stat_activity_rows(0, "regression")
    }

    fn pg_stat_database_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let stats = DatabaseStatsStore::with_default_io_rows();
        build_pg_stat_database_rows(self.database_rows(), &stats)
    }

    fn pg_stat_checkpointer_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let stats = DatabaseStatsStore::with_default_io_rows();
    ...
    fn pg_stat_wal_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        build_pg_stat_wal_rows(&DatabaseStatsStore::with_default_io_rows())
    }

    fn pg_stat_slru_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        build_pg_stat_slru_rows(&DatabaseStatsStore::with_default_io_rows())
    }

    fn pg_stat_archiver_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        build_pg_stat_archiver_rows(&DatabaseStatsStore::with_default_io_rows())
    }

    fn pg_stat_bgwriter_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        build_pg_stat_bgwriter_rows(&DatabaseStatsStore::with_default_io_rows())
    }

    fn pg_stat_recovery_prefetch_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        build_pg_stat_recovery_prefetch_rows(&DatabaseStatsStore::with_default_io_rows())
    }

    fn pg_stat_all_tables_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
        let stats = DatabaseStatsStore::with_default_io_rows();
    ...
    fn pg_stat_user_tables_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
        let stats = DatabaseStatsStore::with_default_io_rows();
    ...
    fn pg_statio_user_tables_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
        let stats = DatabaseStatsStore::with_default_io_rows();
    ...
    fn pg_stat_user_functions_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        let Some(catcache) = &self.catcache else {
            return Vec::new();
        };
        let stats = DatabaseStatsStore::with_default_io_rows();
    ...
    fn pg_stat_io_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
        build_pg_stat_io_rows(&DatabaseStatsStore::with_default_io_rows())
    }

    fn pg_locks_rows(&self) -> Vec<Vec<pgrust_nodes::Value>> {
...
fn bound_relation_from_relcache_entry(
    relcache: &RelCache,
    entry: &pgrust_catalog_store::relcache::RelCacheEntry,
) -> BoundRelation {
    BoundRelation {
        rel: entry.rel,
        ...
        toast: (entry.reltoastrelid != 0)
            ...
            .flatten()
            .map(|toast| pgrust_nodes::primnodes::ToastRelationRef {
                rel: toast.rel,
    ...
    relcache
        ...
        .flat_map(|(name, entry)| {
            ...
            if entry.row_type_oid != 0 {
                rows.push(pgrust_catalog_data::composite_type_row_with_owner(
                    relname,
                    entry.row_type_oid,
                    entry.namespace_oid,
                    entry.owner_oid,
            ...
            if entry.array_type_oid != 0 {
                rows.push(pgrust_catalog_data::composite_array_type_row_with_owner(
                    relname,
                    entry.array_type_oid,
                    entry.namespace_oid,
                    entry.owner_oid,
...
mod tests {
    ...
    use pgrust_catalog_data::TEXT_TYPE_OID;
    use pgrust_catalog_data::desc::column_desc;
    use pgrust_catalog_store::Catalog;
