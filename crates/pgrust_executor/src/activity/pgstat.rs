...
pub enum StatsFetchConsistency {
    None,
    Cache,
    Snapshot,
}
...
pub struct DatabaseStatsStore {
    ...
    pub shared: SharedStatsState,
    pub database_sessions: i64,
...
impl DatabaseStatsStore {
    ...
    pub fn record_database_session_start(&mut self) {
        self.database_sessions += 1;
    }

    pub fn apply_pending_flush(&mut self, pending: &mut StatsDelta) {
    ...
    pub fn snapshot_with_pending(&self, pending: &StatsDelta) -> Self {
    ...
    pub fn merged_io_entry(&self, pending: &StatsDelta, key: &IoStatsKey) -> Option<IoStatsEntry> {
    ...
    pub fn remove_relation(&mut self, oid: u32) {
        self.relations.remove(&oid);
    }

    pub fn remove_function(&mut self, oid: u32) {
    ...
    pub fn reset_relation(&mut self, oid: u32) {
        self.relations.remove(&oid);
    }

    pub fn reset_function(&mut self, oid: u32) {
        self.functions.remove(&oid);
    }

    pub fn reset_shared(&mut self, target: &str) -> Result<(), String> {
        let now = now_timestamptz();
    ...
    pub fn reset_slru(&mut self, target: Option<&str>) {
        let now = now_timestamptz();
    ...
    pub fn apply_global_io_delta(&mut self, key: IoStatsKey, delta: &IoStatsDelta) {
        self.io.entry(key).or_default().apply_delta(delta);
    }

    pub fn wal_write_bytes(&self) -> i64 {
        self.io
            .iter()
...
impl SessionStatsState {
    ...
    pub fn force_clear_snapshot(&mut self) {
        self.cache_snapshot = StatsReadCache::default();
        self.snapshot_store = None;
        self.snapshot_timestamp = None;
    }

    pub fn set_fetch_consistency(&mut self, fetch_consistency: StatsFetchConsistency) {
    ...
    pub fn set_track_functions(&mut self, track_functions: TrackFunctionsSetting) {
        self.track_functions = track_functions;
    }

    pub fn visible_relation_entry(
        &mut self,
    ...
    pub fn visible_relation_entries(
        &mut self,
        db_stats: &Arc<RwLock<DatabaseStatsStore>>,
    ...
    pub fn has_visible_relation_stats(
        &mut self,
        db_stats: &Arc<RwLock<DatabaseStatsStore>>,
        oid: u32,
    ) -> bool {
    ...
    pub fn note_relation_have_stats_false_once(&mut self, oid: u32) {
        self.relation_have_stats_false_once.insert(oid);
        self.clear_snapshot();
    }

    pub fn take_relation_have_stats_false_once(&mut self, oid: u32) -> bool {
        self.relation_have_stats_false_once.remove(&oid)
    }

    pub fn visible_function_entry(
        &mut self,
        db_stats: &Arc<RwLock<DatabaseStatsStore>>,
        oid: u32,
    ) -> Option<FunctionStatsEntry> {
    ...
    pub fn visible_function_entries(
        &mut self,
        db_stats: &Arc<RwLock<DatabaseStatsStore>>,
        oids: impl IntoIterator<Item = u32>,
    ...
    pub fn has_visible_function_stats(
        &mut self,
        db_stats: &Arc<RwLock<DatabaseStatsStore>>,
        oid: u32,
    ) -> bool {
    ...
    pub fn snapshot_timestamp(&self) -> Option<TimestampTzADT> {
        self.snapshot_timestamp
    }

    pub fn flush_pending(&mut self, db_stats: &Arc<RwLock<DatabaseStatsStore>>) {
        db_stats
            .write()
    ...
    pub fn backend_wal_write_bytes(&self) -> i64 {
        self.backend_io
            .iter()
            .filter(|(key, _)| key.object == "wal")
            .map(|(_, entry)| entry.write_bytes)
