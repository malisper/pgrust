...
pub enum TrackFunctionsSetting {
    None,
    Pl,
    All,
}
...
impl TrackFunctionsSetting {
    ...
    pub const fn tracks_plpgsql(self) -> bool {
...
impl FunctionStatsEntry {
    pub fn apply_delta(&mut self, delta: &FunctionStatsDelta) {
...
impl SessionStatsState {
    pub fn note_function_drop(&mut self, oid: u32, db_stats: &Arc<RwLock<DatabaseStatsStore>>) {
        if self.xact_active {
    ...
    pub fn begin_function_call(&mut self, funcid: u32) {
    ...
    pub fn finish_function_call(&mut self, funcid: u32) {
