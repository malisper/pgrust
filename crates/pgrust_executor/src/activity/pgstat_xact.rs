...
impl SessionStatsState {
    pub fn begin_top_level_xact(&mut self) {
    ...
    pub fn commit_top_level_xact(&mut self, db_stats: &Arc<RwLock<DatabaseStatsStore>>) {
    ...
    pub fn rollback_top_level_xact(&mut self) {
        let effects = std::mem::take(&mut self.stats_effects);
        for (oid, state) in std::mem::take(&mut self.relation_xact) {
    ...
    pub fn restore_after_savepoint_rollback(&mut self, mut saved: SessionStatsState) {
