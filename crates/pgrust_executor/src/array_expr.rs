...
use pgrust_expr::{
    ...
    eval_tsvector_matches_tsquery, format_array_value_text, not_equal_values, order_values,
};
use pgrust_nodes::SqlType;
use pgrust_nodes::datum::{ArrayDimension, ArrayValue, RecordValue, Value};
use pgrust_nodes::parsenodes::{ParseError, SqlTypeKind, SubqueryComparisonOp};

pub fn eval_quantified_array(
    left_value: &Value,
    ...
    if let Some(array) = normalize_array_value(array_value) {
        ...
        for item in &array.elements {
            ...
            match compare_quantified_values(left_value, item, op, collation_oid)? {
                Value::Bool(result) => {
                    if !is_all && result {
...
fn compare_quantified_values(
    ...
    if let (Value::Record(left), Value::Record(right)) = (left, right) {
        return match op {
            SubqueryComparisonOp::Eq => {
                compare_quantified_record_values(left, right, collation_oid)
            }
            SubqueryComparisonOp::NotEq => {
                match compare_quantified_record_values(left, right, collation_oid)? {
    ...
    match op {
        ...
        SubqueryComparisonOp::Match => match (&left, &right) {
            ...
            (Value::TsVector(vector), Value::TsQuery(query)) => {
                Ok(Value::Bool(eval_tsvector_matches_tsquery(vector, query)))
            }
            (Value::TsQuery(query), Value::TsVector(vector)) => {
                Ok(Value::Bool(eval_tsquery_matches_tsvector(query, vector)))
            }
        ...
        SubqueryComparisonOp::RegexMatch | SubqueryComparisonOp::NotRegexMatch => {
            let matched =
                eval_regex_match_operator(&left, &right, TextCollationSemantics::Default)?;
            match (op, matched) {
...
fn compare_quantified_record_values(
    left: &RecordValue,
    right: &RecordValue,
    collation_oid: Option<u32>,
) -> Result<Value, ExecError> {
    ...
    for (left_value, right_value) in left.fields.iter().zip(&right.fields) {
        match compare_quantified_values(
            left_value,
            right_value,
            SubqueryComparisonOp::Eq,
            collation_oid,
...
#[derive(Clone)]
pub struct ResolvedArraySubscript {
    pub is_slice: bool,
    pub lower_provided: bool,
    pub upper_provided: bool,
    pub lower: Option<Value>,
    pub upper: Option<Value>,
}

pub fn apply_array_subscripts(
    value: Value,
...
pub fn eval_array_ndims_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null] => Ok(Value::Null),
...
pub fn eval_array_dims_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null] => Ok(Value::Null),
        [value] => {
...
pub fn eval_array_fill_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
...
pub fn eval_string_to_array_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
...
pub fn eval_string_to_table_rows(values: &[Value]) -> Result<Vec<Value>, ExecError> {
    match values {
...
pub fn eval_array_to_string_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [Value::Null, _, _] => Ok(Value::Null),
...
pub fn eval_array_length_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [_, Value::Null] => Ok(Value::Null),
        [array, dim] => {
...
pub fn eval_array_upper_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [_, Value::Null] => Ok(Value::Null),
        [array, dim] => {
            let Some(array) = normalize_array_value(array) else {
...
pub fn eval_cardinality_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null] => Ok(Value::Null),
...
pub fn eval_array_append_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
...
pub fn eval_array_prepend_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
...
pub fn eval_array_cat_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [_, Value::Null] => Ok(Value::Null),
        [left, right] => {
            let left_array =
...
pub fn eval_array_position_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [Value::Null, _, _] => Ok(Value::Null),
...
pub fn eval_array_positions_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
...
pub fn eval_array_remove_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] => Ok(Value::Null),
...
pub fn eval_array_replace_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
...
pub fn eval_trim_array_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [_, Value::Null] => Ok(Value::Null),
...
pub fn eval_array_shuffle_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null] => Ok(Value::Null),
...
pub fn eval_array_sample_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [_, Value::Null] => Ok(Value::Null),
...
pub fn eval_array_reverse_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null] => Ok(Value::Null),
...
pub fn eval_array_sort_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
...
fn array_sort_value(
    ...
    if array.element_type_oid == Some(XID_TYPE_OID) && array.elements.len() > 1 {
        return Err(ExecError::DetailedError {
...
pub fn eval_width_bucket_thresholds(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [_, Value::Null] => Ok(Value::Null),
        [operand, thresholds] => {
...
pub fn eval_array_lower_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [_, Value::Null] => Ok(Value::Null),
...
pub fn eval_array_overlap(left: Value, right: Value) -> Result<Value, ExecError> {
...
pub fn eval_array_contains(left: Value, right: Value) -> Result<Value, ExecError> {
    eval_array_contains_internal("@>", left, right)
}

pub fn eval_array_contained(left: Value, right: Value) -> Result<Value, ExecError> {
...
mod tests {
    ...
    use pgrust_catalog_data::C_COLLATION_OID;
    use pgrust_expr::ExecError;
    use pgrust_nodes::datum::{ArrayValue, Value};
    use pgrust_nodes::parsenodes::SubqueryComparisonOp;
