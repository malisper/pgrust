...
pub fn numeric_input_would_overflow(text: &str) -> bool {
...
fn parse_tid_text(
    ...
    let offset_number = offset
        ...
        .parse::<u16>()
        .map_err(|_| invalid_tid_text_error(text))?;
    Ok(crate::compat::include::access::itemptr::ItemPointerData {
        block_number,
        offset_number,
    })
}

pub fn invalid_interval_text_error(text: &str) -> ExecError {
    ExecError::DetailedError {
...
pub fn render_interval_text(value: IntervalValue) -> String {
    render_interval_text_with_style(value, IntervalStyle::Postgres)
}

pub fn render_interval_text_with_config(value: IntervalValue, config: &DateTimeConfig) -> String {
...
pub fn parse_interval_text_value(text: &str) -> Result<IntervalValue, ExecError> {
    parse_interval_text_value_with_style(text, IntervalStyle::Postgres)
}

pub fn parse_interval_text_value_with_style(
    text: &str,
    style: IntervalStyle,
) -> Result<IntervalValue, ExecError> {
...
pub fn canonicalize_interval_text(text: &str) -> Result<String, ExecError> {
    parse_interval_text_value(text).map(render_interval_text)
}

pub fn parse_bytea_text(text: &str) -> Result<Vec<u8>, ExecError> {
...
pub fn parse_text_array_literal_with_op(
    raw: &str,
...
pub fn parse_text_array_literal_with_catalog_and_op(
    raw: &str,
    element_type: SqlType,
    op: &'static str,
    catalog: Option<&dyn CatalogLookup>,
...
pub fn parse_text_array_literal_with_catalog_op_and_explicit(
    raw: &str,
    element_type: SqlType,
    op: &'static str,
    explicit: bool,
...
pub fn parse_text_array_literal_with_options(
    raw: &str,
    element_type: SqlType,
    op: &'static str,
    explicit: bool,
...
fn cast_record_to_composite_value(
    ...
    if record.fields.len() != columns.len() {
        return Err(record_field_count_error(
            &crate::compat::backend::executor::value_io::format_record_text(&record),
            record.fields.len(),
            columns.len(),
        ));
    }
...
pub fn parse_composite_literal_fields(text: &str) -> Result<Vec<Option<String>>, ExecError> {
...
pub fn render_pg_lsn_text(value: u64) -> String {
    format!("{:X}/{:X}", value >> 32, value & 0xFFFF_FFFF)
}

pub fn pg_lsn_out_of_range() -> ExecError {
    ExecError::DetailedError {
...
pub fn parse_pg_lsn_text(text: &str) -> Result<u64, ExecError> {
...
pub fn datetime_parse_error_details(
    ty: &'static str,
    text: &str,
...
pub fn input_error_info(err: ExecError, text: &str) -> InputErrorInfo {
    match err {
        ExecError::JsonInput {
            message,
            detail,
...
pub fn soft_input_error_info_with_config(
    text: &str,
    type_name: &str,
    config: &DateTimeConfig,
) -> Result<Option<InputErrorInfo>, ExecError> {
...
pub fn soft_input_error_info_with_catalog_and_config(
    text: &str,
    type_name: &str,
    catalog: Option<&dyn CatalogLookup>,
    config: &DateTimeConfig,
...
pub fn cast_value_with_source_type_and_config(
    value: Value,
...
pub fn enforce_domain_constraints_for_value(
    value: Value,
    ty: SqlType,
    catalog: Option<&dyn CatalogLookup>,
) -> Result<Value, ExecError> {
...
fn domain_from_array_element_type(
    value: &Value,
    catalog: &dyn CatalogLookup,
) -> Option<crate::compat::backend::parser::DomainLookup> {
...
pub fn cast_value_with_source_type_catalog_and_config(
    ...
    let result = match value {
        ...
        Value::TsVector(vector) => match ty.kind {
            ...
            SqlTypeKind::Json => {
                let rendered = crate::compat::backend::executor::render_tsvector_text(&vector);
                validate_json_text(&rendered)?;
                Ok(Value::Json(CompactString::from_owned(rendered)))
            }
            SqlTypeKind::Jsonb => {
        ...
        Value::TsQuery(query) => match ty.kind {
            ...
            SqlTypeKind::Json => {
                let rendered = crate::compat::backend::executor::render_tsquery_text(&query);
                validate_json_text(&rendered)?;
                Ok(Value::Json(CompactString::from_owned(rendered)))
            }
            SqlTypeKind::Jsonb => {
        ...
        Value::Tid(value) => match ty.kind {
            ...
            SqlTypeKind::Text | SqlTypeKind::Name | SqlTypeKind::Char | SqlTypeKind::Varchar => {
                Ok(Value::Text(CompactString::from_owned(
                    crate::compat::backend::executor::value_io::render_tid_text(&value),
                )))
            }
            _ => Err(ExecError::TypeMismatch {
                op: "::tid",
        ...
        Value::Uuid(value) => match ty.kind {
            ...
            SqlTypeKind::Text | SqlTypeKind::Name | SqlTypeKind::Char | SqlTypeKind::Varchar => {
                Ok(Value::Text(CompactString::from_owned(
                    crate::compat::backend::executor::value_io::render_uuid_text(&value),
                )))
            }
            _ => Err(ExecError::TypeMismatch {
                op: "::uuid",
        ...
        Value::Inet(value) => match ty.kind {
            ...
            SqlTypeKind::Cidr => Ok(Value::Cidr(
                crate::compat::backend::executor::network_prefix(&value),
            )),
            SqlTypeKind::Text | SqlTypeKind::Name | SqlTypeKind::Char | SqlTypeKind::Varchar => {
        ...
        Value::Array(items) => match ty.kind {
            SqlTypeKind::Text | SqlTypeKind::Name | SqlTypeKind::Char | SqlTypeKind::Varchar => {
                Ok(Value::Text(CompactString::from_owned(
                    crate::compat::backend::executor::value_io::format_array_text(&items),
                )))
            }
...
pub fn cast_text_value(text: &str, ty: SqlType, explicit: bool) -> Result<Value, ExecError> {
...
pub fn cast_text_value_with_config(
    text: &str,
    ty: SqlType,
    explicit: bool,
    config: &DateTimeConfig,
    ...
    match ty.kind {
        ...
        SqlTypeKind::Xml => {
            crate::compat::backend::executor::validate_xml_input(text, config.xml.option)?;
        ...
        SqlTypeKind::TsVector => {
            crate::compat::backend::executor::parse_tsvector_text(text).map(Value::TsVector)
        }
        SqlTypeKind::TsQuery => {
            crate::compat::backend::executor::parse_tsquery_text(text).map(Value::TsQuery)
        }
...
pub fn cast_text_value_with_catalog_and_config(
    text: &str,
    ty: SqlType,
    explicit: bool,
    catalog: Option<&dyn CatalogLookup>,
...
pub fn parse_uuid_text(text: &str) -> Result<[u8; 16], ExecError> {
    let mut src = text;
...
pub fn parse_pg_float(text: &str, kind: SqlTypeKind) -> Result<f64, ExecError> {
    let trimmed = text.trim_matches(|ch: char| ch.is_ascii_whitespace());
...
mod tests {
    ...
    use crate::compat::backend::executor::exec_expr::parse_numeric_text;
    ...
    fn date_timestamp_cast_reports_postgres_range_error() {
        let date = DateADT(
            crate::compat::backend::utils::time::datetime::days_from_ymd(2_202_020, 10, 5).unwrap(),
        );
