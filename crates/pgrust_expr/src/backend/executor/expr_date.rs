...
use crate::compat::backend::executor::expr_datetime::current_date_value_with_config;
use crate::compat::backend::utils::misc::guc_datetime::DateTimeConfig;
use crate::compat::backend::utils::time::datetime::{DateTimeParseError, TimeZoneSpec};
...
pub fn timezone_target_offset_seconds(
    value: &Value,
...
pub fn eval_date_part_function(values: &[Value]) -> Result<Value, ExecError> {
    eval_date_part_function_with_config(values, &DateTimeConfig::default())
}

pub fn eval_date_part_function_with_config(
    values: &[Value],
    config: &DateTimeConfig,
...
pub fn eval_extract_function_with_config(
    values: &[Value],
    config: &DateTimeConfig,
) -> Result<Value, ExecError> {
...
pub fn eval_extract_function(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_isfinite_function(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_date_bin_function(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn timezone_interval_seconds(zone: IntervalValue) -> Result<i64, ExecError> {
...
pub fn eval_justify_days_function(values: &[Value]) -> Result<Value, ExecError> {
    let [value] = values else {
        return Err(ExecError::DetailedError {
...
pub fn eval_justify_hours_function(values: &[Value]) -> Result<Value, ExecError> {
    let [value] = values else {
        return Err(ExecError::DetailedError {
...
pub fn eval_justify_interval_function(values: &[Value]) -> Result<Value, ExecError> {
    let [value] = values else {
        return Err(ExecError::DetailedError {
...
pub fn eval_date_trunc_function(
    values: &[Value],
    config: &DateTimeConfig,
) -> Result<Value, ExecError> {
...
pub fn eval_datetime_add_function(values: &[Value], subtract: bool) -> Result<Value, ExecError> {
...
fn symbolic_age_interval(
    ...
    while days < 0 {
        ...
        days += i64::from(
            crate::compat::backend::utils::time::datetime::days_in_month(borrow_year, borrow_month),
        );
        months -= 1;
    }
    if left_less {
...
pub fn eval_age_function(values: &[Value], config: &DateTimeConfig) -> Result<Value, ExecError> {
    if values.iter().any(|value| matches!(value, Value::Null)) {
        return Ok(Value::Null);
    }
...
pub fn add_interval_to_local_timestamp(
    ...
    let max_day = crate::compat::backend::utils::time::datetime::days_in_month(new_year, new_month);
...
pub fn eval_to_timestamp_function(
    values: &[Value],
    config: &DateTimeConfig,
) -> Result<Value, ExecError> {
...
pub fn eval_make_date_function(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_make_time_function(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_make_timestamp_function(values: &[Value]) -> Result<Value, ExecError> {
    let [
        year_value,
        month_value,
        day_value,
    ...
    let time = crate::compat::backend::utils::time::datetime::time_usecs_from_hms(
...
pub fn eval_make_interval_function(values: &[Value]) -> Result<Value, ExecError> {
    let [
        years_value,
        months_value,
        weeks_value,
...
pub fn eval_make_timestamptz_function(
    values: &[Value],
    config: &DateTimeConfig,
) -> Result<Value, ExecError> {
    if values.iter().any(|value| matches!(value, Value::Null)) {
...
pub fn eval_timestamptz_constructor_function(
    values: &[Value],
    config: &DateTimeConfig,
) -> Result<Value, ExecError> {
    if values.iter().any(|value| matches!(value, Value::Null)) {
...
pub fn eval_to_date_function(values: &[Value]) -> Result<Value, ExecError> {
...
mod tests {
    ...
    fn to_timestamp_text_format_supports_horology_templates() {
        ...
        for (input, format, expected) in [
            ...
            let expected = crate::compat::backend::utils::time::timestamp::parse_timestamptz_text(
                expected, &config,
            )
            .unwrap();
    ...
    fn date_part_handles_bc_and_iso_fields() {
        assert_eq!(
            eval_date_part_function(&[
                ...
                Value::Date(DateADT(
                    crate::compat::backend::utils::time::datetime::days_from_ymd(-2019, 8, 11,)
                        .unwrap()
                )),
            ])
            .unwrap(),
        ...
        assert_eq!(
            eval_date_part_function(&[
                Value::Text("dow".into()),
                Value::Date(DateADT(
                    crate::compat::backend::utils::time::datetime::days_from_ymd(2020, 8, 16,)
                        .unwrap()
                )),
            ])
            .unwrap(),
