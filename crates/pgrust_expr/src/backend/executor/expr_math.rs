...
pub fn eval_abs_function(values: &[Value]) -> Result<Value, ExecError> {
    match &values[0] {
...
pub fn eval_unary_float_function(
    op: &'static str,
    values: &[Value],
...
pub fn eval_binary_float_function(
    op: &'static str,
    values: &[Value],
...
pub fn eval_float_send_function(
    op: &'static str,
    values: &[Value],
    narrow: bool,
) -> Result<Value, ExecError> {
...
pub fn eval_lgamma(value: f64) -> Result<f64, ExecError> {
    if value.is_nan() {
        return Ok(f64::NAN);
    }
...
pub fn eval_bitcast_bigint_to_float8(values: &[Value]) -> Result<Value, ExecError> {
    match values.first() {
        Some(Value::Null) | None => Ok(Value::Null),
...
pub fn eval_sqrt(value: f64) -> Result<f64, ExecError> {
    if value < 0.0 {
...
pub fn eval_power(base: f64, exp: f64) -> Result<f64, ExecError> {
...
pub fn eval_exp(value: f64) -> Result<f64, ExecError> {
...
pub fn eval_ln(value: f64) -> Result<f64, ExecError> {
    if value == 0.0 {
...
pub fn eval_acosh(value: f64) -> Result<f64, ExecError> {
    if value < 1.0 {
        return Err(float_domain_error("input is out of range"));
    }
    Ok(value.acosh())
...
pub fn eval_atanh(value: f64) -> Result<f64, ExecError> {
    if value.is_nan() {
        return Ok(f64::NAN);
    }
...
pub fn eval_asind(value: f64) -> Result<f64, ExecError> {
...
pub fn eval_acosd(value: f64) -> Result<f64, ExecError> {
    if !(-1.0..=1.0).contains(&value) {
        return Err(float_domain_error("input is out of range"));
    }
...
pub fn snap_degree(value: f64) -> f64 {
    for landmark in [
...
pub fn eval_gcd_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [_, Value::Null] => Ok(Value::Null),
...
pub fn eval_lcm_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [_, Value::Null] => Ok(Value::Null),
        [Value::Int16(left), Value::Int16(right)] => {
            let gcd = gcd_i128(i128::from(*left), i128::from(*right));
