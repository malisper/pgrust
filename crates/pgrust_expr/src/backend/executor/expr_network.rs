...
pub fn parse_inet_text(text: &str) -> Result<InetValue, ExecError> {
    parse_network_text(text, false)
}

pub fn parse_cidr_text(text: &str) -> Result<InetValue, ExecError> {
    parse_network_text(text, true)
}

pub fn parse_inet_bytes(bytes: &[u8]) -> Result<InetValue, ExecError> {
...
pub fn parse_cidr_bytes(bytes: &[u8]) -> Result<InetValue, ExecError> {
    let text = std::str::from_utf8(bytes).map_err(|_| ExecError::InvalidStorageValue {
        column: "cidr".into(),
...
pub fn render_network_text(value: &Value) -> Option<String> {
    match value {
...
pub fn network_bitwise_not(value: Value) -> Result<Value, ExecError> {
    match value {
...
pub fn network_add(left: Value, right: Value) -> Result<Value, ExecError> {
    match (left, right) {
...
pub fn network_sub(left: Value, right: Value) -> Result<Value, ExecError> {
    match (left, right) {
        (Value::Inet(left) | Value::Cidr(left), Value::Inet(right) | Value::Cidr(right)) => {
            if max_bits(left.addr) != max_bits(right.addr) {
...
pub fn compare_network_values(left: &InetValue, right: &InetValue) -> Ordering {
...
pub fn encode_network_bytes(value: &InetValue, cidr: bool) -> Vec<u8> {
    if cidr {
...
pub fn network_broadcast(value: &InetValue) -> InetValue {
    InetValue {
...
pub fn network_merge(left: &InetValue, right: &InetValue) -> InetValue {
...
pub fn network_contains(container: &InetValue, value: &InetValue, strict: bool) -> bool {
...
mod tests {
    ...
    fn cidr_requires_host_bits_to_be_clear() {
        ...
        match err {
            crate::compat::backend::executor::ExecError::DetailedError {
                message, detail, ..
            } => {
