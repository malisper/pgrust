...
use crate::compat::backend::executor::jsonb::{
...
pub fn compare_order_values(
    ...
    match (left, right) {
        ...
        | (Value::Cidr(a), Value::Inet(b)) => Ok(
            crate::compat::backend::executor::compare_network_values(a, b),
        ),
        (Value::Cidr(a), Value::Cidr(b)) => Ok(
            crate::compat::backend::executor::compare_network_values(a, b),
        ),
        ...
        (Value::TsVector(a), Value::TsVector(b)) => {
            Ok(crate::compat::backend::executor::compare_tsvector(a, b))
        }
        (Value::TsQuery(a), Value::TsQuery(b)) => {
            Ok(crate::compat::backend::executor::compare_tsquery(a, b))
        }
...
fn parse_tid_comparison_text(
    text: &str,
) -> Result<crate::compat::include::access::itemptr::ItemPointerData, ExecError> {
    match cast_value(
...
fn compare_tid_to_text(
    left: &crate::compat::include::access::itemptr::ItemPointerData,
    right: &str,
) -> Result<Ordering, ExecError> {
...
fn compare_text_to_tid(
    left: &str,
    right: &crate::compat::include::access::itemptr::ItemPointerData,
) -> Result<Ordering, ExecError> {
    Ok(parse_tid_comparison_text(left)?.cmp(right))
}

pub fn eval_and(left: Value, right: Value) -> Result<Value, ExecError> {
    match (left, right) {
...
pub fn eval_or(left: Value, right: Value) -> Result<Value, ExecError> {
    match (left, right) {
...
pub fn compare_values_with_type(
    op: &'static str,
    left: Value,
...
pub fn compare_values_with_type_and_catalog(
    op: &'static str,
    left: Value,
    left_type: Option<SqlType>,
    right: Value,
...
pub fn not_equal_values_with_type(
    left: Value,
    left_type: Option<SqlType>,
    right: Value,
    right_type: Option<SqlType>,
...
pub fn not_equal_values_with_type_and_catalog(
    left: Value,
    left_type: Option<SqlType>,
    right: Value,
    right_type: Option<SqlType>,
...
pub fn add_values(left: Value, right: Value) -> Result<Value, ExecError> {
    if matches!(left, Value::Null) || matches!(right, Value::Null) {
        return Ok(Value::Null);
    }
...
pub fn sub_values(left: Value, right: Value) -> Result<Value, ExecError> {
    if matches!(left, Value::Null) || matches!(right, Value::Null) {
        return Ok(Value::Null);
    }
    if matches!(left, Value::Inet(_) | Value::Cidr(_))
...
pub fn mixed_date_timestamp_ordering(
    left: &Value,
    right: &Value,
...
pub fn mul_values(left: Value, right: Value) -> Result<Value, ExecError> {
    if matches!(left, Value::Null) || matches!(right, Value::Null) {
        return Ok(Value::Null);
    }
    match (&left, &right) {
...
pub fn shift_left_values(left: Value, right: Value) -> Result<Value, ExecError> {
    if matches!(left, Value::Null) || matches!(right, Value::Null) {
        return Ok(Value::Null);
    }
    match (&left, &right) {
...
pub fn shift_right_values(left: Value, right: Value) -> Result<Value, ExecError> {
    if matches!(left, Value::Null) || matches!(right, Value::Null) {
        return Ok(Value::Null);
    }
    match (&left, &right) {
...
pub fn bitwise_and_values(left: Value, right: Value) -> Result<Value, ExecError> {
    if matches!(left, Value::Inet(_) | Value::Cidr(_))
        || matches!(right, Value::Inet(_) | Value::Cidr(_))
    {
...
pub fn bitwise_or_values(left: Value, right: Value) -> Result<Value, ExecError> {
    if matches!(left, Value::Inet(_) | Value::Cidr(_))
        || matches!(right, Value::Inet(_) | Value::Cidr(_))
    {
        return network_bitwise_binary("|", left, right);
...
pub fn bitwise_xor_values(left: Value, right: Value) -> Result<Value, ExecError> {
    match (left, right) {
        (Value::Bit(l), Value::Bit(r)) => Ok(Value::Bit(bitwise_binary_bits("#", &l, &r)?)),
        (Value::Int16(l), Value::Int16(r)) => Ok(Value::Int16(l ^ r)),
        (Value::Int32(l), Value::Int32(r)) => Ok(Value::Int32(l ^ r)),
...
pub fn div_values(left: Value, right: Value) -> Result<Value, ExecError> {
    if matches!(left, Value::Null) || matches!(right, Value::Null) {
        return Ok(Value::Null);
    }
...
pub fn mod_values(left: Value, right: Value) -> Result<Value, ExecError> {
    if matches!(left, Value::Null) || matches!(right, Value::Null) {
        return Ok(Value::Null);
    }
    if matches!(
...
pub fn concat_values(left: Value, right: Value) -> Result<Value, ExecError> {
    concat_values_with_cast_context(left, None, right, None, None, &DateTimeConfig::default())
}

pub fn concat_values_with_cast_context(
    left: Value,
    left_type: Option<SqlType>,
    right: Value,
    right_type: Option<SqlType>,
    ...
    match (&left, &right) {
        ...
        (Value::TsVector(l), Value::TsVector(r)) => Ok(Value::TsVector(
            crate::compat::backend::executor::concat_tsvector(l, r),
        )),
        (Value::TsQuery(l), Value::TsQuery(r)) => Ok(Value::TsQuery(
            crate::compat::backend::executor::tsquery_or(l.clone(), r.clone()),
        )),
...
pub fn order_values(
    ...
    match (&left, &right) {
        ...
        | (Value::Cidr(l), Value::Cidr(r)) => {
            let ordering = crate::compat::backend::executor::compare_network_values(l, r);
            Ok(Value::Bool(match op {
                "<" => ordering == Ordering::Less,
                "<=" => ordering != Ordering::Greater,
                ">" => ordering == Ordering::Greater,
        ...
        (Value::TsVector(l), Value::TsVector(r)) => {
            let ordering = crate::compat::backend::executor::compare_tsvector(l, r);
            Ok(Value::Bool(match op {
                "<" => ordering == Ordering::Less,
                "<=" => ordering != Ordering::Greater,
                ">" => ordering == Ordering::Greater,
        ...
        (Value::TsQuery(l), Value::TsQuery(r)) => {
            let ordering = crate::compat::backend::executor::compare_tsquery(l, r);
            Ok(Value::Bool(match op {
                "<" => ordering == Ordering::Less,
                "<=" => ordering != Ordering::Greater,
                ">" => ordering == Ordering::Greater,
...
pub fn ensure_builtin_collation_supported(collation_oid: Option<u32>) -> Result<(), ExecError> {
    match collation_oid {
        None
        | Some(
...
pub fn ensure_text_collation_supported(
    collation_oid: Option<u32>,
    catalog: Option<&dyn CatalogLookup>,
) -> Result<(), ExecError> {
...
pub enum TextCollationSemantics {
    Default,
    Ascii,
    PgCUtf8,
    PgUnicodeFast,
...
pub fn text_collation_semantics(
    collation_oid: Option<u32>,
    catalog: Option<&dyn CatalogLookup>,
...
pub fn compare_text_values_with_catalog(
    left: &str,
    right: &str,
    collation_oid: Option<u32>,
    catalog: Option<&dyn CatalogLookup>,
...
pub fn interval_div_float(span: IntervalValue, factor: f64) -> Option<IntervalValue> {
...
pub fn parse_numeric_text(text: &str) -> Option<NumericValue> {
...
mod tests {
    ...
    use crate::compat::backend::utils::misc::guc_datetime::{DateStyleFormat, DateTimeConfig};
    ...
    fn compare_values_rejects_unsupported_collation_oid() {
        assert!(matches!(
            ...
            Err(crate::compat::backend::executor::ExecError::DetailedError { sqlstate, .. })
                if sqlstate == "0A000"
        ));
    }
    ...
    fn timestamp_interval_arithmetic_checks_postgres_finite_range() {
        ...
        assert!(matches!(
            ...
            Err(crate::compat::backend::executor::ExecError::DetailedError {
                message,
                sqlstate: "22008",
                ..
