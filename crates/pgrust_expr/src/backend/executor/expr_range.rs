...
pub fn parse_range_text(text: &str, ty: SqlType) -> Result<Value, ExecError> {
...
pub fn render_range_value(range: &RangeValue) -> String {
    render_range_value_with_config(range, &DateTimeConfig::default())
}

pub fn render_range_value_with_config(
    range: &RangeValue,
    datetime_config: &DateTimeConfig,
) -> String {
    if range.empty {
...
pub fn compare_range_values(left: &RangeValue, right: &RangeValue) -> Ordering {
...
pub fn encode_range_bytes(range: &RangeValue) -> Result<Vec<u8>, ExecError> {
    let mut flags = 0u8;
    if range.empty {
...
pub fn decode_range_bytes(range_type: RangeTypeRef, bytes: &[u8]) -> Result<RangeValue, ExecError> {
...
pub fn range_intersection_agg_transition(
...
pub fn empty_range(range_type: RangeTypeRef) -> RangeValue {
    RangeValue {
        range_type,
        empty: true,
        lower: None,
...
pub fn range_contains_range(left: &RangeValue, right: &RangeValue) -> bool {
    if right.empty {
        return true;
    }
    if left.empty {
...
pub fn range_contains_element(range: &RangeValue, value: &Value) -> Result<bool, ExecError> {
...
pub fn range_overlap(left: &RangeValue, right: &RangeValue) -> bool {
...
pub fn range_adjacent(left: &RangeValue, right: &RangeValue) -> bool {
    if left.empty || right.empty {
        return false;
    }
...
pub fn range_strict_left(left: &RangeValue, right: &RangeValue) -> bool {
    !left.empty
        && !right.empty
        && cmp_upper_to_lower(left.upper.as_ref(), right.lower.as_ref()) == Ordering::Less
}

pub fn range_strict_right(left: &RangeValue, right: &RangeValue) -> bool {
    range_strict_left(right, left)
}

pub fn range_over_left_bounds(left: &RangeValue, right: &RangeValue) -> bool {
    !left.empty
        && !right.empty
        && compare_upper_bounds(left.upper.as_ref(), right.upper.as_ref()) != Ordering::Greater
}

pub fn range_over_right_bounds(left: &RangeValue, right: &RangeValue) -> bool {
    !left.empty
        && !right.empty
        && compare_lower_bounds(left.lower.as_ref(), right.lower.as_ref()) != Ordering::Less
}

pub fn range_intersection(left: &RangeValue, right: &RangeValue) -> RangeValue {
...
pub fn range_merge(left: &RangeValue, right: &RangeValue) -> RangeValue {
    if left.empty {
        return right.clone();
    }
    if right.empty {
...
pub fn range_union(left: &RangeValue, right: &RangeValue) -> Result<RangeValue, ExecError> {
...
pub fn range_difference_segments(
    left: &RangeValue,
    right: &RangeValue,
...
pub fn compare_lower_bounds(left: Option<&RangeBound>, right: Option<&RangeBound>) -> Ordering {
    match (left, right) {
...
pub fn compare_upper_bounds(left: Option<&RangeBound>, right: Option<&RangeBound>) -> Ordering {
    match (left, right) {
        (None, None) => Ordering::Equal,
...
pub fn bounds_adjacent(upper: Option<&RangeBound>, lower: Option<&RangeBound>) -> bool {
    match (upper, lower) {
        (Some(upper), Some(lower))
...
pub fn compare_scalar_values(left: &Value, right: &Value) -> Ordering {
