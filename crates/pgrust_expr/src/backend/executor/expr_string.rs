...
use crate::compat::backend::executor::jsonb::render_jsonb_bytes;
use crate::compat::backend::libpq::pqformat::format_bytea_text;
use crate::compat::backend::parser::{ParseError, SqlType, SqlTypeKind};
use crate::compat::backend::utils::crc32c;
use crate::compat::backend::utils::misc::guc_datetime::DateTimeConfig;
use crate::compat::backend::utils::record::assign_anonymous_record_descriptor;
...
pub fn eval_to_char_float4_function(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_to_number_function(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_pg_size_pretty_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
...
pub fn eval_pg_size_bytes_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(value, Value::Null) {
...
pub fn eval_to_bin_function(values: &[Value]) -> Result<Value, ExecError> {
    values.first().map_or(Ok(Value::Null), |value| {
        render_integer_base("to_bin", value, 2)
    })
}

pub fn eval_to_oct_function(values: &[Value]) -> Result<Value, ExecError> {
    values.first().map_or(Ok(Value::Null), |value| {
        render_integer_base("to_oct", value, 8)
    })
}

pub fn eval_to_hex_function(values: &[Value]) -> Result<Value, ExecError> {
    values.first().map_or(Ok(Value::Null), |value| {
...
fn value_output_text_with_config(
    ...
    Ok(match value {
        ...
        Value::PgLsn(v) => crate::compat::backend::executor::render_pg_lsn_text(*v),
        Value::Money(v) => crate::compat::backend::executor::money_format_text(*v),
        Value::Float64(v) => v.to_string(),
        Value::Numeric(v) => v.render(),
        Value::Interval(v) => render_interval_text_with_config(*v, datetime_config),
        Value::Uuid(v) => crate::compat::backend::executor::value_io::render_uuid_text(v),
        ...
        Value::Multirange(_) => {
            crate::compat::backend::executor::render_multirange_text_with_config(
                value,
                datetime_config,
            )
            .unwrap_or_default()
        ...
        Value::PgArray(array) => {
            crate::compat::backend::executor::value_io::format_array_value_text(array)
        }
        Value::Record(record) => {
            crate::compat::backend::executor::value_io::format_record_text_with_config(
                record,
                datetime_config,
            )
        }
...
pub fn eval_concat_ws_function(
    values: &[Value],
    func_variadic: bool,
    datetime_config: &DateTimeConfig,
) -> Result<Value, ExecError> {
...
pub fn eval_left_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
...
pub fn eval_right_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(count_value) = values.get(1) else {
...
pub fn eval_length_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(value, Value::Null) {
...
pub fn eval_bit_length_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(value, Value::Null) {
...
pub fn eval_octet_length_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(value, Value::Null) {
...
pub fn eval_repeat_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(count_value) = values.get(1) else {
...
pub fn eval_lower_function(values: &[Value]) -> Result<Value, ExecError> {
    eval_lower_function_with_collation(values, TextCollationSemantics::Default)
}

pub fn eval_lower_function_with_collation(
    values: &[Value],
...
pub fn eval_upper_function(values: &[Value]) -> Result<Value, ExecError> {
    eval_upper_function_with_collation(values, TextCollationSemantics::Default)
}

pub fn eval_upper_function_with_collation(
    values: &[Value],
    collation: TextCollationSemantics,
) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
...
pub fn eval_text_starts_with_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
...
pub fn eval_unistr_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(text_value, Value::Null) {
...
pub fn eval_unicode_version_function(values: &[Value]) -> Result<Value, ExecError> {
    if !values.is_empty() {
...
pub fn eval_unicode_assigned_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
...
pub fn eval_unicode_normalize_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [_, Value::Null] => Ok(Value::Null),
        [value, form] => {
...
pub fn eval_unicode_is_normalized_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
        [Value::Null, _] | [_, Value::Null] => Ok(Value::Null),
        [value, form] => {
...
pub fn eval_initcap_function(values: &[Value]) -> Result<Value, ExecError> {
    eval_initcap_function_with_collation(values, TextCollationSemantics::Default)
}

pub fn eval_initcap_function_with_collation(
    values: &[Value],
    collation: TextCollationSemantics,
) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
...
pub fn eval_casefold_function_with_collation(
    values: &[Value],
    collation: TextCollationSemantics,
) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
...
pub fn eval_replace_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
...
pub fn eval_split_part_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
...
pub fn eval_lpad_function(values: &[Value]) -> Result<Value, ExecError> {
    eval_pad_function("lpad", values, true)
}

pub fn eval_rpad_function(values: &[Value]) -> Result<Value, ExecError> {
    eval_pad_function("rpad", values, false)
}

pub fn eval_translate_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(from_value) = values.get(1) else {
...
pub fn eval_ascii_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(text_value, Value::Null) {
...
pub fn eval_chr_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(value, Value::Null) {
...
pub fn eval_trim_function(op: &'static str, values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_text_substring(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
...
pub fn eval_bytea_substring(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_md5_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(value, Value::Null) {
...
pub fn eval_reverse_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(value, Value::Null) {
...
pub fn eval_quote_literal_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(value, Value::Null) {
...
pub fn eval_quote_nullable_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
...
pub fn eval_quote_ident_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(value, Value::Null) {
...
pub fn eval_parse_ident_function(values: &[Value]) -> Result<Value, ExecError> {
    match values {
...
pub fn eval_encode_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(bytes_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(format_value) = values.get(1) else {
...
pub fn eval_decode_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(format_value) = values.get(1) else {
...
pub fn eval_sha224_function(values: &[Value]) -> Result<Value, ExecError> {
    eval_sha2_function::<Sha224>("sha224", values)
}

pub fn eval_sha256_function(values: &[Value]) -> Result<Value, ExecError> {
    eval_sha2_function::<Sha256>("sha256", values)
}

pub fn eval_sha384_function(values: &[Value]) -> Result<Value, ExecError> {
    eval_sha2_function::<Sha384>("sha384", values)
}

pub fn eval_sha512_function(values: &[Value]) -> Result<Value, ExecError> {
    eval_sha2_function::<Sha512>("sha512", values)
}

pub fn eval_crc32_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(value, Value::Null) {
...
pub fn eval_crc32c_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(value, Value::Null) {
...
pub fn eval_bpchar_to_text_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(text_value, Value::Null) {
...
pub fn eval_position_function(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_strpos_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
...
pub fn eval_bytea_position_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(needle_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(haystack_value) = values.get(1) else {
...
pub fn eval_bytea_overlay(values: &[Value]) -> Result<Value, ExecError> {
    let Some(bytes_value) = values.first() else {
        return Ok(Value::Null);
    };
...
pub fn eval_text_overlay(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(place_value) = values.get(1) else {
...
pub fn eval_get_bit_bytes(values: &[Value]) -> Result<Value, ExecError> {
    let Some(bytes_value) = values.first() else {
        return Ok(Value::Null);
    };
...
pub fn eval_set_bit_bytes(values: &[Value]) -> Result<Value, ExecError> {
    let Some(bytes_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(index_value) = values.get(1) else {
...
pub fn eval_bit_count_bytes(values: &[Value]) -> Result<Value, ExecError> {
    let Some(bytes_value) = values.first() else {
        return Ok(Value::Null);
    };
    if matches!(bytes_value, Value::Null) {
...
pub fn eval_get_byte(values: &[Value]) -> Result<Value, ExecError> {
    let Some(bytes_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(index_value) = values.get(1) else {
...
pub fn eval_set_byte(values: &[Value]) -> Result<Value, ExecError> {
    let Some(bytes_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(index_value) = values.get(1) else {
...
pub fn eval_convert_from_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(bytes_value) = values.first() else {
        return Ok(Value::Null);
    };
...
pub fn eval_convert_to_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(encoding_value) = values.get(1) else {
...
pub fn eval_pg_rust_test_enc_setup(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_pg_rust_test_opclass_options_func(values: &[Value]) -> Result<Value, ExecError> {
    if values.iter().any(|value| matches!(value, Value::Null)) {
        return Ok(Value::Null);
    }
    if values.len() != 1 {
...
pub fn eval_pg_rust_test_fdw_handler(values: &[Value]) -> Result<Value, ExecError> {
    if values.iter().any(|value| matches!(value, Value::Null)) {
        return Ok(Value::Null);
    }
    if !values.is_empty() {
...
pub fn eval_pg_rust_is_catalog_text_unique_index_oid(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_pg_rust_test_widget_in(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_pg_rust_test_widget_out(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_pg_rust_test_int44in(values: &[Value]) -> Result<Value, ExecError> {
    let [input] = values else {
        return Err(ExecError::TypeMismatch {
...
pub fn eval_pg_rust_test_int44out(values: &[Value]) -> Result<Value, ExecError> {
    let [input] = values else {
        return Err(ExecError::TypeMismatch {
...
pub fn eval_pg_rust_test_pt_in_widget(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_pg_rust_test_enc_conversion(values: &[Value]) -> Result<Value, ExecError> {
...
mod tests {
    ...
    use crate::compat::backend::executor::expr_ops::TextCollationSemantics;
