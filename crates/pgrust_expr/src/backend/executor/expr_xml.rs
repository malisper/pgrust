...
pub fn unsupported_xml_feature_error() -> ExecError {
    ExecError::XmlInput {
        raw_input: String::new(),
...
pub fn validate_xml_input(text: &str, option: XmlOptionSetting) -> Result<(), ExecError> {
...
pub fn xml_is_well_formed(text: &str, option: XmlOptionSetting) -> bool {
...
pub fn strip_xml_declaration(text: &str) -> &str {
    find_xml_declaration(text).map_or(text, |(_, body)| body)
}

pub fn render_xml_output_text(text: &str) -> &str {
...
pub fn concat_xml_texts<'a>(values: impl IntoIterator<Item = &'a str>) -> String {
...
pub fn xml_comment(text: &str) -> Result<String, ExecError> {
...
fn render_scalar_text(value: Value, ctx: &ExecutorContext) -> Result<String, ExecError> {
    match value {
        ...
        other => match cast_value_with_config(
            other,
            crate::compat::backend::parser::SqlType::new(
                crate::compat::backend::parser::SqlTypeKind::Text,
            ),
            &ctx.datetime_config,
        )? {
            Value::Text(text) => Ok(text.to_string()),
...
fn render_xml_date_text(value: DateADT) -> Result<String, ExecError> {
    ...
    Ok(crate::compat::backend::utils::time::date::format_date_text(
        value,
        &crate::compat::backend::utils::misc::guc_datetime::DateTimeConfig::default(),
    ))
}
...
fn render_xml_timestamptz_text(
    ...
    let offset = crate::compat::backend::utils::time::datetime::timezone_offset_seconds_at_utc(
        &ctx.datetime_config,
        value.0,
    );
...
fn format_xml_timestamp_usecs(usecs: i64, offset_seconds: Option<i32>) -> String {
    let (days, time_usecs) =
        crate::compat::backend::utils::time::datetime::timestamp_parts_from_usecs(usecs);
    let (mut year, month, day) = crate::compat::backend::utils::time::datetime::ymd_from_days(days);
    let bc = year <= 0;
    if bc {
        year = 1 - year;
    }
    let mut rendered = format!(
        ...
        crate::compat::backend::utils::time::datetime::format_time_usecs(time_usecs)
    );
...
fn xml_option_to_setting(option: crate::compat::backend::parser::XmlOption) -> XmlOptionSetting {
    match option {
        crate::compat::backend::parser::XmlOption::Document => XmlOptionSetting::Document,
        crate::compat::backend::parser::XmlOption::Content => XmlOptionSetting::Content,
    }
}
...
pub fn eval_xml_comment_function(
    values: &[Value],
    ctx: Option<&ExecutorContext>,
) -> Result<Value, ExecError> {
...
pub fn eval_xml_is_well_formed_function(
    values: &[Value],
    option: XmlOptionSetting,
    ctx: Option<&ExecutorContext>,
) -> Result<Value, ExecError> {
...
fn eval_xml_root(
    ...
    let standalone = match xml.standalone {
        Some(crate::compat::backend::parser::XmlStandalone::Yes) => Some(true),
        Some(crate::compat::backend::parser::XmlStandalone::No) => Some(false),
        Some(crate::compat::backend::parser::XmlStandalone::NoValue) => None,
...
pub fn eval_xpath_function(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_xpath_exists_function(values: &[Value]) -> Result<Value, ExecError> {
    let Some(path_value) = values.first() else {
...
mod tests {
    use super::*;
    use crate::compat::backend::parser::XmlStandalone;
