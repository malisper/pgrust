...
pub fn compile_pg_regex_predicate(pattern: &str) -> Result<CompiledPgRegex, ExecError> {
    compile_pg_regex(pattern, &PgRegexFlags::default(), PgRegexPurpose::Boolean)
}

pub fn pg_regex_is_match(compiled: &CompiledPgRegex, text: &str) -> Result<bool, ExecError> {
    compiled.is_match(text)
}

pub fn eval_regex_match_operator(
    left: &Value,
    right: &Value,
...
pub fn eval_sql_regex_substring(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_similar_substring(values: &[Value]) -> Result<Value, ExecError> {
...
pub fn eval_jsonpath_like_regex(text: &str, pattern: &str, flags: &str) -> Result<bool, ExecError> {
...
pub fn validate_jsonpath_like_regex(pattern: &str, flags: &str) -> Result<(), ExecError> {
    let flags = parse_jsonpath_like_regex_flags(flags)?;
    let pattern = normalize_jsonpath_regex_pattern(pattern, &flags);
...
pub fn eval_regexp_match(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(pattern_value) = values.get(1) else {
...
pub fn eval_regexp_count(values: &[Value]) -> Result<Value, ExecError> {
    if values.iter().any(|value| matches!(value, Value::Null)) {
        return Ok(Value::Null);
    }
...
pub fn eval_regexp_instr(values: &[Value]) -> Result<Value, ExecError> {
    if values.iter().any(|value| matches!(value, Value::Null)) {
        return Ok(Value::Null);
    }
...
pub fn eval_regexp_substr(values: &[Value]) -> Result<Value, ExecError> {
    if values.iter().any(|value| matches!(value, Value::Null)) {
        return Ok(Value::Null);
    }
...
pub fn eval_regexp_replace(values: &[Value]) -> Result<Value, ExecError> {
    let Some(text_value) = values.first() else {
        return Ok(Value::Null);
    };
    let Some(pattern_value) = values.get(1) else {
...
pub fn eval_regexp_split_to_array(values: &[Value]) -> Result<Value, ExecError> {
    if values.iter().any(|value| matches!(value, Value::Null)) {
        return Ok(Value::Null);
    }
...
pub fn eval_regexp_matches_rows(values: &[Value]) -> Result<Vec<Value>, ExecError> {
...
pub fn eval_regexp_split_to_table_rows(values: &[Value]) -> Result<Vec<Value>, ExecError> {
    if matches!(values.first(), Some(Value::Null))
        || matches!(values.get(1), Some(Value::Null))
        || matches!(values.get(2), Some(Value::Null))
    {
...
pub fn explain_similar_pattern(pattern: &str, escape: Option<&str>) -> Result<String, ExecError> {
