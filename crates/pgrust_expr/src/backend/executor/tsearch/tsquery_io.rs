...
pub fn parse_tsquery_text(text: &str) -> Result<TsQuery, ExecError> {
    TsQuery::parse(text).map_err(|message| tsquery_input_parse_error(text, message))
}

pub fn render_tsquery_text(query: &TsQuery) -> String {
    query.render()
}

pub fn encode_tsquery_bytes(query: &TsQuery) -> Vec<u8> {
    let mut out = Vec::new();
    encode_node(&query.root, &mut out);
    out
}

pub fn decode_tsquery_bytes(bytes: &[u8]) -> Result<TsQuery, ExecError> {
...
pub fn tsquery_input_error(message: String) -> ExecError {
