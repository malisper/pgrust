use super::*;
use crate::compat::backend::executor::expr_casts::{
...
pub fn encode_array_bytes(element_type: SqlType, array: &ArrayValue) -> Result<Vec<u8>, ExecError> {
...
pub fn encode_anyarray_bytes(array: &ArrayValue) -> Result<Vec<u8>, ExecError> {
...
fn encode_array_element_payload(
    ...
    match coerced {
        ...
        Value::Range(range) => crate::compat::backend::executor::encode_range_bytes(&range),
        Value::Bit(v) => {
        ...
        Value::Inet(v) => Ok(crate::compat::backend::executor::encode_network_bytes(
            &v, false,
        )),
        Value::Cidr(v) => Ok(crate::compat::backend::executor::encode_network_bytes(
            &v, true,
        )),
        ...
        Value::TsVector(vector) => Ok(crate::compat::backend::executor::encode_tsvector_bytes(
            &vector,
        )),
        Value::TsQuery(query) => Ok(crate::compat::backend::executor::encode_tsquery_bytes(
            &query,
        )),
...
pub fn decode_array_bytes(element_type: SqlType, bytes: &[u8]) -> Result<Value, ExecError> {
    decode_array_bytes_internal(Some(element_type), bytes, "<array>")
}

pub fn decode_anyarray_bytes(bytes: &[u8]) -> Result<Value, ExecError> {
...
pub fn builtin_type_oid_for_sql_type(sql_type: SqlType) -> Option<u32> {
...
fn decode_array_element_value(
    ...
    if let Some(range_type) = range_type_ref_for_sql_type(element_type) {
        return crate::compat::backend::executor::decode_range_bytes(range_type, bytes)
            .map(Value::Range);
    }
    if element_type.is_multirange() {
        ...
        return crate::compat::backend::executor::decode_multirange_bytes(multirange_type, bytes)
    ...
    match element_type.kind {
        ...
        SqlTypeKind::Inet => {
            crate::compat::backend::executor::parse_inet_bytes(bytes).map(Value::Inet)
        }
        SqlTypeKind::Cidr => {
            crate::compat::backend::executor::parse_cidr_bytes(bytes).map(Value::Cidr)
        }
        SqlTypeKind::MacAddr => {
            crate::compat::backend::executor::parse_macaddr_bytes(bytes).map(Value::MacAddr)
        }
        SqlTypeKind::MacAddr8 => {
            crate::compat::backend::executor::parse_macaddr8_bytes(bytes).map(Value::MacAddr8)
        }
        SqlTypeKind::Point => {
            if bytes.len() != 16 {
                return Err(ExecError::InvalidStorageValue {
...
pub fn format_array_text(items: &[Value]) -> String {
    format_array_text_with_config(items, &DateTimeConfig::default())
}

pub fn format_array_text_with_config(items: &[Value], datetime_config: &DateTimeConfig) -> String {
    ...
    for idx in 0..len {
        ...
        *offset += 1;
        match item {
            ...
            Value::Money(v) => {
                out.push_str(&crate::compat::backend::executor::money_format_text(*v))
            }
            Value::Float64(v) => {
            ...
            Value::Bytea(v) => {
                let rendered = crate::compat::backend::libpq::pqformat::format_bytea_text(
                    v,
            ...
            Value::MacAddr(v) => push_array_text_element(
                &mut out,
                &crate::compat::backend::executor::render_macaddr_text(v),
            ),
            Value::MacAddr8(v) => push_array_text_element(
                &mut out,
                &crate::compat::backend::executor::render_macaddr8_text(v),
            ),
            Value::Json(v) => {
            ...
            Value::TsVector(v) => {
                let rendered = crate::compat::backend::executor::render_tsvector_text(v);
                out.push('"');
            ...
            Value::TsQuery(v) => {
                let rendered = crate::compat::backend::executor::render_tsquery_text(v);
                out.push('"');
                for ch in rendered.chars() {
                    match ch {
                        '"' | '\\' => {
            ...
            Value::PgLsn(v) => {
                out.push_str(&crate::compat::backend::executor::render_pg_lsn_text(*v))
            }
            Value::Jsonb(v) => {
            ...
            Value::Range(_) => {
                let rendered = crate::compat::backend::executor::render_range_text_with_config(
                    item,
                    datetime_config,
                )
                .unwrap_or_default();
