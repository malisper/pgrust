...
pub struct TextSearchTokenKind {
    pub tokid: i32,
...
pub struct ParsedTextSearchToken {
    pub tokid: i32,
    pub token: String,
}
...
pub const TOKEN_KINDS: [TextSearchTokenKind; 23] = [
    TextSearchTokenKind {
        tokid: ASCIIWORD,
        alias: "asciiword",
...
pub fn token_kinds() -> &'static [TextSearchTokenKind] {
    &TOKEN_KINDS
}

pub fn token_kind(tokid: i32) -> Option<&'static TextSearchTokenKind> {
    TOKEN_KINDS.iter().find(|kind| kind.tokid == tokid)
}

pub fn parse_default(text: &str) -> Vec<ParsedTextSearchToken> {
    let mut out = Vec::new();
...
pub fn document_tokens(text: &str) -> Vec<DocumentToken> {
...
pub fn normalized_query_node_from_text(
...
pub fn phrase_node_from_positioned_terms(terms: Vec<(u16, TsQueryNode)>) -> Option<TsQueryNode> {
...
pub fn token_has_dictionary(tokid: i32) -> bool {
