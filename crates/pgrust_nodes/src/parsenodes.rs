...
use pgrust_core::{AttributeCompression, AttributeStorage, PolicyCommand, RelFileLocator};
...
pub struct ColumnDef {
    ...
    pub storage: Option<AttributeStorage>,
    pub compression: Option<AttributeCompression>,
...
impl SqlType {
    ...
    pub const fn interval_precision(self) -> Option<i32> {
        ...
        if self.typmod <= crate::datetime::MAX_TIME_PRECISION {
    ...
    pub const fn interval_range(self) -> Option<i32> {
        if !matches!(self.kind, SqlTypeKind::Interval)
            || self.typmod <= crate::datetime::MAX_TIME_PRECISION
        {
            return None;
        }
