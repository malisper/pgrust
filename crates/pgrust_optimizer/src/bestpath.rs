...
use crate::pathnodes::PathMethods;
use pgrust_nodes::pathnodes::{Path, PathKey, RelOptInfo};
use pgrust_nodes::primnodes::Expr;
use pgrust_nodes::primnodes::JoinType;
...
pub fn pathkeys_satisfy(actual: &[PathKey], required: &[PathKey]) -> bool {
...
pub fn set_cheapest(rel: &mut RelOptInfo) {
...
pub fn get_cheapest_path_for_pathkeys<'a>(
    rel: &'a RelOptInfo,
...
pub fn cheaper_than(candidate: &Path, current: Option<&Path>, cost: CostSelector) -> bool {
...
fn order_only_btree_index_path(path: &Path) -> Option<(usize, &[PathKey])> {
    match path {
        ...
        | Path::IndexScan {
            ...
            pathkeys,
            ..
        } if *am_oid == pgrust_catalog_data::BTREE_AM_OID
            && keys.is_empty()
            && !pathkeys.is_empty() =>
        {
...
pub fn preferred_small_full_merge_join(preferred: &Path, other: &Path) -> bool {
    let Path::MergeJoin {
        left,
        right,
        kind: JoinType::Full,
...
pub fn preferred_small_nested_loop_left_join(preferred: &Path, other: &Path) -> bool {
...
pub fn preferred_unqualified_left_join_above_nulltest(preferred: &Path, other: &Path) -> bool {
...
fn top_left_join_restricts(path: &Path) -> Option<&[pgrust_nodes::pathnodes::RestrictInfo]> {
    match path {
        Path::NestedLoopJoin {
            kind: JoinType::Left,
            restrict_clauses,
...
pub fn preferred_field_select_hash_join(preferred: &Path, other: &Path) -> bool {
    let Path::HashJoin {
        kind: JoinType::Inner,
        hash_clauses,
        ..
...
pub fn preferred_parameterized_index_nested_loop(path: &Path) -> bool {
    match path {
        Path::NestedLoopJoin {
            left,
            right,
...
pub fn preferred_parameterized_nested_loop(path: &Path) -> bool {
    match path {
        Path::NestedLoopJoin {
            left,
            right,
...
pub fn preferred_function_outer_hash_join(path: &Path) -> bool {
    match path {
        Path::HashJoin {
            left,
            right,
...
fn path_has_runtime_index_scan(path: &Path) -> bool {
    match path {
        ...
        | Path::IndexScan {
            ...
            matches!(
                key.argument,
                pgrust_nodes::plannodes::IndexScanKeyArgument::Runtime(_)
            )
        }),
        Path::Filter { input, .. }
        | Path::Projection { input, .. }
...
pub fn preferred_scalar_aggregate_outer_cross_join(path: &Path) -> bool {
    match path {
        Path::NestedLoopJoin {
            left,
            right,
...
fn values_cross_like_join(
    kind: JoinType,
    restrict_clauses: &[pgrust_nodes::pathnodes::RestrictInfo],
) -> bool {
