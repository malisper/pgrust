use crate::runtime::eval_geometry_function;
use crate::runtime::eval_power_function;
use crate::runtime::{ExecError, cast_value};
use crate::runtime::{
...
use pgrust_catalog_data::builtin_type_row_by_oid;
use pgrust_catalog_data::pg_proc::builtin_aggregate_function_for_proc_oid;
    ...
    Ok(match call {
        SetReturningCall::RowsFrom {
            ...
            items: items
                .into_iter()
                .map(|item| {
                    Ok(pgrust_nodes::primnodes::RowsFromItem {
...
) -> Result<pgrust_nodes::primnodes::WindowSpec, ParseError> {
    Ok(pgrust_nodes::primnodes::WindowSpec {
...
fn simplify_expr(expr: Expr, case_test_value: Option<&Value>) -> Result<Expr, ParseError> {
    match expr {
        ...
        Expr::GroupingFunc(grouping_func) => Ok(Expr::GroupingFunc(Box::new(
            pgrust_nodes::primnodes::GroupingFuncExpr {
                args: grouping_func
                    .args
                    .into_iter()
        ...
        Expr::SetReturning(srf) => Ok(Expr::SetReturning(Box::new(
            pgrust_nodes::primnodes::SetReturningExpr {
        ...
        Expr::SubLink(sublink) => {
            ...
            Ok(Expr::SubLink(Box::new(pgrust_nodes::primnodes::SubLink {
                testexpr,
                subselect: Box::new(subselect),
                ..*sublink
            })))
        }
        Expr::SubPlan(subplan) => Ok(Expr::SubPlan(Box::new(pgrust_nodes::primnodes::SubPlan {
            testexpr: subplan
                .testexpr
                .map(|expr| simplify_expr(*expr, case_test_value).map(Box::new))
                .transpose()?,
        ...
        Expr::ScalarArrayOp(saop) => Ok(Expr::ScalarArrayOp(Box::new(
            pgrust_nodes::primnodes::ScalarArrayOpExpr {
            ...
            if let Expr::Row { descriptor, fields } = expr {
                    ...
                    .collect::<Option<Vec<_>>>()
                {
                    return Ok(Expr::Const(Value::Record(
                        pgrust_nodes::datum::RecordValue::from_descriptor(descriptor, values),
                    )));
                }
...
fn evaluate_const_op(
    ...
    let value = match (op, args) {
        ...
        (OpExprKind::Eq, [left, right]) => {
            crate::runtime::compare_values("=", left.clone(), right.clone(), collation_oid)?
        }
...
mod tests {
    use super::*;
    use pgrust_nodes::primnodes::{ScalarArrayOpExpr, Var};
    ...
    fn where_scalar_array_null_preserves_plan_qual() {
        let expr = Expr::ScalarArrayOp(Box::new(ScalarArrayOpExpr {
            op: pgrust_nodes::parsenodes::SubqueryComparisonOp::Eq,
            use_or: true,
