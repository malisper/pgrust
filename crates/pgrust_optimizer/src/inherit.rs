...
use crate::runtime::compare_order_values;
use pgrust_analyze::CatalogLookup;
use pgrust_analyze::{PartitionBoundSpec, PartitionRangeDatumValue, partition_value_to_value};
use pgrust_catalog_data::PgInheritsRow;
use pgrust_nodes::datum::Value;
use pgrust_nodes::parsenodes::{PartitionStrategy, RangeTblEntry, RangeTblEntryKind, RangeTblEref};
...
fn compare_list_value_for_ordering(
    left: (
        &pgrust_analyze::SerializedPartitionValue,
        &pgrust_analyze::SerializedPartitionValue,
    ),
) -> Ordering {
    match left {
        (
            pgrust_analyze::SerializedPartitionValue::Null,
            pgrust_analyze::SerializedPartitionValue::Null,
        ) => Ordering::Equal,
        (pgrust_analyze::SerializedPartitionValue::Null, _) => Ordering::Greater,
        (_, pgrust_analyze::SerializedPartitionValue::Null) => Ordering::Less,
...
pub(super) fn translate_append_rel_expr(expr: Expr, info: &AppendRelInfo) -> Expr {
    match expr {
        Expr::Var(var) if var.varlevelsup == 0 && var.varno == info.parent_relid => info
            .translated_vars
            .get(pgrust_nodes::primnodes::attrno_index(var.varattno).unwrap_or(usize::MAX))
            .cloned()
