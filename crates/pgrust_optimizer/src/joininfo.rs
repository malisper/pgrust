...
use pgrust_nodes::pathnodes::{PlannerInfo, RestrictInfo, SpecialJoinInfo};
...
pub fn make_restrict_info_with_pushdown(clause: Expr, is_pushed_down: bool) -> RestrictInfo {
    make_restrict_info_inner(clause, is_pushed_down, 0, false)
}

pub fn make_restrict_info_with_security(
    clause: Expr,
...
pub fn make_restrict_info_with_pushdown_and_security(
    clause: Expr,
    is_pushed_down: bool,
    security_level: usize,
    catalog: &dyn CatalogLookup,
...
pub fn translated_restrict_info(clause: Expr, source: &RestrictInfo) -> RestrictInfo {
...
fn builtin_comparison_op_is_leakproof(op: pgrust_nodes::primnodes::OpExprKind) -> bool {
    matches!(
        op,
...
fn classify_join_clause(clause: &Expr) -> Option<(Vec<usize>, Vec<usize>, Option<u32>)> {
    ...
    let hashjoin_operator =
        matches!(op.op, pgrust_nodes::primnodes::OpExprKind::Eq).then_some(op.opno);
    Some((left_relids, right_relids, hashjoin_operator))
}

pub fn build_special_join_info(query: &Query) -> Vec<SpecialJoinInfo> {
    #[derive(Debug, Clone)]
    struct JoinTreeInfo {
        relids: Vec<usize>,
...
pub fn relids_union(left: &[usize], right: &[usize]) -> Vec<usize> {
...
pub fn relids_intersection(left: &[usize], right: &[usize]) -> Vec<usize> {
    left.iter()
        .copied()
...
pub fn relids_subset(required: &[usize], available: &[usize]) -> bool {
    required.iter().all(|relid| available.contains(relid))
}

pub fn relids_overlap(left: &[usize], right: &[usize]) -> bool {
    left.iter().any(|relid| right.contains(relid))
}

pub fn relids_disjoint(left: &[usize], right: &[usize]) -> bool {
    !relids_overlap(left, right)
}

pub fn expand_join_rte_vars(root: &PlannerInfo, expr: Expr) -> Expr {
    flatten_join_alias_vars_query(&root.parse, expr)
}

pub fn expand_join_rte_vars_query(query: &Query, expr: Expr) -> Expr {
    flatten_join_alias_vars_query(query, expr)
}

pub fn flatten_join_alias_vars(root: &PlannerInfo, expr: Expr) -> Expr {
    flatten_join_alias_vars_query(&root.parse, expr)
}

pub fn flatten_join_alias_vars_query(query: &Query, expr: Expr) -> Expr {
    match expr {
...
pub fn strict_relids(expr: &Expr) -> Vec<usize> {
    match expr {
...
pub fn expr_relids(expr: &Expr) -> Vec<usize> {
    ...
    fn full_join_is_recorded_as_hard_barrier() {
        let query = query_for_jointree(
            JoinTreeNode::JoinExpr {
                ...
                kind: JoinType::Full,
                rtindex: 3,
                quals: Expr::Const(pgrust_nodes::datum::Value::Bool(true)),
            },
