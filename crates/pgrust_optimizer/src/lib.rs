...
pub mod runtime;
pub mod setrefs;
pub mod sublink_pullup;
mod upperrels;
pub mod util;

use crate::pathnodes::PathMethods;
pub use runtime::{OptimizerServices, with_optimizer_services};

use pgrust_analyze::{BoundIndexRelation, CatalogLookup};
use pgrust_catalog_data::PgStatisticRow;
use pgrust_catalog_data::statistics_payload::{
    PgDependenciesPayload, PgMcvListPayload, PgNdistinctPayload,
};
use pgrust_nodes::parsenodes::{JoinTreeNode, Query, SqlType};
use pgrust_nodes::pathnodes::{Path, PlannerConfig, PlannerInfo, RestrictInfo};
use pgrust_nodes::plannodes::{IndexScanKey, IndexScanKeyArgument, Plan, PlannedStmt};
use pgrust_nodes::primnodes::{Expr, JoinType, OpExprKind};
...
#[derive(Debug, Clone)]
pub struct HashJoinClauses {
    pub hash_clauses: Vec<RestrictInfo>,
    pub outer_hash_keys: Vec<Expr>,
    pub inner_hash_keys: Vec<Expr>,
    pub join_clauses: Vec<RestrictInfo>,
}

#[derive(Debug, Clone)]
pub struct MergeJoinClauses {
    pub merge_clauses: Vec<RestrictInfo>,
    pub outer_merge_keys: Vec<Expr>,
    pub inner_merge_keys: Vec<Expr>,
    pub join_clauses: Vec<RestrictInfo>,
}
...
) -> (Plan, Vec<pgrust_nodes::plannodes::ExecParamSource>) {
...
fn create_plan_with_param_base(
    ...
    subplans: &mut Vec<Plan>,
    next_param_id: usize,
) -> (Plan, Vec<pgrust_nodes::plannodes::ExecParamSource>, usize) {
...
pub fn make_restrict_info(clause: Expr) -> RestrictInfo {
...
pub fn pull_up_sublinks(query: Query, catalog: &dyn CatalogLookup) -> Query {
...
pub fn build_join_paths(
    ...
    path::build_join_paths(
        ...
        kind,
        restrict_clauses,
        pgrust_nodes::pathnodes::PathTarget::with_sortgrouprefs(exprs, sortgrouprefs),
        output_columns,
    )
}
...
pub fn fold_query_constants(query: Query) -> Result<Query, pgrust_nodes::parsenodes::ParseError> {
    constfold::fold_query_constants(query)
}

pub fn fold_expr_constants(
    expr: pgrust_nodes::primnodes::Expr,
) -> Result<pgrust_nodes::primnodes::Expr, pgrust_nodes::parsenodes::ParseError> {
    constfold::fold_expr_constants(expr)
}

pub fn planner_with_param_base(
    query: Query,
    catalog: &dyn CatalogLookup,
    next_param_id: usize,
...
pub fn planner_with_param_base_and_config(
    query: Query,
    catalog: &dyn CatalogLookup,
    next_param_id: usize,
    config: PlannerConfig,
...
pub fn finalize_expr_subqueries(
    expr: Expr,
    catalog: &dyn CatalogLookup,
    subplans: &mut Vec<Plan>,
) -> Expr {
...
pub fn finalize_plan_subqueries(
    plan: Plan,
    catalog: &dyn CatalogLookup,
    subplans: &mut Vec<Plan>,
) -> Plan {
