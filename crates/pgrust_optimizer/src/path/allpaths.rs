...
use crate::runtime::DateTimeConfig;
use crate::runtime::compare_order_values;
use crate::runtime::parse_date_text;
use pgrust_analyze::{
    ...
    is_binary_coercible_type, partition_value_to_value,
};
use pgrust_analyze::{bind_relation_constraints, predicate_implies_index_predicate};
use pgrust_catalog_data::PG_LARGEOBJECT_METADATA_RELATION_OID;
use pgrust_catalog_data::desc::column_desc;
use pgrust_catalog_data::{BTREE_AM_OID, HASH_AM_OID, SPGIST_AM_OID, SPGIST_TEXT_FAMILY_OID};
use pgrust_core::RelFileLocator;
use pgrust_nodes::datum::Value;
use pgrust_nodes::parsenodes::{
    JoinTreeNode, PartitionStrategy, Query, RangeTblEntryKind, SetOperator, SqlType, SqlTypeKind,
    SubqueryComparisonOp, TableSampleClause, WindowFrameMode,
};
...
fn append_partition_prune_plan(
    spec: Option<pgrust_analyze::LoweredPartitionSpec>,
...
fn partition_key_fixed_by_clause(root: &PlannerInfo, key_expr: &Expr, clause: &Expr) -> bool {
    match clause {
        Expr::Op(op) if matches!(op.op, pgrust_nodes::primnodes::OpExprKind::Eq) => {
...
fn relation_display_name(
    catalog: &dyn CatalogLookup,
    rte: &pgrust_nodes::parsenodes::RangeTblEntry,
    relation_oid: u32,
...
fn access_method_supports_index_scan(am_oid: u32) -> bool {
    ...
    crate::runtime::access_method_supports_index_scan(am_oid)
}
...
fn access_method_supports_bitmap_scan(am_oid: u32) -> bool {
    crate::runtime::access_method_supports_bitmap_scan(am_oid)
}

fn brin_partial_bitmap_allowed(index: &BoundIndexRelation, config: PlannerConfig) -> bool {
    index.index_meta.am_oid != pgrust_catalog_data::BRIN_AM_OID
...
fn target_index_only_unique_btree(
    index: &pgrust_analyze::BoundIndexRelation,
    target_index_only: bool,
) -> bool {
    target_index_only
...
fn plan_set_operation_child_path(
    mut query: pgrust_nodes::parsenodes::Query,
    sorted: bool,
    catalog: &dyn CatalogLookup,
    config: PlannerConfig,
) -> (PlannerInfo, Path) {
            ...
            if !force_sorted_union && can_hash && (config.enable_hashagg || !can_sort) {
                ...
                optimize_path_with_config(
                    Path::Aggregate {
                        ...
                        phase: pgrust_nodes::plannodes::AggregatePhase::Complete,
...
fn builtin_aggregate_is_count(aggfnoid: u32) -> bool {
    pgrust_catalog_data::builtin_aggregate_function_for_proc_oid(aggfnoid)
...
fn window_comparison_parts<'a>(
    ...
    &'a pgrust_nodes::primnodes::OpExpr,
    &'a WindowFuncExpr,
    Expr,
    OpExprKind,
)> {
...
fn make_window_left_comparison(
    template: &pgrust_nodes::primnodes::OpExpr,
    func: &WindowFuncExpr,
    op: OpExprKind,
    other: Expr,
) -> Expr {
...
fn visible_query_targets(query: &Query) -> Vec<&pgrust_nodes::primnodes::TargetEntry> {
    query
        .target_list
        .iter()
        .filter(|target| !target.resjunk)
...
fn set_operation_identity_targets(
    desc: &RelationDesc,
) -> Vec<pgrust_nodes::primnodes::TargetEntry> {
    desc.columns
        .iter()
        .enumerate()
        .map(|(index, column)| {
            pgrust_nodes::primnodes::TargetEntry::new(
                column.name.clone(),
                Expr::Var(Var {
                    varno: 1,
                    varattno: user_attrno(index),
...
fn rewrite_filter_for_subquery_relaxed(
    expr: Expr,
    rtindex: usize,
    targets: &[&pgrust_nodes::primnodes::TargetEntry],
) -> Option<Expr> {
    match expr {
        Expr::Var(var) => {
    ...
    match expr {
        ...
        Expr::Case(mut case_expr) => {
            ...
            case_expr.args = case_expr
                ...
                .into_iter()
                .map(|arm| {
                    Some(pgrust_nodes::primnodes::CaseWhen {
...
fn rewrite_subquery_filter_subscript(
    subscript: pgrust_nodes::primnodes::ExprArraySubscript,
    rtindex: usize,
    targets: &[&pgrust_nodes::primnodes::TargetEntry],
    query: &Query,
) -> Option<pgrust_nodes::primnodes::ExprArraySubscript> {
    Some(pgrust_nodes::primnodes::ExprArraySubscript {
...
struct PartitionKeySpec {
    ...
    keys: Vec<(i16, pgrust_nodes::parsenodes::SqlType)>,
}
