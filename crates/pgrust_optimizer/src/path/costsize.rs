...
use crate::runtime::{
    cast_value, compare_order_values, network_btree_upper_bound, network_prefix,
    statistics_value_key,
};
use pgrust_analyze::predicate_implies_index_predicate;
use pgrust_analyze::{BoundIndexRelation, CatalogLookup};
use pgrust_analyze::{bind_expr_with_outer_and_ctes, scope_for_relation};
use pgrust_catalog_data::sql_type_oid;
use pgrust_catalog_data::statistics_payload::{
    PgDependencyItem, PgMcvItem, PgMcvListPayload, decode_pg_dependencies_payload,
    decode_pg_mcv_list_payload, decode_pg_ndistinct_payload,
};
...
use pgrust_core::access::{
    BRIN_DEFAULT_PAGES_PER_RANGE, REVMAP_PAGE_MAXITEMS, SIZEOF_HEAP_TUPLE_HEADER,
    SPGIST_CONFIG_PROC,
};
use pgrust_core::storage::{BLCKSZ, ITEM_ID_SIZE, MAXALIGN, SIZE_OF_PAGE_HEADER_DATA};
use pgrust_nodes::datetime::{TIMESTAMP_NOBEGIN, TIMESTAMP_NOEND};
...
use pgrust_nodes::parsenodes::{
    JoinTreeNode, ParseError, Query, RangeTblEntryKind, SqlType, SqlTypeKind, SubqueryComparisonOp,
    TableSampleClause, WindowFrameMode,
};
        ...
        .filter_map(|row| {
            ...
            let expressions = expression_texts
                ...
                .filter_map(|(idx, expr_text)| {
                    let parsed = pgrust_parser::parse_expr(expr_text).ok()?;
                    let expr =
...
fn btree_index_natural_pathkeys(
    ...
    (0..index_key_count(index))
        .filter_map(|index_pos| {
            ...
            let backward = matches!(direction, pgrust_nodes::access::ScanDirection::Backward);
            Some(PathKey {
                expr: Expr::Var(Var {
                    varno: source_id,
...
fn planner_query_is_dml_row_source(root: &PlannerInfo) -> bool {
    ...
    matches!(
        ...
        pgrust_nodes::CommandType::Update | pgrust_nodes::CommandType::Delete
...
fn hash_clause_from_runtime_index_key(
    ...
    let clause = Expr::Op(Box::new(pgrust_nodes::primnodes::OpExpr {
        opno: 0,
        opfuncid: 0,
        op: OpExprKind::Eq,
...
fn parameterized_subquery_inner_path(
    ...
    pathtarget: &PathTarget,
    rtindex: usize,
    subroot: &pgrust_nodes::pathnodes::PlannerSubroot,
    query: &pgrust_nodes::parsenodes::Query,
    input: &Path,
...
fn target_entry_uses_outer_relids_at_level(
    target: &pgrust_nodes::primnodes::TargetEntry,
    relids: &[usize],
    sublevels_up: usize,
) -> bool {
...
fn sort_group_uses_outer_relids_at_level(
    item: &pgrust_nodes::primnodes::SortGroupClause,
    relids: &[usize],
    sublevels_up: usize,
) -> bool {
    expr_uses_outer_relids_at_level(&item.expr, relids, sublevels_up)
...
fn window_clause_uses_outer_relids_at_level(
    clause: &pgrust_nodes::primnodes::WindowClause,
    relids: &[usize],
    sublevels_up: usize,
) -> bool {
    clause
...
fn backward_full_index_scan_path(path: Path) -> Path {
    match path {
        Path::IndexOnlyScan {
            ...
            && !pathkeys.is_empty()
            && !matches!(direction, pgrust_nodes::access::ScanDirection::Backward) =>
        {
            ...
            Path::IndexOnlyScan {
                ...
                keys,
                order_by_keys,
                direction: pgrust_nodes::access::ScanDirection::Backward,
                pathkeys,
            }
        }
        Path::IndexScan {
...
pub(super) fn build_index_path_spec(
    ...
    Some(IndexPathSpec {
        ...
        direction: order_match
            ...
            .unwrap_or(pgrust_nodes::access::ScanDirection::Forward),
...
pub(super) fn full_index_scan_spec(
    ...
    IndexPathSpec {
        ...
        filter_quals,
        direction: pgrust_nodes::access::ScanDirection::Forward,
        removes_order: false,
...
fn clause_selectivity_internal(
    ...
    match expr {
        ...
        Expr::ScalarArrayOp(saop)
            if matches!(
                saop.op,
                pgrust_nodes::parsenodes::SubqueryComparisonOp::Eq
                    | pgrust_nodes::parsenodes::SubqueryComparisonOp::NotEq
                    | pgrust_nodes::parsenodes::SubqueryComparisonOp::Lt
                    | pgrust_nodes::parsenodes::SubqueryComparisonOp::LtEq
                    | pgrust_nodes::parsenodes::SubqueryComparisonOp::Gt
                    | pgrust_nodes::parsenodes::SubqueryComparisonOp::GtEq
            ) =>
        {
...
pub(super) fn dependency_clause_target_id(expr: &Expr, ext: &ExtendedStatistic) -> Option<i16> {
    match expr {
        ...
        Expr::ScalarArrayOp(saop)
            if saop.use_or
                && matches!(saop.op, pgrust_nodes::parsenodes::SubqueryComparisonOp::Eq) =>
        {
...
fn scalar_array_selectivity(
    ...
    for element in &array.elements {
        ...
        if saop.use_or {
            ...
            if matches!(saop.op, pgrust_nodes::parsenodes::SubqueryComparisonOp::Eq) {
    ...
    if ((saop.use_or && matches!(saop.op, pgrust_nodes::parsenodes::SubqueryComparisonOp::Eq))
        || (!saop.use_or
            && matches!(
                saop.op,
                pgrust_nodes::parsenodes::SubqueryComparisonOp::NotEq
...
fn scalar_array_column_containment_selectivity(
    ...
    let stats = stats?;
    let is_contains =
        saop.use_or && matches!(saop.op, pgrust_nodes::parsenodes::SubqueryComparisonOp::Eq);
...
fn scalar_array_element_selectivity(
    op: pgrust_nodes::parsenodes::SubqueryComparisonOp,
    left: &Expr,
    element: &Value,
    stats: Option<&RelationStats>,
    reltuples: f64,
    ...
    match op {
        ...
        pgrust_nodes::parsenodes::SubqueryComparisonOp::Gt => ineq_selectivity(
            left,
            &element_expr,
            stats,
            reltuples,
        ...
        pgrust_nodes::parsenodes::SubqueryComparisonOp::GtEq => ineq_selectivity(
            left,
            &element_expr,
            stats,
            reltuples,
...
fn scalar_array_key_matches(
    ...
    match op {
        pgrust_nodes::parsenodes::SubqueryComparisonOp::Eq => ordering == Ordering::Equal,
        pgrust_nodes::parsenodes::SubqueryComparisonOp::NotEq => ordering != Ordering::Equal,
        pgrust_nodes::parsenodes::SubqueryComparisonOp::Lt => ordering == Ordering::Less,
        pgrust_nodes::parsenodes::SubqueryComparisonOp::LtEq => {
            matches!(ordering, Ordering::Less | Ordering::Equal)
        }
        pgrust_nodes::parsenodes::SubqueryComparisonOp::Gt => ordering == Ordering::Greater,
        pgrust_nodes::parsenodes::SubqueryComparisonOp::GtEq => {
            matches!(ordering, Ordering::Greater | Ordering::Equal)
        }
        _ => false,
...
fn strip_casts(expr: &Expr) -> &Expr {
    match expr {
        Expr::Func(func)
            if matches!(
                func.implementation,
                pgrust_nodes::primnodes::ScalarFunctionImpl::Builtin(
...
fn array_literal_element_type_oid(element_type: SqlType) -> u32 {
    match element_type.kind {
        SqlTypeKind::Int2Vector => pgrust_catalog_data::INT2VECTOR_TYPE_OID,
        SqlTypeKind::OidVector => pgrust_catalog_data::OIDVECTOR_TYPE_OID,
...
fn const_gist_argument_value(expr: &Expr) -> Option<Value> {
    ...
    let values = func
        ...
        .collect::<Option<Vec<_>>>()?;
    if let Some(result) = crate::runtime::eval_range_function(
        builtin,
        &values,
        func.funcresulttype,
        func.funcvariadic,
        None,
        &crate::runtime::DateTimeConfig::default(),
    ) {
        return result.ok();
    }
    if let Some(result) = crate::runtime::eval_geometry_function(builtin, &values) {
        return result.ok();
    }
    None
}
...
fn row_prefix_indexable_qual(
    op: &pgrust_nodes::primnodes::OpExpr,
    expr: &Expr,
    argument_for: fn(&Expr) -> Option<IndexScanKeyArgument>,
) -> Option<IndexableQual> {
...
fn index_order_match(
...
) -> Option<(usize, pgrust_nodes::access::ScanDirection)> {
    const BT_DESC_FLAG: i16 = 0x0001;
    ...
    for index_pos in 0..equality_prefix.min(index_key_count(index)) {
        let Some(item) = items.get(item_start) else {
            return Some((item_start, pgrust_nodes::access::ScanDirection::Forward));
        };
    ...
    for (idx, item) in items.iter().skip(item_start).enumerate() {
        ...
        let item_direction = match (forward_matches, backward_matches) {
            (true, _) => pgrust_nodes::access::ScanDirection::Forward,
            (false, true) => pgrust_nodes::access::ScanDirection::Backward,
    ...
    (item_start + matched == items.len()).then_some((
        item_start + matched,
        direction.unwrap_or(pgrust_nodes::access::ScanDirection::Forward),
    ))
}
...
fn gist_order_match(
    ...
    (
        keys,
        Some((items.len(), pgrust_nodes::access::ScanDirection::Forward)),
    )
}
