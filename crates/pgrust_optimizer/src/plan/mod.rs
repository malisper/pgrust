...
use pgrust_nodes::parsenodes::Query;
use pgrust_nodes::pathnodes::{PathTarget, PlannerConfig, PlannerInfo, RelOptInfo};
use pgrust_nodes::plannodes::{Plan, PlannedStmt};
use pgrust_nodes::primnodes::Expr;

pub(crate) fn planner(
    query: Query,
    catalog: &dyn CatalogLookup,
) -> Result<PlannedStmt, pgrust_nodes::parsenodes::ParseError> {
...
pub(crate) fn planner_with_param_base(
    ...
    catalog: &dyn CatalogLookup,
    next_param_id: usize,
) -> Result<(PlannedStmt, usize), pgrust_nodes::parsenodes::ParseError> {
