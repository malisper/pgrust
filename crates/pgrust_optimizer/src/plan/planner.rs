...
use pgrust_nodes::plannodes::{AggregatePhase, AggregateStrategy, PlanEstimate, PlannedStmt};
...
fn grouping_contains_inner_join_key(
    ...
    root.inner_join_clauses.iter().any(|restrict| {
        ...
        if !matches!(op.op, pgrust_nodes::primnodes::OpExprKind::Eq) {
            return false;
        }
    ...
    root.join_info_list.iter().any(|join| {
        matches!(
            ...
            pgrust_nodes::primnodes::JoinType::Left
                | pgrust_nodes::primnodes::JoinType::Right
                | pgrust_nodes::primnodes::JoinType::Full
        ) && join
            .syn_lefthand
            .iter()
            .chain(join.syn_righthand.iter())
...
fn expand_window_clause(root: &PlannerInfo, clause: &WindowClause) -> WindowClause {
    let expand_frame_bound = |bound: pgrust_nodes::primnodes::WindowFrameBound| match bound {
...
fn builtin_window_optimized_frame(func: BuiltinWindowFunction) -> Option<WindowFrame> {
    match func {
        ...
        | BuiltinWindowFunction::Ntile => Some(WindowFrame {
            mode: pgrust_nodes::parsenodes::WindowFrameMode::Rows,
            start_bound: WindowFrameBound::UnboundedPreceding,
            end_bound: WindowFrameBound::CurrentRow,
            exclusion: pgrust_nodes::parsenodes::WindowFrameExclusion::NoOthers,
        }),
        _ => None,
    }
}
...
fn sort_group_pathkeys(
    clauses: &[pgrust_nodes::primnodes::SortGroupClause],
    target_list: &[TargetEntry],
) -> Vec<PathKey> {
...
fn expr_equated_to_constant(root: &PlannerInfo, predicate: &Expr, key: &Expr) -> bool {
    match predicate {
        ...
        Expr::Op(op)
            if matches!(op.op, pgrust_nodes::primnodes::OpExprKind::Eq) && op.args.len() == 2 =>
        {
...
fn make_lock_rows_rel(
    root: &PlannerInfo,
    input_rel: RelOptInfo,
    row_marks: &[pgrust_nodes::parsenodes::QueryRowMark],
    catalog: &dyn CatalogLookup,
) -> RelOptInfo {
    let mut rel = RelOptInfo::new(
        input_rel.relids.clone(),
...
fn render_sort_key_expr(
    ...
    match expr {
        ...
        Expr::Var(var) if var.varlevelsup == 0 => root
            ...
            .get(var.varno.saturating_sub(1))
            .and_then(|rte| {
                pgrust_nodes::primnodes::attrno_index(var.varattno).and_then(|index| {
        ...
        Expr::Cast(inner, ty)
            if matches!(ty.kind, pgrust_nodes::parsenodes::SqlTypeKind::RegClass)
                && expr_contains_tableoid_var(inner) =>
        {
            format!(
                "(({})::regclass)",
            ...
            ScalarFunctionImpl::Builtin(BuiltinScalarFunction::GeoPoint)
                if func.args.len() == 1
                    && expr_sql_type(&func.args[0]).kind
                        == pgrust_nodes::parsenodes::SqlTypeKind::Polygon =>
            {
                format!(
                    "poly_center({})",
...
fn render_sort_key_function_expr(
    root: &PlannerInfo,
    func: &pgrust_nodes::primnodes::FuncExpr,
    catalog: &dyn CatalogLookup,
    qualify_relation_vars: bool,
) -> String {
    let name = match func.implementation {
        ...
        _ => {
            return crate::runtime::render_explain_expr(&Expr::Func(Box::new(func.clone())), &[]);
        }
    };
    let args = func
        .args
