...
use pgrust_nodes::plannodes::{Plan, PlannedStmt, TidScanCond, TidScanSource};
...
fn lower_sublink_to_subplan(
    ...
    let [subselect] = crate::runtime::pg_rewrite_query(*sublink.subselect, catalog)
...
pub(super) fn finalize_expr_subqueries(
    ...
    match expr {
        ...
        Expr::Aggref(aggref) => Expr::Aggref(Box::new(pgrust_nodes::primnodes::Aggref {
            args: aggref
                .args
                .into_iter()
                .map(|arg| finalize_expr_subqueries(arg, catalog, subplans))
        ...
        Expr::WindowFunc(window_func) => {
            Expr::WindowFunc(Box::new(pgrust_nodes::primnodes::WindowFuncExpr {
                kind: match window_func.kind {
                    pgrust_nodes::primnodes::WindowFuncKind::Aggregate(aggref) => {
                        pgrust_nodes::primnodes::WindowFuncKind::Aggregate(
                    ...
                    pgrust_nodes::primnodes::WindowFuncKind::Builtin(kind) => {
                        pgrust_nodes::primnodes::WindowFuncKind::Builtin(kind)
                    }
                },
                args: window_func
        ...
        Expr::SetReturning(srf) => {
            Expr::SetReturning(Box::new(pgrust_nodes::primnodes::SetReturningExpr {
...
fn rebase_window_clause_subplan_ids(
    ...
    base: usize,
) -> pgrust_nodes::primnodes::WindowClause {
    pgrust_nodes::primnodes::WindowClause {
        ...
        functions: clause
            .functions
            .into_iter()
            .map(|func| pgrust_nodes::primnodes::WindowFuncExpr {
...
fn rebase_window_frame_bound_subplan_ids(
    bound: pgrust_nodes::primnodes::WindowFrameBound,
    base: usize,
) -> pgrust_nodes::primnodes::WindowFrameBound {
    match bound {
        pgrust_nodes::primnodes::WindowFrameBound::OffsetPreceding(offset) => {
            let expr = rebase_expr_subplan_ids(offset.expr.clone(), base);
            pgrust_nodes::primnodes::WindowFrameBound::OffsetPreceding(offset.with_expr(expr))
        }
        pgrust_nodes::primnodes::WindowFrameBound::OffsetFollowing(offset) => {
            let expr = rebase_expr_subplan_ids(offset.expr.clone(), base);
            pgrust_nodes::primnodes::WindowFrameBound::OffsetFollowing(offset.with_expr(expr))
        }
        other => other,
    }
}
...
fn rebase_partition_prune_info(
    mut info: pgrust_nodes::plannodes::PartitionPrunePlan,
    base: usize,
) -> pgrust_nodes::plannodes::PartitionPrunePlan {
    ...
    match plan {
        ...
        Plan::NestedLoopJoin {
            ...
            nest_params: nest_params
                .into_iter()
                .map(|param| pgrust_nodes::plannodes::ExecParamSource {
        ...
        Plan::Projection {
            ...
            targets: targets
                .into_iter()
                .map(|target| pgrust_nodes::primnodes::TargetEntry {
            ...
            targets: targets
                ...
                .map(|target| match target {
                    ProjectSetTarget::Scalar(entry) => {
                        ProjectSetTarget::Scalar(pgrust_nodes::primnodes::TargetEntry {
