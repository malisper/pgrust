...
use pgrust_nodes::pathnodes::{AppendRelInfo, Path, PlannerInfo};
...
pub fn rewrite_semantic_expr_for_path(expr: Expr, path: &Path, layout: &[Expr]) -> Expr {
    let original = expr;
...
pub fn rewrite_semantic_expr_for_path_or_expand_join_vars(
    root: &PlannerInfo,
    expr: Expr,
    path: &Path,
    layout: &[Expr],
