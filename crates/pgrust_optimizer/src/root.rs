...
pub fn prepare_query_for_planning(query: Query, catalog: &dyn CatalogLookup) -> Query {
...
fn prepare_agg_accum_for_locking(accum: AggAccum) -> Result<AggAccum, ParseError> {
    Ok(AggAccum {
        ...
        order_by: accum
            ...
            .into_iter()
            .map(|item| {
                Ok(pgrust_nodes::primnodes::OrderByEntry {
                    expr: prepare_expr_for_locking(item.expr)?,
                    ..item
                })
            })
...
) -> Result<pgrust_nodes::primnodes::WindowClause, ParseError> {
    Ok(pgrust_nodes::primnodes::WindowClause {
...
fn prepare_window_frame_for_locking(
...
) -> Result<pgrust_nodes::primnodes::WindowFrame, ParseError> {
    use pgrust_nodes::primnodes::WindowFrameBound;
    ...
    Ok(pgrust_nodes::primnodes::WindowFrame {
        mode: frame.mode,
...
fn prepare_window_func_for_locking(
    func: pgrust_nodes::primnodes::WindowFuncExpr,
) -> Result<pgrust_nodes::primnodes::WindowFuncExpr, ParseError> {
    use pgrust_nodes::primnodes::WindowFuncKind;

    Ok(pgrust_nodes::primnodes::WindowFuncExpr {
...
) -> Result<pgrust_nodes::parsenodes::RecursiveUnionQuery, ParseError> {
    Ok(pgrust_nodes::parsenodes::RecursiveUnionQuery {
...
fn prepare_set_operation_for_locking(
    ...
    Ok(pgrust_nodes::parsenodes::SetOperationQuery {
    ...
    Ok(match expr {
        ...
        Expr::Case(case_expr) => Expr::Case(Box::new(pgrust_nodes::primnodes::CaseExpr {
            ...
            args: case_expr
                ...
                .into_iter()
                .map(|arm| {
                    Ok(pgrust_nodes::primnodes::CaseWhen {
...
pub fn planner_info_new(parse: Query, aggregate_layout: AggregateLayout) -> PlannerInfo {
    planner_info_new_with_config(parse, aggregate_layout, PlannerConfig::default())
}

pub fn planner_info_new_with_config(
    parse: Query,
...
fn unique_lookup_equality_column(expr: Expr) -> Option<i16> {
    ...
    if op.op != pgrust_nodes::primnodes::OpExprKind::Eq || op.args.len() != 2 {
        return None;
    }
...
fn make_window_input_target(
    ...
    for clause in &parse.window_clauses {
        ...
        match &clause.spec.frame.start_bound {
            ...
            pgrust_nodes::primnodes::WindowFrameBound::UnboundedPreceding
            | pgrust_nodes::primnodes::WindowFrameBound::CurrentRow
            | pgrust_nodes::primnodes::WindowFrameBound::UnboundedFollowing => {}
        }
