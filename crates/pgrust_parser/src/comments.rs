pub fn strip_sql_comments_preserving_layout(sql: &str) -> String {
...
pub fn normalize_string_continuation_preserving_layout(sql: &str) -> String {
    let bytes = sql.as_bytes();
...
pub fn find_comment_blocked_string_continuation(sql: &str) -> Option<String> {
    let bytes = sql.as_bytes();
    let mut i = 0usize;
...
pub fn normalize_position_syntax_preserving_layout(sql: &str) -> String {
    let bytes = sql.as_bytes();
...
pub fn sql_is_effectively_empty_after_comments(sql: &str) -> bool {
