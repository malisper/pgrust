...
pub fn with_restrict_nonsystem_view_expansion<T>(enabled: bool, f: impl FnOnce() -> T) -> T {
...
fn rewrite_query(
    ...
    let mut rewritten = Query {
        ...
        recursive_union: query
            .recursive_union
            .map(|recursive_union| {
                Ok(Box::new(pgrust_nodes::parsenodes::RecursiveUnionQuery {
        ...
        set_operation: query
            .set_operation
            .map(|set_operation| {
                Ok(Box::new(pgrust_nodes::parsenodes::SetOperationQuery {
                    ...
                    inputs: set_operation
                        ...
                        .into_iter()
                        .map(|input| {
                            rewrite_query(input, catalog, expanded_views, active_policy_relations)
                        })
                        .collect::<Result<Vec<_>, _>>()?,
                }))
            })
...
pub fn relation_has_security_invoker(catalog: &dyn CatalogLookup, relation_oid: u32) -> bool {
    catalog
...
fn mark_selected_relation_column(
    ...
    local_privileges: &mut [Option<RelationPrivilegeRequirement>],
    privileges: &mut Vec<RelationPrivilegeRequirement>,
    var: &pgrust_nodes::primnodes::Var,
) {
    ...
    let kind = match rte.kind {
        RangeTblEntryKind::Relation {
            ...
            if restrict_nonsystem_view_expansion_enabled() {
                ...
                if class_row.relnamespace != pgrust_catalog_data::PG_CATALOG_NAMESPACE_OID {
...
fn rewrite_semantic_expr(
    ...
    Ok(match expr {
        ...
        Expr::Xml(xml_expr) => {
            let pgrust_nodes::primnodes::XmlExpr {
                op,
                name,
                named_args,
                arg_names,
            ...
            Expr::Xml(Box::new(pgrust_nodes::primnodes::XmlExpr {
                op,
                name,
