use pgrust_analyze::{CatalogLookup, sql_type_name};
use pgrust_catalog_data::PgRewriteRow;
use pgrust_nodes::datum::Value;
use pgrust_nodes::parsenodes::{
    Assignment, AssignmentTarget, AssignmentTargetIndirection, InsertSource, InsertStatement,
    OnConflictAction, OnConflictClause, OnConflictTarget, RawTypeName, ReturningAliasKind,
    ReturningClause, SelectItem, SelectStatement, SqlCallArgs, SqlExpr, SqlType, SqlTypeKind,
    Statement, UpdateStatement, ValuesStatement,
};
use pgrust_nodes::primnodes::RelationDesc;
use pgrust_parser::parse_statement;

pub fn split_stored_rule_action_sql(ev_action: &str) -> Vec<&str> {
...
pub fn format_stored_rule_definition(rule: &PgRewriteRow, relation_name: &str) -> String {
    format_stored_rule_definition_inner(rule, relation_name, None)
}

pub fn format_stored_rule_definition_with_catalog(
    rule: &PgRewriteRow,
    relation_name: &str,
