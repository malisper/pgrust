...
fn return_rule_row(
    ...
    relation_oid: u32,
    display_name: &str,
) -> Result<pgrust_catalog_data::PgRewriteRow, ParseError> {
...
pub fn format_view_definition_unpretty(
    relation_oid: u32,
    relation_desc: &RelationDesc,
    catalog: &dyn CatalogLookup,
) -> Result<String, ParseError> {
...
pub fn render_relation_expr_sql_for_constraint(
    expr: &Expr,
    relation_name: Option<&str>,
    desc: &RelationDesc,
    catalog: &dyn CatalogLookup,
...
pub fn refresh_query_relation_descriptors(query: &mut Query, catalog: &dyn CatalogLookup) {
...
fn load_view_return_select_from_rule(
    rule: pgrust_catalog_data::PgRewriteRow,
    alias: Option<&str>,
    ...
    if is_unsupported_select_statement(&stmt) {
        stmt = parse_statement(&normalize_deparsed_view_sql_for_parser(sql))?;
    }
    let select = match stmt {
        Statement::Select(select) => select,
        Statement::Values(values) => wrap_values_as_select(values),
        _ => {
            return Err(ParseError::UnexpectedToken {
...
pub fn rewrite_view_relation_query(
    relation_oid: u32,
    relation_desc: &RelationDesc,
    alias: Option<&str>,
    catalog: &dyn CatalogLookup,
...
fn json_table_function_name(kind: pgrust_nodes::primnodes::JsonTableFunction) -> &'static str {
    match kind {
        ...
        pgrust_nodes::primnodes::JsonTableFunction::ArrayElementsText => "json_array_elements_text",
        ...
        pgrust_nodes::primnodes::JsonTableFunction::JsonbArrayElements => "jsonb_array_elements",
...
fn regex_table_function_name(kind: pgrust_nodes::primnodes::RegexTableFunction) -> &'static str {
    match kind {
...
fn string_table_function_name(kind: pgrust_nodes::primnodes::StringTableFunction) -> &'static str {
    match kind {
...
fn text_search_table_function_name(
    kind: pgrust_nodes::primnodes::TextSearchTableFunction,
) -> &'static str {
    match kind {
...
fn render_sort_group_clause(
    sort: &pgrust_nodes::primnodes::SortGroupClause,
    ctx: &ViewDeparseContext<'_>,
) -> String {
...
fn render_overlaps_bool_expr(
    bool_expr: &pgrust_nodes::primnodes::BoolExpr,
    ctx: &ViewDeparseContext<'_>,
) -> Option<String> {
...
fn render_xml_expr(xml: &pgrust_nodes::primnodes::XmlExpr, ctx: &ViewDeparseContext<'_>) -> String {
    match xml.op {
        pgrust_nodes::primnodes::XmlExprOp::Concat => format!(
            "XMLCONCAT({})",
            xml.args
                .iter()
                .map(|arg| render_expr(arg, ctx))
        ...
        pgrust_nodes::primnodes::XmlExprOp::Forest => format!(
            "XMLFOREST({})",
            xml.args
                .iter()
                .zip(xml.arg_names.iter())
        ...
        pgrust_nodes::primnodes::XmlExprOp::Root => {
            let version = match xml.root_version {
                XmlRootVersion::Value => xml
                    .args
                    .get(1)
                    .map(|arg| format!("VERSION {}", render_expr(arg, ctx)))
                    .unwrap_or_else(|| "VERSION NO VALUE".into()),
                XmlRootVersion::NoValue | XmlRootVersion::Omitted => "VERSION NO VALUE".into(),
            };
            let standalone = match xml.standalone {
                Some(XmlStandalone::Yes) => ", STANDALONE YES",
                Some(XmlStandalone::No) => ", STANDALONE NO",
                Some(XmlStandalone::NoValue) => ", STANDALONE NO VALUE",
                None => "",
            };
            format!(
...
fn render_xml_option(option: Option<XmlOption>) -> &'static str {
    match option {
        Some(XmlOption::Document) => "DOCUMENT",
        Some(XmlOption::Content) | None => "CONTENT",
    }
}
...
fn render_bytea_text_literal(text: &str) -> String {
    match pgrust_expr::parse_bytea_text(text) {
...
fn render_literal(value: &Value) -> String {
    match value {
        ...
        Value::IndirectVarlena(indirect) => {
            pgrust_expr::value_io::indirect_varlena_to_value(indirect)
