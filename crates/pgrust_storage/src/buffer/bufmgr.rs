...
impl<S: StorageBackend + Send> BufferPool<S> {
    ...
    pub fn new_with_wal<W>(storage: S, capacity: usize, wal: Arc<W>) -> Self
    where
        W: WalSink + 'static,
    {
        let wal: Arc<dyn WalSink> = wal;
    ...
    pub fn wal_writer<W>(&self) -> Option<Arc<W>>
    where
        W: WalSink + 'static,
    {
        let wal = Arc::clone(self.wal.as_ref()?);
        let any: Arc<dyn std::any::Any + Send + Sync> = wal;
        Arc::downcast::<W>(any).ok()
    }
...
mod tests {
    ...
    impl WalSink for TestWalSink {
        ...
        fn write_commit(&self, _xid: u32) -> Result<Lsn, String> {
            Ok(self.write_next())
        }

        fn write_prepare(&self, _xid: u32, _data: &[u8]) -> Result<Lsn, String> {
            Ok(self.write_next())
        }

        fn write_abort(&self, _xid: u32) -> Result<Lsn, String> {
            Ok(self.write_next())
        }
        ...
        fn write_record(&self, _xid: u32, _tag: BufferTag, _page: &Page) -> Result<Lsn, String> {
            Ok(self.write_next())
        }
    ...
    fn pool_with_wal(
        ...
        let wal = Arc::new(TestWalSink::new());
    ...
    fn integ_wal_stamps_pd_lsn_in_page() {
        ...
        let wal = pool.wal_writer::<TestWalSink>().expect("test wal sink");
        assert_eq!(
            lsn_after,
            wal.last_lsn(),
            "pd_lsn should match the assigned WAL record end LSN"
        );
    }
