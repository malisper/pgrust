...
fn plpgsql_compile_from_source_inner(facts: &ProcCompileFacts) -> PgResult<PLpgSQL_function> {
    ...
    function.fn_signature = {
        ...
        let s = regproc_seams::format_procedure::call(ctx.mcx(), facts.fn_oid)?;
        s.as_str().to_string()
    };
...
fn parse_function_body(src: &str) -> PgResult<Box<PLpgSQL_stmt_block>> {
    ...
    let scanner = plpgsql_scanner::plpgsql_scanner_init(ctx.mcx(), &scanbuf, src);
    ...
    let compile_cb_id =
        utils_error::push_emit_context_callback(Box::new(move |err: &mut PgError| {
    ...
    let prev_armed =
        scan_seams::set_plpgsql_body_warnings_armed(plpgsql_check_syntax());
    ...
    let parse_result = plpgsql_gram::plpgsql_yyparse_with_lineno(scanner);
    scan_seams::set_plpgsql_body_warnings_armed(prev_armed);
    utils_error::pop_emit_context_callback(compile_cb_id);
