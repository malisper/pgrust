...
pub(crate) use plpgsql_exec_seams as exec_seams;
...
/// PL/pgSQL function OIDs — the dual-home `fmgr`↔`nodes` fcinfo
...
fn deconstruct_composite_into_columns(
    ...
    with_query_mcx(|mcx| {
        ...
        let tupdesc = cache_typcache::lookup_rowtype_tupdesc(
            mcx, src_typeid, src_typmod,
        )?;

        let deformed = heaptuple::heap_deform_tuple(
            mcx,
            &ft.tuple,
            &tupdesc,
            ft.data.as_slice(),
...
fn coerce_function_result_tuple(
    ...
    with_query_mcx(|mcx| {
        ...
        let mut out = match map {
            Some(m) => next::tupconvert::execute_attr_map_tuple(
                mcx,
                &ft.tuple,
                ft.data.as_slice(),
                &m,
