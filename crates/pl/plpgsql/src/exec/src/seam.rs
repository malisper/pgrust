...
/// execstate — the dual-home `fmgr`↔`nodes` fcinfo keystone.
...
pub(crate) fn put_rows_into_sink(
    ...
    fmgr::mat_srf::with_top(|sink| {
        if let Some(sink) = sink {
            for row in rows {
                let cells: std::vec::Vec<fmgr::mat_srf::MatCell> = row
                    .into_iter()
                    .map(|c| fmgr::mat_srf::MatCell {
                        value: c.value,
                        ref_payload: c
                            .byref
                            .map(fmgr::boundary::RefPayload::Varlena),
                        isnull: c.isnull,
                    })
                    .collect();
...
pub fn case_rebuild_temp_var_datatype(
    ...
    let dt = comp_seams::plpgsql_build_datatype::call(
        typoid,
        typmod,
