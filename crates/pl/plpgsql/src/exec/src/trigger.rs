...
pub fn plpgsql_fulfill_promise_impl(
    ...
    let (word, byref, isnull): (usize, Option<Vec<u8>>, bool) = match promise {
        ...
        PLPGSQL_PROMISE_TG_EVENT => {
            ...
            let event = event_trigger_seams::event_trigger_get_event::call()?
                .ok_or_else(|| {
        ...
        PLPGSQL_PROMISE_TG_TAG => {
            ...
            let tag = event_trigger_seams::event_trigger_get_tag_name::call()?
                .ok_or_else(|| {
                    types_error::PgError::error(
                        "event trigger promise is not in an event trigger function".to_string(),
                    )
...
fn text_image(s: &str) -> Vec<u8> {
    ...
    image[..VARHDRSZ].copy_from_slice(&datum::varlena::set_varsize_4b(VARHDRSZ + bytes.len()));
