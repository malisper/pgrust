...
fn set_variable_conflict(v: int32) {
    ...
    comp::set_plpgsql_variable_conflict(opt);
}
...
fn set_print_strict_params(v: bool) {
    ...
    comp::set_plpgsql_print_strict_params(v);
}
...
pub fn plpgsql_extra_warnings_assign_hook(extra: int32) {
    ...
    comp::set_plpgsql_extra_warnings(extra);
}
...
pub fn plpgsql_extra_errors_assign_hook(extra: int32) {
    ...
    comp::set_plpgsql_extra_errors(extra);
}
...
pub fn plpgsql_call_handler(fcinfo: &mut FunctionCallInfoBaseData) -> PgResult<Datum> {
    ...
    let body = (|| -> PgResult<Datum> {
        ...
        // takes the live arena-lifetimed `nodes` fcinfo; the fmgr
        // `PGFunction` boundary carries the non-lifetimed `fmgr` fcinfo, so
...
pub fn plpgsql_inline_handler(codeblock: InlineCodeBlock) -> PgResult<()> {
    ...
    let mut func = comp::plpgsql_compile_inline(source)?;
    ...
    funcs::plpgsql_free_function_memory(&mut func)?;
...
/// built-ins.
pub fn init_seams() {
    functioncmds_seams::execute_inline_handler::set(|_laninline, codeblock| {
        plpgsql_inline_handler(codeblock)
    });
    ...
    ::plpgsql_exec_seams::convert_value_to_string::set(
        |value: usize, byref: Option<Vec<u8>>, valtype| {
            ...
            let (typoutput, typisvarlena) =
                lsyscache_seams::get_type_output_info::call(valtype)?;
            ...
            let datum = match byref {
                ...
                Some(image) => {
                    ...
                    let typlen = lsyscache_seams::get_typlen::call(valtype)?;
                    if typlen == -2 {
    ...
    ::plpgsql_exec_seams::exec_cast_value_via_spi::set(
        |value: usize, value_byref, isnull, valtype, _valtypmod, reqtype, reqtypmod| {
            ...
            const TYPTYPE_DOMAIN: u8 = b'd';
            let req_is_domain =
                lsyscache_seams::get_typtype::call(reqtype)? == TYPTYPE_DOMAIN;
            if !req_is_domain {
                let (pathtype, funcid) =
                    coerce_seams::find_coercion_pathway_plpgsql::call(
                        reqtype, valtype,
                    )?;
                match pathtype {
                    coerce_seams::CoercionPathType::Func => {
                        ...
                        let finfo =
                            fmgr_seams::fmgr_info::call(mcx, funcid)?;
                        let result = if finfo.fn_nargs >= 3 {
                            fmgr_seams::function_call3_coll_datum::call(
                                mcx,
                                funcid,
                                src_datum.clone(),
                    ...
                    coerce_seams::CoercionPathType::Relabeltype
                        if reqtypmod == -1 =>
                    {
            ...
            let (typoutput, _typisvarlena) =
                lsyscache_seams::get_type_output_info::call(valtype)?;
            let text = fmgr_seams::oid_output_function_call::call(
            ...
            let (typinput, typioparam) =
                lsyscache_seams::get_type_input_info::call(reqtype)?;
            let result = fmgr_seams::input_function_call::call(
                mcx,
                typinput,
                Some(s.as_str()),
                typioparam,
                ...
                |td: &mut ::types_tuple::heaptuple::TupleDescData<'static>|
                 -> PgResult<::plpgsql_exec_seams::RowCompositeDatum> {
                    ...
                    if td.tdtypeid == 2249 && td.tdtypmod < 0 {
                        typcache_seams::assign_record_type_typmod::call(td)?;
                    }
    ...
    ::plpgsql_exec_seams::rollback_and_release_current_subtransaction::set(|| {
        ...
        transam_xact::RollbackAndReleaseCurrentSubTransaction()
    });
...
fn foreach_iterate_via_array_impl(
    ...
    //   if (!OidIsValid(get_element_type(arrtype)))
    let elem_type =
        lsyscache_seams::get_element_type::call(arrtype)?.unwrap_or(0);
    if elem_type == 0 {
    ...
    let arr = detoast_seams::detoast_attr::call(mcx, &arr_bytes)?;
    ...
    // extraction below.
    let mstate = lsyscache_seams::get_typlenbyvalalign::call(elem_type)?;
