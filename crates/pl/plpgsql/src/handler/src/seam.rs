...
// `misc_guc::custom::*` directly with the handler's storage
...
/// non-lifetimed `fmgr` fcinfo whose `flinfo->fn_oid` names the function
...
fn call_expr_from_fcinfo(
    ...
    match flinfo.fn_expr.as_deref() {
        Some(::fmgr::fmgr::FnExpr::External(ext)) => {
            ...
            Some(funcapi::polymorphic::CallExpr::from_erased(erased))
        }
        _ => None,
    }
}
...
fn compile_proc_from_row(
    ...
    let row = syscache_seams::proc_compile_row::call(mcx, funcoid)?
        .ok_or_else(|| {
    ...
    let resolved_argtypes: Vec<Oid> = if for_validator {
        ...
        match funcapi::polymorphic::resolve_polymorphic_argtypes(
            &mut argtypes,
            argmodes_opt,
            call_expr.as_ref(),
        ) {
...
pub fn check_function_validator_access(validator_oid: Oid, funcoid: Oid) -> PgResult<bool> {
    pg_proc_seams::check_function_validator_access::call(validator_oid, funcoid)
}
...
pub fn validate_function_body(funcoid: Oid) -> PgResult<()> {
    ...
    let functyptype = lsyscache_seams::get_typtype::call(proc.prorettype)?;
    ...
    if check_function_bodies() {
        ...
        match spi::SPI_finish() {
            Ok(rc) if rc == spi::SPI_OK_FINISH => {}
