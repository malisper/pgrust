...
extern "C" fn wrapper_handler(postgres_signal_arg: i32) {
    ...
    debug_assert!(backend_utils_init_small_seams::my_proc_pid::call() != 0);

    // SAFETY: getpid() is async-signal-safe.
    if backend_utils_init_small_seams::my_proc_pid::call() != unsafe { libc::getpid() } {
...
mod tests {
    ...
    fn concrete_handler_dispatches_through_wrapper() {
        ...
        if !backend_utils_init_small_seams::my_proc_pid::is_installed() {
            backend_utils_init_small_seams::my_proc_pid::set(|| unsafe { libc::getpid() });
        }
