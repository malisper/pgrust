...
[").map(|x| x + 1).unwrap_or(after.len());
    ...
    const CONTRACT_RECONCILE_PENDING: &[(&str, &str)] = &[
        ...
        // no GIN reloptions lane). Their real owner is `ginutil` (the `GinOptions`
        // owner), but it cannot resolve them until the relcache carries the GIN
        // reloptions bytea — so they are genuinely uninstalled / loud-panic
        // (mirror-pg-and-panic). The fast-update path
