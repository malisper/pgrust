//! `enum BackendType` (`miscadmin.h`): what kind of process this backend is.
//! The discriminants match the C enum order.
...
// Auxiliary processes. These have PGPROC entries, but they are not attached
// to any particular database, and cannot run transactions or even take
// heavyweight locks. There can be only one of each of these running at a
// time, except for IO workers.
