# README-TODO: arrayfuncs_diff (crates/backend/utils/adt/arrayfuncs)
...
|---|---|---|
| `array_in` | 750 | `arrayfuncs.c` |
| `array_out` | 751 | `arrayfuncs.c` |
| `array_get_element` | 0 | `arrayfuncs.c` |
| `array_get_slice` | 0 | `arrayfuncs.c` |
| `array_set_element` | 0 | `arrayfuncs.c` |
| `array_set_slice` | 0 | `arrayfuncs.c` |
| `deconstruct_array` | 0 | `arrayfuncs.c` |
| `construct_md_array` | 0 | `arrayfuncs.c` |
| `array_contains_nulls` | 0 | `arrayfuncs.c` |
| `width_bucket_array` | 3218 | `arrayfuncs.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_arrayfuncs_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_arrayfuncs_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/arrayfuncs_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `arrayfuncs::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin arrayfuncs_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/arrayfuncs_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/arrayfuncs_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run arrayfuncs_diff` — floor for any fuzz-only claim:
