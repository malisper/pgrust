# README-TODO: arrayutils_diff (crates/backend/utils/adt/arrayutils)
...
|---|---|---|
| `ArrayGetOffset` | 0 | `arrayutils.c` |
| `ArrayGetNItems` | 0 | `arrayutils.c` |
| `ArrayGetNItemsSafe` | 0 | `arrayutils.c` |
| `ArrayCheckBounds` | 0 | `arrayutils.c` |
| `ArrayCheckBoundsSafe` | 0 | `arrayutils.c` |
| `mda_get_range` | 0 | `arrayutils.c` |
| `mda_get_prod` | 0 | `arrayutils.c` |
| `mda_get_offset_values` | 0 | `arrayutils.c` |
| `mda_next_tuple` | 0 | `arrayutils.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_arrayutils_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_arrayutils_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/arrayutils_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `arrayutils::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin arrayutils_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/arrayutils_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/arrayutils_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run arrayutils_diff` — floor for any fuzz-only claim:
