# README-TODO: fmt_dch_diff (crates/backend/utils/adt/formatting)
...
|---|---|---|
| `timestamp_to_char` | 2049 | `formatting.c` |
| `timestamptz_to_char` | 1770 | `formatting.c` |
| `interval_to_char` | 1768 | `formatting.c` |
| `to_timestamp` | 1778 | `formatting.c` |
| `to_date` | 1780 | `formatting.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_fmt_dch_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_fmt_dch_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/fmt_dch_diff.rs`)
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin fmt_dch_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/fmt_dch_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/fmt_dch_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run fmt_dch_diff` — floor for any fuzz-only claim:
