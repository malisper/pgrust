# README-TODO: fmt_num_diff (crates/backend/utils/adt/formatting)
...
|---|---|---|
| `numeric_to_char` | 1772 | `formatting.c` |
| `int4_to_char` | 1773 | `formatting.c` |
| `int8_to_char` | 1774 | `formatting.c` |
| `float4_to_char` | 1775 | `formatting.c` |
| `float8_to_char` | 1776 | `formatting.c` |
| `numeric_to_number` | 1777 | `formatting.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_fmt_num_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_fmt_num_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/fmt_num_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `adt_formatting::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin fmt_num_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/fmt_num_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/fmt_num_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run fmt_num_diff` — floor for any fuzz-only claim:
