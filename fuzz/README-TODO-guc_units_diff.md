# README-TODO: guc_units_diff (crates/backend/utils/misc/guc)
...
| `parse_int` | 0 | `guc.c` |
| `parse_real` | 0 | `guc.c` |
| `convert_to_base_unit` | 0 | `guc.c` |
| `convert_int_from_base_unit` | 0 | `guc.c` |
| `convert_real_from_base_unit` | 0 | `guc.c` |
| `get_config_unit_name` | 0 | `guc.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_guc_units_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_guc_units_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/guc_units_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `guc::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin guc_units_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/guc_units_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/guc_units_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run guc_units_diff` — floor for any fuzz-only claim:
