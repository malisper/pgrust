# README-TODO: hashfn_diff (crates/common/hashfn)
...
|---|---|---|
| `hash_bytes` | 0 | `hashfn.c` |
| `hash_bytes_extended` | 0 | `hashfn.c` |
| `hash_bytes_uint32` | 0 | `hashfn.c` |
| `hash_bytes_uint32_extended` | 0 | `hashfn.c` |
| `string_hash` | 0 | `hashfn.c` |
| `tag_hash` | 0 | `hashfn.c` |
| `uint32_hash` | 0 | `hashfn.c` |
| `hash_combine` | 0 | `hashfn.h` |
| `hash_combine64` | 0 | `hashfn.h` |
| `murmurhash32` | 0 | `hashfn.h` |
| `murmurhash64` | 0 | `hashfn.h` |
| `rotate_high_and_low_32bits` | 0 | `hashfn.h` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_hashfn_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_hashfn_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/hashfn_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `hashfn::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin hashfn_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/hashfn_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/hashfn_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run hashfn_diff` — floor for any fuzz-only claim:
