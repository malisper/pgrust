# README-TODO: json_diff (crates/backend/utils/adt/json)
...
|---|---|---|
| `json_in` | 321 | `json.c` |
| `json_typeof` | 3968 | `jsonfuncs.c` |
| `json_array_length` | 3956 | `jsonfuncs.c` |
| `json_strip_nulls` | 3261 | `jsonfuncs.c` |
| `json_validate` | 0 | `json.c` |
| `json_object_field` | 3947 | `jsonfuncs.c` |
| `json_object_field_text` | 3948 | `jsonfuncs.c` |
| `json_array_element` | 3949 | `jsonfuncs.c` |
| `json_array_element_text` | 3950 | `jsonfuncs.c` |
| `json_extract_path` | 3951 | `jsonfuncs.c` |
| `json_extract_path_text` | 3953 | `jsonfuncs.c` |
| `json_object` | 3202 | `json.c` |
| `json_object_two_arg` | 3203 | `json.c` |
| `escape_json` | 0 | `json.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_json_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_json_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/json_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `adt_json::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin json_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/json_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/json_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run json_diff` — floor for any fuzz-only claim:
