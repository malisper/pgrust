# README-TODO: oraclefam_diff (crates/backend/utils/adt/oracle_compat)
...
|---|---|---|
| `lower` | 870 | `oracle_compat.c` |
| `upper` | 871 | `oracle_compat.c` |
| `initcap` | 872 | `oracle_compat.c` |
| `lpad` | 873 | `oracle_compat.c` |
| `rpad` | 874 | `oracle_compat.c` |
| `ltrim` | 875 | `oracle_compat.c` |
| `rtrim` | 876 | `oracle_compat.c` |
| `translate` | 878 | `oracle_compat.c` |
| `btrim` | 884 | `oracle_compat.c` |
| `ascii` | 1620 | `oracle_compat.c` |
| `chr` | 1621 | `oracle_compat.c` |
| `repeat` | 1622 | `oracle_compat.c` |
| `byteatrim` | 2015 | `oracle_compat.c` |
| `text_left` | 3060 | `varlena.c` |
| `text_right` | 3061 | `varlena.c` |
| `text_reverse` | 3062 | `varlena.c` |
| `bytealtrim` | 6195 | `oracle_compat.c` |
| `byteartrim` | 6196 | `oracle_compat.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_oraclefam_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_oraclefam_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/oraclefam_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `adt_oracle_compat::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin oraclefam_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/oraclefam_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/oraclefam_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run oraclefam_diff` — floor for any fuzz-only claim:
