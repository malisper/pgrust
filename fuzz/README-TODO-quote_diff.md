# README-TODO: quote_diff (crates/backend/utils/adt/quote)
...
|---|---|---|
| `quote_ident` | 1282 | `quote.c` |
| `quote_literal` | 1283 | `quote.c` |
| `quote_nullable` | 1289 | `quote.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_quote_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_quote_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/quote_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `adt_quote::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin quote_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/quote_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/quote_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run quote_diff` — floor for any fuzz-only claim:
