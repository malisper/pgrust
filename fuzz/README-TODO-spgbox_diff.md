# README-TODO: spgbox_diff (crates/backend/access/spgist/spgist_box)
...
|---|---|---|
| `spg_bbox_quad_config` | 5010 | `geo_spgist.c` |
| `spg_poly_quad_compress` | 5011 | `geo_spgist.c` |
| `spg_box_quad_config` | 5012 | `geo_spgist.c` |
| `spg_box_quad_choose` | 5013 | `geo_spgist.c` |
| `spg_box_quad_picksplit` | 5014 | `geo_spgist.c` |
| `spg_box_quad_inner_consistent` | 5015 | `geo_spgist.c` |
| `spg_box_quad_leaf_consistent` | 5016 | `geo_spgist.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_spgbox_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_spgbox_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/spgbox_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `spgist_box::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin spgbox_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/spgbox_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/spgbox_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run spgbox_diff` — floor for any fuzz-only claim:
