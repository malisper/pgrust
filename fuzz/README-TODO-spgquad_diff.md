# README-TODO: spgquad_diff (crates/backend/access/spgist/spgist_quadtree)
...
|---|---|---|
| `spg_quad_config` | 4018 | `spgquadtreeproc.c` |
| `spg_quad_choose` | 4019 | `spgquadtreeproc.c` |
| `spg_quad_picksplit` | 4020 | `spgquadtreeproc.c` |
| `spg_quad_inner_consistent` | 4021 | `spgquadtreeproc.c` |
| `spg_quad_leaf_consistent` | 4022 | `spgquadtreeproc.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_spgquad_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_spgquad_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/spgquad_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `spgist_quadtree::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin spgquad_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/spgquad_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/spgquad_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run spgquad_diff` — floor for any fuzz-only claim:
