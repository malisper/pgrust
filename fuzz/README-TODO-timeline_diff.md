# README-TODO: timeline_diff (crates/backend/access/transam/timeline)
...
|---|---|---|
| `readTimeLineHistory` | 0 | `timeline.c` |
| `tliOfPointInHistory` | 0 | `timeline.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_timeline_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_timeline_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/timeline_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `timeline::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin timeline_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/timeline_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/timeline_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run timeline_diff` — floor for any fuzz-only claim:
