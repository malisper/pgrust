# README-TODO: timestamp_diff (crates/backend/utils/adt/adt_timestamp)
...
|---|---|---|
| `timestamp_in` | 1312 | `timestamp.c` |
| `timestamp_out` | 1313 | `timestamp.c` |
| `timestamptz_in` | 1150 | `timestamp.c` |
| `timestamptz_out` | 1151 | `timestamp.c` |
| `timestamp_recv` | 2474 | `timestamp.c` |
| `timestamptz_recv` | 2476 | `timestamp.c` |
| `timestamp_send` | 2475 | `timestamp.c` |
| `interval_in` | 1160 | `timestamp.c` |
| `interval_out` | 1161 | `timestamp.c` |
| `interval_recv` | 2478 | `timestamp.c` |
| `interval_send` | 2479 | `timestamp.c` |
| `timestamp_trunc` | 2020 | `timestamp.c` |
| `timestamptz_trunc` | 1217 | `timestamp.c` |
| `interval_trunc` | 1218 | `timestamp.c` |
| `timestamp_part` | 2021 | `timestamp.c` |
| `timestamptz_part` | 1171 | `timestamp.c` |
| `interval_part` | 1172 | `timestamp.c` |
| `extract_timestamp` | 6202 | `timestamp.c` |
| `extract_timestamptz` | 6203 | `timestamp.c` |
| `extract_interval` | 6204 | `timestamp.c` |
| `timestamp_age` | 2058 | `timestamp.c` |
| `timestamptz_age` | 1199 | `timestamp.c` |
| `make_timestamp` | 3461 | `timestamp.c` |
| `make_timestamptz` | 3462 | `timestamp.c` |
| `make_interval` | 3464 | `timestamp.c` |
| `interval_mul` | 1618 | `timestamp.c` |
| `interval_div` | 1326 | `timestamp.c` |
| `timestamp_mi` | 1188 | `timestamp.c` |
| `interval_justify_hours` | 1175 | `timestamp.c` |
| `interval_justify_days` | 1295 | `timestamp.c` |
| `interval_justify_interval` | 2711 | `timestamp.c` |
| `timestamp_bin` | 6177 | `timestamp.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_timestamp_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_timestamp_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/timestamp_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `adt_timestamp::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin timestamp_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/timestamp_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/timestamp_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run timestamp_diff` — floor for any fuzz-only claim:
