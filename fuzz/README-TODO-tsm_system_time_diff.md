# README-TODO: tsm_system_time_diff (crates/contrib/tsm_system_time)
...
|---|---|---|
| `system_time_beginsamplescan` | 0 | `tsm_system_time.c` |
| `system_time_nextsampleblock` | 0 | `tsm_system_time.c` |
| `system_time_nextsampletuple` | 0 | `tsm_system_time.c` |
| `system_time_samplescangetsamplesize` | 0 | `tsm_system_time.c` |
| `random_relative_prime` | 0 | `tsm_system_time.c` |

## 1. Vendor the C oracle (compile gate)

- [ ] Paste VERBATIM upstream C into `core/csrc/pg_tsm_system_time_io.c` at every
...
- [ ] Uncomment the `.file("csrc/pg_tsm_system_time_io.c")` line in `core/build.rs`.

## 2. Implement the Rust driver (`core/src/tsm_system_time_diff.rs`)
...
- [ ] Fill each `*_diff` arm: C oracle call, shipped-Rust core call, ALL
      ...
      `todo!()` with its arm.
- [ ] fc-wrapper plane per arm via `fc_call` / `tsm_system_time::builtins::fc_*`
...
- [ ] Uncomment the extern decls; drop `#![allow(dead_code)]`; un-ignore and
      ...
      seed-corpus replay).
- [ ] `cargo check --manifest-path fuzz/Cargo.toml --bin tsm_system_time_diff` and
      `cargo test --manifest-path fuzz/core/Cargo.toml` green on stable.

## 3. Seeds, dictionary, corpus

- [ ] Extend `fuzz/tsm_system_time_diff.dict` (CmpLog + dictionary day-one for
      parser-shaped targets; tokens from the vendored regress SQL literals).
- [ ] Seed `fuzz/corpus/tsm_system_time_diff/` (>=30 seeds; `gen_seeds.sh` pattern) and
...
- [ ] `cargo +nightly fuzz run tsm_system_time_diff` — floor for any fuzz-only claim:
