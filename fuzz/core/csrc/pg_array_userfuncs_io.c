...
/* ARRAY_ELEMENT_ERROR is the same sqlstate 2202E as ARRAY_SUBSCRIPT (PG alias) */
#define PG_DIFF_ERR_ARRAY_ELEMENT PG_DIFF_ERR_ARRAY_SUBSCRIPT
...
/*
 * The message shims must still EVALUATE their arguments: real errmsg/errdetail
 * are functions, and PG's error-message helpers (format_type_be) can
 * themselves raise. A `#define errmsg(...) 0` discards those calls and hides
 * whole error arms — it produced one false divergence before this note.
 */
static int
pg_diff_au_msg(const char *fmt,...)
{
	(void) fmt;
	return 0;
}
...
#define errmsg(...) pg_diff_au_msg(__VA_ARGS__)
#define errmsg_plural(fmt, fmtp, n, ...) pg_diff_au_msg(fmt, __VA_ARGS__)
#define errdetail(...) pg_diff_au_msg(__VA_ARGS__)
#define errhint(...) pg_diff_au_msg(__VA_ARGS__)
#define elog(elevel, ...) do { pg_diff_errcode = PG_DIFF_ERR_ELOG; pg_diff_au_error(); } while (0)
#define ERROR 21

/*
 * format_type_be: message text is out of scope, BUT the function is NOT
 * total — on a type oid with no pg_type row it elog(ERROR)s "cache lookup
 * failed for type %u" (format_type.c format_type_extended, flags 0), and
 * that error WINS over the errcode of the ereport whose errdetail called it
 * (the inner longjmp escapes first). Modelling it as a constant string hid
 * that arm and produced a false divergence against the shipped Rust, which
 * raises the cache-lookup error faithfully. Class 99 = the generic
 * internal-error class the Rust side's cache-lookup PgError maps to.
 */
static const char *
