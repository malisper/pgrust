/*
 * pg_hashfn_io.c: vendored PostgreSQL C oracle for the hashfn_diff
 * differential fuzz target (100%-coverage campaign; crate crates/common/hashfn).
 *
 * Provenance (all bodies VERBATIM unless a shim is listed below):
 *   - src/common/hashfn.c @ postgres-src
 *     62d6c7d3df6287f1bd83199c1a746e50d31571a0 (Stamp-18.3): lines 45..692
 *     byte-for-byte (UINT32_ALIGN_MASK, rot, mix, final macros; hash_bytes,
 *     hash_bytes_extended, hash_bytes_uint32, hash_bytes_uint32_extended,
 *     string_hash, tag_hash, uint32_hash). WORDS_BIGENDIAN is undefined on
 *     every supported target, so the little-endian arms compile — the same
 *     arms real PostgreSQL runs there. The vendored aligned AND unaligned
 *     word-fetch paths are both live: the driver offsets its key buffer to
 *     steer alignment, which is exactly the split the Rust crate claims is
 *     value-identical on little-endian.
 *   - src/include/common/hashfn.h @ same ref: ROTATE_HIGH_AND_LOW_32BITS,
 *     hash_combine, hash_combine64, murmurhash32, murmurhash64 — verbatim
 *     (the Datum-returning hash_any* wrappers are not vendored; the crate
 *     does not port them).
 *   - src/include/port/pg_bitutils.h @ same ref: pg_rotate_left32 verbatim.
 *
 * Shims (plumbing only, never logic):
 *   - shim postgres.h supplies uint32/uint64 typedefs, UINT64CONST, Assert
 *     (noop, NDEBUG parity).
 *   - Size = size_t; Min(x,y) -> c.h's exact ternary definition.
 *   - No ereport/palloc surface: nothing here allocates or raises;
 *     pg_diff_errcode is untouched (always 0).
 *
...
/* c.h @ 62d6c7d3df: exact definition */
#define Min(x, y)		((x) < (y) ? (x) : (y))

/* ==== src/include/port/pg_bitutils.h @ 62d6c7d3df (VERBATIM) ==== */
...
/* ========= SECTION 1: src/common/hashfn.c lines 45..692 (VERBATIM) ========= */
...
/* ==== SECTION 2: src/include/common/hashfn.h @ 62d6c7d3df (VERBATIM inlines) ==== */
...
uint32
pg_diff_hash_bytes(const unsigned char *k, int keylen)
{
	pg_diff_errcode = 0;
	return hash_bytes(k, keylen);
}

uint64
pg_diff_hash_bytes_extended(const unsigned char *k, int keylen, uint64 seed)
{
	pg_diff_errcode = 0;
	return hash_bytes_extended(k, keylen, seed);
}

uint32
pg_diff_hash_bytes_uint32(uint32 k)
{
	pg_diff_errcode = 0;
...
uint64
pg_diff_hash_bytes_uint32_extended(uint32 k, uint64 seed)
{
	pg_diff_errcode = 0;
	return hash_bytes_uint32_extended(k, seed);
}

uint32
pg_diff_string_hash(const void *key, Size keysize)
{
	pg_diff_errcode = 0;
...
uint32
pg_diff_tag_hash(const void *key, Size keysize)
{
	pg_diff_errcode = 0;
...
uint32
pg_diff_uint32_hash(uint32 k)
{
	pg_diff_errcode = 0;
	return uint32_hash(&k, sizeof(uint32));
}

uint32
pg_diff_hash_combine(uint32 a, uint32 b)
{
	pg_diff_errcode = 0;
...
uint64
pg_diff_hash_combine64(uint64 a, uint64 b)
{
	pg_diff_errcode = 0;
	return hash_combine64(a, b);
}

uint32
pg_diff_murmurhash32(uint32 h)
{
	pg_diff_errcode = 0;
...
uint64
pg_diff_rotate_high_and_low_32bits(uint64 v)
{
	pg_diff_errcode = 0;
	return ROTATE_HIGH_AND_LOW_32BITS(v);
}
