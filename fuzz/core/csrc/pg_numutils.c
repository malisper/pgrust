 ...
 *     pg_ultoa_n, pg_ltoa, pg_ulltoa_n, pg_lltoa, pg_ultostr_zeropad,
 *     pg_ultostr — verbatim.
 *   - src/include/common/int.h: pg_neg_u16_overflow, pg_neg_u32_overflow,
 *     pg_neg_u64_overflow — verbatim (HAVE__BUILTIN_OP_OVERFLOW arm; every
 *     supported compiler has __builtin_sub_overflow).
 *   - src/include/port/pg_bitutils.h: pg_leftmost_one_pos32/64 — verbatim
 *     (HAVE__BUILTIN_CLZ arm, SIZEOF_LONG == 8).
 *
 * Shims (plumbing only, never logic):
 *   - ereturn(escontext, dummy, (errcode(X), errmsg(...))) -> record X in the
 *     thread-local pg_numutils_errcode and return dummy — i.e. every error is
 *     captured "soft", exactly the observable the escontext channel carries
 *     (errcode plane); errmsg arguments are unevaluated (message text is out
 *     of the harness contract). Per-iteration reset happens in each
 *     pg_diff_numutils_* wrapper below.
 *   - Node* escontext: opaque, always NULL through the verbatim bodies (the
 *     ereturn shim above never dereferences it).
 *   - strtou64: c.h's SIZEOF_LONG == 8 arm — ((uint64) strtoul(...)) — this
 *     is an LP64-only oracle, same as the shipped CI cluster/laptop hosts.
 *
...
/* ---- c.h plumbing (LP64 arm; see header) ---- */
...
/* utils/builtins.h prototypes for this TU (the vendored file includes it) */
...
uint32		uint32in_subr(const char *s, char **endloc,
...
static _Thread_local int pg_numutils_errcode;

#define ereturn(escontext, dummy, rest) \
	do { rest; return dummy; } while (0)
#define errcode(x) (pg_numutils_errcode = (x))
#define errmsg(...) 0

/* ================================================================
 * src/include/common/int.h — VERBATIM (HAVE__BUILTIN_OP_OVERFLOW arm)
 * ================================================================ */
...
/* ================================================================
 * src/include/port/pg_bitutils.h — VERBATIM
 * (HAVE__BUILTIN_CLZ arm, SIZEOF_LONG == 8)
 * ================================================================ */
...
/* ================================================================
 * src/backend/utils/adt/numutils.c — VERBATIM from here down to the
 * pg_diff_* wrapper section
 * ================================================================ */
...
/* ================================================================
 * pg_diff_* entry wrappers (shim: FFI surface for the Rust driver)
 *
 * Every wrapper resets the thread-local errcode first (per-iteration
 * reset), calls the verbatim body, and returns the errcode plane:
 * 0 = success, 1 = invalid-syntax (22P02), 2 = out-of-range (22003).
 * ================================================================ */

int
pg_diff_numutils_strtoint16(const char *s, int16_t *out)
{
	pg_numutils_errcode = 0;
	*out = pg_strtoint16_safe(s, NULL);
	return pg_numutils_errcode;
}

int
pg_diff_numutils_strtoint32(const char *s, int32_t *out)
{
	pg_numutils_errcode = 0;
	*out = pg_strtoint32_safe(s, NULL);
	return pg_numutils_errcode;
}

int
pg_diff_numutils_strtoint64(const char *s, int64_t *out)
{
	pg_numutils_errcode = 0;
	*out = pg_strtoint64_safe(s, NULL);
	return pg_numutils_errcode;
}

/* use_endloc selects the endloc arm; *end_off = consumed prefix length */
int
pg_diff_numutils_uint32in_subr(const char *s, int use_endloc,
							   uint32_t *out, size_t *end_off)
{
	char	   *endloc = NULL;

	pg_numutils_errcode = 0;
	*out = uint32in_subr(s, use_endloc ? &endloc : NULL, "oid", NULL);
	if (pg_numutils_errcode == 0 && use_endloc)
		*end_off = (size_t) (endloc - s);
	return pg_numutils_errcode;
}

int
pg_diff_numutils_uint64in_subr(const char *s, int use_endloc,
							   uint64_t *out, size_t *end_off)
{
	char	   *endloc = NULL;

	pg_numutils_errcode = 0;
	*out = uint64in_subr(s, use_endloc ? &endloc : NULL, "oid", NULL);
	if (pg_numutils_errcode == 0 && use_endloc)
		*end_off = (size_t) (endloc - s);
	return pg_numutils_errcode;
}
...
int
pg_diff_numutils_ultoa_n(uint32_t value, char *a)
{
	return pg_ultoa_n(value, a);
}

int
pg_diff_numutils_ulltoa_n(uint64_t value, char *a)
{
	return pg_ulltoa_n(value, a);
}

int
pg_diff_numutils_ltoa(int32_t value, char *a)
{
	return pg_ltoa(value, a);
}

int
pg_diff_numutils_lltoa(int64_t value, char *a)
{
	return pg_lltoa(value, a);
}

int
pg_diff_numutils_itoa(int16_t i, char *a)
{
	return pg_itoa(i, a);
}

/* returns bytes written (C's end pointer as an offset) */
int
pg_diff_numutils_ultostr(char *str, uint32_t value)
{
	return (int) (pg_ultostr(str, value) - str);
}

int
pg_diff_numutils_ultostr_zeropad(char *str, uint32_t value, int32_t minwidth)
{
	return (int) (pg_ultostr_zeropad(str, value, minwidth) - str);
}
