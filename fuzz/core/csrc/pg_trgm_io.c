 ...
 * .file("csrc/pg_trgm_io.c") line in core/build.rs is uncommented. A
 * half-filled shim can therefore never silently build or link.
 *
 * Provenance (fill in as you paste; follow csrc/pg_uuid_io.c):
 *   - Vendor sections 1..N byte-for-byte from src/backend/utils/adt/trgm_op.c / src/backend/utils/adt/trgm_regexp.c
 ...
 *   - Functions to vendor: similarity, word_similarity, strict_word_similarity, show_trgm, generate_wildcard_trgm, createTrgmNFA.
...
/* ==================== SECTION 1: trgm_op.c (VERBATIM) ==================== */

/*
 * TODO(scaffold): paste here, byte-for-byte from
 * src/backend/utils/adt/trgm_op.c @ 62d6c7d3df6287f1bd83199c1a746e50d31571a0,
 * the bodies backing: similarity, word_similarity, strict_word_similarity, show_trgm, generate_wildcard_trgm
 ...
 */
#error "SCAFFOLD-TODO(trgm_diff): verbatim C from trgm_op.c not pasted yet"

/* ==================== SECTION 2: trgm_regexp.c (VERBATIM) ==================== */

/*
 * TODO(scaffold): paste here, byte-for-byte from
 * src/backend/utils/adt/trgm_regexp.c @ 62d6c7d3df6287f1bd83199c1a746e50d31571a0,
 * the bodies backing: createTrgmNFA
 * (rename with a pg_ prefix; unwrap fmgr wrappers; document every shim in
 * the file header above). Remove the #error line together with the paste.
 */
#error "SCAFFOLD-TODO(trgm_diff): verbatim C from trgm_regexp.c not pasted yet"
 ...
 */
/*
 * TODO(scaffold): int pg_diff_similarity(...)   [oid 0, trgm_op.c]
 * (first line of the body: pg_diff_arena_reset(); — see the arena header)
 */
#error "SCAFFOLD-TODO(trgm_diff): pg_diff_similarity driver entry not written yet"
/*
 * TODO(scaffold): int pg_diff_word_similarity(...)   [oid 0, trgm_op.c]
 * (first line of the body: pg_diff_arena_reset(); — see the arena header)
 */
#error "SCAFFOLD-TODO(trgm_diff): pg_diff_word_similarity driver entry not written yet"
/*
 * TODO(scaffold): int pg_diff_strict_word_similarity(...)   [oid 0, trgm_op.c]
 * (first line of the body: pg_diff_arena_reset(); — see the arena header)
 */
#error "SCAFFOLD-TODO(trgm_diff): pg_diff_strict_word_similarity driver entry not written yet"
/*
 * TODO(scaffold): int pg_diff_show_trgm(...)   [oid 0, trgm_op.c]
 * (first line of the body: pg_diff_arena_reset(); — see the arena header)
 */
#error "SCAFFOLD-TODO(trgm_diff): pg_diff_show_trgm driver entry not written yet"
/*
 * TODO(scaffold): int pg_diff_generate_wildcard_trgm(...)   [oid 0, trgm_op.c]
 * (first line of the body: pg_diff_arena_reset(); — see the arena header)
 */
#error "SCAFFOLD-TODO(trgm_diff): pg_diff_generate_wildcard_trgm driver entry not written yet"
/*
 * TODO(scaffold): int pg_diff_createTrgmNFA(...)   [oid 0, trgm_regexp.c]
 * (first line of the body: pg_diff_arena_reset(); — see the arena header)
 */
#error "SCAFFOLD-TODO(trgm_diff): pg_diff_createTrgmNFA driver entry not written yet"
