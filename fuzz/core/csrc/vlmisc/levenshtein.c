 ...
 * postgres-src @ REL_18_6 (PostgreSQL 18.6):
 *   src/backend/utils/adt/levenshtein.c  (entire file, byte-for-byte).
 * pg_vlmisc_io.c #includes this file TWICE (without and with
 * LEVENSHTEIN_LESS_EQUAL), exactly as src/backend/utils/adt/varlena.c
 * does, after #define-renaming varstr_levenshtein[_less_equal] with a
 * pg_vlmisc_ prefix (SYMBOL ISOLATION, see fuzz/core/build.rs note).
 */
/*-------------------------------------------------------------------------
 *
 * levenshtein.c
