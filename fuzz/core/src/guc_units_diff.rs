...
//! glibc on the CI cluster floor is the oracle of record).
...
//!     cast, UB-saturating on aarch64 exactly like Rust `as` — the CI cluster
...
mod hexfloat_underflow_witness {
    /// CI floor-2 crash-83e8055b5c: a hex-float literal with a >2^32
    ...
    /// 0) and produced a bogus 2.99e-300. The banked CI cluster input replays
    ...
    fn fleet_divergence_input_replays_clean() {
        let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/../corpus/guc_units_diff");
        let f = format!("{dir}/seed-CI-crash-83e8055b5c");
