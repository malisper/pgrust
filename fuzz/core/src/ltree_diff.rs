...
/// per divergence — enough to bury the finding and the CI cluster log. Report the
...
fn expand_text(raw: &[u8]) -> Option<Vec<u8>> {
    ...
    // OOM-killed a CI cluster campaign (job -64906) while pgrust parsed the same
...
/// UPSTREAM u16-OVERFLOW CARVE (lquery only), decoded from CI cluster job -42152.
///
...
/// SECOND, WORSE WITNESS (CI cluster job -741, ASan): `lquery_level.totallen` is
