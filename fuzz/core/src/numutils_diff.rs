...
mod tests {
    ...
    fn exhaustive_gate(name: &str) -> bool {
        if std::env::var_os("NUMUTILS_EXHAUSTIVE").is_some_and(|v| v == "1") {
            ...
            println!("{name}: SKIPPED (full 2^32 sweep; set NUMUTILS_EXHAUSTIVE=1 — CI cluster job)");
            false
        }
    }

    /// a0 EXHAUSTIVE-DIFF, CI-gated: full u32 domain through pg_ultoa_n
    ...
    /// a0 EXHAUSTIVE-DIFF, CI-gated: full u32 domain through
