...
/// false) and A <= B+EPSILON (above false) — witnessed by CI cluster floor-1
...
/// C-parity witness for the FINITE eps-boundary impossible case (CI cluster
...
mod eps_boundary_witness {
    ...
    fn floor1_divergence_input_replays_clean() {
        // The banked CI cluster input routes through the fence now.
        let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/../corpus/spgquad_diff");
        let f = format!("{dir}/seed-CI-crash-068b32635f");
