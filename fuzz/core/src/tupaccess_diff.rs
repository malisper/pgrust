...
const NMENU: usize = 12;
const MENU: [MenuEnt; NMENU] = [
...
fn natts_bucket(natts: usize) -> usize {
    match natts {
        0 => 0,
        1 => 1,
        7 => 2,
...
struct AttSpec {
    menu: u8,
    aflags: u8, // bit0 dropped, 1 notnull, 2 hasmissing, 3 null-invalid,
                // 4 !islocal, 5 ndims1, 6 typmod77, 7 coll999
    nameidx: u8, // low5 idx, bit5 uppercase
    xflags: u8, // bit0 hasdef, 1 identity, 2 generated, 3 typid+100000,
                // 4 inhcount1, 5 compression
    missing: Vec<u8>, // datum image bytes (only when hasmissing)
}

#[derive(Clone)]
struct DescSpec {
    dflags: u8, // bit0 has_constr, 1 has_not_null, 2 gen_stored, 3 gen_virtual,
                // 4 tdtypeid alt
    tdtypmod: i32,
    atts: Vec<AttSpec>,
    defvals: Vec<(u8, Vec<u8>)>,      // (adnum, adbin) adnum strictly increasing
    checks: Vec<(u8, Vec<u8>, Vec<u8>)>, // (cflags, ccname, ccbin) sorted by name
}
...
fn decode_att(cur: &mut Cursor<'_>, has_constr: bool) -> AttSpec {
...
fn decode_desc(cur: &mut Cursor<'_>) -> DescSpec {
...
fn spec_wire(s: &DescSpec) -> Vec<u8> {
...
fn build_rust_desc<'m>(mcx: Mcx<'m>, s: &DescSpec) -> TupleDescData<'m> {
    let natts = s.natts();
...
fn decode_values(cur: &mut Cursor<'_>, s: &DescSpec, nvals: usize) -> Vec<Option<Vec<u8>>> {
...
fn values_wire(vals: &[Option<Vec<u8>>]) -> Vec<u8> {
    let mut w = Vec::new();
    for v in vals {
        match v {
            None => w.push(1),
