#!/usr/bin/env bash
# ld8-handverify: apply a deck to BOTH engines (A = in-lane cpg-ref
...
# Usage: fuzz/ld8-handverify.sh <deck.sql> <outdir>
set -u
REPO="$(cd "$(dirname "$0")/.." && pwd)"
DECK="${1:?deck}"; OUT="${2:?outdir}"
PGBIN="$REPO/cpg-ref/install/bin"
PGRUST_BIN="${PGRUST_BIN:-$REPO/target-ld8-b/debug/postgres}"
PORT_A="${LD8_PORT_A:-55971}"
PORT_B="${LD8_PORT_B:-55972}"
WORK="$(mktemp -d /tmp/ld8.XXXX)"
mkdir -p "$OUT"
