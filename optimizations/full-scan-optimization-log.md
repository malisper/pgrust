...
7. **Arena only helps if you can avoid converting back.** A flat arena that must split into per-row Vecs at the API boundary is worse than just using a faster allocator.
