...
> the CI-run regress corpus — but only 1 of the 1,538 (`interval_div`, via one
...
- Run evidence: repeated CI cluster jobs with S3 artifacts ("pg_regress ALL: 230/230
  ok", `notes/batchemit-lane.md:105`,
  `scripts/lane-gates.floors.CI-pod-pgdg:26-30`); latest dated pass ~2026-07-21.
...
`.github/`) — gates fire only when a human submits a CI cluster job; standing
...
cross-replay. The per-train leg manifest lives in the reference CI cluster repo — from this
...
an author box, not a recorded CI cluster gate. No coverage guidance. Known holes:
...
post-merge with a dated CI-job ledger (`notes/loom-exhaustive-ledger.md`). Sync
...
Regress and the e2e CI cluster remain the smoke/regression rail and the oracle plumbing
