...
// ===========================================================================
// brin_minmax_multi summary_send lane (w2-brin 2026-07-30; ledger 4641)
// ===========================================================================
//
// Rust side (shipped): brin_minmax_multi/src/builtins.rs fc_summary_send —
// its whole body delegates to varlena::builtins::fc_byteasend, mirroring the
// C body `return byteasend(fcinfo);`. The theorem is WRAPPER-LEVEL through
// the public MINMAX_MULTI_BUILTINS table (oid 4641): detoast of the 4B input
// image, the byteasend delegation, and result-image header/payload pack are
// all in-theorem.
//
// C side: c/pg_brin_multi_send.c (byteasend == identity copy of the
// detoasted payload; see its header). SEPARATE c-lib (mbconv law) — send
// harnesses link ONLY that file.
//
// Cells are LITERAL-length (derived-length-copy law; bytea-cmp byteasend
// precedent: eq_byteasend_l0/l8, DEFAULT solver, mcx-stubs). Content fully
// symbolic. Claim: result payload byte-parity + varlena size parity, modulo
...
mod send_proofs {
    ...
    extern "C" {
        fn pg_mm_summary_send(d: *const u8, len: c_int, out: *mut u8) -> c_int;
        // negative control only — NOT postgres code
        fn pg_mm_summary_send_wrong(d: *const u8, len: c_int, out: *mut u8) -> c_int;
    }

    /// Compile-time oid -> shipped fc_* lookup (same pattern as dist_proofs).
    const fn builtin(oid: u32) -> PGFunction {
        let t = brin_minmax_multi::MINMAX_MULTI_BUILTINS;
        let mut i = 0;
        while i < t.len() {
    ...
    const FC_SUMMARY_SEND: PGFunction = builtin(4641);
    ...
    macro_rules! send_cell {
        ($h:ident, $w:literal, $cfn:ident, $expect_note:literal) => {
            #[kani::proof]
            #[kani::unwind(14)]
            #[kani::stub(mcx::Mcx::allocate, mcx_stubs::stub_mcx_allocate)]
            #[kani::stub(mcx::Mcx::grow, mcx_stubs::stub_mcx_grow)]
            ...
            fn $h() {
                ...
                let ctx = mcx::MemoryContext::new_bump("kani-mm-send");
    ...
    send_cell!(eq_summary_send_l0, 0, pg_mm_summary_send, "empty payload");
    send_cell!(eq_summary_send_l8, 8, pg_mm_summary_send, "8-byte symbolic payload");

    // negative control (MUST FAIL, DEFAULT solver): C drops/flips the last
    // payload byte — decodable counterexample at any payload (rig
    // non-vacuity for the send lane).
    send_cell!(control_summary_send_lastbyte, 8, pg_mm_summary_send_wrong, "must-fail");
}
