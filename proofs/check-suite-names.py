...
resolves, kani list failure, missing native bin).
"""
...
def main():
    ...
    errors, warnings, checked, skipped = [], [], 0, 0
    listed = {}
    for fam in families:
        try:
            listed[fam] = kani_list(fam, cache_dir)
    ...
    for lineno, parts in rows:
        ...
        if names is None:
            continue  # family list failed; already an error
    ...
    for e in errors:
        print(f"ERROR {e}")
    print(f"\ncheck-suite-names: {checked} rows OK, {skipped} skipped "
          f"(non-kani script rows), {len(warnings)} warnings, "
          f"{len(errors)} errors")
