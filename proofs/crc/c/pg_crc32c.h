 ...
 *   https://raw.githubusercontent.com/postgres/postgres/REL_18_STABLE/src/include/port/pg_crc32c.h
 ...
 *
 * SHIMS (plumbing only):
 *   - #include "port/pg_bswap.h" removed (pg_bswap32 is referenced only in
 *     the WORDS_BIGENDIAN FIN_CRC32C arm, which is pinned off -- see below).
 *   - None of USE_SSE42_CRC32C / USE_SSE42_CRC32C_WITH_RUNTIME_CHECK /
 *     USE_ARMV8_CRC32C / USE_ARMV8_CRC32C_WITH_RUNTIME_CHECK /
 *     USE_LOONGARCH_CRC32C is defined, so the preprocessor selects the final
 *     "slicing-by-8" branch: COMP_CRC32C == pg_comp_crc32c_sb8. This pins
 *     the C reference to the same software kernel the Rust theorem uses
 *     (the hardware-CRC arms are out of scope: core::arch intrinsics are
 *     not provable under Kani).
 *   - WORDS_BIGENDIAN not defined: little-endian FIN_CRC32C arm.
 */
/*-------------------------------------------------------------------------
 *
 * pg_crc32c.h
...
/* #include "port/pg_bswap.h" -- shimmed out, see provenance header */
