 ...
 *   https://raw.githubusercontent.com/postgres/postgres/REL_18_STABLE/src/port/pg_crc32c_sb8.c
 ...
 *   of California)
 *
 * Contains the COMPLETE file: pg_comp_crc32c_sb8 and the full 8x256
 * pg_crc32c_table -- bodies and table literals unmodified.
 *
 * SHIMS (plumbing only, no logic changed):
 *   - #include "c.h"              -> ../../support/c/pg_proof_shim.h
 *     (scalar typedefs uint8/uint32 etc.)
 *   - #include "port/pg_crc32c.h" -> "pg_crc32c.h" (vendored alongside)
 *   - WORDS_BIGENDIAN is NOT defined: pins the little-endian arms of the
 *     #ifdef WORDS_BIGENDIAN conditionals (CRC8 macro, the c0..c7 byte
 *     extraction, and the LE table halves). Both proof target platforms
 *     (aarch64-apple-darwin, aarch64/x86_64-linux) are little-endian, as is
 *     the shipped Rust port (crates/port/crc32c compile_errors on BE).
 */
/*-------------------------------------------------------------------------
 *
 * pg_crc32c_sb8.c
