...
// ===========================================================================
// WAVE W3 (2026-07-30): macaddr_send (pg_proc oid 2495) — wire row.
//
// WRAPPER-level per the eq_pg_lsn_send / eq_uuid_send precedent: the theorem
// invokes the SHIPPED fmgr entry point adt_mac::builtins::fc_macaddr_send on
// a real LocalFcinfo frame over a real result-mcx (proof_support mcx-stubs
// recipe; theorem "modulo static-buffer allocator model"), so the arg_fixed
// datum unwrap, macaddr_send core (pq_begintypsend + 6x pq_sendbyte +
// pq_endtypsend) and varlena_result packing are all inside the claim.  The
// full 10-byte wire image (4B LE varlena header = 10<<2, then the 6 payload
// bytes) is compared against vendored REL_18_STABLE C
// (csrc/mac_shim.c pgc_macaddr_send; pq_* shimmed to a caller out-buffer).
// control_macaddr_send_skew (C fed payload byte 0 ^ 1) MUST FAIL — run it
...
mod w3_send {
    ...
    extern "C" {
        fn pgc_macaddr_send(bin: *const u8, out: *mut u8) -> i32;
    }

    #[kani::proof]
    #[kani::unwind(16)] // send/copy loops <= 12 iterations
    ...
    fn eq_macaddr_send() {
        let b: [u8; 6] = kani::any();
        let mut cbuf = [0u8; 10];
        let clen = unsafe { pgc_macaddr_send(b.as_ptr(), cbuf.as_mut_ptr()) };

        let ctx = mcx::MemoryContext::new_bump("kani-mac-send");
        ...
        let d = match adt_mac::builtins::fc_macaddr_send(None, &mut f) {
            Ok(d) => d,
            Err(e) => {
                core::mem::forget(e);
                panic!("macaddr_send errored")
            }
        };
    ...
    #[kani::stub(std::sync::OnceLock::get_or_init, stubs::stub_once_lock_get_or_init)]
    #[kani::stub(std::fmt::format, stubs::stub_format)]
    fn control_macaddr_send_skew() {
        let b: [u8; 6] = kani::any();
        let mut skew = b;
        skew[0] ^= 1;
        let mut cbuf = [0u8; 10];
        let _ = unsafe { pgc_macaddr_send(skew.as_ptr(), cbuf.as_mut_ptr()) };

        let ctx = mcx::MemoryContext::new_bump("kani-mac-send-ctl");
        let mut f = LocalFcinfo::<1>::new(0);
