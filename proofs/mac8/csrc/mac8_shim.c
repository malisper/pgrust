...
/* =======================================================================
 * macaddr8_send (pg_proc oid 3447) — REL_18_STABLE src/backend/utils/adt/
 * mac8.c, fetched 2026-07-30.
 *
 * SHIMS for this section (wire plumbing only, the pg_uuid_send pattern —
 * see proofs/uuid/c/pg_uuid.c shim U1 and the identical section in
 * proofs/mac/csrc/mac_shim.c; the body's pq_* call sequence is verbatim):
 *  - fmgr unwrapping: PG_GETARG_MACADDR8_P -> locally-built pgc_macaddr8
 *    from the 8 input bytes; PG_RETURN_BYTEA_P(pq_endtypsend(&buf)) -> the
 *    image is written into a caller-provided out[12] buffer and the total
 *    varlena length (12) is returned.
...
/* macaddr8_send body verbatim under the pq_* shims; out must hold 12 bytes.
 * Returns the total varlena length (always 12). */
int
pgc_macaddr8_send(const unsigned char *bin, unsigned char *out /* [12] */ )
{
	pgc_macaddr8 addr_ = pgc_from_bytes(bin);
	pgc_macaddr8 *addr = &addr_;
	pgc_typsend_buf buf;
	...
	pgc_pq_sendbyte(&buf, addr->g);
	pgc_pq_sendbyte(&buf, addr->h);
