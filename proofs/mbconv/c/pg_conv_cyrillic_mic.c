/*
 * pg_conv_cyrillic_mic.c — vendored from cyrillic_and_mic/cyrillic_and_mic.c
 * (fetched 2026-07-28 from https://raw.githubusercontent.com/postgres/
 * postgres/REL_18_STABLE/src/backend/utils/mb/conversion_procs).
 * Tables VERBATIM; per-proc shims generated by gen_shim.awk (fmgr unwrap ->
 * scalar args, CHECK_ENCODING_CONVERSION_ARGS dropped — covered by the
 * eq_check_encoding_conversion_args harness; engine call statements
 * verbatim). Error paths: PROOF_EREPORT_FLAG convention, see pg_mbconv.h.
 */
#include "pg_mbconv.h"

/*
 * Cyrillic support
 * currently supported Cyrillic encodings:
 *
 * KOI8-R (this is also the charset for the mule internal code for Cyrillic)
 * ISO-8859-5
 * Microsoft's CP1251 (windows-1251)
 * Alternativny Variant (MS-DOS CP866)
 */

/* ISO-8859-5 to KOI8-R */
static const unsigned char iso2koi[] = {
	
};

/* KOI8-R to ISO-8859-5 */
static const unsigned char koi2iso[] = {
	
};

/* WIN1251 to KOI8-R */
static const unsigned char win12512koi[] = {
	
};

/* KOI8-R to WIN1251 */
static const unsigned char koi2win1251[] = {
	
};

/* WIN866 to KOI8-R */
static const unsigned char win8662koi[] = {
	
};

/* KOI8-R to WIN866 */
static const unsigned char koi2win866[] = {
	
};

/* WIN866 to WIN1251 */
static const unsigned char win8662win1251[] = {
	
};

/* WIN1251 to WIN866 */
static const unsigned char win12512win866[] = {
	
};

/* ISO-8859-5 to WIN1251 */
static const unsigned char iso2win1251[] = {
	
};

/* WIN1251 to ISO-8859-5 */
static const unsigned char win12512iso[] = {
	
};

/* ISO-8859-5 to WIN866 */
static const unsigned char iso2win866[] = {
	
};

/* WIN866 to ISO-8859-5 */
static const unsigned char win8662iso[] = {
	
};


int
pg_koi8r_to_mic(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = latin2mic(src, dest, len, LC_KOI8_R, PG_KOI8R, noError);
	return converted;
}

int
pg_mic_to_koi8r(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = mic2latin(src, dest, len, LC_KOI8_R, PG_KOI8R, noError);
	return converted;
}

int
pg_iso_to_mic(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = latin2mic_with_table(src, dest, len, LC_KOI8_R, PG_ISO_8859_5, iso2koi, noError);
	return converted;
}

int
pg_mic_to_iso(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = mic2latin_with_table(src, dest, len, LC_KOI8_R, PG_ISO_8859_5, koi2iso, noError);
	return converted;
}

int
pg_win1251_to_mic(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = latin2mic_with_table(src, dest, len, LC_KOI8_R, PG_WIN1251, win12512koi, noError);
	return converted;
}

int
pg_mic_to_win1251(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = mic2latin_with_table(src, dest, len, LC_KOI8_R, PG_WIN1251, koi2win1251, noError);
	return converted;
}

int
pg_win866_to_mic(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = latin2mic_with_table(src, dest, len, LC_KOI8_R, PG_WIN866, win8662koi, noError);
	return converted;
}

int
pg_mic_to_win866(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = mic2latin_with_table(src, dest, len, LC_KOI8_R, PG_WIN866, koi2win866, noError);
	return converted;
}

int
pg_koi8r_to_win1251(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_KOI8R, PG_WIN1251, koi2win1251, noError);
	return converted;
}

int
pg_win1251_to_koi8r(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_WIN1251, PG_KOI8R, win12512koi, noError);
	return converted;
}

int
pg_koi8r_to_win866(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_KOI8R, PG_WIN866, koi2win866, noError);
	return converted;
}

int
pg_win866_to_koi8r(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_WIN866, PG_KOI8R, win8662koi, noError);
	return converted;
}

int
pg_win866_to_win1251(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_WIN866, PG_WIN1251, win8662win1251, noError);
	return converted;
}

int
pg_win1251_to_win866(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_WIN1251, PG_WIN866, win12512win866, noError);
	return converted;
}

int
pg_iso_to_koi8r(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_ISO_8859_5, PG_KOI8R, iso2koi, noError);
	return converted;
}

int
pg_koi8r_to_iso(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_KOI8R, PG_ISO_8859_5, koi2iso, noError);
	return converted;
}

int
pg_iso_to_win1251(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_ISO_8859_5, PG_WIN1251, iso2win1251, noError);
	return converted;
}

int
pg_win1251_to_iso(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_WIN1251, PG_ISO_8859_5, win12512iso, noError);
	return converted;
}

int
pg_iso_to_win866(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_ISO_8859_5, PG_WIN866, iso2win866, noError);
	return converted;
}

int
pg_win866_to_iso(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_WIN866, PG_ISO_8859_5, win8662iso, noError);
	return converted;
}

