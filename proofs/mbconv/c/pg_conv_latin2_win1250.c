/*
 * pg_conv_latin2_win1250.c — vendored from latin2_and_win1250/latin2_and_win1250.c
 * (fetched 2026-07-28 from https://raw.githubusercontent.com/postgres/postgres/REL_18_STABLE/src/backend/utils/mb/conversion_procs). Tables/helpers VERBATIM; shims per gen_shim.awk conventions (see pg_conv_cyrillic_mic.c header); error paths per PROOF_EREPORT_FLAG (pg_mbconv.h).
 */
#include "pg_mbconv.h"

/* WIN1250 to ISO-8859-2 */
static const unsigned char win1250_2_iso88592[] = {
	
};

/* ISO-8859-2 to WIN1250 */
static const unsigned char iso88592_2_win1250[] = {
	
};

int
pg_latin2_to_mic(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = latin2mic(src, dest, len, LC_ISO8859_2, PG_LATIN2, noError);
	return converted;
}

int
pg_mic_to_latin2(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = mic2latin(src, dest, len, LC_ISO8859_2, PG_LATIN2, noError);
	return converted;
}

int
pg_win1250_to_mic(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = latin2mic_with_table(src, dest, len, LC_ISO8859_2, PG_WIN1250,
									 win1250_2_iso88592, noError);
	return converted;
}

int
pg_mic_to_win1250(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = mic2latin_with_table(src, dest, len, LC_ISO8859_2, PG_WIN1250,
									 iso88592_2_win1250, noError);
	return converted;
}

int
pg_latin2_to_win1250(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_LATIN2, PG_WIN1250,
							iso88592_2_win1250, noError);
	return converted;
}

int
pg_win1250_to_latin2(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;

	pg_mbconv_err = 0;
	converted = local2local(src, dest, len, PG_WIN1250, PG_LATIN2,
							win1250_2_iso88592, noError);
	return converted;
}

