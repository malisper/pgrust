/*
 * pg_conv_utf8_euc_jp.c — vendored from utf8_and_euc_jp.c + map files
...
#include "orig/maps/euc_jp_to_utf8.map"
#include "orig/maps/utf8_to_euc_jp.map"

int
pg_euc_jp_to_utf8(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;
...
int
pg_utf8_to_euc_jp(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;
