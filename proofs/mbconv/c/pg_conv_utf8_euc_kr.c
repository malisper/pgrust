/*
 * pg_conv_utf8_euc_kr.c — vendored from utf8_and_euc_kr.c + map files
...
#include "orig/maps/euc_kr_to_utf8.map"
#include "orig/maps/utf8_to_euc_kr.map"

int
pg_euc_kr_to_utf8(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;
...
int
pg_utf8_to_euc_kr(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;
