/*
 * pg_conv_utf8_euc_tw.c — vendored from utf8_and_euc_tw.c + map files
...
#include "orig/maps/euc_tw_to_utf8.map"
#include "orig/maps/utf8_to_euc_tw.map"

int
pg_euc_tw_to_utf8(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;
...
int
pg_utf8_to_euc_tw(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;
