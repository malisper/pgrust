/*
 * pg_conv_utf8_gbk.c — vendored from utf8_and_gbk.c + map files
...
#include "orig/maps/gbk_to_utf8.map"
#include "orig/maps/utf8_to_gbk.map"

int
pg_gbk_to_utf8(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;
...
int
pg_utf8_to_gbk(const unsigned char *src, unsigned char *dest, int len, bool noError)
{
	int			converted;
