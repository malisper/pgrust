...
# pgrust overlay (applied over the pristine vendored copy by the TAP runner;
# the vendor file is never edited): the "slot terminated while the walsender
# is active" arm freezes the walsender and walreceiver with SIGSTOP so the
# slot stays active while max_slot_wal_keep_size is exceeded, then expects
# the checkpointer to terminate the walsender process. In pgrust the
# walsender and walreceiver are threads of their server processes and their
# pids are virtual: an OS SIGSTOP cannot reach one (Perl's kill would land
# on an unrelated process or fail with ESRCH), so the scenario has no
# analogue. The four checks of that arm are skipped; everything before and
# after (the wal_status transitions, checkpoint blocking, inactive_since)
# runs unchanged.
note "pgrust: skipping the SIGSTOP-the-walsender arm (walsenders are threads with virtual pids)";
