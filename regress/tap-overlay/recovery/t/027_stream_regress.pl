# pgrust overlay of src/test/recovery/t/027_stream_regress.pl (applied over the
# pristine vendored copy by the TAP runner; the vendor file is never edited).
# Delta vs upstream: the C-parity parallel-planner pin block after
# synchronize_seqscans — nothing else.
...
# pgrust overlay: pgrust SHIPS re-tuned parallel-planner defaults (setup 100 /
# tuple 0.01 / table 1MB / index 64kB / per-gather 4). The regress corpus's
# EXPLAIN expectations assume C's values, so pin them exactly as the internal
# pg_regress sweep does (scripts/lib/configprofiles.sh c-parity-parallel); every
# value IS the C default, a no-op on the reference engine.
