...
// :HACK: root compatibility shim while GiST multirange support lives in `pgrust_access`.
use pgrust_access::gist::support::multirange_ops as access_multirange_ops;
...
pub(crate) fn consistent(
    ...
    catalog_result(access_multirange_ops::consistent(
        strategy,
        key,
        query,
        is_leaf,
...
pub(crate) fn union(values: &[Value]) -> Result<Value, CatalogError> {
    catalog_result(access_multirange_ops::union(values, &RootAccessServices))
}

pub(crate) fn penalty(original: &Value, candidate: &Value) -> Result<f32, CatalogError> {
    catalog_result(access_multirange_ops::penalty(
        original,
        candidate,
        &RootAccessServices,
    ))
...
pub(crate) fn picksplit(values: &[Value]) -> Result<GistColumnPickSplit, CatalogError> {
    catalog_result(access_multirange_ops::picksplit(
        values,
        &RootAccessServices,
    ))
}
...
pub(crate) fn same(left: &Value, right: &Value) -> Result<bool, CatalogError> {
    catalog_result(access_multirange_ops::same(
        left,
        right,
        &RootAccessServices,
    ))
...
pub(crate) fn sort_compare(left: &Value, right: &Value) -> Ordering {
    access_multirange_ops::sort_compare(left, right, &RootAccessServices)
}
