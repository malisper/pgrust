...
use crate::include::access::spgist::{F_LEAF, spgist_page_append_tuple, spgist_page_init};
...
pub(crate) fn spginsert(ctx: &IndexInsertContext) -> Result<bool, CatalogError> {
    ...
    let needs_new_page = match spgist_page_append_tuple(&mut page, &tuple) {
        Ok(_) => false,
        Err(crate::include::access::spgist::SpgistPageError::Page(PageError::NoSpace)) => true,
        Err(err) => {
    ...
    let mut new_page = [0u8; crate::backend::storage::smgr::BLCKSZ];
    spgist_page_init(&mut new_page, F_LEAF)
        .map_err(|err| CatalogError::Io(format!("spgist new-page init failed: {err:?}")))?;
    spgist_page_append_tuple(&mut new_page, &tuple)
        .map_err(|err| CatalogError::Io(format!("spgist new-page append failed: {err:?}")))?;
    write_logged_pages(
        &ctx.pool,
        ctx.client_id,
        ctx.snapshot.current_xid,
