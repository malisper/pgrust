...
impl CatalogStore {
    ...
    pub fn set_sequence_owned_by_dependency_mvcc(
        &mut self,
        sequence_oid: u32,
        ...
        let sequence = class_row_by_oid_mvcc(self, ctx, sequence_oid)?
            .ok_or_else(|| CatalogError::UnknownTable(sequence_oid.to_string()))?;
        if sequence.relkind != 'S' {
            return Err(CatalogError::UnknownTable(sequence_oid.to_string()));
        }
        if let Some((relation_oid, attnum)) = owned_by {
            let relation = class_row_by_oid_mvcc(self, ctx, relation_oid)?
                .ok_or_else(|| CatalogError::UnknownTable(relation_oid.to_string()))?;
            if !matches!(relation.relkind, 'r' | 'p' | 'f') || attnum <= 0 {
                return Err(CatalogError::UnknownTable(relation_oid.to_string()));
            }
        }

        let mut old_rows =
            depend_rows_for_object_mvcc(self, ctx, PG_CLASS_RELATION_OID, sequence_oid)?
                .into_iter()
                .filter(|row| {
        ...
        let mut new_rows = owned_by
            .map(|(relation_oid, attnum)| {
                vec![sequence_owned_by_depend_row(
                    sequence_oid,
                    relation_oid,
                    attnum,
                    deptype,
            ...
            .unwrap_or_default();
        sort_pg_depend_rows(&mut new_rows);
        new_rows.dedup();

        let kinds = [BootstrapCatalogKind::PgDepend];
        ...
        let mut effect = CatalogMutationEffect::default();
        effect_record_catalog_kinds(&mut effect, &kinds);
        effect_record_oid(&mut effect.relation_oids, sequence_oid);
        if let Some((relation_oid, _)) = owned_by {
    ...
    pub fn create_namespace_mvcc(
        ...
        if !temp_namespace {
            rows.shdepends.extend(owner_shdepend_row(
                self.scope_db_oid(),
            ...
            kinds.push(BootstrapCatalogKind::PgShdepend);
        }
        insert_catalog_rows_subset_mvcc(ctx, &rows, self.scope_db_oid(), &kinds)?;
    ...
    pub fn create_collation_mvcc(
        &mut self,
        mut row: PgCollationRow,
        ctx: &CatalogWriteContext,
    ) -> Result<(u32, CatalogMutationEffect), CatalogError> {
        row.oid = self.allocate_next_oid(row.oid)?;
        let kinds = [
        ...
        let rows = PhysicalCatalogRows {
            collations: vec![row.clone()],
            depends: collation_depend_rows(&row),
            ..PhysicalCatalogRows::default()
        };
        insert_catalog_rows_subset_mvcc(ctx, &rows, self.scope_db_oid(), &kinds)?;
    ...
    pub fn create_ts_config_with_maps_mvcc(
        ...
        let mut depends = ts_config_depend_rows(&row);
        depends.extend(ts_config_map_depend_rows(row.oid, &map_rows));
        sort_pg_depend_rows(&mut depends);
        let rows = PhysicalCatalogRows {
            ts_configs: vec![row.clone()],
    ...
    pub fn replace_foreign_table_mvcc(
        &mut self,
        old_row: &PgForeignTableRow,
        row: PgForeignTableRow,
        ctx: &CatalogWriteContext,
    ) -> Result<CatalogMutationEffect, CatalogError> {
        let kinds = [BootstrapCatalogKind::PgForeignTable];
        let old_rows = PhysicalCatalogRows {
            foreign_tables: vec![old_row.clone()],
            ..PhysicalCatalogRows::default()
        };
        delete_catalog_rows_subset_mvcc(ctx, &old_rows, self.scope_db_oid(), &kinds)?;
    ...
    pub fn drop_proc_by_oid_mvcc(
        ...
        let rows = PhysicalCatalogRows {
            ...
            depends: depend_rows_for_object_mvcc(self, ctx, PG_PROC_RELATION_OID, proc_row.oid)?,
            shdepends: owner_shdepend_row(
                self.scope_db_oid(),
                PG_PROC_RELATION_OID,
                proc_row.oid,
    ...
    pub fn drop_access_method_mvcc(
        &mut self,
        row: PgAmRow,
        ctx: &CatalogWriteContext,
    ) -> Result<CatalogMutationEffect, CatalogError> {
        let kinds = [BootstrapCatalogKind::PgAm, BootstrapCatalogKind::PgDepend];
        let rows = PhysicalCatalogRows {
            ams: vec![row.clone()],
            depends: access_method_depend_rows(&row),
            ..PhysicalCatalogRows::default()
        };
        delete_catalog_rows_subset_mvcc(ctx, &rows, self.scope_db_oid(), &kinds)?;
    ...
    pub fn create_triggers_mvcc(
        &mut self,
        mut rows: Vec<crate::include::catalog::PgTriggerRow>,
        ctx: &CatalogWriteContext,
    ) -> Result<(Vec<u32>, CatalogMutationEffect), CatalogError> {
        if rows.is_empty() {
            return Ok((Vec::new(), CatalogMutationEffect::default()));
        }

        let mut relation_oids = rows.iter().map(|row| row.tgrelid).collect::<Vec<_>>();
        relation_oids.sort_unstable();
        relation_oids.dedup();

        let mut existing_names = BTreeSet::new();
        let mut old_classes = BTreeMap::new();
        for relation_oid in &relation_oids {
            for existing in relation_triggers_mvcc(self, ctx, *relation_oid)? {
                existing_names.insert((*relation_oid, existing.tgname.to_ascii_lowercase()));
            }
            let old_class = class_row_by_oid_mvcc(self, ctx, *relation_oid)?
                .ok_or_else(|| CatalogError::UnknownTable(relation_oid.to_string()))?;
            old_classes.insert(*relation_oid, old_class);
        }

        let mut incoming_names = BTreeSet::new();
        for row in &rows {
            let name_key = (row.tgrelid, row.tgname.to_ascii_lowercase());
            if existing_names.contains(&name_key) || !incoming_names.insert(name_key) {
                return Err(CatalogError::UniqueViolation(
                    "pg_trigger_tgrelid_tgname_index".into(),
                ));
            }
        ...
        let mut control = self.control_state()?;
        let mut trigger_oids = Vec::with_capacity(rows.len());
        for row in &mut rows {
            if row.oid == 0 {
                row.oid = control.next_oid;
            }
            control.next_oid = control.next_oid.max(row.oid.saturating_add(1));
            trigger_oids.push(row.oid);
        }
        self.persist_control_values(control.next_oid, control.next_rel_number)?;
        ...
        for old_class in old_classes.values() {
            if !old_class.relhastriggers {
    ...
    pub fn replace_ts_config_maps_mvcc(
        ...
        let config_oid = old_rows
            .first()
            .map(|row| row.mapcfg)
            .or_else(|| new_rows.first().map(|row| row.mapcfg))
            .unwrap_or(0);
        let old_depends = ts_config_map_depend_rows(config_oid, &old_rows);
        let new_depends = ts_config_map_depend_rows(config_oid, &new_rows);
        let kinds = [
            BootstrapCatalogKind::PgTsConfigMap,
            BootstrapCatalogKind::PgDepend,
        ];
    ...
    pub fn replace_publication_row_mvcc(
        ...
        let old_rows = PhysicalCatalogRows {
            publications: vec![existing.clone()],
            shdepends: owner_shdepend_row(
                self.scope_db_oid(),
                PG_PUBLICATION_RELATION_OID,
                existing.oid,
    ...
    pub fn alter_table_set_column_fdw_options_mvcc(
        &mut self,
        relation_oid: u32,
        column_name: &str,
        ...
        let (_old_entry, new_entry, _, kinds) =
            mutate_visible_relation_entry_mvcc(self, relation_oid, ctx, move |entry, _control| {
                ...
                let column_index = relation_column_index_visible(&entry.desc, column_name)?;
                entry.desc.columns[column_index].fdw_options = fdw_options;
                Ok(((), vec![BootstrapCatalogKind::PgAttribute]))
            })?;
    ...
    pub fn rename_relation_mvcc(
        ...
        let entry = catalog_entry_by_oid_mvcc(self, ctx, relation_oid)?;
        if !matches!(entry.relkind, 'r' | 'p' | 'S' | 'i' | 'I' | 'v' | 'f') {
            return Err(CatalogError::UnknownTable(relation_oid.to_string()));
        }
        if self
    ...
    pub fn set_relation_is_partition_mvcc(
        ...
        let old_class = class_row_by_oid_mvcc(self, ctx, relation_oid)?
            .ok_or_else(|| CatalogError::UnknownTable(relation_oid.to_string()))?;
        if old_class.relispartition == relispartition {
            return Ok(CatalogMutationEffect::default());
        }
        let new_class = PgClassRow {
            relispartition,
    ...
    pub fn alter_namespace_owner_mvcc(
        ...
        let new_row = PgNamespaceRow {
            nspowner: new_owner_oid,
            ..existing_row
        };
        let new_rows = PhysicalCatalogRows {
            namespaces: vec![new_row.clone()],
            shdepends: owner_shdepend_row(
                self.scope_db_oid(),
                PG_NAMESPACE_RELATION_OID,
                new_row.oid,
    ...
    pub fn alter_namespace_rename_mvcc(
        &mut self,
        namespace_oid: u32,
        new_name: &str,
        ctx: &CatalogWriteContext,
...
fn type_row_by_oid_mvcc_with_extra_type_rows(
    store: &CatalogStore,
    ctx: &CatalogWriteContext,
    type_oid: u32,
    extra_type_rows: &[PgTypeRow],
) -> Result<Option<PgTypeRow>, CatalogError> {
    if let Some(row) = type_row_by_oid_mvcc(store, ctx, type_oid)? {
        return Ok(Some(row));
    }
    Ok(extra_type_rows
        .iter()
...
fn catalog_index_meta_from_index_row_for_drop(
    index: crate::include::catalog::PgIndexRow,
) -> CatalogIndexMeta {
    CatalogIndexMeta {
        indrelid: index.indrelid,
        indkey: index.indkey,
        ...
        indisready: index.indisready,
        indislive: index.indislive,
        indclass_options: vec![Vec::new(); index.indclass.len()],
...
fn relation_desc_from_catalog_rows_mvcc(
    store: &CatalogStore,
    ctx: &CatalogWriteContext,
    attributes: &[PgAttributeRow],
    constraints: &[PgConstraintRow],
    attrdefs: &[PgAttrdefRow],
    extra_type_rows: &[PgTypeRow],
    ...
    let not_null_constraints = constraints
        ...
        .collect::<BTreeMap<_, _>>();
    let attrdefs = attrdefs
        .iter()
        .map(|row| (row.adnum, row))
        .collect::<BTreeMap<_, _>>();
...
fn catalog_entry_by_oid_mvcc_for_drop(
    store: &CatalogStore,
    ctx: &CatalogWriteContext,
    relation_oid: u32,
    extra_type_rows: &[PgTypeRow],
    ...
    let desc = relation_desc_from_catalog_rows_mvcc(
        store,
        ctx,
        &attributes,
        &constraints,
    ...
    let array_type_oid = if class_row.reltype == 0 {
        ...
        type_row_by_oid_mvcc_with_extra_type_rows(store, ctx, class_row.reltype, extra_type_rows)?
            .map(|row| row.typarray)
            .unwrap_or(0)
    };
    let partitioned_table = if class_row.relkind == 'p' {
        partitioned_table_row_mvcc(store, ctx, relation_oid)?
    } else {
        None
    };
    let index_meta = if matches!(class_row.relkind, 'i' | 'I') {
        index_row_by_index_oid_mvcc(store, ctx, relation_oid)?
            .map(catalog_index_meta_from_index_row_for_drop)
    } else {
        None
    };
    Ok(CatalogEntry {
        ...
        relacl: class_row.relacl,
        reloptions: class_row.reloptions,
        of_type_oid: class_row.reloftype,
...
fn rows_for_existing_relation_mvcc(
    ...
    let class_owner = class_row.relowner;
...
> {
    ...
    let dropped = oids
        .iter()
        .copied()
        .map(|oid| catalog_entry_by_oid_mvcc_for_drop(store, ctx, oid, extra_type_rows))
        .collect::<Result<Vec<_>, _>>()?;
    let dropped_oids = dropped
        .iter()
        .map(|entry| entry.relation_oid)
...
fn drop_relation_oids_by_oid_mvcc(
    ...
    let class_row = class_row_by_oid_mvcc(store, ctx, relation_oid)?
        .ok_or_else(|| CatalogError::UnknownTable(relation_oid.to_string()))?;
    if !(relkind_is_droppable_table(class_row.relkind) || class_row.relkind == 'S') {
        return Err(CatalogError::UnknownTable(relation_oid.to_string()));
    }
    let mut seen = BTreeSet::new();
    let mut order = Vec::new();
