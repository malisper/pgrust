...
fn json_object_key_text(value: &Value, _op: &'static str) -> Result<String, ExecError> {
    match value {
        Value::Null => Ok("".into()),
        ...
        | Value::Json(_)
        | Value::Jsonb(_)
        | Value::EnumOid(_) => Err(ExecError::DetailedError {
        ...
        Value::Array(_) | Value::PgArray(_) | Value::Record(_) => Err(ExecError::DetailedError {
            message: "key value must be scalar, not array, composite, or json".into(),
            detail: None,
            hint: None,
            sqlstate: "22023",
