...
impl PgPrngState {
    ...
    pub(crate) fn fseed(&mut self, seed: f64) {
    ...
    pub(crate) fn double_normal(&mut self) -> f64 {
        let u1 = 1.0 - self.double();
        let u2 = 1.0 - self.double();
        (-2.0 * u1.ln()).sqrt() * (2.0 * std::f64::consts::PI * u2).sin()
    }

    pub(crate) fn int64_range(&mut self, min: i64, max: i64) -> i64 {
        self.ensure_seeded();
        if max <= min {
            return min;
        }
    ...
    pub(crate) fn numeric_range(&mut self, min: &NumericValue, max: &NumericValue) -> NumericValue {
        self.ensure_seeded();
