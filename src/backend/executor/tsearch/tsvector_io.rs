...
pub(crate) fn weight_code(weight: Option<TsWeight>) -> u8 {
    match weight {
        None => 0,
        Some(TsWeight::D) => 1,
        Some(TsWeight::C) => 2,
...
pub(crate) fn decode_weight(code: u8) -> Result<Option<TsWeight>, ExecError> {
    match code {
        0 => Ok(None),
...
impl<'a> Cursor<'a> {
    ...
    pub(crate) fn read_str(&mut self, len: usize) -> Result<String, ExecError> {
    ...
    pub(crate) fn ensure_finished(&self) -> Result<(), ExecError> {
        if self.offset == self.bytes.len() {
            Ok(())
        } else {
