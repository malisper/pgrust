...
use pgrust_executor::{
    WindowFrameError, WindowRangeOrder, WindowRangeRow, rows_frame_end, rows_frame_start,
};
...
fn range_frame_start_from_offset(
    ...
    let item = &order_by[0];
    pgrust_executor::range_frame_start_from_offset(
        &window_range_rows(partition_rows),
        WindowRangeOrder {
...
fn range_frame_end_from_offset(
    ...
    let item = &order_by[0];
    pgrust_executor::range_frame_end_from_offset(
        &window_range_rows(partition_rows),
        WindowRangeOrder {
            descending: item.descending,
            nulls_first: item.nulls_first,
...
fn window_range_rows(partition_rows: &[PreparedWindowRow]) -> Vec<WindowRangeRow> {
    partition_rows
        .iter()
        .map(|row| WindowRangeRow {
            order_keys: row.order_keys.clone(),
        })
        .collect()
}

fn window_frame_error(error: WindowFrameError) -> ExecError {
    match error {
        WindowFrameError::UnsupportedRangeOffset => unsupported_range_offset_error(),
        WindowFrameError::InvalidRangeOffset => invalid_range_offset_error(),
    }
}
...
fn row_is_included_by_frame_exclusion(
    ...
    pgrust_executor::row_is_included_by_frame_exclusion(
        clause.spec.frame.exclusion,
        row_index,
        candidate_index,
        |left, right| {
            same_peer(
                &clause.spec.order_by,
                &partition_rows[left],
                &partition_rows[right],
            )
        },
    )
}

fn same_peer_indexes(
    clause: &WindowClause,
    partition_rows: &[PreparedWindowRow],
    left: usize,
    right: usize,
...
fn first_included_frame_row_index(
    ...
    pgrust_executor::first_included_frame_row_index(
        clause.spec.frame.exclusion,
        row_index,
        frame_start,
        frame_end,
        |left, right| same_peer_indexes(clause, partition_rows, left, right),
    )
}
...
fn last_included_frame_row_index(
    ...
    pgrust_executor::last_included_frame_row_index(
        clause.spec.frame.exclusion,
        row_index,
        frame_start,
        frame_end,
...
fn nth_included_frame_row_index(
    ...
    pgrust_executor::nth_included_frame_row_index(
        clause.spec.frame.exclusion,
        row_index,
        frame_start,
        frame_end,
