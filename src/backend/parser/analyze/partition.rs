...
pub(crate) fn lower_partition_clause(
    ...
    if stmt.partition_spec.is_none() && stmt.partition_of.is_none() && stmt.partition_bound.is_none() {
...
pub(crate) fn partition_value_to_value(value: &SerializedPartitionValue) -> Value {
    match value {
        ...
        SerializedPartitionValue::Date(v) => Value::Date(crate::include::nodes::datetime::DateADT(*v)),
        SerializedPartitionValue::Time(v) => Value::Time(crate::include::nodes::datetime::TimeADT(*v)),
...
pub(crate) fn value_to_partition_value(value: &Value) -> Result<SerializedPartitionValue, ParseError> {
    Ok(match value {
        ...
        other => {
            return Err(ParseError::FeatureNotSupported(format!(
                "partition key type {:?}",
                other.sql_type_hint().unwrap_or(SqlType::new(crate::backend::parser::SqlTypeKind::Text))
            )));
        }
    })
}
...
fn lower_partition_bound(
    ...
    match (bound, parent_spec.strategy) {
        (
            RawPartitionBoundSpec::List { values, is_default },
            PartitionStrategy::List,
        ) => {
            let key_type = parent_spec_key_types(parent_spec, parent_desc)
                .into_iter()
                .next()
                .ok_or_else(|| ParseError::InvalidTableDefinition("missing list partition key".into()))?;
            let lowered = values
                .iter()
...
fn lower_range_datums(
    ...
    values
        ...
        .map(|(value, key_type)| match value {
            ...
            RawPartitionRangeDatum::Value(expr) => evaluate_partition_bound_expr(expr, *key_type, catalog)
...
fn evaluate_partition_bound_expr(
    ...
    cast_value(value, target).map_err(|_| ParseError::InvalidTableDefinition(
...
fn ensure_matching_partition_shape(
    ...
    for (child, parent_column) in child_columns.iter().zip(parent_columns.iter()) {
        if !child.name.eq_ignore_ascii_case(&parent_column.name) || child.sql_type != parent_column.sql_type {
            return Err(ParseError::DetailedError {
                message: format!(
