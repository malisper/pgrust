        ...
        other => {
            if matches!(
                ...
                let bound_args = args
                    ...
                    .collect::<Result<Vec<_>, _>>()?;
                let row_columns = resolved_row_columns.clone();
                let output_columns = row_columns.clone().ok_or_else(|| ParseError::UnexpectedToken {
                    expected: "function row description in FROM",
                    actual: format!("{other} without row description"),
                })?;
                let desc = RelationDesc {
                    columns: output_columns
                        .iter()
                ...
                let scope = scope_for_relation(Some(name), &desc);
                Ok((
                    AnalyzedFrom::function(SetReturningCall::JsonPopulateRecordSet {
                        func_oid: resolved_proc_oid,
                        func_variadic: resolved_func_variadic,
                        args: bound_args,
                        row_columns,
                        output_columns,
                        recordset: true,
                        return_record_value: false,
                    }),
                    scope,
                ))
            } else if other.eq_ignore_ascii_case("json_populate_record")
                && let Some(resolved) = resolved.as_ref()
                && resolved.prokind == 'f'
                && !matches!(resolved.row_shape, ResolvedFunctionRowShape::None)
            {
                let empty_scope = empty_scope();
                let bound_args = args
                    .iter()
