...
fn exec_error_position(sql: &str, e: &ExecError) -> Option<usize> {
    ...
    let value = match e {
        ...
        ExecError::Parse(crate::backend::parser::ParseError::DetailedError {
            ...
            if message == "cannot specify USING when altering type of generated column" {
...
fn create_table_error_position(sql: &str, message: &str) -> Option<usize> {
    match message {
        ...
        | "set-returning functions are not allowed in column generation expressions" => {
            find_generated_expr_identifier_position(sql, true)
        }
        "cannot use subquery in column generation expression" => {
            find_generated_expr_keyword_position(sql, "select").map(|position| {
                if position > 1
                    && sql
                        .as_bytes()
                        .get(position - 2)
        ...
        "cannot use whole-row variable in column generation expression" => {
            find_generated_expr_identifier_position(sql, false)
        }
    ...
    .or_else(|| {
        if message.starts_with("multiple generation clauses specified for column ") {
            find_second_option_occurrence(sql, "generated")
        } else if message.starts_with("multiple identity specifications for column ") {
            find_second_option_occurrence(sql, "generated")
        } else if message
            .starts_with("both default and generation expression specified for column ")
        {
            ...
            (default_position < generated_position).then_some(generated_position)
        } else if message
            .starts_with("both identity and generation expression specified for column ")
        {
            find_second_option_occurrence(sql, "generated")
        ...
        } else if message == "for a generated column, GENERATED ALWAYS must be specified" {
            find_case_insensitive_token_position(sql, "BY DEFAULT")
                .or_else(|| find_case_insensitive_token_position(sql, "GENERATED BY DEFAULT"))
                .or_else(|| find_case_insensitive_token_position(sql, "GENERATED"))
        } else if let Some(column_name) = generated_column_error_name(message) {
            find_generated_expr_token_position(sql, column_name)
        ...
        } else if message == "cannot use generated column in partition key" {
            find_partition_key_expr_position(sql)
        } else if message.starts_with("cannot use system column ")
            && message.contains("column generation expression")
        {
            find_generated_expr_identifier_position(sql, false)
        } else if message.starts_with("cannot use system column ") {
            find_partition_key_expr_position(sql)
        } else if let Some(column_name) = extract_missing_column_name(message) {
            find_generated_expr_token_position(sql, column_name)
...
fn generated_column_error_name(message: &str) -> Option<&str> {
    message
        .strip_prefix("cannot use generated column \"")?
        .split_once("\" in column generation expression")
        .map(|(name, _)| name)
}
...
fn find_generated_expr_start(sql: &str) -> Option<usize> {
    ...
    Some(sql[start - 1..].find('(')? + start)
}

fn find_generated_expr_identifier_position(sql: &str, want_function_call: bool) -> Option<usize> {
    let start = find_generated_expr_start(sql)?;
    find_default_expr_identifier_position(sql, start, want_function_call)
}

fn find_generated_expr_keyword_position(sql: &str, keyword: &str) -> Option<usize> {
    let start = find_generated_expr_start(sql)?;
    let relative = find_case_insensitive_token_position(&sql[start..], keyword)?;
    Some(start + relative)
}

fn find_generated_expr_token_position(sql: &str, token: &str) -> Option<usize> {
    let start = find_generated_expr_start(sql)?;
    let relative = find_case_insensitive_token_position(&sql[start..], token)?;
    Some(start + relative)
}
...
fn find_partition_key_expr_position(sql: &str) -> Option<usize> {
    ...
    let position = find_default_expr_identifier_position(sql, open, false)
        .or_else(|| find_default_expr_identifier_position(sql, open, true))?;
    if let Some(relation_name) = relation_name_after_keyword(sql, "CREATE TABLE")
        && sql[position - 1..]
            .get(..relation_name.len())
            .is_some_and(|token| token.eq_ignore_ascii_case(&relation_name))
        && sql
            .as_bytes()
            .get(position - 2)
            .is_some_and(|byte| *byte == b'(')
...
fn apply_alter_table_regression_error_compat(sql: &str, response: &mut ExecErrorResponse) {
    let compact = sql
        ...
        .join(" ")
        .to_ascii_lowercase();
    let alter_add_generated_column = compact.starts_with("alter table ")
        && compact.contains(" add column ")
        && compact.contains(" generated always as ");
    ...
    if alter_add_generated_column
        && response
            .message
            .starts_with("cannot use generated column \"")
    {
        response.position = None;
        return;
    }
    ...
    if alter_add_generated_column
        && response.message.starts_with("column \"")
        && response.message.contains("\" of relation \"")
        && response.message.ends_with("\" does not exist")
        && let Some((column_name, _)) = response
            .message
            .strip_prefix("column \"")
            .and_then(|rest| rest.strip_suffix("\" does not exist"))
            .and_then(|rest| rest.split_once("\" of relation \""))
    {
    ...
    if compact.starts_with("alter table ")
        && compact.contains(" set expression ")
        && let Some((relation, constraint)) = check_violation_message_parts(&response.message)
    {
        response.message = format!(
...
fn suppress_mapped_join_regression_detail(sql: &str, response: &ExecErrorResponse) -> bool {
    ...
    (response.message == "table reference \"x\" is ambiguous"
        && lower.contains("cross join lateral (select x.f1)"))
        || (lower.contains(" set expression ")
            && response.message.starts_with("check constraint \"")
            && response.message.ends_with(" is violated by some row"))
}
            ...
            if include_attrdef {
                row.push(
                    column
                        ...
                        .map(|expr| {
                            let mut formatted =
                                format_psql_default(db, session, column.sql_type, expr);
                            if column.generated.is_some() {
                                formatted =
                                    format_psql_generated_default(column.sql_type, &formatted);
                            }
                            Value::Text(formatted.into())
                        })
                        .unwrap_or(Value::Null),
                );
            }
...
fn format_psql_generated_default(sql_type: SqlType, expr_sql: &str) -> String {
    let mut formatted = strip_integer_literal_typecasts(expr_sql);
    if matches!(sql_type.kind, SqlTypeKind::Numeric)
...
fn strip_integer_literal_typecasts(expr_sql: &str) -> String {
    let mut out = String::with_capacity(expr_sql.len());
    let mut index = 0usize;
    let mut in_string = false;
    while index < expr_sql.len() {
        let rest = &expr_sql[index..];
        ...
        if ch == '\'' {
            out.push(ch);
            index += ch.len_utf8();
            if in_string && expr_sql[index..].starts_with('\'') {
                out.push('\'');
                index += 1;
                continue;
            }
...
fn integer_literal_precedes_cast(rendered_prefix: &str) -> bool {
    let mut chars = rendered_prefix.chars().rev().peekable();
