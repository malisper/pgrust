...
pub(crate) fn default_text_search_parser_token_types() -> Vec<TextSearchParserToken> {
...
pub(crate) fn default_text_search_parser_token_type(tokid: i32) -> Option<TextSearchParserToken> {
    default_text_search_parser_token_types()
        .into_iter()
        .find(|token| token.tokid == tokid)
}

pub(crate) fn parse_default_text_search_tokens(text: &str) -> Vec<(i32, String)> {
