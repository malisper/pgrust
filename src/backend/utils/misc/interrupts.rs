...
pub use pgrust_core::InterruptReason;
...
mod tests {
    ...
    #[test]
    fn statement_guard_arms_and_restores_deadline() {
        let state = Arc::new(InterruptState::new());
        assert!(check_for_interrupts(state.as_ref()).is_ok());

        {
            let _guard = state.statement_interrupt_guard(Some(Duration::from_millis(1)));
            std::thread::sleep(Duration::from_millis(5));
            assert_eq!(
                check_for_interrupts(state.as_ref()),
                Err(InterruptReason::StatementTimeout)
            );
        }
    ...
    #[test]
    fn pending_interrupt_takes_precedence_over_deadline() {
        let state = Arc::new(InterruptState::new());
        let _guard = state.statement_interrupt_guard(Some(Duration::from_secs(1)));
        state.set_pending(InterruptReason::StatementTimeout);
        assert_eq!(
            check_for_interrupts(state.as_ref()),
            Err(InterruptReason::StatementTimeout)
