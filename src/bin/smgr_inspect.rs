...
fn main() {
    ...
    smgr.extend(rel, ForkNumber::Main, 0, &[0xAAu8; BLCKSZ], true).unwrap();
    // Block 1: all bytes = 0xBB
    smgr.extend(rel, ForkNumber::Main, 1, &[0xBBu8; BLCKSZ], true).unwrap();
    // Block 2: all bytes = 0xCC
    smgr.extend(rel, ForkNumber::Main, 2, &[0xCCu8; BLCKSZ], true).unwrap();

    // Also write the FSM fork with a distinct pattern.
    smgr.create(rel, ForkNumber::Fsm, false).unwrap();
    smgr.extend(rel, ForkNumber::Fsm, 0, &[0xFFu8; BLCKSZ], true).unwrap();
    ...
    let mut entries: Vec<_> = fs::read_dir(&db_dir).unwrap()
        .map(|e| e.unwrap().path())
        .collect();
    entries.sort();
    ...
    for path in &entries {
        ...
        println!("  size: {} bytes = {} block(s) of {} bytes", size_bytes, size_blocks, BLCKSZ);
        ...
        for blk in 0..size_blocks {
            ...
            println!("  block {}: first 32 bytes = {} ... (all_same={})",
                blk, hex.join(" "), all_same);
        }
        println!();
    }
    ...
    for &(blk, expected_byte) in expected_patterns {
        ...
        smgr.read_block(rel, ForkNumber::Main, blk, &mut buf).unwrap();
    ...
    println!("  main fork: file size {} bytes / {} = {} blocks, nblocks()={}  match={}",
        main_meta.len(), BLCKSZ, main_meta.len() / BLCKSZ as u64,
        main_nblocks,
        main_meta.len() / BLCKSZ as u64 == main_nblocks as u64);

    println!("  fsm  fork: file size {} bytes / {} = {} blocks, nblocks()={}  match={}",
        fsm_meta.len(), BLCKSZ, fsm_meta.len() / BLCKSZ as u64,
        fsm_nblocks,
    ...
    println!();
    println!("All checks passed. Files left in {:?} for manual inspection.", db_dir);
}
