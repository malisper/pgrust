...
pub const PG_CREATE_SUBSCRIPTION_OID: u32 = 6304;
pub const PG_MAINTAIN_OID: u32 = 6337;
...
pub fn bootstrap_pg_authid_rows() -> Vec<PgAuthIdRow> {
    vec![
        ...
        predefined_role(PG_CREATE_SUBSCRIPTION_OID, "pg_create_subscription"),
        predefined_role(PG_MAINTAIN_OID, "pg_maintain"),
    ]
}
...
mod tests {
    ...
    fn bootstrap_pg_authid_rows_include_predefined_roles() {
        ...
        assert!(
            rows.iter()
                .any(|row| row.rolname == "pg_create_subscription")
        );
        assert!(rows.iter().any(|row| row.rolname == "pg_maintain"));
    }
}
