...
impl Database {
    pub(crate) fn execute_alter_table_attach_partition_stmt_with_search_path(
        ...
        let result = self.execute_alter_table_attach_partition_stmt_in_transaction_with_search_path(
            client_id,
            stmt,
            xid,
            0,
    ...
    pub(crate) fn execute_alter_table_attach_partition_stmt_in_transaction_with_search_path(
        ...
        let parent = catalog
            ...
            .ok_or_else(|| ExecError::Parse(ParseError::TableDoesNotExist(stmt.parent_table.clone())))?;
        if parent.relkind != 'p' {
        ...
        let child = catalog.lookup_any_relation(&stmt.partition_table).ok_or_else(|| {
        ...
        let bound =
            lower_partition_bound_for_relation(&parent, &stmt.bound, &catalog).map_err(ExecError::Parse)?;
        ...
        let inherit_effect = self
            .catalog
            .write()
            .create_relation_inheritance_mvcc(child.relation_oid, &[parent.relation_oid], &inherit_ctx)
...
mod tests {
    ...
    fn partition_introspection_functions_follow_declarative_tree() {
        ...
        assert_eq!(
            query_rows(&mut session, &db, "select * from pg_partition_ancestors(null)"),
            Vec::<Vec<Value>>::new()
        );
        assert_eq!(
        ...
        assert_eq!(
            query_rows(
                &mut session,
                &db,
                "select pg_partition_root('plain_table')",
            ),
            vec![vec![Value::Null]]
        );
    }
    ...
    fn partitioned_root_dml_routes_rows_and_only_root_is_empty() {
        ...
        match session.execute(&db, "update routed set a = 2 where a = 1") {
            Err(ExecError::Parse(ParseError::FeatureNotSupported(message))) => {
                assert_eq!(message, "updating partition key columns on partitioned tables");
            }
