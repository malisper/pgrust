...
impl Database {
    ...
    pub(crate) fn execute_create_sequence_stmt_in_transaction_with_search_path(
        ...
        options.owned_by = resolve_owned_by_clause(&catalog, create_stmt.options.owned_by.as_ref())?;
                ...
                match result {
                    ...
                    Ok((entry, effect)) => {
                        ...
                        sequence_effects.push(
                            self.sequences.apply_upsert(entry.relation_oid, data, true),
                        );
                        Ok(StatementResult::AffectedRows(0))
                    }
                }
    ...
    pub(crate) fn execute_alter_sequence_stmt_with_search_path(
        ...
        let result =
            self.finish_txn(client_id, xid, result, &[], &[], &sequence_effects);
        guard.disarm();
    ...
    pub(crate) fn execute_alter_sequence_stmt_in_transaction_with_search_path(
        ...
        let current = self
            ...
            .ok_or_else(|| ExecError::Parse(ParseError::TableDoesNotExist(alter_stmt.sequence_name.clone())))?;
        let (mut options, restart) =
            apply_sequence_option_patch(&current.options, &alter_stmt.options).map_err(ExecError::Parse)?;
        if matches!(alter_stmt.options.owned_by, Some(SequenceOwnedByClause::Column { .. }))
            || matches!(alter_stmt.options.owned_by, Some(SequenceOwnedByClause::None))
        {
            options.owned_by =
        ...
        for sequence_name in &drop_stmt.sequence_names {
            ...
            if !refs.is_empty() && !drop_stmt.cascade {
                result = Err(ExecError::DetailedError {
                    message: format!("cannot drop sequence {} because other objects depend on it", sequence_name).into(),
                    detail: None,
                    hint: Some("Use DROP SEQUENCE ... CASCADE to drop the dependent objects too.".into()),
                    sqlstate: "2BP01",
                });
                break;
            }
