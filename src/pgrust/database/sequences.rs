...
impl SequenceRuntime {
    pub(crate) fn load(base_dir: Option<&Path>, catalog: &CatalogStore) -> Result<Self, CatalogError> {
        ...
        for effect in effects {
            match effect {
                ...
                SequenceMutationEffect::Drop {
                    ...
                    self.currvals.write().retain(|(_, oid), _| *oid != *relation_oid);
                    if *persistent {
    ...
    pub(crate) fn next_value(
        ...
        self.currvals.write().insert((client_id, relation_oid), next);
        Ok(next)
    }
    ...
    pub(crate) fn curr_value(
        ...
        self.currvals
            ...
            .ok_or_else(|| ExecError::DetailedError {
                message: format!("currval of sequence {relation_oid} is not yet defined in this session"),
                detail: None,
                hint: None,
                sqlstate: "55000",
            })
    ...
    pub(crate) fn set_value(
        ...
        {
            let mut data = self.data.write();
            let entry = data.get_mut(&relation_oid).ok_or_else(|| missing_sequence_error(relation_oid))?;
        ...
        if is_called {
            self.currvals.write().insert((client_id, relation_oid), value);
        } else {
...
pub(crate) fn default_sequence_name_base(table_name: &str, column_name: &str) -> String {
    ...
    format!("{}_{}_seq", table.to_ascii_lowercase(), column_name.to_ascii_lowercase())
}
...
pub(crate) fn resolve_sequence_options_spec(
    ...
    let minvalue = spec.minvalue.unwrap_or(Some(defaults.minvalue)).unwrap_or_else(|| {
        default_minvalue(type_oid, increment)
    });
    let maxvalue = spec.maxvalue.unwrap_or(Some(defaults.maxvalue)).unwrap_or_else(|| {
        default_maxvalue(type_oid, increment)
    });
    let start = spec.start.unwrap_or_else(|| {
        if increment > 0 { minvalue } else { maxvalue }
    });
...
fn load_sequence_file(base_dir: Option<&Path>, relation_oid: u32) -> Result<SequenceData, CatalogError> {
