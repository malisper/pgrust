...
export class WasmEngine {
    ...
    execute(sql) {
        ...
        try {
            ...
            const ret = wasm.wasmengine_execute(this.__wbg_ptr, ptr0, len0);
            var ptr2 = ret[0];
            var len2 = ret[1];
            if (ret[3]) {
                ptr2 = 0; len2 = 0;
    ...
    /**
     * @param {number | null} [pool_size]
     */
    constructor(pool_size) {
        const ret = wasm.wasmengine_new(isLikeNone(pool_size) ? 0x100000001 : (pool_size) >>> 0);
        if (ret[2]) {
     ...
     */
    reset(pool_size) {
        const ret = wasm.wasmengine_reset(this.__wbg_ptr, isLikeNone(pool_size) ? 0x100000001 : (pool_size) >>> 0);
        if (ret[1]) {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('pgrust_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
